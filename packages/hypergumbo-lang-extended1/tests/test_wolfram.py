...
class TestWolframCyclomaticComplexity:
    """INV-loguk slice C: callable wolfram symbols carry non-null CC + LOC.
    Real-grammar verification (degenerate grammar: CC from &&/|| only (base 1 otherwise))."""
    ...
    def test_branchy_callables_have_expected_cc(self, tmp_path) -> None:
        ...
        (tmp_path / 'c.wl').write_text('classify[x_] := Module[{r},\n  If[x > 0 && x < 10, r = 1, If[x >= 10 || x == -1, r = 2, r = 3]];\n  r\n]\nsimple[y_] := y + 1\ngv = 42\n')
        result = analyze_wolfram(tmp_path)
        assert not result.skipped
