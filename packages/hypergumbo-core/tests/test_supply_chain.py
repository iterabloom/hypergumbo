...
class TestIsGeneratedFileAxis:
    ...
    def test_generated_header_js_at_generated(self, tmp_path: Path) -> None:
        """WI-pofin: ``// @generated`` header marks a JS/TS file generated."""
    ...
    def test_generated_header_go_do_not_edit(self, tmp_path: Path) -> None:
        """WI-pofin: Go stdlib 'Code generated ... DO NOT EDIT' is flagged."""
        target = tmp_path / "pkg" / "api" / "types.go"
        target.parent.mkdir(parents=True)
        target.write_text(
    ...
    def test_generated_header_python_at_generated(
        self, tmp_path: Path,
    ) -> None:
        """WI-pofin: ``# @generated`` in a Python header is flagged."""
        target = tmp_path / "models" / "user.py"
        target.parent.mkdir(parents=True)
        target.write_text(
    ...
    def test_generated_header_c_block_comment(
        self, tmp_path: Path,
    ) -> None:
        """WI-pofin: ``/* @generated */`` block-comment marker works."""
        target = tmp_path / "src" / "api.ts"
        target.parent.mkdir(parents=True)
        target.write_text(
            "/* @generated by some-tool, do not modify */\n"
            "export {};\n",
        )
        result = classify_file(target, tmp_path)
        assert result.is_generated is True

    def test_generated_header_autogenerated_keyword(
        self, tmp_path: Path,
    ) -> None:
        """WI-pofin: alternate Autogenerated marker works."""
        target = tmp_path / "lib" / "client.rb"
        target.parent.mkdir(parents=True)
        target.write_text(
            "# Autogenerated by rbs-gen\n"
            "class Client\nend\n",
        )
        result = classify_file(target, tmp_path)
        assert result.is_generated is True

    def test_no_generated_header_hand_written(
        self, tmp_path: Path,
    ) -> None:
        """WI-pofin: hand-written files with no header marker stay un-flagged."""
        target = tmp_path / "src" / "lib.ts"
        target.parent.mkdir(parents=True)
        target.write_text(
    ...
    def test_generated_keyword_in_prose_not_flagged(
        self, tmp_path: Path,
    ) -> None:
        """WI-pofin: a mid-file mention of the word doesn't trigger the pattern
        because the regex requires a comment-line start anchor."""
        target = tmp_path / "README.md"
        target.write_text(
            "# Project\n\n"
            "This project was generated in 2024.\n",
        )
        result = classify_file(target, tmp_path)
        assert result.is_generated is False

    def test_binary_extension_skipped(self, tmp_path: Path) -> None:
        """WI-pofin: content scan skips extensions outside _CONTENT_SCAN_EXTS.

        Even if a binary file contained the marker bytes, the content
        scan should not open it — the fallback returns False.
        """
        target = tmp_path / "assets" / "logo.png"
        target.parent.mkdir(parents=True)
        # Write bytes that would match the regex if scanned as text,
        # to prove the extension gate is what keeps this safe.
        target.write_bytes(b"// @generated by image tool\n")
        result = classify_file(target, tmp_path)
        # No path pattern matches and the content scan is skipped for .png.
        assert result.is_generated is False
