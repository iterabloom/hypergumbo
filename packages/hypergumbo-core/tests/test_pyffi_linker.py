...
class TestPyO3PythonStyleNameMatching:
    """Tests for PyO3 matching via Python-style name conversion (Py prefix stripping)."""

    def test_py_prefix_stripped_for_matching(self, tmp_path: Path) -> None:
        """PyTokenizer::encode matches Python's Tokenizer.encode via Py prefix stripping."""
        from hypergumbo_core.linkers.pyffi import link_pyffi

        rs_file = tmp_path / "src" / "lib.rs"
        rs_file.parent.mkdir(parents=True)
        rs_file.write_text('#[pyo3]\nfn encode() {}\n')

        py_file = tmp_path / "app.py"
        py_file.write_text("t.encode()\n")

        py_func = _make_python_symbol("app", kind="module", path=str(py_file), start_line=1, end_line=1)
        rust_func = _make_rust_symbol(
            "PyTokenizer::encode",
            path=str(rs_file),
            annotations=[{"name": "pyo3", "args": [], "kwargs": {}}],
        )

        call_edge = Edge.create(
            src=py_func.id,
            dst=f"python:{py_file}:0-0:Tokenizer.encode:unresolved",
            edge_type="calls",
            line=1,
            evidence_type="function_call",
            confidence=0.50,
