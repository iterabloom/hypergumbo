...
- --max-tier 2: First-party + internal deps (configured internal_package_roots)
...
""")
        ...
        # Tier 1 (role file): examples are in-repo → first_party with is_example
        # (INV-naduh / ADR-0041 §1); tier 2 is now config-only (internal_package_roots).
...
""")
        ...
        # Tier 4 (excluded-derived directory): dist/ is excluded from
        # discovery entirely, so it yields no nodes regardless of max-tier.
        dist = tmp_path / "dist"
        dist.mkdir()
        (dist / "bundle.js").write_text("var a=1,b=2;")

        # Tier 4 (in-tree, content heuristic): a generated file in an analyzed
        # dir IS discovered and classified tier 4 via its "DO NOT EDIT" header,
        # so it yields a real tier-4 node (the fixture must span >1 tier, and
        # post-INV-naduh examples are tier 1 while tier 2 is config-only). It
        # has no route role, so route-promotion does not apply.
    ...
    def test_max_tier_4_includes_all(self, mixed_tier_repo: Path, tmp_path: Path):
        ...
        # Should have nodes from multiple tiers including tier 4 (derived).
        tiers = {n.get("supply_chain", {}).get("tier", 1) for n in nodes}
        assert 4 in tiers, f"Expected a tier-4 (derived) node; got tiers {tiers}"
        assert 1 in tiers, f"Expected a tier-1 (first-party) node; got tiers {tiers}"
    ...
    def test_max_tier_2_excludes_external_and_derived(
        self, mixed_tier_repo: Path, tmp_path: Path
    ):
        """--max-tier 2 keeps tiers 1-2 and excludes derived (tier 4).

        Post-INV-naduh the fixture has no tier-2 content (tier 2 is now
        config-only via internal_package_roots) and no discovered tier-3
        content (node_modules/ is excluded from discovery), so at this
        threshold the observable behavior is that the derived tier-4 node
        (dist/) is excluded while first-party code (incl. examples) is kept.
        """
        out_path = tmp_path / "results.json"
        ...
        # The in-tree derived (tier 4) file must be excluded at max-tier 2.
        paths = {n["path"] for n in nodes}
...
class TestRoutePromotionFromDerived:
    """Test that route symbols in derived (tier 4) files are promoted to tier 1.
    ...
    Per INV-naduh / ADR-0041 §1 in-repo routes are distance 0 → tier 1
    (first_party), not tier 2.
    """

    def test_route_symbol_promoted_from_tier_4(self, tmp_path: Path) -> None:
        """A route symbol in a generated file (tier 4) gets promoted to tier 1.
        ...
        # The route symbol should be present (promoted from tier 4 to tier 1).
        # INV-naduh / ADR-0041 §1: in-repo routes are distance 0 → tier 1
        # (first_party), not tier 2; the "route promoted from derived" reason
        # records the generated origin.
        route_nodes = [
            n for n in nodes
...
class TestMaxTierLimitsReporting:
    ...
    def test_limits_records_tier_filtered_files(self, tmp_path: Path):
        """WI-tulit: files dropped by the tier filter are recorded in
        limits.tier_filtered_files. A DERIVED (tier-4) generated file, excluded
        by the default effective_tier=3 filter, must appear there so a consumer
        can see WHAT was dropped (not just max_tier_applied's WHY)."""
        src = tmp_path / "src"
        src.mkdir()
        (src / "app.py").write_text("def main(): pass\n")
        # "DO NOT EDIT" header → classified DERIVED (tier 4) → dropped by default.
        (src / "generated.py").write_text(
            "# Code generated by tool; DO NOT EDIT.\ndef gen(): pass\n"
        )

        out_path = tmp_path / "results.json"
        run_behavior_map(tmp_path, out_path, include_sketch_precomputed=False)

        data = json.loads(out_path.read_text())
        dropped = data.get("limits", {}).get("tier_filtered_files", [])
        assert any("generated.py" in p for p in dropped), (
            f"tier-dropped generated.py should be recorded in "
            f"tier_filtered_files; got {dropped}"
        )
