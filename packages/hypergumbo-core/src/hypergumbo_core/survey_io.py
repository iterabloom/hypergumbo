...
"""Consumer-side helper for reading hypergumbo survey maps (WI-mokim).

Renamed from ``behavior_map_io`` per ADR-0042 (the "survey" concept rename);
``behavior_map_io`` survives as a deprecation shim that re-exports these names
(warning on import) for one minor version, then is removed at window-close.
