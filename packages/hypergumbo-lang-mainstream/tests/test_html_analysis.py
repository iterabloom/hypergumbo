...
def test_detects_script_src_tag(tmp_path: Path) -> None:
    ...
    script_edges = [e for e in data["edges"] if e["type"] == "references" and (e.get("meta") or {}).get("construct") == "script_src"]
    ...
    # the construct in meta — NOT the old endpoint-shape edge_type "script_src".
    assert script_edges[0]["type"] == "references"
    assert script_edges[0]["meta"]["construct"] == "script_src"
