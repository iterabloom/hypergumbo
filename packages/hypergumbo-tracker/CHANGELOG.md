...
- **NINE `tracker-sync/*` PRs SAT UNMERGEABLE, THE OLDEST SINCE AUGUST 24, AND EVERY ONE OF THEM WAS EMPTY (WI-torug).** `.gitattributes` declares `merge=union` on `.ops` — the whole point of an append-only journal. That is a CLIENT-SIDE driver; the forge's server-side merge does not read `.gitattributes` and does a plain three-way merge instead. Measured on `.INV-juvul-*.ops`: merge-base **32** lines, branch tip **54** (+22 appended), current dev **61** (+29 *different* lines appended). Both sides appended at the end of one file from a common base — a trivial union anywhere else, a textual conflict there.
- **Nothing had regressed; prompt merges were hiding it.** While each sync landed before any sibling touched its files, dev never diverged at the append point and the server merge succeeded. The moment one PR sits unmerged and another sync appends to the same log, that PR is permanently unmergeable — and the Sept 1-2 axis campaign (ADR-0050 / ADR-0051) plus INV-foluz drove the throughput that closed the window.
- **The action is CLOSE, not merge, and the measurement is what says so.** Two independent methods agree: a per-file set difference finds **0 lines** in any of the nine that dev lacks, and `git merge-tree --write-tree` produces a tree **identical to dev's own** for all nine. They contribute zero bytes. This is the durability design working — `do_sync` gates its destructive cleanup on merge success (WI-lufal), so a failed sync leaves its ops in the working tree and the next sync rebuilds from a fresh base and carries them forward. The data always arrived; the branches were litter.
- **`find_superseded_sync_branches` keys on "merging changes nothing", never on "the forge says conflict".** Those are different questions and conflating them is how a tidying step becomes data loss: a branch holding unique ops conflicts for exactly the same `merge=union` reason as one holding none. Only the tree comparison distinguishes them, and only it licenses closing anything. Pinned by a real conflicting fixture (in a non-`.ops` path, since `.ops` cannot conflict on a client) asserting a conflict is *not* supersession, and by a unique-op fixture with a second test proving the op really is absent from dev so the first cannot pass vacuously.
- **It fails safe in every undecidable case.** `git merge-tree --write-tree` needs git ≥ 2.38; an old client, a missing base ref, or a non-repo all yield an empty list rather than a guess. A blank base tree is guarded explicitly — the comparison is a string equality, so an empty value would match every branch and turn the predicate into a mass PR-closer. That is the one path from tidying tool to accident, and it has its own test.
- **`tracker sync --prune` runs it explicitly; a successful auto-sync runs it automatically.** `--prune` deliberately executes BEFORE preflight: a superseded branch is by definition one whose ops already landed, so gating on `changed_files` would make the command a no-op exactly when it is wanted. Combine with `--dry-run` to report without closing.
- **The automatic call lives in `_maybe_auto_sync`, not `do_sync`, and the first attempt proved why.** Wiring it to do_sync's post-merge path broke **23** existing tests with `StopIteration` — they script an exact sequence of `_git` calls and the extra fetch ran off the end. That is a fair signal: do_sync implements a transaction and is pinned call-for-call, and litter collection is not part of it. The safety property never depended on do_sync's sync gate either, which was the stated reason for the original placement: a concurrent sync pushes ops dev does not have, so its branch cannot satisfy "merging changes nothing" and can never be a candidate. **The predicate carries the guarantee.**
- **A failing prune cannot damage a sync that already merged.** The call is wrapped, and the test asserts all three halves of that: the handler reports, `PR #N` is still reported, and `sync failed` is NOT — because `_maybe_auto_sync` disables autonomous mode after enough consecutive sync failures, so a forge hiccup during litter collection must never be counted as one.
