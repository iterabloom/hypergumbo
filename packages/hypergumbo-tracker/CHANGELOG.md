...
- **Tag-name validation now applies uniformly across every tag-write path (INV-pahoj):** the CLI `tracker add --tag` / `update --add-tag` paths validated against `_TAG_NAME_RE = ^[a-z_][a-z0-9_]*$` for free (via `tag_catalog.save_catalog`), but the TUI's tag-edit modal and `serve.py` routed straight through `trackerset.update()` without ever invoking `touch_tags` or `save_catalog`. A user editing tags in the TUI could persist `analysis-quality` to an item's `.ops` log even though the catalog would refuse the same name — item-side queries (`tracker list --tag analysis-quality`) found it, catalog-side surfaces couldn't. Fix: centralize the regex check in `trackerset.add()` and `trackerset.update()` so every non-CLI caller (TUI, `serve.py`, future APIs) gets uniform enforcement; the TUI edit-item handler now catches the resulting `ValueError` and surfaces it as a user-facing notification. `remove_fields["tags"]` is intentionally exempt so historically-bad tags remain cleanable (unblocks WI-fanus). +11 tests.

- **`tracker delete` now sweeps dangling `isbefore` references (INV-vudit):** a deleted item's `isbefore` list and any inbound holders kept the now-dangling ID. `TrackerSet.ready()` filtered these at query time, but `tracker show` / `tracker deps` and any future `isbefore` walker treated the deleted ID as a real dependency. `_cmd_delete` now clears the deleted item's own `isbefore` and emits `remove_fields={"isbefore": [deleted_id]}` ops on every inbound holder across every tier, bundled with the status flip. CLI prints `Cleared N inbound isbefore references.` when non-zero; JSON output gains `inbound_isbefore_cleared`. +3 tests.

- **`tracker tags rename` no longer rejects renames FROM legacy-format old tags (WI-hohov):** the per-item update succeeded (every item got the new tag, lost the legacy one), but the trailing `setdefault(old, TagCatalogEntry(...))` left a regex-violating key in the catalog dict; `save_catalog`'s validator raised and the verb exited non-zero. Hit during the 2026-05-07 BVT-discipline migration (`for-deep-bakeoff` → `for_deep_bakeoff`, `for-uat-bakeoff` → `for_uat_bakeoff` across 21 items). Fix: when `old` violates `_TAG_NAME_RE`, drop any pre-existing catalog entry under that name and skip the audit-trail bump on the legacy half. Catalog converges to the snake_case entry only; valid → valid renames still record both endpoints unchanged. +2 tests.
