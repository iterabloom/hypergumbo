...
- **`sync.py` resolves the forge backend and threads it through preflight → `_poll_ci` (Codeberg→GitHub migration, dormant).** `_detect_api_base` gains a `github.com` arm (→ `https://api.github.com/repos/...`, no `/api/v1`) via a shared `_api_base_for` helper; `PreflightResult` carries a `backend` field derived from the resolved `api_base` (forced to `forgejo` under CI failover, which always targets the self-hosted Forgejo); `_api_call` adds GitHub's `Accept: application/vnd.github+json` + pinned `X-GitHub-Api-Version: 2022-11-28` for github URLs (the existing `token` auth scheme works on both forges); and `do_sync` now passes `backend` into `_poll_ci`, wiring the single-status stale-pending fix so it activates automatically when `origin` is re-pointed to GitHub. Backend follows the origin host, so everything stays dormant while Codeberg is origin.
- **`do_sync`'s write path (push → create-PR → merge → branch-delete) is GitHub-aware (Codeberg→GitHub migration, dormant).** On the `github` backend `do_sync` pushes a normal branch (`refs/heads/…:refs/heads/…`, no AGit `-o` push options), opens the PR explicitly via a new `_create_pr` (`POST /pulls`, 201 → number+head-SHA, 422-already-exists falling back to `_find_open_pr` for idempotent retries), merges via `_merge_pr`'s new `github` mode (`PUT /pulls/{n}/merge` with `{merge_method: rebase}` then `merge`, success = 200 `{merged: true}`), and deletes the server-side head branch afterward via a new `_delete_remote_branch` (`DELETE /git/refs/heads/{branch}`, parity with AGit which never created one). PR URLs render as `https://github.com/{slug}/pull/{n}`. The post-sync local cleanup is UNCHANGED and backend-agnostic — it keeps the path-scoped `checkout HEAD -- <ops>` + `merge --ff-only` dance (the migration design's "reset --hard for github" idea was dropped: local `dev` never leaves `base`, so `origin/dev` — even after GitHub's rebase-merge rewrites the sync-commit SHA — is always a fast-forward, exactly as it already is on Forgejo, which also rebase-merges these branchless PRs; a whole-tree `reset --hard` would instead silently discard unstaged edits to tracked non-ops files that the ops-only INV-dalup snapshot does not restore). The Forgejo/AGit path is byte-identical (backend defaults `forgejo`), so everything is dormant until `origin` is re-pointed to GitHub.

### Fixed

- **`sync.py` preflight resolves the GitHub credential for the github backend (Codeberg→GitHub migration, dormant).** The dormant github write path authenticated every push and API call with `preflight.forgejo_token`, which was populated only from `FORGEJO_TOKEN` — so after cutover it would have presented the Codeberg token to `github.com`/`api.github.com` and 401'd on the very first sync (the held-ops flush). `preflight_check` now also resolves `HG_GITHUB_TOKEN` (the local maintainer PAT documented in PR-D) and, once the backend is known, selects the effective forge credential by backend: github → `HG_GITHUB_TOKEN` + the conventional `x-access-token` PAT-auth username, forgejo → `FORGEJO_TOKEN`/`FORGEJO_USER` as before. A github origin with no `HG_GITHUB_TOKEN` now fails preflight with a clear message instead of a cryptic 401 mid-push. The `forgejo_*` result fields keep their names (they now carry the resolved forge credential for either backend). Dormant while Codeberg is origin; closes the Python-sync gap that PR-B2 left when it wired the github endpoints/verbs but not the credential (the bash tooling already resolved `HG_GITHUB_TOKEN`).
