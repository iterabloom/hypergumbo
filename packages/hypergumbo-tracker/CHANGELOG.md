...
- **Per-message edit-mode for tracker discussions.** Three new ops (`delete-msg`, `undelete-msg`, `edit-msg-text`) tombstone, restore, or rewrite individual discussion entries by their existing 4-char nonce. Authorization is window-based: a human-only `edit-mode-on` op opens a half-open `[start, start+ttl)` window (default 30m, max 60m) capped at 500 mutation ops (`undelete-msg` is uncapped); `edit-mode-off` closes it early and a new `edit-mode-on` preempts an open window; ops outside any valid window are filtered at compile, not removed from disk. A defense-in-depth OS-perm gate backs the window: the edit-mode log lives in a dedicated subdirectory (created mode 0755 by the writer) whose owner uid is mapped to a username and matched against `agent_usernames` at read time — agent-owned files have every op filtered (preserved on disk for forensics, never auto-removed); in single-user dev setups the gate is moot because `edit_mode_on` already raises `HumanAuthorityError` before any write. CLI: `tracker edit-mode {on|off|status}` (human only for on/off), `tracker delete-msg`/`undelete-msg ITEM NONCE --reason TEXT`, `tracker edit-msg-text ITEM NONCE --new-text TEXT --reason TEXT`. `tracker show` suppresses tombstones by default with a `(not shown: N deleted messages)` footer; `--include-deleted` renders them with a `[DELETED]` marker; `--include-history` shows prior text revisions for edited messages. The TUI gains a `Ctrl-E` toggle binding and a top-of-screen banner (`EDIT MODE ON — Nm SSs remaining — ops_used/cap_max`) that auto-updates on the reload tick and hides when off. An end-to-end self-test materializes a fold-audit-shaped extraction: a parent invariant's long bundled-text discussion entry → human opens window → agent creates three new items mirroring the proposals → agent tombstones the parent message with an `extracted to <new-id>` reason → window closes → subsequent delete attempts on the same nonce are rejected.
