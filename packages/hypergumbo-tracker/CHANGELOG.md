...
- **Fold-induced nonce injection in YAML serializer** (WI-pusif-bukor): `_serialize_op` now sets `ry.width = sys.maxsize` so ruamel.yaml never folds long double-quoted scalars across physical lines. The previous `width=4096` let any description (or other long string field) over ~4080 UTF-8 bytes trigger fold-then-nonce-injection: the post-processor would append `  # <nonce>` to every physical line including continuation lines, and CSafeLoader treats `#` as literal inside double-quoted scalars on read-back, embedding the nonce comment INSIDE the value the user wrote. The nonce-on-every-line invariant remains intact — each scalar now occupies one physical line, so the post-processor cannot land a comment inside any string. New `TestLongStringRoundtrip` covers 5000-byte descriptions, multi-byte chars, 10000-byte single-line scalars, adversarial literal `# NNNN` patterns, the INV-gajap-as-of-2026-04-08 size class, a multi-fold-length canary, and a width-guard observable-property test that detects any future refactor re-introducing folding.
