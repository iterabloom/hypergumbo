...
- **Out-of-repo write-ahead journal makes pending ops durable against any working-tree git command (`tracker recover`).** Pending tracker ops live in the git working tree until auto-sync commits them, so any working-tree-destroying command — `git reset --hard` (the one that lost a human's in-flight approval messages), `git checkout` to a stale base, `git clean -fd` — could drop them. The prior fixes (WI-buhov, INV-dalup, INV-lovih) each hardened only *one* path (auto-pr's rebase, auto-sync's cleanup), which is whack-a-mole. This closes the class **structurally**: every op is mirror-appended (inside `Store`'s existing per-file `flock`, so no new lock and atomic with the in-repo write) to a journal at a location git cannot see — `~/hypergumbo_lab_notebook/tracker-ops-journal/<sha256(repo-toplevel)[:12]>/` (override via `HYPERGUMBO_TRACKER_JOURNAL_ROOT`; per-clone keyed so multiple clones don't bleed). The write is best-effort — a journal failure (unwritable `$HOME`, full disk) degrades silently and never fails the mutation. The new **`tracker recover`** command (and, in a follow-up, an auto-firing `reference-transaction` hook) union-restores journalled ops back into the worktree `.ops` files. Empirically the op *data* survives `reset --hard`, `checkout`, `clean -fdx`, even `rm -rf .git`; for the common vectors (`.git` survives) `tracker recover` runs directly, and after `rm -rf .git` the data is still safe under the journal root and recoverable once the repo is re-init'd / re-cloned (the journal is keyed by the toplevel's absolute path). Durability no longer depends on anyone avoiding a particular command.

- **One canonical op-block `.ops` restore primitive (shared by `recover` and `do_sync`).** Both `tracker recover` and `do_sync`'s INV-dalup post-cleanup restore now union `.ops` content at the **op-block** granularity (`journal._union_op_blocks`) — deduping whole ops (the semantic unit of an append-only log) rather than individual lines. This collapses two equivalent line-level union implementations into one: `do_sync`'s private `_union_lines` is deleted in favour of the shared primitive. For the current format the two are **byte-identical** — `store._serialize_op` stamps a per-op `# <nonce>` comment on every non-empty line (scalar folding disabled via `width=sys.maxsize`), so every line is unique to its op and a line-level dedup never collides. Op-block is chosen purely so a single implementation serves both restorers, at the op granularity — not because the line-level union corrupts the real format (it does not; an earlier note claiming so was a misdiagnosis from a synthetic repro without nonce comments).

### Fixed

- **`recover` serializes its `.ops` rewrite with concurrent `Store` appends (INV-hakuv).** `journal.recover()` did a non-locked read → union → rewrite of each worktree `.ops` file, so a concurrent `Store` append — which holds an `fcntl.flock(LOCK_EX)` on that file's own fd (`store.py` create / `_append_op`) — landing between recover's read and its rewrite was clobbered from the worktree. The op survived in the append-only journal (so the *next* `recover` restored it — transient, not permanent loss), but the unsynchronized whole-file rewrite racing an `O_APPEND` could also leave a torn `.ops` that the op-block union then carries forward, freezing the item until a human excises the fragment. Recover's per-file rewrite now routes through `_locked_union_restore`, which takes the **same** `LOCK_EX` on the `.ops` inode around the read → union → rewrite and rewrites **in place** (`truncate` + `write` on the held fd — never `os.replace`/rename, which would swap the inode out from under the lock and leave the flocked `O_APPEND` writing to a now-unlinked inode). Exposure had risen after the PR4 `reference-transaction` hook made `recover` fire on every committed ref transaction. Reproduced by a deterministic threaded test (`test_recover_does_not_clobber_concurrent_store_append`) that loses the concurrent op on the old code and passes on the fix; a lock-spy test asserts recover takes and releases `LOCK_EX` on the worktree inode, and an inode-preservation test guards the in-place contract.

- **`scripts/tracker`'s `python -m hypergumbo_tracker.cli` fallback no longer silently no-ops.** `cli.py` had no `if __name__ == "__main__"` guard, so `python -m hypergumbo_tracker.cli <cmd>` imported the module and exited 0 **without dispatching**. Every tracker command routed through `scripts/tracker`'s fallback path — taken when the `hypergumbo-tracker` console script is not on `PATH` — silently did nothing, including the reference-transaction recovery hook's `recover` (so durability self-healing would silently fail in such environments). Added `if __name__ == "__main__": main()  # pragma: no cover`; regression test `test_cli.py::TestModuleEntrypoint` runs a real `python -m … recover` subprocess and asserts it dispatches. The bug hid because nothing exercised the `-m` entry (`test_hooks.sh` used a stub tracker; `test_journal.py` calls `journal.recover()` directly).

- **`do_sync` suppresses the recovery hook during its own git operations (`tracker-recover-disabled` marker).** The auto-firing `reference-transaction` hook restores journalled-but-uncommitted ops as *untracked* files on ref updates; during `do_sync`'s post-sync cleanup that collided with the very `merge --ff-only` that reconciles them — the merge re-fires the hook via its `ORIG_HEAD` write right before the overwrite check, so the restored untracked file aborts the fast-forward and local dev perpetually lagged the synced remote. `do_sync` now sets a `<git-dir>/tracker-recover-disabled` marker around **all** its git operations (the try body *and* the `finally` cleanup ff), created-guarded so it never clobbers an outer `auto-pr` marker, and the hook skips `recover` while it is present. User-initiated `reset --hard`/`checkout` still self-heal (they never set the marker). Lands with companion `.githooks/reference-transaction` (marker check) and `scripts/auto-pr` (sets the marker around its fetch + fast-forward) changes.

- **Tracker sync stages ops additively — never commits a deletion of an op-log absent from a stale working tree (INV-lovih).** The sync seeds its temp index from `origin/dev`'s tree (which holds every item's append-only op-log) then stages the working-tree ops with `git add`. But a plain `git add -- <ops dirs>` reconciles the index *to* the working tree, staging a DELETION for any op-log present in the seeded base but missing from a stale/incomplete working tree — so a sync run while local dev is behind origin (or after an intervening `git checkout` dropped files) silently committed those items' removal, dropping them from the corpus. Observed live: WI-vojik (the R0 baseline record) and WI-durub (a bakeoff bug) vanished exactly this way — local dev was 2 commits behind origin, a feature-branch rebase made the op-logs tracked, a `git checkout dev` to the stale base deleted them, and the next sync committed the absence — both recovered intact from git history (`d77dd0a368`). Both staging sites (the main plumbing path and the post-CI-divergence rebase path) now route through a shared `_stage_tracker_ops_additive` helper that passes `--ignore-removal` (stage additions and modifications only, never removals). Op-logs are append-only union-merge CRDT logs and a sync is always additive toward them — even a tombstoned item keeps its file with a delete *op* appended — so suppressing removals is purely loss-preventing and drops no legitimate content. Complements the INV-dalup cleanup-race fix below.

- **Op-compilation tolerates explicit-null `set`/`add`/`remove` dicts instead of crashing whole-corpus reads.** `compile_ops` (store.py) and the lock-violation scan (validation.py) read `op_dict.get("set"/"add"/"remove", {})` — but `dict.get(k, default)` returns the default only when the key is *absent*; an op serialized with an explicit `"remove": null` yields `None`, so `None.items()`/`None.keys()` raised `AttributeError` and crashed compilation of the entire corpus. Single-item reads (`show`) survived; everything that compiles all items (`list`, `tags`, `add`, the TUI) crashed once the SQLite read-cache — which had been masking one such pre-existing op — was invalidated by a sync. Both modules now use `.get(k) or {}`, treating a null `set`/`add`/`remove` as "no changes" (the correct semantics). No data is lost: a null remove removes nothing.
