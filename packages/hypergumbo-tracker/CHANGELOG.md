...
- **A host-protected `config.yaml`, because file permissions never protected it (INV-mizid).** ADR-0013 claimed twice that the agent "literally cannot `write()`" to the governance config, since the human owns it at mode 644. **That is not how filesystems work.** Replacing a file needs write permission on its *directory*, not on the file — create a tempfile, `rename()` over the target — and the agent must have directory write there, because the tracker's own `tag_catalog.save_catalog` writes atomically into that same directory. On a two-account deployment the agent also owns the repo root and `.agent/`, and a directory's owner can re-`chmod` it at will, so **no file anywhere in the checkout can be protected from the agent by permissions**: "the agent writes files here" and "one file here is protected from the agent" are mutually exclusive. Three divergences turned up on the live deployment while confirming this — the code targets `0444` not the documented `644`; `handle_init` had no actor guard while calling `config_lock`, whose cross-user fallback *transfers ownership of the config to the caller* (documented in its own docstring); and the config had in fact become agent-owned. The fix is a layer the agent owns no component of: `/etc/hypergumbo-tracker/<12-hex>_<legible path tail>/config.yaml`, root-owned `0755`/`0644`, satisfying the exact requirement — **agent reads, cannot write the file, cannot write the containing directory or any parent**. When it applies it is authoritative and the in-repo file is ignored entirely, never merged. **Absence is fatal by design**: if a missing protected config fell through to the in-repo one, an agent would not need to write the protected file at all, only to stop it being found — so the *existence of the root directory* is the opt-in signal, and once it exists a repo with no config under it refuses to load. The agent can neither create nor remove that directory, so it cannot forge either answer; hosts that never create it are unaffected and single-account deployments are unchanged. **No environment override**, since the agent controls its own environment. `/etc` rather than the human's home because finding `~<human>/` requires knowing which user is the human, which is read from the config being looked for. The hash is the key and comes first, but lookup globs on the prefix, so an administrator may rename `325ef6540f2a_home_jgstern_hypergumbo` to `325ef6540f2a_prod` freely; two directories claiming one hash is refused rather than guessed. Scope stated rather than implied: an agent that relocates the checkout or `pip install`s its own tracker is outside what config protection can reach. `htrac setup` gains a check printing the exact root commands, and the in-repo ownership/permission checks stay as accident guards.

- **A custom `--field` key may no longer shadow a core attribute, and the rule lives in the STORE (INV-varil, INV-linan).** `--field key=value` split on the first `=` and stored the key verbatim; nothing checked it. Over a 2,233-item corpus that produced **11 items carrying a core attribute name as a custom field** — 18 field instances — and every one is a stale creation-time snapshot that never tracked the attribute afterwards, so the two homes disagree: `INV-dabov` read `satisfied` at the top and `violated` in its fields, `INV-vaduk` read `satisfied`/P1 against `todo_soft`/P3, and `todo_soft` is not even a legal status for an invariant. "What is the status of this item" had stopped having one answer. Three tiers keyed on **normalisation**: a core attribute name is refused outright and the message names the flag that actually moves it (`use --status instead`), with **no `--force`**, because an escape hatch is how the corpus acquired eighteen of these; a key that normalises to a *declared* schema key but is spelled differently (`Root_Cause`, `root-cause`) is refused and `--force-field-key` opens that tier and only that tier; anything else is allowed, because the open dict is genuinely used and `dogfood_anon_id` was a convention before it was a declared field. **The gate is in `Store.add` and `Store.update`, not the CLI** — `Store.update` ran no schema check at all and never looked inside `set_fields["fields"]`, which is exactly where a custom key goes and exactly how eleven items acquired one. A **removal** (`--remove-field status`, value `None`) is deliberately exempt: deleting the offending key is the remedy, and a gate that refused it would freeze those items in their broken state permanently. The eleven items are cleaned in this change, one decision at a time rather than in a batch — `INV-muhij`'s core description was **empty** and its whole 3,284-character description lived in the shadow, so there the content was moved up rather than deleted, and `INV-toguh`'s shadow carried a `dead_code` tag the core lacked, which was added before the shadow went.

- **`tracker clusters` predicts clusters of the open items.** TF-IDF cosine on each item's masked full text (title, description, string fields, live discussion; item IDs and ADR numbers masked so a shared ID cannot stand in for shared vocabulary), each item's top-k neighbours, and Jarvis-Patrick clustering — an edge only where two items are in each other's top-k and share at least `--shared` of those neighbours — ranked by mean pairwise cosine. Pure standard library: the vectorizer reproduces scikit-learn's `TfidfVectorizer(sublinear_tf=True, min_df=2, max_df=0.8)`, and the neighbour search is an inverted index. The vocabulary and IDF are fitted on every non-deleted item so the IDF is stable while only the selected items (every non-resolved status by default; `--status`, `--all`) are clustered; 332 open items against a 2059-item fit take 2.4 s. The defaults `k=6, shared=3` come from a parameter sweep on the tracker (308 clusters, largest 17, 12.5% of within-cluster pairs being declared parent/isbefore/duplicate links against a chance rate of 0.04%); `k>=10` chains into one giant component. The choice of method is the outcome of the 2026-09-04..06 lab-notebook investigation: TF-IDF cosine beat a dense embedding model, BM25 and BMX at recovering declared families, and plain top-k neighbours matched or beat a NeighborNet port, its 96-seed block ensemble with group-level consensus, and average/complete linkage at equal pair budgets in about a second (`clusters.py` module docstring).
...
- **`tracker init` could silently take ownership of the human's governance config, and now refuses to run as an agent (INV-mizid).** ADR-0013's command table has always designated `init` as a **human** command; nothing enforced it. That mattered because `handle_init` ends by calling `config_lock` on `config.yaml` *whether or not the file already existed* — so an agent running `init` against an established, human-owned config hit the cross-user `chmod` path, and its fallback "fixed" the failure by rewriting the file, making the agent the owner. Its own docstring says so: the copy "is now owned by the current user". That is the most likely way this deployment's config became agent-owned. Two fixes, because the call site is the instance and the fallback is the family: `init` now mirrors `configure`'s actor guard (including the bootstrap — with no config yet there is nothing to read `agent_usernames` from, so `resolve_actor` falls back to `["*_agent"]`, which is exactly the case `init` runs in), and **`_config_chmod_fallback` refuses outright when the caller is an agent**, so every present and future caller inherits the guard. An agent that cannot change the mode is the correct outcome rather than an error, so `config_lock`/`config_unlock` report the refusal on stderr and continue, leaving the file's permissions untouched; the legitimate human direction is unchanged and still tested.

- **`_check_config_ownership` named every config in its remediation line, not the broken one.** It computes `wrong_owner` but interpolated `existing`, so a deployment with one bad config was told to `chown` both — harmless to run, but it hides which file is actually wrong. The adjacent `_check_config_permissions` already did this correctly against `wrong_mode`.

- **The fields-schema check ran against each op in isolation, so a field that a later op deleted was flagged forever — and that alone blocked `--strict` in CI.** An op log is append-only: a custom field written at create and removed by a later `--remove-field` is *already repaired*, but `_validate_fields_schema` was called from `_validate_create_values` with an op index, so it re-reported op 0's content for the life of the item. On the live tracker that was **26 warnings**, of which every single one named a defect that no longer existed in the compiled item — 17 `severity` and 9 core attributes (`status`, `priority`, `tags`, `description`) across four items whose repairs had landed months earlier. The check now runs against the **compiled** fields dict, beside the `allowed_statuses` check that moved there for exactly this reason in `f148a13a22` ("historical ops may have used statuses that were valid when written but are now restricted"); the fields half was simply never migrated with it. `compile_ops` already applies per-key last-write-wins, so remove-then-re-add is still flagged — "some op removed it" is not repair, the last write decides — and the required-field check gets *stricter*, since a required field deleted by a later op used to pass (op 0 had it) and now fails. Live warnings fall 26 → 0 and **`tracker validate --strict` passes**, which retires the standing claim that immutable history made it unreachable. Two real defects surfaced on the way and were repaired rather than silenced: `severity` (17 items, written by everything and read by nothing — priority is the only ranking axis) was dropped, and `INV-hazov` carried `pr_ref` in *both* homes with the core attribute null and the field holding `9e3a93a75b`, so the value was migrated up before the field was removed.

- **The managed AGENTS.md governance block shipped the broken `--json` position to every adopting repo.** `setup.py`'s `MANAGED_BLOCK_CONTENT` — injected by `htrac setup` and marked "edits may be overwritten" — instructed agents to read item state with a trailing `--json`, which exits 2 with a usage error on stderr and empty stdout, so a JSON consumer misreads a bad invocation as an absent item. Corrected to the global-flag-first form, with the failure mode stated in the block itself.
