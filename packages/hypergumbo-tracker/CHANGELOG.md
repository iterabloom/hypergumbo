...
- **Op-block union-restore primitive (surfaces a latent line-union corruption).** Recovery unions `.ops` content at the **op-block** granularity, not the line level. Building the journal surfaced a latent bug in the legacy line-level `sync._union_lines` (and its `forgejo-api.sh` / `do_merge` twins): `.ops` ops share lines like `by: agent` / `actor:`, so a global line-dedup strips those keys from every op after the first and corrupts the log. The op-block union dedups whole ops instead (identical `clock`+`nonce` ⇒ byte-identical block). A follow-up will collapse the three historical restorers onto this one primitive, fixing that latent corruption in the sync paths too.
