...
- **`cache-rebuild` writes to XDG cache dir, not legacy `.agent` path**: `_cmd_cache_rebuild` hardcoded the pre-ADR-0013 cache location (`.agent/.cache-*.db`), so running it silently rebuilt SQLite files at a dead path while the live XDG-style caches were unaffected. Fix: reuse the Cache instances the dispatcher already wires via `_get_cache_dir()`. Handles the no-cache-dir case (no git repo fingerprint) with a clear error instead of silent success.
