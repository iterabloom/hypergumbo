...
- **A host-protected `config.yaml`, because file permissions never protected it.** ADR-0013 claimed twice that the agent "literally cannot `write()`" to the governance config, since the human owns it at mode 644. That is not how filesystems work: replacing a file needs write permission on its *directory*, and the agent must have that, because the tracker's own catalog saves atomically into the same directory — whose owner can re-`chmod` it at will. So **no file in the checkout can be protected from the agent by permissions**: "the agent writes files here" and "one file here is protected from the agent" are mutually exclusive. The fix is a layer the agent owns no component of — `/etc/hypergumbo-tracker/<12-hex>_<path tail>/config.yaml`, root-owned, authoritative when present, with the in-repo file then ignored rather than merged. **Absence is fatal by design**, since otherwise an agent would only need to stop the protected file being found: the *existence of the root directory* is the opt-in signal, and once it exists a repository with no config under it refuses to load. No environment override, since the agent controls its own environment. An agent that relocates the checkout or installs its own tracker remains outside what this can reach.
- **A custom `--field` key may no longer shadow a core attribute, and the rule lives in the store.** `--field key=value` split on the first `=` and stored the key verbatim with nothing checking it. Over a 2,233-item corpus that produced **11 items carrying a core attribute name as a custom field**, every one a stale creation-time snapshot, so the two homes disagreed — one item read `satisfied` at the top and `violated` in its fields, and "what is the status of this item" had stopped having one answer. A core attribute name is now refused outright with **no `--force`**, since an escape hatch is how the corpus acquired eighteen of these; a differently-spelled declared key (`Root_Cause`) opens only under `--force-field-key`. The gate is in `Store.add` and `Store.update` rather than the CLI, because `Store.update` ran no schema check at all — exactly how eleven items acquired one. Removal is exempt: deleting the key is the remedy, and refusing it would freeze those items broken permanently.

#### Commands

- **`tracker clusters` predicts clusters of the open items.** TF-IDF cosine over each item's masked full text (item IDs and ADR numbers masked, so a shared ID cannot stand in for shared vocabulary), each item's top-k neighbours, and Jarvis-Patrick clustering — an edge only where two items are in each other's top-k and share at least `--shared` of those neighbours. Pure standard library; 332 open items against a 2,059-item fit take 2.4 s. The defaults `k=6, shared=3` come from a parameter sweep: 12.5% of within-cluster pairs carry a declared parent/isbefore/duplicate link against a chance rate of 0.04%, and `k>=10` chains into one giant component. TF-IDF cosine beat a dense embedding model, BM25 and BMX at recovering declared families.
- **`tracker reconcile` runs the three steps a failed post-merge pull leaves behind; `tracker recover` is deprecated in its favour.** When `auto-pr` merges and its post-merge pull aborts on colliding `.ops`, the way out was `tracker sync` → a raw fast-forward → `tracker recover`, none of which reports whether the next is still needed. `reconcile` runs all three, skipping each when unneeded and reporting what every step did *or why it did nothing* — a silent step is indistinguishable from an unnecessary one. It refuses while `.git/PR_PENDING` exists, stops rather than fast-forwarding when the flush fails, and is a clean no-op on a healthy repository. It deliberately does not set the `tracker-recover-disabled` marker, since restoring ops the pull disturbs is precisely the self-healing hooks' job. `recover` remains as the primitive `reconcile` calls and as the hooks' entry point.
- **`tracker sync --prune` closes superseded `tracker-sync/*` branches, and a successful auto-sync does so automatically.** A branch is superseded only when `git merge-tree --write-tree` against dev yields dev's own tree — never merely because the forge reports a conflict, since a branch holding unique ops conflicts for exactly the same reason as one holding none. Every undecidable case yields an empty list rather than a guess.

### Fixed

- **A leaked `tracker-recover-disabled` marker switched ops self-healing off and nothing said so — observed open for ten hours.** The marker is how `do_sync` and `auto-pr` stop the `reference-transaction` and `post-checkout` hooks from fighting their own reconciliation; while it exists both hooks skip `tracker recover`, so a `git reset --hard` or `git checkout` drops pending `.ops` with nothing to restore them (the journal still holds the data — degraded safety, not loss). `do_sync` clears it in a `finally` and `auto-pr` in an EXIT trap, and **bash runs that trap on SIGTERM and SIGINT but not on SIGKILL**, so the leak path is a hard kill and nothing else. Every tracker command now reports an *unowned* marker on stderr, naming its age, what is degraded and the exact `rm`. **Ownership is asked before age**, because age alone cannot decide it: the marker is not scoped to the fetch+fast-forward it guards — `do_pr` touches it as its first act and only the exit trap removes it, so it spans the whole run including a CI poll with a 2400s default timeout and a 300s soft-retry. A live `auto-pr` is named by `.git/PR_PENDING` and a live `do_sync` by the `TRACKER_SYNC_PENDING` flock, which cannot lie because the OS releases it even on SIGKILL; only an unowned marker is timed, and then solely to cover the two ends of a run where `PR_PENDING` does not exist yet or no longer exists — before the push, and after `cleanup_local` drops the gate but before the process exits. The second is precisely where the observed incident died. **That grace is sized against a measurement and was refuted once by a live run within the hour of shipping:** on PR #975, an ordinary run with fast CI, the marker sat unowned for **172s** before `PR_PENDING` appeared — 51% of the entire 336s run, against a first draft of 300s sized by an estimate that had considered only the other end. It was raised to 900s, and then **deleted outright** when the marker became an fcntl lock (see the main changelog): a lock answers "is anybody there" as a fact, so the detector now reports a lock whose recorded owner pid is gone rather than timing an unowned file. The grace existed only because a flag cannot name its owner. Nothing deletes the marker: a process that clears one it does not own races a long `auto-pr` and re-opens the fight the marker prevents.
- **Human authority ran on agent-writable code, and the setup wizard told it to.** The two-account split moved the governance *data* and the authority *decision* out of the agent's reach (`resolve_actor` keys on `os.getuid()`, unforgeable) — but never the *code*. There was one install: an editable one, owned by the agent, pointing into a checkout the agent must be able to write — and the shim positively instructed the human to activate that venv. Every human-authority command therefore executed code the agent could rewrite in place, including the function deciding what human authority *means*, and an in-place edit passes no PR, CI or review. The shim now emits a **non-editable install under the human's own account, pinned to a reviewed commit**. The SHA is offered for verification on the forge, explicitly not as something to trust: the pin is the entire mechanism, since an agent cannot make a given SHA contain different bytes. A new `install_provenance` check reports who can modify the running code and is **documented as an accident guard, not a security control**, because it lives inside the artifact it audits.
- **Two-user mode was wired to directories nobody chgrps, and the wizard undid its own config hardening.** Four defects, each of which let a correctly configured two-account deployment report as healthy while being none of those things. **(1)** A relative `--tracker-root` silently disabled host protection — the repo-root walk terminates at `.` without seeing `.git`, and the loader read that as "outside a repository" and fell through to the one config the agent can write, a one-flag downgrade of the whole layer. **(2)** The shared-group probe examined directories a correct setup leaves at the owning user's primary group, so it reported "single-user" on a properly configured box and the three checks gated on it did nothing. **(3)** One check set `config.yaml` to `0444` and a later group sweep put group-write back, landing on `0464`, both reporting "fixed" every run. **(4)** Under host protection the wizard inspected and edited a decoy, so a human would validate a file the loader ignores — worse than no wizard, because it looks like it worked.
- **`tracker init` could silently take ownership of the human's governance config, and now refuses to run as an agent.** ADR-0013 has always designated `init` a human command and nothing enforced it, which mattered because `handle_init` ends by calling `config_lock` *whether or not the file already existed* — so an agent running it against an established human-owned config hit the cross-user `chmod` path, whose fallback "fixed" the failure by rewriting the file and making the agent the owner. Two fixes, because the call site is the instance and the fallback is the family: `init` gains an actor guard, and `_config_chmod_fallback` refuses outright when the caller is an agent, so every future caller inherits it. Separately, the ownership check named every config in its remediation line rather than the broken one.
- **Thirteen tests passed or failed depending on whose machine ran them.** Protected-config loading treats the *existence* of `/etc/hypergumbo-tracker/` as the opt-in signal and refuses to fall back when it holds no entry for the repository being loaded — deliberately, since silent fallback is the downgrade it exists to close. Tests building a repo under `tmp_path` inherited whatever the host had: on a host that never opted in they passed, and on one running the two-account setup all thirteen raised. Same tree, same commit, two answers — and **CI never saw it**, because its containers have no such directory, so the host that hid the defect was the one that matters. One autouse fixture now pins the protected root to a path guaranteed absent — deliberately not a skip-if-present guard, which would leave these tests untested on exactly the hosts running the configuration they are about.
- **The fields-schema check ran against each op in isolation, so a field a later op deleted was flagged forever — and that alone blocked `--strict` in CI.** An op log is append-only: a custom field written at create and removed later is *already repaired*, but the check was called with an op index and re-reported op 0's content for the life of the item. On the live tracker that was **26 warnings**, every one naming a defect that no longer existed in the compiled item. It now runs against the **compiled** fields dict, beside the `allowed_statuses` check that moved there for exactly this reason and with which the fields half was never migrated. Remove-then-re-add is still flagged — the last write decides. Live warnings fall 26 → 0 and **`tracker validate --strict` passes**, retiring the standing claim that immutable history made it unreachable.
- **The managed AGENTS.md governance block shipped the broken `--json` position to every adopting repo.** `MANAGED_BLOCK_CONTENT` — injected by `htrac setup` and marked "edits may be overwritten" — instructed agents to read item state with a trailing `--json`, which exits 2 with a usage error on stderr and empty stdout, so a JSON consumer misreads a bad invocation as an absent item. Corrected to the global-flag-first form, with the failure mode stated in the block itself.
- **Nine `tracker-sync/*` PRs sat unmergeable, the oldest since August 24, and every one of them was empty.** `.gitattributes` declares `merge=union` on `.ops`, but that is a client-side driver: the forge's server-side merge does a plain three-way merge, so two syncs appending to the same journal from a common base conflict there. Nothing was lost — `do_sync` gates its cleanup on merge success, so each later sync carried the stranded ops forward — and both a per-file set difference and `git merge-tree` confirmed all nine contributed zero lines to dev. `--prune` above is the remedy.
- **The modal test harness gave up silently, and one TUI test then masked three unrelated CI gates for a day and a half.** `_wait_for_modal` polled up to 30 event-loop ticks and then returned as though it had succeeded, so callers drove a DOM that was not ready and failed later on an assertion naming neither the widget nor the wait; because Woodpecker collapses a workflow's steps into one aggregate commit status, the red full-suite run made three other gates unreadable alongside it. The helper now raises, naming what it waited for and for how long, and waits for layout rather than DOM presence — a synthesised click at an un-laid-out widget's region lands nowhere.
