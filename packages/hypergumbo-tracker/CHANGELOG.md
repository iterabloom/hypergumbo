...
- **`tracker delete` now sweeps dangling `isbefore` references (INV-vudit):** previously a deleted item's `isbefore` list and any inbound holders kept the now-dangling ID. `TrackerSet.ready()` filtered these at query time, so `ready` was semantically correct — but `tracker show` / `tracker deps` and any future tooling walking `isbefore` without status-checking treated the deleted ID as a real dependency. `_cmd_delete` now clears the deleted item's own `isbefore` and emits `remove_fields={"isbefore": [deleted_id]}` ops on every inbound holder across every tier, bundled into the same op as the status flip. The CLI prints `Cleared N inbound isbefore references.` when non-zero (silent on leaf deletions); JSON output gains an `inbound_isbefore_cleared` field. +3 tests.

- **`tracker tags rename` no longer rejects renames FROM legacy-format old tags (WI-hohov):** `_cmd_tags_rename` succeeded at the per-item update (every item got the new tag, lost the legacy one), but the trailing `setdefault(old, TagCatalogEntry(...))` left a regex-violating key in the catalog dict, so `save_catalog`'s validator raised and the verb exited non-zero. Hit empirically during the 2026-05-07 BVT-discipline migration (`for-deep-bakeoff` → `for_deep_bakeoff`, `for-uat-bakeoff` → `for_uat_bakeoff` across 21 items); users either abandoned the rename or fell back to a per-item loop with no audit trail. Fix: when `old` violates `_TAG_NAME_RE`, drop any pre-existing catalog entry under that name and skip the audit-trail bump on the legacy half. The rename completes and the catalog converges to a valid state with the snake_case entry only; snake_case → snake_case renames still record both endpoints unchanged. +2 tests.
