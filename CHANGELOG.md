...
- **Canonical edge-type registry** (`hypergumbo_core/edge_types.py`): single source of truth for `Edge.edge_type` values, each annotated with an axis classification (`relationship`, `endpoint_shape`, or `pending_classification`). `scripts/generate-schema` consumes the registry and emits an `x-axis-of-values` JSON Schema extension on `Edge.type`. A property test AST-walks the package source and fails CI if any module-level `*EDGE_TYPE*` set contains an unregistered value.
...
- **Pre-commit drift linter** (`scripts/check-edge-type-drift`): catches consumer-side hardcoded `*EDGE_TYPE*` sets that drift from the registry. Implementation is field-agnostic (`axis_drift.find_drift(...)`) so future axis-bearing fields inherit the pattern per ADR-0024.
...
- **`docs/migrating-edge-types.md` — downstream consumer migration guide**: rename table, meta-key vocabulary (`bridge_kind`, `channel_kind`, `mechanism`, `construct`, `dispatch_kind`, `protocol`), worked patterns, and the deprecation timeline.
...
- **Pre-commit drift gate flipped to `--strict`**: with the protocol-call producer migration clearing the last load-bearing consumer reference, `.githooks/pre-commit` now runs `check-edge-type-drift --strict` by default. Future endpoint_shape regressions fail at commit time.
...
- **Earlier phases also landed this cycle**:
    - **Phase 4a** — `Edge.type` gained an `x-deprecated` JSON Schema extension listing every endpoint_shape value as a removal candidate (`SCHEMA_VERSION` 0.2.7 → 0.3.0).
    ...
    - **Registry-completeness sweep** — 25 new entries (`SCHEMA_VERSION` 0.3.0 → 0.3.1) triaged from 30 emitted-but-unregistered values: 5 control-flow noise (kept unregistered), 7 missing relationship canonicals (`inherits`, `decorated_by`, `includes`, `defines_target`, `data_flows_to`, `module_exports`, `overrides`), 18 endpoint_shape candidates seeded for future per-pattern audits.
    - **Schema-enum completeness sweep** — 13 already-emitted values added (`SCHEMA_VERSION` 0.2.4 → 0.2.5).
- **Drift detector generalised to multi-field, scope broadened**: AST walker now lives in `hypergumbo_core.axis_drift` (parameterised on `name_filter` and `registry_names`). Default search scope expands from `packages/` only to `packages/` + `scripts/` + `.agent/`. Surfaced and fixed one phantom-value bug in `bakeoff-deep::_FFI_EDGE_TYPES` (`jni_bridge` / `pyffi_bridge` — neither was ever emitted).
...
- **ADR-0025 / ADR-0026 reclassified as `docs/audits/0001-dispatch-publish-family.md` / `0002-ipc-family.md`**: both were per-value verdict tables, not architecture decisions. Permanent redirect stubs preserve URL discoverability. ADR-0024 absorbs §"Family-audit verdict methodology" (CANONICAL/FOLD/DEPRECATE-NO-FOLD trichotomy) and §"Fold-residue discipline" (rules for promoting recurring meta keys to dedicated fields).
...
- **Post-Phase-4b consumer docs corrected**: `docs/concept-axes.md` preamble (an inverted "producers no longer emit" statement) and `docs/migrating-edge-types.md` "What's NOT migrated yet" (restructured into three groups by next-ship: pending-classification (4), protocol-call family (3), long-tail sweep (22)).
