...
Three themes. First, **receiver typing is drained as a class across the analyzer fleet.** A method call whose receiver the repository already describes now reaches the I/O catalogue instead of ADR-0051's `external` sentinel — the marker for *unreachable to the catalogue*. Python, Java, Kotlin, Swift, Objective-C, Scala, Rust, JavaScript, Elixir and Haskell each learn shapes they held the evidence for and were not reading: own instance fields, bare parameters, expression and chained receivers, loop and optional bindings, declared types, return-type registries, and the Django ORM surface end to end. A clean verdict that rests on a typed receiver now says so rather than going quiet.

Second, **taint precision is measured under a ratified frame, and the shipped catalogues are corrected against it.** ADR-0046 makes precision two numbers (correctness and usefulness), ADR-0048 brings the benchmark frame into the repository as a CI gate, and ADR-0049 rules that a call which merely opens, registers, subscribes, schedules or defers a network crossing is disclosed rather than minted as untrusted input. Measurements 0003 through 0012 follow: useful precision, which had been tripping the `≤25%` band, moves 24.1% → 30.9% after sweeps that stopped minting taint from server launches, connection setup, lifecycle callbacks, in-memory buffers, `/dev/null` writes and reads mis-filed as logging.

Third, **the catalogues become extensible and the vocabularies become declared.** ADR-0047 makes the shipped catalogues stdlib-only and moves 291 third-party rows into community overlays that load by default; ADR-0045 adds user and project configuration plus a per-repository trust grant for the code-executing Rust backend; ADR-0050 and ADR-0051 give the I/O-boundary vocabulary and the module key declared axes. The ADR-0017 §3a walk gains removal authority (ADR-0052), and `verify-claims` says more about itself — the fidelity it ran at, the receivers it could not type, the completeness grants it rested on, and the analyzers blind by construction. `SCHEMA_VERSION` advances 0.20.1 → 0.20.7 and the `verify-claims --json` envelope 2.0 → 2.4.

### Added

#### Receiver typing across the fleet

- **The Django ORM surface is typed end to end, and the edges say what typed them.** A QuerySet chain keeps its ORM module past the first hop (`Order.objects.filter(...).exists()`); reverse-relation managers, custom manager names and pretix's soft-delete `Checkin.all` carry `django.db.models`; the instance write is recognised through the model lineage rather than one direct base (2 → 75 call sites on a repository holding about 2,900); `super().save()` is typed on a walk that no longer crosses reachability edges, where 70% of pretix's 306 typed sites override `save`; and test fixtures assigned in `setUp` / `setUpTestData` carry the model instance (+1,447 ORM call sites, 0 gone). Finally, a relation accessor the project's own models declare types the ORM slot on an *untyped* root — **+2,943 ORM-slot edges with 0 lost**, 99.6% of them promoted off `external`, against a shuffled-index ablation mean of +37.3 (**78.9:1**, on a kill threshold of 10 fixed before the number existed).
- **Lazy combinators leave the minting boundary, under a new `db_compose` value.** `filter` / `exclude` / `order_by` and the other eighteen QuerySet-returning members compose a query and read nothing, yet sat under `db_read` and minted `untrusted_input`; the evaluation they stood in for gets its own call site.
- **Ten analyzers learn the receiver shapes they had evidence for.** Own instance fields of external type (Python, Rust); bare parameters and declared types (Java, Swift, Objective-C, Kotlin); expression receivers such as `store.session().invalidateAndCancel()` (Swift); loop and optional bindings (Swift, Objective-C); chained receivers and the `?` / `.unwrap()` forms (Rust); `self.<property>` (Swift, Objective-C); `obj.field.method()` attribute chains (Python); and an unknown receiver emitting the same placeholder inline as assigned. Kotlin emits an instance-method call edge on an external receiver at all, where every gate in the branch required an in-repo lookup a JDK method can never satisfy; JavaScript emits one for every instance-method call, with a `new`-constructed receiver carrying its constructor's module.
- **Return-type registries are consumed, and a library-signature catalogue supplies what no repository declares.** Objective-C, Swift, Java and Go type a local bound to a call result from registries they had only been building; `library_signatures/` supplies the return type of a *library* call, which no analysed declaration can provide. Swift binds a project member only when some overload's argument labels admit it (`removeItem(at:)` and `removeItem(atPath:)` are different methods), and Rust emits a method call on a signature-declared receiver even when the name sits on the generic-trait denylist.
- **Two grammar-level enablers.** tree-sitter-swift is bumped 0.0.1 → 0.7.3, which parses a `#if` block inside a type body — Alamofire's `open class Session` had never been a symbol — and an unparseable file is retried through a byte-preserving rewrite of the spellings 0.7.3 lags on. Three Apple-SDK macro families break the Objective-C parse and the grammar silently falls back to C: error-parsing files go **74 → 38 (19.1% → 9.8%)**, and on AFNetworking primitives reached through Objective-C edges go 12 → 21 with `net_recv` 0 → 2.
- **A verdict typed from a name stays qualified rather than going silent.** Typing a receiver removes it from `untyped_receiver_sites`, so a recall gain would have bought a quieter all-clear — the one direction this project treats as unsafe. Edges filled by the accessor-name rule carry `resolution_quality="accessor_name"`, and `CAVEAT_ACCESSOR_NAME_RECEIVER` is its complement over one population: *"I could not type N receivers"* becomes *"I typed N receivers from a declared accessor name, not from a typed root"*. `call_construct` is stamped for every receiver shape, since that is what the disclosure channel reads — Java, Objective-C and Rust never stamped `method` on an external instance-method call, so a clean verdict on the languages holding 55% of catalogued method-kind sinks disclosed nothing.

#### I/O boundaries: the deferred crossing (ADR-0049)

- **ADR-0049: a deferred crossing is disclosed, not minted.** One question decides whether a network call is a transfer — does it return, or write into a caller-visible location, a value whose content the far side chose? A call that merely opens, registers, subscribes, schedules or defers is a deferred crossing. **`net_listen`** is its disclosure boundary: it mints no taint, is reported in its own `net_listen_edges` count rather than the `total_io_edges` headline, and shadows `net_recv` so a clean network verdict over a listener is qualified rather than granted.
- **A multi-boundary primitive declares why it has several boundaries, and names its own fallback.** Of 29 such primitives across the 15 catalogues, 12 declared nothing, so C's deliberately undecidable `unistd.write` was indistinguishable from an unruled row; `boundary_ruling` (`simultaneous` / `call_site_undecidable` / `unruled`) resolves that, and `abstains_to` lets a `call_site_undecidable` row name where an unstamped call falls back — C's `fgets` (conservative direction `fs_read`) and Go's `bufio.NewScanner` (`ipc_recv`) could not both be right under one global constant.
- **`io_target_kind` gains `in_memory`, `pipe` and `net_stream`**, and `_DISCARDING_TARGET_KINDS` becomes `_NON_CROSSING_TARGET_KINDS` so a `/dev/null` write and an in-memory read read one vocabulary. The seam runs in both directions, with `_WRITE_TARGET_KIND_BOUNDARY` answering writes.
- **A DNS lookup is a network receive, in five languages.** The content of a resolver's answer is chosen by the far side, so `socket.gethostbyname` / `getaddrinfo` / `getnameinfo` / `getfqdn` and their peers in Python, Go, Java, Erlang and Node are `net_recv`. The ruling was decided by parity and its reach measured per language rather than assumed: the rows add 6 situations across an 11-repository DNS-dense cohort, move 0 verdicts, and 0 of the 6 are correct — a number that measures the walk, not the rows. Three Erlang rows sort *out* of `net_recv` in the same pass (`inet:gethostname` reads the local node's name; `getservbyname` / `getservbyport` read `/etc/services`), and `socket` is granted module completeness after enumerating all 35 of its module-level callables.

#### Catalogue extensibility and user configuration (ADR-0047, ADR-0045)

- **Community overlays ship in the wheel, load by default, and say so on stderr every run.** `python-http-clients.yaml` and `go-web-frameworks.yaml` had sat under `docs/`, which `pyproject.toml` does not package, so they shipped to nobody. Their rows are stamped `unvouched` at merge, so a call they classify cannot silently vouch for a verdict.
- **User catalogue channels work end to end.** `hypergumbo init-catalogs` creates `$XDG_CONFIG_HOME/hypergumbo/` with one `<family>.d/` directory per extensible family; `io_primitives.d/` is now actually scanned on every run, where every overlay-loading run had been telling users to edit a directory nothing read. `hypergumbo catalog-inventory` reports what an installation actually knows across its 156 YAML files, with extensible and readable reported as separate facts.
- **User and project configuration files, and one setting they are forbidden to hold (ADR-0045).** `$XDG_CONFIG_HOME/hypergumbo/config.toml` and `<repo>/.hypergumbo.toml` carry `io_primitives`, extending the precedence chain user config < project config < claims-file `extra_catalogs` < `--io-primitives`; a repository may not grant itself the Rust backend. **`hypergumbo trust-backend`** is a durable per-repository grant for the SCIP Rust backend, which executes the analysed crate's `build.rs` and proc macros as the invoking user; the only persistence had been a global environment variable.

#### verify-claims — what a verdict says about itself

- **The §3a walk publishes its own verdict, so `flows_removed_by_walk: 0` is readable (ADR-0052).** `dataflow_coverage` reported how many flows the walk removed without reporting whether it ever ran, and the rollup hid the difference by folding three walk verdicts into `ddg_mixed`. On the measured corpus the honest answer is `unavailable` 224 of 224.
- **A verdict reports the analysis fidelity it was reached at** (envelope 2.2). `analysis_fidelity` maps language → the pass IDs that produced its call edges, and a `higher_fidelity_available` caveat distinguishes a backend installed but not enabled from one that does not exist.
- **A clean verdict names what it could not see.** New ADR-0016 §4 caveat kinds `untyped_receiver` and `unknown_receiver_scope` name the call sites where a method catalogued for the claimed boundary is reached through an untyped receiver, with a commensurable denominator; a language whose analyzer emits no external instance-method call says so from a dated, fail-closed per-language declaration (Kotlin and JavaScript, covering 232 catalogued method-kind sinks); a catalogued row only a non-call construct reaches is disclosed; and the completeness grants a verdict rested on are reported by module, shipped catalogue and overlay alike.
- **A verdict says how much of the catalogue behind it is unverified.** A row asserts that a *name* crosses a boundary *kind*, and nothing checked that the named primitive performs that operation — the defect that had `newIORef` filed under `db_read`. `catalog_provenance.kind_adjudication` reports the unadjudicated share, derived from the catalogues the run loaded rather than restated.

#### Declared blindnesses

- **`untrusted_input` does not distinguish a value the far side authored from one it merely chose from a constrained set**, and that is now a dated declared blindness rather than a pending fix. Of the 324 catalogue names that mint the label, 24 return a chosen value. The field was declined on a threshold fixed *before* any number existed: a 23-repository cohort sampled deliberately for socket ownership — maximally favourable to the field — produced **2 of 568 findings (0.35%)** against a 5% bar. Reaching a choice-shaped row almost never produces a finding.
- **A catalogue row can declare that an argument merely NAMES the resource its sink acts on** (`VERIFY_CLAIMS_SCHEMA_VERSION` 2.3 → 2.4). Tainted data reaching `os.Chmod` to select *which* file stays a true positive on the correctness axis and is excluded on usefulness, disclosed in a per-verdict `resource_naming_flows` count. Suppression requires a third condition the ruling did not name, because the walk carries no argument identity: the annotation is keyed per *function*, not per row, since a row is not a primitive — `c.yaml`'s stdio row lists fourteen functions spanning all four classes at once.
- **`analysis_runs[].silence_reason` says WHY a pass produced nothing, on a closed axis rather than in prose** (`SCHEMA_VERSION` 0.20.6 → 0.20.7). "61 of 84 passes produce 0 edges" counts an undifferentiated population: **274 of 322 zero-edge pass-runs analysed zero files** and only 22 were truly silent. An absent reason means *not applicable*; `unreported` means *cannot determine* — collapsing them would repeat this project's recurring absent-versus-empty defect, and a non-zero `unreported` count is expected, its size being the size of the remaining work. The reason is shown, not just recorded: one line on stderr at the end of a run, since a field nothing reads back is not a disclosure. Scope is stated rather than implied — this ships the recorded-reason half only, not the pass-dependency graph.
- **Go's `call_construct` is a declared blindness, priced at zero rather than fixed.** Go writes a package-qualified free function with the same syntax as a value-receiver method, so `http.Get(u)` and `c.Get(u)` are byte-identical in the fields available, and the slot records the *syntax*, not the construct. Measured across 14 Go repositories and 72,396 unresolved `cc=method` call edges carrying no module slot, the ambiguity costs a verdict zero times.

#### Measurement — the frame, the series and the instruments

- **ADR-0046: taint precision is two numbers.** Correctness precision (`TP / adjudicated`) is unchanged, so measurements 0001–0006 stay comparable; useful precision deducts the vacuous true positives, since the model cannot separate a value that leaks from one a config-driven program was told to act on.
- **ADR-0048: the benchmark frame is in the repository, and rule F8 is a gate.** `scripts/check-measurement-frame` runs in CI and requires a machine-readable `## Frame` block naming unit, allocation, seed, cohort, claim set, rubric, analyzer SHA and language scope.
- **The series prices the arc.** Measurements 0003–0006 and 0012 price precision; 0007, 0008 and 0011 price the walk's reach, finding ADR-0017 §7a's addressable domain to be zero on this corpus (90.8% of `ddg_mixed` rows rest on a walk that never ran); 0009 and 0010 test the deferred-crossing ruling and grow its shape table from four shapes to seven; 0029 reports that the ten-cell receiver-typing drain moved user-visible answers in six of seven languages with zero primitives lost anywhere. `scripts/measure-catalogue-exposure.py` replaces a hand-built exposure table that had ranked languages by method-kind share alone and concluded C, C++, Elixir, Erlang and Haskell were not at risk.
- **The instruments are in-repo and tested.** The adjudication packet builder ships as `measure-taint-precision.py packet` with 42 tests, after 0006's own packets carried an empty sink listing for 66 of 112 situations (58.9%) because they searched only the source symbol's span; an honest empty now names what was searched.

#### Developer tooling and CI

- **`ci-debug` reads what Woodpecker's one aggregate bit hides.** `cron-status` reports the most recent cron verdict per workflow off whatever commit carries it — a cron status attaches to whatever was `dev`'s tip at firing, which HEAD-only `status` could never see — with exit `2` for a gate with no readable verdict in the window.
- **A production function reachable only from its own tests fails CI.** `scripts/check-test-only-reachability` asks whether every direct caller lives in a test module, the question `dead-code-maybe` cannot ask of exported API; six ADR-0017 artifacts had been closed on "N tests, 100% coverage" with no production caller.
- **A node or edge id is minted, never hand-spelled, and the check reads the producers** (ADR-0034). The runtime spec-validator classifies a malformed `Symbol.id` only for ids a given run actually produced, so a linker firing on one template language stayed unchecked until someone surveyed a matching repository; what is forbidden is not the f-string but the ungrammatical id.
- **Smaller.** A gate fails on any stdlib import whose debut is above the declared `requires-python` floor unless it sits inside a `try` catching `ImportError`, with the watched set derived from the floor; `tag-release` verifies push credentials before it signs, after tagging v8.0.0 signed cleanly and died at the push; `contribute` gains `--help`; and `check-self-claims` gains `--minimal`, taking 30.4% off the analysis at byte-identical output.

### Changed

#### Vocabulary and axes (ADR-0050, ADR-0051)

- **The I/O-boundary vocabulary is a declared axis (ADR-0050).** Six consumers branch on its nineteen values and it had none of ADR-0024's four artifacts; the axiom is ADR-0049 ruling 1 verbatim, over four sections (`data_crossing`, `opacity`, `speculative`, `deferred_crossing`) with `catalog_declarable` and `counts_in_headline` as declared properties.
- **The module key did not lack an axis declaration — it carried a false one (ADR-0051).** `ExternalRef.module_path` read `free-text — consumers never branch on the value itself` while `io_boundary._module_matches` decided type-versus-sub-package from orthography, a heuristic that is information-free wherever module names are capitalised. Unlike `Edge.edge_type` (ADR-0023), `Symbol.kind` (ADR-0027) and `Edge.evidence_type` (ADR-0028), the module key had no declared axis at all. The axis-declaration lint and the drift collector are widened to match: `DEFAULT_CORE_FILES` gains `io_boundary.py`, surfacing 20 undeclared fields across 7 dataclasses.
- **`env_read` splits into `env_read` and `host_info_read`**, so the auto-derived taint label means what it says: 134 of 195 `env_read` rows were host description or identity reads (`runtime.GOOS`, `os.uname`, `navigator.platform`) deriving `host_secret` sources, the weakest family in measurement 0001 at 22.9%. **The shipped I/O catalogues are stdlib-only for the first time and every one is gated** (ADR-0047 rulings 1 and 8), with 291 third-party rows moving row-for-row into five community overlays. **Four `call_construct` values that named a different axis are drained**, and a meta key's edge scope is declared on `applicable_edge_types` (ADR-0038 ruling 2) rather than in prose.

#### Taint adjudication and reporting

- **Taint refutation is confirm-only in practice, and the escape-closing goal is retired (ADR-0052).** ADR-0017 §3a's removal *capability* is untouched — a flow whose walk returns `unconfirmed` is still removed — but the goal of driving escapes to zero is not worth its cost: 66.7–90.6% of blocked-walk sites have no callee recorded at the line at all, and the mechanism fired on zero of 153 `ddg_mixed` rows across 11 repositories. **`False` now removes the flow**, where the §3a arm had read `adjudicated = walk_result is True` and so treated "the walk exhausted every route" and "the walk lost the value" as one event that removed nothing.
- **An unadjudicated flow is reported once per situation, not once per call-site pair** (envelope 2.0 → 2.1). Six repositories reported 359 flows describing 78 situations, caddy's `cmdRun` alone emitting 76; each is now one row keyed on (label, source symbol, sink zone, sanitizer state).
- **`analysis_method` no longer answers two questions at once.** It says which analysis produced a finding; `walk_verdict` (`confirmed` / `unconfirmed` / `escaped` / `not_attempted` / `unavailable`) records what the DDG walk concluded and `walk_blocked_by` names the first guard that stopped it. **What `untrusted_input` asserts is written down, and what it does not** — ADR-0049 ruling 1's fact, that a party on the far side had a say in the value, not that the far side authored its bytes.
...
- **`SCHEMA_VERSION` 0.20.1 → 0.20.7, six patch bumps, every one additive** — `Edge.meta` gains `call_arg_shape` (0.20.2, sparse and opt-in), `callee_name` (0.20.3, the full-fidelity name ADR-0036 Ruling 1 tells consumers to read instead of the id's lossy slot, stamped unconditionally), the bash per-call-site keys `redirect_target` / `redirect_target_resolved` / `env_var` with their `<key>_values` collapse companions (0.20.4) and `io_target_kind` (0.20.5); `TaintFlowFinding` gains `walk_verdict` and `walk_blocked_by` (0.20.6); `AnalysisRun` gains `silence_reason` (0.20.7). Every artifact written before each bump stays valid. **`VERIFY_CLAIMS_SCHEMA_VERSION` advances 2.0 → 2.4.**
- **`MetaKeySpec` gains `per_call_site`**, and a collapsed edge no longer reports one call site's fact as the whole relationship's: a per-site key whose collapsed sites disagree is removed and its distinct values move to `<key>_values`, mirroring `call_lines`. A function that opens a path for read and then for write had reported `fs_read` only — the truncating write vanished and a `must_not_exist: fs_write` claim confirmed on it, the ADR-0033 false-confirm class reached through the edge collapse; both boundaries are now reported.
...
- **Six constructs emitted no call edge at all, so no downstream evidence could rescue them.** Python's bare builtins, because the permitting set was the I/O catalogue itself (`frozenset({"open"})`, so `print`, `len`, `input`, `eval` and every unresolvable bare name emitted nothing); every Go call under a package-level `var`, which made each cobra `Run:` handler invisible; a Python method call on an own instance field of external or unknown type, so a `net_send must_not_exist` claim over `self.sock.sendall(payload)` returned a bare `confirmed` at exit 0; Haskell zero-argument IO actions (`getLine`, `getArgs`, `exitFailure` — 505 sites, not the 19 first scoped); Erlang's OTP `?LOG_*` macros, which are 91% of rabbitmq's logging surface at 1,910 macro uses against 183 direct calls; and a Rust grouped `use` list, which registered no import aliases at all across 37.9% of the corpus's `use` statements in 121 repositories.
- **A JavaScript handler *assignment* emits a registration edge, so `ws.onmessage = h` reaches the catalogue.** It is the browser receive surface short of `addEventListener` and it is a property assignment, not a call, so every branch of the member-call cascade missed it. Scope was set by parity with the call path rather than by the construct, and the property must be one the catalogue rows. It was predicted to buy zero findings; measured on the production path it does, and the prediction is recorded as refuted rather than quietly dropped.

#### Call resolution and module slots

- **The same Elixir code over the same repository did not produce the same answer twice.** A bare stdlib-named call was attributed to `next(iter(imported_modules))`, and Python randomises string hashing per process: on phoenix-framework, identical tree and identical blob, two hash seeds produced surveys differing by 101 edges. Every Elixir number this project has published was taken through that. Ordering the set would have fixed determinism and shipped a deterministic wrong answer — the underlying fault is that one name held two populations, Kernel auto-imports and genuinely imported names, with ownership written beside them as prose and never used as data. **653 of 653 edges this branch emitted were false attributions**, so it never once fired on the population it was written for; after the fix both seeds produce byte-identical surveys.
- **Elixir reaches OTP at all, and stops resolving qualified calls by bare name.** An atom-module call (`:ets.insert`, `:httpc.request`) required an `alias` child the grammar does not give it, so it emitted no edge: **+122 call edges across 25 distinct OTP modules, 0 lost**, taking `io-boundaries` 250 → 277 chains and reaching `db_read` and `db_write` for the first time. Separately, a substring ownership test let a qualified call bind by bare name — 202 false edges, including `Logger.error` landing on an `error/1` inside a generator template and `Logger.info` on a `defp` Elixir does not permit to be called across modules. Tightening the gate proved necessary and not sufficient, so bare-name resolution is removed: **208 edges redirected, total unchanged at 11,222, 0 real losses.**
- **A Scala receiver's type survives a qualified or generic type node.** The analyzer read a declared type with a bare-`type_identifier` lookup, so `val b: java.io.File` and `val c: Buffer[String]` produced no hint at all — hinted share 9.03% → 23.43% (sbt) and 4.50% → 8.02% (lila), `calls_total` identical in both. A generic base Scala imports implicitly is deliberately refused: sampling found 44 of 185 newly-resolved edges (24%) binding a standard-library `Map.get` to the project's own, and the refusal costs nothing where the catalogue is reached.
- **Node's globals and promise APIs resolve.** `fs.promises.<fn>` reached none of the 31 catalogued `fs.promises` rows because the receiver was a member expression (**+303 edges**, `calls_total` identical in three repos), and `process` and `performance` were simply missing from the known-globals set, landing on `external` — **892 edges promoted, every one off `external`**, 278 of them onto an actual catalogue row.
- **C stamps the file's `#include` set on ambiguous bare calls**, so `send` / `recv` / `read` / `write` reach the catalogue for the first time in a language with no namespaces; it also moves 15 of 21 claim verdicts from `confirmed_with_caveats` to `inconclusive`, with none moving toward `confirmed`. **A Java wildcard import is a candidate package, not a blanket file-level prefix** — under `import java.io.*;` the implicitly-imported `System.currentTimeMillis()` had landed as `java.io.System` — and an unresolved Java call's name slot now names the callee rather than the receiver it was called on.
- **An import edge's dst is a canonical id.** Objective-C, CSS and Solidity emitted the bare import path, and a bare path is not an id, so the language slot carried a header path. **A generated file's edges keep the file's path**, where every generated file in a language had collapsed onto one pathless anchor, leaving a reader with no file to open.

#### The standard-input surface and deferred crossings (ADR-0049)

- **Server launch and connection setup stop minting taint sources.** 28 launch rows across Go, Python, Haskell and Erlang (`ListenAndServe`, `serve_forever`, `asyncio.start_server`, `flask.Flask.run`, `uvicorn.run`) and Go's eleven setup rows register or open a crossing and transfer nothing.
- **A handle wrapper's boundary follows its argument.** Go's `bufio.{NewScanner,NewReader}` had been `ipc_recv` unconditionally on a note reading "when wrapping os.Stdin", a condition holding at **zero of 83** resolvable sites; the analyzer now stamps `io_target_kind` from the argument. `io.WriteString` and `fmt.Fprint*` take any `io.Writer` and were rowed at one fixed boundary each — measurement 0012's entire vacuous class, with gocryptfs writing to a subprocess's `StdinPipe()` reported as a host-filesystem crossing.
- **Standard input is `ipc_recv` in every catalogue, and the call that transfers the bytes is rowed.** Java's `BufferedReader.readLine` and `Scanner.next*` shipped at a fixed `fs_read` — the Scanner row's note read, in full, "When reading from a file" — so the canonical `new BufferedReader(new InputStreamReader(System.in))` was a filesystem read. C++'s `std::getline` had no row and C's POSIX `getline` none either, so the idiomatic `std::getline(std::cin, s)` produced no source at all; Go's `fmt.Scan*`, Python's `builtins.input`, C's `getchar`/`scanf`, Haskell's `getContents`/`interact` and all ten `scala.io.StdIn` readers are corrected. **A launch that hands back the child's bytes mints a source in eight languages.**
- **Thirteen class-qualified receivers were unclassifiable, so all eighteen self-claims fell to `inconclusive`** — `compute_boundary_coverage` returns `qualifying_only = not unknown`, so one unclassifiable name withholds every qualified verdict at once. Adjudicating them exposed a real filesystem-escape hazard and correctly moved `install-gitleaks-no-host-fs` to `violated`.

#### Catalogue rows — direction, kind and reach

- **Python and JavaScript complete their catalogue legs.** Python rows the 41 stdlib module strings still reaching the coverage gate uncatalogued and adds 44 dated grants; JavaScript is audited and granted — ten node builtins become examined negatives, 76 rows are added, seven deleted, and the catalogue is promoted to `provenance_declared` from a state where zero `module_completeness` grants meant no node module could ever be an examined negative. **Python's `builtins` is enumerated**, since once bare builtins emitted edges the coverage gate correctly read an unenumerated module and sent all 18 self-claims to `inconclusive`.
- **Rows that manufactured taint out of operations reading nothing are removed.** 31 Haskell `IORef`/`STRef`/`MVar`/`STM` rows under `db_read`, `socket`/`bind`/`listen` in ten languages under `net_recv`, and the JPA/JDBC/`Ecto.Query` builder rows; `io_lib:format` is no longer a sink in Erlang and Elixir, since it returns an iolist and writes nothing. Adjacency is not a crossing — "typically written immediately" describes a different call one hop later — and a derived gate now fails on any row whose note rationalises a crossing that happens elsewhere.
- **Direction, kind and family corrections across all fifteen catalogues.** A C socket send or receive is classified by the descriptor's *family*, not by name, so tmux — which opens no `AF_INET` socket anywhere — no longer reports network egress over its `AF_UNIX` control channel. Erlang's `io:read` / `io:get_line` / `io:get_chars` were declared `logging`, an outbound sink, for functions that read input. Seven receiverless Rust callees were catalogued as methods, so every verdict on a repository that opens a file was withheld; Rust's standard library is audited and rowed, where `std::fs` had carried three associated functions and not one of its eleven stable methods. **A clock read is `host_info_read` in every one of the fifteen catalogues**, enumerated per language against named evidence. Duplicate YAML keys are now refused at load, where PyYAML had silently kept the last and `c.yaml` had lost a `boundary_ruling` rationale that way.

#### Taint propagation

- **The extractor recorded a statement and then accounted for nothing inside it, and no coverage predicate at any width could see that.** The coverage gate asks whether code sits *outside* every statement the CFG recorded; it is silent by construction when the statement *is* recorded and its def/use is empty. A four-language differential sweep with controls found seven such shapes across three languages. The remedy originally proposed — a per-statement "I understood this construct" flag — was refuted by that sweep and is not what shipped: in six of the seven shapes the handler *is* registered for the node type and is merely incomplete, so dispatch-table membership would have vouched for all six. What ships is `unaccounted_names`, which diffs each recorded statement's defines and uses against the semantic leaves in its own extent, reusing `_is_semantic_leaf` so "worth accounting for" keeps one spelling with the coverage gate. A leaf is attributed to *any* enclosing recorded statement, not the innermost. The signal is per *variable*, not per function, and the walk escapes on it rather than forfeiting, so a `True` is never downgraded. Floor-checked before it was wired (12.6% of functions on alertmanager, 4.9% on hypergumbo) and measured effect on production verdicts today is **zero, reported as zero**.
- **The walk's accounted-for exits are pinned as a census.** `_ddg_taint_reaches` continues without setting `escaped` in exactly five places — seen-set cycle guard, barrier line, `followed` with no call at the line, `followed` with a terminating callee, and `no_heir` with a terminating callee — now counted structurally against the shipped function's AST with controls that add a sixth, delete an `escaped = True` and rename the walk. Two of the five rest entirely on the summary catalogue, where `fmt.Fprintf` and `fmt.Fprintln` carried the dead-end marker while writing into their first argument, so the walk closed a step that was still open.
- **The §3a coverage gate asked only about call nodes**, so a def/use omission at a construct with no call in it licensed an unearned refutation; it now compares semantic leaves. A declared per-language list of binding node types was considered and rejected on a floor check. **A recorded heir vouched for an unrecorded one**, so a statement the extractor half-saw licensed a removal — 1,978 sites carried an unrecorded heir. **`coverage_granularity` was the constant `"language"`**, carrying its own re-evaluation trigger reading *"becomes `function` when the per-function coverage gate lands"*; the gate landed on 2026-08-26 and the constant did not move, so a clean verdict now says how much of the repository could not have refuted it.
- **Four ways the walk reached a wrong answer.** `subprocess.Popen(tainted)` verified clean while `subprocess.run(tainted)` verified violated, because PascalCase types the call as `instantiates` and `TAINT_CALL_EDGE_TYPES` did not contain it — one capital letter decided whether a command-injection flow was visible. A dispatch edge out of a *type* node is reachability, not dataflow, so the walk stopped consuming `function --instantiates--> Class --dispatches_to--> Class.save` as a value-carrying step (22 removed, of which 0 of 22 are correct; class-node hops 33 → 0). An I/O primitive's return no longer inherits its argument's taint, since taint flows through in-program computation and not through a resource the tainted value merely named. A sink can no longer be matched by bare name when the edge carries no module to check against — the same ungated escape measured 30 of 30 false elsewhere. A sink provably unable to receive the value no longer reports a flow, via a `call_arg_shape = 'literal_only'` stamp; 24 of measurement 0003's 34 false positives took no arguments at all.
- **`verify-claims` no longer crashes with `RecursionError` on deeply nested generated code**, where it exited 1 with empty stdout — the same code it returns for a violated claim. The walk is an explicit stack, byte-identical on every repository that did not crash. **Bash: what the shell itself contributes decides** — a redirect to `/dev/null` is no longer a filesystem write on either the boundary or the taint path.

#### verify-claims — coverage and disclosure

- **The coverage gate counts what it should.** A repository's own code is no longer reported as an unexamined third-party dependency (Rust `crate::`/`super::`/`self::`, JavaScript relative specifiers, and a repo's own `lib/utils` compared raw against a folded `lib.utils`); references are counted rather than parse candidates, where a scoped Rust path emitted once per nesting depth had contributed N−1 entries naming modules nothing called into; and a named global's rows are function-kind, so the gate and the classifier now agree about the same call.
- **A synthetic call site minted by a linker carries its host file's language**, where the solidity-abi linker stamped a literal `typescript` (the synthetic node's own `language` is `None`, ADR-0031 Class B) and so withheld every claim on a `.js` contract call as "typescript has no I/O catalog". **A withheld verdict names the blocker that actually withheld it** — two blockers were evaluated and the function returned on the first, so a repository with both was told only about launches.
- **Two disclosures named the wrong thing**, and arms 2 and 3 of `_module_matches` stop reading Go's naming convention into fifteen catalogues: arm 3 matched a catalogue *type* against a receiver *variable* after casefolding both sides, so a urfave/cli flag read classified as network egress.

#### Symbol kinds, ids and dead code

- **Twenty-six id-emitting sites produced ids hypergumbo's own validator calls malformed**, the dominant defect being a line number where the grammar wants a span; found by a static check over the producers rather than by a run. **`Edge.id` gets a format validator for its own grammar**, which is not `Symbol.id`'s — checked against a live survey first, where every one of apollo-server's 18,283 edge ids is `edge:sha256:<16hex>`.
- **Ruby object creation emits `instantiates`** like the eight other analyzers that emit it, so a `survey` filter on it no longer silently omits every Ruby object creation, and construction edges confer dead-code reachability. **Solidity contract members are `method`, not `function`**, making 2,025 members per repository visible to every consumer keyed on `kind == "method"`; eight sites had also answered "is this an inheritance edge?" their own way, and Ruby and Solidity lost dispatch to the disagreement.
...
- **The commit-message brand scrub rewrites a vendor-named trailer key and forces its value through the rewriter**, where it had passed one through verbatim on 13 commits; `Signed-off-by`, `Co-authored-by` and `Co-committed-by` are never renamed.
- **A transcript watcher can no longer outlive the session that launched it**, nor a killed watcher its `inotifywait`. Two orphans holding per-user inotify instances were found after a session crash, the older at 50 hours.

#### Performance

- **The symbol suffix index belongs to the registry, not the resolver: pretix's Python analysis goes 1700 s → 95 s, a 17.9× speedup with byte-identical output.** The index is whole-registry, and caching it on the resolver rebuilt it per resolver instance; per-file cost is now flat (0.77× over a 4× file-count range) where it had grown 5.70×, and pretix at 0.079 s/file sits below kserve's pre-fix 0.198.

#### CI, release and developer workflow

- **Pull requests that ran no tests now run the right ones.** A `conftest.py` change matched neither the changed-test-file nor the changed-source-file set, so smart-test reported "no test-relevant files changed" and wrote a manifest with **0 selected tests** — for the one file that governs a whole suite. A docs-only PR selected zero tests, so no doc gate ever ran on the change class it exists for, and a catalogue-only PR ran only the two tests it added. `smart-test --full` ran nothing and exited 0 whenever the diff held nothing test-relevant, which is exactly what verifying already-merged work looks like.
- **Git notes were never once pushed — 727 of them, since 2026-01-22.** `auto-pr` and `merge-pr` attach the original commit body as a git note whenever a rebase or squash merge rewrites the SHA, and the documented recovery command fetched nothing, because the DCO gate rejected the notes ref and the push was swallowed.
- **hypergumbo was broken on Python 3.10, its own declared minimum**, by one unguarded `import tomllib` — 2905 failed tests on the nightly py3.10 leg, unreadable behind the matrix's aggregate status until `ci-debug cron-status` surfaced it.
- **Forge scripts stop lying about the forge.** `merge-pr` printed a `codeberg.org` link on a GitHub remote, wrong in host and path; `pr_web_url()` now lives in the shared library and derives the non-GitHub host from `API_BASE`. **The bakeoff scripts stop looking for pool repos where they are not**, where cohort selection resolved a repo one level down in a collection subdir and the run step did not.

### Documentation

- **AGENTS.md documented a tracker invocation that exits 2, and the failure was unreadable exactly where it landed.** `--json` is registered on the top-level parser, so the trailing form is a usage error written to stderr with nothing on stdout — which a caller piping stdout into a JSON parser reads as a missing item rather than as a bad invocation. The always-loaded governance text, ADR-0013 in three places and two playbooks are corrected, and a test locks the contract: it scans per backtick code span rather than per line, because prose discussing the hazard legitimately names both orders in one sentence, and requires the working form to be shown positively, since a forbid-only guard is satisfiable by deleting the documentation. The guard immediately found a third occurrence two hand greps had missed.
- **[`docs/VERIFY-CLAIMS-SCOPE.md`](docs/VERIFY-CLAIMS-SCOPE.md) publishes what `verify-claims` cannot see**, linked from README and SECURITY.md: all eight caveat kinds, the exit-code contract in precedence order (`1` violated → `2` inconclusive → `3` caveated → `0` clean, with `2` not a pass), the four limits on a clean verdict, and what a `violated` verdict is worth.
- **The user-facing release notes were three major versions stale, because nothing promoted them.** `docs/RELEASE-NOTES-7.X.md` and `docs/RELEASE-NOTES-8.X.md` are written, each opening with a TL;DR naming the breaking changes; 6.1.0's stranded section is promoted; and `prepare-release` now promotes `## Unreleased` in the current line's notes file and refuses to release when it is missing.
- **ADR-0017's implementation-status markers are re-measured against the tree, five of them corrected**, distinguishing "not implemented" from "implemented but unwired" (§4a `infer_summary` and §7a `is_field_tainted` are written and tested with zero production callers); ADR-0049's "no catalogue row uses `net_listen`" is excised, eleven do. ADR-0046 through ADR-0051 and ADR-0053 are new this cycle, and ADR-0006's variable-type-inference support table gains measured Objective-C and Swift rows in place of *not assessed*, with its Java “Chained” column flipped.
- **Surveys and audits.** The stdlib `module_completeness` scope survey finds the audit necessary and not sufficient — tens of modules per language, only 2 of 42 surveyed repositories flippable by stdlib work alone. The I/O primitive kind-conformance sweep verifies Rust and Python across 947 method-kind entries and refuses 726 for want of a toolchain, unexamined rather than clean. Module docstrings are swept for the vocabulary retired by the ADR-0023 §6 and ADR-0027 folds, and for functionality ten of them had stopped describing.
