...
- **`hypergumbo run` no longer touches the network on cached embedding loads.** The embeddings model loader passed `local_files_only=True`, but HF Hub's metadata API, the xet cache-freshness ping, and the `transformers` safetensors auto-conversion *background thread* still issued outbound `huggingface.co` requests on every runtime invocation — violating the `runtime-cli-no-network` claim that the generated `SECURITY.md` advertises. `_load_st_model_offline_first` now sets `HF_HUB_OFFLINE=1` around the cached load (the only switch that hard-disables every HF Hub sub-API) and leaves it set after a successful cached load so `encode()`-time background threads stay offline too; the one-time first-install download restores the prior setting so it can still fetch. A new "Environment variables" section in `docs/hypergumbo-spec.md` documents `HF_HUB_OFFLINE`, `HYPERGUMBO_VERBOSE`, and `HYPERGUMBO_MIN_MEMORY_MB` (previously undocumented).
