...
- **Two Apple SDK macros break the Objective-C parse, and the grammar they break is dormant (WI-lafom).** tree-sitter-objc 3.0.2 does not preprocess, so a macro that expands to syntax is a syntax error to it and its error recovery falls back to C — silently, because `has_error` is read by nothing. 74 of 387 real ObjC files across four repositories parsed with an ERROR node, and `@property` never became a node in 14.7% / 26.3% / 11.8% of the files that declare one. The trigger was established BEFORE the fix, by parsing each suspected construct in isolation rather than by reading the source at the first ERROR node's line — that line is where error recovery re-parented TO, and reading it as the cause is what gave INV-bisok's residual half a root cause that had to be withdrawn. `nullable`, generics, block properties, `NS_DESIGNATED_INITIALIZER`, `NS_SWIFT_NAME` and a LONE `NS_ASSUME_NONNULL_BEGIN` all parse clean; what errors is `NS_ASSUME_NONNULL_BEGIN` in the WRAPPING position and `NS_ENUM` / `NS_OPTIONS` / `NS_CLOSED_ENUM`, which the item did not name at all. **The grammar-bump option is closed, not skipped**: 3.0.2 is the newest published, upstream's last substantive commit is 2024-12-16, and `tree-sitter-grammars/tree-sitter-objc#21` has been open since 2025-02-25 — so this takes INV-bisok's route, a byte-length-preserving pre-parse rewrite behind `TreeSitterAnalyzer.parse_source`, guarded twice (runs only on a file that already fails to parse; kept only when it strictly reduces ERROR nodes, so a macro name in a string, a comment or a `#define` is safe because the bytes are thrown away). The `NS_ENUM` rule consumes a leading `typedef`, because `typedef enum S {…};` has no declarator and yields a MISSING `type_identifier` — a new parse failure inside the fix for parse failures, which the first draft shipped and which counting MISSING separately from ERROR caught. Two further rules were measured and are deliberately absent: Apple's availability macros by name recover ZERO further files, and blanking any `SHOUTY(...)` before a `;` recovers two more but sheds 17 real `property_declaration` nodes. Measured by [measurement 0027](docs/measurements/0027-objc-macro-parse-recovery.md): ERROR files **74 → 39 (19.1% → 10.1%)**, `property_declaration` **436 → 486**, `class_interface` **322 → 346**, no count falling anywhere; through the analyzer, symbols **+109 / +59 / +160** and fmdb **+252** edges, with Mantle byte-identical exactly as pre-registered (its trigger is Quick's test macros, not Apple's). The one count that fell is a precision gain: AFNetworking's typed module slots **1189 → 1148**, all 41 re-keys of `AFURLSessionManager → external` — a first-party class leaving the slot because `_project_classes` contains it for the first time, at the count WI-lafom itself predicted. 85 of 85 newly typed fmdb slots read back correct. **What it does not close is stated on the item**: 10.1% of objc files still fail (`QuickSpecBegin`, a project-local availability macro, vendored C preprocessor bodies), and the disclosure half — `has_error` read by nothing — is untouched and cross-language.
