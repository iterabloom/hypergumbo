...
Seven framework-dispatch correctness fixes land. Six linkers (`airflow_framework_dispatch`, `django_orm_dispatch`, `jackson_dispatch`, `kafka_streams_dispatch`, `view_template`, `react_component`, `grpc`, `type_hierarchy`) extend their reasoning beyond direct base-class matches to transitive inheritance, skip-level overrides, and Django CBV View lifecycle methods; the `inheritance` linker tightens cross-language gating; `jackson_dispatch` recognizes JPA `@Entity` types as Spring REST response bodies. `bakeoff-deep` no longer counts synthetic dispatch edges in reverse-slice seed selection.

Internal: `docs/audits/` becomes a 12-entry per-cluster verdict series; the Fundamental Concept Audit playbook gains an indirection-aware producer-trace step after Wave 6 PR 4 caught three reclassifications a literal-grep blind spot had missed; a regression-guard property test now blocks DEPRECATE-NO-FOLD verdict drift at every commit.
...
- **Six linkers walk transitive base-class chains** (WI-halat / WI-vigih / WI-pogus): `airflow_framework_dispatch`, `django_orm_dispatch`, `jackson_dispatch`, `kafka_streams_dispatch`, `view_template`, `react_component`, and the Go ttrpc / CSI path in `grpc.py` now BFS over `extends`/`implements` edges to discover ancestors whose `meta.base_classes` names a framework base. Fixes the dominant real-world pattern where projects extend an in-tree intermediate (`AlloyDBWriteBaseOperator(BaseOperator)`, JPA `@Entity` extending `@MappedSuperclass`, Kafka Streams SAM wrappers, `LeafController(ApplicationController)`, project-internal React base components, ttrpc `UserHealth` → `BaseHealthImpl` → `HealthService`) rather than the framework class directly. New shared helper `linkers/_transitive_bases.py` (cycle-guarded BFS) is the single source of truth; `collect_transitive_base_names` accepts a `meta_keys` tuple so kafka_streams can fold `extends` and `implements` together. UAT v4.0.0 round 04 (airflow) and round 05 (pretix) had seen 0/9 and 0/6 transitive cases under the prior direct-only matcher. The `react_component` change also implements the base-class branch its docstring previously claimed (the code matched only on PascalCase). 20 new property tests + 4 helper-extension tests.
- **Django generic CBVs inherit View lifecycle methods** (WI-nipan / UAT DQ-02): `django_orm_dispatch.DJANGO_BASE_METHODS` entries for `ListView` / `DetailView` / `CreateView` / `UpdateView` / `DeleteView` / `TemplateView` now fold in `dispatch`, `setup`, `http_method_not_allowed`, `options`, the seven HTTP verbs, `head`, and `trace`. Django's class hierarchy is external to the project, so the WI-halat transitive walk had no in-tree edge to follow — a project class `class Foo(ListView)` previously matched only `ListView`'s frozenset and never reached `View`. UAT round 05 (pretix) had seen zero `dispatches_to` edges to any `*.dispatch` method graph-wide. New module-level `_VIEW_LIFECYCLE` constant is the single source of truth.
- **`type_hierarchy` linker emits skip-level overrides** (WI-firuj): the parent→children map is now closed transitively before edge emission. When `Grandparent.foo` is overridden only in `Grandchild` (intermediate `Parent` doesn't override), the edge `Grandparent.foo → Grandchild.foo` is emitted; previously `Grandchild` was missing from `parent_to_children[Grandparent]` because the map was one-hop. New `close_parent_to_children_transitively` BFS helper preserves diamond-no-double-emit and direct-override semantics. 6 closure-helper + 4 property tests.
- **Jackson dispatch linker recognizes JPA `@Entity` / `@MappedSuperclass` / `@Embeddable`** (WI-sokaz / UAT BUG-02): the prior matcher triggered only on Jackson, JAX-B, and Spring-binding annotations, missing the Spring Data JPA + Spring MVC pattern that Jackson-serializes JPA-mapped types as REST response bodies. On spring-petclinic, 6 `@Entity` classes (Owner, Pet, Visit, Vet, Specialty, PetType) had produced zero edges; bean-convention accessors now receive `dispatches_to` edges as expected.
