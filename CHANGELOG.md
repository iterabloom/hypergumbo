...
- **Protocol-call family Phase 3 fold (closes WI-vumum-juvil-fusiv-humoz-loluj-dasuk-vitod-zibug)**: per ADR-0023 §6, migrated the protocol-call subset (`http_calls` / `grpc_calls` / `graphql_calls`) to canonical `calls` + `meta["protocol"]`, mirroring the bridge / IPC family folds. Producer-side: `linkers/http.py` / `linkers/grpc.py` / `linkers/graphql.py` now emit `edge_type="calls"` with `meta={"protocol": "http"|"grpc"|"graphql", ...}`. Consumer-side: dropped `http_calls` from `compact.CROSS_CUTTING_EDGE_TYPES`, `grpc_calls` from `taint.TAINT_CALL_EDGE_TYPES` and `io_boundary._TRACEABLE_EDGE_TYPES`, and the dead `grpc_calls: 0.9` weight from `ranking.DEFAULT_EDGE_TYPE_WEIGHTS`; canonical `calls` (already in those sets) transparently picks up the folded edges. Registry: introduces `PROTOCOL_KINDS = frozenset({"ipc", "http", "grpc", "graphql"})` in `hypergumbo_core.edge_types` per ADR-0015's `access_mode` closed-enumeration shape (parallel to `BRIDGE_KINDS`). The three deprecated `endpoint_shape` registry entries are kept through Phase 4b' to allow downstream consumers to reach a clean state. Strict-mode drift linter now reports zero off-axis consumer references — this fold cleared the last load-bearing endpoint_shape consumer reference, removing the immediate-next blocker for the WI-mumok strict-mode pre-commit flip. Tagged `awaits_bakeoff_validation` per AGENTS.md §Bakeoff Validation Discipline.

- **ADR-0028 (Draft): Edge.evidence_type Names the Inference Pathway** (closes WI-pilit-tokuh-nabij-fonin-sodis-salut-pibop-dirog): instantiates ADR-0024's four-part axis-declaration template for `Edge.evidence_type` per the WI-turin-pajuk deep audit's DEPRECATE verdict (210 values across 4 clusters; all four leakage tests fire). Confirms ADR-0023 Open Question #2's prediction. Decision: the field names the inference pathway only; resolution status (Cluster B — 6 `*_unresolved` values) promotes to a new sibling field `Edge.is_resolved: bool = True` per ADR-0024 §"Fold-residue discipline" rule 3 (recurrence-promotion threshold met); framework-specific dispatch conventions (Cluster C — ~37 values across ~30 producer modules) fold to canonical inference label + `meta["framework_dispatch"]` / `meta["detection_pattern"]`; apex/peer call variants (Cluster D — `function_call` / `method_call` / `function_application` / `typed_receiver_call` / `ambiguous_method_call` etc.) collapse to canonical apex (`ast_call` or `function_call` — apex selection deferred to cluster-D audit-findings doc) plus `meta["call_construct"]`. High-level cluster taxonomy (210 values across 4 clusters) lives in §3 Detailed analysis; per-cluster verdicts file as audit-findings docs at `docs/audits/<NN>-<topic>.md`. Migration follows ADR-0023's four-phase shape with Phase 1 also landing the new sibling field, drift linter at `scripts/check-evidence-type-drift` reusing `axis_drift.find_drift` with `name_filter="EVIDENCE_TYPE"`. **Largest concept-axis migration on the project's roadmap** — ~140 production files have `evidence_type` literals, Phase 3 estimated 7–12 days plus per-cluster bakeoff validation; Phase 4 ships piecewise across 5–7 sub-ships gated on bakeoff validation cycle time, total wall-clock 6–10 weeks. Sibling axis to ADR-0027 (Symbol.kind); the two ADRs ship in parallel.

- **ADR-0027 (Draft): Symbol.kind Names the Source-Language Syntactic Construct** (closes WI-dahim-vupif-lavin-kukor-gufih-gutad-kanuv-johor): instantiates ADR-0024's four-part axis-declaration template for `Symbol.kind` per the WI-dumiz-bikul deep audit's DEPRECATE verdict. Decision: the field names the source-language syntactic construct only; framework-participation values (Cluster D — `event_publisher`, `graphql_resolver`, `*_caller` etc.) fold to `Cluster A canonical + meta["framework_role"]`; edge-label values (Cluster E — `call`, `import`, `inherit`, `reference`) reclassify as `call_site` or drop entirely (the relationship was already on the corresponding Edge). High-level cluster taxonomy (192 values across 8 clusters) lives in §3 Detailed analysis; per-cluster CANONICAL/FOLD/DEPRECATE-NO-FOLD verdicts file as audit-findings docs at `docs/audits/<NN>-<topic>.md` per the convention captured in the ADR's tracker discussion. Migration follows ADR-0023's four-phase shape (registry + drift linter at `scripts/check-symbol-kind-drift` reusing `axis_drift.find_drift` with `name_filter="SYMBOL_KIND"`; consumer migration to `(kind, edge_type, meta)` queries; producer migration per cluster lowest-risk-first; Phase 4 schema bump split into additive 4a + per-cluster hard-fail 4b shipped piecewise as bakeoff validation lands). Estimated effort 7–15 days plus per-cluster bakeoff validation. ADR sets up the migration; subsequent phase tracker items file once Phase 1 lands.

- **`docs/audits/` — new audit-findings document series** (closes WI-rodub-bupun-sukol-pobud-zojip-gajug-ruzop-mikol): introduced a sibling-of-`docs/adr/` directory for per-value verdict tables produced by applying existing methodology (typically the Fundamental Concept Audit) to a specific scope. Audit-findings docs record case rulings under existing law (an axis-declaration ADR), not new principles. Format: markdown narrative + a fenced YAML block tagged `kind: audit_verdicts` immediately following a `## Verdicts` heading (belt-and-suspenders sentinel). Each verdict row carries `value` / `verdict` (CANONICAL | FOLD | DEPRECATE-NO-FOLD) / `fold_target` / `status` (UNRESOLVED | PRELIM_RESOLVED | RESOLVED) / `diagnostic_test` ({cmd, expect}) / `rationale`. Format spec: `docs/audits/README.md`. Parser/validator: `hypergumbo_core.audit_findings`. Pre-commit gate: `scripts/check-audit-findings`. Property test: `packages/hypergumbo-core/tests/test_audit_findings.py` (live-tree pass + 44 unit tests, 100% coverage on `audit_findings.py`). The three-state lifecycle is explicitly orthogonal to bakeoff validation, which lives on the migration's tracker item via the `awaits_bakeoff_validation` tag.

### Changed

- **ADR-0025 reclassified as `docs/audits/0001-dispatch-publish-family.md`; ADR-0026 reclassified as `docs/audits/0002-ipc-family.md`**: both were per-value verdict tables under existing law (ADR-0023's axis principle + ADR-0024's template), not architecture decisions; they now live in the new audit-findings series. Permanent redirect stubs at the old paths preserve URL-level discoverability. ADR-0024 gained two new sub-sections: §"Family-audit verdict methodology" (lifts the CANONICAL/FOLD/DEPRECATE-NO-FOLD trichotomy as host-dataclass-agnostic methodology, with refactor trigger if future axis declarations need a different scheme) and §"Fold-residue discipline" (operationalizes the "promote a recurring meta key to a dedicated host-dataclass field" reservation as three rules: endpoint-property check, axis-leakage check, recurrence-promotion threshold N=3 distinct axis values OR N=2 producer modules). ADR-0024 status promoted from Draft to Accepted with a 2026-05-02 update footnote. The seven-step workflow's Step 3 / Step 7 outputs are redirected from "follow-on ADR" to "audit-findings document at `docs/audits/<NN>-<topic>.md`." Worked-example pointer extended to cite both relocated audits as the canonical Phase 3 / Phase 5 outputs of the methodology. ADR-0024 §3 open question 2 (pre-commit detection of new multi-value field additions) marked resolved-deferred via WI-busij-kadoz-bolok-lavig-lofaz-fogub-gajod-danif (P4). Inbound prose references across 21 production/test/analyzer files and 3 docs (ADR-0023, migrating-edge-types, blind-spots) rewritten from "ADR-0025/0026" to "audit-findings 0001/0002"; CHANGELOG historical entries left intact (they record what was actually committed at the time). `docs/concept-axes.md` regenerated.

- **`docs/adr/README.md` — bucket rubric for ADRs vs audit-findings vs surveys**: positive/expansive (defines valid shapes), not restrictive. Bucket 1 (ADR) for load-bearing decision documents — substantial Context is fine when it serves the decision; Bucket 2 (audit-findings) for per-value verdict tables under existing methodology; Bucket 3 (survey/snapshot) for pure inventories — the `docs/surveys/` directory is forward-declared but not created, the first survey to land materializes it. Bucket boundary: "decision present?" Bucket-1 self-justification guard: "If the deliverable is fundamentally a description with a 'we should...' paragraph at the end, that's bucket 3 with weak decision content, not bucket 1." ADR-0025 / ADR-0026 entries removed from the index; `Analysis pipeline` thematic grouping updated. AGENTS.md "Required Checks" section gains a one-paragraph essentialization pointing at the rubric. The Fundamental Concept Audit playbook's Step 6 ("Decide: deprecate, document, or keep") revised to specify the filing path for the Deprecate verdict — case rulings file as audit-findings, new principles file as ADRs, with worked-example links and a note that ADR-0025/0026 were originally filed as ADRs in error.

- **`agentic-session-retrospective.md` — corrected the trackerize-precision worked example**: the playbook's "What ARE valid concerns from the injection log" section had used `trackerize` as the canonical example of a selector precision miss ("trackerize selected when the user has not typed the trigger word"). Re-reading all 38 trackerize-injected events from one session showed ~7 events of literal trackerize work, ~10–13 events of tracker-filing-adjacent work (scope expansion, "Preparing Tracker Updates", decomposition decisions), and ~18 events of code/refactor/monitoring with likely item-filing at completion — so the selector was correctly recognizing that the trackerize playbook's *body* (steps 1–10: choose kind, assign priority, sequence with isbefore, apply tags, dedup-check, create with full bash example, summarize) is the canonical reference for filing *any* well-formed tracker item, not just for batch decomposition on the literal word. The misreading conflated trigger-condition (cheapest signal that a playbook applies, encoded in the registered hook summary) with applicability (where the playbook's body actually load-bears, which is what the selector judges). Fix: added a new "common misreading 4" using trackerize as the *cautionary* example and naming the bucket-counting prescription before flagging selection-precision; rewrote the "Selection precision" bullet in "valid concerns" to measure body applicability rather than trigger-word matching. Retroactively withdraws prior retrospective findings that flagged trackerize on the same false premise.

- **ADR-0023 §6 revised: protocol-call family (`http_calls` / `grpc_calls` / `graphql_calls`) named as the next Phase 3 subset; Phase 4 plan corrected to many-small-ships shape**: the original §6 plan assumed Phase 4b would land as a single big-bang removal of all endpoint_shape values. The post-shipping reality (WI-vomoj-suhaz at SCHEMA_VERSION 0.4.0) covered 33 of 58 endpoint_shape values — the cohort whose Phase 3 producer migrations had bakeoff-validated cleanly. The remaining 25 fall into two groups: (a) the protocol-call family (3 values: `http_calls`, `grpc_calls`, `graphql_calls`) whose fold pattern is uniform (`calls` + `meta["protocol"]`) and is filed as the natural-next Phase 3 subset; (b) a long-tail of ~22 individual values from the WI-tavas-voror sweep with heterogeneous fold targets, scheduled for smaller subsequent micro-phases. The §6 deprecation table gains a row for the protocol-call family; §6 Phase 3 producer-migration order names all five subsets explicitly (dst-kind / bridge / publish-dispatch / IPC / protocol-call) rather than the original three; §6 Phase 4b text replaces "endpoint_shape section becomes empty" with the actual shape ("first 4b ships the cohort that bakeoff-validates; subsequent micro-4b's ship the remaining subsets one family at a time"). The protocol-call family Phase 3 work is the immediate-next blocker for the strict-mode pre-commit flip (WI-mumok), since its three values are the only off-axis consumer references currently load-bearing in `compact.CROSS_CUTTING_EDGE_TYPES`, `taint.TAINT_CALL_EDGE_TYPES`, and `io_boundary._TRACEABLE_EDGE_TYPES`.

### Added

- **`docs/blind-spots.md` — generative-question catalog for the audit lifecycle** (closes WI-mivir-motom): the diagnostic complement to the Fundamental Concept Audit playbook. Catalogs recurring question-*shapes* that empirically go unasked in this codebase — Shape 1 (typing axis vs values), Shape 2 (assumed input boundaries), Shape 3 (silently-load-bearing failure modes), Shape 4 (null results read as confirmation) — each with example, trigger, and "how to add an entry" preamble. The Fundamental Concept Audit playbook's Cadence section now forward-references the doc as the source of suspect-domain candidates: cadence hook tells you *when* to audit, blind-spots.md tells you *what* to look at. Pruning rule: shapes get removed when consistently caught upstream by automation.

- **Generative-question bake-ins at post-incident and pre-refactor moments** (closes WI-juvaf-pubib): two of the three remaining inflection points (the per-release bake-in was dropped per the "audit triggers should couple to slack, not deadlines" rule). The agentic-session-retrospective playbook gains a 5-minute generative-question check between Phase 4 (Synthesize Findings) and Phase 5 (Record and Route) — incidents are direct evidence of unasked questions, so the broader lesson ("what other questions of this shape exist?") is now an explicit prompt rather than ad-hoc. The structural-fix-scope-expansion-protocol playbook gains a pre-refactor 5-minute blind-spots check before the new frame solidifies — refactors are one of the cheapest moments to catch frame-level bugs. Both bake-ins gracefully degrade when `docs/blind-spots.md` doesn't exist (bare "5 minutes to think" prompt). Domain-shift bake-in deferred as future work; threshold of "2 of 3" satisfied.

- **Drift-linter `--strict` mode** (closes WI-variv-lujug, design Option A + permissive pending): `hypergumbo_core.axis_drift.find_drift` and the Edge.edge_type wrapper `edge_types.find_axis_drift` gain an opt-in axis-principle check. The default check (subset-of-registry) is unchanged; passing `strict=True` (or `--strict` to `scripts/check-edge-type-drift`) additionally enforces that consumer-set values are on the relationship axis (or pending_classification — permissive by design, since pending values are real edges produced by GraphQL/OpenAPI/RPC analyzers awaiting per-family audit, and consumers like `io_boundary._TRACEABLE_EDGE_TYPES` legitimately reference `implements_rpc` today). When a per-family audit moves a pending value to canonical-relationship, the existing membership check catches any consumer that didn't follow the rename. Implementation is field-agnostic via `axis_drift.find_drift(allowed_axis_names=, name_to_axis=)` so future axis-bearing fields (Symbol.kind, evidence_type, etc.) inherit the pattern per ADR-0024's enforcement template. The repo currently has 3 known-pending off-axis consumer references (`http_calls` in `compact.CROSS_CUTTING_EDGE_TYPES`, `grpc_calls` in `taint.TAINT_CALL_EDGE_TYPES` and `io_boundary._TRACEABLE_EDGE_TYPES`) — these are load-bearing because their producers (`http.py` / `grpc.py` linkers) still emit those endpoint_shape values; pinned in `test_find_axis_drift_strict_known_off_axis_consumers` so a fourth accidental off-axis reference fails the test loudly. Flipping the pre-commit gate to `--strict` is governance work pending separate human approval.

### Changed

- **ADR-0023 §6 Phase 4b: deprecation removal — schema major-style minor bump** (closes WI-vomoj-suhaz): the actual hard-fail half of the dual-validity window announced in Phase 4a. 33 endpoint_shape values whose Phase-3 producer migrations bakeoff-validated cleanly (DEEP cohort `~/hypergumbo_lab_notebook/bakeoff_artifacts/deep-20260501-125217`, 8 repos, 0 no_move verdicts) are removed from `EDGE_TYPES`: the 6 dst-kind leakage cases (`imports_module`, `imports_component`, `model_reference`, `type_ref`, `query_references`, `renders_component`); the 7 bridge family values (`cgo_bridge`, `ffi_bridge`, `napi_bridge`, `wasm_bridge`, `wasm_load`, `bridge_invokes`, `native_bridge`); the 7 IPC family values (`ipc_calls`, `ipc_event`, `message_send`, `message_receive`, `websocket_message`, `websocket_connection`, `message_queue`); and the 13 dispatch/publish values (`routes_to`, `delegates_to`, `annotated_dispatches`, `uses_dispatch_table`, `di_registers`, `di_resolves`, `registers_routes`, `message_dispatch`, `crdt_publishes`, `annotated_publishes`, `emits`, `enqueues`, `event_subscribes`). The 25 endpoint_shape values added by the WI-tavas-voror sweep (`abi_call`, `association`, `base_image`, `build_tag_alternative_of`, `caller_invokes`, `contains_routes`, `crypto_flow`, `depends`, `extends_template`, `graphql_calls`, `grpc_calls`, `http_calls`, `includes_class`, `includes_template`, `invokes_callback`, `kernel_launch`, `links_to`, `notifies_resource`, `renders`, `requires_resource`, `script_src`, `signal_receiver`, `template_calls`, `uses_mixin`, `uses_vocabulary`) stay — their producers haven't been migrated yet. Consumer-side enumerations cleaned up: `IMPORT_EDGE_TYPES` shrinks to `{"imports"}`; `compact.CROSS_CUTTING_EDGE_TYPES` drops `routes_to` / `di_resolves`; `ranking.DEFAULT_EDGE_TYPE_WEIGHTS` drops 9 dead-but-harmless weights; `io_boundary._TRACEABLE_EDGE_TYPES` and `taint.TAINT_CALL_EDGE_TYPES` drop the bridge / IPC entries (post-Phase-3 these all fold to `calls`, which is already a member); `cli._REACHABILITY_EDGE_TYPES` drops `routes_to`; `scripts/bakeoff-deep::_CALL_FLOW_EDGE_TYPES` drops `routes_to` / `ipc_calls` / `wasm_bridge`. Test-fixture migrations: bridge-shape fixtures in `test_io_boundary.py` and `test_taint.py` now use canonical `calls` + `meta["bridge_kind"]` / `meta["protocol"]`; route-shape fixture in `test_cli_dead_code.py` uses `dispatches_to` + `meta["dispatch_kind"]="route"`; assertion-shape tests for `CROSS_CUTTING_EDGE_TYPES` / `DEFAULT_EDGE_TYPE_WEIGHTS` / `TAINT_CALL_EDGE_TYPES` / `endpoint_shape` registry inverted to assert the deprecated values are absent. `SCHEMA_VERSION` 0.3.1 → **0.4.0** marks the hard contract change. `docs/schema.json` regenerates without the 33 values in either the `Edge.type` enum or the `x-deprecated` extension list (the remaining 25 endpoint_shape candidates are still listed under `x-deprecated` until their producers migrate). `docs/concept-axes.md` regenerates: 24 relationship + 25 endpoint_shape + 4 pending = 53 values total (was 86). `docs/migrating-edge-types.md` annotated with a "Phase 4b shipped at SCHEMA_VERSION 0.4.0" note. 3573 tests pass; 100% coverage on all changed source files.

### Added

- **`docs/migrating-edge-types.md` — downstream consumer migration guide**: a single user-facing doc consolidating ADR-0023 §6, ADR-0025 §Migration impact, and ADR-0026 §Migration impact into one rename table, plus the meta-key vocabulary (`bridge_kind`, `channel_kind`, `mechanism`, `construct`, `dispatch_kind`, `protocol`), worked migration patterns ("find all bridge calls" / "find all pub-sub edges" / etc.), the deprecation timeline (deprecation announcement at 0.3.0, target removal at 0.4.0), and what's NOT migrated yet (4 pending_classification resolver/openapi/RPC values + 18 endpoint_shape candidates from the recent registry-completeness sweep). Targeted at consumers of the behavior-map JSON; doesn't lean on PR / tracker / commit references. `docs/concept-axes.md` preamble points at it.

- **Registry-completeness sweep — 25 new edge_type entries** (closes WI-tavas-voror): re-ran the WI-tavas-voror grep against current production code (post-Phase-3) and triaged the 30 emitted-but-unregistered values. Result: 5 noise (cfg.py's `_FringeEdge` dataclass uses an `edge_type=` kwarg whose values like `true`/`false`/`always`/`case`/`exception` are control-flow-graph labels, not canonical `Edge.edge_type` values), 7 missing relationship-axis canonicals (per ADR-0023's "Edge types that stay" list — `inherits`, `decorated_by`, `includes`, `defines_target`, `data_flows_to`, `module_exports`, `overrides` — producers were already emitting them, the registry was just incomplete), 18 endpoint_shape candidates (`abi_call`, `association`, `build_tag_alternative_of`, `caller_invokes`, `contains_routes`, `crypto_flow`, `depends`, `extends_template`, `includes_class`, `includes_template`, `invokes_callback`, `links_to`, `notifies_resource`, `renders`, `requires_resource`, `signal_receiver`, `template_calls`, `uses_mixin`, `uses_vocabulary` — each carries a plausible canonical fold target in its description, seeding a future per-language or per-pattern audit). `SCHEMA_VERSION` 0.3.0 → 0.3.1 (additive: 25 new enum values; the 5 noise values stay unregistered as documented). The drift property test and pre-commit linter now have full coverage of production emits.

### Changed

- **ADR-0023 §6 Phase 4a: deprecation annotations + schema minor bump** (progress on WI-vomoj-suhaz; Phase 4b gated on bakeoff validation): `docs/schema.json`'s `Edge.type` field gains a new `x-deprecated` extension key listing every endpoint_shape value (29 entries) — an additive schema annotation marking each value as a deprecation candidate scheduled for removal in a future `SCHEMA_VERSION` minor bump. Producers no longer emit any of these (Phase 3 / WI-mokam-jalig complete); the values stay valid in the enum for this dual-validity window so external consumers can adapt before the hard contract break. `SCHEMA_VERSION` 0.2.7 → **0.3.0** marks the deprecation announcement. Per-value migration guidance lives in the registry's `EdgeTypeSpec.description` (canonical fold target + meta key, citing the relevant ADR — 0023 / 0025 / 0026). Phase 4b (the actual removal + cleanup of dead-but-harmless consumer-side enumeration entries in `compact.CROSS_CUTTING_EDGE_TYPES`, `bakeoff-deep _CALL_FLOW_EDGE_TYPES`, `ranking.DEFAULT_EDGE_TYPE_WEIGHTS`, `io_boundary._TRACEABLE_EDGE_TYPES`, `taint.TAINT_CALL_EDGE_TYPES`) is gated on the `awaits_bakeoff_validation` tags on Phase 2 / 3 tracker items clearing — the dual-validity window is the safety net for any centrality / slice / sketch regression the migration introduced. **Intended removal target: SCHEMA_VERSION 0.4.0** (one minor version after this announcement).

- **ADR-0023 §6 Phase 3: publish/dispatch family producer migration** (closes WI-vasik-jofiv, tagged `awaits_bakeoff_validation`): 17 production emit sites across 12 linkers/analyzers folded per ADR-0025's rename table — `routes_to → dispatches_to + meta["dispatch_kind"]="route"` (grpc, route_handler ×2); `delegates_to → references + meta["mechanism"]="delegate"` (ruby ×3); `annotated_dispatches → dispatches_to + meta["mechanism"]="annotation"` (annotation_convention); `uses_dispatch_table → references + meta["construct"]="dispatch_table"` (c, cpp); `di_registers → references + meta["mechanism"]="di_registration"` (di_resolution); `di_resolves → dispatches_to + meta["mechanism"]="di"` (di_resolution); `registers_routes → references + meta["mechanism"]="route_registration"` (router_routes); `message_dispatch → event_publishes + meta["channel_kind"]="message_bus"` (cross-family fold per ADR-0025; message_dispatch); `crdt_publishes → event_publishes + meta["channel_kind"]="crdt"` (yjs_crdt); `annotated_publishes → event_publishes + meta["mechanism"]="annotation"` (annotation_convention); `emits → references + meta["construct"]="event_emit"` (solidity); `enqueues → event_publishes + meta["channel_kind"]="queue"` (ruby ×3, ActiveJob). The `event_subscribes` producer in `event_sourcing.py` is **dropped entirely** per ADR-0025's DEPRECATE-NO-FOLD verdict — the subscriber-is-enclosed-by-method information is recoverable from `Symbol.span` and the explicit edge was a denormalization with no downstream consumer that needed the materialized form (same verdict pattern as `message_receive` in ADR-0026). Consumer-side updates: `compact.CROSS_CUTTING_EDGE_TYPES` adds `calls` + `event_publishes` (both are now bridge/IPC/route/queue fold targets) — preserves cross-cutting endpoint seeding after the rename. `scripts/bakeoff-deep` (`_CALL_FLOW_EDGE_TYPES` and the routes_to filter), `scripts/bakeoff-broad` (route-edge detection), and `scripts/hypergumbo_diag.py` updated to query canonical `dispatches_to + meta["dispatch_kind"]=="route"` (with the deprecated `routes_to` alternation kept until Phase 4). Test files updated via `replace_all` on the relevant edge_type literals (12 linker test files); the `TestEventSubscriberToMethodEdges` class in `test_event_sourcing_linker.py` is rewritten to assert the producer-side decision sticks (no `event_subscribes` emits at all). 3512 tests passing, 100% coverage on all 13 changed source files. Tagged `awaits_bakeoff_validation` per the WI's acceptance — the centrality / slice / sketch implications of canonical fold should be confirmed by a DEEP cycle on dispatch-heavy / route-heavy / pub-sub-heavy repos.

- **ADR-0023 §6 Phase 3: IPC family producer migration** (closes WI-hahap-farid, tagged `awaits_bakeoff_validation`): expanded WI-hahap-farid's scope from the originally-named `ipc_calls` + `ipc_event` to all seven IPC-family values audited in ADR-0026, executed in a single PR. 9 production emit sites across 6 linkers (tauri_ipc, ipc, phoenix_ipc, websocket×2, message_queue) folded to canonical relationships per the ADR-0026 rename table — `ipc_calls → calls + meta["protocol"]="ipc"`; `ipc_event` / `message_send` (×2) / `websocket_message` / `message_queue` → `event_publishes` + `meta["channel_kind"]` in `{"ipc", "websocket", "queue"}`; `websocket_connection → references + meta["construct"]="websocket_endpoint"`. The `message_receive` producer emits in `ipc.py` and `phoenix_ipc.py` are **dropped entirely** per ADR-0026's DEPRECATE-NO-FOLD verdict — the forward `event_publishes` edges already capture the relationship and hypergumbo's slice handles reverse traversal natively. Consumer-side updates: `io_boundary._TRACEABLE_EDGE_TYPES` and `tag_io_boundaries`'s `call_types` default both add `event_publishes` so I/O-boundary tracing keeps crossing async-channel boundaries after the rename. Test files updated via `replace_all` on the relevant edge_type literals (5 linker test files); the `test_edge_preserves_dataflow_annotations_receive` test in `test_ipc_linker.py` is rewritten to assert the producer-side decision sticks (no `message_receive` emits at all). 3353 tests passing, 100% coverage on all 6 changed source files. Tagged `awaits_bakeoff_validation` per the WI's acceptance criteria — IPC/WebSocket/MQ-heavy repos (Tauri, Electron, Phoenix LiveView, RabbitMQ, Kafka) should see the centrality / slice / sketch implications of the canonical fold confirmed by a DEEP cycle.

- **ADR-0026 — IPC Family Classifications** (closes WI-fugun-butil): completes the per-family audit ADR-0023 §5 deferred for the IPC-shaped subset. Sibling to ADR-0025 (dispatch/publish); same four-leakage-test methodology applied to seven IPC-shaped values across Tauri, Electron, Phoenix Channels, WebSocket, and message-queue producers. Verdicts: zero CANONICAL (the family is entirely fold candidates; the canonicals it folds to — `calls`, `event_publishes`, `references` — already exist), six FOLD (`ipc_calls → calls + meta["protocol"]="ipc"`; `ipc_event` / `message_send` / `websocket_message` / `message_queue` → `event_publishes` + `meta["channel_kind"]` in `{"ipc", "websocket", "queue"}`; `websocket_connection → references + meta["construct"]="websocket_endpoint"`), one DEPRECATE-NO-FOLD (`message_receive` — same converse-direction shape problem as ADR-0025's `event_subscribes`; Phase 3 producer rewrite picks the canonical replacement, likely drop). Cross-ADR consolidation: `message_queue` and ADR-0025's `enqueues` fold to the same target (`event_publishes + channel_kind="queue"`). All seven values were already classified `endpoint_shape` per ADR-0023 §6's first cut; the audit ratifies the classification with concrete fold targets, so registry descriptions update but no axis reclassifications and no `SCHEMA_VERSION` bump. The IPC-subset Phase 3 producer migration is filed as `WI-hahap-farid` (gates on this ADR; isbefore Phase 4).

- **ADR-0023 §6 Phase 3: bridge/FFI subset migrated** (closes WI-mifor-vabul, tagged `awaits_bakeoff_validation`): 16 production emit sites and 8 test files updated to fold the bridge family edge_types into the canonical `calls` (or `imports` for `wasm_load`, since file→module isn't a call) plus `meta['bridge_kind']`. Producer renames: `cgo_bridge → calls + meta['bridge_kind']='cgo'` (cgo.py); `ffi_bridge → calls + meta['bridge_kind']='ffi'` (lua_ffi×2, pyffi×4, ruby_ffi×3); `napi_bridge → calls + meta['bridge_kind']='napi'` (napi.py); `wasm_bridge → calls + meta['bridge_kind']='wasm'` (wasm_bindgen.py:283); `wasm_load → imports` (wasm_bindgen.py:418 — the file→wasm_module edge is import-shaped, parallel to module imports; the dst's `kind=='wasm_module'` carries the WASM-specificity); `native_bridge → calls + meta['bridge_kind']='native'` (jni.py); `bridge_invokes → calls + meta['bridge_kind']='context_bridge'` (ipc.py — Electron contextBridge mediation, currently the only bridge-shaped emit not in the FFI/cross-language family). New `BRIDGE_KINDS = frozenset({"cgo", "ffi", "napi", "wasm", "native", "context_bridge"})` constant in `hypergumbo_core.edge_types` documents the closed enumeration. Consumer-side update: `scripts/bakeoff-deep::_FFI_EDGE_TYPES` switches from edge_type-set membership to a meta-based query (`edge_type == "calls" AND meta["bridge_kind"] in BRIDGE_KINDS`) — Phase-2-shape consumer pattern. `test_ipc_linker.py`'s 18 `bridge_invokes` filters updated to the same shape. The other Phase-3 follow-ups (IPC family migration `WI-fugun-butil`, publish/dispatch family migration `WI-vasik-jofiv`) remain open; this PR is progress toward `WI-mokam-jalig`'s "every endpoint_shape value has zero production emit sites" acceptance.

- **ADR-0023 §6 Phase 3: dst-kind leakage subset migrated** (progress on WI-mokam-jalig, tagged `awaits_bakeoff_validation`): 11 production emit sites and 14 test files updated to remove dst-kind-leaking edge_type names per ADR-0023 §6's "start with dst-kind leakage cases" ordering directive. Producer renames: `imports_module → imports` (js_module linker); `imports_component → imports` (vue_component linker, vue/svelte/astro analyzers); `model_reference → references` (orm linker); `type_ref → references` (js_ts analyzer); `query_references → references` (database_query linker); `renders_component → references + meta["construct"]="jsx"` (react_component linker — the only one carrying a `meta` payload, since the JSX render shape isn't fully captured by `dst.kind` alone). One consumer-side filter caught while migrating: `vue_component.py:158` was filtering by `edge_type == "imports_component"`; rewritten to `edge_type == "imports" and meta.get("import_path") is not None` so the linker still finds unresolved Vue imports after the analyzer rename. Test fixture helper `_edge` in `test_vue_component_linker.py` now mirrors the production analyzer's emit shape (raw path also stored in `meta["import_path"]`). Bridge/FFI, IPC, and publish/dispatch family producer migrations are tracked as follow-up items (each subset has its own complexity profile — bridge_kind enumeration design, IPC family audit dependency, ADR-0025 fold-target rename plus consumer cascade respectively); WI-mokam-jalig stays open until those land. The endpoint_shape values for the dst-kind subset remain in the canonical registry as deprecated (Phase 4 / WI-vomoj-suhaz removes them after a minor version of additive deprecation per ADR-0023 §6 Phase 4).

- **ADR-0025 — Dispatch and Publish Family Classifications** (closes WI-kofuh-fovar and WI-kusif-gamup): completes the per-family audit ADR-0023 §5 deferred for the dispatch and publish families. 13 family-adjacent edge types audited via the playbook's four leakage tests at per-pair level. Verdicts: 2 CANONICAL (`dispatches_to`, `event_publishes` — promoted from pending_classification → relationship axis), 10 FOLD (each rewritten in Phase 3 to its canonical fold target plus a meta key — `dispatch_kind`, `mechanism`, `channel_kind`, or `construct`), 1 DEPRECATE-NO-FOLD (`event_subscribes` — production emit shape is structural-containment under a name suggesting pub-sub; Phase 3 producer rewrite picks the canonical replacement). One cross-family error caught: `message_dispatch` is misnamed — its production emit shape (publisher→subscriber) puts it in the publish family despite the name. `SCHEMA_VERSION` 0.2.6 → 0.2.7 (additive: 11 new enum values + 4 axis reclassifications). `docs/schema.json` and `docs/concept-axes.md` regenerated.

- **ADR-0023 §6 Phase 2: consumer migration** (closes WI-sahab-fatoz, tagged `awaits_bakeoff_validation`): centralizes the import-edge predicate so consumers stop maintaining drift-prone hardcoded sets. New `hypergumbo_core.edge_types.IMPORT_EDGE_TYPES` includes `imports`, `imports_module`, and `imports_component` — the third entry was the silent bug from ADR-0023 §1 case 1, where Vue/Svelte/Astro/React component imports were miscategorized as generic edges in `ranking.py:1053`'s `_IMPORT_EDGE_TYPES = {"imports", "imports_module"}` and `slice.py:640`'s tuple of the same shape. Both call sites now reference `IMPORT_EDGE_TYPES`. Behavior change is observable only on framework-component-importing repos; tagging awaits a DEEP cycle to confirm the centrality and slice-exclusion deltas. Existing audited sets that were already canonical (`ranking._STRUCTURAL_EDGE_TYPES`, `slice._STRUCTURAL_EDGE_TYPES`, `io_boundary._TRACEABLE_EDGE_TYPES`, `taint.TAINT_CALL_EDGE_TYPES`) carry comments documenting the Phase-2 audit verdict (forward-compatible through Phase 3 because each already includes `calls`, so bridge-edge folding leaves the membership tests still firing). Phase 3 / 4 work tracked separately as WI-mokam-jalig / WI-vomoj-suhaz.

### Added

- **Edge-type runtime coherence checker** (`scripts/check-edge-type-runtime-coherence`, closes WI-funis-funam): the runtime half of ADR-0023 §3's two-layer enforcement — the static drift linter (PR #3460) catches consumer-side hardcoded set drift; this new checker catches producer-side endpoint-shape leakage by partitioning emitted edges in a behavior-map JSON by `(src.kind, src.language, dst.kind, dst.language)` and reporting partitions where `edge_type` varies. Logic in `hypergumbo_core.runtime_coherence` with allow-list at `docs/edge-type-runtime-allowlist.yaml` (initially empty per the warn-only Phase-1 deployment posture; ADR-0023 §3 mandates ADR amendments for allow-list growth). Tool exits 0 if clean (or all variance is allow-listed) and 1 otherwise. Initial offender count when run on hypergumbo's own behavior map: 14 partitions across 91,898 edges; most look like legitimate construct distinctions (e.g., `calls` vs `instantiates` between method→class) that will be allow-listed via amendment, while genuine endpoint-shape leakage will narrow as ADR-0023 §6 Phase 2/3 migrations land. Implementation note: the partition key is four fields rather than the six the ADR specifies — `framework` isn't a top-level `Symbol` field in hypergumbo today (it lives in `meta.concepts[*].framework`), and the coarser key catches every leak the finer key would.
...
- **Drift detector generalized to multi-field, scope broadened** (closes WI-pilam-jukus and WI-zisit-hagud, designed jointly per both items' "land as one PR" guidance): the AST walker that catches consumer-side hardcoded sets drifting from the canonical registry now lives in `hypergumbo_core.axis_drift` (parameterized on `name_filter` and `registry_names`). `hypergumbo_core.edge_types.find_axis_drift` becomes a thin Edge-type-specific wrapper preserving the existing single-argument signature, so the property test, the pre-commit linter (`scripts/check-edge-type-drift`), and any other caller need no changes. Default search scope expands from `packages/` only → `packages/`, `scripts/`, `.agent/` (overridable via `search_roots=`); default exclusion is `/tests/` (overridable via `excluded_path_substrings=`). ADR-0024 §Enforcement updated to show the one-call-site pattern future axis declarations follow. The broader scope surfaced one phantom-value bug fixed in this PR — `scripts/bakeoff-deep`'s `_FFI_EDGE_TYPES` listed `jni_bridge` and `pyffi_bridge`, both of which no analyzer ever emitted (the JNI linker emits `native_bridge`); same shape as ADR-0023 §1 cases 4–5. Reconciled by replacing the phantoms with `native_bridge`.

- **ADR-0023 status: Draft → Accepted (§6 migration in progress)**: the typing principle (`edge_type` names the relationship, not the endpoints) is in force as of the canonical-registry + drift-linter landing earlier in this Unreleased window. ADR text edited to (a) cite the landed Phase 1 commits; (b) cross-reference ADR-0024 as the abstracted template; (c) replace the old "Open questions (Draft status)" with per-question resolutions — three questions are settled (#3 schema-bump window, #5 stick-with-`meta`, #1/#4 in-flight as tracker items), two are explicitly deferred to follow-on ADRs (#2 evidence_type boundary, #6 Symbol.kind audit); (d) add tracker IDs for §6 Phase 2 (`WI-sahab-fatoz`), Phase 3 (`WI-mokam-jalig`), Phase 4 (`WI-vomoj-suhaz`), and the four enforcement follow-ons (`WI-funis-funam` runtime corpus test, `WI-zisit-hagud` scope expansion, `WI-pilam-jukus` multi-field generalization, `WI-variv-lujug` axis-principle tightening); (e) reframe the "Property test" section as three complementary defenses (static / runtime / cadence-hook) rather than two. `docs/adr/README.md` index updated.
