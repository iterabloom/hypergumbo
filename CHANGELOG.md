...
- **INV-dubam: `hypergumbo explain` provenance surfacing + PROV-vocabulary ADR (closes META).** `explain` now always shows `Origin:` with contributing passes, annotates callers/callees with edge type (e.g., `[imported_call]`), and a new `--provenance` flag shows per-edge `Derived from:` chains. New ADR-0030 maps hypergumbo's provenance fields to the W3C PROV-DM vocabulary. All 5 META closure criteria now met (Edge.derived_from, origin-as-list, Pass.depends_on CNF, explain surfacing, PROV ADR).

- **WI-hahor: Orphan-node triage + sentinel ratchet test.** Self-analysis audit of symbols with zero edges. Orphan rate dropped from 1783 (5.5%) to 664 (2.0%) since filing — class orphans 13→0, variable 1301→109, module 44→0. Remaining orphans categorized into 10 buckets with three actionable root causes (linker call_sites without edges, module-level constants with bare-name references, test functions using only builtins). New `test_orphan_node_audit.py` with 3 structural invariant tests plus 1 opt-in per-kind ratchet ceiling test.

- **INV-jidat: `Symbol.origin` and `Edge.origin` changed from `str` to `list[str]` — multi-source attribution.** Schema-breaking change (SCHEMA_VERSION 0.9.1 → 0.10.0). When multiple passes contribute to a Symbol or Edge, all contributing passes are now credited as an ordered list. Auto-normalization in `__post_init__` wraps scalar→list transparently for backward compatibility. `from_dict()` handles both legacy scalar and new list JSON. 20 tests in `test_origin_as_list.py`.

- **INV-rukor: `Edge.derived_from: list[str]` — linker derivation provenance.** Every Edge produced by a linker now records which Symbol IDs the linker consumed to construct it. Populated across all 55 linker modules (88 `Edge.create()` call sites). Defaults to `None` for analyzer-originated edges; omitted from JSON when null. An AST-scan test enforces the field on every linker's `Edge.create()` calls.

- **WI-jagus: Intra-file variable reference edges for Python module-level constants.** Functions that read module-level constants (e.g., `if TIMEOUT > 0:`, `return DELAY * 2`) now emit `references` edges with `evidence_type='ast_name_read'` at confidence 0.85. Previously these constants had zero in-degree. Shadow detection via `_collect_local_bindings` prevents false positives from parameters, assignments, for-loop targets, and local imports. Module-level reads also emit. Expected impact: orphan variable Symbols on self-analysis drop from ~1,301 to near zero. 12 tests in `test_py_variable_refs.py`.

- **WI-hupaz + WI-dilab (closes INV-hujog): `Pass.depends_on` in Conjunctive Normal Form + per-linker population across all 57 linkers.** New `Pass.depends_on: list[list[str]]` field declares analyzer prerequisites for every linker — outer-AND of inner-OR clauses (e.g., JNI requires "java AND (c OR cpp OR rust)"). Two validators: `validate_pass_name_resolution` (static spelling check, CI-time) and `validate_pass_dependencies` (runtime CNF satisfaction). All 57 linkers populated by ADR-0003-ext subcategory — Bridge (10, multi-clause), Framework (29, single-clause), Protocol (11, OR-clause), Infrastructure (6, may be empty). Property test enforces the non-empty invariant per subcategory. INV-hujog transitions violated → pending_validation (runtime gating not yet wired).

- **WI-titor (INV-dihos Phase 3, Rust): Rust analyzer joins the return-type registry chain.** Third phase of the ADR-0006 cross-language return-type-registry rollout. Pre-fix, Rust had zero local-variable type tracking — parameters, `let` bindings, and chained method-call returns all fell through to the short-name fallback where ubiquitous names like `send`/`parse`/`execute` were blocked. Three new helpers: return-type extraction (with `Result<T,E>`/`Option<T>`/`Box<T>` unwrapping), parameter-type extraction, and file-scoped `var_types` map from four sources (params, annotated lets, constructor inference, struct expressions) plus a new chained-call path via the cross-file method-return-type registry. New Strategy 1.8 resolves `receiver.method()` when receiver is type-bound, bypassing the generic-trait blocklist. Cross-file registry aggregated in `base.py` (step §4c, mirrors the existing field-type pattern). 28 new tests. C++ and Scala deferred to separate WIs.

- **WI-busij: CI lint enforcing axis declaration on every str-typed field of a core dataclass.** Closes ADR-0024 §"Open questions" Q2. New `multi_value_field_axis.py` AST-walks `@dataclass` classes in `ir.py` and `datamodels.py` and requires each `str`/`Optional[str]`/`Literal[str, ...]` field to carry a `# axis: <category>` comment. Four categories: `<known-axis-name>` (registry-backed), `identity`, `bounded-enum`, or `free-text — <justification>`. Five known axes: `edge-type`, `symbol-kind`, `evidence-type` (heavyweight) plus `language` and `pass-id` (catalog-derived). Retroactive annotations on ~40 fields. CI script + 36 tests including a live-tree gate that fails if a future PR adds an untagged field.


- **WI-hatip (PR-2 of INV-nilud): new `linkers/inherited_calls.py` + Ruby `include`/`extend` mixin visibility.** New Tier-2 Infrastructure linker walks ancestor chains to resolve unresolved `calls` edges carrying an `enclosing_class` hint. Per-language MRO via `_MRO_WALKERS` dispatch table; ships `_walk_insertion_order` for Ruby/Groovy (declaration-order BFS through extends/implements/includes, depth cap=10, cycle guard). Ruby analyzer now extracts `include`/`extend` modules and emits `includes` edges. Closes WI-puluf (Ruby Sidekiq workers / ActiveModel validations invisible to call graph). ADR-0029 documents the contract and 5-PR campaign. 31 new tests.
...
#### INV-nilud `inherited_calls` campaign PRs 1 + 3–5 (Java side)

Four PRs (WI-gifar → WI-dukog → WI-sivuk → WI-puvil) lift Java's inheritance-aware call resolution out of `java.py` into the centralized `linkers/inherited_calls.py` linker. The campaign's Ruby side (WI-hatip, PR-2) is listed separately in Added.

- **WI-gifar (PR-1): contract + shared-helper refactor.** `make_unresolved_edge` gains `enclosing_class`, `receiver_type_hint`, `inherited_field_receiver` kwargs. Five linker helpers promoted to public. Behaviour-preserving; 31 new tests.
- **WI-dukog (PR-3): Java Site-1 lifted.** New `_walk_single_then_interfaces` walker for Java MRO (extends → implements → includes priority). Java's inline 10-hop parent-chain walk replaced by a single `make_unresolved_edge` call; the linker resolves it. Forward improvement: interface default-methods now resolve (the old walk only chased `extends`). 8 new tests.
- **WI-sivuk (PR-4): Java hint threading + double-emit regression fix.** All three hint types thread through a single final unresolved emit. Fixed a PR-3 regression where bare/`this.method()` sites emitted two unresolved edges (one with hint, one without). 6 new tests.
- **WI-puvil (PR-5): Java Sites 2+3 lifted; META INV-nilud closes.** Two new resolvers: Site 2 (`receiver_type_hint` → direct lookup, MRO walk, or fallback to type) and Site 3 (`inherited_field_receiver` + `enclosing_class` → parent-chain field walk). Java analyzer populates `meta["fields"]` for class symbols; ~95 LOC deleted from the analyzer. Dispatch by hint specificity (Site 3 > Site 2 > Site 1). 13 new tests + 9 migrated to pipeline-mode. INV-nilud transitions to `pending_validation`.
...
- **INV-kotiz: `AnalysisRun.config_fingerprint` now consistently populated across all analysis passes.** Previously, 11 analyzers that construct `AnalysisRun(...)` directly (bypassing the `create()` factory) left `config_fingerprint` as empty string — observable as 3 empty-string runs (thrift, proto, starlark) in self-analysis output. Root cause: the dataclass default was `""` and only the factory method called `_default_config_fingerprint()`. Fix: `__post_init__` auto-defaults empty `config_fingerprint` to the sha256 hash, following the INV-jidat auto-normalization pattern. All current and future direct constructions get the correct default.


- **WI-pusad: `refine_frameworks` promote phase extends to bare-name patterns via a compound-import-required gate (Mechanism B).** Structural-fix sibling of WI-palol, closes the bare-name surface of INV-rojip (cohorts 2+3 of the closure spec: bakerydemo Django, chatwoot Rails with Gemfile declaration). New helper `_module_match_kind` returns `"exact"` / `"prefix"` / `None` instead of just bool; `_has_prod_import_match` gains `require_prefix_arm` kwarg. Bare-name patterns (`django`, `flask`, `rails`, `react`) promote only when at least one prod-non-test importer matched via the prefix arm — e.g., `from django.db import X` (module `django.db` matches pattern `django` via the dot-prefix arm). Bare exact imports alone (just `import graphql`) do NOT promote, preserving the WI-rofiz lesson on bare-name FPs. The previous `_module_matches` bool wrapper was removed; callers consult `_module_match_kind` directly. Cross-ecosystem FP guard added during bakeoff: a new per-module `module_languages` map and `allowed_langs` kwarg on `_has_prod_import_match` prevent Python's `http.client` stdlib import from spuriously promoting the Julia `http` framework (both have bare pattern `http`, both would otherwise match via the prefix arm). The gate is principled — it mirrors the demote phase's existing `fw_langs & import_edge_langs` short-circuit, applied per-module rather than per-framework. Materialize-route-symbols path gains comma-split for method lists: starlette's YAML pattern extracts `metadata.methods` which produces `"GET,POST"` for `Route('/x', h, methods=['GET', 'POST'])`; without per-method splitting in materialize, the WI-tizad dedupe (keyed on the joined string) would miss the analyzer-emitted per-method routes and duplicate. Surfaced by the smoke-test starlette case once WI-pusad's bare-name arm started loading starlette.yaml on `starlette.routing` compound imports without a manifest declaration. Bakeoff (7 cases): synthetic `react-exact` → no promote (WI-rofiz preserved); synthetic `react-compound` (`react/jsx-runtime`) → react promoted; synthetic `django-no-manifest` → django promoted; synthetic `flask-no-manifest` → flask promoted; real `flask-restful` → flask still detected (manifest-declared, regression check); real django source → django promoted (its own `from django.X import Y` internal imports drive promotion); real hypergumbo self-analysis → zero FPs (no frameworks promoted from stdlib/dependency imports). Thirteen new tests in test_profile.py pin `_module_match_kind` arms, the bare-name compound-promote path, the WI-rofiz preservation, test-only/explicit-mode/no-double-promote bypass, and the cross-ecosystem FP guard (one per direction: bare-pattern from foreign-lang prefix, specific-pattern from foreign-lang prefix). Two new tests in test_framework_patterns.py pin the materialize comma-split behavior (per-method emission and dedupe against analyzer-emitted per-method routes). 100% coverage on `profile.py` and `framework_patterns.py` preserved. Closes WI-pusad (P3, framework_detection + profile + structural + bare_name_promotion); INV-rojip transitions `violated` → `pending_validation` pending the BROAD cycle that confirms cohorts 1-4 hold across the prospector corpus.

- **WI-palol: `refine_frameworks` gains a promote phase — frameworks reached only by prod-non-test import edges (manifest silent) are now added to `profile.frameworks` when their import pattern is specific enough.** Structural-fix child of INV-rojip; closes the workspace-import surface of the gap that produced four targeted patches in five weeks across four ecosystems (WI-lohok Ruby autoload, WI-himas Python `-r` chains, WI-rofiz bare-`graphql` FP guard, WI-donud Apollo YAML relocation). New helper `_is_specific_pattern` defines the gate: a pattern is specific when it is scoped (`@apollo/server`), slash-compound (`github.com/gin-gonic/gin`), or dot-compound (`org.springframework.boot`). Bare single-token names (`react`, `flask`, `rails`, `tokio`) do not qualify — manifest detection remains the path for them, per the WI-rofiz lesson. New helper `_has_prod_import_match` collapses the existing demote-phase inner loop and the new promote-phase inner loop into one implementation. `_module_matches` extended to also recognize slash-prefix subpath imports (`@apollo/server/standalone` matches `@apollo/server`, `next/link` matches `next`) in addition to dot-prefix submodules (Python `django.db`) — necessary to handle the apollo-server smoke-test motivating case where the imported module is the scoped subpath. The promote phase runs before the existing demote phase; newly promoted frameworks still flow through demote so test-only-importer corrections continue to apply. The empty-`profile.frameworks` early-return is removed because an empty profile can now legitimately gain promoted frameworks from imports. Caller-supplied `profile.dev_frameworks` (currently always empty in real callers; `refine_frameworks` is the only producer) is preserved unchanged — promote and demote both honor pre-existing classifications. Eleven new tests pin the invariants: `_is_specific_pattern` for each of the three accept-branches (scoped, slash, dot) and the bare-name reject-branch; promote on workspace-imported scoped package (`@apollo/server/standalone` → apollo + graphql both promoted); promote skipped for test-only imports; promote skipped for bare-name `react`; promote on Go compound path (`github.com/gin-gonic/gin`); promote bypass in explicit mode; no double-promote when framework already in `profile.frameworks`; no re-promote when framework already in `profile.dev_frameworks`. INV-rojip's bakeoff closure criterion specifies four cohorts (apollo-server/smoke-test, bakerydemo, chatwoot, Node monorepo); WI-palol fixes cohorts 1 + 4 (workspace-scoped imports), defers cohorts 2 + 3 (bare-name Python/Ruby) to a future WI under INV-rojip. Closes WI-palol (P3, framework_detection + profile + structural); INV-rojip remains `violated` pending the bare-name extension.

- **WI-donud: Apollo HTTP-entrypoint route patterns (`startStandaloneServer`, `runHttpQuery`, `executeHTTPGraphQLRequest`) relocated from `graphql.yaml` (framework-gated) to `node-http.yaml` (always-loaded).** Closes the WI-tisam end-to-end validation gap on apollo-server's `smoke-test/` consumer fixture: `hypergumbo routes` returned "No API routes found" because the local `package.json` declares only `graphql` + `make-fetch-happen` (Apollo is imported via the workspace), so framework detection produced `frameworks=[]`, `graphql.yaml` never loaded, and the `startStandaloneServer` UC emitted by `_extract_http_handler_contexts` (shipped by INV-rolul) had no pattern to match. The three name regexes are high-specificity (no realistic collisions in non-Apollo codebases), so always-loading them is safe; the comment in `framework_patterns.py:1342` already documented this as the intended home. Verification on the apollo-server fixture: smoke-test `hypergumbo routes` count 0 → 1 (`nodenext/src/smoke-test.ts`); parent apollo-server route count unchanged at 6 (node-http.yaml is always loaded, so detection result is identical when graphql IS detected too). New regression test `test_apollo_patterns_fire_without_graphql_framework` exercises `enrich_symbols` with `detected_frameworks=set()` to lock in the closure criterion; two existing pattern-load tests (`test_apollo_startStandaloneServer_match`, `test_apollo_runHttpQuery_match`) updated to load `node-http` instead of `graphql`. Closes WI-donud (P3, analysis_quality).
...
- **WI-nokoh: SCIP generalization vision sketch.** New `docs/future/scip-generalization-vision.md` documents the design direction for extending the SCIP backend beyond Rust (potential Go, TypeScript, Java coverage via language-server SCIP export).

- **WI-luvig: `Symbol.canonical_name` scope clarified.** Docstring in `ir.py` and `containment.py` now states that `canonical_name` is populated only by niche analyzers (Verilog, VHDL) for language-level name canonicalization, not by mainstream analyzers.
