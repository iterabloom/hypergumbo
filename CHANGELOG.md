...
- **Python variable- and field-kind symbols now carry a structural `shape_id` (WI-luzut).** `_compute_shape_id` only handled body-bearing `FunctionDef`/`ClassDef` nodes, so module-level variables and class attributes emitted `shape_id=None`. They now hash the assignment statement's AST skeleton (identifiers/literals stripped), so `X = 5` / `X = foo(a)` / `X: int = compute()` get distinct shape_ids, with the symbol kind folded into the prefix (WI-linon discipline) — making them visible to the `repeat-finder` clone detector once its kind filter is widened (WI-vogij). Additive: no existing shape_id value changes, so `shape_id_scheme` is unchanged (WI-rihob precedent for null→value gap-fills).
...
- **Five config-family analyzers emit canonical `stable_id`s (WI-banod, first batch).** `ini` (section/setting), `requirements` (requirement), and `properties` (property) set `stable_id` to the non-canonical composite `Symbol.id`; they now use the shared `make_doc_stable_id` factory (canonical `sha256:<16hex>`), matching the doc/markup analyzers already migrated. `node.id` is unchanged (still the location key); the span fold additionally distinguishes same-named siblings the line-less composite could collide. These three are default-output-neutral (noise-kind symbols); the `bitbake`/`bibtex` remainder is deferred as observable default-output identity churn pending a coordinated scheme decision.
...
- **`analysis_runs[]` is serialized in a deterministic order (WI-haguz).** The array was written in accidental pass-completion order (out-of-order on ~25% of transitions); it is now sorted by `started_at` (pass-id tiebreak), a deterministic ordering contract consumers can rely on.
...
- **AnalysisRun `repo_fingerprint` field carries the `sha256:` scheme prefix (WI-bosog; `repo_fingerprint_scheme` v1 → v2).** Within one analysis-run record the three sha256-derived identity fields rendered inconsistently: `run_signature` / `config_fingerprint` used `sha256:<16hex>` while `repo_fingerprint` was bare 64-hex, so a consumer could not strip a scheme prefix uniformly across them. The AR-record field is now `sha256:<64hex>` — prefix added, full digest retained (a content hash, unlike the derived 16-hex signatures), matching the `pass_version` convention. The prefix is applied only at the field-rendering site (`compute_repo_fingerprint_field`, stamped in `finalize`); the bare digest returned by `compute_repo_fingerprint` is unchanged where the value doubles as the colon-free analysis-cache path segment.
...
- **Documented the `is_test_file` concept boundary (WI-popok fundamental-concept-audit KEEP verdict).** Two predicates share the name: the NARROW supply-chain role flag `Symbol.is_test_file` ("is this test *code*?", spec §14) vs the BROAD `paths.is_test_file` ranking/scan heuristic (also covers mocks/fixtures/testdata/benches/`spec_*`). WI-popok proposed folding the entrypoint detector onto the first; the audit found them distinct-purpose (all four leakage tests pass → KEEP) — the broad predicate is not a coarser re-derivation, so a swap would flood the entry list with mock/fixture/benchmark entrypoints. Documented both boundary docstrings + the entrypoint penalty site, disambiguated two self-styled "canonical" functions, annotated ADR-0041 §2, and added a regression-guard test that doubles as the executable re-evaluation trigger. Advances the INV-numat drain, removes a false META-bifif member. No behavior change.
- **Documented the `evidence_type` ⊥ `is_resolved` boundary + added the META-bifif property-test template (WI-molit KEEP verdict).** WI-molit claimed the `ast_call` (98.5% unresolved) vs `ast_call_direct` (8.3%) split redundantly encodes `is_resolved`. A 3-lens audit (governance + spec + consumer census) returned **KEEP**: `evidence_type` names the inference *pathway* (ADR-0028), resolution lives solely on `Edge.is_resolved` (ADR-0037), the two are distinct call-construct pathways appearing on *both* resolution states, and no consumer reads the label as a resolution proxy (the 0.40/0.50 confidence delta is a genuine pathway signal). Added two "no-consumer-re-derives" property tests in `test_evidence_types.py`. WI-molit → `wont_do`; INV-numat drain −1; a second false META-bifif member removed. No production-code change. Full audit: `~/hypergumbo_lab_notebook/concept-audit-evidence_type_is_resolved_07172026.md`.
...
- **Spec contract-accuracy pass.** Documented the `metrics.languages` attribution rule, five relocated `supply_chain` booleans, the `feature`/`query` record shape (dropping a phantom `summary`), and a 47-key `Node.meta` catalog (Wave-4); added the missing §9 prose for `limits.max_tier_applied` (WI-sumur); and documented why `profile.languages` and `metrics.languages` legitimately differ in their language key-sets (WI-ziror).

### Fixed

#### `verify-claims` violated-flow evidence is now drillable (WI-kikis)

- **A violated taint-flow claim now surfaces per-flow drill-down evidence instead of collapsing thousands of flows to five indistinguishable rows.** The `details` string previously rendered each violation as a bare `<source_primitive> -> <sink_primitive>` pair and showed the first five — which, at high counts (3,969 flows on the self-corpus), were frequently *verbatim duplicates* (many distinct source/sink **symbols** share a primitive **name**), so a consumer saw the same string five times with no source, line, edge, or path and no way to locate any single flow (99.87% of evidence invisible). Now the verdict (a) **deduplicates** on the full per-flow identity (source/sink symbol + primitive + call-graph path) so identical findings collapse and shown rows are distinct; (b) **renders each shown row with its symbol IDs** (`cmd_run [<sym>] -> replace [<sym>]`) plus a `via N hop(s)` path indicator, so distinct flows are visibly distinct and drillable; (c) discloses the true total-vs-distinct count (`N flow(s) (D distinct)`); and (d) attaches a bounded, deduplicated structured **`evidence`** array (`source_symbol` / `sink_symbol` / primitives / `path`, capped at 100 distinct flows) to the `--format json` envelope so a consumer can triage true from false positives programmatically even at high evidence counts. The `verify-claims` view version bumps `1.0` → `1.1` (additive field).

#### Kind-conditioned writer-contract population contract (WI-libib / WI-hudug)

- **The writer-contract validator gains a per-`(language, kind, field)` population contract, catching regressions the record-class-level checks structurally cannot see.** The existing sub-patterns partition only by `(record_class, field)`, so a field populated on one Symbol *kind* masks a 100%-NULL partition on another kind — `qualified_name` on `function` symbols keeps the class-level partition non-empty even if every `method` symbol has lost it. The new `_check_kind_conditioned_population` check partitions symbols by `(language, kind)` and flags a **registered** cell that is 100% NULL across its non-empty kind partition, emitting one umbrella `writer_contract` warning (an absent partition is skipped — absence is not a regression; a single populated symbol satisfies the contract). Rather than enforce the full 312-cell `(kind, field)` matrix (most NULL cells are legitimately NULL — a `variable` has no meaningful `qualified_name`), the contract is registered cell by cell as a sound-by-construction population ratchet (WI-libib's reframe). The inaugural entries — `qualified_name` on Python function/method/class (WI-hudug; producer done by WI-fagab / ADR-0032) — emit **zero** violations on current output and stand as a regression guard: a future producer that drops the field trips them, and the resulting `writer_contract|warning` cell (unbaselined → ceiling 0) breaks the `validation_ratchet` self-tree gate. Subsequent per-kind population contracts register against the same table.
...
- **The opt-in rust-analyzer SCIP backend maps edge endpoints to hypergumbo symbol ids.** It previously translated edges without a resolve map, so every SCIP edge was dangling and the endpoint-integrity finalize silently dropped the entire SCIP call graph (2978 → 0 edges on zoxide); endpoints now map to real symbol ids (survey output 0 → 739 edges for backend users).
...
- **Degenerate route-marker twins are excluded from `features[]` (WI-rijop).** `features[]` was double-populated with content-free route-marker twins (`node_count=1, edge_count=0` — 16 entries for 8 routes on self-analysis); markers are now skipped from the `features[]` append (the per-slice file/index is still written).
...
- **`config_info` name/version/dependency extraction scans monorepo `packages/` manifests (WI-lirub).** The heuristic extractor only scanned flat root/subdir manifests, so a monorepo's sub-package `pyproject.toml` identity (name/version/dependencies) never surfaced; it now walks the `packages/` layout (8 sub-package manifests now appear on the self-repo).
...
- **smart-test honors coverage-only test dependencies (WI-zaziz).** smart-test selects tests via `hypergumbo slice --files` — an *import*-dependency reverse slice — so it is blind to a test that *covers* a source file through the analysis pipeline without *importing* it. The concrete case: a PR touching `hypergumbo_lang_mainstream/java.py` selected the java-specific tests but not `test_polyglot_call_site_coverage.py`, the sole cover of java.py's static-import `ExternalRef` branch (`java.py:1872-1878`), so the changed-file 100% coverage gate false-red-ed at 99% even though the full suite was 100% — and the manual "add the test to the manifest" workaround did not survive `auto-pr`'s commit-diff-driven manifest regeneration. A checked-in declarative map (`.ci/coverage-only-deps.txt`) of `source → covering-test` pairs, consumed by a new `scripts/coverage_only_deps.py` helper, is now folded into the selection *inside* smart-test (mirroring the per-package fallback layer), so the covering tests are always included — in both the run and the regenerated manifest. Entries are validated (every source/test must exist on disk) and defensively skipped if a mapped test is missing, so a stale entry can neither silently disable the augmentation nor inject a phantom test.
