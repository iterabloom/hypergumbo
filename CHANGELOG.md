...
- **README index sync regression guard property test**: `audit_findings.find_readme_index_drift(repo_root, findings_list)` parses the audit-findings index table at `docs/audits/README.md` and verifies, for every audit-findings doc, that the index's Status column agrees with the doc's verdict YAML row counts. Two comparison shapes supported per the existing README phrasing conventions: (1) cells with explicit numeric counts (`Mixed (6 RESOLVED, 11 PRELIM_RESOLVED)`, `All rows resolved (57 RESOLVED, 3 PRELIM_RESOLVED)`) must match YAML counts exactly; (2) cells without numeric counts (`All RESOLVED`, `All PRELIM_RESOLVED`, `All RESOLVED at relocation (2026-05-02)`) require the YAML to have all rows in a single status, and the cell to contain `All <STATUS>` matching it. Closes the README-index-drift gap called for in `~/hypergumbo_lab_notebook/notebookjournal_05072026_0316.md` §"High-value (file as tracker items) #4". Wave 6 PR 5 demonstrated the recurrence pattern (four index rows fell behind their per-doc Status fields after Wave 5 + Wave 6 advances landed and were resynced by hand in PR #185); this test blocks the next recurrence at every-commit time. 8 new unit tests at `tests/test_audit_findings.py::test_readme_drift_*` exercise the clean paths (explicit-counts match, All-marker match, decorative-prose-around-counts), the catch paths (count mismatch, missing-marker, mixed-YAML-with-bare-marker), the orphan-doc path, and the live-tree assertion `test_live_tree_readme_index_in_sync_with_audit_docs`.
- **DEPRECATE-NO-FOLD-zero-producer regression guard infrastructure (WI-viluk)**: `audit_findings.find_zero_producer_violations(repo_root, findings_list)` enumerates DEPRECATE-NO-FOLD verdict rows across the three registered axes (`Symbol.kind`, `Edge.edge_type`, `Edge.evidence_type`) and returns one violation entry per value with at least one producer emit site found via literal kwarg or assignment-form-to-Name. Companion enumerators in `producer_coherence.py`: `find_emitted_literal_values(...)` plus the per-axis wrappers `find_emitted_symbol_kinds` / `find_emitted_evidence_types` / `find_emitted_edge_types` reuse the existing WI-nubuv ext A function-local + module-level Name resolver to surface every static literal emitted at producer call sites. Closes the regression-class gap demonstrated by Wave 6 PR 4 — where the original audit's literal-substring grep returned zero hits, but real producers existed via indirection patterns the grep missed; three DEPRECATE-NO-FOLD verdicts shipped wrong before the re-trace caught them. **Coverage gap, documented:** helper-call indirection (e.g. `add_symbol(node, name, "<value>")`), f-string interpolation, and dict-subscript-target assignment are NOT enumerated; the Fundamental Concept Audit playbook §"Step 4.5" requires manual greps for those at audit-write time, with WI-nubuv ext B / ext C as the planned structural backstops. 7 new unit tests (`tests/test_audit_findings.py::test_zero_producer_violations_*`) exercise the clean path (no emits → no violations), the literal-kwarg-emit catch path, the assignment-form-to-Name catch path, the non-DEPRECATE-NO-FOLD ignore path, the unknown-axis silent-skip, the empty-findings short-circuit, and the cross-axis-disambiguation false-positive control. The live-tree assertion (strict `len(errors) == 0`) is **deferred to the verdict-correctness re-audit PR** that absorbs whatever leak set the function surfaces — splitting prevents the test infrastructure from being held hostage to the (separate) re-audit deliverable. The minimal smoke check `test_live_tree_deprecate_no_fold_function_runs_without_crashing` ships in this PR to confirm the function executes end-to-end against the real registry. Mentioned in playbook §"Step 4.5 Forward-looking enforcement" as the regression trace planned for every-commit gating once the re-audit PR lands.
...
- **`Symbol.kind` Cluster E `import` and `reference` reclassification + DEPRECATE-NO-FOLD verdict-correctness re-audit (audit-findings 0010)**: Closes the verdict-correctness re-audit called for in `~/hypergumbo_lab_notebook/notebookjournal_05072026_0316.md` §"High-value (file as tracker items) #1". The Fundamental Concept Audit playbook §"Step 4.5" indirection-aware producer trace was systematically run against all 19 DEPRECATE-NO-FOLD-flagged values in audit-findings 0005 / 0006 / 0007 / 0010 / 0012 — `build-dependency`, `call`, `component_ref`, `config`, `dev-dependency`, `event_subscribes`, `extends`, `heading`, `import`, `include`, `inherit`, `message_receive`, `model`, `read`, `reference`, `tsconfig`, `unresolved`, `work_item`, `write`. Two leaks surfaced and reclassified: (1) **`reference`** at `swift_objc.py:167` — Wave 5's framework-role fold made `selector_ref → kind="reference" + meta["framework_role"]="selector_ref"` per audit-findings 0011's `_ref` shape disposition, adding a real producer that the original literal-grep at the time of audit-findings 0010 did not surface. (2) **`import`** at `wasm_bindgen.py:266` — Wave 6 PR 3's wasm_import fold (`kind="import" + meta["compilation_target"]="wasm"`) added a producer for a different purpose than the originally-inventoried css/jsonnet/astro/r_lang producers (the slicer BFS needs the synthetic boundary node for continuity). Both rows reclassified DEPRECATE-NO-FOLD → CANONICAL with status RESOLVED in audit-findings 0010; both registry entries move from `endpoint_shape` to `language_construct` in `symbol_kinds.py` with rewritten descriptions. The other 17 values were verified clean: ID-construction tail tokens (`model`, `tsconfig`, `unresolved` in dst-id format strings preserve stable-id contracts after fold; the Symbol emission uses the fold target), linker-internal dataclass fields (`read`/`write` in `dataflow.py` / `crypto_flow.py` / `yjs_crdt.py` are `CryptoSite`/`YjsSite`/`DispatchSite` `kind` fields, not `Symbol.kind`), tracker `Item.kind` (`work_item` at `screenshot_save.py` and `migration.py` is `ParsedItem.kind` and `ts.add()` API, not `Symbol.kind`), Edge dst-id format-string components, and prose strings. Audit-findings 0010 §"Re-audit (2026-05-07) — Indirection-aware producer trace" records the methodology, the two reclassifications, and the false-positive analysis. Index in `docs/audits/README.md` updated to "Mixed (2 RESOLVED, 10 PRELIM_RESOLVED)" for 0010. The WI-viluk regression-guard property test `tests/test_audit_findings.py::test_live_tree_deprecate_no_fold_has_zero_producers` is promoted from smoke check to strict assertion now that the two surfaced leaks are absorbed; literal-kwarg + assignment-form-to-Name regressions are now blocked at every-commit time. The remaining three Step 4.5 shapes (helper-call indirection, f-string interpolation, dict-subscript-target) require manual grep per the playbook until WI-nubuv ext B / ext C land. `SCHEMA_VERSION` patch-bumps `0.5.7` → `0.5.8` (2 `x-axis-of-values` axis reclassifications in `docs/schema.json`).
- **Fundamental Concept Audit playbook gains Step 4.5 — Indirection-aware producer trace**: The audit-methodology playbook (`.agent/agent_playbooks_protocols_sops_skills/what-if-we-dont-know-what-the-fuck-we-are-talking-about-audit-aka-fundamental-concept-audit.md`) is amended to require checking five producer-emit shapes — literal kwarg, helper-call positional/kwarg, assignment-form-to-Name, f-string interpolation, dict-subscript-target — for every value with a verdict that asserts producer existence or non-existence (especially DEPRECATE-NO-FOLD verdicts claiming "no producer"). The amendment is the methodology hardening called for in `~/hypergumbo_lab_notebook/notebookjournal_05072026_0316.md` §"High-value (file as tracker items) #2". Trigger: Wave 6 PR 4 reclassified `theorem`/`inductive`/`message` from DEPRECATE-NO-FOLD to CANONICAL after a re-trace surfaced producers via `add_symbol(...)` / `_make_proto_symbol(...)` indirection at `lean.py`/`tlaplus.py`/`proto.py` — three of seven verdicts in that cluster were wrong because the original audit relied on literal-substring grep alone. Companion: a self-test bullet in §"Self-test before stopping" makes the indirection trace mandatory at audit-write time; an Examples entry records the methodology hardening event. The forward-looking regression guard (executing each DEPRECATE-NO-FOLD verdict's `diagnostic_test.cmd` at CI time) is filed as a separate tracker work item to ship in its own PR.
- **`Symbol.kind` Cluster H boundary-pseudo-symbol resolution + `Symbol.is_resolved` ADR withdrawal (ADR-0027 §"Phase 3", WI-runod Wave 6 PR 6)**: Closes the last 2 rows in audit-findings 0007 — Cluster H is now fully resolved. The original audit deferred `external_symbol` and `unresolved` pending a hypothetical `Symbol.is_resolved` sibling field ADR mirroring ADR-0028's `Edge.is_resolved`. A re-trace at Wave 6 PR 6 produced a different verdict: (1) `Symbol(kind="unresolved")` has no producer — the string `"unresolved"` appears only as a trailing token in dangling-edge dst IDs created by `analyze/base.py:make_unresolved_call_edge` (format: `{lang}:{module_hint}:0-0:{name}:unresolved`), which is captured by `Edge.is_resolved=False` per ADR-0028, not as a Symbol.kind value; (2) the IR boundary materializer (`ir.py:954-967`) always emits `kind="external_symbol"` regardless of what `_parse_dangling_id` extracts from the trailing slot; (3) no consumer reads `Symbol.kind == "external_symbol"` as a discriminator — boundary-status checks go through the `is_external_boundary(sym)` helper which reads `meta["external_boundary"]=True`, so the kind is a label not a load-bearing classifier; (4) ADR-0024 §"Fold-residue discipline" rule 3's recurrence threshold for promoting a meta key to a sibling field (≥3 distinct values OR ≥2 producer modules) is not met (1 producer, 0 consumer reads, 0 paired-suffix variants — vs ADR-0028's 10 paired `*_resolved`/`*_unresolved` evidence types across 4+ producers). Resolution: `external_symbol` reclassified DEPRECATE-NO-FOLD → CANONICAL with status RESOLVED — promoted to `language_construct` as a pipeline-DSL top-level construct, parallel to other Cluster H domain-DSL constructs (`playbook`, `participant`, `fragment`); `unresolved` advanced UNRESOLVED → PRELIM_RESOLVED on AXIS_ENDPOINT_SHAPE (registry seed error, no producer to migrate). The `Symbol.is_resolved` follow-on ADR is **withdrawn** — the trace showed there is no leak to absorb. Audit-findings 0007 §"Diagnostic findings #3" rewritten to record the trace finding and the withdrawn ADR; top-of-file Status header advances from "Mixed" to "All rows resolved (57 RESOLVED, 3 PRELIM_RESOLVED)". Index in `docs/audits/README.md` updated to "All rows resolved". `SCHEMA_VERSION` patch-bumps `0.5.6` → `0.5.7` (2 `x-axis-of-values` axis reclassifications in `docs/schema.json`). With this PR ADR-0027 Phase 3 producer migration is empirically complete on `Symbol.kind` — every value in the registry has a verdict and a status of RESOLVED or PRELIM_RESOLVED.
