...
**Breaking: `hypergumbo io-boundaries` output has changed.** The I/O catalog was refactored to hold only true stdlib primitives; the wrapper chains that previously counted under `net_send` / `fs_read` / `fs_write` / `db_*` / `logging` now surface through a new `external_potential` boundary instead, paired with a per-language catalog `status: complete | in_progress` declaration. `--json` gains `boundaries.external_potential` and `dst_classification_unreliable` per chain; schema 0.2.2 → 0.2.3 (additive). See [`docs/MIGRATION-IO-BOUNDARIES.md`](docs/MIGRATION-IO-BOUNDARIES.md) for the full migration guide.

### Added

#### IO boundaries

- **`external_potential` bucket** (Plan C, PR C): `compute_boundary_map` gains a second pass that synthesizes an `IoChain` for every edge whose `meta.io_boundary` is unset and whose destination is a synthetic tier-3 boundary node. Carries `dst_tier` / `dst_tier_name` / `dst_external_boundary` plus a composed `module.name` primitive. Chains from an `in_progress` source language carry `dst_classification_unreliable=True` (text output annotates `[unreliable]`). Destinations in the source's `stdlib_other` set are skipped — they are stdlib non-IO symbols, not catalog gaps. Replaces the wrapper-catalog-growth treadmill with a first-class "untrusted-territory reach" signal.
- **`IoChain.dst_tier` / `dst_tier_name` / `dst_external_boundary`** populated when `compute_boundary_map` receives the `nodes_by_id` lookup. `--json` exposes them per-chain; text output annotates external-boundary destinations with `[tier-N tier_name]`. Backward-compatible when the argument is omitted.
- **Per-language catalog `status` and `stdlib_provenance`** (Plan C, PR B): catalogs declare `status: complete | in_progress` (default `complete` when absent) plus a `stdlib_provenance.source_url`. Python is `complete` (3.13 stdlib, cross-checked against `sys.stdlib_module_names`); the other 12 are `in_progress`. A new `ALLOWED_PROVENANCE_HOSTNAME_SUFFIXES` allowlist plus a load-time `_validate_catalog_dict()` reject missing or off-allowlist citations. Catalogs may also declare `stdlib_other:` (stdlib non-IO symbols that `external_potential` skips). `IoBoundaryCatalog.merge` unions `stdlib_other` across child + parent so Kotlin will inherit Java's once promoted.
- **C++ scoped attribute reads on the `std::` namespace produce `module_attr_ref` edges** (WI-zojid, follow-up to WI-vipur): the C++ tree-sitter analyzer now wires `emit_module_attribute_refs` with `scoped_path=True` over `qualified_identifier` (fields `scope` / `name`), injecting `std` as an implicit import and merging the namespace aliases from `_extract_namespace_aliases` (so `namespace fs = std::filesystem;` followed by `fs::path::preferred_separator` resolves to `std.filesystem`). Pairs with a new `io_primitives/cpp.yaml` declaring `ipc_send: [{module: std, attributes: [cout, cerr]}]` and `ipc_recv: [{module: std, attributes: [cin]}]`. C++ moved from `_CATALOG_ALIASES` (was an alias of C) to `_CATALOG_PARENTS` so the cpp child catalog adds the iostream surface while still inheriting libc / POSIX from c.yaml — without that move, `module: std` would have polluted the C-only catalog. Bare `cout`/`cerr`/`cin` after `using namespace std;` remains out of scope (needs a `using` declaration model). Catalog declares `status: in_progress` per Plan C, PR B; promotion to `complete` requires enumerating the full `<iostream>` / `<fstream>` / `<sstream>` surface and adding `stdlib_provenance`. Tagged `awaits_bakeoff_validation`: a C++ cohort run on iostream-heavy CLIs should observe non-empty `ipc_send` / `ipc_recv` boundaries that previously fell off the C-alias path.
- **Attribute-style IO primitives across six languages** via a new `module_attr_ref` edge type. Shared tree-sitter helper `emit_module_attribute_refs` (WI-gapam), per-language wire-ups (WI-lozug for Go `os.Stdout` / JS-TS `process.env` / `window.*` / `document.*` / `navigator.*` / Java `System.out` + imported class fields), plus Python `os.environ` (WI-guhok; env_read chain count 3 → 39), Rust tree-sitter via scoped-path mode for `std::env::consts::OS` (WI-vipur), and C non-shadowed `stdout` / `stderr` / `stdin` (WI-gotuv). Previously-inert YAML catalog entries now actually fire. `tag_io_boundaries` and `_TRACEABLE_EDGE_TYPES` accept the new type. C++ `qualified_identifier` + `using namespace std;` analog tracked as WI-zojid.
- **Python stdio reclassified from `ipc_send` to `logging`** (WI-tolif): `sys.stdout` / `sys.stderr` move to a new `logging` block (the same fix Go's `log` / `log/slog` / `fmt` already received). Eliminated 70 of 77 `ipc_send` false-positives in self-analysis. `sys.stdin` stays in `ipc_recv` — untrusted piped input is a real IPC concern. Cross-language analogs tracked separately.

#### verify-claims

- **Project-local taint catalogs** (WI-votan): repeatable `--taint-sources PATH` / `--taint-sinks PATH` / `--taint-sanitizers PATH` flags (YAML file or directory globbed as `*.yaml`). Same paths can live under `extra_catalogs:` in the claims YAML (relative paths resolve against the claims-file directory). User source/sink entries matching `(module, name, kind)` replace auto-derived/built-in entries; user sanitizers concatenate. New public helper `taint.load_full_taint_catalog(...)`.

#### Framework detection

- **Name-form normalization at matcher boundaries** (WI-pisab, from WI-zigah response): new `NameMatcher` utility with canonical (alphanumeric + dots, dotted-segment suffix match) and regex (terminal-segment fallback) modes. `Pattern._base_class_re` / `_parent_base_class_re` / `_annotation_re` / `_param_type_re` use it, so a YAML pattern like `^BaseModel$` matches source-side `pydantic.BaseModel` (from `import pydantic` + `class Foo(pydantic.BaseModel)`); same for annotations (e.g. `^RestController$` matches `org.springframework.web.bind.annotation.RestController`) and parameter types (e.g. `^Depends$` matches `fastapi.Depends`). `_decorator_re` intentionally stays on raw `re.compile` — dispatch-set triplets (e.g. `^task$` paired with `^(celery|app)\.task$`) would double-fire under terminal fallback; a regression test guards the exception. A matcher-boundary discipline lint (`test_matcher_boundary_constructor_discipline`) AST-walks `Pattern.__post_init__` and a polyglot call-site coverage fixture tracks the WI-zigah invariant across future analyzer additions.

#### Transcript playbook injection — recognition reframe (WI-bodog)

- **Reshape the injection output block so it reads as a reference document, not a list of bare ids.** Empirical signal from the WI-fusak overlap sweep (48 sessions via `scripts/measure-playbook-overlap.py`) found 11 Read tool calls on playbook files, 5 of which happened *after* the pipeline had already injected the same playbook — strong indication the agent didn't recognise the injected block as the document it was about to fetch. Pre-WI-bodog format had three failure modes: the first visible line was an SPDX HTML comment on 12 of 21 playbooks, the divider was a bare kebab-case id (`--- recover-state-playbook ---`), and there was no repo-relative path the agent could grep against the Read it would otherwise issue. Post-WI-bodog: the wrapper header is now `[Transcript Analysis — N relevant document(s)]` (neutral noun — 9 of 21 entries are protocols/guides/SOPs); each block opens with `--- <natural title> — <repo-relative path> ---` (title parsed from the first H1/H2/H3 heading via `extract_natural_title`, falling back to the id); the leading SPDX comment is stripped via `strip_spdx_header`; and a one-line framing hint follows the divider: `If the task below matches, consult this instead of re-reading the file.`.
- **Three new presentation helpers in `.agent/hooks/_shared/on_transcript_change.py`** — `strip_spdx_header`, `extract_natural_title`, `format_playbook_block` — covered by 17 focused tests in `tests/test_on_transcript_change.py`. Helpers are pure (no I/O, no dependency on `read_playbook`'s file-existence path), so the test surface mirrors the change's risk surface 1:1.

#### auto-pr / orphan tracker-sync detection (WI-dofaz)

- **`auto-pr` post-success path now warns about orphaned tracker-sync PRs**: a tracker-sync PR (deterministic shape `head.ref` starts with `tracker-sync/` per `sync.py:854`) created mid-cycle but never picked up would previously sit unmerged for hours, leaving local tracker state ahead of origin and seeding conflicts on the next mutation (motivating incident: PR #3376 sat orphaned for ~3 hours during the 2026-04-25→26 session). Both `do_pr` and `flush_queue` now capture `_autopr_run_started_epoch` at entry; after a successful merge they call `find_open_tracker_sync_prs` and emit a warning for any tracker-sync PR whose `created_at` predates this run. Sync PRs created mid-cycle are intentionally ignored (the next auto-sync threshold-cross will pick them up). Warning is best-effort and never affects the auto-pr exit code; auto-merging is deliberately not attempted, preserving the "one auto-pr invocation = one PR" mental model.
- **`find_open_tracker_sync_prs` helper in `scripts/lib/forgejo-api.sh`**: enumerates open PRs whose `head.ref` begins with `tracker-sync/`, emitting `PR_NUM<TAB>BRANCH<TAB>CREATED_EPOCH<TAB>AGE_SECONDS` per row. Reuses the existing `api_get` plumbing (no new auth path, no `gh` dependency).

### Changed

- **Shared `compute_dampened_centrality` helper extracts the duplicated centrality+dampener pipeline** (WI-tahum, follow-up to WI-lidum / WI-dohaf): new `ranking.compute_dampened_centrality(symbols, edges, *, first_party_priority=True, edge_type_weights=None, hub_threshold=100, within_file_weight=0.3, max_per_file_in=5, exclude_dampeners=())` is the single source of truth for the "compute_centrality + canonical 8-stage dampener stack" pipeline that previously appeared inline in three places. Defaults match `rank_symbols`' tuned values; dampeners run in canonical order tier → noise → utility → common_method → sibling_impl → trivial_sink → generated → file_kind. Internal `_apply_canonical_dampeners` lets `rank_symbols` keep `raw_centrality` separate (so `RankedSymbol.raw_centrality` stays meaningful) while still sharing the dampener logic. Three call sites collapse to one-liners: `sketch._format_symbols` (passes `exclude_dampeners=("file_kind",)` since `KEY_SYMBOL_KINDS` already excludes `kind="file"`), `compact.select_by_coverage`, and `compact.select_by_connectivity` (centrality-is-None branch). The `format_tiered_behavior_map` post-budget victim-removal step also moves to the helper via a new `_VICTIM_REMOVAL_EXCLUDE_DAMPENERS` constant that opts every non-tier dampener out — this makes the "tier-only victim ordering" choice explicit instead of a silent omission. `select_by_tokens`'s separate `compute_centrality(symbols, edges)` call (no tuned params, used for coverage normalization rather than ranking) is intentionally untouched. No behavior change — all 4,293 tests pass at 100% coverage.
- **External boundary nodes no longer stripped from JSON output** (INV-miniz, supersedes the WI-sikur strip step): synthetic boundary `Symbol`s now serialize into `behavior_map["nodes"]` with `kind="external_symbol"`, `path="<external>"`, `meta.external_boundary=True`, and `supply_chain.tier` populated (3 for most externals; 2 for Go/Java/Kotlin direct deps via `DependencyManifest`). Satisfies INV-miniz for disk-load consumers (`cmd_slice`, `cmd_test_coverage`, `cmd_verify_claims`) — previously the in-memory pipeline saw them but the serialized output silently re-introduced the dangling-reference problem. Display surfaces filter via the new `ir.is_external_boundary()` helper, so sketch / compact / search output is unchanged; `cmd_explain` intentionally surfaces boundary symbols. Schema 0.2.2 → 0.2.3 (additive): `external_symbol` added to `Symbol.kind`; `Span.start_line` / `end_line` minimum loosened from 1 to 0 for zero-span nodes.
- **Taint catalog auto-derivation** (WI-lokuv): sources/sinks auto-derive from `io_primitives/*.yaml`. Default zone/label map: `fs_write`→`host_fs`, `net_send`→`network`, `subprocess`→`host_fs`, `env_write`→`host_env`, `ipc_send`→`ipc`, `browser_storage_write`→`browser_storage`; reads `env_read`→`host_secret`, `net_recv` / `ipc_recv`→`untrusted_input`. Hand-written YAML overrides on `(module, name, kind)`. 419 previously-uncovered primitives now flow through `verify-claims`. Shipped `taint_sinks/host_filesystem.yaml` + `taint_sinks/network_send.yaml` removed (they duplicated io_primitives); WI-hizik drift-guard retired. New zones `host_env` / `ipc` / `browser_storage` and labels `host_secret` / `untrusted_input`. `TaintSource` / `TaintSink` `kind` accepts `"attribute"`; `module_attr_ref` joins `TAINT_CALL_EDGE_TYPES`.
- **Browser-local storage split out of filesystem categories**: new `browser_storage_write` primitive (`localStorage.setItem` / `sessionStorage.setItem` / `.clear` / `.removeItem`, moved from `javascript.yaml#fs_write`) and new `browser_storage_read` category (`localStorage.getItem` / `sessionStorage.getItem` / `indexedDB.open` / `caches.{open,match,has,keys}`, moved from `fs_read` — WI-kanir-huzuj). Auto-import routes writes to the `browser_storage` zone; reads intentionally omitted from `AUTO_SOURCE_LABEL_MAP` since sensitivity depends on stored content — project-local `taint_sources` fill the gap. `document.cookie` stays under `env_read` pending a getter/setter split.

### Added

- **Python `pyproject.toml` dependency manifest for tier-2 classification** (WI-nunuj): new `hypergumbo_lang_mainstream.py_deps.parse_python_dependencies(repo_root)` parses `[project].dependencies`, `[project.optional-dependencies]`, and `[tool.poetry.dependencies]` (the `python` interpreter constraint is excluded). Distribution-name → import-name resolution via `importlib.metadata.packages_distributions()` (e.g. `PyYAML` → `yaml`, `beautifulsoup4` → `bs4`, `scikit-learn` → `sklearn`); unknown dist names fall through to the dist name verbatim with hyphens swapped for underscores. Stdlib carve-out via `sys.stdlib_module_names`. Wired into `analyze_python` (manifest is attached to `AnalysisResult.dependency_manifest`); `ir.create_boundary_nodes`'s manifest-aware language allow-list extends to `python` with reason `"direct dependency (pyproject.toml)"`. Boundary nodes whose import path matches a declared dep are now classified tier-2 instead of tier-3.

### Fixed

- **`compute_centrality` parameter alignment across sketch / select_by_coverage / select_by_connectivity / format_tiered_behavior_map** (WI-dohaf, sibling to WI-lidum): the four selection surfaces previously called `compute_centrality(symbols, edges)` with bare defaults while `rank_symbols` called it with `hub_threshold=100`, `within_file_weight=0.3`, `max_per_file_in=5`, and `edge_type_weights=DEFAULT_EDGE_TYPE_WEIGHTS`. Backfills the tuned parameters at all four call sites via a shared `_RANK_CENTRALITY_KWARGS` constant in `compact.py` (sketch reuses the kwargs inline). `max_per_file_in=5` is the parameter that fixes the WI-lidum-cited 'createYAMLNode called 46 times from 1 file' pattern; `hub_threshold=100` saturates infrastructure utility hubs; `within_file_weight=0.3` dampens local-variable inflation; `edge_type_weights` makes import edges count less than calls. The 6-repo audit showed 19-71 of top-100 churn across all 6 repos × 4 surfaces (50%+ on most cells), with chatwoot's Vue/JS-heavy callgraph showing the smallest movement (parameters were tuned against Go/Java/Python centrality patterns). Tagged `awaits_bakeoff_validation`. Drive-by fix: `test_first_party_priority_disabled` was using a `type("Edge", ...)` mock that lacked `edge_type`; now uses `Edge.create()`.
- **`compact.select_by_connectivity` dampener stack aligned with `rank_symbols`** (WI-lidum, Phase B / patch 3 of 3): Connectivity-aware compact selection previously computed raw centrality with no dampeners (the `centrality is None` branch at compact.py line ~605). Backfills the full 8-dampener stack — `apply_tier_weights`, `apply_noise_weights`, `apply_utility_symbol_weights`, `apply_common_method_name_weights`, `apply_sibling_impl_weights`, `apply_trivial_sink_weights`, `apply_generated_code_weights`, `apply_file_kind_weights` — only when the caller does not supply a precomputed `centrality` dict. The internally-computed centrality drives both the empty-seeds fallback (`max(symbols, key=centrality)`) and the tertiary tiebreaker in `_compute_connectivity_score`. The 6-repo audit showed `tier × select_by_connectivity` was the largest single cell in the surface (7-22 of top-100 across all 6 repos: external `Sprintf` / `Background` / `package` / `module` / `assertSequenceEqual` had been leaking into the seed pick); `trivial_sink` moved 26-49 of top-100 across all 6, `utility` 4-26, `generated` 0-14 (kserve only), `file_kind` 0-5 (Go/Kotlin only). Tagged `awaits_bakeoff_validation`.
- **`compact.select_by_coverage` dampener stack aligned with `rank_symbols`** (WI-lidum, Phase B / patch 2 of 3): Coverage-based compact selection previously ran centrality + 1 of 8 dampeners (`apply_tier_weights` only, conditional on `first_party_priority`). Backfills `apply_noise_weights`, `apply_utility_symbol_weights`, `apply_common_method_name_weights`, `apply_sibling_impl_weights`, `apply_trivial_sink_weights`, `apply_generated_code_weights`, and `apply_file_kind_weights` in `rank_symbols`'s canonical order. The 6-repo audit showed `trivial_sink` moved 14-34 of top-100 across all 6 repos, `utility` 4-22, `generated` 0-22 (kserve only), `noise` 0-8 (django only), `file_kind` 0-9 (Go/Kotlin only). Tagged `awaits_bakeoff_validation`; next DEEP cycle's reflect-aggregate cross-references the `select_by_coverage` top-N drops.
- **`sketch._format_symbols` dampener stack aligned with `rank_symbols`** (WI-lidum, Phase B / patch 1 of 3): Key Symbols selection previously ran centrality + 3 of 8 dampeners (tier / utility / trivial-sink). Backfills `apply_noise_weights`, `apply_common_method_name_weights`, `apply_sibling_impl_weights`, and `apply_generated_code_weights` in `rank_symbols`'s canonical order. `apply_file_kind_weights` is intentionally skipped — `KEY_SYMBOL_KINDS` already excludes `kind="file"` symbols. Empirical 6-repo audit (alertmanager, prometheus, kserve, chatwoot, detekt, django) showed kserve's OpenAPI model classes (`V1beta1InferenceService` et al.) accounted for 42 of the top-100 sketch entries before this fix; django's `ModelState` / `ProjectState` from `db/migrations/` accounted for 7. Tagged `awaits_bakeoff_validation`; next DEEP cycle's reflect-aggregate cross-references the sketch top-N drops in those two repos.
- **Tier 1 dataflow `_build_line_index` lets sibling comments shadow the real-code node** (WI-likab): when a real-code node and a trailing comment both start on the same line — `v := compute(x)  // godoc` — the comment is a sibling that, in DFS order, would overwrite the real-code entry in the line→deepest-node index. The dataflow positional walk then runs from the comment leaf, finds no matching ancestor, and the call edge is left unannotated. The bug surfaced on real Go code where many call sites carry trailing comments: the empirical Go fixture went from 43 % → 71 % calls-edge `access_mode` coverage after the fix (L24 `s := compute(n)  // user-call inside short_var` and L30 `return doWork(x)   // return_statement` flip from `mode=None` to `mode=read`). The fix is a one-line predicate in `_build_line_index` that skips nodes whose `type` contains `"comment"` (covers `comment` / `line_comment` / `block_comment` / `doc_comment` across every tree-sitter grammar without per-language hardcoding); since the line index is shared by `annotate_dataflow`, every tree-sitter language analyzer (Go, Kotlin, Rust, TypeScript, Erlang, Java) benefits. Standalone calls in `expression_statement` context (`fmt.Println(x)`) intentionally remain unannotated — that path falls through graceful degradation per the 790e3c12a INV (tagging them as `read` would cause the forward dataflow slicer to drop call chains). Tagged `awaits_bakeoff_validation`; the next DEEP cohort that re-runs alertmanager/kafka/prometheus should observe a non-trivial uplift in `slice.dataflow.0.json` calls-edge `access_mode` coverage.
- **`auto-pr` convergence: PR-merged verification gate before close, post-rebase poll-and-merge loop** (INV-rahib regression, PR #3392/#3393): Scenario B (timeout-recovery and hung-run paths in `scripts/auto-pr`) now consults a `_pre_scenario_b_gate` helper in `scripts/lib/forgejo-api.sh` before each close-and-repush. The gate checks `_check_pr_merged` (catches the merged-during-timeout sub-case where the poll endpoint was 502'ing while the merge actually completed) and falls back to a `mergeable=true` + short-timeout `poll_ci` retry (catches the about-to-merge / poll-flake sub-case). When either fires the close is skipped — merged → exit success, about-to-merge → fall through to the merge cascade. Surface 2: `do_merge`'s post-rebase block is now a labeled `while [ "$rebase_attempts" -lt "$rebase_max" ]` loop with explicit `continue` after re-rebase (cap 3 iterations) instead of a single merge attempt → `Recovery:` hint. Recovery hint moved to after the loop exhausts. No recursion / guard-flag re-entry pattern (Decision 2 from human triage on 2026-04-27). Validation criterion: the next auto-pr invocation that traverses the rebase-then-merge path completes without emitting `Recovery:` and merges to dev.
...
- **External boundary nodes carry `stable_id` / `canonical_name`; file-id pseudo-symbols collapse per language** (WI-fozoh): `create_boundary_nodes` now derives stable identity for every synthesized boundary Symbol from its dedupe key (`sha256` of `(language, path, name, kind)`), so `stable_id` and `canonical_name` are no longer null. For `kind="file"` pseudo-IDs (every Python file's `make_file_id`-style import-edge src — 732 of them on hypergumbo self-analysis), all per-reference variants per language collapse into one canonical `<external>` boundary Symbol; per-file attribution survives via `meta.referring_paths` on the surviving edges (capped at 50 entries). Other externals (module imports, unresolved calls) preserve their full path-slot identity, so distinct logical externals like `urllib.request.urlopen` vs `urllib.parse.urlopen` stay distinct boundaries. Tier classification is **tier-min** across collapsed groups: any tier-2 referring site promotes the canonical node. New `apply_external_id_remap` helper (called by `cli.py`'s `run_behavior_map`) rewrites edge src/dst per the remap and unions referring paths on collisions. Self-analysis: ~37% drop in external-symbol count (2,405 → 1,514), zero null `stable_id`/`canonical_name` (was 100% null), 2,152 edges carry referring-path attribution. The structural file-id mismatch driving the symptom is tracked separately as a Plan B follow-up.
- **Python analyzer — module-level `NAME = ...` not indexed as Symbols** (WI-gafog E2): the symbol-extraction pass walked classes / functions / methods only, so module-level constants (`PASS_VERSION`, `LANGUAGE_ALIASES`, `EXIT_SUCCESS`, …) were absent from `global_symbols`. Any `from <mod> import NAME` for such bindings missed the cross-file lookup and `create_boundary_nodes` synthesised a tier-3 `external_symbol` instead — 151 ALL-CAPS externals on hypergumbo self-analysis. New helper `_emit_module_level_assign_symbols` walks `tree.body` (top-level only) and emits `Symbol(kind="variable", ...)` for `ast.Assign` and `ast.AnnAssign` with `Name` targets, including tuple-unpacking (`A, B = 1, 2`); skips `AugAssign` (mutation, not fresh binding) and Subscript / Attribute targets. The CSS-only `variable`-kind exclusion in the noise-filter is now language-conditional (`{css, scss, sass, less}`) — Python / Go / YAML / Make module-level bindings stay in the output.
- **Python analyzer — single-root `_detect_src_layout` misses monorepo `packages/<pkg>/src/<mod>/` layouts** (WI-davan E1): the previous helper only inspected `repo_root/src/`, so files under hatch / PDM / Poetry monorepo layouts (and hypergumbo's own `packages/<pkg>/src/`) fell back to a path-shaped `rel_path.relative_to(repo_root)` qualifier. Module names came out as `packages.hypergumbo-core.src.hypergumbo_core.taxonomy` — invalid Python (hyphen) and not the real importable name. Replaced with a tree-walking `_detect_source_roots(repo_root) -> list[Path]` (skips `DEFAULT_EXCLUDES` and dotfile dirs) and a `_module_name_from_path` that picks the *most-specific* (deepest) matching source root for each file. Single-root layouts collapse to the previous behaviour. New regression tests cover monorepo detection, deepest-root preference, and an end-to-end assertion that no synthesized id carries `packages.` / `.src.` segments in the module-qualifier slot.
- **Markdown analyzer — link-target URL leaking into `Symbol.language`** (WI-diruj): `_extract_links_from_paragraph` previously emitted `Edge(dst=url)` with the raw link target (e.g. `../../AGENTS.md`) as the `dst`. Downstream, `ir.create_boundary_nodes` parses dangling ids as `{lang}:{path}:{span}:{name}:{kind}` — for a colon-free URL the whole string landed in the language slot, producing 9 orphan `external_symbol` nodes whose `language` was `../../AGENTS.md`, `../hypergumbo-spec.md`, etc., and contaminating `io-boundaries.unsupported_languages`. Markdown now constructs a properly-formed dst id via a new `_make_link_dst_id` helper: strips the `#anchor` fragment for file identity, derives the basename for the name slot, and replaces colons in the path so the 5-part shape is preserved on URLs containing `:`. Synthesized boundary nodes correctly carry `language="markdown"`.
- **Transcript-change hook: TOCTOU race on injection-state file** (WI-ritut): `on_transcript_change.py` wraps its load → decide → save critical section in a new `injection_state_lock` context manager (advisory `fcntl.flock` on a per-session `.lock` sibling of the state file). Before the fix, parallel PostToolUse hooks (fired by the agent's parallel tool calls) all read the same pre-write state, independently selected the same playbooks, and emitted duplicate injections into the agent's context — measured at a 33 % session-wide violation rate across 6 different playbooks, with some duplicates landing 0.4 s apart. Mirrors the flock pattern already used by `scripts/smart-test` (WI-sinap) and rolls up under INV-filaj (concurrent-session state-integrity invariant).
- **Python analyzer — dotted-submodule call resolution** (WI-zigah): `_process_call` now emits `calls` edges for `from pkg.subpkg import X` + bare `X(...)` (e.g. `urlopen` from `urllib.request`) and `import pkg.subpkg` + `pkg.subpkg.X(...)` (multi-segment chain with an `ast.Attribute` receiver). Both were silently dropped, blocking io-boundaries and taint-flow from matching dotted stdlib primitives (urllib, http.client, os.path, shutil, xml.etree, concurrent.futures, asyncio.subprocess). `_extract_imports` now records the top-level binding for dotted imports — Python-semantically, `import pkg.subpkg` binds only `pkg`, so the previous `module_imports["pkg.subpkg"]` entry was unreachable. New helper `_unwind_attribute_chain` canonicalizes dotted chains back to qualified form.
- **IO catalog — `replace` / `rename` added to `ambiguous_names`** (INV-maluk): the matcher's short-name fallback was matching every `something.replace(...)` as `pathlib.Path.replace` (a filesystem rename), producing 40+ false-positive `fs_write` chains in self-analysis from string-normalization sites like `name.replace("-", "_")`. Resolved calls with a `pathlib.Path` module hint still tag correctly (regression test). `fs_write` chain count: 138 → 98 with real detections preserved. Cross-language audit in `java.yaml` / `javascript.yaml` / `go.yaml` / `rust.yaml` carried by INV-maluk's scope-expansion clause.
- **`sketch_embeddings` loads HuggingFace models offline-first**: new `_load_st_model_offline_first` helper used by both `_load_embedding_model` and `_load_modernbert_model`. Tries `SentenceTransformer(name, local_files_only=True, ...)` first; falls back to a normal load only on `(OSError, ValueError)`. Eliminates the "unauthenticated requests to the HF Hub" warning that fired on every `hypergumbo .` run with the embeddings extra installed, and makes hypergumbo's own `network_send` chain through this constructor genuinely absent rather than hidden.
...
- **`ci.yml` / `release.yml` pip-audit ignore for CVE-2026-3219**: pip concatenated-ZIP+tar archive confusion (CVSS 4.6 MEDIUM; AV:L, UI:A, VI:L). Fix is pip 26.1 via `pypa/pip#13870`, but that release was not yet on PyPI when the CVE published 2026-04-20; the existing `pip install --upgrade pip` step at install time will pick it up automatically once it ships. Zero attack surface on the self-hosted runner (no user-supplied archives ever reach pip). Both gates drop the ignore once pip-audit stops flagging pip.

### Removed

- **Third-party wrappers purged from every I/O catalog** (Plan C, PR A): catalog membership is now strictly "the language ships it" — the previous grandfathered HTTP-client carve-out was a slippery slope. Per-language removals: **Python** — `requests` / `requests.Session`, `aiohttp.ClientSession`, `httpx.Client` / `AsyncClient`. **Java** — Apache Commons IO, Netty, OkHttp, Spring Web (`RestTemplate` / `WebClient`), Apache HttpClient 4/5, Unirest, Retrofit, Spring Data + Hibernate, SLF4J, Log4j 1.x / 2.x, Logback (JDK + Jakarta EE stay). **JavaScript** — npm HTTP clients (`axios`, `node-fetch`, `ky`, `superagent`, `got`, `undici`), Express, Fastify, Koa (Node built-ins and browser globals stay). **Rust** — `tokio::fs`, `tokio::net::*`, `hyper`, `axum`, `actix_web`, `reqwest` (`std::*` stays). **Scala** — rewritten: fs2, cats-effect, sttp, http4s, akka, pekko, Play, Slick, Doobie, Quill, ScalikeJDBC, Anorm, ReactiveMongo, ZIO, scala-logging (`scala.*` + inherited `java.*` stay). Structural tests iterate every catalog primitive and assert a stdlib module prefix, so future accidental third-party adds fail at PR review. Dropped chains resurface in `external_potential` (PR C). Validation tagged `awaits_bakeoff_validation`.
- **Earlier HuggingFace Hub wrapper additions reverted** (supersedes WI-jihuj): `huggingface_hub.{snapshot_download, hf_hub_download}`, `HfApi.{model_info, list_repo_files, download_file}`, and `sentence_transformers.SentenceTransformer` were briefly added to `python.yaml#net_send` then removed. The structural answer is tier-aware reasoning via `IoChain.dst_tier` plus the `external_potential` bucket: `verify-claims` reads the destination's `supply_chain.tier` and treats tier-3 boundary destinations as "may make network calls" without per-library catalog enumeration.
