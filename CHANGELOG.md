...
- **Django** — two strategies. `DjangoExplicitStringStrategy` finds `render(request, "<template>", ctx)` and class-body `template_name = "<template>"` by re-parsing Python source (symbol meta doesn't carry call-site string args). `DjangoCBVDefaultStrategy` finds classes transitively extending `DetailView` / `ListView` / `CreateView` / `UpdateView` / `DeleteView` / `FormView` and derives the default template from `model = <Name>` (`UserDetailView` + `model = User` → `users/user_detail.html`).
- **Phoenix (MethodNameStrategy)** — handles 1.x layouts (`lib/my_app_web/templates/user/show.html.{eex,heex,leex,…}`), 1.7+ co-located layouts (`lib/my_app_web/controllers/user_html/show.html.heex`), and the parallel `MyAppWeb.UserHTML.show` function-component shape. Namespaced controllers become nested directories. PhoenixStrategy splits on the final `"."` (via a new `extract_class_method` hook on the shared core), drops `defp` private functions, and excludes Plug callbacks (`init` / `call` / `action`). LiveView modules out of scope (templates inline).
- **Spring MVC (ExplicitStringStrategy)** — gates on `@Controller` (excludes `@RestController`) + a mapping annotation (`@RequestMapping` / `@GetMapping` / etc.). Recognizes plain string returns and `new ModelAndView("users/show", model)`; skips `redirect:` / `forward:`. Probes Thymeleaf / FreeMarker / Velocity templates under `src/main/resources/templates/<view>.{html,ftlh,ftl,vm}`, then JSP at `src/main/webapp/WEB-INF/views/<view>.{jsp,html}`. Re-parses Java source via tree-sitter for return-statement expressions (not captured by the analyzer's symbol meta).
- **Laravel Blade (ExplicitStringStrategy)** — gates on `app/Http/Controllers/` path + transitive extension of `Controller`. Walks PHP method bodies via tree-sitter for `view(...)` calls and `View::make(...)` scoped calls; non-string first arguments block resolution. Dotted view names map to slash-separated directories; probes `.blade.php` first, then plain `.php`.
...
- **`SCHEMA_VERSION` 0.7.1 → 0.7.2 → 0.8.0.** Field lands at 0.7.2, becomes canonical at the producer at 0.8.0 with consumers preferring it over colon-split heuristics. `Edge.create()` accepts a `dst_ref` kwarg; `from_dict()` defensively reads `d.get("dst_ref")` so pre-0.7.2 cached JSON loads with `dst_ref=None`.
- **`hypergumbo_core.ir.ExternalRef`** — frozen dataclass `(lang, module_path, name)`. Aliased imports bind `name` to the imported symbol, not the local alias (`from X import Y as Z` → `name=Y`).
- **`hypergumbo_core.import_scope.ImportScope`** — shared analyzer-layer abstraction whose five population methods (`add_named`, `add_namespace`, `add_wildcard`, `add_aliased_module`, alias-of-namespace) mirror construct shapes across the 8 audited languages. `resolve(local_name) → CanonicalName(module, name)`; convenience `dst_ref_for(local_name, lang) → ExternalRef`.
- **Per-analyzer adoption:**
  - **Java** threads static-import maps through `_extract_edges`.
  - **Go** adds `dot_imports` to `FileAnalysis` for `import . "strings"`.
  - **Elixir** resolves aliased modules (`alias String, as: S` → `String`) and emits unresolved edges for `import Mod, only: [name: N]`.
  - **JS/TS** attributes named-import calls to the canonical name (`writeFile`), not the local alias (`wf`).
  - **C++** pre-walks system `#include` paths; unresolved external calls in files with system includes carry a comma-joined module hint.
  - **Rust** consults `use_aliases` in three new cases, eliminating the historical 6-segment dst.
  - **Ruby** emits a constant-external fallback so `Set.new(...)` and `JSON.parse(...)` are no longer silently swallowed.
- **Consumer migration.** `io_boundary._classify_call_edges` and `_compose_primitive_chains` prefer `edge.dst_ref.module_path` / `.name` over `_extract_module_hint` / `_extract_callee_name`. `ir.create_boundary_nodes` builds a `dangling_id → ExternalRef` map and short-circuits `_parse_dangling_id` for refs-carrying ids.
- **Polyglot call-site coverage.** `test_polyglot_call_site_coverage.py` extends from 1 Python fixture to 8. All 7 `xfail strict=True` markers strip; the test now demands 8 fixtures pass. Re-introducing an `xfail_reason` is a regression signal.
...
- **Claims YAML** at `docs/hypergumbo.claims.yaml`: 18 per-entry-point taint-flow claims. Runtime CLI subcommands are prohibited from reaching `host_fs` / `network` / `subprocess` / `install_artifact` / `dev_zone` — all writes must go through `safety_zones`. Extras subcommands declare their specific allowed zones. Dev-zone reachability is prohibited from every CLI entry-point.
- **Project-local catalog** under `docs/hypergumbo-self-catalog/`: six entry-point source YAMLs (runtime CLI + install-gitleaks / install-embeddings / build-grammars / install-rust-analyzer / add-extras), five peer-zone sink YAMLs, one sanitizer YAML, README. Hypergumbo's self-audit only (not shipped to PyPI). Loaded via `extra_catalogs:` in the claims YAML.
- **Wrapper-function discipline.** New `hypergumbo_core.safety_zones` ships eight thin wrappers around fs-write primitives (`cache_write` / `user_out_write` / `user_out_open_json_dump` / `tmp_artifact_write` / `cache_write_bytes` / `cache_save_npy` / `install_artifact_write_bytes` / `install_artifact_copy`). Distinct callee names let the catalog place each in its own peer zone; each calls `_safety_zone_barrier()`, declared as a sanitizer for every entry-point taint label, so BFS stops at the wrapper. The 15 raw fs-write sites in `cli.py` / `build_grammars.py` / `gitleaks.py` / `sketch_embeddings.py` migrate.
- **Algorithm extensions:**
  - `TaintSource.start_at` (`"caller"` default, `"callee"` opt-in) makes the structural and DDG propagators seed BFS at the source-callee itself rather than its caller. Synthetic entry-point sources use `callee` so reachability is precisely scoped.
  - `_sink_module_compatible` filters short-name collisions when the edge's dst carries a module hint — `socket.socket.write` no longer matches every `.write()`.
  - Sanitizers index as a list per callee name (was: flat dict overwrite), preserving every `(qualified_name → input_taint)` declaration.
  - `verify-claims` runs taint propagation **per-language** to avoid cross-language pollution. An Elixir `HTTPoison.get` sink was previously matching every Python `.get()`, producing ~15K spurious findings on self-analysis.
- **CFG ↔ DDG bridge (ADR-0017 §1c follow-through).** The CFG builder's def/use extractors were registered via decorator but never invoked. New `populate_def_use_for_cfg(cfg, body_node, src, language)` walks the AST in parallel with the CFG; `CfgNodeMapping.atomic_statements` (new field, populated from `atomic_statement:` in cfg_nodes YAMLs) tells the builder to stop decomposing statement-level nodes into bare identifiers. `verify-claims` now invokes `build_function_cfg → populate_def_use_for_cfg → solve_reaching_defs` for every Python function.
- **SECURITY.md generator.** New `scripts/generate-security-md` reads the claims YAML and produces the audited-IO-surface section between sentinel comment markers. Determinism contract: same input → byte-identical output. Vulnerability-reporting policy below the sentinels preserved verbatim. `--check` mode gates CI.
- **Known limitation.** Short-name sink matching still overapproximates for receivers the DDG can't resolve (call-return bindings, parameter receivers, closure captures). SECURITY.md documents the residual; load-bearing claims (dev-zone and install-zone unreachability from runtime CLI) verify cleanly. Receivers bound by file-scope imports or local module-attribute assignment are resolved by the post-DDG IR refinement pass (below).
- **Stop-the-line repair.** `io_primitives/javascript.yaml` had `methods: [on]` for `process.on` — `on` is YAML 1.1 boolean `True`, so the auto-derived TaintSource landed with `name=True` and crashed `propagate_taint_structural` the first time source-side propagation fired. Quoted to `["on"]`.

#### Third-party-base allow-list for Django-ecosystem dispatch BFS

New linker `django-third-party-dispatch` (`linkers/_third_party_bases.py`) emits `dispatches_to` edges from Python subclasses of four well-known third-party Django-ecosystem bases to their framework-called override methods:

- `HierarkeyForm` (hierarkey)
- `FilterSet` / `WagtailFilterSet` (django-filter)
- DRF `Serializer` / `ModelSerializer` / `HyperlinkedModelSerializer`
- Wagtail `Page`

Activation-gated on `LinkerActivation(frameworks=["django"])` — companion to the always-on `django-orm-dispatch` for Django-stdlib bases. Edges carry `meta["framework_dispatch"]="django_third_party"`. Disambiguation-fallback discipline preserved: unqualified short-name match colliding with an in-tree Python class downgrades to `confidence=0.5` + `meta["disambiguation_fallback"]=True`.

Across pretix + bakerydemo + lutris the new linker contributes ~38 method-edges with the method-override filter applied (`ModelAdmin` excluded because `django-orm-dispatch` already covers it).

#### Post-DDG IR refinement pass

When the Python analyzer can't type-infer a method-call receiver (`x = os.environ; x.get(...)`), it emits the call as `python:external:0-0:get:unresolved`. `_sink_module_compatible` exempts these `external`-module edges from the short-name-collision check (a strict rule would suppress legitimate findings), so short-name matching falls through and conflates `dict.get` / `args.get` / `os.environ.get` / `multiprocessing.Queue.get`. The refinement pass narrows this surface where the DDG can prove what the receiver was bound to.

- **New module** `taint_refine.py` runs between `solve_reaching_defs` and propagation. For each `recv.method()` site it finds the DDG edge `(variable=recv, use_line=call_line)`, walks the AST to the RHS at `def_line`, and yields `(call_line, attr_name) → module_hint` for import-rooted attribute chains and `from`-import aliases.
- **Edge rewriter** `refine_external_edges(edges, hints_by_caller)` rewrites unresolved-external dsts (`python:external:0-0:get:unresolved` → `python:os.environ:0-0:get:unresolved`); idempotent on edges without an applicable hint. `cmd_verify_claims` invokes it once per outer-loop iteration before either propagator runs.
- **Scope** follows ADR-0017 §1c accretion: Python today; Rust / TypeScript when those ship a def/use extractor. Call-RHS bindings, parameter receivers, and closure captures stay unresolved — the pass does not invent hints. ADR-0017 §3a's `external`-exemption paragraph updated in-place.

#### Safety-zone wrappers expanded — `rmtree` / `chmod` / `unlink`

SECURITY.md documented that short-name callees like `.rmtree`, `.chmod`, `.unlink` overapproximate because the edge's dst resolves to `python:external:0-0:<name>:unresolved`. The first concrete tightening: four new wrappers in `safety_zones` give each of hypergumbo's own mutating callsites a distinct, zone-tagged callee.
...
Self-catalog YAMLs (`user_cache_sinks.yaml`, `tmp_artifact_sinks.yaml`, `install_artifact_sinks.yaml`) updated in lockstep. `build_grammars.py`'s now-unused top-level `import shutil` removed. Complements the post-DDG refinement pass (above) — that pass resolves receivers bound by file-scope imports; wrapper discipline handles the residual the DDG can't reach.

#### Solidity `contract` kind registered canonically

`SYMBOL_KINDS` gains `SymbolKindSpec("contract", AXIS_LANGUAGE_CONSTRUCT, ...)`. Smart-contract declarations are a source-language top-level construct (sibling to `class` / `interface` / `struct`), not a relabel. `solidity.py:265` already emitted `kind="contract"`; this PR registers the value canonically. `docs/audits/0003-symbol-kind-cluster-a-language-constructs.md` gains the verdict row. Companion producer-side folds for CUDA / Android XML appear in Changed below.

#### Canonical dampener stack — formulas pinned end-to-end

`TestCanonicalDampenerStackPinned` (4 tests in `test_ranking.py`) closes the soft spots in dampener-formula coverage found during a walkthrough.

- **Stage order pinned.** `_CANONICAL_DAMPENERS` tuple order (eight names, `tier`→`file_kind`) is now pinned verbatim. `ranking.py:1106` already said "Order is load-bearing"; this test backs it up.
- **`apply_common_method_name_weights` multipliers pinned** at three collision counts: 20 → 0.5×, 50 → 0.2×, 15 → 10/15.
- **`apply_sibling_impl_weights` tail factor pinned.** 7-member group, members 4-7 each ×0.15.
- **Integration smoke test.** `compute_dampened_centrality(...)` must agree with manual application of the eight dampeners in canonical order — catches the case where `_CANONICAL_DAMPENERS` is unchanged but the helper internally reordered stages.

#### RCT-consumer public-API surface pinned via introspection test

`tests/test_rct_public_api_pinned.py` adds 9 introspection tests pinning the four monkey-patch surfaces the four-arm bundle RCT depends on. The variants (`mgumbo` perturbing `Edge.confidence`, `dgumbo` selectively disabling linker subcategories) attach by import path + signature, so a signature change that keeps behavior identical still breaks the variants. Failure messages name the specific RCT dependency (mgumbo/dgumbo) so a future contributor sees the dependency before `sed`-to-passing.

Surfaces pinned:

- `hypergumbo_core.ranking.rank_symbols` — six-parameter tuple list.
- `hypergumbo_core.ir.Edge` — eight RCT-critical dataclass fields, including `confidence=0.85`, `is_resolved=True`, `dst_ref`.
- `hypergumbo_core.cli.run_behavior_map` — ten of sixteen parameters with stable defaults.
- `_LINKER_SUBCATEGORIES` tuple in `scripts/generate-architecture` + the `LinkerActivation` field surface.

Source-comment markers at each definition site (`ranking.py:1215`, `ir.py:414`, `cli.py:6647`, `scripts/generate-architecture:711`).

#### IO-boundary noise reduction — F3 Filters 1 / 2 / 3

Three filters in `_compute_external_potential` reduce `external_potential` chain volume. Filters 1 + 3 ship the cheap filters; Filter 2 adds the closed-world stdlib gating substrate.
...
- **Filter 2 — closed-world stdlib gating.** `IoBoundaryCatalog` gains three new fields and a chain-skip rule.
  - New fields: `stdlib_modules` (frozenset of authoritative names — Python's `sys.stdlib_module_names`); `stdlib_prefixes` (tuple for hierarchical namespaces like Java's `java.*` or Go's `encoding/*`; prefix match accepts `.` and `/`); `stdlib_module_completeness` (dict module → audit `retrieved:` date; listed modules are closed-world). `merge()` unions all three with child winning on `completeness` collisions.
  - YAML parser accepts flat strings and dicts with inline `completeness: complete` + `retrieved:`; hard-errors when `completeness: complete` lacks a `retrieved:` ISO date.
  - Skip rule in `_compute_external_potential`: after the existing `stdlib_other` check, if `module_hint` is non-empty AND `catalog.is_stdlib_module_complete(module_hint)`, skip the chain. No skip when `module_hint == "external"`.
  - `scripts/refresh-stdlib-modules <language> --version X.Y` regenerates `stdlib_modules` deterministically from the live interpreter, validating it matches both `--version` and `stdlib_provenance.version`. Python only for now.
  - Initial population: `python.yaml` ships 300 stdlib modules from 3.12 (provenance corrected from claimed-but-unverified `"3.13"`); `math` is the inaugural worked-example closed-world entry. Surface starts narrow and expands per future hand audits.
...
- **Stop-the-line repair**: `test_c.py::TestCStdioIdentifierRefs::test_io_boundary_tags_emitted_edges` was asserting the pre-reclassification `ipc_send` value for `stdio.stdout` / `stdio.stderr` and was missed in 8de2f67015; updated to assert the post-reclassification `logging` value. `stdio.stdin` stays `ipc_recv` (piped untrusted input).

#### `io-boundaries --json` envelope `schema_version`

`IO_BOUNDARIES_SCHEMA_VERSION = "1.0"` is the inaugural wire-contract version for the `hypergumbo io-boundaries --json` envelope — a separate contract from the behavior-map envelope (which has its own `schema_version`). `BoundaryMap.to_dict` emits `schema_version` at the top; the filtered path in `cmd_io_boundaries` injects it explicitly. Locked top-level keys (any change must bump): `schema_version`, `total_io_edges`, `boundaries`, `unsupported_languages`. Bumping rules (minor for additive, major for renames / removals / type changes) live in the module docstring. `TestIoBoundariesEnvelopeSchema` loud-fails on silent drift.

#### Other

- **`hypergumbo_core.axis_meta_keys` — canonical registry for `Symbol.meta` / `Edge.meta` keys.** Structural sibling of `symbol_kinds.py` / `evidence_types.py`: tuple of `MetaKeySpec(name, axis, description)` entries with axis constants (`symbol_meta`, `edge_meta`) plus accessors. Seeded with the keys empirically observed across producer code — fold residues, ADR-0023 protocol / bridge / dispatch vocabularies, dataflow access modes, and common Symbol.meta annotations. Property test enforces no duplicates, valid axes, no name collisions with typed fields. Drift detection (subscript-access AST walker) is a separate follow-on.
...
#### CUDA / Android XML canonical-kind producer folds
...
- **CUDA.** `cuda.py:_determine_function_kind` now returns `(kind, execution_space)` where `kind` is always canonical `"function"` and the execution space (`"global"` / `"device"` / `"host_device"` / `"host"`) lives on `Symbol.meta["cuda_execution_space"]`. `language-conventions.yaml`'s three CUDA rules migrate from `symbol_kind: "^global$"` (never matched the producer output) to `symbol_kind: "^function$"` + `meta_match: {cuda_execution_space: ...}`; a new fourth rule covers `host_device`. Legacy `meta["is_kernel"]` flag preserved on `__global__` symbols (kernel_launch edges + GPU-entry-point detection key on it).
- **Android XML.** `xml_config.py:_process_android_application` now emits `kind="component"` for AndroidManifest `<activity>` / `<service>` / `<receiver>` / `<provider>` (was: bare element name, none in the canonical registry); the original element name lives on `meta["component_type"]`. `config-conventions.yaml`'s four `android_component` rules migrate to `symbol_kind: "^component$"` + `meta_match: {component_type: "^activity$"}` (etc.) via the new `Pattern.meta_match` field. Routing Android `<provider>` through `component`+`meta["component_type"]="provider"` keeps the canonical `provider` kind (Apex/Salesforce `[Provider]`) disjoint from Android content providers.
...
`_get_repo_state_hash` (`sketch_embeddings.py`) keyed only by the analyzed tree's state (HEAD SHA + tracked-file diff + untracked-file mtimes), not the analyzer's identity, so two hypergumbo installs analyzing the same source tree shared a cache entry — whichever wrote first won, the other read stale results. Triggered live by stable+dev coexistence, wheel-pin RCT cross-arm poisoning, and lang-package partial upgrades that don't bump `hypergumbo_core.__version__`.

New module `analyzer_identity.py` exposes `compute_analyzer_identity_hash() → str` (16-char hex), computed as `sha256(__version__ || sorted(<pkg>=<content_hash> per installed hypergumbo_* package))`. Per-package content hash walks each module's `__path__` for `.py` files in sorted relative-path order; skips `__pycache__` deterministically. Memoized per-process; walks via `importlib.metadata.distributions()` (robust to pipx / pip / editable / virtualenv).
...
UAT 4.1.0 measured 90-95% gzip-reducibility on raw `hypergumbo run` JSON (airflow 320MB → 18MB, kafka 572MB → 34MB). Two new flags:

- `--gzip` writes main + budget-tier outputs as gzipped JSON. Pairs with `--out foo.json.gz` for a literal-path UX. Inner encoding unchanged (`indent=2, sort_keys=True`), so `gunzip` produces byte-identical content.
- `--no-sketch-fan-out` is an explicit named alias for `--budgets none`, surfaced in `--help` alongside `--no-handler-slices`. Wins over `--budgets ...` because the named flag captures more specific user intent.

New safety-zone helper `user_out_open_json_dump_gzip`. `--out OUT` help text and the run-command epilog point at the gzip incantation.
...
- **DEPENDENCY-linker TOML warning.** The `dependency` linker activates `always=True`, so the existing language-pair gate didn't suppress its partial-install warning on Go / JVM repos lacking TOML manifests. `partial_install_warnings.py` gains a per-requirement relevance gate: `LINKER_LANGUAGE_REQUIREMENTS` now records `dependency / toml_dependencies → [rust, python]`, and the warning loop suppresses any diagnostic whose unmet-requirement relevant-languages have zero overlap with `detected_languages`.

CGO / LUA_FFI / RUBY_FFI partial-install warnings were already gated by the existing `language_pairs` activation.

#### Harden remaining `git rev-parse <ref>` call-sites against unverified-ref stdout contamination

`scripts/release-check:95` was previously fixed by switching from `git rev-parse <ref>` to `git rev-parse --verify <ref>^{commit}`. The underlying invariant — `git rev-parse` without `--verify` echoes the input ref to stdout *and* returns non-zero when the ref is unresolvable, contaminating any command-substitution capture — applies to ten other sites in `scripts/auto-pr`, `scripts/lib/forgejo-api.sh`, and `scripts/ci-failover`. None currently bite (their refs `origin/dev` and local `dev` are effectively always resolvable), but they're hardened for consistency so the pattern isn't copy-pasted forward. Each becomes `git rev-parse --verify <ref>^{commit} 2>/dev/null || <fallback>`, with a rationale header comment near the first site in each file.

#### `hypergumbo run --out` help text surfaces side-output files at point of first contact

`--out analysis.json` also writes compact-tier previews (`<stem>.{4k,16k,64k}.json` per `--budgets`) and a `<stem>.slices/` directory of per-route handler slices — surprising users who expected one output file. Both suppression flags already existed; the `--out OUT` help line just didn't mention them. The help string now lists the side-output paths up front and points at the suppression flags; the `Examples:` epilog gains a `--budgets none --no-handler-slices` incantation producing exactly one output file. New `Side-outputs alongside --out:` epilog section documents the full layout. No behavior change.
...
- **JS/TS `access_mode` annotation coverage on call edges.** The YAMLs declared only three assignment-node shapes and one deletion shape; calls inside `return` / `throw` / `yield` / `await` stayed unclassified, leaving `--dataflow` slices useless on TypeScript repos (DEEP cohort 1: 0/61 NestJS and 0/80 Apollo sampled call edges had `access_mode != "none"`). Adds positional rules for return / throw / yield / await, an `update_expression` assignment rule, and expanded `library_patterns:` covering in-place mutators, ORM/persistence verbs, RxJS / EventEmitter / pub-sub, Promise / Observable readers, and iteration / collection readers. One test documents an out-of-scope `_build_line_index` limitation (trailing `;` of a `lexical_declaration` shadows contained siblings).

- **`is_utility_file` no longer false-positives on `<pkg>/utils/`, `<pkg>/contrib/`, `<pkg>/tools/`.** The ambiguous-name check fired at any depth, so `django/utils/html.py`, `requests/utils.py`, `urllib3/contrib/socks.py` etc. were all flagged. Concrete impact: django's `rslice.1.prod.json` collapsed from 75 non-test edges to 0 (every reachable caller path travelled through `django/contrib/admin/*` or `django/forms/utils.py`). `contrib` / `hack` now move from the unconditional `utility_dirs` set into `_AMBIGUOUS_UTILITY_DIRS`, and the ambiguous-dir check is tightened to `path_parts[0]` only (repo-root depth). Removes the now-unused `seen_source_root` tracker and `_SOURCE_ROOTS` constant.
...
- **`--backend rust-analyzer` install advice mentions `--force` and `pipx inject`.** Three integration-missing messages (the `add-extras` skip, the parse-time error, the `install-rust-analyzer` error) previously pointed users at `pipx install 'hypergumbo[rust-analyzer]'`, but pipx no-ops with `already seems to be installed` when bare `hypergumbo` is on the system (the common case). The messages now suggest `pipx install 'hypergumbo[rust-analyzer]' --force` or `pipx inject hypergumbo hypergumbo-lang-rust-analyzer`.

- **`yaml_catalogs` registry — `function_summaries` loader attribution corrected.** The `CatalogSpec` shipped in c00ec84615 named `hypergumbo_core.cli` as loader, but the real loader is `load_function_summaries()` in `hypergumbo_core.function_summaries` (`cli.py` only stashes the directory path, it doesn't parse YAML). The architecture doc's "YAML Catalogs (auto)" table picks up the correction on next regen. New property test `test_each_catalog_loader_module_is_importable` calls `importlib.import_module(spec.loader)` for every `CatalogSpec` and fails on stale or typo'd loader names.

- **`hypergumbo explain Symbol | head` no longer prints a BrokenPipeError traceback.** Piping `explain` (or any other long-stdout command) through `head` produced correct output followed by a traceback when the downstream pipe closed. `main()` now restores the POSIX default SIGPIPE handler (`signal.signal(SIGPIPE, SIG_DFL)`) at startup; Windows is unaffected (`signal.SIGPIPE` attribute absent; branch skipped).

### Documentation

- **ADR-0022 (Language Profile Registry) — status update.** Adds a "Status Update (2026-05-13)" section between Decision and Consequences. Records that the by-category drift-detection half landed (`hypergumbo_core.yaml_catalogs` + auto-generated `ARCHITECTURE.md` table) and the by-language `LanguageProfile` half is deferred, with three trigger conditions for revisiting. Original Decision text preserved.

- **ADR-0017 (Taint-Zone Dataflow) — implementation note for `taint_sinks/` absorption.** Hypergumbo no longer ships a built-in `taint_sinks/` directory; sinks are derived from `io_primitives/*.yaml` (every write-side IO primitive becomes an `untrusted` sink in a zone chosen by `AUTO_SINK_ZONE_MAP`). ADR §2b "Taint sink catalogs" opens with a note clarifying that the schemas still describe **project-local** sink catalogs passed via `--taint-sinks`. `taint.py` and `cli.py` docstrings updated; the dead `_TAINT_SINKS_DIR` binding is removed.
