...
- **Closes the ADR-0027 / ADR-0028 concept-axis program**: 111 `Edge.evidence_type` and 71 `Symbol.kind` endpoint_shape values retire from their registries, completing the producer-side fold.
- **Disambiguation-fallback discipline + new linter**: thirteen linker slices adopt the `confidence ≤ 0.5` + `meta["disambiguation_fallback"]=True` contract on ambiguous simple-name resolutions; a new fallback-coherence linter pins it statically and the producer-coherence linter learns to see through inline ternaries, f-strings, and Constant sentinels.
...
- **Per-entry-point safety claims** ship hypergumbo's own audited-IO-surface self-audit via a project-local taint catalog, wrapper discipline for fs-write sites, and a SECURITY.md generator.
...
The `view_template.py` linker (Rails) used to be the only producer of `renders` edges; its docstring claimed "Future: Django, Laravel, Phoenix, Spring" but no equivalent existed. The probe-and-emit core moves into `_view_template_core.py` and Django strategies land first; three sister implementations build the remaining strategies against it.
...
An ADR-0028-shape sibling field lands alongside Python reference adoption; the remaining 7 analyzers retrofit against it and four high-leverage consumers migrate, eliminating the historical Rust 6-segment dst fabrication at the producer.

- **`SCHEMA_VERSION` 0.7.1 → 0.7.2 → 0.8.0.** `Edge.dst_ref: ExternalRef | None = None` lands carrying the canonical `(lang, module_path, name)` identity, then becomes canonical at the producer with consumers preferring it over colon-split heuristics. The legacy `Edge.dst` string stays populated alongside as the back-compat / cache form. `Edge.create()` accepts `dst_ref` kwarg; `from_dict()` defensively reads `d.get("dst_ref")` so pre-0.7.2 cached JSON loads cleanly with `dst_ref=None`.
...
- **Per-analyzer adoption.** Java threads static-import maps through `_extract_edges`. Go adds `dot_imports` to `FileAnalysis` for `import . "strings"`. Elixir resolves aliased modules (`alias String, as: S` → `String`) and emits unresolved edges for `import Mod, only: [name: N]`. JS/TS attributes named-import calls to the canonical name (`writeFile`) not the local alias (`wf`). C++ pre-walks for system `#include` paths; unresolved external calls in files with system includes carry a comma-joined module hint. Rust consults `use_aliases` in three new cases, eliminating the historical 6-seg dst. Ruby emits a constant-external fallback so `Set.new(...)` and `JSON.parse(...)` are no longer silently swallowed.
...
- **Polyglot call-site coverage.** `test_polyglot_call_site_coverage.py` extends from 1 fixture (Python baseline) to 8; at audit time, 7 were `xfail strict=True`. All 7 `xfail_reason` fields are stripped — the test now demands 8 fixtures pass. Re-introducing an `xfail_reason` is a regression signal.

#### Per-entry-point safety claims + wrapper-function discipline

Lands the substrate for hypergumbo's own safety story: a per-entry-point taint-flow model that distinguishes what each CLI subcommand category is allowed to do, verified by `hypergumbo verify-claims docs/hypergumbo.claims.yaml`.
...
UAT 4.0.0 surfaced that contributors and agents are surprised when `--out analysis.json` writes additional files at the same prefix beyond the requested path: compact-tier previews (`<stem>.{4k,16k,64k}.json` per `--budgets`) and a `<stem>.slices/` subdirectory of per-route handler slices (per `--no-handler-slices` / `--max-handler-slices`). The capability to suppress both already existed; the `--out OUT` help line didn't mention them. The help string now lists the side-output paths up front and points at the suppression flags; the `Examples:` epilog gains a `--budgets none --no-handler-slices` incantation that produces exactly one output file. New `Side-outputs alongside --out:` epilog section documents the full layout. No behavior change.
