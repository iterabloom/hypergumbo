...
- **`verify-claims` no longer silently confirms `must_not_exist` boundary claims on an analysis blind to the I/O.** `verify_claim` returned `confirmed` whenever `chain_count == 0`, with no distinction between "the boundary genuinely has zero I/O" and "the boundary map couldn't see the I/O." Two ways the map is empty for the wrong reason: an analysis that produced no call edges at all (empty repo / wrong cwd — INV-bitig), and a *supported* language analyzed but producing zero call edges, so `io_boundary` tagged none of its I/O (F69.A1 — e.g. a Node+Python service that provably does `http.get` / `fs.readFileSync` / `child_process.exec` getting `confirmed` on all four `must_not_exist` claims at exit 0). A new `BoundaryCoverage` signal — derived from call-edge production per supported language (single-sourced `io_boundary.IO_CALL_EDGE_TYPES`), *not* from the dead `limits.skipped_languages` field (WI-nihir) — now downgrades a would-be `confirmed` zero-chain `must_not_exist` / within-limit `max_chains` verdict to `inconclusive` (CLI exit 2) when coverage is incomplete. Coverage never masks a real `violated` verdict: found evidence is positive regardless of blind spots. Closes WI-kajil (P0) and INV-bitig (P0).
- **`verify-claims` taint propagation now honors the module qualifier and `ambiguous_names`, ending a false-VIOLATION cascade.** Both propagation passes (`propagate_taint_structural` and `propagate_taint_ddg`) indexed sources/sinks by bare callee name and matched purely on the name with no module or ambiguity gate — so every `str.replace` / `dict.replace` call matched the filesystem-write `Path.replace` sink and `sys.stdout.write` mis-routed into the `asyncio.StreamWriter.write` **net_send** sink (F156.A1), producing thousands of false `violated` rows on the project's own self-claims doc. Matching now mirrors `io_boundary.IoBoundaryCatalog.lookup_with_module` (the sibling consumer of the same catalog that already did this right, INV-maluk) via a shared `_lookup_named_entry` helper: a *resolved* first-party callee matches by exact name; an *unresolved* callee with a module hint filters by `io_boundary._module_matches` (returns no match when the module differs), and an `ambiguous_names` short name (`replace`/`write`/`run`/…) with no usable module hint matches nothing instead of the first entry. The per-language `ambiguous_names` (loaded from `io_primitives/*.yaml`) is threaded onto `TaintCatalog` and passed into propagation by `cmd_verify_claims`; the `TaintCatalog.match_sink` / `match_source` methods were hardened the same way. End-to-end on the self-claims doc, violated evidence dropped from 5,975 to 1,266 (the `replace`→host_fs and `write`→network cascades eliminated; two claims flipped to confirmed); module-mismatched matches (`os.environ.copy`, `uvicorn.run`) are suppressed while genuine module-matched flows (`subprocess.run`, `shutil.copy`) are retained — a real chain is never downgraded. Closes WI-razol (P0). (A small residual — `copy` is not yet in the catalog's `ambiguous_names` — is tracked separately.)
- **`verify-claims --taint-sources/-sinks/-sanitizers` now actually override claims-file `extra_catalogs` entries.** `cmd_verify_claims` concatenated the CLI flag paths and the claims-file `extra_catalogs` paths into one user layer before `load_full_taint_catalog`, and that layer had no intra-layer dedup — so a CLI `--taint-sources` entry matching a claims-declared `(module, name, kind)` triple was *added* as a duplicate rather than *replacing* it, silently breaking the documented override and the CLI status line's promise (a downstream project narrowing its threat model via `--taint-sources` got a false result). `load_full_taint_catalog` now takes the claims-file extras (`extra_*_paths`) and CLI extras (`cli_*_paths`) as **distinct layers** and applies `_merge_with_user_override` between them — CLI wins over claims-file on `(module, name, kind)` for sources/sinks (sanitizers concatenate) — before the unified user layer overrides the built-in catalog. End-to-end: a `--taint-sources` file that relabels a claims-file-declared source now displaces it, flipping the dependent claim's verdict. Closes INV-hukug (P1).
- **`verify-claims` now validates the claims file up front instead of tracebacking or silently confirming on bad input.** `load_claims` had no input validation: a malformed YAML raised a raw `yaml.YAMLError` traceback; a non-mapping root (scalar / top-level list / binary / directory) or a non-list `claims:` raised a bare `AttributeError`/`TypeError`; an unknown `constraint.boundary` value (e.g. `network`, `banana`) made `verify_claim`'s `boundary_map.entries.get(...)` return `None` → chain count `0` → a silent `must_not_exist` **confirmed** against a boundary the analyzer never produces; and a typo'd field name (`constrant`, `must-not-exist`) was silently dropped into a defaults-populated claim whose `inconclusive` verdict was indistinguishable from a genuinely-uncheckable one. `load_claims` now raises a single `ClaimsFileError` (→ CLI exit `2`, with a clean stderr message, never a traceback) for: directory/non-UTF-8/unparseable input; an unexpected root, claim, or constraint shape; an unknown top-level / claim / constraint / taint-flow field name (with a did-you-mean hint); and a `constraint.boundary` outside the canonical io-boundaries vocabulary. The vocabulary is single-sourced from `io_boundary.CATALOG_BOUNDARY_TYPES` / `KNOWN_IO_BOUNDARIES` (validated against the full universe of boundary names, not the keys present in a given map, so `must_not_exist: net_send` still confirms on a repo with zero net-send chains). `claims: null` / `~` / `[]`, `{}`, and an empty file continue to load as zero claims. `verify-claims --help` now documents the claims YAML shape and exit codes. Closes the META-jurig `load_claims` family (INV-zurih, WI-fuhaf, INV-gobob/WI-ruzib, WI-bopoz).
- **`verify-claims --taint-sources/--taint-sinks/--taint-sanitizers` no longer silently confirm — or traceback — on a bad taint-catalog path.** Two gaps closed under INV-nufob. (1) The taint block ran only when a claim carried a `taint_flow` constraint (`if has_taint_claims`), so `--taint-sources <bad-path>` alongside boundary-only claims silently fell through to "all CONFIRMED" at exit 0 — the flag was never even resolved. The CLI now resolves and validates the `--taint-*` (and claims-file `extra_catalogs`) paths whenever they are present, regardless of whether any claim consumes them: a nonexistent / malformed / wrong-shape path errors at exit `2` (inconclusive — a broken taint config can never be a `confirmed`/`violated` verdict), and valid-but-unused flags print a warning instead of being silently ignored. (2) The catalog loaders (`_load_source_yaml` / `_load_sink_yaml` / `_load_sanitizer_yaml`) called `yaml.safe_load` with no error handling and no shape check, so a malformed file raised a raw `yaml.YAMLError` traceback and a wrong-shape file (e.g. `sources:` a scalar) a bare `AttributeError`. A new `TaintCatalogError` — the single umbrella for taint-catalog load failures (parse error, non-mapping document, wrong-typed `sources`/`sinks`/`transforms` section, invalid `start_at`), raised via a shared `_safe_load_catalog_yaml` helper — now surfaces a clean stderr message (→ exit `2`), never a traceback. The pre-existing nonexistent-path behavior is realigned from exit 1 to exit 2 for the same reason (a load failure is not a violation). Closes INV-nufob (P3) — reopened during the v6.0.0 release re-verification after its original closure was found to have credited the unrelated `ClaimVerdict.inconclusive` (INV-mofih) fix, which never touched the taint-flag loaders.
...
- **`hypergumbo run` no longer touches the network on cached embedding loads.** The embeddings model loader passed `local_files_only=True`, but HF Hub's metadata API, the xet cache-freshness ping, and the `transformers` safetensors auto-conversion *background thread* still issued outbound `huggingface.co` requests on every runtime invocation — violating the `runtime-cli-no-network` claim that the generated `SECURITY.md` advertises. The fix forces `HF_HUB_OFFLINE=1` **before the first `huggingface_hub` import** (its offline switch is frozen into a module constant at import time and is not re-read from the environment afterward — so setting it inside the loader was too late), gated on every embedding model already being cached (a filesystem probe of the hub cache) so the one-time first-install download is unaffected. Verified end-to-end with a process-global socket guard: a cached `_load_embedding_model()` + `encode()` now makes zero non-loopback connections (previously it connected out and emitted the "unauthenticated requests to the HF Hub" warning). A new "Environment variables" section in `docs/hypergumbo-spec.md` documents `HF_HUB_OFFLINE`, `HYPERGUMBO_VERBOSE`, and `HYPERGUMBO_MIN_MEMORY_MB` (previously undocumented).
