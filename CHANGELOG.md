...
- **Route deduplication preserves cross-file registrations**: The `routes` command now keeps route symbols from different files even when they share the same (method, path). Previously, alertmanager v2 API routes (go-swagger handler cache in `alertmanager_api.go`) were deduplicated against v1 deprecation routes in `v1_deprecation_router.go`, hiding 6 of 9 v2 handlers. Concept-enriched handlers still dedup against materialized route symbols.
