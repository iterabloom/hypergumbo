...
- **Rust**: ListNameResolver with `ambiguity_threshold=3` replaces ad-hoc `min(1, 3/N)` penalty for method calls. Method calls with 3+ candidates now produce no edge (previously: low-confidence edge to arbitrary target).
