...
- **`SCHEMA_VERSION` bumped 0.6.0 → 0.7.0 (ADR-0028 §6 Phase 4b — hard contract change).** All 111 endpoint_shape values on `Edge.evidence_type` removed from `EVIDENCE_TYPES` after Phase 3 producer migration completed across Waves 3-5 (audits 0008, 0012, 0014) and the corresponding per-cluster bakeoff-validation tags cleared. Removed:
  - **Cluster B resolution-status leakage (18):** `ast_annotation_unresolved`, `ast_attribute_unresolved`, `ast_call_unresolved_import`, `ast_decorator_unresolved`, `ast_method_unresolved_*`, `chained_call_unresolved`, `django_signal_receiver_unresolved`, `grpc_unresolved_resolution`, `luajit_ffi_unresolved`, `ruby_ffi_attach_unresolved`, `trait_impl_unresolved`, `unresolved_*` → fold to canonical inference label + `Edge.is_resolved=False` per audit-findings 0008 (WI-nunal Wave 3).
  - **Cluster C framework-specific dispatch (65):** `django_orm_dispatch`, `kafka_streams_dispatch`, `phoenix_event_match`, `nestjs_module_registration`, `vue_event_handler`, `activerecord_association`, `rails_callback`, `airflow_framework_dispatch`, `tauri_emit_listen`, `wasm_bindgen_import`, `route_mount`, `resolver_*`, `grpc_*`, `openapi_*`, dynamic `socketio_emit` / `native_emit` / `ws_emit` / `fastapi_emit` / `django_channels_emit` (and `_endpoint` variants) → fold to canonical inference + `meta["framework_dispatch"]` / `meta["detection_pattern"]` per audit-findings 0014 (WI-kagik Wave 5).
  - **Cluster D apex/peer call constructs (28):** `function_call`, `method_call`, `ambiguous_method_call`, `bare_method_call`, `call`, `chained_return_type_call`, `constructor_call`, `cross_file_call`, `pipe_call`, `remote_call*`, `stdlib_method_call`, `typed_*_call`, `unexported_method_call`, `function_application` → fold to `ast_call` apex + `meta["call_construct"]` (and `meta["receiver"]` / `meta["resolution_quality"]` as appropriate) per audit-findings 0012 (WI-nibis Wave 4).
  `AXIS_ENDPOINT_SHAPE` constant retained as a backwards-compat import target (no longer in `VALID_AXES`, no live spec carries it). `generate-concept-axes` drops the evidence_type endpoint_shape section (the section was always contingent on the registry having endpoint_shape entries). 91 audit-findings rows across docs 0008, 0012, 0014 advance PRELIM_RESOLVED → RESOLVED. Closes WI-porim / WI-runod Wave 8 on the Edge.evidence_type axis — the underlying program for ADR-0027 + ADR-0028 cross-axis Phase 3+4 migration is now complete.

- **`SCHEMA_VERSION` bumped 0.5.8 → 0.6.0 (ADR-0027 §6 Phase 4b — hard contract change).** All 71 endpoint_shape values on `Symbol.kind` removed from `SYMBOL_KINDS` after Phase 3 producer migration completed across Waves 3-6 and per-cluster bakeoff validation cleared. Removed:
  - **Cluster C apex/peer (4):** `fn`, `proc`, `structure`, `var` → fold to canonical `function` / `procedure` / `struct` / `variable` per audit-findings 0009.
  - **Cluster D framework roles (29):** `event_publisher` / `event_subscriber` / `ipc_*` / `crypto_*` / `message_*` / `mq_*` / `grpc_*` / `websocket_*` / `dispatcher` / `graphql_*` / `http_client` / `route*` / `openapi_operation` / `objc_bridge` / `selector_ref` / `rpc` / `service` → fold to canonical `function` / `method` / `interface` / `class` + `meta["framework_role"]=<role>` per audit-findings 0013.
  - **Cluster E edge labels (9):** `call` / `inherit` / `read` / `write` / `include` / `extends` shipped DEPRECATE-NO-FOLD (the relationship lives on the Edge); `function_call` / `subprocess_call` / `db_query` / `abi_call` folded to `call_site` + `meta["call_kind"]` per audit-findings 0010.
  - **Cluster F component refs (1):** `component_ref` shipped DEPRECATE-NO-FOLD per audit-findings 0011.
  - **Cluster B file-shape (11):** `module_file`, `component_file`, `npm_package`, `composer_package`, `main_entry`, `library_export`, `export_entry`, `wasm_module`, `wasm_import`, `tsconfig`, `script` → fold to canonical `file` / `package` + meta keys (`module_system`, `component_framework`, `package_ecosystem`, `entry_role`, `config_format`) per audit-findings 0005.
  - **Cluster G build/config (9):** `test_case` / `editable` / `url_requirement` / `python_task` / `devDependency` / `dev-dependency` / `build-dependency` / `config` / `work_item` → fold to canonical `test` / `requirement` / `dependency` / `task` / `block` + meta keys (`test_dialect`, `install_mode`, `install_source`, `task_implementation`, `dependency_scope`, `block_type`) per audit-findings 0006.
  - **Cluster H long-tail (8):** `heading`, `model`, `unresolved` shipped DEPRECATE-NO-FOLD (no producer or pseudo-symbol shape); related Cluster H residuals already shipped in Wave 6 PRs 1-6 per audit-findings 0007.
  Consumer-side cleanup sweep: `selection/filters.py::EXCLUDED_KINDS` drops 5 entries and gains a sibling `EXCLUDED_FRAMEWORK_ROLES` set so the post-fold `is_excluded_kind` dual predicate can address the meta-key vocabulary directly; `linkers/registry.py::SYNTHETIC_KINDS` renamed to `SYNTHETIC_FRAMEWORK_ROLES` with the pre-Phase-4b legacy Symbol.kind branch removed; `linkers/containment.py::CONTAINABLE_KINDS` drops `rpc`; `linkers/containment.py::CONTAINER_KINDS` drops `service`; `cli.py::_DEPENDENCY_KINDS` collapses to `{"dependency"}`; `cli.py::_NOISE_KINDS` drops `heading` / `config` / `script`; `sketch.py::KEY_SYMBOL_KINDS` drops `structure`. 72 audit-findings rows across docs 0005-0011, 0013 advance PRELIM_RESOLVED → RESOLVED. `AXIS_ENDPOINT_SHAPE` constant retained as a backwards-compat import target (no longer in `VALID_AXES`, no live spec carries it). Closes WI-butol / WI-runod Wave 8 on the Symbol.kind axis.

### Added

#### INV-zuhub item 1 conformance — disambiguation_fallback discipline per linker

Nine linker families adopt the `confidence <= 0.5` + `meta["disambiguation_fallback"] = True` contract for simple-name resolutions whose chosen target is structurally ambiguous. Each entry below is one PR-shaped slice of the program; combined they close INV-zuhub item 1 except for the two follow-on WIs explicitly tracked (WI-husor JNI dual-key, WI-sifiz PyFFI three-key).

- **INV-zuhub item 1 conformance for the Go analyzer's interface_dispatch short-name resolution (WI-kogid).** The Go analyzer's WI-likab residual case named explicitly in INV-zuhub: when a Go method call `x.Method()` cannot be resolved to a typed receiver and the short name resolves to >=2 global candidates, the analyzer preferentially picks an interface-method candidate so that the type-hierarchy linker's `dispatches_to` edges can route the slice through concrete implementations. The pre-fix code emitted these `evidence_type="interface_dispatch"` edges at a flat `confidence=0.75` regardless of whether the interface-method preference uniquely disambiguated, missing the INV-zuhub fallback contract when >=2 of the candidates were themselves interface methods (e.g. `r.Close()` where both `io.ReadCloser.Close` and a project-local `Worker.Close` interface method are in scope). Fix:
  - Collect ALL interface-method candidates from `global_symbols[callee_name]` rather than breaking on the first match.
  - When exactly one interface method is among the candidates, the interface-method check IS the precision disambiguator → keep `confidence=0.75` and emit with `meta["call_construct"]="interface_dispatch"` (no fallback flag).
  - When >=2 interface-method candidates exist, pick the deterministic-by-id minimum and emit with `confidence=0.5` + `meta["disambiguation_fallback"]=True` per INV-zuhub.
  - The Edge.create call uses the canonical IfExp pattern on `_is_iface_fallback` for both `confidence` and `meta`. The L4 fallback-coherence walker is scoped to `packages/hypergumbo-core/src/hypergumbo_core/linkers/` by default and does not statically gate this language-analyzer site (per the WI-kogid scope note); the conformance is verified by runtime property tests.
  Adds 2 property tests under a new `TestINVZuhubConformanceForInterfaceDispatch` class (single-interface-method-precision regression guard, multi-interface-method-fallback). This closes one of the three INV-zuhub item 1 follow-on WIs (WI-husor JNI dual-key and WI-sifiz PyFFI three-key remain).
- **INV-zuhub item 1 conformance for `route_handler.py` symbol_by_name primary index (WI-rusuh).** The route-handler linker's `symbol_by_name: dict[str, Symbol]` is single-valued with a pre-existing partial mitigation (prefer non-route over route on overwrite). The non-route vs non-route collision case — two Django views both named `list_users`, two Express functions both named `list`, two un-qualified Ruby methods named `index`, etc. — remained unresolved: the dict overwrote silently, and downstream bare-name resolution lands on whichever non-route was inserted first. This is the BUG-04/05 shape (WI-milak / WI-kunoz catalog) applied to the route_handler primary index. Fix:
  - `link_routes_to_handlers` builds `name_collisions: frozenset[str]` up front — names (and `meta["qualified_name"]` keys) shared by 2+ non-route symbols. This is the structural-ambiguity oracle.
  - At each successful handler resolution (main `dispatches_to` edge AND React Router loader/action edges), the resolved Symbol's `name` and `meta["qualified_name"]` are checked against `name_collisions`. A hit drops the edge to `confidence=0.5` + `meta["disambiguation_fallback"]=True`; a miss keeps the existing confidence (0.9 main, 0.85 loader/action).
  - The Edge.create calls use the canonical IfExp pattern on `handler_is_fallback` / `la_is_fallback` (dict-spread True branch carries the flag) so the L4 fallback-coherence walker can statically gate the conformance.
  Adds 5 property tests under `TestINVZuhubConformanceForRouteHandler` (Django bare-name collision drops to fallback, Django unique-name precision regression guard, Express bare-name collision, Rails qualified-name immune to unrelated bare collision, React Router loader-ref collision). The 6 internal resolvers (`_resolve_rails_handler`, `_resolve_laravel_handler`, `_resolve_phoenix_handler`, `_resolve_express_handler`, `_resolve_django_handler`, `_resolve_go_handler`) keep their existing signatures — the conformance is enforced at the caller via post-resolution check on the resolved Symbol's name keys, not by changing resolution logic. This is the conservative INV-zuhub-conformant minimum: it flags edges whose resolved symbol's name is structurally ambiguous in the codebase, regardless of which key the resolver consulted. The audit's optional precision improvement (extend multi-value `symbols_by_short_name` to all resolution paths, or scope the primary index by `(file_path, symbol_name)`) remains future work; the conformance gate is closed.
- **INV-zuhub item 1 conformance for `grpc.py` service-symbol lookup (WI-patiz).** The same `link_grpc` function whose Go method-mapping bug WI-kunoz / BUG-05 fixed (PR #3598) carries a second instance of the BUG-04/05 single-value-dict shape: `service_sym_by_name: dict[str, str]` keyed on the bare proto service short name, used by both the routes_to lookup (route → service) and the bridge dispatches_to (grpc_server/servicer → grpc_service). Two `.proto` files declaring `service Foo` in different packages (the containerd-style `services/containers/v1/containers.proto` + `services/containers/v2/containers.proto` shape) collapsed onto whichever file iteration processed last; routes_to and bridge edges from one package's RPCs landed on the other package's service symbol. Fix:
  - `GrpcPattern` gains a `package: str = ""` field; `_scan_proto_file` propagates the proto `package` declaration to each `type="service"` pattern.
  - `grpc_service` Symbols carry `meta["proto_package"]=<package>` when the .proto declares one (omitted when no `package` line). The new meta key joins `framework_role` on the service Symbol's meta.
  - `service_sym_by_name` and `service_by_normalized` become `dict[str, list[Symbol]]`.
  - **routes_to lookup** uses `rpc.package` as a precision disambiguator: when exactly one candidate's `meta["proto_package"]` matches the RPC's package, the route → service edge is precision (no flag, confidence=0.90). When the package match is missing or ambiguous (multiple same-package or no same-package candidates), it drops to `confidence=0.5` + `meta["disambiguation_fallback"]=True`. Single-candidate (no collision) stays precision.
  - **Bridge dispatches_to** has no impl-side proto package metadata — Go struct embeddings (`pb.UnimplementedXxxServer`), Python servicer classes, etc. carry no `proto_package`. Multi-candidate normalized matches always drop to `confidence=0.5` + `disambiguation_fallback=True`; single-candidate stays at 0.90.
  Adds 5 property tests under a new `TestINVZuhubConformanceForServiceLookup` class (cross-package routes precision via package, cross-package bridge fallback, single-service bridge precision regression guard, proto_package meta presence, proto_package meta absence on package-less protos). All existing 60+ grpc-linker tests stay green; the L4 fallback-coherence walker continues to pass (both new fallback emit-sites use the canonical IfExp form).
- **INV-zuhub item 1 conformance for `subprocess_cli` (WI-jifiv).** The protocol linker that connects Python `subprocess.run(["myapp", "list"])` invocations to CLI command symbols (concept=command, decorated with `@click.command` / `@app.command` / Django `BaseCommand` / Flask CLI / etc.) previously kept a single-value `command_by_name: dict[str, Symbol]` lookup keyed by the bare command function name. In projects whose `pyproject.toml` declares multiple `[project.scripts]` entries (each shipping its own click/typer command tree), two binaries can each register a top-level subcommand with the same short name (e.g. both `myapp` and `myapp-helper` define a `list`). The pre-fix dict overwrote silently — last command symbol scanned won, so every `subprocess.run(["myapp", "list"])` call resolved to whichever binary's `list` was inserted last. Per the BUG-04/05 fix-pattern catalog (WI-milak PR #3597, WI-kunoz PR #3598): `command_by_name` is now `dict[str, list[Symbol]]`; multi-candidate resolution picks deterministic-by-id and drops the edge to `confidence=0.5` with `meta["disambiguation_fallback"] = True` alongside the existing `meta["detection_pattern"] = "subprocess_cli"` and other call-context keys. Single-candidate resolutions stay at the base confidence (0.85 literal, 0.80 python-m, 0.70 variable) with no flag. +2 property tests (collision-drops-to-fallback, no-collision-preserves-base-confidence). The Edge.create call also moves from the mutation-form `edge.meta = {...}` post-assign to the canonical kwarg `meta=edge_meta` IfExp pattern so the L4 fallback-coherence walker can statically gate the conformance.
- **INV-zuhub item 1 conformance for `method_call_recovery` (WI-ruzin PR2).** The Path B' chained-method recovery linker (WI-gigoz) resolves `CliRunner().run(args)` by pairing each class-hint edge with a sibling `unresolved-call` edge and picking the candidate whose hint is closest in line number to the call site. When more than one class hint each contains a method matching the unresolved short name, the line-proximity tiebreaker is a heuristic — the chosen method may not be the runtime dispatch target. Such resolutions now drop to `confidence=min(base_confidence, 0.5)` (caps below the ceiling without raising sub-0.5 confidences) and carry `meta["disambiguation_fallback"] = True` alongside the existing `meta["call_construct"] = "method"` and `meta["resolution_quality"] = "recovery"`. Single-hint resolutions stay at the base confidence with no flag. +2 property tests.
- **INV-zuhub item 1 conformance for the simple FFI-bridge `calls` subset (WI-ruzin PR1).** Four FFI bridges that resolve C/C++ functions by short name now downgrade cross-file collisions. Each previously kept a single-value `dict[str, Symbol]` lookup keyed on the bare function name — first-writer-wins on any cross-file collision (the Apr 2026 Go `Notifier` struct shape; commit d52a1ee2b).
  - `linkers/cgo.py`: `_build_c_function_lookup` returns `dict[str, list[Symbol]]`. When `C.funcName()` resolves to more than one in-tree C/C++ function symbol, pick deterministic-by-id and emit `confidence=0.5` + `meta["disambiguation_fallback"] = True`. +2 property tests.
  - `linkers/napi.py`: multi-value `c_cpp_lookup`; export_map now carries `is_fallback` per export. Same downgrade rule. +2 property tests.
  - `linkers/lua_ffi.py`: multi-value `c_lookup` shared between both ffi.C / ffi.load phase 1 and Phase 2 unresolved-call match via a new `_resolve` closure that returns `(c_sym, is_fallback)`. +2 property tests.
  - `linkers/ruby_ffi.py`: multi-value `c_lookup`. Both phases (FFI `attach_function` and C `rb_define_method`) carry the fallback flag — `_scan_c_file_for_rb_define` returns `list[tuple[Symbol, int, bool]]`. +2 property tests.
- **INV-zuhub item 1 conformance for `jackson_dispatch` and `go_cobra` (WI-bojok PR4).** Two more linkers join the conformance pattern using the shared helpers from PR3:
  - `linkers/jackson_dispatch.py`: `_find_bean_target_classes` now returns `list[tuple[Symbol, bool]]` paired with an `is_fallback` flag. The class-level / method-level Jackson annotation paths stay precision (the Jackson / JPA / Spring annotation namespaces are the canonical disambiguators). The bean-marker base path (`ConfigurationProperties` short-name match) is flagged when the raw entry is unqualified AND the short name collides with an in-tree JVM class. Mixed paths (one precision base + one fallback base) resolve to precision. `_JACKSON_BEAN_FQN_PREFIXES = ("org.springframework.", "jakarta.persistence.", "javax.persistence.")` is the precision-qualifier set. +4 property tests + 5 existing test sites updated to unpack the new tuple shape.
  - `linkers/go_cobra.py`: `ctx.find_symbols_by_name(handler_name)` returns every in-tree Go symbol whose short name matches. When the lookup yields more than one candidate (cross-package short-name collision, common in Go where each package may declare its own `Run` / `Execute` / `Validate`), the dispatch target is unresolvable from the cobra assignment alone — every emitted edge in the batch is a simple-name fallback. +2 property tests (single-candidate precision, multi-candidate emit-all fallback).
- **INV-zuhub item 1 conformance for the framework-base-class `dispatches_to` subset (WI-bojok PR3).** Two more linkers join the conformance pattern established in WI-bojok PR1 (kafka), now sharing a single reusable helper:
  - **Shared helper at `hypergumbo_core.linkers._transitive_bases`.** New `build_short_name_collisions(symbols, keys, kinds, languages)` returns the subset of `keys` that have an in-tree symbol collision. New `short_name_fallback(raw_entry, short_name, in_tree_collisions, fqn_prefixes)` returns `True` iff the match is a simple-name fallback under INV-zuhub. The pair captures the contract once for every framework-base-class dispatch linker (kafka_streams, django_orm, airflow, and future jackson PR4 work). +7 new helper tests in `test_transitive_bases_helper.py`.
  - `linkers/django_orm_dispatch.py`: `_find_django_subclasses` now returns a 3-tuple `(class_sym, methods, is_fallback)` and consults the new collision set against `DJANGO_BASE_METHODS` keys. Raw entries prefixed with `django.` (e.g. `django.db.models.Model`) are precision; unqualified entries (`Model`) with a Python in-tree collision are fallback. Edges drop to `confidence=0.5` with the disambiguation_fallback flag in the mutation form. +4 property tests (FQN-precision, no-collision-precision, deterministic-fallback, cross-language-collision-doesn't-trigger).
  - `linkers/airflow_framework_dispatch.py`: same shape with `airflow.` FQN prefix and `AIRFLOW_BASE_METHODS` keys. +3 property tests + 1 existing test updated to unpack the new 3-tuple shape.
  All four conformance shapes (kafka inline, django/airflow via shared helper) share the L4-compatible mutation-form meta dict.
- **INV-zuhub item 1 conformance for the emit-all-matches `dispatches_to` subset (WI-bojok PR2).** Two linkers that emit one `dispatches_to` edge per short-name candidate now downgrade the entire candidate batch when the short name is structurally ambiguous:
  - `linkers/annotation_convention.py`: `@hg:dispatches X` matches every in-tree symbol whose `Symbol.name == X`. When `sym_by_name[X]` yields more than one candidate, the developer's intended target is unresolvable from the directive alone; each emitted edge drops to `confidence=0.5` and carries `meta["disambiguation_fallback"] = True` alongside the existing `meta["mechanism"] = "annotation"`.
  - `linkers/di_resolution.py::_create_di_edges`: for each interface method, every impl-class method that shares the short name receives an edge. When `impl_by_short[short]` yields more than one candidate (Java/Kotlin/Scala overloads), confidence is `min(binding.confidence, 0.5)` (caps below the ceiling without raising sub-0.5 confidences) and the disambiguation_fallback flag is set. Mirrors the mutation-form meta shape used by the sibling `_create_di_registers_edges` (WI-harim) for consistency across the DI linker.
  Adds 6 new property tests (3 per linker). The L4 fallback-coherence gate continues to pass: the IfExp shape on `annotation_convention` is the canonical inheritance.py form, and the `di_resolution` mutation form is structurally the same shape as the sibling function the L4 walker already accepts.
- **INV-zuhub item 1 conformance for `kafka_streams_dispatch` (WI-bojok PR1).** The single named concrete violation in INV-zuhub's statement table — JVM `dispatches_to` from `kafka_streams_dispatch` for the `Transformer<T>` simple-name collision (WI-lisov discuss-note round 08 / DQ-14). The linker matches a class's transitive base chain against the literal `KAFKA_STREAMS_CALLBACKS` short-name set; an in-tree JVM type that shares the short name of a kafka callback interface (e.g. an oauthbearer-side `class Transformer<T>` unrelated to `org.apache.kafka.streams.kstream.Transformer`) creates a structural ambiguity the static analysis cannot resolve. The fix introduces `_callback_interface_matches` (preserving the raw analyzer entry alongside the short name), `_build_in_tree_callback_name_collisions` (an FQN-prefix-aware index of in-tree {java,kotlin,scala} types whose short name lives in `KAFKA_STREAMS_CALLBACKS`), and `_is_fallback_match` (precise when the raw entry is `org.apache.kafka.*`-qualified OR when no in-tree collision exists; fallback when an unqualified raw entry collides). Fallback dispatch edges drop to `confidence=0.5` and carry `meta["disambiguation_fallback"] = True` alongside the existing `meta["framework_dispatch"] = "kafka_streams"`. Adds 8 new tests (3 helper-shape, 4 INV-zuhub property tests, 1 partial-prefix ambiguity test) and keeps the L4 fallback-coherence gate clean (the new conditional dict/confidence pair is the canonical IfExp-with-shared-predicate shape).
- **INV-zuhub item 1 conformance for the `references` edge-type family (4 linkers).** Closes WI-harim. The disambiguation_fallback discipline established for the inheritance linker in PR #3545 now extends to every linker that emits `references` edges via simple-name resolution:
  - `linkers/orm.py`: `_build_model_lookup` now returns `dict[str, list[Symbol]]` instead of dropping collisions at index time; `_resolve_model_with_fallback` applies the same-file-preferred / deterministic-by-id cascade at edge-creation time. Single-candidate model references stay at `confidence=0.85` with no flag; multi-candidate collisions with a same-file match win precision; cross-file ambiguity drops to `confidence=0.5` + `meta['disambiguation_fallback']=True`.
  - `linkers/database_query.py`: SQL table lookup similarly switches to multi-candidate (a Postgres and a SQLite migration both declaring `users` no longer silently picks last-writer-wins); deterministic-by-id fallback when names collide on the lower-cased key. No same-file branch (SQL DDL doesn't share files with application queries by convention).
  - `linkers/di_resolution.py::_create_di_registers_edges`: NestJS `@Module` provider lookups switch to multi-candidate per short class name; deterministic-by-id fallback for monorepo cross-package collisions. Confidence is `min(binding.confidence, 0.5)` when fallback (caps below ceiling without raising sub-0.5 confidences).
  - `linkers/react_component.py`: `_build_component_map` switches to multi-candidate; `_resolve_component_with_fallback` applies the cascade for JSX use sites. Same-file Button gets precision over cross-file Button collisions.
  Adds 12 new property tests across the four linkers (3 each: single-candidate-precision, same-file-preferred-or-no-same-file-branch, deterministic-fallback-with-flag). Mirrors the inheritance-linker test shape from PR #3545. The L4 fallback-coherence gate (PR dd2a93550e) continues to pass since every site that sets the flag also sets `confidence <= 0.5` in the same `Edge.create` call.

#### Axis-discipline gates (L3 / L4)

- **L4 fallback-coherence linter at `hypergumbo_core.fallback_coherence` and CLI script `scripts/check-fallback-coherence`.** Closes INV-zuhub item 2 (AST-walking property test asserting every fallback-emitting linker code path sets `confidence <= 0.5` and `meta["disambiguation_fallback"] = True`). The walker descends each `Edge.create(...)` / `Edge(...)` call site under `packages/hypergumbo-core/src/hypergumbo_core/linkers/`; for every site that statically declares `meta["disambiguation_fallback"] = True` (inline dict, inline IfExp with flag-bearing True branch, or single function-local Name assignment to either shape), it requires `confidence` on the same call to be statically resolvable to a value <= 0.5. Predicate-aware resolution: when both fields are IfExp ternaries on the same predicate, only the True branch of confidence is checked (the inheritance.py canonical pattern); when predicates differ or are absent, all candidates are checked. Live-tree property test at `tests/test_fallback_coherence.py::test_no_fallback_coherence_violations_in_linkers` runs on every commit. The L4 ladder rung sits below the L1 axis-drift linters (`check-edge-type-drift` etc) and the L3 producer-coherence linter (`check-producer-axis-coherence`); collectively they form the L1-through-L4 axis-discipline gates. The companion meta-key spec `disambiguation_fallback` was added to `axis_meta_keys.py::AXIS_EDGE_META` so the canonical key name lives in the registry rather than being duplicated. Currently only `linkers/inheritance.py` sets the flag; per-linker conformance for the four remaining edge-type families (calls / dispatches_to / references / module_exports) is tracked under INV-zuhub item 1 as separate work items.
- **Pre-commit hook wiring for the L4 fallback-coherence gate (WI-fikib).** Adds a block to `.githooks/pre-commit` that runs `scripts/check-fallback-coherence` whenever a `packages/hypergumbo-core/src/hypergumbo_core/linkers/*.py` file is staged. Parallel to the existing L1 (axis-drift) and L3 (producer-axis-coherence) hook blocks. Faster-feedback equivalent of the live-tree CI property test — catches violations before they enter the commit instead of after the push. Closes INV-zuhub item 3.

- **L3 producer-coherence linter now resolves inline ternaries, f-strings (via expansion), and skips non-string-Constant sentinel assignments.** Three coordinated fixes close blind spots in the static walker that previously silently skipped real producer-side leaks:
  - **Inline ternary classifier:** `Edge.create(..., evidence_type="a" if cond else "b")` at the kwarg site now resolves through `_resolve_simple_rhs` (previously only resolved when bound to a function-local name). Surfaced two pre-existing leaks: `linkers/ipc.py:546` (`ipc_channel_match`) and `linkers/message_queue.py:516` (`topic_match`).
  - **Non-string Constant sentinel:** `_resolve_simple_rhs` returns an empty frozenset (not `None`) for `= None` / `= 42` / `= True` assignments, so the function-local walker no longer poisons when a Symbol/Edge kwarg variable is initialized to a sentinel before its real string assignments (`linkers/inheritance.py:209` shape).
  - **F-string expansion (WI-nubuv ext B):** new `fstring_mode: "advisory" | "expand" | "strict"` parameter on `find_producer_coherence_violations` and the three per-axis wrappers. `expand` (now the wrapper default) tries to resolve every `FormattedValue` segment through the function-local walker and checks the Cartesian product against the registry; `strict` promotes unexpandable f-strings to violations. Capped at 32 expansions to prevent combinatorial explosion. `linkers/inheritance.py:250` (`evidence_type=f"ast_{edge_type}"`) now silently expands to `{ast_extends, ast_implements}` instead of surfacing as an advisory.
  - **Variable-form structural backstop (WI-nubuv ext C):** new `variable_form_mode: "silent" | "advisory" | "strict"` parameter. `silent` is the wrapper default and preserves the existing skip-on-unresolvable contract; `advisory` and `strict` are opt-in per-axis gates that surface or block variable-form producers (function params, for-loop unpacks, function-call returns, dict-subscript-target lookups). Provides the structural rule the WI-nubuv discussion empirically motivated.
  - **Registry entries** for six values surfaced by the new classifier (all AXIS_PENDING, follow-on cluster audit candidates): `ipc_channel_match`, `topic_match`, `qualified_call`, `ast_call_namespace` (inline-ternary discoveries from ipc.py / message_queue.py / r_lang.py / js_ts.py); `ctypes_stdlib_call`, `cffi_stdlib_call` (the dict-subscript-target leaks called out in WI-nubuv's 2026-05-06 empirical addendum that were never followed up); `error_set` on Symbol.kind AXIS_LANGUAGE_CONSTRUCT (Zig error-set declaration, sibling of struct/enum/union, surfaced from the same `kind = None`-poisoned walker shape).
  - **`SCHEMA_VERSION` bumped 0.7.0 → 0.7.1** (additive enum widening, patch bump per the schema-versioning contract). Closes WI-nubuv extensions B + C.

#### Other additions

- **Canonical registry for `Symbol.meta` / `Edge.meta` keys at `hypergumbo_core.axis_meta_keys`.** Structural sibling of `symbol_kinds.py` and `evidence_types.py`: a tuple of `MetaKeySpec(name, axis, description)` entries with axis constants (`symbol_meta`, `edge_meta`), plus accessors `all_meta_key_names`, `meta_keys_on_axis`, and `find_meta_key`. Seeded with the meta keys empirically observed in producer code across the language-analyzer packages: Wave 5 framework-dispatch fold residues (`framework_dispatch`, `detection_pattern`, `call_construct`, `call_kind`, `receiver`, `resolution_quality`), Wave 6 file/build-shape fold residues (`module_system`, `component_framework`, `package_ecosystem`, `entry_role`, `dependency_scope`, `install_mode`, `install_source`, `config_format`, `task_implementation`, `test_dialect`, `block_type`), ADR-0023 protocol / bridge / dispatch vocabularies (`protocol`, `bridge_kind`, `dispatch_kind`, `channel_kind`, `ffi_mechanism`), I/O boundary annotations (`io_primitive`, `io_boundary`), dataflow access modes (`access_mode`, `dest_access_mode`), and the common Symbol.meta annotations (`framework_role`, `base_classes`, `decorators`, `visibility`, `exported`, etc.). Property test enforces no duplicates, valid axes, all axes have specs, and no name collisions with typed fields on `Symbol` / `Edge`. Closes WI-vusot / WI-runod Wave 9. Drift detection (a subscript-access AST walker, distinct from `axis_drift.find_drift`'s set-literal shape) is a separate follow-on.
- **Ansible `include_tasks` / `import_tasks` with Jinja templates now fan out to candidate target files instead of collapsing to a single `unresolved:` edge.** Two Jinja shapes are recognized: (a) **basename-Jinja** (`include_tasks: "{{ ansible_os_family }}.yml"` in a role's `tasks/main.yml`) emits one `imports` edge per sibling `.yml` file matching the literal portion of the template (e.g. `Debian.yml`, `RedHat.yml`), scoped to the source's directory so cross-role candidates are excluded; (b) **path-prefix-Jinja** (`import_tasks: "{{ tasks_path }}/yumrepos.yml"` in a playbook) emits one edge per file in the repo whose basename matches the literal portion — the templated prefix could resolve to any path at runtime, so we surface all candidates. Each fan-out edge carries `confidence=0.30` to reflect the runtime uncertainty. Previously both shapes collapsed to a single edge with `dst='{{ ... }}'` and `confidence=0.50`. Empirically validated against the fedora-infra ansible repo: 191/192 unresolved `imports` edges now fan out to real targets (the remaining 1 is genuinely unresolvable — no matching basename). Sources the cohort-3 DEEP regression (ansible 28/35) signal. Closes WI-mipod.

### Fixed

- **`scripts/prepare-release` Step 7 no longer swallows push failures (WI-vurus).** The dev→main push was suffixed with `2>&1) || true`, which made every failure mode invisible: the URL-extraction regex would find no `https://...pulls/N` in the empty output and fall through to a misleading `✓ PR branch pushed (check Codeberg for PR)` message. The primary multi-line-description failure mode was fixed in 9fb301083d (single-line `PR_DESCRIPTION` + a static regression test), but the mask itself remained — meaning any future regression (network, auth, refspec rejection, schema change) would still ship invisibly. Fix wraps the push in `if !; then exit 1; fi`, prints the captured output on failure, and reuses the existing `RED` colour code for the failure banner. The downstream URL-extraction's `| head -1 || echo ""` is unchanged — that's a grep-no-match parsing fallback, not an error swallower, and runs only after the push has already succeeded. +1 regression test (`test_step7_push_does_not_swallow_errors`) asserting the Step 7 push command's trailer contains no `|| true`.
- **`access_mode` annotation coverage on JS/TS call edges (WI-kohah).** Pre-fix the JS/TS dataflow YAML only declared three node types in `assignments:` (`assignment_expression`, `augmented_assignment_expression`, `variable_declarator`) and one in `deletions:` (`delete_expression`). Call edges whose AST node sat inside a `return_statement` / `throw_statement` / `yield_expression` / `await_expression` got no positional rule match and stayed unclassified, leaving the `--dataflow` slice useless on the TypeScript stack that dominates the prospector corpus (DEEP cohort 1 reflect 2026-05-10: 0/61 nestjs and 0/80 apollo-server sampled calls edges had `access_mode != "none"`). The accompanying `library_patterns:` regex list was also conservative — RxJS / EventEmitter / Promise / ORM / iteration vocabulary common in NestJS / Apollo / NextJS production code was absent. Fix extends both javascript.yaml and typescript.yaml with:
  - `returns:` positional rules for `return_statement`, `throw_statement`, `yield_expression`, `await_expression` (call inside any → read via the existing `_default: read` machinery in `dataflow.py::_classify_by_position`).
  - `update_expression` assignment rule (`a++` / `--a` / `++a` / `a--` mutates the operand).
  - Expanded `library_patterns:` with: in-place mutators (`.sort`, `.reverse`, `.fill`, `.copyWithin`), ORM/persistence verbs (`.save`, `.persist`, `.insert`, `.update`, `.replace`, `.assign`), RxJS / EventEmitter / pub-sub (`.emit`, `.publish`, `.send`, `.dispatch`, `.next`, `.complete`, `.error`), Promise/Observable readers (`.subscribe`, `.observe`, `.then`, `.catch`, `.finally`), iteration / collection readers (`.map`, `.filter`, `.reduce`, `.forEach`, `.every`, `.some`, `.values`, `.keys`, `.entries`), and additional read verbs (`.fetch`, `.load`, `.query`, `.lookup`, `.findOne`, `.findMany`, `.exec`, `.toString`, `.toJSON`).
  +8 property tests in `TestAccessModeClassificationOnCallEdges` exercising the full `annotate_dataflow → library_patterns` pipeline on JS/TS source (`return` + `throw` positional, `emit/save/subscribe/forEach/sort` library patterns); one test documents a known limitation in `_build_line_index` where the trailing `;` of a `lexical_declaration` shadows the contained `variable_declarator` and `await_expression` siblings — fixing that is out of scope for this YAML-enrichment WI (the deeper `_build_line_index` heuristic rewrite would need its own WI if the bakeoff signal warrants).
- **`is_utility_file` no longer false-positives on `<pkg>/utils/`, `<pkg>/contrib/`, `<pkg>/tools/` (WI-gigib).** The pre-fix heuristic considered a path utility if any of its components matched an "ambiguous" name (`dev`, `tools`, `bin`, `utils`, `utilities`) AND the iteration hadn't yet passed a hardcoded source root (`{src, lib, app, crates, packages}`). The `contrib` and `hack` names were in the unconditional `utility_dirs` set and matched at any depth. Packages whose top-level directory is the package name itself (Django, requests, urllib3, flask, …) had no recognized source root, so every nested `utils/` or `contrib/` flagged as utility. Concrete reproduction (bakeoff-deep cohort 1 reflect on 2026-05-10): django's rslice.1 had 141 edges in the regular reverse slice, of which 75 had non-test endpoints. The corresponding `rslice.1.prod.json` (built with `--exclude-tests --exclude-utility`) collapsed to **0 edges** because the seed `format_html` at `django/utils/html.py` was flagged as utility, and every reachable caller path traveled through `django/contrib/admin/*` / `django/forms/utils.py` / `django/template/backends/utils.py` — all also flagged. Fix:
  - Move `contrib` and `hack` from the unconditional `utility_dirs` set into `_AMBIGUOUS_UTILITY_DIRS`.
  - Change the ambiguous-dir check from "not yet seen a source root" to "at `path_parts[0]` only" (depth 0 == repo root). The Kubernetes / etcd / Django-South-style convention is that `contrib/`, `tools/`, `utils/`, `hack/`, `bin/`, `dev/` are tooling **only when they sit at the repo root** — when nested under a package name they're production modules.
  - Drop the now-unused `seen_source_root` tracker and the `_SOURCE_ROOTS` constant from `is_utility_file` (the source-root concept is no longer load-bearing for this heuristic).
  +1 regression test (`test_ambiguous_dirs_under_named_package_root_not_utility`) covering Django (`django/utils/*`, `django/contrib/admin/*`, `django/forms/utils.py`, `django/template/backends/utils.py`), requests (`requests/utils.py`), urllib3 (`urllib3/contrib/socks.py`), flask (`flask/cli/utils.py`), plus regression guards for root-level `contrib/`, `hack/`, `utils/` still being utility. The 14 existing `TestIsUtilityFile` tests (including `test_ambiguous_dirs_inside_source_root_not_utility` for `src/`, `lib/`, `crates/`, `packages/` paths) all stay green. The chatwoot / nestjs "test edges leak into prod rslice" sub-symptoms from the same WI were investigated and not confirmed in the current artifacts (chatwoot's two leaked edges in rslice.0 ARE removed in rslice.0.prod; nestjs's "5 test edges leak" pattern was not reproducible on the current pipeline — likely fixed by an earlier closure).
- **Phoenix/Elixir test files now classify as supply_chain.tier=1 with is_test=True (WI-pugas).** Phoenix's conventional layout puts ExUnit test files at `test/<context>/<thing>_test.exs` — files that match both the `(?:^|/)tests?/` TEST_DIR_PATTERN (→ Tier.INTERNAL_DEP) and a future Elixir test-file pattern. Pre-fix the dir-pattern check fired first and Phoenix's 1328 test files all landed at tier=2 (internal_dep), conflating "is_test" with "is vendored / private-dependency" on the supply-chain axis (FCA-style conceptual leak — the bakeoff-deep cohort 1 reflect on 2026-05-10 showed 1328/1328 tier=2 nodes were tests, 0 vendored). Fix introduces a new `TEST_FILE_PATTERNS_DIR_OVERRIDE = [r"_test\.exs$"]` list checked BEFORE `TEST_DIR_PATTERNS`. The override is narrow on purpose: only files whose name carries the canonical Elixir `_test.exs` suffix get routed to tier 1; auxiliary files in `test/` (e.g., `test_helper.exs`) keep their previous dir-pattern routing (still tier 2 with `is_test=True`). +4 property tests: Phoenix-style `test/<ctx>/foo_test.exs` → tier 1; tests at the `test/` root → tier 1; `test_helper.exs` falls through to dir pattern → tier 2; co-located `_test.exs` outside `test/` (existing TEST_FILE_PATTERNS path) regression guard. The broader "should tests ever appear on the tier axis at all?" question (Go's `_test.go` co-located case was promoted to tier 1 by WI-gifuz; Phoenix is the analogous Elixir fix here) is documented in the WI but explicitly out of scope.
- **`yjs_crdt` linker is now gated on a real Yjs ecosystem dependency in package.json (WI-vurig).** The framework linker text-scans every JS/TS file for write/read patterns like `yMap.set('k', v)` / `awareness.observe(...)` / `store.addBlock(...)` and emits `event_publishes` edges between them. The token vocabulary overlaps with generic Vue / Rails / Express patterns (`.set(`, `.on(`, `.observe(`), so on a non-Yjs codebase like chatwoot (Rails + Vue, zero `y*` deps in package.json) the scanner produced 68 spurious `event_publishes` edges in DEEP cohort 1 reflect (2026-05-10). Fix: `link_yjs_crdt` now performs a manifest-presence check up front via `_repo_has_yjs_dependency(symbols)` — at least one `Symbol(kind="dependency", language="json")` must have a `name` in `YJS_DEPENDENCY_NAMES = {"yjs", "y-protocols", "y-websocket"}`. When the gate fails, the linker returns an empty `LinkerResult` before any text scanning. Real Yjs apps depend on the core `yjs` package directly (it is a runtime dependency of `y-monaco`, `y-prosemirror`, `y-quill`, `y-codemirror`, …, and is also commonly listed in `dependencies` for clarity), so the gate is structurally correct: false negatives require deliberate exclusion of the core dep. +3 property tests (gate suppresses when absent, gate accepts on `y-protocols`, unrelated deps `react`/`lodash`/`vue` don't satisfy gate). Existing 38 yjs_crdt tests gain a `_make_yjs_dep_sym()` helper symbol in their `syms` lists so the linker body still runs.
- **Blade analyzer now enrols on Laravel repos (WI-navaf).** `FileIndex.match_pattern` misclassified `*.blade.php` (compound-suffix glob) as a pure-extension fast-path lookup. The lookup keys on `Path.suffix.lower()`, which only retains the LAST suffix segment — `foo.blade.php` is indexed under `.php`, not `.blade.php` — so the `_by_ext[".blade.php"]` lookup returned an empty list. On koel (a 9-`.blade.php`-file Laravel repo), `profile._detect_languages()` saw `files=0` for `blade`, dropped the language entry, and the producer-routing layer never invoked `analyze_blade`; all 9 `.blade.php` files were analyzed by `php-v1` only, and 0 `extends_template` edges appeared in `hg.json` despite 4 `@extends` directives in the views. Fix tightens the pure-extension branch in `FileIndex.match_pattern` to require no `.` in `pattern[2:]`, so compound suffixes (`*.blade.php`, `*.d.ts`, `*.lagda.md`, …) fall through to the general `fnmatch` branch which correctly matches against the full filename. The slow-path `find_files` fallback was already correct (pathlib's `rglob` handles compound suffixes); only the fast-path FileIndex was affected. +2 regression tests (`test_match_pattern_compound_suffix_blade_php`, `test_match_pattern_compound_suffix_d_ts`) and +1 profile-level test (`test_detects_blade_language`).
- **`type_hierarchy` linker now dispatches through interface-extends-interface chains in deny-listed languages.** The `WI-sukav A1` per-language gate at `type_hierarchy.py:_extends_admits_dispatch` filters all `extends` edges in Go / C++ / Rust / C# to suppress concrete-extends shadow-method dispatch. The filter was too aggressive: in Go (interface embedding via `type Bar interface { Foo; ... }`) and C# (interface inheritance), `extends` edges between interfaces represent virtual dispatch via interface satisfaction, not concrete inheritance. With the filter dropping these edges, `Foo.method → BarImpl.method` skip-level dispatches were silently absent on any Go/C# repo with multi-interface hierarchies. Surfaced via the WI-firuj cross-language test hedge (Go interface-embedding skip-level test failed with `dispatches=set()` despite a correct IR shape). The gate now accepts a `child_kind` parameter and overrides the deny-list when the child symbol's kind is `'interface'`. Concrete-extends suppression for `class`/`struct` symbols in the same languages is unaffected.
- **`bakeoff-deep-reflect` now injects `awaits_bakeoff_validation` claims for items in status `done`.** The queue won't drain otherwise: per the BVT playbook, items receive the tag at PR-merge time when work has shipped, so `done` is the canonical status. `load_awaits_bakeoff_validation_items()` previously filtered for `todo_soft` / `in_progress` only, so the "Active Bakeoff Validation Claims" section was empty in every reflect prompt and assessors omitted `awaits_bakeoff_validation_verdicts` from their YAML output. Symptom: 12-repo DEEP cycle on 2026-05-10 produced zero BVT verdicts despite a 16-item queue. Status filter now includes `done` alongside the two pre-existing values.
- **CI pip-audit step pins `urllib3>=2.7.0` to clear CVE-2026-44431 / CVE-2026-44432.** Both CVEs (published 2026-05-11, fixed in 2.7.0) reach the project transitively through `requests`. The pip-audit runner's baseline urllib3 was older, so the audit failed regardless of `pyproject.toml`. Mirrors the existing `requests~=2.33.0` (CVE-2026-25645) and `pygments~=2.20.0` (CVE-2026-4539) pins in `.github/workflows/ci.yml`. No code change.
