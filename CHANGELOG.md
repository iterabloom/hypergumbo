...
- **A DNS lookup is a network receive, in five languages (WI-dozul, owner ruling 2026-09-06).** ADR-0049's question — does the call return a value whose content the FAR SIDE chose? — is answered by the resolver, so `socket.gethostbyname` / `getaddrinfo` / `getnameinfo` / `getfqdn` / `gethostbyname_ex` / `gethostbyaddr` (python), `net.Lookup*` and `net.Resolver`'s methods (go), `netdb.getaddrinfo` / `gethostbyname` / `getnameinfo` / `gethostbyaddr` and the `_r` variants (c), and the whole `dns` / `dns.promises` / `dns.Resolver` surface (javascript, which WI-nolut had left unaudited) now carry `net_recv` and mint `untrusted_input` — a resolver-poisoning threat model is exactly what that expresses. **The ruling was decided by parity, not by the ADR in the abstract:** erlang had rowed the same family under `net_recv` since 2026-05-24, so the real choice was parity-with-erlang versus retag-erlang, and a vocabulary that means different things per language is the failure the axis discipline exists to prevent. Rust gets a `std::net::ToSocketAddrs` row for correctness that is **inert on arrival** and says so — it is a trait method on a receiver the analyzer cannot type (WI-tuzaf) — and is deliberately absent from the parity test rather than counted as coverage.
