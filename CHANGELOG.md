...
- **SCIP symbol-string descriptor parser**: new `hypergumbo_core.scip.descriptor` parses the Sourcegraph `<scheme> <manager> <package> <version> <descriptor>+` chain used by rust-analyzer, scip-python, scip-typescript, scip-java, scip-clang. Returns a frozen `ScipSymbol` with header fields + descriptor tuple; handles backtick quoting, method disambiguators, trait-dispatch shape, local symbols. No protobuf dependency at this phase.
- **Vendored SCIP protobuf binding**: new `scripts/build-scip-proto` regenerates `hypergumbo_core/scip/_generated/scip_pb2.py` from Sourcegraph `scip.proto` at pinned SHA `9330cbd49aeb85aee026842770a61ad28e5c4093`. Developer-run (not CI-run); generated module checked in so end users pay no install-time toolchain cost. Adds `protobuf~=6.33` to hypergumbo-core.
- **SCIP Index → Symbol translator**: `scip.index.scip_index_to_symbols` walks a parsed SCIP `Index` and emits hypergumbo `Symbol` objects with `origin="scip"` and `stable_id` preserving the raw SCIP symbol string. Local symbols preserved with `kind="local"`; malformed input skipped silently.
- **SCIP Index → Edge translator**: `scip.edges.scip_index_to_edges` emits Edges from `SymbolInformation.relationships` — `is_implementation` → `implements`, `is_type_definition` → `has_type`, `is_reference` → `references`, `is_definition` → `defined_by`. Self-relationships dropped.
- **SCIP Occurrence-ref → call-edge translator**: `scip.calls.scip_index_to_call_edges` attributes every non-`Definition` Occurrence to its enclosing `Definition` via innermost-span containment, emitting `references` edges with `symbol_roles` preserved for downstream specialization.
- **`hypergumbo-lang-rust-analyzer` package skeleton + SCIP→IR translate surface**: new optional package sitting on top of the core SCIP shim. `translate_scip_to_hg(scip_bytes, source_reader) -> (symbols, edges)` parses a serialized Index and post-processes Rust function Symbols by rewriting their `stable_id` through the `rust.py` parity helper so cross-pass dedup works.
- **`rust-analyzer scip` shell-out wrapper**: `invoke.run_rust_analyzer_scip(workspace)` returns the emitted `index.scip` bytes. Three dedicated exceptions — `RustAnalyzerNotInstalled`, `RustAnalyzerInvocationFailed`, `RustAnalyzerNoOutput` — discriminate failure modes. Default timeout 600 s.
- **`hypergumbo install-rust-analyzer` / `uninstall-rust-analyzer`**: new `rust_analyzer_install` module + CLI subcommands mirroring `install-gitleaks` / `install-embeddings`. Rustup-only in first cut; static-binary-download fallback deferred.
- **`install-extras` / `uninstall-extras` umbrella subcommands**: single-call installer over every optional component (gitleaks, embeddings, rust-analyzer) with `--check` status table and `--skip NAME,...` exclusion. A failure in one installer doesn't abort the rest.
- **Graceful-degrade orchestrator**: `try_analyze_with_rust_analyzer(workspace, source_reader)` returns `(symbols, edges)` on the happy path, `None` on binary-missing / invocation-failure / no-output / malformed-SCIP. Fall-through messages deduplicated per `(exception_type, workspace)`.
- **Opt-in gate**: `should_use_rust_analyzer_backend(backend_flag, environ, is_available)` returns True iff the user opted in (via env var or flag) AND the binary is resolvable on PATH. Silent False when opted-in-but-binary-missing.
- **Registered analyzer**: `analyze_rust_with_scip(repo_root)` registered at priority 45 alongside `rust.py` at 50. Empty result when the opt-in gate is False; SCIP pass when True. Discovered via `hypergumbo.analyzers` entry point.
- **`--backend rust-analyzer` CLI flag**: new root-parser flag with choices `tree-sitter` / `rust-analyzer`. Sets `HYPERGUMBO_RUST_ANALYZER=1`; stripped from argv before subcommand parsing so it works in any position.
- **SCIP → `rust.py` `stable_id` parity helper** (ADR-0014 §3): `hypergumbo_lang_mainstream.rust_scip.compute_rust_stable_id_from_source(source, start_line, end_line)` returns the same stable_id `rust.py` would assign, so tree-sitter + SCIP symbols dedup under a single identity. Graceful-degrades to `None` when tree-sitter-rust is unavailable.

#### Linkers (Framework subcategory)

- **Controller-routes linker**: `contains_routes` edges from `concept: controller` classes to nested `concept: route` handler methods. Covers decorator-based families — NestJS, Spring Boot, ASP.NET, Laravel, Symfony, Phoenix, Micronaut, Ktor, Grails, CakePHP. Rails and Django CBVs without `@api_view` are explicitly out of scope.
- **Router-routes linker**: `registers_routes` edges from `concept: router` symbols to nested `concept: route` registrations. Effective for Phoenix, http4s, http4k, Yesod, giraffe, pedestal, ring-compojure, cowboy, sveltekit/remix/nuxt, vertx, plumber, laminas. Span-nesting picks innermost router.
- **Rust trait-impl method-dispatch linker**: consumes `implements` edges from `impl Trait for Struct` blocks and fans `dispatches_to` edges from each trait symbol to every concrete method on the implementing struct. Generic-bound / `dyn Trait` call-site resolution deferred.
- **Django ORM method-dispatch linker**: `dispatches_to` edges from Django subclasses (`Model`, `Manager`, `QuerySet`, `ModelAdmin`, `ModelForm`, `View`, …) to user-defined overrides of framework-called methods. Per-base method maps prevent spurious cross-base edges.
- **Jackson / JavaBean serialization dispatch linker**: `dispatches_to` edges from Java/Kotlin/Scala classes with Jackson / JAX-B / Spring-binding annotations to their bean-convention accessors (`getX`/`setX`/`isX`) and method-level-annotated handlers.
- **Airflow framework-dispatch linker**: `dispatches_to` edges from classes inheriting `BaseOperator`/`BaseHook`/`BaseSensor`/`BaseTrigger`/`BaseSensorOperator` to framework-called lifecycle methods (`execute`, `execute_complete`, `pre_execute`, `post_execute`, `on_kill`, `poke`, `run`, `hook`, `get_conn`, `test_connection`). Python-internal only.
- **Kafka Streams topology-callback dispatch linker**: `dispatches_to` edges from Java/Kotlin/Scala classes implementing any of 17 Kafka Streams callback interfaces (`ValueMapper`, `Transformer`, `Processor`, `Aggregator`, plus `*Supplier` factory forms) to framework-called methods (`apply`, `transform`, `process`, `get`, `init`, `close`, `test`). Call-site → impl edges deferred.

#### HTTP linker (cross-language)

- **Elm client detection**: the HTTP linker scans `*.elm` files and emits `http_calls` edges to server-side route handlers. Recognises `Utils.Api.<method>` wrapper style, `Http.get`/`Http.post` record forms, and both orderings of `Http.request`.
- **JS/TS backtick template-literal `fetch`/`axios` with module-const folding**: folds backtick-quoted URLs against module-scope string constants. Unresolved `${NAME}` slots handled heuristically — leading `${VAR}/` as base URL, middle slot as path-segment parameter `{NAME}`, inline slot truncates. Prefix-match fallback when the literal prefix has ≥ 2 segments.
- **Elm `let`-bound `String.join "/"` URL folding**: covers the indirect `let url = String.join "/" [ apiUrl, "silence", uuid ]` pattern. First list item dropped as base-URL variable; literals become path segments, identifiers become `{name}` placeholders.

#### Entrypoints (concept → entrypoint mapping)

- **`error_handler` concept → `ERROR_HANDLER` entrypoint**: maps the #1 inert concept (37 framework YAMLs — fastapi, express, django, aspnet, flask, actix, axum, gin, nestjs, rails, laravel, symfony, phoenix, …) to a new `EntrypointKind.ERROR_HANDLER` at confidence 0.95 with framework-aware labelling.
- **`form` concept → `FORM` entrypoint**: maps the #4 inert concept (12 framework YAMLs — Django, Flask-WTF, Laminas, fuelphp, cakephp, laravel, symfony, yii, pyramid, rails, remix, sveltekit, yesod) to `EntrypointKind.FORM` at confidence 0.90. Internal-method dispatch deferred to a per-framework registry follow-up.
- **`serializer` concept → `SERIALIZER` entrypoint**: uniform class-level `base_class` match across 9 frameworks (Django DRF, Flask Marshmallow, grape, laravel, litestar, plumber, pyramid, quart, rails) at confidence 0.90. Per-method dispatch (`to_representation` → field methods) deferred.

#### Behavior map

- **Per-handler forward slices emitted by `run`**: emits `slice.handler.<METHOD>.<path-sanitized>.json` per detected route handler, using bakeoff-proven parameters (`--exclude-tests --exclude-imports --hub-threshold 100 --max-hops 10 --max-files 200`). Capped at 25 by default; overflow recorded in `slice.handler.index.json`. `--no-handler-slices` / `--max-handler-slices N` control behaviour.
- **Method-call recovery linker for chained `Class().method()`**: new linker (priority 35) rewrites `calls→Class` + `unresolved-call(name=foo)` pairs into direct `calls→Class.foo` edges when the class has a `contains` child with that name. Language-agnostic; runs after `containment`.
- **Route materializer dedupes against analyzer-emitted routes**: pre-populates dedupe set with `(method, path)` pairs from existing `kind="route"` symbols. Fixes Django CBV double-counting on pretix (985 reported, ~500 unique). ANY-method routes excluded from pre-populate so CBV-specific-method variants still emit.
- **Class-level annotations propagate to methods for `--exclude-annotated`**: `dead-code-maybe --exclude-annotated` now consults the containing class's annotations when a method's own meta is empty. Helps Spring controllers, Django CBVs, any framework registering via class-level decoration.
- **IO-boundary leaf-caller roll-ups**: `BoundaryMapEntry` gains `leaf_callers` and `entry_points_per_leaf` fields so shared helpers (e.g. `notify/util.go:request`) no longer collapse disjoint caller chains into a single entry-point union.
- **Gradle / Maven dependency manifest for Java/Kotlin tiers**: boundary nodes from unresolved JVM imports classified using declared deps from `build.gradle`, `build.gradle.kts`, `pom.xml`. Direct deps → tier 2, unknown → tier 3. New `jvm_deps.py` module parses both Groovy and Kotlin DSL Gradle files plus pom.xml.
- **Framework manifest / JVM-dep scan skips test-fixture directories**: `_find_manifest_files` and `parse_gradle/maven_dependencies` skip `testFixtures/`, `testdata/`, `fixtures/`, `tests/`, etc. via `paths.is_test_file`. Fixes detekt reporting `react` as a detected framework from test-fixture `package.json` files.

#### Language support

- **Haskell module exports recognized as dead-code seeds**: Haskell analyzer parses module headers (`module Foo (publicFn, Type(..)) where`) and marks listed symbols `is_exported=True`. No-export-list modules get the Haskell default. Instances follow their class/type.
- **Yesod framework detection + pattern set**: detected from `*.cabal` / `package.yaml` deps; `frameworks/yesod.yaml` covers `mkYesod`/`parseRoutes` quasi-quoter, Warp runner, `Yesod`/`YesodSubsite` typeclasses, and the `<method><Resource>R` handler naming convention.
- **Kotlin extension-function call-site dispatch**: `receiver.extFn()` call sites emit `calls` edges to their extension-function definition when receiver type matches. Pass 1 builds `extension_index`; Pass 2 consults as Case 3b after class-method resolution. Generic receivers collapse to base name. Evidence `ast_call_extension` at confidence 0.80.
- **Unresolved-call edges for bare global JS/TS calls**: extends the unresolved-call fallback to the no-import case — `console.log()`, `localStorage.setItem()`, `navigator.sendBeacon()`, `window.fetch()`, `Deno.readFile()`, etc. emit `calls→{lang}:{obj}:0-0:{method}:unresolved` when the object is a known browser/runtime global and no import binding shadows it.

#### I/O primitive catalogs

- **TypeScript/JavaScript I/O boundary tracing**: JS/TS analyzer emits unresolved-call edges for bare-name calls to named imports (`import { existsSync } from 'node:fs'; existsSync()`). Browser-API entries added to the JavaScript catalog (WebSocket, EventSource, BroadcastChannel, XMLHttpRequest, localStorage / sessionStorage / indexedDB / caches, …). Verified on `create-next-app`: total_io_edges 0 → 27.
- **Unresolved-call edges for TS/JS namespace / default imports**: extends to `import * as fs from 'node:fs'; fs.readFileSync()` and `import axios from 'axios'; axios.get()`. Verified: apollo-server 7 → 14 boundaries; create-next-app 27 → 35.
- **Java I/O primitive catalog expansion**: grew ~136 → 312 primitives. Full JDBC + JPA + Hibernate + Spring Data coverage; logging facades (SLF4J, Log4j 1.x/2.x, Logback, `java.util.logging`); Apache HttpClient 4.x/5.x, Spring WebClient, Unirest, Retrofit, Apache Commons IO. Kotlin inherits via catalog alias.
- **Elixir I/O primitive catalog**: new `io_primitives/elixir.yaml` covers stdlib, HTTP clients (HTTPoison, Tesla, Req, Finch, Mint.HTTP*, Erlang `:httpc`), Phoenix/Plug, Ecto/Postgrex/MyXQL/Redix, GenServer/Process/Oban/Task IPC. Added to `_CATALOG_PARENTS` with `erlang` parent.
- **Kotlin I/O primitive catalog**: new `io_primitives/kotlin.yaml`. Previously Kotlin was aliased to `java.yaml` verbatim, producing only 1 boundary on detekt. Covers `kotlin.io` File / Path extensions, top-level `println`/`print`, ktor client + server, `android.util.Log`, `kotlin-logging`, Exposed ORM. Kotlin moved from `_CATALOG_ALIASES` to `_CATALOG_PARENTS` so Java parent still provides raw JDK entries.

#### Stop hook & bakeoff validation

- **Stop-hook nudge for `awaits_bakeoff_validation` backlog**: `awaits_bakeoff_nudge.py` appends an `## AWAITS_BAKEOFF_VALIDATION BACKLOG` section to the active guidance file when the count of tag-bearing items in a blocking status reaches `threshold` AND the latest DEEP cycle's `state.json` is older than `stale_cycle_hours` (defaults 5 / 72 h). Both knobs under `stop_hook.awaits_bakeoff_validation_nudge` in `.agent/tracker/config.yaml`.
- **`awaits_bakeoff_validation` cross-reference in reflect pipeline**: `bakeoff-deep-reflect` loads active tag-bearing items and injects a per-claim section into every repo's prompt. Output schema gains `awaits_bakeoff_validation_verdicts` keyed by `item_id` with `verdict ∈ {moved, no_move, inconclusive}` + evidence. `aggregate_assessments` collects per-item plurality and surfaces it in the run summary.
- **Auto-strip / regression-spawn for `awaits_bakeoff_validation` verdicts**: `bakeoff-deep-reflect aggregate --apply-verdicts` executes tracker mutations implied by verdicts — `moved` strips the tag + discuss-notes the cohort; `no_move` spawns a regression sub-item; `tied` / `inconclusive` no-op. Dry-run by default.

#### CI & smart-test

- **`test-agent-infra` full-suite CI job runs the top-level `tests/` directory**: new job in `full-suite.yml` mirroring existing package-job shape, wired into `aggregate`'s `needs` list and `conclusion` calculation as a hard gate. Closes the 4-hour cadence gap where `scripts/agent-supervisor`, `.agent/hooks/_shared/*.py`, and tracker-sync glue previously caught regressions locally at best.
- **Per-PR smart-test coverage for top-level infrastructure**: new `top_level_test_map.py` maps changed top-level source paths to their matching `tests/test_<basename>.py`; `smart-test` folds the output into `AFFECTED_TESTS` so a PR touching only `.agent/hooks/_shared/watched_process.py` now runs `tests/test_watched_process.py` in `ci.yml`.

#### Agent-supervisor

- **`scripts/agent-supervisor` daemon + replacement sequence**: Python daemon that monitors reserved-prefix tmux sessions (`hypergumbo-session-*`) and replaces stuck ones with fresh CLIs seeded with `HYPERGUMBO_RESPAWN=1`. Subcommands `run` / `status` / `stop`; single-instance via `fcntl.flock`; state under `~/hypergumbo_lab_notebook/agent-supervisor/`. Decision matrix keyed on pane-byte delta (≥ 15 min unchanged → replace); heartbeat is telemetry only. Rate-limited at 24 spawns per 24 h with auto-shutdown after 20 consecutive saturation ticks. Graceful-exit polls `tmux has-session` for the pane to disappear before the hard-kill fallback (no PID tracking, no hook cooperation).
- **Per-session heartbeat helper**: new `.agent/hooks/_shared/touch_heartbeat.sh` sourced from every per-turn hook, touches `<supervisor-state-dir>/<session_id>.heartbeat` on each tool-use / model-call. Telemetry only; fault-silent on empty session_id / unwritable directory.
- **Respawn-aware session-start hook**: when the supervisor spawns a fresh CLI via `tmux new-session -e HYPERGUMBO_RESPAWN=1`, vendor session-start hooks branch on the env var to auto-enable autonomous mode per `autonomous_intent.txt` (narrow-write via `loop-toggle --set-session-mode`) and emit a generic seed prompt. Defensive fall-through on intent=OFF / missing / garbage; never writes intent.
- **Vendor Parity for Respawn AGENTS.md section**: authoritative table in `AGENTS.md` documenting per-vendor (Claude Code, Codex CLI, Cursor, Gemini CLI) per-turn hook path, session-start hook path, graceful-exit keystroke, and CLI invocation string. Claude Code's `/quit` verified; the other three marked unverified with a documented verification procedure.
- **Meta-circuit-breaker: chain tracking + no-progress kill switch**: classifies every replacement as no-progress (dying pane ≤ 512 bytes) or progress (> 512); tracks chains via `replaces` / `chain_length` / `consecutive_no_progress` in `meta.json`. Auto-pauses after 5 consecutive no-progress failures (time-agnostic). `agent-supervisor resume` clears the sentinel.
- **Non-interactive seed-prompt bootstrap**: after `tmux new-session`, `spawn_fresh` polls `tmux capture-pane` for pane-content stability (250 ms cadence, 32-byte floor, 3 consecutive identical samples, 15 s deadline) then injects `"begin"` via `tmux send-keys` to trigger the first model turn. Vendor-agnostic.
...
- **Intent/mode split in `scripts/loop-toggle`**: new `autonomous_intent.txt` (gitignored) records project-level intent separately from `AUTONOMOUS_MODE.txt` (current session runtime mode). `--set-intent MODE` / `--set-session-mode MODE` write each independently; bareword form writes both. Stop-hook circuit-breaker trips now use `--set-session-mode off` so a stuck session deactivates itself without suppressing project intent.

### Changed

- **Linker subcategory vocabulary restored** (ADR-0003-ext): the Protocol / Bridge / Framework subcategory taxonomy from ADR-0003 §2.4 is now first-class. Every linker module docstring declares its subcategory; `docs/LINKERS.md` renamed and enumerates all 45 linkers with a Subcategory column. "Infrastructure" added as a fourth subcategory for graph-structural utilities.
- **Stop hook: process-aware pause replaces 150 s blanket sleep**: `stop_logic.sh` inspects running processes against a configurable watched-pattern list (`pytest`, `python -m pytest`, `smart-test`, `bash ./scripts/auto-pr`, `bash ./scripts/merge-pr`) and polls every 3 s (1800 s cap) when alive; returns immediately when none. Matching goes through a dedicated `watched_process.py` that splits each cmdline, strips argv[0]'s directory prefix, collapses `python3.11` → `python`, and requires the full pattern to match leading argv tokens — so `inotifywait /tmp/pytest-.../foo` isn't treated as a live pytest. `bash -c` / `sh -c` wrappers filtered. Configurable via `stop_hook.watched_*` keys in `.agent/tracker/config.yaml`.
- **Dead-code-prospector categorizer expanded 8 → 46 gap categories**: `_categorize_candidate` gains language-gated rules (Rust trait impls, Python dunders / Django ORM / Airflow, Go receiver methods / k8s watchers / Cilium eBPF dispatch, Java JavaBean / Kafka Streams / Spring bean, TS/JS React / Redux / Superset / Apollo). Reduces `uncategorized` rate on the 2026-04-11 corpus (92,218 candidates, 11 polyglot repos) from **94.0 % → 43.5 %**, below the 50 % threshold.
- **Behavior map node IDs use repo-relative paths**: `run_behavior_map` calls `_relativize_ir_paths` after `resolve_deferred_symbol_refs` to strip the `str(repo_root) + "/"` prefix from every Symbol id/path, Edge src/dst, and UsageContext path/symbol_ref/id. Paths outside `repo_root` preserved; UsageContext.id recomputed from the relativized path.
- **generate-concepts scans Python source for programmatic concept emitters**: `scan_producers` optionally takes analyzer source trees and scans them with `_PROGRAMMATIC_PRODUCER_RE` to catch cases like `py.py` emitting `main_guard` from its AST walker. Variable-expression forms deliberately not matched (would double-credit YAML concepts). `main_guard` flips ghost → live; ghost count 1 → 0.
- **generate-concepts detects variable-name and tuple-membership consumer patterns**: consumer regex now matches any identifier containing `concept` for `==`/`!=` comparisons in either order, plus a two-step membership-test extractor for `concept_type in (...)` / `{...}` / `[...]` and `not in`. 30 concepts flip inert → live (`error_handler`, `websocket_handler`, `middleware`, `liveview`, `graphql_resolver`, `main_function`, …). Coverage moves 7 live / 309 inert / 0 ghost → 37 / 279 / 1.
- **`test-coverage` surfaces per-language false-negative caveats**: text-format runs print a footer documenting the ~20 % recall gap and per-language blind spots (Java/Spring MockMvc, Kotlin PSI, Go YAML reflection, Scala macros, Ruby `described_class`, Python fixtures + `parametrize`, JS/TS `describe.each` + ESM tree-shaking, C# `[Theory]`/Moq proxies). JSON gains a structured `caveats` field.
- **Unified path argument across subcommands**: every subcommand accepts both `hypergumbo <cmd> /path` and `hypergumbo <cmd> --path /path`. Setting both is a user error. New `_add_path_argument()` centralizes the convention.
- **`routes` excludes test-file routes by default**: 14 % of plausible's reported routes were from test files. New default excludes; `--include-tests` opts back in. Legacy `-x/--exclude-tests` preserved as a no-op alias.
...
- **`tree-sitter-c-sharp` named-argument grammar flattened at 0.23.5**: the loose pin `~=0.23.1` resolved to 0.23.5, which flattened named-argument nodes and broke detection. Fixed `csharp.py` named-arg handling and tightened the pin to `~=0.23.5`.
...
- **Top-level `tests/` drift**: three pre-existing failures surfaced when the top-level `tests/` directory got instrumented. `test_committed_file_is_up_to_date` now passes `ANALYZER_SRC_DIRS` to `scan_producers`; two `TestLogTrainingExampleCohortMetadata` tests updated to assert `pipeline_version == "v2"`.

#### auto-pr

- **`.ops` backup/restore no longer overwrites concurrent tracker writes**: `do_merge` in `forgejo-api.sh` backed up tracker `.ops`, rebased, then restored with `cp` — clobbering any ops appended during the run. New `_ops_union_restore_file` helper performs an order-preserving line-level union (`awk '!seen[$0]++'`), semantics-correct for append-only ops files, and the restore loop now enables `shopt -s dotglob` locally so dotfile `.ops` paths actually match.
- **`auto-pr` Exit 2 (timeout) soft-retry**: the hung-run retry loop only fired on Exit 3. On Exit 2 the script now re-polls once with a 300 s timeout, then does one close-PR + repush, before escalating to Scenario B.

#### Stop hook

- **`stop_hook_state.json` write discipline: unmaintained keys silently dropped**: jq merge in `stop_logic.sh` used to start from `.` (full existing object), so dropped keys from old migrations lingered. Now starts from an explicit extraction of the maintained field set; any unlisted key is dropped on the next write. Recover-state playbook documents the maintained-field table.

#### Analyzers & edges

- **Solidity file-level `using X for Y;` applies to calls inside contracts**: the edge extractor keyed `using_libraries` by enclosing contract only, so file-level directives (keyed `""`) were invisible. Fix: union contract-scoped with file-level set before probing `{Library}.{method}`.
- **`test-coverage` recognises framework-tagged tests outside test paths**: any function/method whose `meta.concepts` contains a concept starting with `test` is treated as a test in addition to the path-based heuristic. Fixes the shellcheck scenario where Template-Haskell `$forAllProperties` discovers 2214 `prop_*` functions in `src/` (0 % reported coverage before the fix).
- **`dead-code-maybe` unconditionally drops symbols in generated files**: any candidate whose `supply_chain.is_generated_file` is True is now filtered out before ranking. Language-agnostic; generated code is never actionable dead code.
- **Django CBV routes now expand to one route per declared HTTP method**: `path("/foo/", FooView.as_view())` previously emitted a single `[GET]` route regardless of declared view methods. Python analyzer now flags CBV registrations with `is_class_based_view=True`; new `expand_class_based_view_routes` post-pass replaces each ANY route with one variant per declared method. Out-of-repo view classes stay `[ANY]` (honest unknown beats false specificity).
- **Java `size`, `length`, `copy`, `find` no longer misclassified as fs_read**: missing from the io-boundary `ambiguous_names` list, so `List.size()`, `String.length()`, etc. were tagged as `java.nio.file.Files.size`/`.length`/`.copy`/`.find`. Added to the ambiguous filter; resolved calls with proper module context still match.
- **Scala framework detection reads `project/*.scala` and `project/*.sbt`**: the SBT meta-build convention puts real library coordinates in `project/Dependencies.scala`; `build.sbt` only references scala helpers. Docspell's http4s imports were invisible to `profile.frameworks`. Detector now concatenates `build.sbt` plus all `*.scala` / `*.sbt` under `project/`.
- **Laravel `apiResource()` no longer emits phantom HTML-form routes; `.except()` / `.only()` honored**: `Route::apiResource('posts', ...)` now produces 5 routes (index/store/show/update/destroy) instead of 7. Chained `.except([...])` / `.only([...])` modifiers parsed. On koel: ~40 phantom routes eliminated (~19 % of 207).

#### CLI

- **`--config-extraction=embedding/hybrid` warns when sentence-transformers is missing**: both modes silently degraded to heuristic. Dispatcher now pre-checks embedding availability and emits a one-shot stderr notice before falling back; `_embedding_fallback_warned` dedupes across calls.
- **`verify-claims` surfaces languages where taint-flow has no catalog**: stderr notice fires when a `taint_flow` constraint evaluates on languages with zero coverage. Before this, a trivially-passing claim against an unanalyzed language gave false security confidence. JSON schema unchanged.
- **`io-boundaries` distinguishes "no I/O detected" from "language unsupported"**: `IoBoundaryCatalog` gains `is_supported: bool`, stderr notice on unsupported languages, and JSON output includes `unsupported_languages: []`.
- **Subcommand parser cleanup**: (1) `hypergumbo foobar` no longer silently inserts `sketch`; `main()` detects unknown-subcommand attempts and prints a `Did you mean: ...` via `difflib`, exits 2. (2) `hypergumbo sketch . --debug` works — `--debug` is stripped from argv in any position.
- **Embedding-model load no longer dumps progress bars to stderr**: `_hf_noise.suppress_hf_noise()` runs at `sketch_embeddings` import time (before `sentence_transformers` caches env), setting `HF_HUB_DISABLE_PROGRESS_BARS=1`, `TRANSFORMERS_VERBOSITY=error`, etc. via `setdefault` so user overrides are preserved.
- **`-e/--exclude` glob normalization**: `ui/`, `ui/**`, `**/ui/**`, `**/ui` now behave consistently with the bare-name form `ui`. Path-anchored patterns like `cmd/server.go` honored against the relative path. Affects `run` and `sketch`.
- **README / markdown heading bleed**: when a `.md`/`.mdx`/`.markdown`/`.rst` file renders as Additional Files Content, ATX headings are demoted 2 levels so they cannot compete with hypergumbo's H2 structural sections.
- **Token budget validation**: `-t 0` and negative values rejected by argparse. Previously `-t 0` was silently treated as "no budget" and `-t -1` produced header-only output with exit 0. Applies to `sketch` and `explain`.
- **Single-file input exits cleanly**: `hypergumbo run` / `sketch` on a file prints a hint and `sys.exit(1)` instead of crashing with `NotADirectoryError`.
- **Quieter partial-linker warnings on polyglot repos**: suppress partial-install warning when the only met requirement is a language-file presence check (`*_files` suffix). Verified on alertmanager: 8 warnings → 1 (−87 %).
- **`--require-section` now actually works**: `max_tokens <= base_tokens` early-return bypassed all section gates. Fix skips the early-return when required sections were requested, forces analysis for required-section dependencies, makes `_section_ok` return True unconditionally for required sections, and skips the final `truncate_to_tokens`. Verified on alertmanager `-t 500`.

### Performance

- **Cached secret-scan results across warm sketch runs**: `scan_content_cached` keys gitleaks output by sha256 of the sketch content (up to 8 entries). Warm `hypergumbo sketch` now completes in roughly the `--no-secret-scan` time (~7 s on alertmanager) instead of paying the ~8 s gitleaks cost each invocation. Cache invalidates automatically when the repo state hash changes.

### Documentation

- **`docs/agent-supervisor.md` operator guide**: net-new user-facing doc covering prerequisites, first-time setup (`loop-toggle DEEP` + `agent-supervisor run &`), daily operations (attach / detach / pause / resume / shutdown), `status` JSON field semantics, edge cases, troubleshooting matrix, state-directory layout, and cross-references. Linked from `README.md` Links section.
