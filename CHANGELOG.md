...
**Calls credited to the right function.** Most analyzers found a call's enclosing function by name, so a call could be credited to a same-named function elsewhere, sometimes 1,900 lines away. They now find it by position. Scala and Kotlin also let an explicit import outrank a same-named project symbol.
...
- **A recorded statement whose def/use misses a name no longer passes the coverage gate unseen.** The gate asked only whether code sat *outside* every recorded statement. `unaccounted_names` now compares each recorded statement's defines and uses with the names inside it. The proposed alternative, a per-statement "understood this construct" flag, was refuted by a four-language sweep: in six of the seven shapes found, the handler is registered but incomplete, so the flag would have vouched for all six. Measured effect on verdicts today: **zero, reported as zero**.
...
- **Three coverage-gate gaps close.** The §3a gate asked only about call nodes, so an omission at a construct with no call in it licensed an unearned refutation; it now compares semantic leaves. A recorded heir vouched for an unrecorded one (1,978 sites). `coverage_granularity` still read `"language"` after the per-function gate it was waiting for had landed.
...
- **An unresolved bare-name call no longer installs a sanitizer that is not there.** A barrier deletes findings. The guard listed the ways receiver evidence could be *absent* and permitted anything else, so a bare Java `doFinal(p)` bound `javax.crypto.Cipher.doFinal`. In every language it now requires evidence to be *present*: an exact qualified name, a module slot that completes it, or a callee name carrying its own owner. This reverses an earlier ruling that a bare free-function call could bind a receiver-shaped sanitizer.
- **Taint reaches a JavaScript callback.** `addEventListener`, `http.createServer` and `process.on` classified correctly and produced no findings: the source sits on the enclosing function, the sink inside the callback, and JavaScript links the two with `references`, which taint did not follow. A registration edge is now call-shaped for taint; `references` as a whole is not, since it also carries type references. On eight JavaScript repositories: 78 → 80 findings, one verdict `inconclusive` → `violated`, none removed. The change is not purely additive: a callback handed to a sanitizer now installs a barrier.
...
- **A call is credited to the function that contains it, in 24 analyzers.** Each found a call's enclosing function by name, a name that overloads, companion `apply`s, redefinitions, `#ifdef` branches and one method name in two classes all share, and credited whichever declaration registered last. They now find it by the declaration's position, the rule Elixir and Erlang first adopted. Calls lying outside their caller's span go to 0: D 15,126, Elixir 7,166, Kotlin 2,014, Objective-C 1,849, Erlang 1,306, Scala 964, GLSL 592, Groovy 449, Swift 273, JavaScript 135, R 53, Perl 39, C 35, TypeScript 33, Rust 5, Ruby 3, C# 3. Calls between overloads now resolve. A few edges that existed only because of the wrong anchor are gone. A Nix file's top-level function now owns its calls (postgrest 13 → 159). An Elixir `def` head is no longer a call. An Erlang `-ifdef` branch keeps its calls. A test fails on any analyzer that emits calls and still anchors them by name.
- **A Haskell function's span covers all of its equations**, where it had covered only the first: 119 calls on xmonad and hadolint fell outside the function they were credited to. A multi-equation function's id and `line_span` change with it.
...
- **A Scala or Kotlin call follows the file's explicit import.** Both resolved a call by its simple name across the whole project, so `import scala.sys.process.Process` then `Process(cmd)` bound to spark's own `case class Process`, and an imported JUnit `assertEquals` bound to a test fixture of the same name. An import now outranks a same-named project symbol in another package. Kotlin moves the call to the imported path when the project defines it (2,864 detekt and 2,555 okhttp calls). Scala leaves the call unresolved with the import path in its module slot, so a launch reaches its catalogue row: on 11 repositories 5,631 resolved calls became unresolved, and all 30 in a random sample had been false edges.
- **Swift and C# properties are back in the default map.** The noise filter dropped every `property` symbol as CSS noise, but CSS emits none. The kind comes from Swift and C# properties, which have bodies and calls, and from `.properties` keys, QML and Objective-C `@property`, which have neither; only the last three are dropped now. Alamofire gains 278 call edges and Kingfisher 680, and Kingfisher loses 2 false dead-code candidates. C# properties still carry no edges, because its analyzer credits no call inside an accessor.
...
- **Elixir gives the same answer twice, reaches OTP, and stops resolving qualified calls by bare name.** A bare stdlib-named call was attributed to `next(iter(imported_modules))`, so two hash seeds gave surveys 101 edges apart, and all 653 edges from that branch were false. Every Elixir number published before this fix went through it. An atom-module call (`:ets.insert`) emitted no edge (+122 edges across 25 OTP modules, 0 lost). A substring ownership test let a qualified call bind by bare name (202 false edges; 208 redirected, 0 real losses).
...
- **Python's `urllib.request.Request` no longer counts as the send; `urlopen` does.** The module's 46 public callables were audited first and it is declared complete, since an unclassified `Request(...)` would otherwise withhold every clean verdict. Proxy discovery (`getproxies`, `proxy_bypass`, `ProxyHandler()`, `build_opener()`) is rowed as an environment read, `localhost()`/`thishost()` as DNS receives, `ftpwrapper` (it logs in when constructed) as a send, and `urlcleanup` as a file write. No verdict moved on beads, django, litellm, superset or hypergumbo's own self-claims, and every function that lost evidence at `Request` gained it at `urlopen`.
- **A Go call on a stdlib package variable reaches its catalogued boundary.** `http.DefaultClient.Do(req)` fell to `external`, or bound by bare name to an unrelated project `Do` (79 false edges on keda, loki and grafana). The library-signature catalogue gains a `package_variables` section typing `http.DefaultClient`, `http.DefaultTransport` and `net.DefaultResolver`, and all 79 now reach `net/http`, where `Client.Do` is the send.
...
- **Scala's most common process launch, `Process(cmd).!`, is classified as `subprocess` and `fs_write`,** under the explicit import and the `scala.sys.process._` / `.*` wildcard alike. No row carried the name the analyzer emits, so on sbt 6 of 6 explicit-import launches classified as nothing. On 11 Scala repositories: 22 classifications gained, none lost or changed.
...
- **Swift's sixteen constructor rows are decided one by one.** Keyed as methods, they had never matched a call. Eight cross nothing and are removed: `URLRequest`, `HTTPClientRequest`, `ClientBootstrap`, `MultiThreadedEventLoopGroup`, `Logger(label:)` and three `NIOSSL*` constructors. `NWListener`, `ServerBootstrap` and `NIOWebSocketServerUpgrader` become the disclosed `net_listen`, and `NSFetchRequest` becomes `db_compose`. `NWConnection`, `NIOAsyncChannel` and `ModelContext` keep their boundary, and `CommandLine` becomes the read of `CommandLine.arguments`. Vapor gains 3 classified calls and hummingbird 2; no `verify-claims` verdict changes.
...
- **The dead-code walk reaches an initializer through its constructed class.** Python, JS/TS and Dart land a construction edge on the class, and no edge leaving a class node is traversable. So a private helper called only from a constructor read as dead; `Cache._ensure_db`, which opens the tracker's SQLite connection, was one. The walk now hops from a *constructed* class to its initializer, never through `contains` alone, which would make every method of every instantiated class reachable. No emitted edge changes. Revived at `--cross-lang-threshold 0`, 0 newly dead: **21 on self-analysis, 103 on pretix, 74 on kserve**. `symbol_kinds.is_initializer` is the shared predicate; Dart is a declared gap.
...
- **The brand scrub covers everything the scripts publish to the forge, not only commit messages.** A merged PR body carried a vendor session URL, because the scrub lived in the `commit-msg` hook and a PR body is not a commit message. The matcher moves to `.githooks/brand-scrub.sh`, which `lib/forgejo-api.sh` and `commit-msg` both use, with the policy unchanged. **`scripts/check-forge-text-egress`** requires every publishing site to declare `# forge-egress: scrubbed` or `# forge-egress: no-agent-text -- <reason>`.
...
- **`auto-pr` reports what actually happened.**
  ...
  - Its stderr is no longer discarded from the lock line onward. An `exec {FD}>>"$f" 2>/dev/null` meant for one call silenced the rest of the run, so an abort in the push path exited 1 with no output at all.
- **`smart-test` selects the right tests.**
  ...
  - A change to any of the ten YAML catalogue families selects the tests that read it, where 146 of 169 files had selected none. It also selects the whole repo-root suite: 13 root test files read catalogue data through a script, the CLI or an import, and none had been selected. One of them first ran, and failed, on the cron.
  - A `conftest.py` change and a docs-only change each selected **0 tests**, so the file that governs a suite, and every doc gate, went unchecked on exactly the change they exist for. Both now select tests, as does a source change cited by path in a test's string.
  - `--full` includes the repo-root `tests/`, and no longer runs nothing and exits 0 when the diff holds nothing test-relevant.
