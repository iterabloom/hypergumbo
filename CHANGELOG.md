...
- **`hypergumbo install-extras` / `uninstall-extras` are removed; `add-extras` / `remove-extras` are now the single umbrella pair** (WI-josif). The project shipped with two partially-redundant umbrella subcommands — `add-extras` / `remove-extras` (hand-coded, covers grammars + gitleaks + embeddings + rust-analyzer) and `install-extras` / `uninstall-extras` (declarative `_extras_components` table, covers gitleaks + embeddings + rust-analyzer with `--check` and `--skip` flags). Drift between the two recently caused rust-analyzer to silently fall off `add-extras` for an unknown stretch (today's bug); the `install-extras` path also bypassed the BUG-06 SCIP-integration gate that `install-rust-analyzer` enforces, producing a contradictory "rust-analyzer ✓ installed" + BUG-06 runtime error pair. Consolidation: `_extras_components` is the single source of truth and now has four rows (grammars added; rust-analyzer install wrapped through `_install_rust_analyzer_with_bug06_gate` so the table-driven path matches `cmd_install_rust_analyzer`'s gate). `cmd_add_extras` and `cmd_remove_extras` are rewritten as thin loops over the table; `--check` and `--skip` migrate to `add-extras` (and `--skip` to `remove-extras`). The rust-analyzer row's `is_available` predicate is also now holistic — `_is_rust_analyzer_fully_available` returns True iff both the rustup binary AND the `hypergumbo-lang-rust-analyzer` integration package are present, so `add-extras --check` reports `rust-analyzer: ✗ not installed` in the binary-present-integration-missing edge case (e.g. system-package-manager rustup install, or residual binary after a `[rust-analyzer]` extra uninstall) instead of a misleadingly green `✓ installed` that would later hit the BUG-06 runtime gate. The `install-extras` / `uninstall-extras` argparse parsers are deleted; invoking either now exits 2 ("invalid choice"). **Breaking** for anyone scripting against the old names; mitigated by the project's alpha status. The new `tests/test_extras_umbrella.py` (renamed from `test_install_extras.py`) covers the table semantics, both umbrella commands, the BUG-06 gate, the holistic availability predicate, the grammars-row helpers, and a regression assertion that the legacy names are no longer registered.

### Added

- **`hypergumbo[rust-analyzer]` install extra + `hypergumbo-lang-rust-analyzer` published to PyPI** (WI-gozis, BUG-06 path (a) follow-up). The released v4.1.0 distribution did not ship the SCIP integration package and did not declare it as an optional dependency, so `--backend rust-analyzer` had no way to engage end-to-end (the BUG-06 / WI-jinoh runtime gate landed first as a defensive fix that errors clearly instead of falling through silently). This release adds the `[rust-analyzer]` extra to `packages/hypergumbo/pyproject.toml` (pinned in lockstep with the meta-package version, so `scripts/bump-version` keeps the two coordinated) and adds `hypergumbo-lang-rust-analyzer` to the dependency-ordered build loop in `.github/workflows/release.yml` so the next tag-release uploads it to PyPI alongside the other six packages. After this release, `pipx install 'hypergumbo[rust-analyzer]'` actually engages the SCIP backend; the BUG-06 runtime gate stays as a defensive backstop for users on minimal installs without the extra. New regression-guard tests in `packages/hypergumbo/tests/test_meta.py` assert the extra exists and that its version pin tracks `project.version` exactly.

- **`hypergumbo add-extras` / `hypergumbo remove-extras` now cover rust-analyzer too.** The hand-coded `cmd_add_extras` umbrella (grammars + gitleaks + embeddings) had drifted from the declarative `cmd_install_extras` umbrella (gitleaks + embeddings + rust-analyzer, WI-huham), so users invoking the more discoverable `add-extras` command never got rust-analyzer installed and the `--help` text didn't even mention it. `cmd_add_extras` now runs a fourth step that gates on the BUG-06 SCIP integration package the same way `cmd_install_rust_analyzer` does (refuses to install the rustup binary alone when the integration is missing — instead points the user at `pipx install 'hypergumbo[rust-analyzer]'`), and `cmd_remove_extras` runs a third step that uninstalls the rustup binary via `uninstall_rust_analyzer`. The `add-extras` / `remove-extras` `--help` strings list rust-analyzer alongside the other components.
...
- **Smart-test slice-fallback diagnostic file.** When `scripts/smart-test`'s reverse-slice path falls back to full-suite (either `slice command failed` or `no test files in slice result`), it now writes a self-contained diagnostic bundle to `.ci/smart-test-fallback.log` (gitignored, overwritten each fallback). The bundle records the timestamp, fallback reason, `STABLE_HG` path and `--version`, the `hypergumbo slice` command line invoked, slice exit code, slice wall-clock duration, slice stdout line count + first 50 lines, full slice stderr, and the changed-files list passed in. A one-line pointer is printed to stderr when fallback fires (no `--verbose` gate), so the next time the slice fallback fires organically we can inspect what went wrong without re-running. Motivated by a slice → full-suite fallback that cost ~12.5 minutes in the WI-morud session and could not be reproduced afterwards.

- **UAT directed-validation playbook gains a `Mechanism` check.** `process-validation-queue-with-bakeoffs-and-uat.md` adds an optional Phase U1 `Mechanism check` field where the orchestrator pre-commits one or two falsification probes for claims that name a specific mechanism, a `Mechanism` column on the Phase U3 verdict matrix (`matches` / `mismatch` / `n/a`), and a fourth Phase U4 verdict bullet for `moved` + `Mechanism: mismatch` — strip the validation tag (the public-facing claim is satisfied) AND file a `needs_human_review` follow-up to reconcile claim text against linker behavior. Surfaced by hg-uat-v4.1.0 Round 06 (WI-vigih view_template), where the round verdict resolved `moved` per pre-committed quantitative criteria but Round 08 §8.4 falsified the mechanism the claim text described (transitive base-class walk → actually filename convention).
...
- **`hypergumbo run` handler-slice fan-out is now co-located under `<out-stem>.slices/`** (WI-rimos / UX-A). `run_behavior_map` previously passed `out_path.parent` as the slice output directory, so `--out /some/path/foo.json` deposited 20-30 `slice.handler.<METHOD>.<path>.json` files directly in `/some/path/`. The hg-uat-v4.1.0 campaign that surfaced this had been pointing `--out` at `/tmp/round-NN.json`, so the slices ended up in `/tmp/` and successive rounds clobbered each other's slice artifacts (Round 07/containerd's slices were overwritten by Round 09/candle's). The fan-out now goes to a stem-derived subdirectory next to the `--out` target — e.g. `--out /some/path/foo.json` writes the main result at `/some/path/foo.json` and the per-handler slices at `/some/path/foo.slices/slice.handler.*.json` plus `/some/path/foo.slices/slice.handler.index.json`. The companion `index.json` keeps its existing relative-filename entries so consumers reading the index still resolve slice files relative to the index's directory. The `--no-handler-slices` switch is unchanged. The same shape applies when `--out` is omitted: slices land in `<cache_dir>/hypergumbo.results.slices/`.

- **`grpc-linker-v1` Go server method-to-RPC mapping is now file-scoped** (WI-kunoz / BUG-05). `_link_go_methods_to_rpc_routes` keyed its `struct_to_service` map by the bare struct short-name. When multiple Go files declare a struct with the same name (e.g. containerd's eight plugin packages — containers, content, diff, images, leases, mounts, namespaces, snapshots, tasks — each declare `type service struct { ... api.UnimplementedXxxServer ... }` for a different service), the dict overwrote on registration order; whichever file the `go_server_files` set happened to iterate last won the mapping for every other plugin's `service.Create`. On hg-uat-v4.1.0 round 07, this misresolved 7 service families' `implements_rpc` edges onto the unrelated `Leases/*` RPC family — zero correct edges. Distinctive struct names (`controllerService`, `sandboxService`, `server`) escaped the collision because they were unique across files. The map is now keyed by `(file_path, struct_name)`; the method-side lookup uses `(sym.path, struct_name)` so each plugin's methods resolve only against its own file's embedding. Both the Unimplemented-embedding scan and the ttrpc/CSI base-class fallback (the `XxxService` / `XxxServer` chain walker behind WI-pogus) use the same file-scoped key. The TTRPC-only DQ-4 sub-case in the BUG-05 spec — services with no `Unimplemented*Server` and only a TTRPC-flavored interface — is unchanged here; the design question of whether to emit nothing or a TTRPC-flavored route is left for a separate decision.

- **Rust `impl Trait for Type` now resolves trait-vs-struct short-name collisions across files** (WI-milak / BUG-04). The kind-discipline guard from WI-kahaz catches a same-name struct only when the survivor of `global_symbols`'s single-value dict overwrite happens to be the struct (in which case the lookup returns the struct and the kind check rejects it). When the trait wins the overwrite, the lookup returns the trait — but when the struct wins, the canonical trait is gone from `global_symbols` entirely and the impl_item handler emits an unresolved-trait edge instead of the canonical resolved one. On the candle ML framework's hg-uat-v4.1.0 round 04, this misresolved 350 of 559 `impl Module for X` edges (~63%) to `candle-kernels::Module struct` / unresolved (depending on registration order) instead of the canonical `candle-core::Module` trait. `RustAnalyzer.register_symbol` now also populates a kind-segregated multi-value index (`_kind_index_cache`, keyed on `id(global_symbols)` for auto-invalidation between `analyze()` runs); the impl_item trait lookup consults it, prefers `kind == "trait"` candidates, breaks ties by same-file path then stable id, and refuses to fall back to a struct/enum when no trait exists (preserving WI-kahaz). Targeted fix per acceptance path (B) in the WI-milak tracker spec; the architectural fix (path (A) — populate `meta.base_classes` and defer to `linkers/inheritance.py`) and the broader split-dict refactor (path (C) — kind-segregated multi-value `global_symbols` analogous to `go.py:3998`) remain follow-up work.

- **`--backend rust-analyzer` and `install-rust-analyzer` now error clearly when the SCIP integration package is missing** (WI-jinoh / BUG-06). The published v4.1.0 distribution does not include `hypergumbo-lang-rust-analyzer` in `Requires-Dist` (and the package is not on PyPI), so the documented `--backend rust-analyzer` flag and the `hypergumbo install-rust-analyzer` subcommand both passed argparse and ran without error while the SCIP backend silently fell through to the default tree-sitter analyzer — a fully no-op CLI surface that misled multiple bakeoff rounds (Round 09 and 10's null engagement got blamed on flag positioning before the real cause was found). New helper `is_rust_analyzer_integration_installed()` (`rust_analyzer_install.py`) uses `importlib.util.find_spec` to detect the package side-effect-free; `cli.py` now exits non-zero with `"hypergumbo-lang-rust-analyzer Python integration package is not installed"` when the user requests `--backend rust-analyzer` (either form: `--backend rust-analyzer` or `--backend=rust-analyzer`) without the integration package available, and `install-rust-analyzer` refuses to install the rustup binary alone (which would leave the SCIP backend a no-op). `install-rust-analyzer --check` now reports both the binary status AND the integration-package status as separate lines, exiting 1 if either is missing, with a BUG-06 pointer when the integration is absent. Minimum-viable fix per the WI-jinoh tracker spec; the structural fix (publishing `hypergumbo-lang-rust-analyzer` to PyPI as a `[rust-analyzer]` extra) remains follow-up work.

- **VHDL `architecture X of Y` now kind-prefers entity over a same-named package / architecture / component** (WI-morud, sibling to BUG-04 / WI-milak in `rust.py`). `vhdl.py`'s global registry indexed entities, architectures, packages, and components together by lowercased name (`global_symbols[symbol.name.lower()] = symbol`), single-value, last-write-wins. The architecture-of-entity lookup at `vhdl.py:255` was kind-blind, so an IP-block library with both `package Foo` and `entity Foo` could mis-resolve `architecture Bar of Foo` to the package depending on insertion order. The registry is now multi-value (`dict[str, list[Symbol]]`, mirroring `go.py:3998`); the lookup picks the entity candidate via a `next(... for c in candidates if c.kind == "entity")` filter and falls back to the existing `vhdl:external:{name}:entity` synthetic ID at confidence 0.70 when no entity matches. `resolver_class` switches to `ListNameResolver` so the type contract on `extract_edges_from_file`'s `resolver` parameter remains honest (the analyzer doesn't itself call `resolver.lookup`).
