...
### Fixed
- **`ci-debug logs` can now actually fetch Woodpecker logs, and says what is missing when it cannot (WI-solob).** A failing pipeline was undiagnosable from the agent side: `_github_fetch_job_log` was a stub that resolved the run URL from the commit status and printed "not retrievable". This session hit exactly that cost — a PR failed CI twice, and two blind re-runs were spent before the owner read the log and identified the cause in one line (a stale `docs/ARCHITECTURE.md`, not the change under review). **Measured finding: allowlisting the CI host is necessary but NOT sufficient** — an unauthenticated GET of the run URL returns **403**, because Cloudflare Access fronts the instance. Two independent credentials are therefore required, and the function now uses both when present: `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET` (a Cloudflare Access *service token*, sent as `CF-Access-Client-*` headers, to pass the edge) and `WOODPECKER_TOKEN` (a Woodpecker API token, sent as Bearer, to authenticate to the app). Both are read from `.env` alongside the existing `FORGEJO_TOKEN` / `HG_GITHUB_TOKEN` and never echoed. The pipeline coordinates are parsed out of the `target_url` the status already carries, so no new configuration is introduced — the value the stub used to print is now the value it fetches. **When either credential is absent the behaviour degrades to exactly the previous message plus a line naming what to set**, so the script is strictly more capable and never less. `ci.revolving-tactile.com.de` is added to `ALLOWED_WEBSITES.md` (owner-approved) with a comment recording why: without it the run URL the tool itself prints is unfetchable.
