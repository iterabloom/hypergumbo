...
- **HIGH_RISK_PRIMITIVES drift guard, Part 2 (missing-entry direction)** — property test asserts every catalog entry with `boundary=subprocess` is classified in either `HIGH_RISK_PRIMITIVES` or the new `HIGH_RISK_EXEMPTIONS_SUBPROCESS` frozenset, closing the gap Part 1 (WI-gitad) did not cover. Backfilled 48 missing subprocess-launching primitives across Go (`os/exec.Cmd.*`, `syscall.Exec`, `execabs.*`), JVM (`java.lang.ProcessBuilder.command`, `scala.sys.process.*`), Node (`child_process.execFile`/`fork`), C/C++ (`unistd.exec[le|lp|v|ve|vp]`, `spawn.posix_spawn*`), Elixir (`System.cmd`/`shell`, `Port.*`), Haskell (`System.Process.*` and `System.Process.Typed.*`), Swift (`Process.launchPath`), Objective-C (`NSTask.launch*`), and Rust (`std::process::Command.new`). Exempted 18 wait/signal/PATH-lookup/self-exit entries (they're subprocess-boundary for taint tracking but don't represent arbitrary code execution). Failure message follows a 5-element reader-friendly contract (header + WHY + sorted list with source catalogs + HOW TO FIX + re-run command), independently locked by `test_subprocess_drift_message_formatter_shape` so refactors can't silently regress the developer-facing scaffolding (WI-sugav).
