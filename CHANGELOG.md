...
- **Swift Vapor/Hummingbird route extraction**: SwiftAnalyzer now implements `extract_usage_contexts_from_file()` to detect route registrations (`app.get("path")`, `routes.post("users")`, etc.). Creates UsageContext records matching vapor.yaml and hummingbird.yaml usage-based patterns, enabling framework pattern matching, route entrypoint detection, and HTTP linker resolution. Supports `app`, `routes`, and `router` receivers with all HTTP methods.
