...
- **WI-fipab-kivoj: source-built tree-sitter grammars (lean, wolfram, circom) are now vendored under `vendor/tree-sitter-*/` instead of cloned at build time.** Eliminates the upstream-force-push failure mode that broke nightly's source-grammar build on 2026-05-20 (when `github.com/Julian/tree-sitter-lean` was force-pushed and the pinned commit `efe6b87` became unreachable). Both build paths — `scripts/build-source-grammars` (bash, CI/dev) and `hypergumbo_core.build_grammars` (Python, user-facing `hypergumbo build-grammars`) — now read `src/parser.c` + `scanner.{c,cc}` + `tree_sitter/` headers directly from the vendor tree; no `git clone`/`git pull` step remains. Each vendored grammar ships its upstream `LICENSE` (MIT for all three current cases) and an `UPSTREAM` file recording the upstream URL, pinned commit, and sync date for the re-sync procedure. Approach was plain directory copy (rather than git-subtree) so the hypergumbo changelog isn't swamped by thousands of upstream commits — see `docs/grammars/vendor-sync.md` for the rationale and the five-step re-sync procedure. New `TestVendorLayout` class in `test_build_grammars.py` pins five vendor-layout invariants: `VENDOR_ROOT` resolves to `<repo>/vendor`; `vendor_dir_for(name)` constructs the right path; every `SOURCE_GRAMMARS` entry has a vendored `src/parser.c`; each grammar's `LICENSE` exists and contains "MIT"; each grammar's `UPSTREAM` file records the upstream URL and a pinned `Commit:`. Three `TestBuildGrammar` tests removed (`test_build_grammar_git_not_found`, `test_build_grammar_clone_failure`, `test_build_grammar_updates_existing_repo`, `test_build_grammar_git_pull_failure_ignored`) since the build path no longer issues `git` commands; replaced by `test_build_grammar_missing_vendor_source` (returns False with a pointer at the re-sync doc) and `test_build_grammar_runs_pip_install_only` (regression guard: no `git` command appears in the issued subprocess set). Total +12MB to the repo (10.7MB of which is `tree-sitter-lean/src/parser.c`); vendor/ is outside every package's `packages = ["src/<pkg>"]` so the pip-installed wheels are unchanged in size. Closes WI-fipab-kivoj (P2, tree_sitter + grammar_build + nightly + infrastructure + follow_up).

### Added (post-5.0.1)

- **WI-hatip (PR-2 of INV-nilud `inherited_calls` campaign): new `linkers/inherited_calls.py` Tier-2 Infrastructure linker + Ruby `include`/`extend` mixin visibility (WI-puluf closure).** First substantive PR of the 5-PR campaign — canonical plan at `~/puluf-plan.md`. Closes WI-puluf (Ruby Sidekiq workers / ActiveModel validations / Rails concerns invisible to the call graph) via the same substrate that will subsume the Java + Ruby inherited-call walks in PRs 3-5. New `packages/hypergumbo-core/src/hypergumbo_core/linkers/inherited_calls.py` (priority=18, always-active) walks ancestor chains to resolve unresolved `calls` edges carrying the `enclosing_class` hint added by `make_unresolved_edge` in PR-1. Per-language MRO via a hardcoded `_MRO_WALKERS` dispatch table (YAML/decorator-hooks rejected as ceremony for static language semantics, per WI-hatip design discussion); PR-2 ships `_walk_insertion_order` only — covering Ruby and Groovy with declaration-order BFS through `extends`/`implements`/`includes` edges, per-source visited-set cycle guard, depth cap=10 matching the existing Java/Ruby walks. Site-1 resolver (analyzer hint: `enclosing_class`) emits `evidence_type="ast_call_inherited"` at confidence 0.90, preserving Java precedent (the prior Ruby walk used 0.85 — harmonized to 0.90 to match the per-Site contract). Ruby Pass-1 analyzer (`ruby.py::_extract_included_modules`) now scans class/module body for top-level `include ModuleName` / `extend ModuleName` `call` AST nodes and populates `meta["included_modules"]` with declaration-order qualified names (e.g., `["Sidekiq::Worker", "ActiveModel::Validations"]`); only literal `constant`/`scope_resolution` arguments are captured, dynamic `include some_method_call` is silently dropped. Centralized `linkers/inheritance.py` extended with a new `_create_includes_edges` helper that emits `edge_type="includes"` + `evidence_type="ast_includes"` edges from `included_modules` metadata when the named module is a project Symbol (same disambiguation cascade as extends/implements — qualified-name first, short-name fallback, INV-zuhub-compliant confidence downgrade on simple-name fallback); `_build_symbol_maps` grown to return a third `module_by_name` map for the lookup. The existing canonical `includes` edge type description in `edge_types.py` widened to call out Ruby mixin coverage; new `EvidenceTypeSpec("ast_includes", ...)` registered for the mixin pathway (distinct from the existing C/C++ preprocessor `ast_include`). Schema + concept-axes regenerated. Ruby analyzer's prior `_find_inherited_initialize` (~70 LOC at `ruby.py:1901-1956`) **deleted** — the receiver-is-project branch in `_try_receiver_call::new` now emits one unresolved-call edge per candidate class with the `enclosing_class` hint, and the linker performs ancestor walking. Direct-resolution path (`Class#initialize` in global symbols) unchanged. Linker registered in `cli.py`'s explicit import list to fire during the standard pipeline. 31 new tests: 7 in `TestRubyIncludedModulesExtraction` (single/multiple include, extend treated the same, no-include leaves meta absent, module-on-module, combines with base_classes, dynamic-arg ignored); 7 in `TestIncludesEdges` (class/module emitters, extends+includes coexist, external skip, qualified→short fallback, self-include skip, dedup); 18 in `test_inherited_calls_linker.py` covering `_walk_insertion_order` (direct/one-hop/three-hop/no-match/mixin/depth-cap/cycle), `_MRO_WALKERS` registry, and end-to-end resolution via extends + includes including the defensive paths (no enclosing-class hint, unregistered language, dst-not-parseable, src-not-known, no in-tree class). Four existing `TestRubyNewConstructorResolution` integration tests (`test_inherits_initialize_via_three_level_chain`, `test_inherits_initialize_via_parent_chain`, `test_scope_resolution_namespaced_with_leading_colon_colon`) migrated to pipeline-mode via the new `_run_inherited_calls_pipeline` helper at the top of `test_ruby.py` (composes `link_inheritance` + `link_inherited_calls`). 100% coverage on `linkers/inherited_calls.py` (81 statements). New ADR `docs/adr/0029-cross-language-inherited-call-linker.md` documents the contract + per-language MRO table + 5-PR campaign sequence. **Tag at merge:** `awaits_bakeoff_validation` — BROAD cohort: chatwoot (Ruby mixin coverage), hypergumbo-self (Python regression check), spring-petclinic (Java canary; should be byte-identical this PR). WI-puluf-nutuv-mahol-lopov-dahif-sabus-baban-nokag transitions to `done`.

### Changed (post-5.0.1)

- **WI-puvil (PR-5 of INV-nilud `inherited_calls` campaign): Java Sites 2 (typed-receiver) and 3 (inherited-field-receiver) lifted to the Tier-2 `inherited_calls` linker; META INV-nilud closes pending one BROAD bakeoff cycle.** Fifth and final PR of the 5-PR campaign — canonical plan at `~/puluf-plan.md`. Closes META `INV-nilud-zivag-jibak-danov-polak-sibam-fahif-katat` ("Inheritance-aware call resolution lives in `linkers/inherited_calls.py`, not in language analyzers"). Two new resolvers in `linkers/inherited_calls.py` consume PR-4's `Edge.meta` hints + a new analyzer-side class-meta channel. **(1) Site 2 resolver — `_resolve_site2`.** When an unresolved `calls` edge carries `receiver_type_hint`, the linker performs a three-step resolution preserving the analyzer's pre-PR-5 behavior plus a forward-improvement step: (a) direct method lookup on the type via `methods_by_short_name[type_class_id]` → `ast_call_type_inferred` at 0.85 (the same case the analyzer's Case 3 IF branch handled); (b) MRO walk via the registered per-language walker (`_walk_single_then_interfaces` for Java) → `ast_call_inherited_method` at 0.70 (forward improvement: the analyzer never walked the chain for Site 2); (c) fallback to the type symbol itself → `ast_call_inherited_method` at 0.70 (the same case the analyzer's Case 3 else fallback handled). Steps (a) and (c) work without a registered MRO walker; (b) requires one. **(2) Site 3 resolver — `_resolve_site3` + `_walk_parents_for_field` helper.** When an unresolved `calls` edge carries both `inherited_field_receiver` AND `enclosing_class`, the linker walks the enclosing class's parent chain via `extends` / `implements` / `includes` edges (emitted by `inheritance.py`) and reads each parent symbol's `meta["fields"]` (newly populated by the Java analyzer in PR-5) for a matching field name. When found, the field's type name is looked up as a class symbol, and the method is resolved on that type's MRO (direct or walker-driven). Emits `ast_call_inherited_field` at confidence 0.80. **Dispatch by hint specificity.** A new `_try_resolve` helper centralizes the per-edge dispatch: Site 3 fires when both `inherited_field_receiver` and `enclosing_class` are present; Site 2 when `receiver_type_hint` is present; Site 1 when only `enclosing_class` is present. Each unresolved edge emits at most one resolved edge from this linker. Site 3 finding nothing does NOT fall through to Site 1 — the call shape is structurally `field.method`, not bare. **Analyzer changes.** Java analyzer at `java.py` (1) populates each class symbol's `meta["fields"] = {name: type_name}` from the existing `global_class_fields` table after Pass 1 (the linker's single source of truth for "what fields does this class have?"); (2) Case 3 body (~60 LOC, `var.method()` direct + fallback emission) reduced to a `pr4_receiver_type_hint = var_types[receiver_name]` stash plus a side-effect-only `resolver.lookup(...)` that re-populates `resolved_sym` so the downstream return-type-inference cascade keeps chaining `var_types` across factory-method patterns (e.g., `Factory f = new Factory(); Client c = f.createClient(); c.send()` — without the side-effect lookup, `c`'s type would never be inferred and `c.send()` would lose its `receiver_type_hint`); (3) Case 3.5 body (~40 LOC, `field.method()` parent-chain walk + emit) reduced to `pr4_inherited_field_receiver = receiver_name` plus `pr4_enclosing_class_hint = current_class` so the linker has both hints. Net deletion ≈ 95 LOC from the Java analyzer's edge-emission block; net addition ≈ 200 LOC of linker logic in one centralized module. **Bakeoff signal expected:** `ast_call_inherited_method` + `ast_call_inherited_field` counts within ±1 on spring-petclinic and ±0.5% on spring-boot. Forward improvement: cases where the method was on an ancestor of the type (Site-2 step b) previously fell back to the type symbol (analyzer Case 3 else) and now resolve to the actual method — different `dst` target, same evidence-type count. Chatwoot unchanged (no Java effect). 13 new tests in `test_inherited_calls_linker.py`: 6 `TestSite2TypedReceiverResolution` (direct match, MRO walk on parent, fallback to type symbol, no-resolution when type isn't a project class, no-hint, unregistered-language direct-match path); 7 `TestSite3InheritedFieldResolution` (immediate parent, grandparent, no parent declares field, field-type not a project class, method not on field type, no enclosing_class hint, Site-3-priority-over-Site-1). 9 existing Java tests migrated to pipeline-mode via the established `_run_inherited_calls_pipeline` helper at the top of `test_java.py`: `test_extracts_field_access_method_calls`, `test_variable_method_call_resolved_via_type_inference`, `test_return_type_enables_method_resolution`, `test_jpa_inherited_method_falls_back_to_type`, `test_jpa_declared_method_resolves_normally`, `test_parameter_type_inference_basic`, `test_inherited_field_method_call`, `test_inherited_field_from_grandparent`, `test_site2_resolved_fallback_through_linker` (renamed from PR-4's regression-guard `test_site2_resolved_fallback_unchanged`). ADR-0029 updated to record the actual `type_hierarchy` priority of 60 (the original ADR + `~/puluf-plan.md` said 20; transcription error caught during PR-5). `linkers/inherited_calls.py` module docstring rewritten to document all three Sites and the dispatch order. 100% coverage on the linker (177 statements) and the Java analyzer (884 statements). **Tag at merge:** `awaits_bakeoff_validation` — BROAD cohort: spring-petclinic (canonical Java sample), spring-boot (large Java), chatwoot (Ruby canary — should be byte-identical). META INV-nilud transitions to `pending_validation` on merge and to `satisfied` after one BROAD cycle confirms no regression. Closes WI-puvil (P3, cross_language_parity).

- **WI-sivuk (PR-4 of INV-nilud `inherited_calls` campaign): Java analyzer threads Site-1 / Site-2 / Site-3 hints into the single final unresolved emit; PR-3 double-emit regression repaired in the same refactor.** Fourth of five sequential PRs under META `INV-nilud`. The change consolidates the analyzer's hint emission for inherited-call cases through one site instead of two. **Mechanism.** Three new method-local hint vars (`pr4_enclosing_class_hint`, `pr4_receiver_type_hint`, `pr4_inherited_field_receiver`) are initialized to `None` at the top of each call-node visit and populated when the resolution cascade enters the corresponding case: Case 1 (`this.method()` / bare `method()` — receiver_name `None` or `"this"` with a known `current_class`, lookup failed) stashes `current_class`; Case 3 (`var.method()` typed receiver — `receiver_name in var_types`) stashes `var_types[receiver_name]`; Case 3.5 (`field.method()` inherited-field receiver — `receiver_name not in var_types and receiver_name not in class_symbols` with non-empty `class_fields` and known `current_class`) stashes `receiver_name`. All three hints are threaded into the single final `make_unresolved_edge(...)` call (`java.py:~1722`), landing in `Edge.meta` under the keys (`enclosing_class`, `receiver_type_hint`, `inherited_field_receiver`) that PR-1's `make_unresolved_edge` contract already provisioned. **PR-3 double-emit regression repaired (WI-dukog follow-up).** Pre-PR-4 empirical verification on a 2-class extends-chain fixture: every bare/`this.method()` inherited-call site produced *two* unresolved edges, one with `enclosing_class` hint (Case 1's `else` at `java.py:1459-1464`) and one without (the final unresolved emit at `java.py:~1647`). Root cause: PR-3's Case 1 `else` block called `make_unresolved_edge` directly but neglected to set `edge_added = True`, so the final fallback emit fired a second time for the same call site, dangling a redundant hint-less unresolved edge after the linker resolved the hint-bearing one. Existing PR-3 tests didn't catch this because they filtered `is_resolved=True` and only asserted the resolved edge count (= 1) — the redundant unresolved edge stayed invisible. The repair is structural rather than a one-line `edge_added` flag: Case 1's `else` no longer emits at all and now only stashes the hint, deferring to the single final emit. This shape preserves the existing `static_imports` / `wildcard_imports` / `dst_ref` computation that the final emit owns (the polyglot-call-site-coverage test on `import static java.util.Collections.singletonList` regressed under the simpler one-line flag fix because Case 1's standalone emit lacked the static-import lookup; the regression caught the design weakness in TDD and produced the cleaner shape). **Bakeoff signal expected.** Java edge count *decrease* on the cohort, proportional to the count of bare/`this` inherited-call sites (one redundant unresolved removed per site). This violates WI-sivuk's drafted "total Java edge count within ±0.5%" success criterion — updated to "Java unresolved-edge count decreases by approximately the count of `ast_call_inherited` resolved edges on the same cohort, with no decrease in `is_resolved=True` edge count." Spring-petclinic pre-PR-4 baseline will be captured per the campaign verification protocol; PR-5's bakeoff operates on the clean PR-4 baseline. **PR-5 forward path.** Linker will consume the new `receiver_type_hint` and `inherited_field_receiver` meta keys via two new resolver branches in `link_inherited_calls` (typed-receiver MRO walk + inherited-field-via-parent-chain walk) and emit `ast_call_inherited_method` / `ast_call_inherited_field` resolved edges; the still-in-analyzer Site-2/3 resolved paths at `java.py:1518-1549` and `java.py:1555-1594` then become deletable. 6 new tests in `TestJavaSite2Site3UnresolvedHints` (`test_java.py`): `test_case1_else_emits_single_unresolved_edge` + `test_this_else_emits_single_unresolved_edge` cover the regression repair (each asserts exactly one unresolved edge per call site, with `meta == {"enclosing_class": ...}`); `test_site2_unresolved_carries_receiver_type_hint` covers Site-2 hint threading (`InputStream stream` param → `stream.read()` unresolved edge carries `receiver_type_hint=InputStream`); `test_site3_unresolved_carries_inherited_field_receiver` covers Site-3 hint threading (`log.info(...)` on a class extending a parent with `otherField` → unresolved edge carries `inherited_field_receiver=log`); `test_site2_resolved_fallback_unchanged` is a regression guard that the existing `ast_call_inherited_method` edge to the interface symbol still fires on the JPA-repo pattern (i.e., PR-4's hint capture didn't break the still-in-analyzer Site-2 resolved path); `test_case4_unresolved_has_no_site_hints` asserts that `Arrays.asList(...)` (Case 4 imported-class fallback) doesn't accidentally pick up either hint. All 10 cases in `test_polyglot_call_site_coverage.py` still pass (the Java fixture exercises `static_imports`-driven module path resolution on `singletonList` — the regression that disqualified the simpler one-line fix). Java analyzer at 100% coverage; the three hint vars are exercised by Site-1/2/3 tests respectively and stay `None`-default for the Case-4-only test. **Tag at merge:** `awaits_bakeoff_validation` — BROAD cohort: spring-petclinic (canonical Java sample), spring-boot (large Java). Closes WI-sivuk (P3, cross_language_parity).

- **WI-dukog (PR-3 of INV-nilud `inherited_calls` campaign): Java Site-1 (bare / `this.method()`) ancestor walk lifted from `java.py:1437-1461` into the Tier-2 `inherited_calls` linker.** Third of five sequential PRs under META `INV-nilud`. New walker `_walk_single_then_interfaces` added to `linkers/inherited_calls.py`: BFS with per-node parent ordering by edge-type priority (`extends` → `implements` → `includes`), preserving Java's single-superclass MRO before interface default-methods (Java 8+). Walker signatures uniformized to take `inheritance_index: dict[str, list[tuple[str, str]]]` so both walkers can read the per-parent `edge_type`; `_walk_insertion_order` (Ruby/Groovy) keeps its insertion-order semantics by iterating the typed list in order and ignoring `edge_type`. New helper `_build_typed_inheritance_index(edges, edge_types)` replaces the call into `_transitive_bases.build_inheritance_index` inside `link_inherited_calls`; the shared helper keeps its older flat-list return type for the other linkers (`jackson_dispatch` / `airflow_framework_dispatch` / `view_template_django`) that don't care about edge-type tagging. `_MRO_WALKERS` registry adds `"java": _walk_single_then_interfaces`. Java analyzer's `else:` branch at `java.py:1437-1461` (the `class_parents.get(...)`-driven 10-hop chain) is replaced with a single `make_unresolved_edge("java", current_method.id, method_name, ..., enclosing_class=current_class)` call; the linker resolves and emits the `ast_call_inherited` edge at confidence 0.90, byte-identical to the prior in-analyzer emission. The new walker additionally lets interface default-methods resolve (the old in-analyzer walk only chased `extends`, never `implements`), a forward improvement worth bakeoff observation. **Acknowledged behavior change:** the analyzer's downstream return-type-inference cascade (`resolved_sym` populated for the inherited case) no longer fires for inherited Site-1 matches — the analyzer hands off resolution before knowing what was matched. Spring-petclinic / spring-boot bakeoff will quantify any `ast_call_type_inferred` count shift; if material, a follow-up can restore the analyzer-side lookup for return-type purposes only while keeping edge emission in the linker. 8 new tests: 7 in `TestWalkSingleThenInterfaces` (direct match, two-hop extends, extends-before-interface priority with the interface listed first in the index, falls-through to interface when extends chain lacks the method, returns-None, depth-cap, cycle protection); 1 `test_java_resolves_to_single_then_interfaces` registry assertion. 2 new end-to-end `TestEndToEndInheritedCalls` cases (extends-chain resolution, implements-default-method fallback) plus the existing `test_no_op_for_unregistered_language` re-pointed at Python (was Java in PR-2, now registered). All 7 existing `_walk_insertion_order` walker tests + 5 existing end-to-end tests updated to pass tuple-typed indices. 4 `TestJavaInheritedMethodResolution` Site-1 tests (`test_bare_call_to_parent_method_resolves`, `test_this_call_to_parent_method_resolves`, `test_grandparent_method_resolves`, `test_own_method_preferred_over_parent`) migrated to pipeline-mode via a new `_run_inherited_calls_pipeline` helper at the top of `test_java.py` (mirrors the Ruby shape from PR-2). The 2 Site-3 inherited-field tests (`test_inherited_field_method_call`, `test_inherited_field_from_grandparent`) still drive the analyzer directly — they will migrate when PR-5 lifts Site 3. 100% coverage on `linkers/inherited_calls.py` (107 statements). **Tag at merge:** `awaits_bakeoff_validation` — BROAD cohort: spring-petclinic (canonical Java sample), spring-boot (large Java), chatwoot (Ruby canary — should be byte-identical this PR). Closes WI-dukog (P3, cross_language_parity).

- **WI-gifar (PR-1 of INV-nilud `inherited_calls` campaign): contract + shared-helper refactor — additive, behaviour-preserving.** First of five sequential PRs filed under the new META invariant `INV-nilud` ("Inheritance-aware call resolution lives in `linkers/inherited_calls.py`, not in language analyzers"). Surfaced by WI-puluf's Ruby `include`/`extend` mixin-resolution gap which exposed a deeper structural duplication: Java (`java.py:1414-1591`) and Ruby (`ruby.py:1901-1956`) independently re-implement inheritance-aware call resolution, while the other 12 analyzers share the same silent gap. The campaign builds a Tier-2 Infrastructure linker that owns per-language MRO walking; this PR lays only the contract + helpers, no new linker yet. Changes: (1) `analyze/base.py::make_unresolved_edge` gains three optional kwargs — `enclosing_class`, `receiver_type_hint`, `inherited_field_receiver` — that land in `Edge.meta` when non-None; existing call sites unchanged at default. (2) `linkers/inheritance.py::_resolve_target_symbol` promoted to public `resolve_target_symbol`; `_resolve_target_symbol = resolve_target_symbol` alias preserves backward compat for any external pinned callers. (3) `linkers/method_call_recovery.py::_short_name` and `::_parse_unresolved_name` promoted to public `short_name` / `parse_unresolved_name`; underscore aliases preserved. (4) `linkers/_transitive_bases.py::build_inheritance_index(edges)` gains `edge_types: tuple[str, ...] = ("extends", "implements")` kwarg so PR-2's Ruby `includes` edge can participate in transitive ancestor walks; default tuple preserves the historical behaviour every existing caller (`jackson_dispatch`, `airflow_framework_dispatch`, `view_template_django`) relies on. (5) `linkers/type_hierarchy.py::_TypeHierarchyIndex.build` extracted into module-public `build_method_index()`; the classmethod is preserved as a one-line delegate so the upcoming `inherited_calls` linker (priority=18, runs before `type_hierarchy` at priority=20) can independently construct the same short-method-name index without reaching for a private name. 31 new tests: `TestMakeUnresolvedEdge` (7 cases on the 3 kwargs + backward-compat), `TestBuildInheritanceIndexEdgeTypesKwarg` (4 cases on default/custom/includes-only/empty), `TestResolveTargetSymbolPublic` (4 cases including `is`-identity with the private alias), `TestShortNamePublic` + `TestParseUnresolvedNamePublic` (10 cases on dot/hash/colon separators + dst-format + identity), `TestBuildMethodIndexPublic` (5 cases including legacy-classmethod delegation). No behaviour change on any active code path; all 313 tests across the five touched modules pass. No bakeoff claim (pure refactor). PR-1 of WI-gifar / WI-hatip / WI-dukog / WI-sivuk / WI-puvil chain under INV-nilud — canonical plan at `~/puluf-plan.md`.

- **WI-mugog Phase A: URL-folding logic moved out of `linkers/http.py` into a per-idiom YAML + engine substrate under `hypergumbo_core/url_folding/`.** Behaviour-preserving refactor establishing the substrate for Phase B/C/D extensions to URL folding across languages (Python f-strings, Go `fmt.Sprintf`, Kotlin/Ruby/Scala interpolation, Java `String.format`/`StringBuilder`, ...) — the parent invariant INV-miloj's refined closure criterion ("every active route-detector language has a YAML entry under `url_folding/` for at least one composition idiom OR is listed in `url_folding/SCOPE.md` as intentionally literal-only"). New package `hypergumbo_core/url_folding/` ships two engine functions (`fold_string_interpolation(template, consts, placeholder_pattern)` generalises the legacy `_fold_template_literal` parametrised by the slot regex; `fold_array_join(items, separator)` generalises the legacy `_fold_elm_string_join` parametrised by the separator string), an `ENGINES` callable-registry, a `UrlFoldingVariant` record + `load_url_folding_registry()` loader that reads every `*.yaml` sibling, `get_covered_languages()` / `get_scoped_languages()` helpers consumed by the property test, and a `SCOPE.md` documenting Phase-A literal-only languages with one-line justifications. Two YAML files declare current coverage: `string_interpolation.yaml` (idiom=string_interpolation, engine=fold_string_interpolation, languages={javascript, typescript} both carrying `placeholder_pattern: '\$\{([^}]+)\}'` — matches the legacy `_TEMPLATE_SLOT` regex bit-for-bit) and `array_join.yaml` (idiom=array_join, engine=fold_array_join, languages={elm}, `separator: "/"` — matches the legacy hardcode in `_fold_elm_string_join`). `_fold_template_literal` and `_fold_elm_string_join` retained in `linkers/http.py` as thin one-line wrappers that delegate to the engines with the YAML-derived parameters loaded at module import (`_URL_FOLDING_CONFIG_BY_LANGUAGE = {v.language: v.config_dict() for v in load_url_folding_registry()}`, then dict-indexed for `_JS_PLACEHOLDER_PATTERN` and `_ELM_ARRAY_JOIN_SEPARATOR` — missing-key failure is surfaced loud at import, not silently fallen back). Dead `_TEMPLATE_SLOT` regex constant removed. 24 new tests in `tests/test_url_folding.py`: 9 `TestFoldStringInterpolation` cases mirror the existing `TestFoldTemplateLiteral` cases plus a `test_engine_accepts_arbitrary_placeholder_pattern` Phase-B forward check confirming the engine is genuinely generalised (folds a Python f-string-style `{name}` placeholder when supplied with `r'\{([^}!:]+)\}'`), 5 `TestFoldArrayJoin` cases covering the legacy Elm `_fold_elm_string_join` shape including `test_separator_is_parametric` against a `.`-joined hypothetical future variant, 2 `TestActiveRouteLanguageCoverage` cases (the property test asserting every {python, javascript, typescript, go, ruby, java, elm} active route-language is covered by a YAML or scoped in SCOPE.md, and a disjointness assertion that a language is exactly one of covered/scoped never both), 6 `TestRegistryLoader` sanity checks (3 variants total per current YAMLs, every YAML engine name resolves to `ENGINES`, string_interpolation variants carry `placeholder_pattern`, array_join variants carry `separator`, Elm separator is `/`, SCOPE.md parses to exactly `{python, go, ruby, java}`), 2 `TestEngineRegistry` checks (both Phase-A engines present, all entries callable). 100% coverage on `url_folding/__init__.py` (63 statements) and the modified `linkers/http.py` (412 statements). All 197 existing `test_http_linker.py` tests pass unchanged — zero behaviour change on JS/TS template-literal folding or Elm `String.join` folding. Dispatcher helper (`_apply_url_folding_yaml`) intentionally deferred to Phase B: with only two variants today, the abstraction would have to handle Elm's two-pass let-binding scan as a special case from day one; the dispatcher returns when Phase B adds a second variant per idiom and the abstraction earns its complexity. Per-language scanners (`_scan_javascript_file`, `_scan_elm_file`) continue to call `_fold_template_literal` / `_fold_elm_string_join` inline. Closes WI-mugog Phase A (P3, cross_language_linkers + analysis_quality + http_linker + url_folding + refactor). INV-miloj's closure criterion is not yet met by this PR alone — sibling phases B-D under the same parent are still required for the actual scope decision to land.

- **WI-tukif batch 1: Rust and Erlang `io_primitives` catalogs promoted to `status: complete` with `stdlib_provenance`.** First batch of the 12-catalog umbrella under parent INV-pomir ("every catalog with status=complete MUST declare stdlib_provenance whose source_url hostname suffix-matches ALLOWED_PROVENANCE_HOSTNAME_SUFFIXES"). Rust audited against doc.rust-lang.org/std/ (Rust 1.78 stable); strict-stdlib coverage of std::fs, std::io, std::net, std::process::Command, std::env, and std::path::Path is comprehensive; std::os raw-fd extensions and std::ffi (meta-IO, not IO) intentionally absent per the Plan C strict-stdlib rule. Erlang audited against erlang.org/doc/ (OTP 26); two coverage gaps closed before promotion — `code` module added to `fs_read` (`load_file`, `ensure_loaded`, `get_object_code`, `lib_dir`, `priv_dir`, `root_dir`, `get_path`, `is_loaded`, `where_is_file`) since the code server reads `.beam` files from disk, and `inet` module added to `net_recv` (`getaddr`, `getaddrs`, `gethostbyname`, `gethostbyaddr`, `gethostname`, `getservbyname`, `getservbyport`) since DNS resolution queries name servers; gen_tcp/gen_udp/gen_sctp/ssl/httpc/httpd, ets/dets/mnesia, OTP behaviours (gen_server/gen_statem/gen_event/supervisor), os/application/init/rpc/global, and io/io_lib/logger remain covered as before. Two new tests in `TestCatalogStatus` (`test_rust_catalog_is_complete_with_provenance`, `test_erlang_catalog_is_complete_with_provenance`) pin the promotion; `test_other_catalogs_declare_status_in_progress` updated to drop rust and erlang from the still-pending list. Remaining 10 catalogs (c, elixir, go, haskell, java, javascript, kotlin, objc, scala, swift) stay `in_progress`. Discussion entry on WI-tukif documents the per-language sequencing for the remaining work. WI-tukif (P4, analysis_quality + someday) stays open with progress.

### Removed (post-5.0.1)

- **WI-karad: `apply_sibling_impl_weights` removed from the canonical dampener stack (8 stages → 7).** Filed by WI-lidum's 6-repo audit (alertmanager + prometheus + kserve + chatwoot + detekt + django) as "0 top-100 movement across all 24 cells" — the dampener was added based on WI-luvaj's alertmanager finding (19 Notifier.Notify variants flooding top-N) but the audit couldn't measure any user-visible effect. Diagnostic this PR (alertmanager + hypergumbo self + django; 3 repos × 1 surface): `apply_sibling_impl_weights` IS firing on the very groups it was designed for (Notifier.Notify=18 on alertmanager, Command.handle=57 + add_arguments=47 on django) and dampens 108-418 symbols on raw centrality across the three repos, but 0/100 top-100 positions change on any repo in the canonical pipeline — neither membership nor ordering — and none of the 108-418 dampened symbols are in top-100 anyway. Mechanism (Hypothesis 1 in the tracker description, confirmed): `apply_common_method_name_weights` (threshold=10) runs first in the canonical stack and dampens groups of size 11+ uniformly by 10/count (Notifier.Notify=18 → 0.56x; Command.handle=57 → 0.18x), which is already enough to push every tail member out of top-100 by itself; the groups of size 6-10 that fall uniquely to sibling_impl have low enough raw centrality that they were never in top-100 anyway. Removal scope: `apply_sibling_impl_weights` function deleted from `ranking.py`; `"sibling_impl"` dropped from `_CANONICAL_DAMPENERS` tuple (8 → 7 entries) and from `_apply_canonical_dampeners` conditional dispatch; `"sibling_impl"` dropped from `_VICTIM_REMOVAL_EXCLUDE_DAMPENERS` in `compact.py`; docstring on `compute_dampened_centrality` updated (8-dampener → 7-dampener); `TestApplySiblingImplWeights` class (4 tests) deleted from `test_ranking.py`; `test_sibling_impl_tail_factor_pinned` deleted; `test_compute_dampened_centrality_applies_stack_in_order` updated to drop the apply_sibling_impl_weights call; `test_canonical_dampener_order_pinned` updated to expect the 7-tuple with WI-karad rationale comment; sibling_impl removed from the inline canonical-stack replication in `test_dampened_centrality_helper_matches_inline_application`; NOTE comment about sibling_impl removed from `test_sketch.py`. Zero behavior change on the canonical pipeline (verified empirically: 3 repos × 100 top-100 positions × 2 metrics = 600 zero-deltas). Closes WI-karad (P4, analysis_quality + someday).

### Fixed (post-5.0.1)

- **WI-todon: `--backend rust-analyzer` no longer silently falls through to tree-sitter when rust-analyzer crashes; OOM-kill is named explicitly.** Surfaced 2026-05-10 on wasmtime (570K LOC, ~80 crates, Cranelift macros + component-model bindings) on a 22 GiB box with ~17 GiB free at run-start: `rust-analyzer scip` got SIGKILL'd by the OOM-killer (returncode `-9`), the graceful-degrade path swapped in `rust.py` tree-sitter output, and the run reported exit 0 with empty stderr. Two distinct downstream impacts already known: WI-mirus-pupas's "is procMacro.enable worth re-testing?" experiment got byte-identical SCIP symbol sets between flag toggles and was closed `wont_do`, but the result tells us nothing because we now know rust-analyzer may not have engaged on either run; the validity of every prior `--backend rust-analyzer` "completion" on wasmtime-class workspaces is similarly suspect. Three layers of diagnostics added: (1) **Exit code + stderr tail on invoke failure** — `RustAnalyzerInvocationFailed` now carries `returncode: Optional[int]` (None for the timeout path where we killed the process; verbatim subprocess returncode otherwise) and `graceful_degrade._format_invocation_failed` renders `exit=<N>` plus the last 512 bytes of stderr (decoded with `errors=replace`, trailing newlines stripped) in the user-facing log. Empty-stderr SIGKILL'd cases omit the stderr chunk entirely so we don't print `stderr-tail: ` with nothing after it. (2) **Explicit OOM-kill named when returncode in {-9, 137}** — Linux signal-based and shell-convention (128 + 9) representations of SIGKILL both trigger a "process killed by SIGKILL (likely OOM — rust-analyzer on wasmtime-class workspaces may need >16 GiB RAM)" hint. The exit-2 cargo-error path and the timeout path (returncode=None) do NOT carry the OOM hint. (3) **Artifact-level engagement check in analyzer.py** — after a successful invoke+translate, if zero `origin="scip"` edges were produced AND the repo contains at least one `.rs` file (recursive `rglob`, short-circuits on first match), emit a `UserWarning` naming the silent engagement failure. Catches the case where rust-analyzer exits 0 but emits no usable SCIP output. The `RustAnalyzerNoOutput` failure mode (exit 0, no `index.scip` file) also gains the stderr-tail rendering since cargo's diagnostic is the load-bearing signal there. Default log callable in analyzer.py is now `warnings.warn(message, UserWarning, stacklevel=3)` (was no-op) so graceful-degrade diagnostics surface to the user automatically — no CLI plumbing change required. 19 new tests across the three modules: 3 in `TestInvocationFailedCarriesReturncode` (returncode plumbed for nonzero exit, SIGKILL `-9` preserved, timeout sets None); 8 in `TestRichDiagnosticsForInvocationFailed` (exit code surfaces, stderr tail surfaces, `-9` and `137` both emit OOM hint, exit-2 does NOT emit OOM hint, timeout omits exit and OOM, long stderr truncated to tail, empty stderr omits stderr section); 2 in `TestRichDiagnosticsForNoOutput` (stderr tail surfaces, empty stderr omitted); 7 in `TestEngagementCheck` (warns on zero-SCIP-edges + .rs present, silent when SCIP edges present, silent when no .rs files, silent when result is None, silent when gate False, detects .rs in subdirs, real log callable wired into try_analyze). Closes WI-todon (P3, rust_analyzer + developer_experience + diagnostics + self_analysis); unblocks downstream isbefore-linked items WI-vovad / WI-burud / WI-nokoh now that the engagement signal is trustworthy.

- **WI-himas: framework detector now resolves layered `requirements/` and `requirements-*.txt` files plus `-r`/`-c` include chains.** Surfaced during WI-jifib's data-pass on `wagtail/bakerydemo`: the repo has `requirements/base.txt`, `requirements/dev.txt`, `requirements/production.txt` (the canonical Wagtail / many-Django-apps layout), no top-level `requirements.txt`, and the detector reported `frameworks: []` — downstream framework-gated linkers (e.g., `django-third-party-dispatch` from WI-jifib) never activated, dropping AC-2 rescue counts to zero on this whole shape of repo. Root cause was `_detect_python_frameworks` at `profile.py:1713` matching only the literal filename `requirements.txt` via `_collect_parsed_deps(repo_root, "requirements.txt", ...)`; layered files weren't in the manifest set and `-r base.txt` directives inside found files weren't followed. New helper `_collect_pip_requirements_deps(repo_root)` does seeded discovery (`requirements.txt` recursive + `requirements/*.txt` + `requirements-*.txt` up to max_depth=3) plus a worklist that follows every `-r <path>` / `--requirement <path>` / `-c <path>` / `--constraint <path>` reference (including the `=` form variants). Resolution is bounded by `repo_root` — `repo_root.resolve()` is computed up front and every visited file plus every `-r` candidate is `relative_to`-checked against it, so `-r ../host-file.txt`-style directory traversal is dropped at queue-add time (the queue-time check is the primary boundary; the per-iteration check is a defensive symlink-escape guard). Manifest-set discovery also mirrors `_find_manifest_files`'s exclusions for `.venv` / `.git` / `node_modules` / `vendor` / `__pycache__` (a venv's site-packages requirements would otherwise produce framework FPs by parsing installed-dep manifests). New parser helper `_parse_requirements_txt_dash_r_includes(content)` returns the include-list as a separate concern from the existing `_parse_requirements_txt_deps`; both run on the same content. Six new tests in `test_profile.py` pin the invariant: subdir-layout (`requirements/base.txt`), `-r` include chain from the top-level seed, `-r` include from a sibling within the same `requirements/` dir, hyphen-suffix file (`requirements-prod.txt`), `.venv`-shadowed manifest is skipped, and outside-repo `-r ../...` doesn't escape. Closes WI-himas (P3, framework_detection + analysis_quality + python).

- **INV-dogif (partial): React Router route extractor no longer emits false-positive routes for dynamic-path expressions, and now recognizes React Router v5 `render`-prop components.** Closes three of the five residual gaps catalogued in the 2026-05-17 INV-dogif audit; gaps (1)+(2) (JSX `<Route index>` and `{index: true, …}` object-config index routes) deferred pending the index-route output-shape design call. **Gap (5)** — at `js_ts.py::_extract_route_objects`, the object-config branch blindly extracted `_node_text(value_node)` for the `path` key and emitted Route nodes whose `route_path` was the variable's identifier text (`{path: homePath, …}` → `route_path="homePath"`), a junk entry in routes.txt. Fix gates the extraction on `value_node.type == "string"`, leaving `path` as `None` for any non-literal so the existing `if path is not None` guard at line 1164 skips emission. **Gap (4)** — symmetric FP on the JSX side: `<Route path={someVar}>` flowed through the existing `jsx_expression` → `identifier` branch in `_detect_jsx_route` (the same branch that correctly extracts component names from `element={Users}`) and assigned `attr_value="someVar"` → `route_path="someVar"`. Fix introduces an `attr_value_from_string` source-track flag and gates `route_path` extraction on it; component-shaped attrs (`element` / `component`) still accept both string and identifier sources unchanged. **Gap (3)** — React Router v5's component-shaped `render`-prop attr was unrecognized at line 1009 (only `element` and `component` matched); legacy v5 codebases with `<Route render={LegacyView}>` lost the `handler_ref` link to the component. Fix extends the recognized-attr tuple to `("element", "component", "render")`. Three new tests in `TestReactRouterJSXRouteDetection` (`test_jsx_dynamic_path_expression_does_not_emit_route`, `test_object_config_dynamic_path_does_not_emit_route`, `test_v5_render_prop_route`) pin each invariant. Status stays `violated` per the 2026-03-16 discipline (held until all five aspects resolve); the two remaining index-route gaps require a design call on routes.txt output shape (option (a) inherit parent path, (b) parent path + `<index>` marker, (c) emit with `is_index=True` meta + parent path — needs human input). INV-dogif (P4, react_router + analysis_quality + false_positive).

- **INV-zudob: Python class and top-level untyped-function `stable_id` now folds the file's stable identity in as containing scope.** Surfaced by post-INV-fusus self-analysis on hypergumbo's own codebase: 4282 Python class nodes, 0 missing `stable_id`, 3642 unique values — 18.94% (811 nodes) remained in collision groups. Top groups: 49× `TestAdaAnalysisUnavailable` / `TestAsmAnalysisUnavailable` / `TestCapnpAnalysisUnavailable` (per-language fallback test classes with structurally-identical bodies across language modules), 36× `TestAnalysisRun` (same name, same body, different test modules). INV-fusus closed the same-module case (folded body shape into the ClassDef hash); the residue was a different shape — structurally-identical classes in *different* modules still hashed identically because the top-level call site in `py.py::_extract_file_analysis` invoked `_compute_stable_id(node)` with no `containing_stable_id` argument and the function defaulted to the empty string. The same regression also affected top-level untyped functions (line 2479, untyped fallback) and top-level typed functions (line 2473, `make_typed_stable_id` call with no `containing_stable_id` positional argument), so two structurally-identical helpers in different modules collapsed to one stable_id too. Fix threads the file's identity (`make_file_stable_id("python", repo_relative_path)`) into all three top-level call sites. `file_relative_path` computation hoisted to the top of `_extract_file_analysis` (was previously gated on `_has_module_level_code(tree)` because it was only consumed by the module-symbol producer). Methods inside classes already cascaded correctly — they thread `symbol.stable_id` (the class's stable_id) as their containing argument, so file identity now propagates one hop down into per-method hashes automatically. `STABLE_ID_SCHEME` bumped to `hypergumbo-stableid-v4` per the convention established by INV-fusus's v3 bump. Same-module identical-body classes still collide by design (preserves the `Symbol.stable_id` docstring's "join on (stable_id, canonical_name) for absolute uniqueness" contract). New test class `TestCrossModuleIdenticalSymbolsDistinct` in `test_stable_id_class_collisions.py` pins four invariants: 49× class-name collision case across two files now produces 2 distinct stable_ids; 36× test-module name case ditto; top-level untyped function with identical body in two files produces 2 distinct stable_ids (proves the function fallback path is covered); class moved to a different file path changes its stable_id (proves the cross-file move is a structural change). Two pre-existing tests in `TestSurvivesRenamesAndMoves` (`test_class_rename_preserves_stable_id`, `test_method_order_irrelevant`) updated to write to a single shared filename rather than two distinct ones — the original two-file form was implicitly testing the old empty-containing behavior, while the within-module rename/reorder semantics they describe in their docstrings still hold (and are the only correct semantics under INV-zudob). Closes INV-zudob (P3, stable_id + identity + analysis_quality + dogfood_round4_followup); empirical claim: 811-node cross-module collision residue → 0 on hypergumbo's own self-analysis, awaits BROAD bakeoff confirmation across the prospector corpus.

- **Docs cheap-drip bundle (WI-dahar / WI-sosog / WI-safos / WI-bujij).** Four docs/comment-only items bundled per the burn-down plan's docs cheap-drip grouping; no behavior change. **WI-dahar**: `README.md:149` showed `hypergumbo verify-claims claims.yaml \` (positional) but the CLI at `cli.py:6538` declares `--claims` as `required=True` and rejects positional input; line corrected to `hypergumbo verify-claims --claims claims.yaml \`. README line 31 already used the correct named-flag form, plus every other docs site (`docs/USE-CASES.md`, `docs/adr/0016-io-boundary-analysis.md`, `docs/example-output.md`, `docs/hypergumbo-spec.md`, the dogfooding playbook) was already correct — only this one site was stale. **WI-sosog**: hypergumbo's per-language `loc` undercounts raw `wc -l` by 12-22% on self-analysis (Python -16.2%, markdown -21.7%, bash -12.7%, TS -11.9%) because `_count_loc` in `profile.py` counts non-empty lines (the SLOC convention shared by `cloc` and `tokei`'s "code" tally), but the docstring just said "Count non-empty lines in a file" without naming the convention, and `docs/hypergumbo-spec.md` defined LOC without flagging the gap. Docstring expanded to name the SLOC convention, document the expected ~10-20% gap vs `wc -l`, and clarify that comments are NOT stripped; spec entry expanded with the same. **WI-safos**: the audit-findings front-matter `- Status:` lines on 10 docs (0005-0014) narrated the PRELIM_RESOLVED-era — produced before the SCHEMA_VERSION 0.6.0 (Symbol.kind) and 0.7.0 (`Edge.evidence_type`) closures retired the `endpoint_shape` parking axis and advanced every parked row to RESOLVED. Per-row YAML `status:` fields had already migrated (0 PRELIM_RESOLVED entries across 217 rows in the 10 files); the README index `Status` column had already migrated (all rows say "All RESOLVED"); only the per-file front-matter `Status:` lines remained stale. All 10 front-matter lines rewritten to lead with `Status: All RESOLVED` and cite the SCHEMA_VERSION closure that retired the parking axis, preserving the historical CANONICAL-promotion / Wave-5-and-6-PR detail. The `audit_findings.find_readme_index_drift` regression guard reads the README index (not the front-matter), so the YAML / index / front-matter triple is now fully aligned. **WI-bujij**: `_AUTOLOAD_BY_CONVENTION_FRAMEWORKS` at `profile.py:369` is the framework-detection autoload-by-convention exemption set (Rails only today); its 14-line docstring lists inclusion criteria + counter-examples, but a YAML author working in `frameworks/*.yaml` has no breadcrumb pointing at it. Six-line forward-reference comment added near the top of `framework_patterns.py` directing YAML authors at the `profile.py` docstring with a brief rule-of-thumb (Rails-style: declared-but-never-imported = belongs in the set; Sinatra/Django/FastAPI = falls through the standard import-edge check and does NOT). Closes WI-dahar (P3, docs + dogfooding), WI-sosog (P3, docs + profile + self_analysis + dogfooding), WI-safos (P3, docs + consistency + concept_audit), WI-bujij (P4, analysis_quality + developer_experience + framework_detection — downscoped from refactor per the 2026-05-17 investigation).

- **CLI display polish bundle (WI-zunos / WI-jajas / WI-vumos / WI-dubum / WI-fugut).** Five small `cli.py` display fixes filed during 2026-05-17 self-analysis dogfooding rounds 10, 12, 15, and 16; bundled per the burn-down plan's "CLI display fixes" cheap-drip grouping. **WI-zunos**: `hypergumbo --help` usage-line metavar was a hardcoded string that omitted `config`, `dead-code-maybe`, `verify-claims`, `cache-status`, and `cache-clear` (registered later, never added to the literal); now built dynamically from the registered subparsers via the existing `_get_subparsers_by_group(sub)` ordering so newly added subcommands appear automatically. **WI-jajas**: `hypergumbo routes` within-file display order was unstable across full vs `compact` behavior maps because the inner loop iterated `file_routes` in node-insertion order; now sorted by `(start_line, method, route_path)` so two passes over the same logical routes produce byte-identical output. **WI-vumos**: `hypergumbo io-boundaries` text output rendered the `[tier-N tier_name]` annotation at the end of each `<- caller (path:line)` line, where it visually attached to the caller — but the tag describes the boundary destination's supply-chain tier, not the caller's; now moved to the primitive header line (`os.environ (26)  [tier-3 external_dep]`) so the referent is unambiguous. The `[unreliable]` flag stays per-caller because it depends on the source language, not the dst. **WI-dubum**: `hypergumbo explain` printed the queried symbol's source and per-caller/per-callee source dumps interleaved between the `Called by (N):` and `Calls (N):` summaries, pushing the `Calls` line to ~99% of total output on functions with many callers (1583-line output had `Calls (16):` at line 1563); now both summaries print back-to-back after metadata, and all source dumps are deferred to the bottom of each match block. **WI-fugut**: the `Test-Dense` section header ("highest test density — may indicate redundant tests") was actively misleading — the column counts inbound call edges from test files, not test functions, so a high `t/LOC` means a function is widely USED in test code (the opposite of redundant; acting on the hint would delete the wrong code). Header renamed to "Most-Called from Tests (functions reached most often by test code)", per-row metric relabeled `t/LOC` → `edges/LOC` and `(N tests, …)` → `(N test-call edges, …)`, plus a one-line footnote clarifying that the count is inbound calls. Five new regression tests pin each invariant: every registered subparser appears in `sub.metavar`; routes output is byte-identical across insertion orders; no caller line carries a `[tier-` substring while the primitive line does; `Calls (` precedes any `Source ` block; the test-coverage header doesn't contain "redundant tests". Closes WI-zunos (P3, analysis_quality + dogfood_round15), WI-jajas (P3, analysis_quality + dogfood_round16), WI-vumos (P3, analysis_quality + dogfood_round16 + io_boundaries), WI-dubum (P3, explain + ux + self_analysis + dogfooding), WI-fugut (P3, test_coverage + ux + mislabeling + self_analysis + dogfooding).

- **WI-mokim: consumer-side `.gz` handling for `hypergumbo run --gzip` outputs.** Before this fix, `hypergumbo run --gzip --out hg.json.gz` followed by `hypergumbo compact --input hg.json.gz` (or any other `--input`-taking subcommand) dumped gzip bytes into `json.loads` and exploded — the producer side from WI-kojob was uniform but every consumer rolled its own `json.loads(input_path.read_text())` with no `.gz` awareness. New shared module `packages/hypergumbo-core/src/hypergumbo_core/behavior_map_io.py` exposes `load_behavior_map(path)` (suffix-routes on lowercase `.gz` mirroring the producer's path-suffix convention; matches the bakeoff-deep helper's semantics) and `find_behavior_map(directory, basename="hg.json")` (plain wins ties over `.gz` per the same convention). All 11 `cli.py` consumer sites — `cmd_sketch` (cached-results read), `cmd_slice`, `cmd_search`, `cmd_routes`, `cmd_explain`, `cmd_symbols`, `cmd_compact`, `cmd_io_boundaries`, `cmd_verify_claims`, `cmd_test_coverage`, `cmd_dead_code_maybe` — route through `load_behavior_map`. Plus the `runtime_coherence.py` check-edge-type-runtime-coherence CLI entry point. Scripts wired up: `bakeoff-broad`, `bakeoff-broad-reflect`, `analyze-artifacts` (both `find_artifacts` and `load_artifact` now `.gz`-aware), and `bakeoff-deep`'s local `_find_hg_json` / `_open_hg_json` retain their string-returning wrapper shapes but delegate to the library helpers. `bakeoff-deep-reflect` and `bakeoff-map` don't read behavior maps directly (state.json / metadata.json only). Eleven new tests in `test_behavior_map_io.py` plus a static-audit test that grep-asserts no raw `json.loads(input_path.read_text())` pattern remains in `cli.py`; parametrized `test_cmd_compact_accepts_gzipped_input` in `test_cli_commands.py` proves end-to-end `.gz` round-trip through the highest-profile broken subcommand. Closes WI-mokim (P2, cli + dx + consistency + bug).

- **INV-tisid: test directories no longer route to supply_chain.tier=2 (internal_dep); tests are first-party.** Surfaced by 2026-05-16 self-analysis dogfooding (round 11): 460 paths and 24,180 Symbol nodes were classified as `supply_chain.tier_name="internal_dep"` on hypergumbo's own behavior map; 508 of 509 unique tier-2 paths (99.8%) were test files — tier 2 had become a synonym for `is_test=True`, drowning out the single actual internal-dep entry. Same failure mode as WI-pugas (Phoenix, 1328/1328 tier-2 nodes were Elixir tests) but on the workspace-package branch that WI-pugas's `TEST_FILE_PATTERNS_DIR_OVERRIDE` fix never reached: in `supply_chain.py::_classify_file_core`, step 5 (workspace) fires first when a file lives inside a detected workspace package, and its inner `TEST_DIR_PATTERNS` loop returned `Tier.INTERNAL_DEP` before the override at step 5b-pre ever ran. Both halves of the bug (the workspace inner loop and the non-workspace `TEST_DIR_PATTERNS` loop at step 5b) now return `Tier.FIRST_PARTY` with `is_test=True`, matching the prior-fix intent — tier 2 is now reserved for in-repo non-test code (examples, fuzz harnesses, vendored-but-in-tree deps, monorepo subpackages used as deps). Updated existing pinning tests (`test_tests_dir_is_first_party`, `test_test_dir_singular_is_first_party`, `test_jest_tests_dir_is_first_party`, `test_spec_dir_is_first_party`, `test_java_src_test_is_first_party`, `test_unit_tests_dir_is_first_party`, `test_nested_unit_tests_dir_is_first_party`, `test_elixir_test_helper_in_test_dir_is_first_party`, `test_gradle_subproject_test_dir_is_first_party`, `test_workspace_test_dir_is_first_party`, `test_workspace_test_dir_is_test_true`, `test_config_under_tests_is_test_not_config`) from `INTERNAL_DEP` to `FIRST_PARTY`. Added two new regression guards: `test_cargo_workspace_test_dir_is_first_party` (Cargo `crates/<name>/tests/`) and `test_workspace_test_dir_drowns_out_real_internal_dep` (a mixed fixture with both a workspace test and a real `examples/` entry confirms only the example lands in tier 2, the prior 99% drown-out pattern that motivated the fix). Module docstring updated to reflect the new algorithm; `FileClassification` docstring rewritten to call out is_test as mutually exclusive with the other tier-2 role flags by construction. The `TEST_FILE_PATTERNS_DIR_OVERRIDE` list (Elixir-specific) retained — still useful for `_test.exs` files outside `test/` dirs, where neither `TEST_DIR_PATTERNS` nor `TEST_FILE_PATTERNS` would match. Closes INV-tisid (P2, supply_chain + classification + self_analysis + dogfooding).

- **INV-kalif: sketch structure tree no longer renders the synthetic `<external>` placeholder as a root-level file.** Surfaced by 2026-05-16 self-analysis dogfooding: `hypergumbo . -t 8000 | grep '<external>'` showed `├── <external>` as line 48 of the sketch, slotted between real files (`.yamllint.yaml` and `conftest.py`). Root cause was `sketch.py::_collect_important_files` Step 4 ("Source files maximize root directory coverage") iterating `for sym in symbols` with no boundary-node filter; on hypergumbo self-analysis 1,561 external-boundary Symbols all share `path="<external>"` with `meta.external_boundary=True`. `Path("<external>").parts == ("<external>",)` slips past `is_root_level_file`, and `add_file` accepts the placeholder as a legitimate root-level file. Fix: filter `symbols` at the top of `_collect_important_files` using the existing `is_external_boundary` helper from `ir.py:859` — same predicate already used by `compact.py:1405` and `cli.py:1561` for the same class of aggregations. Filter happens *before* centrality aggregation so the placeholder never accrues a score that could displace a real root-level file. One new test in `TestCollectImportantFiles::test_excludes_external_boundary_symbols` pins the invariant: a mixed-symbol fixture with one real `src/app.py` Symbol and one external-boundary Symbol (`path="<external>"`, `meta={"external_boundary": True}`, centrality 0.9 vs 0.5) produces a result containing `src/app.py` and never `<external>`. Closes INV-kalif (P2, sketch + self_analysis + dogfooding).

- **INV-vunaf: framework detection now parses manifests structurally instead of substring-matching raw text.** Surfaced by 2026-05-16 self-analysis dogfooding (round 8): hypergumbo's own `profile.frameworks` reported `["pytorch", "transformers"]` despite the repo having neither dependency. Root cause was `_manifest_has_package` (in `profile.py`) running a word-boundary regex over the lowercased concatenation of every manifest file — picking up `"torch"` as a pytest marker key in `pyproject.toml:9` (`markers = ["torch: tests requiring PyTorch..."]`) and `"transformers"` as a hyphen-boundary substring inside `"sentence-transformers~=5.2.2"` at `packages/hypergumbo-core/pyproject.toml:46`. The same structural risk applied to 11 detectors that called `_manifest_has_package` plus ~10 detectors using even looser `pattern.lower() in content` (Haskell, Clojure, Lua, C++, Erlang, F#, C#, OCaml, Nim, Zig, D, Dart, Julia); JS and PHP already used the safe pattern of JSON-parsing the manifest and inspecting specific dep keys. This PR generalizes the JS/PHP pattern across every detector. New helper layer in `profile.py`: `_parse_pyproject_deps` (PEP 621 `[project]` + Poetry `[tool.poetry]` + PEP 735 `[dependency-groups]` via `tomllib` + `tomli` fallback), `_parse_requirements_txt_deps`, `_parse_setup_py_deps`, `_parse_pipfile_deps`, `_parse_cargo_toml_deps` (`dependencies` + `dev-dependencies` + `build-dependencies` + target tables + workspace), `_parse_go_mod_deps` (require-blocks and single-line, `//`-comment stripped), `_parse_pom_xml_deps` (XML-comment stripped + `<groupId>/<artifactId>/group:artifact`), `_parse_gradle_deps` (configuration declarations + plugin IDs + free-floating Maven-coord-shaped quoted strings to catch Apache-Kafka-style helper map declarations + string-aware C-comment stripper), `_parse_gemfile_deps`, `_parse_mix_exs_deps`, `_parse_sbt_deps` (`libraryDependencies` + `addSbtPlugin`), `_parse_package_swift_deps` (URL last-segment + `.git` strip + `name:` + `path:`), `_parse_description_deps` (R `Imports:/Depends:/LinkingTo:/Suggests:/Enhances:`), `_parse_project_toml_deps` (Julia), `_parse_pubspec_yaml_deps` (indent-aware), `_parse_msbuild_proj_deps` (`<PackageReference Include=>` + `<Reference Include=>`), `_parse_cabal_deps`, `_parse_haskell_yaml_deps` (indent-aware for stack.yaml + package.yaml), `_parse_clojure_deps_edn` + `_parse_clojure_project_clj`, `_parse_rockspec_deps`, `_parse_cmake_deps` (`find_package`), `_parse_qmake_deps` (`QT +=` modules), `_parse_vcpkg_deps`, `_parse_rebar_config_deps` + `_parse_erlangmk_deps`, `_parse_nimble_deps` (`requires`), `_parse_zig_zon_deps` + `_parse_zig_build_deps`, `_parse_dub_json_deps` + `_parse_dub_sdl_deps`, `_parse_dune_project_deps` + `_parse_opam_deps`. New shared `_strip_cstyle_comments` helper walks character-by-character respecting single/double-quoted strings so `//` inside a URL like `"https://github.com/vapor/vapor.git"` survives the strip (critical for Swift / SBT / Gradle URL extraction). Detector dispatch uses a new `_pattern_matches_deps(pattern, deps) -> bool` with five match modes: exact (`fastapi`), Maven coordinate-prefix (`org.springframework.boot` matches `org.springframework.boot:spring-boot-starter`), module-path suffix (`github.com/labstack/echo` matches `github.com/labstack/echo/v4`), dotted-namespace prefix (`androidx.compose` matches `androidx.compose.ui`, `microsoft.aspnetcore` matches `microsoft.aspnetcore.mvc`), and hyphen-prefix package family (`scotty` matches `scotty-0.12.1`, `cohttp` matches `cohttp-lwt-unix`) — the last mode is strict prefix, so `transformers` no longer matches `sentence-transformers` (the documented FP). The legacy `_manifest_has_package` remains as a fallback only for DSL-marker patterns (`"android {"`, `"qt +="`, `"qmake"`) that are intentionally build-script syntax fragments rather than package names; `_is_dsl_marker` / `_is_dsl_marker_or_special` route those through the raw-content path while every package-name pattern goes through the structured parser. Empirical impact on hypergumbo self-analysis: `profile.frameworks` `["starlette", "pytest", "pytorch", "transformers", "lit"]` → `["lit", "starlette"]` plus `dev_frameworks: ["pytest"]` — pytorch (FP) and transformers (FP) eliminated, pytest correctly demoted to dev-only via `refine_frameworks` (no `import pytest` in production code). New tests: `TestInvVunafPythonStructuredParsing` (9 tests covering pytest-marker / sentence-transformers / comments / regression / pipfile / setup.py / optional-deps); `TestInvVunafCrossLanguageFPs` (11 tests covering Rust / Go / pom.xml / Gradle / Gemfile / DESCRIPTION / Groovy / Julia / pubspec / SBT / Swift FPs); `TestInvVunafParserUnits` (40 tests covering each parser's branches: TOML malformed / Poetry / extras_require / triple-quoted strings / Pipfile dev-packages / Cargo target+workspace / go single-line require / pom XML-comment / Gradle plugin-id / Gradle helper-map coord / SBT plugin / R Imports / pubspec sections / MSBuild Reference / cabal build-depends / Haskell YAML deps / Clojure deps.edn + project.clj / rockspec / CMake find_package / qmake QT+= / vcpkg JSON / rebar.config / erlang.mk / nimble requires / Zig zon + build / dub.json + dub.sdl / OCaml dune+opam / pattern-match coord-prefix + module-path-suffix / DSL marker / Java already-detected dedup / Dart empty pubspec / Dart Flutter framework dedup / Haskell YAML blank+commented / vcpkg non-dict / dub non-dict). Closes INV-vunaf (P2, 16-of-18 detectors).

- **WI-dozon: taint_refine now pins parameter-receiver types from function-signature annotations.** Surfaced by 2026-05-16 self-analysis: `cmd_add_extras → replace` (14 flows) was a textbook short-name-collision FP — `_pretty_extras_name(name: str)` has `name.replace("-", " ")` where `name` is annotated `str`, but the edge emitted to `python:external:0-0:replace:unresolved` matched `pathlib.Path.replace` (a fs_write sink) under the `external` exemption in `_sink_module_compatible`. The `taint_refine.py` docstring previously marked function-parameter receivers as OUT OF SCOPE; this PR moves them in-scope under the WI-dozon rule. New helper `extract_python_param_annotations(func_def_node, source, module_imports, imports) -> dict[str, str]` walks `typed_parameter` / `typed_default_parameter` AST shapes and resolves annotations to module hints. Supported shapes: bare builtin names (`str`, `bytes`, `int`, `float`, `bool`, `list`, `tuple`, `dict`, `set`, `frozenset`, `bytearray`, `memoryview`, `complex`, `range`, `object` → `builtins.<name>`); bare imported class names (`Path` when `from pathlib import Path` is in scope → `pathlib.Path`); dotted annotations (`pathlib.Path` → `pathlib.Path`); generic types with a supported outer (`list[int]` → `builtins.list`). Conservatively-skipped shapes: `Optional[X]`, `Union[X, Y]`, `Callable[...]`, `Any`, `Iterable`, `Iterator`, `Sequence`, `Mapping`, `MutableMapping`, `Awaitable`, `AsyncIterable`, forward-reference strings. `extract_python_receiver_hints` gains an optional `param_annotations` kwarg used as a fallback when the DDG has no def visible at the call site (DDG hints still win when both fire — local rebinds are more specific than the signature annotation). `_collect_python_function_ddg` (cli.py) wires param annotations into the refinement pipeline AND runs the refinement when EITHER param annotations OR DDG edges exist (pre-fix it only ran on functions with non-empty DDG, missing short helpers like `_pretty_extras_name` where the parameter annotation IS the only signal). Four new tests in `test_taint_refine.py` pin the invariants: bare `str` annotation; `Path` via from-import; DDG-hint takes precedence over annotation; `Optional[X]` / forward-ref strings yield no hint. Empirical impact on hypergumbo self-analysis: unresolved-external `replace` edges 48 → 29 (40% reduction in FP edge surface); 778 total edges rewritten across the codebase by the refinement pass (extend / append / upper / replace / lower / etc. on type-annotated parameters). `verify-claims` claim-count remains 4/18 violated — the residual violations are unannotated-receiver FPs (closure captures, local rebinds without DDG visibility, untyped parameters) which are out of scope for the WI-dozon spec (`(1) Parameter-receiver type pinning`); the second WI-dozon-spec fix path (`(2) Module-name conflict map in _sink_module_compatible`) is the natural follow-up for the residuals but needs an io_primitives YAML extension or sink-set-derived map, separately filed if pursued. Closes WI-dozon (P2, taint_flow + self_analysis + dogfooding).

- **WI-bibuk: `subprocess` boundary now auto-derives into its own taint zone instead of collapsing into `host_fs`.** Surfaced by 2026-05-16 self-analysis dogfooding: every legitimate `subprocess.run([...pip..., ...])` / rustup / gitleaks / git invocation in `cmd_install_*` and `cmd_build_grammars` triggered the corresponding `*-no-host-fs` claim because `AUTO_SINK_ZONE_MAP[\"subprocess\"] = (\"host_fs\", \"untrusted\")` collapsed both zones together. Shelling out to a trusted external program is structurally a different trust surface than writing to arbitrary filesystem paths — the external program owns where its bytes land (pip → site-packages, rustup → ~/.rustup, gitleaks → its own binary) and hypergumbo's code does not select those paths arbitrarily. Fix: `AUTO_SINK_ZONE_MAP[\"subprocess\"] = (\"subprocess\", \"untrusted\")` introduces a dedicated zone. Claims that prohibit `host_fs` no longer fire on `subprocess.run` calls; claims that need to prohibit shelling out (e.g., `runtime-cli-no-subprocess`) use `prohibited_sink_zone: subprocess` explicitly. Built-in zone vocabulary grows 5 → 6. ADR-0017 §"Built-in zone vocabulary" updated; docs/hypergumbo-spec.md trust-zone layering updated; docs/hypergumbo-self-catalog/README.md note retitled (the previously-described `subprocess_zone_override.yaml` file is no longer needed — the override happens at the AUTO_SINK_ZONE_MAP level). New test `test_wi_bibuk_subprocess_maps_to_subprocess_zone_not_host_fs` pins the invariant: `AUTO_SINK_ZONE_MAP[\"subprocess\"]` → `(\"subprocess\", \"untrusted\")` AND `subprocess.run` lands in `zone=subprocess`. fs_write sinks (e.g., `pathlib.Path.write_text`) remain in `zone=host_fs`. Empirical impact on `hypergumbo verify-claims --claims docs/hypergumbo.claims.yaml`: **7/18 → 4/18 violations**; all four `*-no-host-fs` install/build-grammars claims (build-grammars, install-embeddings, install-rust-analyzer, install-gitleaks) now confirmed. Remaining 4 violations are short-name-collision FPs from `cmd_run -> replace` / `cmd_run -> write` / `cmd_run -> run` / `cmd_add_extras -> replace` — that's WI-dozon's territory (taint_refine parameter-receiver type pinning), filed as a separate fix. Closes WI-bibuk (P2, taint_flow + self_analysis + dogfooding).

- **INV-zudak: `Path.mkdir` callsites in `build_grammars.py` and `gitleaks.py` now route through safety_zones wrappers.** Surfaced by 2026-05-16 self-analysis: `hypergumbo verify-claims` reported `build-grammars-no-host-fs` violated with 6 unsanitized flows from `cmd_build_grammars` — 3 of those were direct `Path.mkdir` calls bypassing the wrapper discipline. The taint catalog had wrappers for `write_text` / `rmtree` / `copy2` / `chmod` / `unlink` but not for `mkdir`; the claim YAML asserted "scaffolding goes through `tmp_artifact_write`" but `mkdir` was unwrapped. Fix: three new wrappers in `safety_zones.py` — `cache_mkdir`, `tmp_artifact_mkdir`, `install_artifact_mkdir` — each invoking `_safety_zone_barrier()` (matches the convention of every other wrapper, so the zone-barrier sanitizer terminates BFS at the wrapper). Migrations: `build_grammars.py` (3 sites: `pkg_dir.mkdir(parents=True)`, `(pkg_dir / module_name).mkdir()`, `build_dir.mkdir(parents=True, exist_ok=True)`) routed to `tmp_artifact_mkdir`; `gitleaks.py` install path (`GITLEAKS_INSTALL_DIR.mkdir(...)`) → `install_artifact_mkdir`; `gitleaks.py` cache path (`cache_dir.mkdir(...)`) → `cache_mkdir`. Three Layer-3 sink catalogs updated: `tmp_artifact_sinks.yaml`, `install_artifact_sinks.yaml`, `user_cache_sinks.yaml`. Three new tests pin the invariants (`test_cache_mkdir_inv_zudak`, `test_tmp_artifact_mkdir_inv_zudak`, `test_install_artifact_mkdir_inv_zudak`). Empirical impact on `hypergumbo verify-claims --claims docs/hypergumbo.claims.yaml`: `build-grammars-no-host-fs` host_fs flow count 6 → 3 (the remaining 3 are all `subprocess.run` — separate issue tracked by WI-bibuk); 7/18 claims still violated overall because the residual FP-style and zone-collapse violations need their own fixes. Closes INV-zudak (P2, dogfooding_round on 2026-05-16). Cross-references: WI-bibuk (subprocess→host_fs zone collapse — separate fix), WI-dozon (taint_refine FP suppression — separate fix); the three items partition the same 7/18 verify-claims violations and were filed as a coordinated cluster.

- **INV-gifoh: `sketch` and `test-coverage` subcommand now report the same test-coverage percentage on identical input.** Surfaced by self-analysis dogfood round 12: on hypergumbo's own behavior map, `sketch` reported ~92% (3705/4016) while `test-coverage` reported 58.2% (2336/4016) — a 34-point discrepancy on the same denominator and same data. Two methodology differences caused the drift: (1) sketch's `_estimate_test_coverage` passed every edge into the transitive-BFS reach computation while `cmd_test_coverage` filtered to `edge_type == "calls"`; the unfiltered edge set included `references`, `inherits`, `imports`, `module_attr_ref`, etc., which artificially widened transitive reach and inflated coverage; (2) sketch identified tests by path heuristic only (`_is_test_path`) while `cmd_test_coverage` also recognized framework-tagged tests via the WI-dulav `_has_test_concept` check (Template-Haskell `prop_*`, hypothesis `@given`, etc.) — sketch missed concept-tagged tests as seeds and counted them as production targets, inflating both numerator and denominator. Fix: factored the test-identification into shared helpers `_is_test_symbol` / `_has_test_concept` in `sketch.py` and filtered the BFS edge set to `edge_type == "calls"`. End-to-end verified on hypergumbo self-analysis: `sketch` 92% → 59% (2461/4205), `test-coverage` 58.2% → 58.5% (2461/4205) — same numerator, same denominator, same methodology, identical numbers (the residual 0.1% display gap is `:.0f` rounding in the sketch line vs `:.1f` in the cli output). Three new tests in `TestEstimateTestCoverage` pin the invariant: non-`calls` edges don't widen reach; concept-tagged tests in `src/` are treated as tests; sketch and cli methodologies produce the same `(tested, total, pct)` tuple on a shared fixture. Closes INV-gifoh (P2, test_coverage + consistency + self_analysis + dogfooding). Cross-references: WI-fugut (test-coverage column header / interpretation hint — separate fix, not in this PR), INV-mofav (nested-function blindness — fixed in PR #3871, affected both numerators; this PR is the next layer up that fixes the *methodology* divergence between the two outputs).

- **INV-mofav: Python analyzer now emits nested function defs as Symbols and resolves bare-name calls to them.** Surfaced by self-analysis dogfood round 10: on `_collect_important_files` (sketch.py), pre-fix 0 of 4 inner functions were Symbols and 0 of ~12 bare-name calls produced call edges; the 16 outbound `calls` edges from this function pointed exclusively at stdlib attribute methods. Self-analysis-wide gap measured at ~121 missing nested-function Symbols + ~360 missing call edges across 294 production `.py` files. Three pieces in this PR: (1) **Symbol emission** — the `_extract_file_analysis` AST walk now emits every `FunctionDef` / `AsyncFunctionDef` at any nesting depth, dropping the prior `is_top_level or has_decorators` filter. A pre-walk parent map computes each function's enclosing-function chain; nested functions take a qualified name `outer.inner` (or `outermost.middle.inner` recursively), mirroring the existing `ClassName.method` convention so the output JSON keeps Symbol identity unambiguous when two different parents define same-named helpers. The immediate parent's name is stamped into `meta.nesting_parent`. (2) **Inner-scope dict** — `FileAnalysis` carries a new `nested_by_parent_id: dict[parent_symbol_id, dict[short_name, nested_Symbol]]` map plus a `func_symbol_by_node_id` AST-node-id-keyed lookup. Nested function short names are NOT registered in the flat `symbol_by_name` dict — that would silently collide between siblings (`outer_a.helper` vs `outer_b.helper`). (3) **Call resolution scope walk** — `_extract_edges` accepts both new maps. The FunctionDef-as-caller loop looks up `caller_symbol` via `func_symbol_by_node_id` first (covers nested), then resolves the inner scope and threads it through `process_code_block`, `_emit_function_ref`, `_resolve_call_target`, and `_process_call`. Each helper checks `inner_scope` BEFORE `local_symbols` for bare-name resolution, matching Python's LEGB rule. Cross-language note: this is a Python-only fix; the other language analyzers (Ruby, Go, JS/TS, etc.) emit nested functions through their own paths and weren't part of the gap. Five new tests in `TestInvMofavNestedFunctionEmissionAndCallResolution` pin the invariant: undecorated nested emitted with qualified name; bare-name call emits the calls edge; three-level deep nesting carries the full chain; sibling parents' same-named helpers stay distinct (both Symbol identity and edge dispatch); the 4-helper validation case mirroring `_collect_important_files`. Four prior tests in `TestNestedFunctionExtraction` updated for the qualified-name convention (these pinned the pre-INV-mofav behavior `"list_entities" in func_names` and similar — now `"get_entity_router.list_entities"`). One test in `TestSymbolIsExportedIntegration` updated for the same reason. End-to-end verified on sketch.py: 0 → 4 inner Symbols and 0 → 4 call edges for the `_collect_important_files` validation case. Closes INV-mofav (P2, dogfood_round10, analyzer + python + call_graph + completeness).

- **INV-kitot: Python analyzer no longer silently drops BOM-prefixed files.** Surfaced by 2026-05-17 self-analysis (dogfood round 15): a Python file with a UTF-8 BOM (`\xef\xbb\xbf`) produced zero Symbol nodes. `Path.read_text()` with the default UTF-8 codec does NOT strip BOM — the U+FEFF reached `ast.parse()` which raised `SyntaxError`, the `except` branch in `analyze_python_file` (`py.py:2152`) caught it, and the file was silently dropped. Python's own interpreter accepts BOM-prefixed source via its lexer; the analyzer was stricter than the language it analyzes. Files saved by Windows Notepad, AWS/Azure code editors with default UTF-8-with-BOM, or any chardet-aware tool were invisibly missing from analysis. Fix: switch both `Path.read_text()` call sites in `py.py` to `read_text(encoding="utf-8-sig")` — the `utf-8-sig` codec silently strips a leading BOM and otherwise behaves identically to UTF-8. Two call sites covered: `py.py:2146` (the main per-file analyzer in `analyze_python_file`) and `py.py:1081` (the cross-file imported-constant resolver `_resolve_imported_string_constant` used by FastAPI/Django route-prefix composition — a BOM-prefixed `constants.py` previously caused prefix resolution to fall back to empty). Two new tests pin the invariants: `test_run_analyzes_bom_prefixed_python_file` (BOM-prefixed `.py` → 1 function-kind Symbol, no entry in `failed_files`); `test_fastapi_apirouter_prefix_resolves_imported_constant_from_bom_file` (BOM-prefixed `constants.py` containing `V2_ROUTE_PREFIX = '/v2'` → cross-file resolver returns the literal, route prefix composes correctly). Companion item: INV-buhur (`limits.failed_files` never populated by Python analyzer) — this PR removes ONE trigger of failed_files emptiness; real user `SyntaxError`s still go unrecorded until INV-buhur is also addressed. Closes INV-kitot (P2, dogfood_round15).

- **Ruby analyzer: constructor-call `.new` redirect now walks the inheritance chain when the named class doesn't define `#initialize` directly.** Surfaced by the WI-vuton MessageBuilder diagnostic: chatwoot's `::Instagram::MessageText` inherits `#initialize` from `Instagram::BaseMessageText`; pre-fix `ruby.py`'s constant-receiver redirect at `_try_receiver_call` required `#initialize` to exist on the named class itself, so the redirect failed and the name-only fallback resolver attached the call to a colliding `DeviseOverrides::SessionsController#new` action — 151 such FP edges across chatwoot. Two-part fix: (1) new helper `_find_inherited_initialize` performs a bounded (max depth 10) breadth-first walk of `base_classes` metadata from Pass-1 class symbols, matching the convention used by Java's inherited-method lookup at `java.py:1436-1461` and the JAX-RS subresource locator resolver in `framework_patterns.py`. When found, it emits a `calls` edge with `evidence_type="ast_call_inherited"` and `meta.call_construct="constructor_inherited"` at confidence 0.85 (slightly below the direct-match 0.90, matching Java's confidence step-down for inherited calls). (2) Pre-strip the leading `::` from `receiver_class` candidates: `::Instagram::MessageText` extracted from the source AST didn't match the registered `Instagram::MessageText` key in `global_symbols`, so even the direct lookup failed for namespaced constants written with the root-namespace prefix idiom common in Rails background jobs. Both the direct and inherited lookups now use the stripped form. Empirical on chatwoot post-fix: 16 new `constructor_inherited` edges; 38 FP edges to `DeviseOverrides::SessionsController#new` eliminated (151 → 113); BFS-reachable function count for dead-code-maybe: 1183 → 1198 (+15). Three new tests in `TestRubyNewConstructorResolution`: inheritance-via-parent-chain (single hop), scope-resolution-namespaced-with-leading-`::` (the exact chatwoot shape with a decoy controller-`#new`), and three-level-chain (C < B < A) plus inheritance-walk-respects-depth-limit (12-class chain with no `#initialize` → no FP edge). Note: the WI-vuton `demoted_dispatch_inherited` count remains at 0 on chatwoot because the now-correctly-resolved chains terminate at Rails jobs that aren't themselves detected as entrypoints (`Webhooks::InstagramEventsJob` is absent from the 75 detected `background_task` entrypoints) — a separate Rails-job entrypoint-coverage gap, not in scope here.

- **WI-kilal: Rails routes are now distinct entrypoints, and `dispatch_inherited` handles Ruby's `Class#method` separator.** Two fixes in one PR, both surfaced by the 2026-05-21 validation drain on chatwoot (Rails+Vue, 13477 production functions). (1) Entrypoint generation in `entrypoints.py:_detect_from_concepts` now falls back to `sym.meta.http_method` / `sym.meta.route_path` when the route concept dict doesn't carry method/path. Before the fix, Rails' 623 distinct route Symbols on chatwoot all produced the same generic `"HTTP route"` label and the label-dedup in `detect_entrypoints` collapsed them to 1 HTTP_ROUTE entrypoint — because `rails.yaml` matches routes via `framework_role: "^route$"` (definition-based enrichment) and the ruby analyzer stamps `http_method` / `route_path` at the Symbol's `meta` level rather than into the concept dict (which is the usage-based `extract:` path that FastAPI / Express take). The concept dict's `method` / `path` still take precedence when present, so FastAPI/Express remain unchanged. (2) `cmd_dead_code_maybe`'s `_method_basename` helper now strips after either `.` or `#`, whichever appears last. Ruby qualifies instance methods as `Class#method`; before the fix the basename was the whole qualified name (no `.` to split on), so the dispatch_inherited heuristic's same-named-method comparison failed for every Ruby override. Empirical result on chatwoot post-(1)+(2): HTTP_ROUTE entrypoints 1→115; BFS-reachable function count 446→1183 (+165%); `dead_percent` 96.7%→91.2% (-5.5pp). Two new tests pin the fixes: `TestSemanticEntryDetection::test_wi_kilal_rails_route_label_from_symbol_meta` (623 chatwoot-shape route Symbols would produce 623 distinct labels; two min-cohort routes survive label-dedup); `test_wi_kilal_concept_dict_method_takes_precedence` (Express/FastAPI shape unaffected). One new test in `TestWiVutonDispatchInheritedDemotion::test_wi_kilal_ruby_hash_separator_basename_match` pins the basename fix. NOTE: `demoted_dispatch_inherited` remains at 0 on chatwoot in this PR — the heuristic logic is correct, but Rails ancestor methods (e.g., `Klass#initialize` called via `Klass.new`, `before_action :foo` callback targets) aren't themselves reachable yet because the Ruby analyzer doesn't model those call patterns. That's a separate Ruby call-graph completeness gap that falls under WI-vuton's broader scope but not WI-kilal's narrow Rails-entrypoint-detection scope. Closes WI-kilal (regression sub-item of WI-vuton filed during the validation drain).

- **WI-misud: `materialize_route_symbols` now produces per-file route Symbols for kind=file source concepts (WI-tisam pipeline completion).** Before this fix, the (method, path) dedupe key in `materialize_route_symbols` collapsed every catch-all `(method=ALL, path=/)` concept enrichment to a single route Symbol — but the WI-tisam YAML patterns in `node-http.yaml` / `graphql.yaml` (matching `startStandaloneServer`, `runHttpQuery`, `http.createServer`, etc.) attach the route concept to kind=file Symbols via UC-enrichment, where the symbol_ref of a kind=call UsageContext resolves to the containing file's `file:file` Symbol. Different files calling the same framework entry point are DISTINCT HTTP deployments, not duplicate registrations of the same handler — but the dedupe treated them as duplicates and dropped 10 of 11 file-level concepts on apollo-server. Found during the 2026-05-21 validation drain: INV-rolul (UC emission) moved cleanly on apollo-server (13 kind=call UCs emitted for the WI-tisam target name set), yet WI-tisam itself stayed at no_move because the downstream route-materialization step collapsed all 11 file-level concepts to a single route Symbol — and that one survivor happened to land in `packages/integration-testsuite/src/apolloServerTests.ts`, which `hypergumbo routes`'s default `--exclude-tests` filter (WI-godos) treats as production source via `is_test_file` but the test-file regex doesn't match `integration-testsuite/`, so it surfaced as 1 PROD route. Fix: when the source symbol's `kind == "file"`, the dedupe key includes the source symbol ID (`f"{method}:{path}:{sym.id}"`); for other kinds the existing `f"{method}:{path}"` key still applies, preserving Django-style cross-file dedupe of identical (method, path) on view methods (urls.py registers `GET /users/`, views.py's UsersView.get also gets concept=route — same key collapses them). Empirical result on apollo-server post-fix: `framework_role=route` symbols 1 → 11; `hypergumbo routes` output 0 → 6 displayed routes (5 in production source under `packages/server/src/` and `packages/integration-testsuite/src/`, 1 in `smoke-test/nodenext/` which `is_test_file` doesn't currently classify as test). New test class `TestMaterializeRouteSymbolsWiMisud` (3 tests) in `test_framework_patterns.py` pins the invariant: two file Symbols with the same catch-all concept → 2 routes materialized; the per-file relaxation does NOT apply to kind=method (Django-style dedupe stays in force); two route concepts on the SAME file Symbol still dedupe (per-file dedupe key includes source id). 100% coverage on framework_patterns.py preserved. Closes WI-misud (regression sub-item of WI-tisam filed during the validation drain).

- **WI-tuhok: Java analyzer resolves wildcard imports (`import java.util.*`) to the source package.** Before this fix, a bare class usage like `Arrays.asList(...)` after `import java.util.*;` emitted an unresolved edge with `dst="java:external:0-0:Arrays.asList:unresolved"` — the wildcard package was discarded entirely, so the receiver `Arrays` was never attributed to its source `java.util`. The explicit-import case (`import java.util.Arrays;` + `Arrays.asList(...)`) already worked; the wildcard branch was missing. Filed during the WI-mafik audit extension (2026-05-21) as the one new gap surfaced when the `java_wildcard` polyglot fixture was added. Fix: new helper `_extract_wildcard_imports(tree, source)` in `java.py` walks `import_declaration` nodes, finds those with an `asterisk` child, and returns each wildcard's package path in insertion order (e.g., `import java.util.*; import java.io.*;` → `["java.util", "java.io"]`). Static wildcards (`import static X.*;`) are intentionally excluded — they introduce methods, not classes, and belong to the static-import resolver (WI-hudud). The new field `_ParsedFile.wildcard_imports: list[str]` plumbs through to `_extract_edges`'s new `wildcard_imports=` kwarg. In the unresolved-edge construction inside `_extract_edges` (the branch that runs when a method call receiver isn't found in any project symbol), an `elif` clause fires when `receiver_name` is capitalised (class-shaped) and not in explicit `imports` but `wildcard_imports` is non-empty: it attributes the receiver to the first wildcard's package, producing `module_path="{wildcard_package}.{receiver_name}"` and a structured `ExternalRef`. The capitalisation gate keeps the FP rate low — lowercase bare names like locals/parameters/static-import targets are NOT attributed to wildcards. Explicit imports take precedence: `import java.util.Arrays; import java.io.*;` correctly resolves `Arrays` to `java.util.Arrays` (not `java.io.Arrays`). New `TestJavaWildcardImports` (6 tests) in `test_java.py` pin the invariants: extractor returns package list; static wildcards excluded; bare class use attributed to wildcard package end-to-end; explicit import wins over wildcard; lowercase bare names not attributed. The `java_wildcard` fixture in `test_polyglot_call_site_coverage.py` loses its `xfail_reason` and asserts pass. Closes WI-tuhok (the analyzer-fix follow-up filed during WI-mafik audit closure); restores the polyglot fixture's all-green status (10 of 10 idioms green, 0 xfails).
