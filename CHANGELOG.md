...
- **A bare Go call that resolves nowhere is no longer dropped from the call graph.** `unknownThing()`, or a same-package helper whose name the resolver withholds as ambiguous, emitted no `calls` edge at all; it now emits the unresolved `external` placeholder every other analyzer emits (INV-guzuj). On alertmanager and beads that restores 124 and 379 call sites. A call on a name the enclosing function binds (a closure, a function-typed parameter, a range variable) stays unemitted, and the dot-import arm, which had attributed such a local call to the first dot-imported package, now refuses it too (one edge on alertmanager).
- **`bakeoff-deep run` / `bakeoff-broad run` no longer mistake a crashed run for a finished one.** A run that crashed after creating its output directory left it empty, and every later `run` in that session exited 0 with "Skipping: hypergumbo code unchanged ... Previous results: <dir>" over a directory holding no files; the `--all` batch path skipped the cohort forever as "already has output" (WI-zilar). All four output-presence checks now require at least one file, and an empty iteration is kept (it records the crash) while the cohort runs again.
- **A sanitizer in the same function as the source now earns its credit on real Python code.** The rule shipped with 8.1.0 and was tested only on hand-built data-flow edges; on ordinary `decrypt -> encrypt -> write` code it never fired (WI-simiv), for two reasons in the data-flow graph. A `with open(...) as f:` clause was never read, so `f` was undefined and the function was treated as not fully seen; and in `open(...).write(e)` the inner call overwrote the outer call's record, so the use of `e` was lost. Both are fixed; on five Python repositories the functions the analysis gives up on fall by 24% to 91% (httpx 238 to 84, mitmproxy 1,968 to 1,487), with no verdict, finding or credit changing on any claim. Separately, a verdict that the coverage check downgraded to `inconclusive` lost `sanitized_flows` and five other fields, so its sentence could say "1 flow(s) ... pass through a sanitizer" beside `"sanitized_flows": 0`; a downgrade now keeps every field.
- **The "receivers of unknown scope" caveat no longer counts Go package functions as method calls.** Go stamped every selector call `call_construct: method`, so `fmt.Println` and `http.Get` sat in the caveat's "N of M method call site(s)" denominator next to real receiver calls, and the untyped share read low: alertmanager 1,141 of 6,183 (18.5%), gocryptfs 284 of 3,345 (8.5%). A package-qualified call is now stamped `function`, and the same runs report 1,141 of 3,557 (32.1%) and 284 of 1,779 (16.0%). No verdict, caveat kind or finding changed on either repository (INV-tanom, WI-fuvaj). Java qualified statics are still counted; they wait on `java.yaml` declaring its statics function-kind.
- **An I/O primitive overlay now works for a language hypergumbo ships no catalogue for.** For ruby, php, lua and the other analysed languages without one, `--io-primitives` (or a claims file's `extra_catalogs`) printed "Loaded 1 project-local I/O primitive overlay(s)" and then ignored it, and a ruby overlay in a repo that also held python aborted the whole run as a supposed misspelling (WI-guhuv). The overlay's rows are now that language's catalogue, in both the boundary and the taint arm, and the run says the language is overlay-only. They cannot vouch for anything they do not name: a call into any other module still withholds a clean verdict. An overlay for a language no analyzer produces is still refused.
...
- **`SCHEMA_VERSION` 0.20.1 → 0.20.13, and every step but one is additive.**
  - 0.20.2–0.20.5: `Edge.meta` gains `call_arg_shape`, `callee_name` (ADR-0036 Ruling 1), the bash per-call-site keys, and `io_target_kind`.
...
| ------- | ---------- | ------------------------------------------------------------ |
| Unreleased | —          | A server launch or composed query now qualifies a clean `untrusted_input` taint verdict, not only the boundary verdict; I/O overlays apply to languages with no shipped catalogue |
