...
Two more concept axes migrate end-to-end from Draft to Phase 4a: `Symbol.kind` (192 values, ADR-0027) and `Edge.evidence_type` (218 values, ADR-0028). Across Waves 3-6, ~28 `Symbol.kind` framework-role values fold to `function`/`method` + `meta["framework_role"]`; ~75 `Edge.evidence_type` framework-dispatch values fold to canonical inference + `meta["framework_dispatch"]` / `meta["detection_pattern"]`; ~28 evidence_type call-construct peers collapse to apex `ast_call`; 18 `*_unresolved` evidence types fold to canonical + new `Edge.is_resolved=False`. Phase 4a `x-deprecated` annotations land for both axes; closed-enum return is gated on per-cluster bakeoff validation. With Wave 6 PR 6, ADR-0027 Phase 3 producer migration is empirically complete on `Symbol.kind` — every registry value has a verdict and a status of RESOLVED or PRELIM_RESOLVED.

Internal: `docs/audits/` becomes a 12-entry per-cluster verdict series; the Fundamental Concept Audit playbook gains an indirection-aware producer-trace step after Wave 6 PR 4 caught three reclassifications a literal-grep blind spot had missed; a regression-guard property test now blocks DEPRECATE-NO-FOLD verdict drift at every commit. Three UAT v4.0.0 framework-dispatch precision regressions are fixed (WI-halat / WI-sokaz / WI-zozuz); `bakeoff-deep` no longer counts synthetic dispatch edges in reverse-slice seed selection.

### Added

#### Concept-axis declarations

- **Canonical `Symbol.kind` registry** (ADR-0027 Phase 1, `symbol_kinds.py`): 192 entries classified across `language_construct` (Cluster A canonicals, ~50 values), `endpoint_shape` (Clusters D/E + `component_ref`, ~40 deprecation candidates folding to canonical + `meta["framework_role"]` or producer-side drop), and `pending_classification` (Clusters B/C/G/H, ~100 values awaiting per-cluster audit-findings). Accessors mirror `edge_types.py`. ADR-0027 status flips Draft → Accepted.
- **Canonical `Edge.evidence_type` registry** (ADR-0028 Phase 1, `evidence_types.py`): 218 entries classified across `inference_pathway` (Cluster A canonicals, 107 values), `endpoint_shape` (Clusters B/C/D, 111 deprecation candidates folding to canonical + `Edge.is_resolved` / `meta["framework_dispatch"]` / `meta["call_construct"]`), and `pending_classification`. ADR-0028 status flips Draft → Accepted.
- **`Edge.is_resolved: bool = True` sibling field** (ADR-0028 §"Sibling-field design call-out"): captures the resolution-status property previously smuggled into `*_unresolved` evidence types. Default `True`; producers set `False` when folding. JSON serialization emits at the top-level `Edge` shape; `from_dict` defaults missing key to `True` for backward compatibility.
- **Pre-commit + CI drift linters for `Symbol.kind` and `Edge.evidence_type`** (`scripts/check-symbol-kind-drift`, `scripts/check-evidence-type-drift`): mirror the existing `check-edge-type-drift` shape. AST-walk `packages/`, `scripts/`, `.agent/` for module-level `*KIND*` / `*EVIDENCE_TYPE*` set assignments and verify every value is in the canonical registry. Companion property tests gate at CI; pre-commit hook gates locally from the same merge moment.
- **L3 producer-coherence linter** (`producer_coherence.py`, `scripts/check-producer-axis-coherence`): walks `Edge.create(...)` / `Edge(...)` / `Symbol.create(...)` / `Symbol(...)` call sites and verifies literal-string keyword arguments to `evidence_type` / `kind` / `edge_type` are in the corresponding canonical registry. F-string emits surface as advisory Phase-3 fold candidates. Closes the producer-introduction gap left open by the consumer-side drift linters across all three sibling axes. Extension A (WI-nubuv) traces simple assignment-form references (`name = "literal"`, ternary, if/else chain) within a function before the strict gate; surfaced 18 latent leaks at landing, all registered AXIS_PENDING for follow-on classification.
- **`docs/concept-axes.md` extends to all three axes**: `scripts/generate-concept-axes` now reads `EDGE_TYPES`, `SYMBOL_KINDS`, AND `EVIDENCE_TYPES`. Re-runnable; CI freshness gate via `--check`.
- **`docs/schema.json` carries `x-axis-of-values` annotations on all three fields**. `Symbol.kind` and `Edge.evidence_type` ship as **open** enums (current production includes dynamic f-string emits); closed-enum return is gated on Phase 4b. `Edge.edge_type` enum remains closed — pre-implementation audit confirmed zero f-string emit sites.
...
The `docs/audits/` series gains 12 new entries covering the two new axes. Each records per-row CANONICAL / FOLD / DEPRECATE-NO-FOLD verdicts plus per-row UNRESOLVED / PRELIM_RESOLVED / RESOLVED status:

- **`Symbol.kind`** — 0003 Cluster A canonicals (50 values RESOLVED), 0005 Cluster B file-shape (17 values), 0006 Cluster G build/config (24 values), 0007 Cluster H long-tail (60 values), 0009 Cluster C apex/peer (4 values), 0010 Cluster E edge-label kinds (12 values), 0011 Cluster F component_ref (5 values), 0013 Cluster D framework-role (29 values).
- **`Edge.evidence_type`** — 0004 Cluster A canonical inference (110 values RESOLVED), 0008 Cluster B `*_unresolved` (18 values), 0012 Cluster D apex/peer call-construct (28 values), 0014 Cluster C framework-dispatch (65 values).
...
- **DEPRECATE-NO-FOLD-zero-producer regression guard** (WI-viluk): `audit_findings.find_zero_producer_violations()` enumerates DEPRECATE-NO-FOLD verdicts across the three registered axes and asserts no producer emits the value. Companion enumerators `producer_coherence.find_emitted_{symbol_kinds,evidence_types,edge_types}()` reuse the WI-nubuv ext A function-local + module-level Name resolver. Catches literal-kwarg + assignment-form-to-Name leaks at every commit; helper-call / f-string / dict-subscript shapes remain manual until WI-nubuv ext B/C land.
- **README index sync regression guard**: `audit_findings.find_readme_index_drift()` parses `docs/audits/README.md` and asserts the Status column agrees with each doc's verdict YAML row counts. Supports both explicit-count cells (`Mixed (6 RESOLVED, 11 PRELIM_RESOLVED)`) and bare-marker cells (`All RESOLVED`). Closes the recurrence pattern Wave 6 PR 5 demonstrated.

#### Methodology hardening

- **Fundamental Concept Audit playbook gains §"Step 4.5 — Indirection-aware producer trace"**: requires checking five producer-emit shapes (literal kwarg, helper-call positional/kwarg, assignment-form-to-Name, f-string interpolation, dict-subscript-target) before claiming "no producer". Trigger: Wave 6 PR 4 reclassified `theorem` / `inductive` / `message` from DEPRECATE-NO-FOLD to CANONICAL after a re-trace surfaced producers via `add_symbol(...)` / `_make_proto_symbol(...)` indirection. A self-test bullet makes the trace mandatory at audit-write time.

#### Hooks & developer experience

- **Session-start hook prompts about prior-session `agent_notes.json`**: a non-empty notes file produces a one-line prompt naming both the notes-file age and the last-session age, asking the agent to ask the user whether to load the handoff via `./scripts/agent-notes --show`. Closes the WI-borur read-side gap (the session-end write rule had no symmetric fresh-session read trigger). Notes content is **not** dumped unprompted. Agent-notes and audit-cadence prompts are marked as separate items when both fire.
...
- **Wave 3 (WI-nunal) — `Edge.evidence_type` Cluster B `*_unresolved` fold**: 18 emit sites across 11 producer files migrate to `evidence_type=<canonical>` + `Edge.is_resolved=False`. First production write of `is_resolved`. Two new Cluster A canonicals (`grpc_stub_resolution`, `luajit_ffi_lookup`) absorb prior emit sites without a canonical inference label. Audit-findings 0008.
- **Wave 4 — Symbol.kind Clusters C/E/F + evidence_type Cluster D**:
    - **WI-rusit / Cluster C apex/peer** — `fn` / `var` / `proc` / `structure` registry classification updates (no producer change; apex emit was already canonical). Audit-findings 0009.
    - **WI-zarov / Cluster E edge-label kinds** — `call_site` added as new Cluster A canonical. Sub-case (a) reclassifies 4 producers (subprocess/db_query/abi/twig function_call) to `kind="call_site"` + `meta["call_kind"]`. Sub-case (b) drops Symbols whose relationship is already on a companion Edge: 3 clean drops (css/jsonnet imports, bitbake inherit), 6 shape-2 edge-endpoint redesigns (json_config/puppet/scss/twig), and 4 shape-3 companion-Edge introductions (r_lang library imports, astro frontmatter imports, make include, blade @extends). Audit-findings 0010.
    - **WI-mihiz / Cluster F component_ref** — vue / svelte / astro drop per-reference Symbols; `imports` Edges re-route src to `make_file_id()` and dst falls back to a 5-part dangling component id when unresolved. DEPRECATE-NO-FOLD verdict (the original fold target `reference` was already deprecated in 0010). Audit-findings 0011.
    - **WI-nibis / Cluster D evidence_type call-construct** — 28 peer values (`function_call`, `method_call`, `function_application`, `pipe_call`, `typed_receiver_call`, `ambiguous_method_call`, …) collapse to apex `ast_call` + `meta["call_construct"]` (the lone non-`ast_call` apex is `cross_file_message_send` → `message_send` + `meta["call_construct"]="cross_file"`). 89 emit sites across 26 producer files. Audit-findings 0012.
- **Wave 5 (WI-habut / WI-kagik) — coordinated framework-role + framework-dispatch fold across six PRs**: ~28 `Symbol.kind` framework-role values fold to `function`/`method`/`interface`/`class`/`reference` + `meta["framework_role"]`; ~75 `Edge.evidence_type` framework-dispatch values fold to canonical inference (`ast_call_direct` / `ast_decorator` / `ast_import` / `naming_convention`) + `meta["framework_dispatch"]` or `meta["detection_pattern"]`. Coverage spans websocket / tauri / grpc / openapi / graphql / http / crypto / ipc / objc / event-dispatch / Go-route_mount / Ruby / Django / NestJS / Vue and ~25 single-row dispatch modules. The highest-blast-radius slice is `Symbol.kind="route"`: 17 source files, 14 production consumers, ~330 sites total. `Pattern.matches()` extends to fall back to `meta["framework_role"]` when `symbol_kind` doesn't match — the shim is removed in WI-fojun (below).
- **WI-fojun — `Pattern.framework_role` field replaces the `Symbol.kind` backward-compat shim**: dedicated `framework_role: str | None = None` field on `Pattern` with its own compiled-regex matcher. Four YAML rules referencing folded role names (`framework_role: "^route$"` in `laravel.yaml`, `phoenix.yaml`, `rails.yaml`, `sinatra.yaml`) migrate; the shim is removed so `symbol_kind` and `framework_role` are now disjoint. Both fields can coexist on a single Pattern (AND condition). New result key `matched_framework_role`.
- **WI-nitil — gRPC + message-queue assignment-form correction**: literal-grep at audit time missed assignment-form producers (`kind = "<value>" if … else "<value>"` followed by `Symbol(kind=kind, …)`). The follow-on sweep folded `grpc_stub` / `grpc_server` / `grpc_service` / `grpc_servicer` / `grpc_client` / `mq_publisher` / `mq_subscriber` to canonical apex + `meta["framework_role"]`. Audit-findings 0013 §"Diagnostic findings" #3 corrected; diagnostic-test patterns updated to assignment-aware regex. Three previously-unregistered values added to `symbol_kinds.py` on AXIS_ENDPOINT_SHAPE.
- **`linkers/phoenix_ipc.py` f-string fold**: same family of miss as WI-nitil but f-string-form (`kind=f"ipc_{pattern.type}"`). Folds to canonical `kind="function"` + `meta["framework_role"]=f"ipc_{pattern.type}"`, mirroring `linkers/ipc.py`.
- **Wave 6 (WI-runod / WI-gajob / WI-dubab / WI-nupus) — Symbol.kind Clusters B/G/H sweep across six PRs**:
    - **PRs 1-2 — 74 canonical promotions**: 6 Cluster B file-shape (`file` / `library` / `package` / `executable` / `program` / `project`), 15 Cluster G build/config DSL constructs, 53 Cluster H domain-DSL constructs. No producer code change; registry axis classification only.
    - **PR 3 (WI-gajob) — 11 Cluster B FOLDs**: framework / ecosystem-qualified file/package/export/module values fold to canonical + meta keys (e.g. `module_file` → `file` + `module_system`, `npm_package` → `package` + `package_ecosystem`, `composer_package` → `package` + `package_ecosystem`, `wasm_module` → `module` + `compilation_target`, `tsconfig` → DEPRECATE-NO-FOLD via the v4.0.0 `is_config_file` boolean). Consumer dual-shape predicates added at `route_handler.is_component` and `cli._is_noise`.
    - **PR 4 (WI-runod) — Cluster G/H DEPRECATE-NO-FOLDs + reclassifications**: G's `config` (Prisma generic block — producer rewrites to `kind="block"`), `dev-dependency`, `build-dependency`, `work_item` (dead vocabulary). H's `heading`, `model` (DEPRECATE-NO-FOLD; advanced PRELIM_RESOLVED). Plus 3 CANONICAL reclassifications (`theorem`, `inductive`, `message` had real producers via indirection at `lean.py` / `tlaplus.py` / `proto.py` — surfaced the methodology gap that Step 4.5 now closes).
    - **PR 5 (WI-dubab) — Cluster G FOLDs**: `test_case` → `test` + `test_dialect`, `editable` / `url_requirement` → `requirement` + `install_mode`/`install_source`, `devDependency` → `dependency` + `dependency_scope`, `python_task` → `task` + `task_implementation`. Consumer surfaces unchanged (canonical fold targets already in `EXCLUDED_KINDS` / `_DEPENDENCY_KINDS`).
    - **PR 6 (WI-runod) — Cluster H boundary-pseudo-symbols**: `external_symbol` reclassifies DEPRECATE-NO-FOLD → CANONICAL on `language_construct` (pipeline-DSL top-level construct); `unresolved` advances UNRESOLVED → PRELIM_RESOLVED (registry seed error, no producer). Hypothetical `Symbol.is_resolved` follow-on ADR **withdrawn** — re-trace showed no leak to absorb (1 producer, 0 consumer reads, fails ADR-0024 fold-residue rule 3 threshold). With this PR, ADR-0027 Phase 3 producer migration is empirically complete on `Symbol.kind`.
- **Phase-2 consumer migration (WI-jukav)**: dual-shape predicates introduced at `linkers/registry._is_synthetic_node` (slice 1) and `selection.filters.is_excluded_kind` (slice 2). Both recognise pre- and post-Wave-5 producer shapes so consumer filters survive the producer fold without inflating selection or compact output. Slice-2 verdict for `containment.{CONTAINABLE_KINDS, CONTAINER_KINDS}`: no dual-shape needed — the post-fold canonicals are already in those sets, and Wave-5 framework-linker producers don't reach this filter.
- **Phase 4a `x-deprecated` annotations** (WI-tikuh / WI-jubak): `scripts/generate-schema` now emits `x-deprecated` on `#/$defs/Symbol/properties/kind` (50 entries) and `#/$defs/Edge/properties/meta/properties/evidence_type` (111 entries), mirroring the pre-existing `Edge.type` Phase 4a shape. Values stay valid in the open schema for the deprecation window. Phase 4b ships piecewise as each Phase 3 cluster's `awaits_bakeoff_validation` tag clears.
- **Verdict-correctness re-audit (audit-findings 0010, 2026-05-07)**: the Step 4.5 indirection-aware producer trace ran against all 19 DEPRECATE-NO-FOLD-flagged values across audit-findings 0005 / 0006 / 0007 / 0010 / 0012. Two leaks surfaced and reclassified DEPRECATE-NO-FOLD → CANONICAL: `reference` (`swift_objc.py:167` from Wave 5's selector_ref fold) and `import` (`wasm_bindgen.py:266` from Wave 6 PR 3's wasm_import fold). Other 17 values verified clean (false-positive analysis: ID-construction tail tokens, linker-internal dataclass fields, tracker `Item.kind`, prose strings). The WI-viluk regression-guard test promotes from smoke check to strict assertion.

#### Schema versions

The `Edge.evidence_type` axis ships across `0.4.1` → `0.5.0` (open enum re-opened at ADR-0028 Phase 1 land). Subsequent patch bumps absorb per-Wave producer migrations: `0.5.1` (WI-nubuv ext A, 18 latent leaks registered), `0.5.2` (Wave 6 PR 1, 6 promotions), `0.5.3` (Wave 6 PR 2, 68 promotions), `0.5.4` (Wave 6 PR 3, 11 reclassifications), `0.5.5` (Wave 6 PR 4, 9 reclassifications), `0.5.6` (Wave 6 PR 5, 5 reclassifications), `0.5.7` (Wave 6 PR 6, 2 reclassifications), `0.5.8` (verdict-correctness re-audit, 2 reclassifications). All changes are widening — no validation that previously passed will now fail.

#### Inheritance linker

- **Inheritance linker annotates simple-name fallback edges per INV-zuhub**: when `_resolve_target_symbol` falls back to deterministic-by-sorted-ID disambiguation (multiple cross-file candidates, no same-file precision match), the resulting `extends` / `implements` edge now carries `confidence=0.5` and `meta["disambiguation_fallback"]=True`. Single-candidate and same-file resolutions remain at `confidence=0.95` with no flag. Provides downstream consumers (slice ranking, dead-code analysis, supply-chain tier classification) a structured signal to filter the fallback population.

#### Bakeoff infrastructure

- **`bakeoff-deep` excludes `dispatches_to` from `pick_reverse_slice_seeds` out-degree counting** (WI-ribav): synthetic dispatch edges from interface stubs were inflating reverse-slice seed scores above real domain functions (the prometheus `genericSeriesSet.Err` regression cited in the function's own comment had recurred). 16 of 18 `dispatches_to` producers emit synthetic 'menu' relationships; the 2 real-dispatch producers (route_handler, grpc) score via the route + API-handler boosts already.

### Fixed

- **Framework-dispatch linkers traverse transitive base-class chains** (WI-halat / UAT BUG-01): `airflow_framework_dispatch` and `django_orm_dispatch` now walk `extends`/`implements` edges to discover ancestors whose `meta.base_classes` names a framework base. Fixes the dominant real-world pattern where projects extend an intermediate (`AlloyDBWriteBaseOperator(BaseOperator)`, `Order(LoggedModel)` / `LoggedModel(models.Model)`, `HierarkeyForm(forms.Form)`) rather than the framework class directly. UAT v4.0.0 round 04 (airflow) and round 05 (pretix) saw 0/9 and 0/6 transitive cases under the prior direct-only matcher. New shared helper `linkers/_transitive_bases.py` (BFS with cycle guard) is the single source of truth; both linkers consume it. Direct-base detection unchanged.
- **Four more framework-dispatch linkers traverse transitive base-class chains** (WI-vigih): `jackson_dispatch`, `kafka_streams_dispatch`, `view_template`, and `react_component` join the WI-halat helper. Fixes the same direct-only-base-match gap surveyed cross-linker after WI-halat: a class extending an in-tree intermediate (Spring Data JPA `@Entity` extending an `@MappedSuperclass` chain; Kotlin/Scala SAM-style wrappers around Kafka Streams `Processor`/`ValueMapper`/`Reducer`/`Predicate`; Rails `LeafController` extending project-defined `ApplicationController`; React class components extending project-internal base components) now matches the same as a direct subclass. Helper extension: `collect_transitive_base_names` accepts `meta_keys: tuple[str, ...] = ("base_classes",)` so kafka_streams (which split extends/implements into separate metadata keys) can pass `("base_classes", "interfaces")`; default keeps WI-halat callers backward-compatible. The `react_component` change additionally implements the base-class detection branch its docstring previously claimed but that the code only matched on PascalCase. 16 new property tests (4 per linker: one-intermediate, two-intermediate, diamond cycle, direct-base regression) plus 4 helper tests for the new `meta_keys` parameter.
- **Django generic CBVs inherit View lifecycle methods in `django_orm_dispatch`** (WI-nipan / UAT DQ-02): `ListView` / `DetailView` / `CreateView` / `UpdateView` / `DeleteView` / `TemplateView` entries in `DJANGO_BASE_METHODS` now fold in the View lifecycle (`dispatch`, `setup`, `http_method_not_allowed`, `options`, the seven HTTP verbs, `head`, `trace`). A project class `class Foo(ListView)` previously matched only `ListView`'s frozenset and never reached the `View` entry — Django's class hierarchy is external to the project, so the WI-halat transitive base walk had no edge to follow. UAT v4.0.0 round 05 (pretix) saw zero `dispatches_to` edges to any `*.dispatch` method anywhere in the graph; this contributed alongside WI-halat to WI-nosug's 8/15. New module-level `_VIEW_LIFECYCLE` constant is the single source of truth.
- **gRPC ttrpc / CSI struct embedding walks transitive chains** (WI-pogus): `_link_go_methods_to_rpc_routes` in `grpc.py` now consumes the WI-halat `_transitive_bases` helper for the `meta.base_classes` lookup that detects ttrpc service interface implementations (e.g. `HealthService`, `*Service`, `*Server`) and CSI-style embedding patterns. A Go struct that embeds an in-tree intermediate (`UserHealth` extends `BaseHealthImpl` extends `HealthService`) is detected the same as a direct embedder. Direct embedding remains unchanged. `link_grpc` accepts `existing_edges` so the registry entrypoint threads `ctx.edges` through. 4 new property tests (one-intermediate, two-intermediate, diamond cycle, direct-embedding regression).
- **`type_hierarchy` linker emits skip-level `dispatches_to` overrides** (WI-firuj): the parent→children map is now closed transitively before edge emission. When `Grandparent.foo` is overridden only in `Grandchild` (intermediate `Parent` doesn't override), the edge `Grandparent.foo → Grandchild.foo` is now emitted; previously `Grandchild` was missing from `parent_to_children[Grandparent]` because the map was one-hop. New `close_parent_to_children_transitively` helper performs BFS with cycle guard. Diamond inheritance still emits one edge per (src, dst) pair (no double-emit). Direct-override semantics unchanged. 6 new closure-helper unit tests + 4 new property tests (skip-level grandparent, three-level with intermediate override, diamond cycle, direct regression).
- **Inheritance linker enforces cross-language gating + Rust kind discipline** (WI-zozuz / UAT BUG-03): drops candidates whose `language` differs from the child symbol's before resolution (no more Python `class FooModule(nn.Module)` → Rust `Module` trait spurious edges in candle, eliminating 31 bogus Python→Rust-trait edges); refuses struct/enum candidates when the child is a Rust struct/enum (Rust permits no struct→struct inheritance). Bridge linkers (PyO3, cffi, wasm_bindgen, jni) remain the sanctioned path for genuine cross-language conformance edges.
- **Jackson dispatch linker recognizes JPA `@Entity` / `@MappedSuperclass` / `@Embeddable`** (WI-sokaz / UAT BUG-02): the Spring Data JPA + Spring MVC pattern routinely Jackson-serializes JPA-mapped types as REST response bodies, but the prior matcher only triggered on Jackson, JAX-B, and Spring-binding annotations. On spring-petclinic the linker emitted 1 edge (a JAX-B `@XmlRootElement`-marked class) while the 6 `@Entity` JPA classes (Owner, Pet, Visit, Vet, Specialty, PetType) produced zero. Bean-convention accessors on JPA classes now receive `dispatches_to` edges as expected.
