...
- **Identity-field parity guard (`shape_id`).** The emission-parity matrix gains a `shape_id` column — its first *identity*-field cell — locking structural-shape emission for the eight core analyzers as a hard-lock ratchet, closing the blind spot that let a `csharp shape_id=None` regression slip past every standing test. Scoped to mainstream coverage (per spec §6, `shape_id` is optional at ~41/70 analyzers); the niche solidity/wgsl gaps remain separately-tracked.
...
- **New `hypergumbo repeat-finder` read-view surfaces structural clones as refactoring leads.** It groups symbols by `(language, shape_id)` — a cluster of ≥2 nodes is a set of structurally-identical implementations (same control-flow skeleton, differing only in identifiers and literals), within-language per ADR-0014 — activating `shape_id`'s one non-redundant capability over `fingerprint` (ADR-0035 §1, spec §367): clustering copy-paste / extract-helper candidates. `--format text` (default) prints ranked refactoring-lead blocks; `--format json` emits a `{schema_version, view: "repeat_finder", summary, clusters}` envelope.
- **Signal-dense defaults.** Trivial clusters (cyclomatic complexity below `--min-complexity`, default 2) are dropped and only production clones (≥2 non-test members) headline, with test-only clusters counted as a labeled disclosure bucket shown via `--include-tests`. Clusters rank by duplication burden (member count × representative LOC); `--min-complexity 1` includes straight-line clones and `--limit` bounds text output.

#### Survey rename (ADR-0042)

- **One canonical artifact name + a merged discovery resolver (survey-rename S2/S3, internal, no behavior change).** `behavior_map_io` now defines the canonical output filename `CANONICAL_SURVEY_FILENAME = "survey.json"` plus the four historical aliases (`hypergumbo.results.json`, `hg.json`, `bm.json`, `behavior_map.json`) and a `find_survey_in_dir()` primitive that discovers any of them; the CLI's input auto-discovery is rewired onto it, widening discovery to the canonical name and every legacy alias with no user-facing change. This is the shim-first foundation for the rest of the ADR-0042 rename.
- **`hypergumbo survey` is the primary analysis verb; the default output is `survey.json` (survey-rename S2-CLI, WI-vatuf).** `hypergumbo survey <repo>` produces `survey.json`; the default cache basename and every internal cache-reader move to the canonical name, resolving legacy aliases on read so pre-rename caches keep working, and all `--input` help strings name `survey.json` / `hypergumbo survey`. **`hypergumbo run` remains a fully-functional deprecated alias** that prints a one-line stderr deprecation warning; per the ADR-0042 one-minor-version window it and the legacy filename aliases are removed in the following minor release.
- **Legacy survey filenames are accepted on read with a deprecation warning (survey-rename, WI-didif).** Every `--input` consumer routes through `load_substrate`, which now warns to stderr when the loaded file uses a legacy basename (`hypergumbo.results.json` / `hg.json` / `bm.json` / `behavior_map.json`, incl. `.gz` forms) — naming the alias and canonical `survey.json` — while still loading it. The aliases stay accepted for one minor version, then are removed at window-close (ADR-0042).
- **`behavior_map_io` module renamed to `survey_io` (survey-rename, WI-kisoj).** The consumer-side substrate loader (`load_substrate`, `load_behavior_map`, `find_survey_in_dir`, `find_behavior_map`, the filename constants, `SubstrateError`) now lives in `hypergumbo_core.survey_io`; `behavior_map_io` survives as a deprecation shim that re-exports the same names and warns on import (removed at window-close). The `cli.run_behavior_map` analysis function is likewise renamed to `cli.run_survey`, with `run_behavior_map` kept as a module-level alias for the deprecation window.
- **Docs concept rename "behavior map" → "survey" (survey-rename, WI-rivuj + WI-zaviv).** README, the spec, USE-CASES, and present-tense docs now call the JSON output artifact a **survey** (produced by `hypergumbo survey` → `survey.json`). The on-disk `view` discriminator value stays `"behavior_map"` for schema compatibility, so consumers parsing the JSON see no change; historical/migration docs keep their original vocabulary.
...
- **Receiver-type-dispatch linker — shared substrate for extension/UFCS calls (INV-vigaf).** A new Infrastructure linker `receiver_type_dispatch.py` resolves unresolved `x.foo()` calls carrying a `receiver_type_hint` to an **extension method** (`meta.extension_receiver`) or a **UFCS free function** (`meta.ufcs_receiver_type`) whose declared receiver / first-parameter type matches — one receiver-type-keyed search unifying the two non-hierarchy call forms, distinct from the ancestor-walking `inherited_calls` linker. Language-scoped (INV-milud) and ambiguity-gated (INV-fahub). A new `ast_call_ufcs` evidence_type (confidence 0.8) names UFCS resolutions; `edge_type` stays `calls` (ADR-0023). The linker is inert on current output until analyzers stamp the receiver-type hints — no output change this release.
- **Kotlin extension-call resolution moved into the shared linker (INV-vigaf, WI-lodij).** Kotlin's analyzer no longer emits the resolved `ast_call_extension` edge for `receiver.extFn()`; it emits an unresolved call carrying `receiver_type_hint`, and the `receiver_type_dispatch` linker emits the resolved edge — bringing Kotlin under the same analyzer→linker contract as inherited calls (INV-nilud analogue). The analyzer retains a local extension lookup solely to track return types for chained navigation calls, so recall is unchanged. The linker gains generic base-name normalization (`List<Int>` ↔ `List<String>` match on `List`).
- **D UFCS receiver gating + linker recovery (INV-vigaf, WI-situj).** D's analyzer misbound an unresolvable-receiver call `thing.exists()` to an arbitrary same-named free function `exists()`. The gate now withholds **any non-module value receiver** — a parameter, local, loop variable, or field (`_is_module_receiver` distinguishes a UFCS value prefix from a genuine imported-module prefix, which still resolves) — emitting the call unresolved with a `receiver_type_hint` when the value's type is known, cutting the dominant dead-code false-positive funnel. Free functions are stamped with `meta.ufcs_receiver_type` (their first-parameter type) so the `receiver_type_dispatch` linker recovers the real UFCS target (`x.foo()` ≡ `foo(x)`) as an `ast_call_ufcs` edge. A follow-up excludes nested-local functions (defined inside another function's body) from the global resolver index — like `field`/`variable` symbols they stay output symbols but are never a cross-scope call target — closing the last funnel.
- **Scala + Swift inherited-method resolution (MRO walkers).** The `inherited_calls` linker gains Scala trait-linearization and Swift superclass-then-protocol-extension walkers, registered for `scala`/`swift`. These recover inherited-method calls on a typed receiver — `receiver.inheritedMethod()` where the method lives on an ancestor/mixed-in trait — completing recall for the Scala/Swift receiver-gating facets. External-base shadowing (INV-guviv) stays Python-scoped; the new walkers resolve in-tree ancestors.
...
- **Python nested-function resolution walks the full LEGB scope chain**, resolving calls from grandparent-or-higher enclosures (additive; fires only after local/import misses).
- **External base classes emit `extends` edges (Python).** A Python class whose base is external/stdlib (`Enum`, `Exception`, `Protocol`, `ABC`, …) now emits an unresolved-external `extends` edge to a boundary placeholder instead of dropping the relationship, so type-hierarchy queries no longer answer "nothing" for the ~1-in-4 Python classes whose bases are all external. Confidence stays evidence-derived (`ast_extends` → 0.95); `is_resolved=False` carries the unresolved target. An in-tree guard keeps a not-yet-extracted in-tree base from leaking as a workspace-prefixed phantom (INV-nuzas), aliased in-tree bases re-resolve, and dotted/qualified bases (`argparse.X`) defer to the core-linker chokepoint. (WI-jubag Python slice; mirrors the landed JS/TS A2 fallback.)
- **External constructors type as `instantiates` (Python).** A call to an imported external PascalCase name — `Path()`, `MagicMock()`, `argparse.ArgumentParser()`, … — now emits an `instantiates` edge (`evidence_type="ast_new"`, `is_resolved=False`) instead of being misfiled as a plain `calls` edge, so `instantiates` records external constructions rather than zero. snake_case callables (`join()`, `os.getcwd()`) stay `calls`. Covers the bare-name and `module.ClassName()` forms. (WI-jubag instantiates half.)
- **Caddy module-registry dispatch linker (WI-zizig).** New Framework-subcategory linker that recovers Caddy's reflective plugin dispatch: modules register at `init` via `caddy.RegisterModule(T{})` and are driven by string ID through `LoadModuleByID`, so their handler methods (`Provision`/`Validate`/`ServeHTTP`/`Start`/`Stop`/`Cleanup`) have zero static incoming edges and fall off the forward slice from `main()`. The linker identifies a Caddy module by its `CaddyModule() ModuleInfo` marker method and emits `dispatches_to` edges from that marker (the one node the static graph already reaches) to each handler, putting the handlers back on a forward slice. The runtime string-ID coupling is not statically recoverable, so specific `LoadModule` sites are deliberately not matched. Residuals: module-namespace-specific handlers beyond the universal interface hooks, and cross-package `init → RegisterModule` resolution.

#### Symbol identity, introspection & axes

- **Symbol ids use real kind-slots (ADR-0036).** Route-materializer, event-sourcing, boundary, and package/config symbol ids now carry real kind-slots (`function`, `external_symbol`, `package`, `file`) instead of fold-source or role values, with framework role moved to `meta`. Boundary identity becomes `(language, path, name)` so the same external reached via different syntax dedupes; the rest are identity-neutral and clear residual `id_format` validator warnings.
...
- **General YAML file-anchor nodes** emit per generic `.yaml`/`.yml` file (coexisting with `yaml_ansible`).
- **`slice --entry <X> --io-boundary <category>`** filters a slice to the edges that reach that I/O boundary.
- **Python variable- and field-kind symbols now carry a structural `shape_id` (WI-luzut).** Module-level variables and class attributes previously emitted `shape_id=None`; they now hash the assignment statement's AST skeleton (identifiers/literals stripped), so `X = 5` / `X = foo(a)` / `X: int = compute()` get distinct shape_ids, with the symbol kind folded into the prefix (WI-linon discipline) — making them visible to the `repeat-finder` clone detector (WI-vogij). Additive: no existing shape_id value changes, so `shape_id_scheme` is unchanged (WI-rihob precedent).
- **Callable-kind declarations that lacked a `stable_id` now carry the typed producer id (WI-vibad).** Solidity `function` / `constructor` / `modifier` / `event` and Go interface-method declarations emitted `stable_id=None` — deliberately excluded from the WI-rihob name-scoped backstop (a name-only key collides on overloads: two `Token.mint(...)`). They now mint the signature-bearing `make_typed_stable_id`, exactly as concrete methods and fields do, so overloads stay distinct. Additive; `stable_id_scheme` unchanged.
- **Five config-family analyzers emit canonical `stable_id`s (WI-banod, first batch).** `ini` (section/setting), `requirements` (requirement), and `properties` (property) previously set `stable_id` to the non-canonical composite `Symbol.id`; they now use the shared `make_doc_stable_id` factory (canonical `sha256:<16hex>`), matching the doc/markup analyzers. `node.id` is unchanged. These three are default-output-neutral (noise-kind symbols); the `bitbake`/`bibtex` remainder is deferred as observable identity churn pending a coordinated scheme decision.
...
- **`Edge.confidence_source` + `Edge.rank_score` / `Entrypoint.rank_score` — the confidence-separation field substrate (ADR-0039 rulings 2 & 3).** `confidence_source` (bounded-enum `evidence_derived`/`emitter_constant`/`composite`) makes the migration off hardcoded per-emitter constants machine-readable; `Edge.create` stamps it based on how the value was derived. `rank_score` is the ranking-prominence home the ranking adjustments will relocate onto (ruling 3); it initializes from `confidence` so **published confidence and ranking are unchanged** until the per-producer relocation PRs land. A guard (`confidence.find_constant_confidence_violations`) flags any emitter shipping one constant across many edges without declaring it, and the axis-conformance validator range-checks `confidence_source`. SCHEMA_VERSION 0.14.4 → 0.14.5 (additive).

#### Framework provenance (WI-napuj)

- **`profile.framework_evidence` traces each declared framework to the nodes that import it.** `profile.frameworks` listed framework names with no graph-level anchor — a consumer reading "frameworks: starlette" could not find *where* starlette was used. `refine_frameworks`'s existing framework-to-importer join is now kept rather than discarded, as `profile.framework_evidence: {framework: [node_id, …]}` (prod, non-test importers, language-gated so Python `http.client` is not counted as Julia `http` evidence). A manifest framework whose modules never appear in an import edge is absent rather than keyed to an empty list (INV-virik omit-when-empty). Additive; `profile` is permissively schema'd, so no SCHEMA_VERSION bump. INV-numat member.

### Changed

#### Edge-type vocabulary — `endpoint_shape` + `pending_classification` folds complete; `Edge.edge_type` drained to one `relationship` axis (schema 0.14.6 → 0.18.0)

- **The 21 long-tail `endpoint_shape` values are producer-migrated to canonical relationships + `meta`, then pruned from the registry (WI-pumav Batches 0–7, WI-hirud; ADR-0023 §6, audit-findings 0017).** Each value folds to the relationship it actually is, its construct/mechanism flavor preserved in `meta`: → `calls` (`abi_call`, `caller_invokes`, `kernel_launch`, `template_calls`, and the `http_calls`/`grpc_calls`/`graphql_calls` protocol family via `meta['protocol']`); `references` (`links_to`, `uses_vocabulary`, `association`, `build_tag_alternative_of`, `renders`, `script_src` — `meta['ref_construct']`); `includes`/`extends` (Twig/Puppet/Sass template & mixin edges); `depends_on` (`depends`, `requires_resource`, `base_image`, `notifies_resource`); `dispatches_to` (`invokes_callback`, `signal_receiver` — kept in the dead-code reachability set so handlers stay reachable); `contains` (`contains_routes`); and `data_flows_to` (`crypto_flow`). Consumers already read the canonical types, so behavior is unchanged apart from the label + `meta`; the dead registry entries are then removed and the `endpoint_shape` axis drained to empty. **`SCHEMA_VERSION` 0.14.6 → 0.17.0** (the protocol-call family, `script_src`, and the consolidated long-tail prune, in staged minor bumps per the ADR's Phase-4b removal contract).
- **The `pending_classification` resolver/OpenAPI/RPC family folds to `implements`/`references`, draining its axis too — leaving `Edge.edge_type` with a single `relationship` axis (WI-sumik, WI-pusuv; ADR-0023, audit-findings 0016).** `resolver_implements` → `implements` + `meta['protocol']='graphql'`; `resolver_for_type` / `openapi_implements` → `references` + `meta['ref_construct']` (direction-preserving); `implements_rpc` → `implements` + `meta['protocol']='grpc'`. Because `implements_rpc` was **call-like** (traced for taint, I/O reachability, ranked at call weight, slice-traversable) where a structural `implements` is not, a single shared predicate `is_grpc_rpc_implementation` recognizes the folded form and is composed into every one of those consumers (`taint` / `io_boundary` / `verify_claims` / `ranking` / `slice`), transferring the coupling without wholesale-including structural `implements` edges. The four dead registry entries are pruned. **`SCHEMA_VERSION` 0.17.0 → 0.18.0**; the total `Edge.edge_type` registry drops 50 → 25 (`AXIS_PENDING` retained but unoccupied). A behavior map produced before the migration no longer validates against the enum.
- **`access_mode` applicability census COMPLETE — every `Edge.edge_type` value is classified applicable-XOR-N/A (WI-pusuv, closing INV-tibob's tail).** With both non-relationship axes drained, the applicability matrix (`axis_meta_keys.py`, ADR-0038 ruling 2) covers the whole 25-value registry: a consumer reading `access_mode=None` can now *always* distinguish "the question does not arise" (an N/A type — `subprocess_calls`, `data_flows_to`, and the structural `constrains`/`includes`/`wraps`/…) from "missing data on an applicable type", locked by a total-census property test. No `SCHEMA_VERSION` change.
- **The two per-value verdict passes are recorded (audit-findings 0016 / 0017; WI-sumik, WI-pumav).** Both find every value FOLD (zero CANONICAL) under the 4-lens ruling method — the docs record of which values fold to what, noting the `openapi_implements` direction wrinkle and the `implements_rpc` consumer-coupling caveat.

#### Meta-layer honesty

- **Per-run and limits reporting lists are present only when non-empty (INV-virik).** `analysis_runs[].{skipped_passes, failed_files, warnings}`, `limits.{failed_files, skipped_languages, truncated_files}`, and `limits.supply_chain.{classification_failures, ambiguous_paths}` are now omitted when empty rather than serialized as an always-`[]` list, so their absence honestly reads as "nothing to report"; an empty `limits.supply_chain` serializes as `{}`. The fields were already optional (no version bump); `limits.skipped_passes` and `limits.test_files_excluded` stay always-present.
- **`nodes[].quality` is omitted when null (INV-nuzal).** The node-level `quality` field has no producer and serialized as a universally-`null` key on every node; it now emits only when a producer sets it, closing the last residual of INV-nuzal (`edge.provenance` already dropped, `edge.quality` derived). Already optional (no version bump); its schema description now states it is unpopulated.
- **`resolution_quality='type_inferred'` no longer mislabels the give-up branch (WI-javus).** Python's unresolved method-call fallback stamped `meta.resolution_quality='type_inferred'` unconditionally, including on the give-up branch (an untyped/duck receiver, INV-fahub-biased to unresolved) — so a consumer branching on the string read the give-up case as a successful inference. The label is now derived from whether the receiver-hint chain actually established a type; the give-up fall-through carries no `resolution_quality` (the field names the resolution pathway per spec §903, and here there was none). Already optional (no version bump).
- **`analysis_runs[]` is serialized in a deterministic order (WI-haguz).** The array was written in accidental pass-completion order; it is now sorted by `started_at` (pass-id tiebreak), a deterministic contract consumers can rely on.

#### Confidence derivation (ADR-0039)

- **Analyzer edge `confidence` is now derived from the inference pathway, not hardcoded.** `Edge.create` derives confidence from `evidence_type` when `confidence=` is omitted, and analyzer sites dropped their hardcoded values so every seeded analyzer edge carries the derived value. Linker edges keep explicit confidences (a separate match-quality model). `confidence_model` v1 → v2.
- **Containment `naming_convention` edges now derive in-band confidence, ending the reliability inversion (ADR-0039 ruling 1; WI-vakuh, WI-lutad).** The containment linker's name-parse heuristic hardcoded `confidence=1.0` on the bulk of `contains` edges — above the linker ceiling and outranking the structurally-certain `span_overlap` — so a consumer trusting confidence treated the heuristic as gold. `naming_convention` is now seeded at **0.85** (below `span_overlap`, in-band) and derived rather than hardcoded, carrying `confidence_source=evidence_derived` with reliability ordering restored.
- **type-hierarchy `dispatches_to` confidence is separated from its ranking dampener (ADR-0039 rulings 1 & 3; WI-botif).** The type-hierarchy linker fused a fan-out dampener (WI-kabom) and a test-file penalty (WI-supok) into `confidence`, driving published values well below the documented linker floor. `confidence` now derives the flat in-band **0.85** `type_hierarchy` base (`confidence_source=evidence_derived`), and the dampener + penalty relocate intact to the new `rank_score`, which the ranking filter re-keys onto — so high-fan-out dispatch edges are still demoted out of centrality (the WI-kabom signal survives) while confidence stops undershooting the floor.
- **Spec §12 reconciled to the implemented confidence model (ADR-0039 ruling 5 Stage B, prose).** The confidence-scoring section no longer lists `naming_convention` as a 1.0-emitting pathway (now derived), the `type_hierarchy` dampener as "not yet derived" (now on `rank_score`), or `confidence_source` / `rank_score` as "later stages" (implemented); it documents both new fields and notes entrypoint `confidence` is pure detection reliability with the `connectivity_based` / frontend kinds deliberately below the 4-tier floor. A stale Appendix-C "planned, not implemented" line is corrected; the generated per-evidence-type table + freshness gate is deferred.
- **Entrypoint `confidence` is now pure detection reliability; ranking adjustments move to `Entrypoint.rank_score` (ADR-0039 ruling 3; WI-lutad, WI-dojor).** The entrypoint ranking adjustments — test / utility / vendor / nested-Go-cmd penalties, the library-export demotion (the "forgejo" fix), and the in-degree (WI-dojor) / out-degree connectivity boosts — used to mutate `confidence` in place, moving nearly every published value off its documented tier. They now accumulate on `rank_score` (initialized from confidence, so the ranking signal is unchanged), and the entrypoint sort, the `MIN_ENTRYPOINT_CONFIDENCE` filter, the sketch entrypoint sorts, and slice auto-entry / `--min-confidence` seed selection all re-key to it. Published entrypoint `confidence` is now the construction-tier detection reliability, with the deliberate low-confidence `connectivity_based` and frontend-suppression kinds below the 4-tier floor by design.

#### CLI read-view contract

- **Read-view JSON single-sources one envelope and one version.** Inlined envelope sites now flow through one helper and six views single-source `READ_VIEW_SCHEMA_VERSION`; the spec documents the three independent version axes (format, tool, per-view). `verify-claims --json` now emits a versioned `{schema_version, view, verdicts, …}` object instead of a bare array (breaking for array-parsing consumers).
- **`explain` errors on an ambiguous symbol name** instead of silently iterating every match (escapable with `--first` / `--language` / `--file`).
- **`io_primitives` catalog completeness is now disclosed (WI-najil).** The `io-boundaries`, `verify-claims`, and `slice --io-boundary` consumers each emit a stderr warning when a query targets a language whose `io_primitives/<lang>.yaml` ships `status: in_progress` — so a zero-match result is not silently read as "no I/O in this code". Most catalogs are `in_progress` (only python/rust/erlang are `complete`), and the `status` field is now documented in the spec. Unsupported languages keep the separate `is_supported=False` signal (INV-javam).
...
- **`access_mode` gains a declared per-edge-type applicability matrix** — the classifier no longer stamps spurious values on edges where access is meaningless. FFI-bridge direction-smuggling moves off `access_mode`/`dest_access_mode` onto a new `data_direction` meta key (`src_to_dst` / `dst_to_src` / `bidirectional`, ADR-0038).
...
- **`metrics.languages.<lang>.files` is now populated (WI-ninaj).** The per-language dict carried `{nodes, edges}` but never a `files` count, so it read `0`/absent despite a non-zero `metrics.total_files`. It now counts file-kind nodes per language, node-derived and consistent with `total_files` rather than the over-counting `profile.languages` sum.
- **`metrics.total_files` no longer counts the `<external>` sentinel as a file (INV-mozaf).** The distinct-node-path count folded in the `<external>` placeholder carried by `external_symbol` boundary nodes, inflating `total_files` by 1 on any repo with external references. It now excludes the sentinel, so `total_files` equals the count of real path-bearing source files; `debug.unique_paths_in_analysis` is corrected in lockstep.
- **`io-boundaries` `total_io_edges` redefined to the verified surface** — it excludes `external_potential` (disclosed separately as `external_potential_edges`); text and JSON now agree (io-boundaries view schema 1.0 → 2.0).
...
- **AnalysisRun `repo_fingerprint` field carries the `sha256:` scheme prefix (WI-bosog; `repo_fingerprint_scheme` v1 → v2).** `run_signature` / `config_fingerprint` used `sha256:<16hex>` while `repo_fingerprint` was bare 64-hex, so a consumer could not strip a scheme prefix uniformly. The field is now `sha256:<64hex>` — prefix added, full digest retained, matching the `pass_version` convention. The bare digest returned by `compute_repo_fingerprint` is unchanged where it doubles as the colon-free analysis-cache path segment.
...
- **`Edge.quality` (`{score, reason}`) is deprecated (ADR-0039 ruling 4; WI-humok / WI-riguh).** It carries zero independent signal — `quality.score` mirrors `confidence` on every verification-corpus edge and `quality.reason` encodes the emitter mechanism, not a confidence tier. The `deprecated` annotation lands in `docs/schema.json` now (SCHEMA_VERSION 0.14.5 → 0.14.6); the field is still emitted this release and removed the next, per the one-version deprecation window. Read `confidence` + `confidence_source` + `is_resolved` instead. (`Symbol.quality` is a separate field, not in scope.)

### Documentation

- **Documented three intentional schema divergences, closing INV-nihug / WI-gabos / WI-palon (INV-luhur children).** All three are correct-by-design, and the spec/schema now say so:
  - **`skipped_passes` two-channel semantics (INV-nihug).** `limits.skipped_passes` is the single authoritative record of pass-level skips; the per-run `analysis_runs[].skipped_passes` is an unpopulated legacy mirror. Corrected in spec §9/§17 and the schema description.
  - **`supply_chain_summary.derived_skipped` carries `{files, paths}` not `{files, symbols}` (WI-gabos).** Tier-4 derived artifacts emit no symbols, so a `symbols` count would be structurally always 0 (cf. ADR-0040); `paths` names the capped sample per ADR-0039. Documented in spec §14.
  - **`profile.languages[L].loc` vs `node.line_span` (WI-palon).** `profile.loc` is the authoritative per-file SLOC; `node.line_span` is a per-symbol span that overlaps across nesting and is not summable. Documented in spec §profile.
- **Corrected identity-field documentation drift (identity-vocabulary concept audit, 2026-07-15).** A Fundamental Concept Audit of the `# axis: identity` fields found the axis conceptually clean but with stale/circular docstrings on the structural-identity family:
  - **`Symbol.fingerprint` docstring was factually wrong** — it read "Content hash of source bytes (sha256)"; the producer (`hgfp2:` scheme) computes a whitespace/comment-invariant structural parse-subtree hash. The "source bytes" text described the ADR-0032-demolished Format-1 scheme.
  - **`Symbol.shape_id` was defined circularly** as "Structural implementation fingerprint"; the docstring + schema now state the distinguishing rule — shape_id **strips** identifiers/literals, `fingerprint` **keeps** them — and note shape_id is a strict coarsening of fingerprint with no current consumer.
  - **`Symbol.id` schema description** now states its location-addressedness, pointing readers to `stable_id` (edits-surviving) and `fingerprint` (rename-tracking).
  - **`Edge.derived_from`** docstring now flags it as provenance (PROV wasDerivedFrom), not identity-of-the-edge; it carries `# axis: identity` only because it holds identity references and does not participate in `edge_key`/dedup.
  - **`AnalysisRun`** docstring now notes that `run_signature` / `repo_fingerprint` / `config_fingerprint` / `pass_version` have no internal readers (serialized for external tooling only), while `execution_id` + the `origin_run_id` FK are load-bearing.
  - **ADR-0032** gained a scheme-drift note: the retained Format-2 hash shipped as `hgfp2:<16-hex>`, not the `hgfp1:`/70-char form the ADR describes.
- **Documented three more identity/tier legibility divergences, closing WI-tisar / WI-niboh / WI-nibul (all correct-by-design).**
  - **`stable_id` and `shape_id` share the `sha256:<16hex>` surface (WI-tisar).** The `sha256:` prefix names the hash algorithm, not the identity axis; the two are discriminated by field name (and their `stable_id_scheme` / `shape_id_scheme` descriptors), not by an in-value prefix, and their value-spaces are disjoint. Documented in `ir.py` + the schema with a caution not to join on bare hash values across the two axes.
  - **`Edge.edge_key` is the Edge counterpart of `Symbol.stable_id` (WI-niboh).** Added the missing cross-reference — `edge_key` is the structural-identity / cross-pass dedup key while `edge.id` is per-instance identity, mirroring `Symbol.stable_id` / `Symbol.id`. Named `edge_key` for historical reasons; renaming would be a breaking schema change.
  - **`metrics.by_supply_chain_tier` omits the `derived` tier that `supply_chain_summary` carries (WI-nibul).** Intentional: `by_supply_chain_tier` enumerates only analyzed tiers (1-3); tier-4 / derived is excluded from analysis (§14) and surfaces solely as `supply_chain_summary.derived_skipped`, so an always-empty `derived` bucket would be structurally always 0 (ADR-0040). Documented in spec §metrics.
- **Corrected the `edge.meta.io_boundary` axis description to reflect the WI-fakuv / WI-puvun on-demand reframe (INV-lifik investigation).** The spec claimed the field is "Set by io_boundary catalog," but the full value-space (`fs_read` / `net_send` / …) is consumer-time-derived by `compute_boundary_map` from the `io_primitives` catalogs and is not persisted — only the `io-boundaries` / `verify-claims` / `slice --io-boundary` paths stamp it in memory. What persists is only analyzer producer-hints — currently just bash's `command_launch` — so the axis description now directs consumers needing the full classification to run `compute_boundary_map`.
- **Documented the `is_test_file` concept boundary (WI-popok fundamental-concept-audit KEEP verdict).** Two predicates share the name: the narrow supply-chain role flag `Symbol.is_test_file` ("is this test code?", spec §14) vs the broad `paths.is_test_file` ranking/scan heuristic (also covers mocks/fixtures/testdata/benches). WI-popok proposed folding the entrypoint detector onto the first; the audit found them distinct-purpose (all four leakage tests pass → KEEP) — a swap would flood the entry list with mock/fixture/benchmark entrypoints. Documented both docstrings, annotated ADR-0041 §2, and added a regression-guard test that doubles as the re-evaluation trigger. Advances the INV-numat drain, removes a false META-bifif member. No behavior change.
- **Documented the `evidence_type` ⊥ `is_resolved` boundary + added the META-bifif property-test template (WI-molit KEEP verdict).** WI-molit claimed the `ast_call` vs `ast_call_direct` split redundantly encodes `is_resolved`. A 3-lens audit returned **KEEP**: `evidence_type` names the inference pathway (ADR-0028), resolution lives solely on `Edge.is_resolved` (ADR-0037), and no consumer reads the label as a resolution proxy. WI-molit → `wont_do`; INV-numat drain −1; a second false META-bifif member removed. No production-code change. Full audit: `~/hypergumbo_lab_notebook/concept-audit-evidence_type_is_resolved_07172026.md`.
- **Documented the `access_mode` / `data_direction` Edge.meta applicability matrix in spec §9 (closes INV-tibob).** The spec's Edge.meta catalog omitted `access_mode`, leaving its "0% coverage on 12 of 17 edge types" reading as a gap. It is the *declared* applicability (ADR-0038 ruling 2): `access_mode` (`read`/`write`/`mutate`) applies only to data-carrying edge types (`calls`/`references`/`module_attr_ref`/`event_publishes`); on structural relationships the question does not arise and the key is absent by design. The `MetaKeySpec` is the schema of record, distinguishing "None = missing data" from "None = does not apply".
- **Authored the META-bifif "no-consumer-re-derives" umbrella property test.** META-bifif's invariant — consumers honor a granular producer verdict rather than re-deriving a coarser one — spans three verdicts (resolution, language, test-code) whose per-verdict guards were distributed across test files. The umbrella now has a single documented home stating the contract + reusable premise-check pattern, with the test-code KEEP-distinct exception made explicit (the entrypoint ranker's broad `paths.is_test_file` is a distinct concept, not a coarser re-derivation — WI-popok) and a completeness guard so its three-verdict coverage cannot drift silently. (META-jalur's finalize round-trip reconcile test already exists.)
- **Spec contract-accuracy pass.** Documented the `metrics.languages` attribution rule, the relocated `supply_chain` booleans, the `feature`/`query` record shape (dropping a phantom `summary`), and the `Node.meta` catalog; added the missing §9 prose for `limits.max_tier_applied` (WI-sumur); and documented why `profile.languages` and `metrics.languages` legitimately differ in their language key-sets (WI-ziror).

### Fixed

#### `verify-claims` violated-flow evidence is now drillable (WI-kikis)

- **A violated taint-flow claim now surfaces drillable per-flow evidence instead of collapsing thousands of flows to five indistinguishable rows.** The `details` string previously showed the first five violations as bare `<source> -> <sink>` primitive pairs — frequently verbatim duplicates with no symbol, line, or path. The verdict now deduplicates on the full per-flow identity, renders each shown row with its symbol IDs plus a `via N hop(s)` path indicator, discloses the true total-vs-distinct count (`N flow(s) (D distinct)`), and attaches a bounded, deduplicated structured **`evidence`** array (`source_symbol` / `sink_symbol` / primitives / `path`, capped at 100 distinct flows) to the `--format json` envelope so consumers can triage programmatically. The `verify-claims` view version bumps `1.0` → `1.1` (additive field).

#### Kind-conditioned writer-contract population contract (WI-libib / WI-hudug)

- **The writer-contract validator gains a per-`(language, kind, field)` population contract, catching regressions the record-class-level checks structurally cannot see.** A field populated on one Symbol kind (`qualified_name` on `function`) previously masked a 100%-NULL partition on another kind (`method`). The new check partitions symbols by `(language, kind)` and flags a registered cell that is 100% NULL across its non-empty kind partition, emitting one umbrella `writer_contract` warning (an absent partition is skipped). The inaugural entries — `qualified_name` on Python function/method/class (WI-hudug; producer done by WI-fagab / ADR-0032) — emit zero violations and stand as a regression guard: a future producer that drops the field trips them, and the resulting `writer_contract|warning` cell breaks the `validation_ratchet` self-tree gate. WI-libib.

#### Manifest CLI entrypoints no longer dropped from the default behavior map (WI-papag)

- **pyproject `[project.scripts]` / console-script CLI entrypoints now survive the default-view noise filter and are detected.** The Phase-D noise filter treated every `entry_role=script` file symbol as noise, silently erasing entrypoint-bearing console-scripts before `detect_entrypoints` could see them (the ADR-0043 §5 "C3" defect). The predicate is now subset-refined by `meta["entry_point"]`: a script node that declares a code target (detected as `CLI_COMMAND` @0.99 `manifest_declared`) is exempt and survives, while a bare npm `package.json` run-script (`meta["command"]`, no `entry_point`) is still filtered. Reconciles audit-findings 0005 (filter npm run-scripts) with ADR-0043 §5 (exempt entrypoint-bearing scripts) by subset.

#### Cross-language call-graph precision — unresolvable-receiver misbinds (INV-fahub)

- **Scala: an unresolvable-receiver call no longer misbinds to an arbitrary same-named internal def (WI-bihit).** A call whose target class couldn't be resolved previously fell through to a bare short-name lookup and bound to the first same-named method (the `copy`/`setTo` funnel). The Scala analyzer now withholds every bare call that resolves only to a class-member `method` without scope evidence, emitting an honest unresolved `calls` edge (`is_resolved=False`) stamped with the caller's `enclosing_class`; the shared `inherited_calls` Site-1 walker then recovers it iff the method is on the enclosing class's linearization (`ast_call_inherited`), while a cross-class magnet stays external. On docspell, cross-class `copy`/`setTo` misbinds drop **242 → 0** and ~3,000 fabricated magnet call-edges become honest externals; conforms to **INV-nilud** / **INV-nogof** (withhold, never pick-first). Swift and D carry the same funnel and follow.
- **Swift: the same bare→method magnet gating (WI-votar).** As in Scala, the dominant `create`/`delete`/`run` funnel (VernissageServer) was a bare call suffix-matching an unrelated type's method. Swift's resolver now applies the shared `defer_bare_method_call` gate — a bare call resolving only to a class-member `method` on weak suffix/ambiguous evidence is withheld and stamped with the caller's `enclosing_class`, so the `inherited_calls` Site-1 walker recovers it iff the method is inherited (`ast_call_inherited`) and a cross-type magnet stays external. Typed-receiver and function/method-parameter receiver calls still resolve. On VernissageServer the magnet is eliminated (302 → 0 high-confidence cross-type misbinds; 23 inherited calls recovered).
- **Shared `defer_bare_method_call` helper (INV-fahub).** The bare-call magnet decision — bind directly to a free function / object / same-enclosing-class method, else withhold a weak-suffix cross-class method to the `inherited_calls` Site-1 walker — is extracted as one shared predicate (generalized with a `separator` param for `Owner::method` languages). Scala and Swift route their bare-call resolution through it; it is the substrate for the fleet-wide rollout below.
- **Fleet-wide rollout of the magnet gate (INV-fahub).** The bare→method magnet gate is now wired into eleven more analyzers — **go, rust, php, js/ts, csharp, groovy, dart, lua, zig, cpp, objc** — through the shared `defer_bare_method_call` helper. Each withholds a weak short-name bind to a cross-class `method` (emitting an unresolved edge stamped with the caller's `enclosing_class`) while free functions/objects, same-enclosing-class implicit-`this`/`self` methods, and strong exact / import-scoped matches still resolve. Per-language scoping is preserved where the resolver path carries genuine routing evidence (Go gates only truly bare calls, leaving package-qualified `pkg.Foo()` selectors untouched).
- **Site-1 MRO walkers for the fleet languages (INV-fahub).** The `inherited_calls` Site-1 walker is now registered for **go, rust, php, javascript, typescript, csharp, cpp, objc**, so a bare inherited implicit-`this`/`self` call the gate defers is recovered (`ast_call_inherited`) when that method lives on the enclosing class's MRO — and left honestly external otherwise. Walker choice follows each language's inheritance shape, and the language-agnostic `inheritance` linker turns each analyzer's `base_classes` / `implements` metadata into the edges the walkers traverse; **dart/lua/zig are deliberately unregistered** — their analyzers model no class inheritance, so nothing is recoverable there.
- **Untyped-receiver-method magnet demotion at finalize (INV-fahub, WI-makiz).** The fleet gate closes the bare-identifier path, but an **untyped-receiver method call** (`d.Val()`, `x.Parse()`) whose receiver type can't be inferred still resolved to an arbitrary same-named internal `method`, collapsing dozens of unrelated call sites onto one `Owner.method`. A language-agnostic detector (`find_receiver_blind_magnets`) now measures these on the reconciled graph, and a new finalize sub-step (6c, before the ADR-0037 edge-resolution verdict) demotes the two cleanly-harmful sub-classes — a **production→test-helper** misbind and a **stdlib-interface-method shadow** (`Close`/`Parse`/`Len`/…) — by redirecting the edge's `dst` to `{lang}:external:0-0:{method}:unresolved` (so the ADR-0037 verdict derives `is_resolved=False`) and stamping `resolution_quality=ambiguous`. Correct-but-unprovable binds are deliberately kept: the Rust trait-dispatch funnel needs real type resolution (ADR-0012), and same-module builder / test→test-helper calls are untouched. Rust `Type::method()` scoped calls are now stamped `receiver=qualified` so a correctly-resolved associated-function call is no longer counted as receiver-blind.
- **Rust qualified-receiver marking extended to the survey `method_resolver` fallback (WI-fazaj).** The `receiver=qualified` stamp above covered the analyzer's scoped-call sites; a `Type::method()` call that instead resolves through the cross-package survey `method_resolver` / bare-name fallback now also carries it, so it is no longer counted as a receiver-blind magnet by `find_receiver_blind_magnets`. Measurement-precision only — these are correct binds the durable INV-fahub gate already ignored (INV-fahub stays satisfied); the branch carries `# pragma: no cover` because it is reachable only through the full survey pipeline.
- **Durable INV-fahub gate in `validate_ir` (WI-makiz).** A new corpus-wide `spec_validator` check (`no_harmful_receiver_blind_magnets`, disclosed in the wired-checks manifest) asserts the finalized graph carries **no un-demoted harmful receiver-blind magnet**, sharing the exact `find_harmful_magnets` predicate with the finalize demotion so the two can never drift. It is green on every clean survey but fires an `error` if the demotion ever runs out of order or a new producer path emits a harmful magnet — the standing regression teeth behind the refined INV-fahub criterion (the trait-dispatch residual stays ADR-0012 scope).

#### Analyzer catalog completeness (WI-didil)

- **Every registered analyzer now resolves to an `analysis_runs[]` entry OR a `limits.skipped_passes[]` record — no analyzer silently vanishes.** Analyzers whose declared language is absent from the taxonomy (`matlab`, `meson`, `puppet`, `racket`, `robot`, `scheme`, `scss`) bypass the file-presence pre-filter and are dispatched defensively; with no matching files each returned a bare `run=None` result that the orchestrator chokepoint silently dropped, recording neither a run nor a skip. It now records a `{"pass": <name>, "reason": "no files matched"}` skip for an empty `run=None` result, honouring a self-declared `skip_reason` when present (also closing a latent drop of grammar-unavailable skips). On the self-corpus the analyzer catalog went from 8 uncovered passes to 0. Non-analyzer passes (linkers/synthesis) are out of scope.
- **The `rust_analyzer` opt-in SCIP backend self-declares its skip reason** (`"rust-analyzer backend not enabled"` when the gate is off; `"rust-analyzer backend produced no output"` on a WI-nohah fall-through) instead of returning a bare empty result, so it is recorded honestly rather than mislabelled `"no files matched"`.
- **The opt-in rust-analyzer SCIP backend maps edge endpoints to hypergumbo symbol ids.** It previously translated edges without a resolve map, so every SCIP edge was dangling and the endpoint-integrity finalize silently dropped the entire SCIP call graph; endpoints now map to real symbol ids (survey output 0 → 739 edges for backend users).

#### Compact / tiered views no longer collapse to ~1 symbol at small budgets (WI-pohuf)

- **The `compact` view and token-budget tiers (`survey.4k.json` / `.16k.json` / `.64k.json`) now emit a slim per-node projection.** They previously serialized the full ~24-field survey node (identity hashes `stable_id`/`shape_id`/`fingerprint`, provenance `origin`/`origin_run_id`, the `supply_chain` block, `meta`, and other internals) at ~250 tokens each, so at a 4k or 16k budget the post-selection shrink loop trimmed the map down to a single symbol. The new `compact_node` projection keeps only the fields a consumer needs to navigate a symbol — `id`, `name`, `qualified_name`, `kind`, `language`, `path`, `span`, `signature`, `docstring` (plus the annotated `centrality`) — dropping the identity/provenance/supply-chain internals that live in the full survey.
- **Token-budget tiers now re-project `features[]` onto the retained node set** (mirroring the compact path) instead of copying the full `features[]` wholesale. That field was frequently the single largest part of a tier, and because the shrink loop only removes nodes it kept the map over budget no matter how many nodes were trimmed; re-projecting it keeps the tier within budget and restores compact/tiered containment monotonicity on large-symbol repos.

#### Python `shape_id` no longer collides across symbol kinds (WI-linon)

- **Structurally-trivial Python symbols of different kinds no longer share a `shape_id`.** The Python shape hash omitted the symbol kind and mis-branched `async def`: a module-level `def f(): pass` and a class method `def m(self): pass` hashed identically, and a docstring-only `async def` shared a `shape_id` with a docstring-only `class`. The hash now folds the symbol kind (`class`/`method`/`function`) and the concrete AST node type into its prefix, discriminating kind and sync-vs-async while still clustering genuine same-kind structural clones. Unblocks the structural-clone / refactoring-lead consumer (WI-vogij), which would otherwise emit false clones.
- **`shape_id_scheme` bumped `hypergumbo-shapeid-v2` → `hypergumbo-shapeid-v3`.** Every Python `shape_id` value changed, so the global scheme identifier bumps per the spec §6 mandate. `shape_id` is not part of `stable_id`, so identity values are unaffected, and `SCHEMA_VERSION` does not change; the tree-sitter shape path (other languages) is unchanged.
- **C# body-bearing symbols now carry a `shape_id` (WI-lutob, csharp facet).** `CSharpAnalyzer.analyze()` reimplements the extraction pipeline and never ran the base-class shape_id auto-stamp loop, so every C# `method`/`class`/`struct`/`constructor` shipped `shape_id=None` — the structural-clone key silently did not exist for C#. The override now stamps `shape_id` from its already-populated `node_for_symbol` (values are new, so no identity churn — `shape_id` is not part of `stable_id`), broadening `repeat-finder`'s reach to C#.
- **Solidity and WGSL body-bearing symbols now carry a `shape_id` (WI-lutob, closing the item).** Both analyzers never populated `node_for_symbol`, so the base-class auto-stamp loop had nothing to stamp and every Solidity `function`/`constructor`/`contract`/… and WGSL `function`/`struct` shipped `shape_id=None` — invisible to `repeat-finder` (which groups by `(language, shape_id)` and drops None). No shared helper was needed: both are ordinary `TreeSitterAnalyzer` subclasses, so populating `node_for_symbol` at their symbol-construction sites is all that's required — the inherited base loop then stamps `shape_id`. No CORE change and no `shape_id` churn on other languages. Completes `repeat-finder`'s reach across C#/Solidity/WGSL and closes WI-lutob.

#### Python call-graph resolution

- **Inherited-method calls resolve correctly across the class hierarchy.** Python inherited-method dispatch now walks a true C3-linearization MRO (fixing uneven-depth diamonds), resolves cross-file `self.method()` and `self.field.method()` (dependency-injection) calls via enclosing-class and field-type hints, biases to unresolved when a builtin base (`dict`/`list`/…) shadows an in-tree method (ADR-0029), and disambiguates same-short-name classes/fields — closing a large class of dead-code false positives. (Requires path relativization to also rewrite id-embedding values in `Edge.meta` and nested `Symbol.meta`.)
...
- **In-tree submodule reads resolve to the submodule's file node (INV-nuzas category B, WI-tanot).** Reading an in-tree subpackage/submodule as a value (`import pkg; … pkg.sub`) now emits a `references` edge to the submodule's file node instead of a workspace-prefixed `external_symbol` phantom. Within-package reads resolve; cross-*package* submodule reads still phantom (they need a global cross-package module map — a tracked residual).
- **Co-referent module aliases retarget instead of self-shadowing (INV-nuzas / INV-fahub).** A function-local `from pkg import sub as m` naming the SAME in-tree module a sibling scope plain-imports as `m` was added to the INV-fahub shadow set unconditionally, so the read `m.attr` suppressed its own `module_attr_ref` retarget and minted a workspace-prefixed `external_symbol` phantom. The shadow guard now excludes a `from`-import whose absolute (`level == 0`) target equals the same name's `module_imports` binding — a co-referent alias, not a value rebind — so the read resolves via a `references` edge; a genuine value rebind or later reassignment still shadows. Relative co-referent imports stay phantom (a safe miss, never a wrong edge).
- **Scope-shadow gaps in the module-attr / reference retarget closed (WI-luhah, INV-fahub).** The `module_attr_ref` / `references` retargets could mint a confidently-wrong resolved edge when a module symbol's name was rebound in a way the immediate-scope shadow guard missed: an enclosing function's param/local captured by closure in a nested read, and a module-scope rebind the caller left unguarded (a reassignment shadowing an import alias, or an import shadowing a same-named variable). Both gaps are now guarded for both emit sites — the nested-scope shadow unions the full enclosing-function chain's bound names, and the module-scope callers pass a reassignment-target / import-alias set — with comprehension for-targets correctly excluded. All INV-fahub-safe (a missed retarget is a phantom, never a wrong edge); documented pathological residuals remain.
...
- **External/stdlib base classes emit unresolved-external `extends` edges instead of being dropped, for every OO language (WI-jubag Approach C).** The framework-agnostic inheritance-linker chokepoint now represents a base that resolves to no in-tree symbol as an `is_resolved=False` extends edge to an `external_symbol` boundary node (`{lang}:external:0-0:{name}:unresolved`, the INV-nuzas-safe sentinel), evidence-derived confidence 0.95 (ADR-0039). This generalizes the per-analyzer external-base fallbacks (previously Python + JS/TS only) to Kotlin, Ruby, Java, C#, Scala, PHP, Swift, and the rest, and recovers Python dotted/qualified bases (`class C(argparse.ArgumentParser)`) that the Python analyzer defers. A base that names an in-tree symbol not extracted as a class, and a dynamic/garbage base name (Ruby `class Foo < Struct.new(:a)`), are still dropped rather than minted as false externals. An ownership split prevents double-emission: the per-analyzer fallback owns a class's bare external bases, so the chokepoint defers those and adds only the dotted bases plus all external bases for languages with no fallback. (`instantiates` external constructors and retiring the redundant per-analyzer resolvers remain follow-ups.)

#### File classification

- **Dense non-web source files are no longer silently dropped as minified (INV-lukop).** The average-line-length minification heuristic (>150 chars) now fires only for web-asset extensions (JS/CSS/HTML bundles). A dense-but-real source file in another language — a Python data/lookup module, a very long function signature, generated protobuf lacking a `@generated` header — was previously misclassified as minified, tiered derived, and had its whole file (every symbol and edge) dropped by the default tier filter with no diagnostic. The `@generated` / sourcemap / webpack heuristics stay universal.

#### Dependency linking

- **`depends_on_manifest` edges attribute to the importer's own package manifest in monorepos (WI-timon).** When the same dependency name is declared in several package manifests (e.g. `rich` in both hypergumbo-core and hypergumbo-tracker), the lookup was a global flat `name → Symbol` map, so every importer's edge pointed to whichever manifest was processed last. The lookup is now `name → [Symbol]` and each import edge attributes to the nearest enclosing manifest of the importing file; an importer under no candidate manifest falls back deterministically and carries the registered `disambiguation_fallback` flag at confidence 0.5 (INV-zuhub) instead of a confident-but-wrong 0.9.

#### Symbol-kind emission correctness

- **Per-language emission fixes.** Go module-variable emission gates on package scope (no more function-locals); Swift field emission requires a direct type-body parent (SwiftyJSON: most "fields" were method-locals); Julia `const` no longer drops multi-name/typed members or clobbers a same-named function; Elixir `def`s inside `quote` blocks attribute to the `use`-ing module; and Nim now extracts EXPORTED (`*`-marked) procs/funcs/methods/types instead of dropping them.
- **JavaScript/TypeScript coverage.** Generator functions, const-bound function expressions, anonymous callbacks, and IIFEs now emit function symbols (closing dead-code false positives); CommonJS `require()` bindings reach ESM call-graph parity and `.mjs`/`.cjs`/`.mts`/`.cts` are discovered; external/library inheritance edges emit as unresolved-external; and route-handler pseudo-symbols populate `signature`.
...
- **C# and Swift member attribution corrected (DEEP dogfood cohort).** Three per-language fixes surfaced by dogfooding: (1) a **user-typed C# property** was named after its return *type*, not the member — `public WaveInfo Info` emitted the symbol `C.WaveInfo` — because name extraction grabbed the first `identifier` child; it now reads the grammar's `name` field. (This disproves INV-dakit's filed "expression-bodied properties are not emitted" mechanism — every property shape does emit `kind=property`; the real defect was the name.) (2) A cross-file **C# `new X(...)`** landed its `instantiates` edge on the class node rather than the constructor, so a reverse-slice on a constructor found zero callers; it now targets the `Type.Type` constructor, falling back to the class only for implicit default ctors — overloaded ctors currently share one anchor (WI-fagit). (3) **Swift `extension T { … }`** members were demoted to bare file-level symbols — functions lost the `method` kind and all members lost their `T.` name/qualified-name prefix — because the extended type name is wrapped in a `user_type` node the resolver did not read; it now reads the `name` field, recovering the whole public API of extension-heavy libraries (WI-kudir).
- **Swift struct-body subscripts attach to their enclosing type (WI-fokag).** The containment linker's `CONTAINABLE_KINDS` set omitted `subscript`, so a correctly-named struct-body subscript (`JSON.subscript(key:)`) received no `contains` edge and looked like an orphaned member — the WI-kudir residual. `subscript` is now containable. Graph-completeness only: the dead-code BFS does not traverse `contains`, so this does not move the dead-code false-positive rate. (The separate gap where a `json["key"]` subscript-*call* site does not resolve to the subscript node stays open.)
- **Rust enum match-arm dispatch resolves to the concrete impl (WI-kodap).** A local destructured from a tuple-struct enum variant — `match cmd { Cmd::Query(q) => q.run() }`, zoxide's subcommand-dispatch shape — was untyped, so `q.run()` fell to short-name resolution and every arm bound to the same `run`, leaving each concrete handler with 0 incoming calls and stranding the forward slice at the dispatch boundary. The binding now adopts the variant's field type (`q: Query`), so `q.run()` resolves to `Query::run`. File-scoped positional binding also covers `if let` / `while let` / destructuring `let`; builtin-typed fields, unit/struct-like variants, and non-enum tuple patterns are skipped. (Overloaded-arity dispatch remains a follow-up.)
- **Rust chained return-position dispatch resolves against the receiver's type (WI-lohup).** A method call whose receiver is itself a call — `Cmd::parse().run()`, the actual `main` entry in zoxide-style CLIs — typed nothing for the receiver, so `.run()` fell to short-name resolution and mis-bound to a same-named sibling. The outer method now resolves against the receiver call's inferred return type (`Cmd::parse()` yields `Cmd`, so `.run()` → `Cmd::run`), also covering `obj.foo().bar()` when `foo`'s return type is known. Together with WI-kodap this closes the zoxide symptom end-to-end: the forward slice from `main()` now reaches the whole subcommand tree.
...
- **Cold-cache auto-analysis returns the map it just wrote.** Commands that auto-run analysis on a cache miss (`slice`, `search`, `explain`, `routes`, `symbols`) re-derived the freshly written map path via a cache-dir content hash; when the repo changed during the multi-second run (concurrent tracker/CI writes under self-analysis + `smart-test`), that hash drifted and the command failed with `Error: Input file not found: None`. The path `run_behavior_map` actually wrote is now returned directly instead of re-derived (INV-somup).
...
- **Build-wrapper scripts no longer seed forward slices (WI-batit).** Maven/Gradle wrappers (`mvnw`/`gradlew`) are checked-in tier-1 repo-root shell scripts, so they escaped every entrypoint-ranking penalty and their large out-degree boosted their base score to outrank real HTTP controllers, becoming the default `slice --entry auto` (and `dead-code`) seed. They're now demoted below the ranking floor (×0.1 with the connectivity boost skipped, the treatment test-file entrypoints get), so the real API surface wins the auto-seed; a genuine non-wrapper shell script keeps its full ranking.
- **Python route `usage_contexts` carry the `http_method` string per method (WI-kohav).** Python was the lone language emitting a route `UsageContext` with `metadata["methods"]=[list]` instead of the documented `metadata["http_method"]=<string>`, raising `KeyError` for spec-conformant consumers; it now emits one `UsageContext` per method with the string form.
- **WebSocket framework attribution is dependency-gated (WI-fizir).** Starlette WS endpoints (FastAPI shares the syntax) were mis-stamped `server_framework`/`framework_dispatch="fastapi"` when only starlette is a dependency; a dependency check at all four attribution sites now attributes them to `starlette`.
- **Degenerate route-marker twins are excluded from `features[]` (WI-rijop).** `features[]` was double-populated with content-free route-marker twins (`node_count=1, edge_count=0`); markers are now skipped from the `features[]` append (the per-slice file/index is still written).

#### Symbol identity (stable_id / node.id)

- **Doc/markup/template analyzers canonicalize identity.** Eleven analyzers now mint `node.id` and `stable_id` from one shared helper (line-less analyzers gain a `start_line` segment), nine emit canonical `sha256:<16hex>` stable_ids instead of raw composites, and sparql/kdl/graphql/SCIP producers route through canonical factories.
...
- **Container/declaration symbol kinds now carry a `stable_id` (WI-rihob).** The tree-sitter producers construct `class` / `struct` / `enum` / `trait` / `protocol` / `contract` Symbols with a `shape_id=` but no `stable_id=`, so they defaulted to `None` — every TypeScript class and enum, plus the container-kind tail on csharp/rust/swift/solidity/wgsl. The INV-sotiv orchestrator backstop already covered `interface`/`type` but not these six; it now stamps them at the chokepoint via `make_declaration_stable_id` (the same file-scoped `sha256("{kind}:{language}:{path}:{name}")` shape, with `kind` threaded in so a class and a same-named struct stay distinct), so cross-version tracking works for the container kinds. Producer-computed ids (Python classes) keep precedence. Callable kinds that were also null (go `method`, solidity `function`/`constructor`) are deliberately left to their producers' typed stable_id, since a name-only key would collide across overloads.
...
- **Evidence fields descoped to the populated set (ADR-0040).** `Edge.evidence_lang` is now central-stamped at `Edge.create` from the src id's language slot (it was null on ~100% of edges) — catalog-guarded so a non-canonical src yields `None` rather than a bogus stamp, and never overriding a value a producer passed. The dead `evidence_spans` field and the never-implemented `meta.evidence[]` multi-pass accumulator (spec prose only) are removed. No SCHEMA_VERSION bump — `evidence_spans` was never serialized and `evidence_lang` is additive (WI-kuluh, WI-vozar, WI-gijus).
- **`receiver` documented as a per-language fold-residue key (audit-findings 0012).** The `edge.meta.receiver` classification is emitted only by analyzers whose call syntax carries receiver flavor (Ruby/Go/C#/Rust/C++), so it is absent *by design* on corpora lacking those languages — a corpus artifact, not an unpopulated-field defect. Spec §841 and the `MetaKeySpec` entry now state this, enumerate the complete producer vocabulary (`bare`/`external`/`constant_external`/`stdlib`/`typed_field`/`typed_var`/`field_chain`/`generic`) in place of the stale illustrative `typed`, and flag that the key mixes a resolution class with the `field_chain` expression shape — kept as one key (no consumer branches on it) with a written re-evaluation trigger to split into `receiver_class` + `receiver_shape` should they ever be needed independently (WI-jokob, WI-lodaz).
- **Route, kind, and language axes.** Route transport protocol (WS/LIVE/RPC) moves off `http_method` to a `route_protocol` axis (ADR-0031); the route framework is single-homed on `route_framework` and `route_of()` unions it instead of dropping it; proto/thrift/smithy `service`/`rpc` fold to `interface`/`method`; Rails view templates register as CONFIG languages; boundary `Symbol.language` normalizes file-paths and labels to a registered language or `None`; and `all_known_languages()` unions taxonomy languages (asciidoc, makefile).
...
- **Linker `pass_id`s conform to the `-linker` suffix convention (WI-nuduv).** Six Tier-2 linkers emitted an `AnalysisRun.pass_id` / `Symbol.origin` / `Edge.origin` value without the shared `-linker` suffix — `decorator-dispatch`, `di-resolution`, `inherited-calls`, `method-call-recovery`, `type-hierarchy`, and `receiver-type-dispatch`. All six now emit `…-linker` so a consumer can substring-filter linker passes uniformly, and a convention ratchet enforces it going forward (the `view_template` per-framework delegates, which emit the shared `view-template-linker`, are documented exceptions). This shifts those six `origin` strings and invalidates cached artifacts naming them. INV-numat member.
- **`framework_dispatch` documented as one coherent dispatch-convention axis; audit-0014 threshold corrected (INV-junid).** The `framework_dispatch` `edge.meta` key was mischaracterized (by the tracker item and audit-findings 0014) as a "framework name" axis leaking mechanism values. A four-lens review found it is instead ADR-0028 Cluster 28C's deliberate "framework-dispatch convention" fold — one coherent axis whose values are dispatch-convention identifiers (framework-specific compounds like `django_orm` / `kafka_streams` and framework-agnostic mechanisms like `registry_dispatch` / `npm_package`), distinct from `detection_pattern`. The `MetaKeySpec`, spec §9, and audit-findings 0014 now say so, and audit-0014's fabricated "≥30 values / ≥30 producers" threshold is corrected to ADR-0024 rule 3's real **"≥3 values OR ≥2 producers"** (promotion to a typed `Edge.framework` stays declined per ADR-0028 OQ2; zero production consumers read the key). Docs/registry only — no producer or output change. Resolves INV-junid, draining the last open child of the INV-numat vocabulary-axis umbrella.
...
- **Workspace-sibling _dependency declarations_ are tier 2 `internal_dep`, not tier 3 external (INV-nuzas / ADR-0041 D8a).** The prior bullet fixed the imported-boundary-node facet; this fixes its complement — the `kind="dependency"` symbols. Every dependency declaration was stamped tier-3 external unconditionally, so a monorepo sibling that another workspace package lists as a dependency was mis-tiered external, corrupting tier-weighted views (centrality, sketch `first_party_priority`) that treat external as low-relevance. In-repo package distribution names (PEP 503-normalized, read from every `pyproject.toml`) are now recognized and a matching declaration tiers tier-2 `internal_dep`; genuine third-party declarations stay tier 3. Python `pyproject.toml`-scoped; Cargo/npm workspace-sibling analogues are a documented cross-language follow-up.
...
- **Coverage.** Bare `open()` emits a `calls` edge to `builtins.open`; `urllib.request.urlretrieve` is flagged high-risk; JavaScript `caches` / `indexedDB` / `fetch` (including Service-Worker callbacks) reach the io-boundary catalog; and `access_mode` no longer mislabels reads as `write` on assignment lines (collection-mutator calls classify as `mutate`, not `write`).
- **Python stdlib database primitives (WI-harin).** The `db_read` / `db_write` boundary categories — already populated for six other languages — now cover Python's stdlib datastore surface: `sqlite3` (`connect`, cursor `execute` / `fetch*`, connection `commit` / `backup` / `iterdump`), `dbm.open`, and `shelve.open`. The free-function opens (`sqlite3.connect` / `dbm.open` / `shelve.open`) are the reliably-matchable anchors; the DB-API method surface is catalogued but stays latent until receivers are typed. Third-party ORMs (Django ORM, SQLAlchemy) are deliberately excluded — their calls arrive as bare untyped unresolved method calls the matcher correctly refuses (INV-tapat), a receiver-inference problem now addressed for Django by WI-sozoj below.
- **`net_recv` false positive removed (WI-harin).** `django.core.management.execute_from_command_line` was misclassified `net_recv`; it is a CLI dispatcher, not a network receive, and third-party framework code out of scope under the strict-stdlib rule — removed.
- **Django ORM database I/O made visible via type-verified receivers (WI-sozoj).** The receiver-inference follow-up to WI-harin: Django ORM calls now classify as `db_read` / `db_write` instead of vanishing (invisible ORM I/O dominated the low io_tag_rate on Django repos). The Python analyzer types two framework-syntax markers and emits a `django.db.models`-module-qualified `calls` edge — the `<Model>.objects.<method>()` Manager convention (reads `filter`/`get`/`all`/…; Manager-position writes `create`/`bulk_create`/`update`/…) and a `models.Model`-subclass `self.save()`/`self.delete()`. io-boundary's module-filter path then classifies each via a new type-verified python.yaml carve-out. Because the match requires the typed module hint, no `dict.get()`/`.save()` false positive is possible (INV-tapat/INV-maluk preserved), and `db_write`→database-sink / `db_read`→untrusted-source taint wiring follows for free. Deferred: SQLAlchemy, `instance.save()` on a typed local, and transitive `Model` bases.

#### Complexity & introspection

- **Cyclomatic complexity & LOC reach parity.** C/C++/bash callables (plus Solidity, WGSL, CMake functions/macros, and Jupyter code cells) now carry `cyclomatic_complexity` and `lines_of_code` — 67 languages total — with the complexity tables relocated to core (ADR-0033 Phase 4); Ruby CC no longer double-counts `if`/`while`/`for`/`case`.
...
- **Python `Symbol.signature` renders real defaults and a wider display window (WI-hopiz).** The signature was hard-capped at 60 chars (dropping parameters and the return type) and elided every default to `=…`; it now renders real default values and preserves the return type on truncation, decoupled from `stable_id` so identities don't churn.
...
- **Binary content no longer leaks into rendered sketches (WI-pubar).** A file containing NUL bytes is now omitted with a `[binary content omitted]` placeholder at the single content-render chokepoint instead of being embedded verbatim — a cold-cache `sketch -t 4000` previously emitted thousands of raw NUL bytes that choke tokenizers and JSON-wrapping consumers.
- **Removed the consumer-less `sketch_precomputed.vocabulary` cache field (INV-padoz).** It had no reader in any package (`compact` strips `sketch_precomputed` entirely); the field is dropped from the producer and the (`x-internal`) schema, with no `SCHEMA_VERSION` bump.
- **`readme_description` no longer embeds raw `> ` blockquote notes (INV-modor).** The README heuristic skips markdown blockquote callouts (in both the paragraph scan and sentence-completion) instead of concatenating their `> ` prefix into the description, which also removes the mid-blockquote truncation that left the visible text cut mid-sentence.
- **`config_info` license detection aggregates every distinct license (WI-gojuz).** It now scans root *and* monorepo package `LICENSE` files (depth 1-2, excluding vendored/hidden dirs) and reports all distinct licenses instead of stopping at the first root file — so a dual-licensed monorepo no longer collapses to one license.
- **`config_info` name/version/dependency extraction scans monorepo `packages/` manifests (WI-lirub).** The heuristic extractor only scanned flat root/subdir manifests, so a monorepo's sub-package `pyproject.toml` identity (name/version/dependencies) never surfaced; it now walks the `packages/` layout.
...
- **Same-file same-name parents are span-disambiguated (WI-vakuh).** When a file holds two same-named classes, a method could get a provably-false `contains` edge to the wrong one; parent resolution now picks the span-enclosing parent or refuses. (The confidence-inversion face of WI-vakuh is under Changed → Confidence derivation.)
...
- **`limits.skipped_languages` is now populated.** Its setter had zero callers, so the field was always `[]` even when a detected language had no analyzer or its pass crashed. A finalize chokepoint now drains the "detected minus analyzed" difference — e.g. a repo with a `Makefile` but no makefile analyzer reports `skipped_languages: ["makefile"]` (WI-nihir).
- **`limits.not_captured` is documented as a universal static disclaimer.** Its module comment, schema description, and spec entry now state it is a fixed list identical for every repo — the construct categories static analysis never captures *anywhere* — not a per-repo measurement (WI-togop).
- **`limits.analysis_depth` removed.** It was a hardcoded `"syntax_only"` constant — never reassigned, false for the semantic maps hypergumbo produces, and read by no consumer. Removed rather than populated: a single map-level depth scalar cannot honestly represent per-language depth, and "is the call graph semantic?" is answerable from the presence of `calls`/`dispatches_to` edges. No `SCHEMA_VERSION` bump (WI-muzus, WI-zusok).
...
- **`smart-test` scopes per-PR selection and coverage to branch-own changes (WI-kalub).** The reverse-slice baseline was the `last-green-sha` marker, which routinely sat tens of commits stale, inflating the "changed files" set with already-merged work. That caused two failures: the scoped coverage gate false-red-ed on a genuinely-green branch (`coverage report --fail-under=100` exit 2) and conflated coverage.py's "No data to report" (exit 1) with a real <100%; and the inflated changed set expanded the slice to ~the entire suite, erasing the smart-test speedup. `get_baseline` now returns `merge-base(HEAD, <authoritative-dev>)` (failover-aware), the scoped gate distinguishes "No data" from a real <100%, the pass/fail re-derivation word-bounds its `failed` match, and the scoped-coverage verdict is written into `.ci/pytest-output.log`. Whole-codebase coverage is enforced separately by `nightly.yml`. See ADR-0011.
- **Full-suite gains whole-codebase coverage teeth (WI-kalub Step 3).** `full-suite.yml` ran each package's full suite with `--cov` but only *scraped* the total for the badge/marker — the job passed regardless, so the blocking path enforced 100% only on changed files and a cross-cutting regression on an unchanged file rested entirely on `nightly.yml`. Each per-package job now fails unless its whole-codebase coverage is exactly 100% (`[ "$COV" = "100" ]`), and the per-PR `ci.yml` scoped gate also gains the "No data to report" ≠ `<100%` distinction. Pre-verified no current coverage debt.
- **`check-package-coverage` now checks all six packages (WI-kalub Step 4).** The tool declared `rust-analyzer` in its `PACKAGES` map but omitted it from the default target list and the arg parser, so a bare run silently skipped one of the six packages (and `check-package-coverage rust-analyzer` errored `Unknown argument`). Both are fixed.
- **Stop-the-line and full-suite/nightly commit-status calls are failover-aware (WI-kalub Step 2).** Stop-the-line (`ci.yml`) reads the Forgejo `Full Test Suite / aggregate` status to decide whether full-suite is broken, but that read — plus full-suite's status write, its "SHA already tested → skip" read, and nightly's status write — hardcoded `https://codeberg.org/api/v1`. Under CI failover the workflows run on the self-hosted Forgejo, so those were dead reads/writes: stop-the-line silently never blocked and full-suite re-ran already-tested SHAs. All four now target `${{ github.server_url }}/api/v1`, so a whole-codebase coverage regression can also *block* PRs under failover. (`release*.yml` codeberg references are intentionally untouched — releases are canonical-on-codeberg and human-gated.)
- **smart-test no longer false-reds (exit 1) on green runs under `FORCE_COLOR` (WI-kalub — the dev-loop symptom).** In a dev shell that exports `FORCE_COLOR`, pytest colorizes even the redirected `.ci/pytest-output.log`, so the summary line begins with an ANSI escape and the anchored `^=+ …` result-line grep failed to match — aborting smart-test with exit 1 *after* the tests passed but *before* the coverage gate, on every run. The captured pytest run now neutralizes `FORCE_COLOR`/`COLORTERM` (aligning the local loop with CI) and the result-line grep is `|| true`-guarded. CI was never affected.
- **smart-test honors coverage-only test dependencies (WI-zaziz).** smart-test selects tests via `hypergumbo slice --files` — an *import*-dependency reverse slice — so it is blind to a test that *covers* a source file through the analysis pipeline without *importing* it. The concrete case: a PR touching `java.py` selected the java-specific tests but not `test_polyglot_call_site_coverage.py`, the sole cover of java.py's static-import `ExternalRef` branch, so the changed-file 100% coverage gate false-red-ed at 99% even though the full suite was 100%. A checked-in declarative map (`.ci/coverage-only-deps.txt`) of `source → covering-test` pairs, consumed by a new `scripts/coverage_only_deps.py` helper, is now folded into the selection inside smart-test (in both the run and the regenerated manifest). Entries are validated and defensively skipped if a mapped test is missing.
- **`smart-test` filters its reverse-slice input to source files (WI-zufij).** It passed the full changed-file set (docs/, `.ci/`, `CHANGELOG.md`) to `slice --files`, which crashed (`Input file not found: None`) and blocked any PR co-changing source + generated docs; the input is now filtered to `packages/*/src/**` and the slice is skipped on empty input.
