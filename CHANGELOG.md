...
- **Forge tooling gains a dormant GitHub backend (Codeberg → GitHub + Woodpecker migration).** `scripts/lib/forgejo-api.sh` becomes forge-aware: a new `detect_forge_backend` sets `FORGE_BACKEND` from the origin host (`github.com` → `github`, else the default `forgejo`; overridable via `HYPERGUMBO_FORGE_BACKEND`, forced `forgejo` under CI failover), and a new `resolve_forge_token` centralizes the failover-aware credential. Since Gitea copied GitHub's REST API, ~85% of the lib is already shape-compatible; the genuine divergences are gated on the backend — `api_call` adds GitHub's `Accept`/`X-GitHub-Api-Version` headers, `detect_api_base` resolves `api.github.com`, `find_merged_pr` reads `merged_at`, and `do_merge` / `fetch_job_log` early-dispatch to a new `scripts/lib/github-api.sh` (`PUT {merge_method: rebase→merge}` merge; a Woodpecker-URL degrade for logs behind Cloudflare Access). `poll_ci`'s per-element status reads are normalized to tolerate GitHub's `state` key **and** Forgejo's `status` key — which also un-breaks the Forgejo Scenario-A sole-holdout path that had been latently reading the wrong key (the one intentional, disclosed Forgejo behavior change); and the pulls-list lookups (`find_open_pr` / `find_merged_pr` / `find_open_tracker_sync_prs`) send GitHub-valid `sort=updated&direction=desc&per_page=100` on the github backend instead of Forgejo's ignored `sort=recentupdate&limit=50`. Everything stays dormant while Codeberg is origin; the 7 sourcing scripts inherit the backend unchanged. The Forgejo path is otherwise behavior-preserving (137 existing bash subprocess tests pass); 12 new GitHub-backend Tier-B tests.
- **The forge tooling's per-script write paths become backend-aware (dormant GitHub arms).** Building on the lib backend, the maintainer/contributor scripts gain GitHub write arms gated on `FORGE_BACKEND`, all inert while Codeberg is origin. `auto-pr` replaces its five AGit `refs/for` pushes with a plain-branch-push + `create_pr` helper (`_autopr_github_push_create`), branches the WI-miriz PR-URL sentinel to `/pull/N` (`_autopr_pr_web_url`), reopens the just-closed PR instead of AGit-re-pushing on the timeout/hung CI-recovery paths, plain-`--force-with-lease`-pushes the rebased head, and — because GitHub rebase-merge rewrites the commit SHA — re-captures the true post-merge dev tip from `origin/<base>` for the sentinel + WI-mokak tracker discussion at **both** the `do_pr` and `flush_queue` merge sites (and consumes `FALLBACK_PR_NUM` on the flush github path so the empty-`PUSH_OUTPUT` `parse_pr_from_push` can't abort under `set -o pipefail`). `contribute` gains a `gh pr create` arm (auto-fork; a shape-compatible `api_post` fallback when `gh` is absent) with the existence check filtered by the bare `--head` branch + head-repo owner, its long-dead create block revived (a `set -u` crash on the unset `API_BASE` fixed), and its hardcoded codeberg URLs generalized off the upstream host. `ci-debug runs` degrades to rendering the Woodpecker commit-status as pseudo-runs (GitHub Actions is disabled, so `/actions/*` is empty); `list-my-prs` sends GitHub pulls-list params and reads the `merged_at` marker; and `resolve_forge_token` sources a dedicated GitHub PAT (`HG_GITHUB_TOKEN`, provisioned in a later PR). `verify-tracker-pr` needs no change (GitHub's `/contents` + `/pulls/{n}/files` are already shape-compatible — a regression guard locks that in). The Forgejo path stays behavior-preserving (full `auto-pr`/`merge-pr`/`ci-status` bash suites pass); new Tier-B tests force `HYPERGUMBO_FORGE_BACKEND=github` (satisfying the C13 rot-guard in the normal suite; the dedicated forced-backend CI job lands with `.woodpecker.yml` at cutover).
- **The GitHub maintainer PAT (`HG_GITHUB_TOKEN`) is documented and governance-sanctioned.** Completes the dormant forge-tooling credential contract: `.env.template` documents the dedicated local PAT the forge scripts' `resolve_forge_token` reads on the github backend (falling back to `FORGEJO_TOKEN` while Codeberg is origin, so it stays blank-safe today), and AGENTS.md's §Secrets exception list is amended to permit it alongside `FORGEJO_TOKEN`. A rot-guard test locks the code↔doc contract (the lib reads it ⇔ the template + §Secrets name it). No behavior change — the token is inert until cutover re-points origin to github.com.
- **CODEOWNERS migrated to GitHub gitignore-glob syntax + full governance coverage (Codeberg → GitHub migration, PR-E).** The root `CODEOWNERS` review gate is rewritten from Forgejo/Gitea regex (`\.githooks/.*`) to GitHub globs (`/.githooks/`) and expanded from 5 entries to the complete AGENTS.md §Governance Files surface (adds `AGENTS.md`, `ALLOWED_WEBSITES.md`, `AUTONOMOUS_MODE.txt.default`, `.agent/`, `merge-pr`, `contribute`, `ci-debug`, `ci-failover`, `scripts/lib/forgejo-api.sh`), all owned by `@jgstern`. A rot-guard test (`tests/test_codeowners_governance.py`) keeps CODEOWNERS in sync with AGENTS.md's authoritative list and forbids a regex regression. While Codeberg remains origin, code-owner enforcement there is backstopped by the human's manual governance-PR merges; the GitHub-side `main` branch protection (require only the `ci/woodpecker/pr/woodpecker` status context) is pre-staged separately as repo-admin config.
- **Woodpecker CI lands in canonical `dev` as a `.woodpecker/` folder (Codeberg → GitHub migration).** The container-based (`python:3.11-bookworm`) port of the GitHub Actions CI, **active on GitHub + self-hosted Woodpecker and inert on Codeberg/Forgejo** (which reads `.github/workflows/**`, not `.woodpecker/`), so it lands dormant while Codeberg is origin and does not reach GitHub until a mirror refresh / cutover. Three workflows: **`woodpecker.yml`** — the Phase-1-validated per-PR gate (lint / mypy / audit / verify-generated / hook-tests / build-grammars / pytest, preserving the committed `.ci/affected-tests.txt` smart-test contract; its stem keeps the `ci/woodpecker/pr/woodpecker` status context branch protection marks required) plus a new `forge-arms` job running the top-level bash-subprocess forge github-arm + governance rot-guard tests under forced `HYPERGUMBO_FORGE_BACKEND=github` (the deferred **C13** rot-guard, since smart-test's import-slice cannot select bash tests); **`full-suite.yml`** — whole-codebase `--cov-fail-under=100` + the WI-jigup self-tree-validation ratchet + the top-level `pytest tests/` suite, on a `full-suite` cron; **`nightly.yml`** — the py3.10–3.13 `matrix` + `integration-test --quick`, on a `nightly` cron (required before the Phase-4 release path). Woodpecker's single-file-vs-folder exclusivity forces the folder (and nightly needs its own matrix); the Forgejo/Codeberg-runner-cap jobs (`*-retry`, `changes`/`prep`/`aggregate`, the Forgejo commit-status `report`, GHA `Install Node.js`) are dropped. The per-PR gate was Phase-1-validated by a real Woodpecker run; the two siblings are new authoring (individual legs Phase-0-smoke-tested) that get their first full Woodpecker run — and their context names / cron bindings verified — at cutover.
...
- **ADR-0041 tier-axis follow-through: vestigial config removed, stale `(tier 2)` annotations corrected (WI-josad, INV-higop).** The `SupplyChainConfig.analysis_tiers` field — round-tripped through `to_dict`/`from_dict` but never read (its only consumer, `should_extract_symbols`, existed solely as ADR-0004 pseudocode and was never built; symbol extraction gates on Tiers 1-2 + the ANALYZABLE role via file classification) — was deleted. Four stale tier-vs-role annotations that survived the ADR-0041 tier redefinition are corrected to match the code: the `DependencyManifest` boundary-node docstring summary (all external refs are tier 3 `EXTERNAL_DEP`, direct/transitive lives on the `directness` meta key — it was contradicting its own body), the `DOCUMENTATION_PATTERNS`/`FUZZ_BENCH_PATTERNS` headers (both classify as first-party tier 1 per INV-naduh, not tier 2), the `by_supply_chain_tier` `edges`-view comment (only third-party tier 3 is a graph sink — tier 2 is now internal/project-side, a source that can carry out-edges), and the taxonomy + spec symbol-extraction rows (`analysis_tiers` → `Tiers 1-2`).
...
- **Spec reproducibility claims corrected to the honest L2 posture (WI-pifaj).** Three sites overclaimed byte-level reproducibility, contradicting the spec's own `reproducibility_context` L2 disclaimer: the `analysis_runs[]` array was called "byte-stable across runs", and the UX key-principle + verification checklist claimed "deterministic and reproducible" / "same input → same output". A two-run survey confirms the array is not byte-stable — every entry stamps a fresh `execution_id` (`uuid:`) + wall-clock `started_at`/`duration_ms`, and because the sort key is wall-clock, even the pass *ordering* varies run-to-run (WI-haguz removed the old random dict-order but did not make the order reproducible). All three sites now scope the guarantee to the L2 semantic graph (nodes / edges / `stable_id`s / `run_signature`), which does reproduce. Byte-level reproducibility remains a separate opt-in (WI-sazap `--reproducible`).

### Fixed

- **`supply_chain_tier` stops carrying the "don't reclassify" mechanism (INV-bonup, ADR-0041 §1).** Six protocol linkers (Tauri IPC, wasm-bindgen, message-dispatch, `@hg:` annotations, Yjs CRDT, Solidity ABI) stamped `supply_chain_tier=2` on their synthetic stand-in nodes purely to trip the `_classify_symbols` skip-guard (which bypassed any symbol with `tier != 1`) — encoding a *mechanism* ("this is a synthetic placeholder, don't reclassify it by host-file path") in a field ADR-0041 §1 reserves for supply-chain **distance** only, and mislabeling those nodes `internal_dep`. The skip now keys on each node's `protocol_origin` field (the canonical ADR-0031 synthetic-stand-in marker, already so-treated at the read boundary), so all 14 tier-2 stamps are dropped and the nodes carry their honest first-party (tier-1) distance while still being skipped from host-path reclassification. `supply_chain_summary` per-tier `symbols` counts shift accordingly (the synthetic nodes are non-`file`, so per-tier `files` counts are unaffected).

#### Migration — first-real-Woodpecker-run red tests (Codeberg → GitHub)

- **RCT public-API pin + `connectivity` docs reconciled with the D12 default flip.** The `run_survey` / `run_behavior_map` `connectivity` default was intentionally flipped `True`→`False` in an earlier change (D12: centrality-ranked default; `--connectivity` opt-in), but three docstrings/help strings still claimed the old `True` default — including a `compact` example referencing a now-removed `--no-connectivity` flag — and the RCT public-API pin still asserted `True`. The mismatch went permanently red the first time real GitHub/Woodpecker CI ran the full suite (Codeberg's dead CI had hidden it). The docs now state the centrality-ranked default, and the pin stops asserting `connectivity`'s default *value* (a product knob the RCT overrides by passing `connectivity=True` explicitly) while keeping the parameter-presence assertion the RCT actually depends on.
- **The per-PR Woodpecker gate's `pytest` step installs `jq`.** Its smart-test manifest can select top-level agent-infra `tests/` (e.g. `test_session_start_agent_notes.py`) whose bash hooks shell out to `jq`; without it, `session_start_logic.sh`'s `command -v jq` guard skips the notes prompt and five "prompt fires" assertions fail in the clean container. `jq` is now installed as the sibling `hook-tests` / `forge-arms` steps already do (full-suite's `test-agent-infra` already had it).
- **`generate-architecture --check` is rebase-stable.** verify-generated's freshness gate required the doc's recorded HEAD SHA to be within the last 15 commits — but GitHub's rebase-merge *rewrites* the merged commits' SHAs, so the SHA recorded when ARCHITECTURE.md was regenerated on a feature branch is orphaned on `dev` the moment the PR merges, red-lighting the *next* code PR's gate (it did, twice, during a rapid PR run). The check now records `commit_count` (`git rev-list --count HEAD`) — the ancestor count survives rebase — and passes when the doc is within the window by commit distance (`current − recorded ≤ 15`); the legacy SHA-membership path stays as a fallback for docs generated before the field.
- **Tracker auto-sync's Woodpecker CI slimmed to a single `tracker-validate` step, unblocking auto-sync.** The retired Forgejo `tracker-ci.yml` was a dedicated lightweight job (~15s); the Woodpecker port folded tracker validation into the general per-PR gate but left `hook-tests` + `prepare-git` (always-on) and `dco` (all PRs) running for a tracker-ops-only PR — four container steps that outran `do_sync`'s CI-poll timeout, so auto-sync PRs timed out and tracker ops couldn't reach GitHub `dev`. Those three steps now carry a `non_tracker_paths` path filter (exclude `.agent/tracker/**` + `.agent/tracker-workspace/**`), so a tracker-sync PR runs only `tracker-validate`. Skipping `dco` for tracker PRs is safe — `do_sync` already writes a `Signed-off-by` trailer (`sync.py`), verified on the pushed commit.
