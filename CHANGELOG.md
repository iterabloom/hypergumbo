...
- **Shared SymbolResolver framework:** Cross-file symbol resolution now uses centralized resolver
  classes (`symbol_resolution.py`) instead of per-analyzer implementations. Three resolver classes
  support different registry formats:
  - `SymbolResolver`: For `dict[tuple[str, str], Symbol]` (Python's module-keyed registry)
