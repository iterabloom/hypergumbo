...
- **The four `pending_classification` `Edge.edge_type` values are pruned from the registry — the axis is now empty (WI-sumik / WI-pusuv Option B, audit-findings 0016 RESOLVED, ADR-0023 Phase 4b′).** After Batches A/B producer-migrated `resolver_implements` / `resolver_for_type` / `openapi_implements` / `implements_rpc` to canonical `implements` / `references` + `meta`, this consolidated prune removes their dead registry entries. `Edge.edge_type` now carries a **single `relationship` axis** (25 values; both `endpoint_shape` and `pending_classification` drained to empty). The `AXIS_PENDING` constant is retained for a future edge type that lands pending its per-family audit (ADR-0023 §5), but no value occupies it. **`SCHEMA_VERSION` 0.17.0 → 0.18.0** (minor, per the ADR's Phase-4b removal contract); a behavior map produced before the migration that still carries these edge types will no longer validate against the enum. Regenerated `docs/schema.json` + `docs/concept-axes.md`; audit-findings 0016 flipped to **All RESOLVED**. This drains the last blocker on the WI-pusuv access_mode census.
- **`access_mode` applicability census COMPLETE — every `Edge.edge_type` value is now classified applicable-XOR-N/A (WI-pusuv, closing INV-tibob's tail).** With both non-relationship axes drained, the `access_mode` applicability matrix (`axis_meta_keys.py`, ADR-0038 ruling 2) now covers the whole 25-value registry: the residual relationship tail is declared **N/A-now** — `subprocess_calls` (a subprocess invocation, with a re-eval trigger to promote it to *applicable* once an emitter stamps `access_mode` on those edges), `data_flows_to` (its direction lives in the `data_direction` key, ADR-0038 ruling 3 — not state access), and the structural `constrains` / `defines_target` / `includes` / `links` / `module_exports` / `sources` / `wraps`. A consumer reading `access_mode=None` can now *always* distinguish "the question does not arise" (an N/A type) from "missing data on an applicable type" — the interpretability gap INV-tibob raised, now closed for the full vocabulary and locked by a total-census property test. No `SCHEMA_VERSION` change (the applicable set — the only types that ever carry the key — is unchanged; this classifies the remainder as intended-absent).
