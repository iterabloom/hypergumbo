...
- **`SCHEMA_VERSION` 0.6.0 → 0.7.0 — `Edge.evidence_type` endpoint_shape closure.** All 111 endpoint_shape values are removed from `EVIDENCE_TYPES` after Phase 3 producer migration completed across Waves 3-5 and per-cluster bakeoff validation cleared:
  - 18 resolution-status leaks (`*_unresolved`) → canonical inference label + `Edge.is_resolved=False` (audit-findings 0008, WI-nunal Wave 3).
  - 65 framework-dispatch values → canonical inference + `meta["framework_dispatch"]` / `meta["detection_pattern"]` (audit-findings 0014, WI-kagik Wave 5).
  - 28 call-construct peers → apex `ast_call` + `meta["call_construct"]` / `meta["receiver"]` / `meta["resolution_quality"]` (audit-findings 0012, WI-nibis Wave 4).

  `AXIS_ENDPOINT_SHAPE` is retained as a back-compat import target (no longer in `VALID_AXES`); `generate-concept-axes` drops the now-empty evidence_type endpoint_shape section. 91 audit-findings rows advance PRELIM_RESOLVED → RESOLVED. Closes WI-porim / WI-runod Wave 8 — the ADR-0027 + ADR-0028 cross-axis Phase 3+4 program is complete.
...
- **`SCHEMA_VERSION` 0.5.8 → 0.6.0 — `Symbol.kind` endpoint_shape closure.** All 71 endpoint_shape values are removed from `SYMBOL_KINDS` after Phase 3 producer migration completed across Waves 3-6:
  ...
  Consumer-side cleanup: `selection/filters.py` adds `EXCLUDED_FRAMEWORK_ROLES` and trims `EXCLUDED_KINDS`; `linkers/registry.py::SYNTHETIC_KINDS` renamed to `SYNTHETIC_FRAMEWORK_ROLES`; containment / dependency / noise / sketch sets purged of removed kinds. 72 audit-findings rows advance PRELIM_RESOLVED → RESOLVED. Closes WI-butol / WI-runod Wave 8.

### Added

#### INV-zuhub item 1 conformance — `disambiguation_fallback` discipline per linker

Eleven linker slices adopt the contract `confidence ≤ 0.5` + `meta["disambiguation_fallback"]=True` when a simple-name resolution lands on a structurally ambiguous target. Combined they close INV-zuhub item 1 except for the two named follow-ons (WI-husor JNI dual-key, WI-sifiz PyFFI three-key). All sites are written in the canonical IfExp / mutation-form shape that the L4 walker accepts.

- **Go `interface_dispatch` short-name resolution (WI-kogid).** Collects all interface-method candidates rather than breaking on the first; single match stays at `confidence=0.75`, multi-candidate drops to fallback. +2 property tests.
- **`route_handler.py` primary index (WI-rusuh).** Builds a `name_collisions` frozenset of names (and `meta["qualified_name"]` keys) shared by 2+ non-route symbols; main `dispatches_to` and React Router loader/action edges drop to fallback on hit. +5 property tests covering Django / Express / Rails / React Router.
- **`grpc.py` service-symbol lookup (WI-patiz).** `service_sym_by_name` and `service_by_normalized` become multi-value; `grpc_service` Symbols carry `meta["proto_package"]` and the routes_to lookup uses it as a precision disambiguator. Bridge `dispatches_to` (Go servicer stubs, Python servicer classes) has no impl-side package metadata, so multi-candidate matches always drop. +5 property tests.
- **`subprocess_cli` protocol linker (WI-jifiv).** `command_by_name` becomes multi-value; multi-`[project.scripts]` repos where two binaries each register a `list` subcommand resolve to deterministic-by-id fallback. +2 property tests.
- **`method_call_recovery` chained-method recovery (WI-ruzin PR2).** When multiple class hints each contain a method matching the unresolved short name, the line-proximity tiebreaker drops to `confidence=min(base, 0.5)` + flag. +2 property tests.
- **Simple FFI-bridge `calls` (WI-ruzin PR1).** `cgo`, `napi`, `lua_ffi`, `ruby_ffi` switch their bare-name C/C++ lookups to `dict[str, list[Symbol]]`; cross-file collisions emit deterministic-by-id with the fallback flag. +8 property tests across the four bridges.
- **Framework-base-class `dispatches_to` (WI-bojok PR3).** Two new shared helpers at `hypergumbo_core.linkers._transitive_bases`: `build_short_name_collisions` and `short_name_fallback` capture the precision-via-FQN-prefix contract once for every dispatch linker. `django_orm_dispatch` (`django.` prefix) and `airflow_framework_dispatch` (`airflow.` prefix) adopt the helpers and switch to a 3-tuple `(class_sym, methods, is_fallback)` return. +14 tests (7 helper, 4 django, 3 airflow).
- **`jackson_dispatch` + `go_cobra` (WI-bojok PR4).** Jackson stays precision on `org.springframework.` / `jakarta.persistence.` / `javax.persistence.` qualifiers; unqualified bean-marker matches that collide with in-tree JVM types drop to fallback (mixed paths resolve to precision). `go_cobra` emits the entire candidate batch as fallback when `find_symbols_by_name` returns >1. +6 property tests.
- **Emit-all-matches `dispatches_to` (WI-bojok PR2).** `annotation_convention` (`@hg:dispatches X`) and `di_resolution._create_di_edges` (Java/Kotlin/Scala interface→impl overloads) drop the whole candidate batch when the short name has multiple matches. +6 property tests.
- **`kafka_streams_dispatch` (WI-bojok PR1).** The named concrete violation in INV-zuhub's statement table. Class transitive-base matches against `KAFKA_STREAMS_CALLBACKS` stay precision when the raw entry is `org.apache.kafka.*`-qualified or when no in-tree collision exists; unqualified raw entries colliding with an in-tree JVM type (oauthbearer-side `Transformer<T>` etc.) drop. +8 tests.
- **`references` edge-type family — 4 linkers (WI-harim).** `orm` (model lookup), `database_query` (SQL table lookup), `di_resolution._create_di_registers_edges` (NestJS providers), `react_component` (JSX use sites) switch to multi-value with the same-file-preferred / deterministic-by-id cascade established for `inheritance` in PR #3545. +12 property tests (3 each).
...
- **Pre-commit wiring for the L4 gate (WI-fikib).** `.githooks/pre-commit` runs `scripts/check-fallback-coherence` whenever a `linkers/*.py` file is staged — faster-feedback parallel to the L1 / L3 hook blocks. Closes INV-zuhub item 3.
