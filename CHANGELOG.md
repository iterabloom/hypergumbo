...
#### Rust analyzer backend (WI-mafut / WI-duzul)

- **SCIP symbol-string descriptor parser** (WI-mafut Phase 1): new `hypergumbo_core.scip.descriptor` parses the Sourcegraph `<scheme> <manager> <package> <version> <descriptor>+` chain used by rust-analyzer, scip-python, scip-typescript, scip-java, scip-clang. Returns a frozen `ScipSymbol` with header fields + descriptor tuple; handles backtick quoting, method disambiguators, the WI-zakub trait-dispatch shape, local symbols. No protobuf dependency at this phase.
- **Vendored SCIP protobuf binding** (WI-mafut Phase 2 Slice A): new `scripts/build-scip-proto` regenerates `hypergumbo_core/scip/_generated/scip_pb2.py` from Sourcegraph `scip.proto` at pinned SHA `9330cbd49aeb85aee026842770a61ad28e5c4093`. Developer-run (not CI-run); generated module checked in so end users pay no install-time toolchain cost. Adds `protobuf~=6.33` to hypergumbo-core.
- **SCIP Index → Symbol translator** (Phase 2 Slice B): `scip.index.scip_index_to_symbols` walks a parsed SCIP `Index` and emits hypergumbo `Symbol` objects with `origin="scip"` and `stable_id` preserving the raw SCIP symbol string. Local symbols preserved with `kind="local"`; malformed input skipped silently.
- **SCIP Index → Edge translator** (Phase 2 Slice C): `scip.edges.scip_index_to_edges` emits Edges from `SymbolInformation.relationships` — `is_implementation` → `implements`, `is_type_definition` → `has_type`, `is_reference` → `references`, `is_definition` → `defined_by`. Self-relationships dropped; Occurrence-level refs are Slice D's job.
- **SCIP Occurrence-ref → call-edge translator** (Phase 2 Slice D, WI-kopav): `scip.calls.scip_index_to_call_edges` attributes every non-`Definition` Occurrence to its enclosing `Definition` via innermost-span containment, emitting `references` edges with `symbol_roles` preserved for downstream specialization.
- **`hypergumbo-lang-rust-analyzer` package skeleton + SCIP→IR translate surface** (WI-duzul Slice A): new optional package sitting on top of the core SCIP shim. `translate_scip_to_hg(scip_bytes, source_reader) -> (symbols, edges)` parses a serialized Index and post-processes Rust function Symbols by rewriting their `stable_id` through the `rust.py` parity helper so cross-pass dedup works.
- **`rust-analyzer scip` shell-out wrapper** (WI-duzul Slice B-first): `invoke.run_rust_analyzer_scip(workspace)` returns the emitted `index.scip` bytes. Three dedicated exceptions — `RustAnalyzerNotInstalled`, `RustAnalyzerInvocationFailed`, `RustAnalyzerNoOutput` — discriminate failure modes. Default timeout 600 s per WI-zakub §4.
- **`hypergumbo install-rust-analyzer` / `uninstall-rust-analyzer`** (WI-dotud): new `rust_analyzer_install` module + CLI subcommands mirroring `install-gitleaks` / `install-embeddings`. Rustup-only in first cut; static-binary-download fallback deferred.
- **`install-extras` / `uninstall-extras` umbrella subcommands** (WI-huham): single-call installer over every optional component (gitleaks, embeddings, rust-analyzer) with `--check` status table and `--skip NAME,...` exclusion. A failure in one installer doesn't abort the rest.
- **Graceful-degrade orchestrator** (WI-nohah): `try_analyze_with_rust_analyzer(workspace, source_reader)` returns `(symbols, edges)` on the happy path, `None` on binary-missing / invocation-failure / no-output / malformed-SCIP. Fall-through messages deduplicated per `(exception_type, workspace)`.
- **Opt-in gate** (WI-duzul Slice C gate): `should_use_rust_analyzer_backend(backend_flag, environ, is_available)` returns True iff the user opted in (via env var or flag) AND the binary is resolvable on PATH. Silent False when opted-in-but-binary-missing.
- **Registered analyzer** (WI-duzul Slice C-final): `analyze_rust_with_scip(repo_root)` registered at priority 45 alongside `rust.py` at 50. Empty result when the opt-in gate is False; SCIP pass when True. Discovered via `hypergumbo.analyzers` entry point.
- **`--backend rust-analyzer` CLI flag** (WI-vozof): new root-parser flag with choices `tree-sitter` / `rust-analyzer`. Sets `HYPERGUMBO_RUST_ANALYZER=1`; stripped from argv before subcommand parsing so it works in any position.
- **SCIP → `rust.py` `stable_id` parity helper** (WI-bajuz, ADR-0014 §3): `hypergumbo_lang_mainstream.rust_scip.compute_rust_stable_id_from_source(source, start_line, end_line)` returns the same stable_id `rust.py` would assign, so tree-sitter + SCIP symbols dedup under a single identity. Graceful-degrades to `None` when tree-sitter-rust is unavailable.

#### Linkers (Framework subcategory)

- **Controller-routes linker** (WI-gudob Phase 2, WI-gokop): `contains_routes` edges from `concept: controller` classes to nested `concept: route` handler methods. Covers decorator-based families — NestJS, Spring Boot, ASP.NET, Laravel, Symfony, Phoenix, Micronaut, Ktor, Grails, CakePHP. Rails and Django CBVs without `@api_view` are explicitly out of scope.
- **Router-routes linker** (WI-gudob Phase 3): `registers_routes` edges from `concept: router` symbols to nested `concept: route` registrations. Effective for Phoenix, http4s, http4k, Yesod, giraffe, pedestal, ring-compojure, cowboy, sveltekit/remix/nuxt, vertx, plumber, laminas. Span-nesting picks innermost router; third-highest producer-count inert concept.
- **Rust trait-impl method-dispatch linker Phase 1** (WI-kivut): consumes `implements` edges from `impl Trait for Struct` blocks and fans `dispatches_to` edges from each trait symbol to every concrete method on the implementing struct. Pinned `rust_trait_impl + rust_instruction_descriptor` at ~3004 dead-code-maybe candidates in the 2026-04-11 aggregate-v5 run. Phase 2 (generic-bound / `dyn Trait` call-site resolution) deferred.
- **Django ORM method-dispatch linker** (WI-nosug): `dispatches_to` edges from Django subclasses (`Model`, `Manager`, `QuerySet`, `ModelAdmin`, `ModelForm`, `View`, …) to user-defined overrides of framework-called methods. Per-base method maps prevent spurious cross-base edges. Pinned `python_orm_dispatch` at 2441 dead-code-maybe candidates in aggregate-v5.
- **Jackson / JavaBean serialization dispatch linker** (WI-gupah): `dispatches_to` edges from Java/Kotlin/Scala classes with Jackson / JAX-B / Spring-binding annotations to their bean-convention accessors (`getX`/`setX`/`isX`) and method-level-annotated handlers. Pinned `java_bean_accessor` at 7614 dead-code-maybe candidates (8.3 % of the 92218-candidate pool) in the 2026-04-16 aggregate-v5 run — #1 non-uncategorized category.
- **Airflow framework-dispatch linker** (WI-nutav): `dispatches_to` edges from classes inheriting `BaseOperator`/`BaseHook`/`BaseSensor`/`BaseTrigger`/`BaseSensorOperator` to framework-called lifecycle methods (`execute`, `execute_complete`, `pre_execute`, `post_execute`, `on_kill`, `poke`, `run`, `hook`, `get_conn`, `test_connection`). Python-internal only.
- **Kafka Streams topology-callback dispatch linker** (WI-lisov): `dispatches_to` edges from Java/Kotlin/Scala classes implementing any of 17 Kafka Streams callback interfaces (`ValueMapper`, `Transformer`, `Processor`, `Aggregator`, plus `*Supplier` factory forms) to framework-called methods (`apply`, `transform`, `process`, `get`, `init`, `close`, `test`). Pinned `kafka_streams_internal` at 2386 dead-code candidates. Call-site → impl edges deferred to Phase 2.

#### HTTP linker (cross-language)

- **Elm client detection** (WI-tinip Phase 1): the HTTP linker scans `*.elm` files and emits `http_calls` edges to server-side route handlers. Recognises `Utils.Api.<method>` wrapper style, `Http.get`/`Http.post` record forms, and both orderings of `Http.request`. Closes UAT DQ-09 for Alertmanager Elm → Go linkage.
- **JS/TS backtick template-literal `fetch`/`axios` with module-const folding** (WI-sijoh, follow-up to WI-tinip): folds backtick-quoted URLs against module-scope string constants. Unresolved `${NAME}` slots handled heuristically — leading `${VAR}/` as base URL, middle slot as path-segment parameter `{NAME}`, inline slot truncates. Prefix-match fallback when the literal prefix has ≥ 2 segments.
- **Elm `let`-bound `String.join "/"` URL folding** (WI-rosan): Phase-2 extension covering the indirect `let url = String.join "/" [ apiUrl, "silence", uuid ]` pattern used throughout Alertmanager. First list item dropped as base-URL variable; literals become path segments, identifiers become `{name}` placeholders.

#### Entrypoints (WI-gudob concept → entrypoint mapping)

- **`error_handler` concept → `ERROR_HANDLER` entrypoint** (WI-gudob Phase 1): maps the #1 inert concept (37 framework YAMLs — fastapi, express, django, aspnet, flask, actix, axum, gin, nestjs, rails, laravel, symfony, phoenix, …) to a new `EntrypointKind.ERROR_HANDLER` at confidence 0.95 with framework-aware labelling.
- **`form` concept → `FORM` entrypoint** (WI-gudob Phase 4): maps the #4 inert concept (12 framework YAMLs — Django, Flask-WTF, Laminas, fuelphp, cakephp, laravel, symfony, yii, pyramid, rails, remix, sveltekit, yesod) to `EntrypointKind.FORM` at confidence 0.90. Internal-method dispatch deferred to a per-framework registry follow-up.
- **`serializer` concept → `SERIALIZER` entrypoint** (WI-gudob Phase 5): uniform class-level `base_class` match across 9 frameworks (Django DRF, Flask Marshmallow, grape, laravel, litestar, plumber, pyramid, quart, rails) at confidence 0.90. Per-method dispatch (`to_representation` → field methods) deferred.

#### Behavior map

- **Per-handler forward slices emitted by `run`** (WI-sihok): emits `slice.handler.<METHOD>.<path-sanitized>.json` per detected route handler, using bakeoff-proven parameters (`--exclude-tests --exclude-imports --hub-threshold 100 --max-hops 10 --max-files 200`). Capped at 25 by default; overflow recorded in `slice.handler.index.json`. `--no-handler-slices` / `--max-handler-slices N` control behaviour.
- **Method-call recovery linker for chained `Class().method()`** (WI-gigoz / UAT BUG-14): new linker (priority 35) rewrites `calls→Class` + `unresolved-call(name=foo)` pairs into direct `calls→Class.foo` edges when the class has a `contains` child with that name. Language-agnostic; runs after `containment`.
- **Route materializer dedupes against analyzer-emitted routes** (WI-tizad / UAT DQ-01): pre-populates dedupe set with `(method, path)` pairs from existing `kind="route"` symbols. Fixes Django CBV double-counting on pretix (985 reported, ~500 unique). ANY-method routes excluded from pre-populate so CBV-specific-method variants still emit.
- **Class-level annotations propagate to methods for `--exclude-annotated`** (WI-rumij / UAT DQ-06): `dead-code-maybe --exclude-annotated` now consults the containing class's annotations when a method's own meta is empty. Helps Spring controllers, Django CBVs, any framework registering via class-level decoration.
- **IO-boundary leaf-caller roll-ups** (WI-darad Phase 1): `BoundaryMapEntry` gains `leaf_callers` and `entry_points_per_leaf` fields so shared helpers (e.g. `notify/util.go:request`) no longer collapse disjoint caller chains into a single entry-point union.
- **Gradle / Maven dependency manifest for Java/Kotlin tiers** (WI-duhom): boundary nodes from unresolved JVM imports classified using declared deps from `build.gradle`, `build.gradle.kts`, `pom.xml`. Direct deps → tier 2, unknown → tier 3. New `jvm_deps.py` module parses both Groovy and Kotlin DSL Gradle files plus pom.xml.
- **Framework manifest / JVM-dep scan skips test-fixture directories** (WI-sudug, WI-bukof): `_find_manifest_files` and `parse_gradle/maven_dependencies` skip `testFixtures/`, `testdata/`, `fixtures/`, `tests/`, etc. via `paths.is_test_file`. Fixes detekt reporting `react` as a detected framework from test-fixture `package.json` files.

#### Language support

- **Haskell module exports recognized as dead-code seeds** (WI-buvun / UAT BUG-12): Haskell analyzer parses module headers (`module Foo (publicFn, Type(..)) where`) and marks listed symbols `is_exported=True`. No-export-list modules get the Haskell default. Instances follow their class/type.
- **Yesod framework detection + pattern set** (WI-vabiv / UAT BUG-16): detected from `*.cabal` / `package.yaml` deps; `frameworks/yesod.yaml` covers `mkYesod`/`parseRoutes` quasi-quoter, Warp runner, `Yesod`/`YesodSubsite` typeclasses, and the `<method><Resource>R` handler naming convention.
- **Kotlin extension-function call-site dispatch** (WI-visaz, deferred half of WI-fuhav): `receiver.extFn()` call sites emit `calls` edges to their extension-function definition when receiver type matches. Pass 1 builds `extension_index`; Pass 2 consults as Case 3b after class-method resolution. Generic receivers collapse to base name. Evidence `ast_call_extension` at confidence 0.80.
- **Unresolved-call edges for bare global JS/TS calls** (WI-pinop, follow-up to WI-banaf / WI-vurop): extends the unresolved-call fallback to the no-import case — `console.log()`, `localStorage.setItem()`, `navigator.sendBeacon()`, `window.fetch()`, `Deno.readFile()`, etc. emit `calls→{lang}:{obj}:0-0:{method}:unresolved` when the object is a known browser/runtime global and no import binding shadows it.

#### I/O primitive catalogs

- **TypeScript/JavaScript I/O boundary tracing** (WI-banaf / UAT BUG-09a): JS/TS analyzer emits unresolved-call edges for bare-name calls to named imports (`import { existsSync } from 'node:fs'; existsSync()`). Browser-API entries added to the JavaScript catalog (WebSocket, EventSource, BroadcastChannel, XMLHttpRequest, localStorage / sessionStorage / indexedDB / caches, …). Verified on `create-next-app`: total_io_edges 0 → 27.
- **Unresolved-call edges for TS/JS namespace / default imports** (WI-vurop, follow-up to WI-banaf): extends to `import * as fs from 'node:fs'; fs.readFileSync()` and `import axios from 'axios'; axios.get()`. Verified: apollo-server 7 → 14 boundaries; create-next-app 27 → 35.
- **Java I/O primitive catalog expansion** (WI-sakan / UAT BUG-09c): grew ~136 → 312 primitives. Full JDBC + JPA + Hibernate + Spring Data coverage; logging facades (SLF4J, Log4j 1.x/2.x, Logback, `java.util.logging`); Apache HttpClient 4.x/5.x, Spring WebClient, Unirest, Retrofit, Apache Commons IO. Kotlin inherits via catalog alias.
- **Elixir I/O primitive catalog** (WI-vibur / UAT BUG-09b): new `io_primitives/elixir.yaml` covers stdlib, HTTP clients (HTTPoison, Tesla, Req, Finch, Mint.HTTP*, Erlang `:httpc`), Phoenix/Plug, Ecto/Postgrex/MyXQL/Redix, GenServer/Process/Oban/Task IPC. Added to `_CATALOG_PARENTS` with `erlang` parent.
- **Kotlin I/O primitive catalog** (WI-rujos / UAT BUG-09d): new `io_primitives/kotlin.yaml`. Previously Kotlin was aliased to `java.yaml` verbatim, producing only 1 boundary on detekt. Covers `kotlin.io` File / Path extensions, top-level `println`/`print`, ktor client + server, `android.util.Log`, `kotlin-logging`, Exposed ORM. Kotlin moved from `_CATALOG_ALIASES` to `_CATALOG_PARENTS` so Java parent still provides raw JDK entries.

#### Stop hook & bakeoff validation

- **Stop-hook nudge for `awaits_bakeoff_validation` backlog** (WI-dolil slice 3): `awaits_bakeoff_nudge.py` appends an `## AWAITS_BAKEOFF_VALIDATION BACKLOG` section to the active guidance file when the count of tag-bearing items in a blocking status reaches `threshold` AND the latest DEEP cycle's `state.json` is older than `stale_cycle_hours` (defaults 5 / 72 h). Both knobs under `stop_hook.awaits_bakeoff_validation_nudge` in `.agent/tracker/config.yaml`.
- **`awaits_bakeoff_validation` cross-reference in reflect pipeline** (WI-dolil slice 2): `bakeoff-deep-reflect` loads active tag-bearing items and injects a per-claim section into every repo's prompt. Output schema gains `awaits_bakeoff_validation_verdicts` keyed by `item_id` with `verdict ∈ {moved, no_move, inconclusive}` + evidence. `aggregate_assessments` collects per-item plurality and surfaces it in the run summary.
- **Auto-strip / regression-spawn for `awaits_bakeoff_validation` verdicts** (WI-dolil slice 2b): `bakeoff-deep-reflect aggregate --apply-verdicts` executes tracker mutations implied by slice-2 verdicts — `moved` strips the tag + discuss-notes the cohort; `no_move` spawns a regression sub-item; `tied` / `inconclusive` no-op. Dry-run by default.

#### CI & smart-test

- **`test-agent-infra` full-suite CI job runs the top-level `tests/` directory** (WI-javan): new job in `full-suite.yml` mirroring existing package-job shape, wired into `aggregate`'s `needs` list and `conclusion` calculation as a hard gate. Closes the 4-hour cadence gap where `scripts/agent-supervisor`, `.agent/hooks/_shared/*.py`, and tracker-sync glue previously caught regressions locally at best.
- **Per-PR smart-test coverage for top-level infrastructure** (WI-jozan): follow-up to WI-javan. New `top_level_test_map.py` maps changed top-level source paths to their matching `tests/test_<basename>.py`; `smart-test` folds the output into `AFFECTED_TESTS` so a PR touching only `.agent/hooks/_shared/watched_process.py` now runs `tests/test_watched_process.py` in `ci.yml`.

#### Agent-supervisor (WI-razub family)

- **`scripts/agent-supervisor` daemon + replacement sequence** (WI-rofuv + WI-nusor): Python daemon that monitors reserved-prefix tmux sessions (`hypergumbo-session-*`) and replaces stuck ones with fresh CLIs seeded with `HYPERGUMBO_RESPAWN=1`. Subcommands `run` / `status` / `stop`; single-instance via `fcntl.flock`; state under `~/hypergumbo_lab_notebook/agent-supervisor/`. Decision matrix keyed on pane-byte delta (≥ 15 min unchanged → replace); the WI-sipov heartbeat is telemetry only.
- **Per-session heartbeat helper** (WI-sipov): new `.agent/hooks/_shared/touch_heartbeat.sh` sourced from every per-turn hook, touches `<supervisor-state-dir>/<session_id>.heartbeat` on each tool-use / model-call. Telemetry only; fault-silent on empty session_id / unwritable directory.
- **Respawn-aware session-start hook** (WI-sakod): when the supervisor spawns a fresh CLI via `tmux new-session -e HYPERGUMBO_RESPAWN=1`, vendor session-start hooks branch on the env var to auto-enable autonomous mode per `autonomous_intent.txt` (narrow-write via `loop-toggle --set-session-mode`) and emit a generic seed prompt. Defensive fall-through on intent=OFF / missing / garbage; never writes intent.
- **Vendor Parity for Respawn AGENTS.md section** (WI-batob): authoritative table in `AGENTS.md` documenting per-vendor (Claude Code, Codex CLI, Cursor, Gemini CLI) per-turn hook path, session-start hook path, graceful-exit keystroke, and CLI invocation string. Claude Code's `/quit` verified; the other three marked "unverified — FIXME WI-batob" with a documented verification procedure.
- **Meta-circuit-breaker: chain tracking + no-progress kill switch** (WI-mujuk): classifies every replacement as no-progress (dying pane ≤ 512 bytes) or progress (> 512); tracks chains via `replaces` / `chain_length` / `consecutive_no_progress` in `meta.json`. Auto-pauses after 5 consecutive no-progress failures (time-agnostic). `agent-supervisor resume` clears the sentinel.
- **Non-interactive seed-prompt bootstrap** (follow-up to WI-sakod / WI-razub): after `tmux new-session`, `spawn_fresh` polls `tmux capture-pane` for pane-content stability (250 ms cadence, 32-byte floor, 3 consecutive identical samples, 15 s deadline) then injects `"begin"` via `tmux send-keys` to trigger the first model turn. Vendor-agnostic.
- **Agent-supervisor spawns vendor CLIs in YOLO / bypass-sandbox mode**: `VENDOR_TABLE.cli_invocation` includes per-vendor flags skipping approval prompts — Claude Code `--dangerously-skip-permissions`, Codex `--dangerously-bypass-approvals-and-sandbox`, Cursor `--force`, Gemini `--approval-mode=yolo`. Operators should run the supervisor inside a snapshotted VM.
- **`agent-supervisor stop` no longer ambushes the next `run`**: `cmd_stop` now checks the pid in `supervisor.lock` before writing the stop sentinel. Stale locks are cleaned up; a running supervisor still gets shut down normally.
- **`agent-supervisor debugging-reset-rate-limit` subcommand**: operator escape hatch that zeroes the rolling 24h spawn counter by deleting `rate_limit.json`. Idempotent; logs the intervention in `respawn_log.log`.
- **Intent/mode split in `scripts/loop-toggle`** (WI-pobon): new `autonomous_intent.txt` (gitignored) records project-level intent separately from `AUTONOMOUS_MODE.txt` (current session runtime mode). `--set-intent MODE` / `--set-session-mode MODE` write each independently; bareword form writes both. Stop-hook circuit-breaker trips now use `--set-session-mode off` so a stuck session deactivates itself without suppressing project intent.

### Changed

- **Linker subcategory vocabulary restored** (ADR-0003-ext): the Protocol / Bridge / Framework subcategory taxonomy from ADR-0003 §2.4 is now first-class. Every linker module docstring declares its subcategory; `docs/LINKERS.md` renamed and enumerates all 45 linkers with a Subcategory column. "Infrastructure" added as a fourth subcategory for graph-structural utilities. Prioritisation follows INV-nimuj (false-positive-reduction volume on prospector corpus, not novelty of language pair).
- **Stop hook: process-aware pause replaces 150 s blanket sleep** (WI-varid): `stop_logic.sh` inspects running processes via `pgrep` against a configurable watched-pattern list (`pytest`, `python -m pytest`, `smart-test`, `bash ./scripts/auto-pr`, `bash ./scripts/merge-pr`) and polls every 3 s (1800 s cap) when alive; returns immediately when none. Shell-eval wrappers filtered. Configurable via `stop_hook.watched_*` keys in `.agent/tracker/config.yaml`.
- **Dead-code-prospector categorizer expanded 8 → 46 gap categories** (WI-vupin): `_categorize_candidate` gains language-gated rules (Rust trait impls, Python dunders / Django ORM / Airflow, Go receiver methods / k8s watchers / Cilium eBPF dispatch, Java JavaBean / Kafka Streams / Spring bean, TS/JS React / Redux / Superset / Apollo). Reduces `uncategorized` rate on the WI-tubot 2026-04-11 corpus (92,218 candidates, 11 polyglot repos) from **94.0 % → 43.5 %**, below WI-vupin's 50 % threshold. Convergence observed at the 3pp-per-iteration bar.
- **Behavior map node IDs use repo-relative paths** (WI-hopug, UAT UX-06): `run_behavior_map` calls `_relativize_ir_paths` after `resolve_deferred_symbol_refs` to strip the `str(repo_root) + "/"` prefix from every Symbol id/path, Edge src/dst, and UsageContext path/symbol_ref/id. Paths outside `repo_root` preserved; UsageContext.id recomputed from the relativized path.
- **generate-concepts scans Python source for programmatic concept emitters** (WI-tasab): follow-up to INV-rikis. `scan_producers` optionally takes analyzer source trees and scans them with `_PROGRAMMATIC_PRODUCER_RE` to catch cases like `py.py` emitting `main_guard` from its AST walker. Variable-expression forms deliberately not matched (would double-credit YAML concepts). `main_guard` flips ghost → live; ghost count 1 → 0.
- **generate-concepts detects variable-name and tuple-membership consumer patterns** (INV-rikis): consumer regex now matches any identifier containing `concept` for `==`/`!=` comparisons in either order, plus a two-step membership-test extractor for `concept_type in (...)` / `{...}` / `[...]` and `not in`. 30 concepts flip inert → live (`error_handler`, `websocket_handler`, `middleware`, `liveview`, `graphql_resolver`, `main_function`, …). Coverage moves 7 live / 309 inert / 0 ghost → 37 / 279 / 1.
- **`test-coverage` surfaces per-language false-negative caveats** (WI-hular / UAT DQ-05): text-format runs print a footer documenting the ~20 % recall gap and per-language blind spots (Java/Spring MockMvc, Kotlin PSI, Go YAML reflection, Scala macros, Ruby `described_class`, Python fixtures + `parametrize`, JS/TS `describe.each` + ESM tree-shaking, C# `[Theory]`/Moq proxies). JSON gains a structured `caveats` field.
- **Unified path argument across subcommands** (WI-munuv / UAT UX-01): every subcommand accepts both `hypergumbo <cmd> /path` and `hypergumbo <cmd> --path /path`. Setting both is a user error. New `_add_path_argument()` centralizes the convention.
- **`routes` excludes test-file routes by default** (WI-godos / UAT DQ-02): 14 % of plausible's reported routes were from test files. New default excludes; `--include-tests` opts back in. Legacy `-x/--exclude-tests` preserved as a no-op alias.

### Fixed

#### CI / build system

- **Argparse sentinel test dropped + nightly retry Node.js ordering**: (1) `test_discuss_rejects_ack_thread_before_message` started failing on Python 3.12/3.13 due to argparse's more-aggressive positional backtracking; deleted per the test's self-documented resolution. (2) `test-matrix-retry` in `nightly.yml` and `release.yml` had `actions/download-artifact@v3` before `Install Node.js`, so the retry spun up full-suite on every matrix value when any primary failed; fixed by moving `Install Node.js` first.
- **`hypergumbo-lang-rust-analyzer` wired into nightly / release / full-suite**: only `ci.yml` had the new package in its install lines. Added `-e packages/hypergumbo-lang-rust-analyzer` to three install blocks in `nightly.yml`, four in `release.yml`, plus a dedicated `test-rust-analyzer` job in `full-suite.yml` wired into `aggregate`.
- **Nightly coverage gaps closed** (`scip_pb2` omit + tree-sitter-c-sharp 0.23.5): (1) `scip_pb2.py` coverage omit lived only in `pyproject.toml`, dropped when nightly passes `--cov-config=.coveragerc.no-embeddings`; duplicated the entry. (2) `tree-sitter-c-sharp~=0.23.1` resolves to 0.23.5 which flattened the named-argument grammar. Fixed `csharp.py` named-arg detection and bumped the pin to `~=0.23.5`.
- **`test-agent-infra` full-suite job fixed** (hypothesis + fetch-depth + root-aware heartbeat test): added `hypothesis~=6.100` to the install line; `fetch-depth: 0` on checkout so `test_backfill_training_data_cohort_tags.py` finds v0-window SHAs; and swapped the unwritable-dir precondition in `test_touch_heartbeat.py` from `chmod 0o555` (bypassed by root) to `HEARTBEAT_DIR=<regular_file>/agent-supervisor` so `mkdir -p` fails with `ENOTDIR` regardless of uid.
- **`concurrency.cancel-in-progress` on tracker-ci.yml prevents stacked runs on retry**: same block as `ci.yml` — `group: tracker-ci-${{ github.head_ref || github.ref_name }}` with `cancel-in-progress: true`. Doesn't fix the underlying sync-retry cancel-via-PR-close logic but makes stacking harmless.
- **Top-level `tests/` drift after WI-tasab and WI-nadud** (WI-bukag): three pre-existing failures surfaced when WI-javan instrumented the top-level `tests/` directory. `test_committed_file_is_up_to_date` now passes `ANALYZER_SRC_DIRS` to `scan_producers`; two `TestLogTrainingExampleCohortMetadata` tests updated to assert `pipeline_version == "v2"`.

#### auto-pr

- **`.ops` backup/restore no longer overwrites concurrent tracker writes** (WI-buhov): `do_merge` in `forgejo-api.sh` backed up tracker `.ops`, rebased, then restored with `cp` — clobbering any ops appended during the run. New `_ops_union_restore_file` helper performs an order-preserving line-level union (`awk '!seen[$0]++'`), semantics-correct for append-only ops files.
- **Rebase restore now matches hidden `.ops` files** (WI-tasuj, companion to WI-buhov): restore loops used bare `"$backup_subdir"/*` which without `shopt -s dotglob` does not match dotfiles — every tracker ops file is a dotfile. Every tracker-ops restore since WI-buhov landed was a silent no-op; lost the 2026-04-19 tracker replies on WI-hopug / WI-nozuj / WI-nokoh. Fix enables dotglob locally for the restore loop.
- **`auto-pr` Exit 2 (timeout) soft-retry** (WI-dotod): the hung-run retry loop only fired on Exit 3. On Exit 2 the script now re-polls once with a 300 s timeout, then does one close-PR + repush, before escalating to Scenario B.

#### Agent-supervisor

- **Rate-limit cap raised 8 → 24, auto-shutdown on sustained saturation, stop-sentinel now logs**: 2026-04-18→04-19 trial hit `RATE_LIMIT_MAX_PER_24H=8` at 16:15Z on a productive 6-session chain; saturation produced 145 identical "rate-limit reached" log lines and no log entry on stop. Raised cap to 24 (~3× observed productive rate); new `RATE_LIMIT_SATURATION_KILL=20` auto-shuts-down after 20 consecutive rate-limited ticks; stop-sentinel branch now logs the exit.
- **Graceful-exit wait now actually waits**: `cli_pid` was `None` at spawn time so the 30 s `kill -0 <cli_pid>` wait was always skipped and every replacement hit the hard-kill path within ~2 s of the exit keystroke. New `tmux_session_exists` / `wait_for_session_gone` helpers poll `tmux has-session` instead — when the CLI exits cleanly, tmux auto-removes the single-pane session. No PID tracking, no hook cooperation.
- **`stop_hook_state.json` write discipline: unmaintained keys silently dropped** (WI-joriv): jq merge in `stop_logic.sh` used to start from `.` (full existing object), so dropped keys from old migrations (5 zombie fields from PR #2926) lingered. Now starts from an explicit extraction of the maintained field set; any unlisted key is dropped on the next write. Recover-state playbook documents the maintained-field table.

#### Stop hook

- **Watched-process detector no longer over-matches on pytest-pathed helpers** (WI-zajob): WI-varid's `pgrep -af "pytest|..."` matched any process whose argv contained a pattern substring — `inotifywait /tmp/pytest-.../foo` was treated as a live pytest and blocked the hook for up to 30 min. New `watched_process.py` splits each cmdline, strips argv[0] directory prefix, collapses `python3.11` → `python`, and requires the full pattern to match LEADING argv tokens. `bash -c` / `sh -c` exclusion preserved.

#### Analyzers & edges

- **Solidity file-level `using X for Y;` applies to calls inside contracts** (WI-jovur / UAT BUG-13): the edge extractor keyed `using_libraries` by enclosing contract only, so file-level directives (keyed `""`) were invisible. Fix: union contract-scoped with file-level set before probing `{Library}.{method}`.
- **`test-coverage` recognises framework-tagged tests outside test paths** (WI-dulav / UAT BUG-11): any function/method whose `meta.concepts` contains a concept starting with `test` is treated as a test in addition to the path-based heuristic. Fixes the shellcheck scenario where Template-Haskell `$forAllProperties` discovers 2214 `prop_*` functions in `src/` (0 % reported coverage before the fix).
- **`dead-code-maybe` unconditionally drops symbols in generated files** (WI-jifup / UAT 2.6.0 round 05): any candidate whose `supply_chain.is_generated_file` is True is now filtered out before ranking. Language-agnostic; generated code is never actionable dead code.
- **Django CBV routes now expand to one route per declared HTTP method** (WI-lojoh / UAT BUG-08): `path("/foo/", FooView.as_view())` previously emitted a single `[GET]` route regardless of declared view methods. Python analyzer now flags CBV registrations with `is_class_based_view=True`; new `expand_class_based_view_routes` post-pass replaces each ANY route with one variant per declared method. Out-of-repo view classes stay `[ANY]` (honest unknown beats false specificity).
- **Java `size`, `length`, `copy`, `find` no longer misclassified as fs_read** (WI-gonav / UAT BUG-10): missing from the io-boundary `ambiguous_names` list, so `List.size()`, `String.length()`, etc. were tagged as `java.nio.file.Files.size`/`.length`/`.copy`/`.find`. Added to the ambiguous filter; resolved calls with proper module context still match.
- **Scala framework detection reads `project/*.scala` and `project/*.sbt`** (WI-piban): the SBT meta-build convention puts real library coordinates in `project/Dependencies.scala`; `build.sbt` only references scala helpers. Docspell's http4s imports were invisible to `profile.frameworks`. Detector now concatenates `build.sbt` plus all `*.scala` / `*.sbt` under `project/`.
- **Laravel `apiResource()` no longer emits phantom HTML-form routes; `.except()` / `.only()` honored** (WI-jorim / UAT BUG-07): `Route::apiResource('posts', ...)` now produces 5 routes (index/store/show/update/destroy) instead of 7. Chained `.except([...])` / `.only([...])` modifiers parsed. On koel: ~40 phantom routes eliminated (~19 % of 207).

#### CLI

- **`--config-extraction=embedding/hybrid` warns when sentence-transformers is missing** (WI-fumap / UAT DQ-07): both modes silently degraded to heuristic. Dispatcher now pre-checks embedding availability and emits a one-shot stderr notice before falling back; `_embedding_fallback_warned` dedupes across calls.
- **`verify-claims` surfaces languages where taint-flow has no catalog** (INV-javam): stderr notice fires when a `taint_flow` constraint evaluates on languages with zero coverage. Before this, a trivially-passing claim against an unanalyzed language gave false security confidence. JSON schema unchanged.
- **`io-boundaries` distinguishes "no I/O detected" from "language unsupported"** (INV-javam / UAT DQ-03 + DQ-04): `IoBoundaryCatalog` gains `is_supported: bool`, stderr notice on unsupported languages, and JSON output includes `unsupported_languages: []`.
- **Subcommand parser cleanup** (WI-balij / UAT UX-03 + UX-04): (1) `hypergumbo foobar` no longer silently inserts `sketch`; `main()` detects unknown-subcommand attempts and prints a `Did you mean: ...` via `difflib`, exits 2. (2) `hypergumbo sketch . --debug` works — `--debug` is stripped from argv in any position.
- **Embedding-model load no longer dumps progress bars to stderr** (WI-gatot / UAT UX-02): `_hf_noise.suppress_hf_noise()` runs at `sketch_embeddings` import time (before `sentence_transformers` caches env), setting `HF_HUB_DISABLE_PROGRESS_BARS=1`, `TRANSFORMERS_VERBOSITY=error`, etc. via `setdefault` so user overrides are preserved.
- **`-e/--exclude` glob normalization** (WI-zirik / UAT BUG-01): `ui/`, `ui/**`, `**/ui/**`, `**/ui` now behave consistently with the bare-name form `ui`. Path-anchored patterns like `cmd/server.go` honored against the relative path. Affects `run` and `sketch`.
- **README / markdown heading bleed** (WI-bilul / UAT BUG-06): when a `.md`/`.mdx`/`.markdown`/`.rst` file renders as Additional Files Content, ATX headings are demoted 2 levels so they cannot compete with hypergumbo's H2 structural sections.
- **Token budget validation** (WI-pokor / UAT BUG-02 + BUG-03): `-t 0` and negative values rejected by argparse. Previously `-t 0` was silently treated as "no budget" and `-t -1` produced header-only output with exit 0. Applies to `sketch` and `explain`.
- **Single-file input exits cleanly** (WI-zujum / UAT BUG-04): `hypergumbo run` / `sketch` on a file prints a hint and `sys.exit(1)` instead of crashing with `NotADirectoryError`.
- **Quieter partial-linker warnings on polyglot repos** (WI-vasir / UAT UX-05): suppress partial-install warning when the only met requirement is a language-file presence check (`*_files` suffix). Verified on alertmanager: 8 warnings → 1 (−87 %).
- **`--require-section` now actually works** (WI-nakam / UAT BUG-05): `max_tokens <= base_tokens` early-return bypassed all section gates. Fix skips the early-return when required sections were requested, forces analysis for required-section dependencies, makes `_section_ok` return True unconditionally for required sections, and skips the final `truncate_to_tokens`. Verified on alertmanager `-t 500`.

### Performance

- **Cached secret-scan results across warm sketch runs** (WI-julir / UAT BUG-20): `scan_content_cached` keys gitleaks output by sha256 of the sketch content (up to 8 entries). Warm `hypergumbo sketch` now completes in roughly the `--no-secret-scan` time (~7 s on alertmanager) instead of paying the ~8 s gitleaks cost each invocation. Cache invalidates automatically when the repo state hash changes.

### Documentation

- **`docs/agent-supervisor.md` operator guide** (follow-up to WI-razub): net-new user-facing doc covering prerequisites, first-time setup (`loop-toggle DEEP` + `agent-supervisor run &`), daily operations (attach / detach / pause / resume / shutdown), `status` JSON field semantics, edge cases, troubleshooting matrix, state-directory layout, and cross-references. Linked from `README.md` Links section.
