...
- **JS/TS function parameter shadowing**: Calls to names that match a parameter of an enclosing function or arrow function (e.g., `resolve()` and `reject()` inside `new Promise((resolve, reject) => {...})`) no longer resolve to global functions. Prevents false cross-package edges in polis where `resolve()` inside Promise callbacks was resolving to a client-side `resolve` function (50 false edges) and `reject()` to a client-side `reject` function (48 false edges). Handles arrow functions (with and without parens), function expressions, and regular function declarations.
