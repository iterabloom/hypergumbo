...
Second, **the analyzers emit call edges they never emitted, and the catalogue becomes extensible.** Python bare builtins, Go calls under a package-level `var`, Haskell zero-argument actions, Erlang `?LOG_*` macros, Rust grouped `use` lists and Java wildcard imports reach the catalogue for the first time, and receiver typing reaches own fields, bare parameters and var-rooted field chains. ADR-0047 makes the shipped catalogues stdlib-only, moves third-party rows into community overlays that load by default, and opens user channels for every extensible family; ADR-0045 adds user and project configuration and a per-repository trust grant for the code-executing Rust backend. Two vocabularies gain declared axes (ADR-0050 for I/O boundaries, ADR-0051 for the module key), the ADR-0017 §3a walk gains removal authority, and `verify-claims` says more about itself: the fidelity it ran at, the receivers it could not type, and the analyzers that are blind by construction. `SCHEMA_VERSION` advances 0.20.1 → 0.20.3 and the `verify-claims --json` envelope 2.0 → 2.3.

### Added

#### Python receiver typing (INV-linub class, Phase 6)

- **A Django QuerySet chain keeps its ORM module past the first hop (INV-mumov, Phase 6 PR 1).** WI-sozoj typed `<Model>.objects.<method>()` and nothing after it, so `Order.objects.filter(...).exists()` and `qs = Order.objects.filter(...); qs.delete()` lost the type where the catalogue's Django rows live. The result of a QuerySet-returning member on the `.objects` root now carries `django.db.models`, and 21 rows in `library_signatures/python.yaml` say which members return a QuerySet, so chained hops propagate through the same mechanism as `pathlib.Path`; `get` / `first` / `create` deliberately do not (a Model instance is a project class). The `.objects` receiver itself stays untyped, so a project-defined manager method never acquires a false owner path. Chosen by the 2026-09-06 derivability census (1,625 pretix sentinel edges on an ORM chain, 976 on names the F3 gate refused for want of a module). Measured by [measurement 0013](docs/measurements/0013-django-queryset-chain-delta.md): on pretix the change adds 40 situations and loses none (kserve control: 0 moved) — 87.5% correct, 17.5% useful, because 28 of the 40 sit on a lazy combinator (`filter(...).delete()`) that reads nothing, ADR-0049's deferred row, which this change makes removable.

- **Django's lazy QuerySet combinators leave the minting boundary, and the evaluation they stood in for gets a call site (WI-fasap, Phase 6 PR 2).** `filter` / `exclude` / `order_by` and the other eighteen QuerySet-returning members compose a query and read nothing, yet they sat under `db_read` — minting `untrusted_input` at `WebHookCall.objects.filter(...).delete()` — because Django runs the query at an implicit `__iter__` no catalogue row could name (INV-nular F3's deliberately-kept control; 28 of the 40 situations measurement 0013 adjudicated). Both halves of ADR-0049 ruling 3 land together: `py.py` emits a `calls` edge to `django.db.models.__iter__` (`__aiter__` async) at a `for`, a comprehension and `list`/`tuple`/`set`/`frozenset`/`sorted`/`dict`/`enumerate`, and `__getitem__` at an index subscript (a slice propagates the type), rowed under `db_read`; and the combinators move to **`db_compose`**, a new `deferred_crossing` registry value — catalogue-declarable, never minted, disclosed in its own `db_compose_edges` count (`IO_BOUNDARIES_SCHEMA_VERSION` 2.3) and shadowing `db_read` so a clean database verdict over a query builder is qualified, not granted. Not named `db_query`, which is already the database-query linker's `call_kind` and means the opposite. The compose rows are pinned equal to the signature table's QuerySet-returning members in both directions; the INV-buzab three-row table runs on a vouched copy of the overlay, since community rows never license an all-clear on their own (ADR-0047). Measurement 0014 prices the change on pretix.

- **Django's reverse-relation managers and custom manager names carry the ORM module (WI-gulaz, Phase 6 PR 3).** The marker was the literal `.objects`, so `order.payments.create(...)`, `event.sponsors.add(s)`, `self.seat_set.filter(...)` and pretix's soft-delete `Checkin.all.filter(...)` were invisible — a related manager is declared on the OTHER model, in a file the caller never imports. `py.py` now builds a project-wide `DjangoRelationIndex` once per repository (reverse FK and M2M accessors under `related_name` or the default `<model>_set`, forward FK / one-to-one fields as instances of their target, class-level `<...>Manager(...)` / `as_manager()` / `from_queryset(...)()` attributes on verified model classes, and the resolved project bases, since an accessor declared through `ForeignKey("self")` on an abstract base is owned by every concrete subclass). A per-block oracle answers whether `<X>.<a>` is a manager — `self` in the enclosing model, a typed Name, or a declared relation hop off either — and feeds the same two consumers the `.objects` marker has (the chain root rule and the bounded emission), so the manager itself stays untyped and a project-defined manager method never acquires a false owner path. The same change binds `x = Order.objects.get(...)`, `x = order.payments.create(...)` and `obj, _ = ....get_or_create(...)` to the model instance they return, walking QuerySet-returning hops back to the root, and rows the RelatedManager's own write surface — `add` / `remove` / `clear` / `set` — under `db_write` (835 pretix sites, none catalogued before; a parity test pins producer set to rows). The pre-registered refutation held by construction: a serializer field or a `@property` named like an accessor, a target string two classes could answer to, `related_name="+"`, a `%(class)s` placeholder and a one-to-one reverse accessor (an instance, not a manager) all register nothing. Sized before the design on pretix (5,441 accessor call sites; 782 reachable through the instance binding, 434 through a related manager's `create`/`get`, 116 on `self`, 1,155 in test `setUp` fields deliberately left for a filed follow-up). Measured by [measurement 0015](docs/measurements/0015-django-related-manager-delta.md): on pretix the change adds 27 situations and loses none (kserve control: 0 moved; no verdict moved) — 66.7% correct and 66.7% useful, 0 wrong-type: 18 real fetched-row → write pairs, 9 false ones that split into a row used only as a check argument two hops away and a structural pairing whose real carrier is an untyped instance `.save()` on a loop variable, both filed.
- **The Django ORM instance write is recognised through the model lineage, not one direct base (WI-gamas, Phase 6 PR 4).** WI-sozoj typed `self.save()` only where the class's DIRECT base was the dotted `models.Model`, which on pretix recognised **two** call sites in a repository containing about 2,900 of them. The gate moves to `DjangoRelationIndex.model_ids` (a `models.Model` base, a `django.db.models` field, or a project base that is one — transitive, repo-wide), and the receiver widens from `self` to any receiver the index resolves to a model instance: a local bound from a manager or a constructor, an annotated parameter, a forward-relation hop. It fires on the unresolved branch only, so a project-defined `save` keeps its first-party edge and is never re-keyed, and `DjangoRelationIndex.__bool__` now counts `model_ids` so a Django app that declares no relation still gets the rules. Measured by [0016](docs/measurements/0016-django-instance-write-delta.md): pretix **2 → 75** typed `save`/`delete` sites, **11 taint situations added, 0 lost, 72.7% correct / 72.7% useful**, kserve control unmoved, 0 wrong-type across the whole production survey. A third rule — `super().save()` inside a model's own override, which on a real Django project is where the write actually is — was built, measured and **withheld**: correct at the site (61 sites read back, 0 wrong-type) but the ADR-0017 walk reaches any model method from any function that merely mentions the class, by crossing the django-orm-dispatch linker's `dispatches_to` edge, so it bought 3 true findings against 20 false ones. Both are filed.
- **The Django `super().save()` write chokepoint is typed, on a walk that no longer crosses reachability edges (WI-sihoh, Phase 6 PR 5).** The rule WI-gamas withheld returns. On a real Django project the ORM instance write is usually not at `x.save()`: of pretix's 306 typed sites, 215 (70%) call a `save` the project OVERRIDES, and 56 of its 66 overrides then call `super().save()` — the chokepoint every upstream write funnels through, which emitted the bare `external` placeholder, a false NEGATIVE on a write primitive. `super().save()` / `super().delete()` lexically inside a class in `DjangoRelationIndex.model_ids` now carries `django.db.models`, REFUSED when a project base defines that method (there the write is typed at that base's own `super()` call, so one logical write is counted once) and refused for the two-argument `super(Cls, self)` form, which names the class the MRO starts AFTER. Its deferral had a written re-open trigger — "when the walk stops advancing along `dispatches_to`, restore the rule and RE-MEASURE" — which INV-putug (PR #841) fired. Measured by [0021](docs/measurements/0021-django-super-write-remeasured.md): pretix **16 situations added, 0 lost**, **9 TP / 7 FP = 56.3% correct** (50.0% under a stricter reading of one site), **0 wrong-type across all 136 marker edges**, kserve control unmoved. The 3-against-20 figure recorded on the deferral was taken THROUGH the defect and is not this rule's precision. The measurement also supplies the first LIVE-POPULATION observation of INV-putug's fix — 67 newly typed sinks sit inside classes carrying 896 refusable `dispatches_to` edges and the walk crossed none — which 0020 could only obtain with harness arms, because the sink it needed was this very rule. Its own pre-registered prediction was refuted: seven of round 1's eight "ordinary adjudication misses" were this rule's false positives all along.

- **A relation accessor the project's own models declare types the ORM slot on an UNTYPED root, and every edge so typed says that it did (INV-mumov / ADR-0053, Phase 6 PR 7).** WI-gulaz reached `DjangoRelationIndex` only through `instance_class`, so a bare parameter (`event.seats.filter(...)`) or an unresolved dotted path (`request.event.seats`) never consulted the index at all and fell to the `external` sentinel — ADR-0051's own marker for *unreachable to the catalogue*, a false negative by construction. `manager_root` now fills the MODULE slot from an accessor name the project declares when nothing is known about the root; no model is stamped, so the instance binding is untouched. The refutation that survives is `_refuted_by_a_known_owner`: a resolved class that does not declare the name, a local bound to a non-model, and a call the resolver already declined all still refuse — the rule opens on *absence of evidence against*, never on absence of evidence, and `make().seats` stays untyped. **Measured on pretix: +2,943 ORM-slot call edges, 0 lost** (2,867 promoted, 99.6% of them off `external`; 58 protocol edges that could not be emitted at all while the receiver had no type — the 11 promotions that never touched `external` and all 58 created edges adjudicated exhaustively against source, all correct). **A shuffled-index ablation** replaced the accessor set with 20 size- and frequency-matched WRONG sets: true +2,943 against a shuffled mean of +37.3, a **78.9:1 ratio against a kill threshold of 10 fixed before the number existed**. Four arms of `verify-claims` priced the direction: **45 findings generated, 0 lost**, both shuffled controls moving **exactly 0**, `sanitizer_scope` byte-identical across all four arms and no verdict changed.

- **A clean security verdict stays qualified when a receiver was typed from a name, instead of going silent (ADR-0053 second half).** Typing such a receiver REMOVES it from `untyped_receiver_sites`, so the disclosure a clean boundary verdict owes its reader would have shrunk exactly where the typing rests on a name rather than on a resolved class — a recall gain bought with a quieter all-clear, which is the one direction this project treats as unsafe. Edges filled by the name rule carry `resolution_quality="accessor_name"` (a new value on the registered `AXIS_EDGE_META` key, following INV-tadup's `chained_return_type` precedent for a resolution MECHANISM), and `CAVEAT_ACCESSOR_NAME_RECEIVER` is its complement over one population: *"I could not type N receivers"* becomes *"I typed N receivers from a declared accessor name, not from a typed root"*, with a commensurable denominator. The catalogue lookup is deliberately NOT scoped to the module the `dst` names — that module is the inference, and the inference is what might be wrong; scoping to it would assume the answer and would also make the caveat unreachable, since a `db_read` claim is violated by the very chain that would qualify it. The four-arm measurement **could not test this direction at all** — the caveat only qualifies CLEAN verdicts and all seven of pretix's claims come back violated, which is structural rather than incidental — so it is pinned by fixture instead, with the producer-side stamp and the consumer-side fixture asserting the same shape so neither can drift silently. The python catalogue's `(type-verified receiver)` notes are corrected to `(module-qualified receiver)`: the module-filter property is unchanged, how the qualified `dst` is reached is not.

#### JavaScript receiver typing

- **Node's `fs.promises.<fn>` member chain reaches the `fs.promises` rows (WI-vihop).** The promise APIs have two spellings and only one worked: `require('fs/promises').readFile(p)` classified, while `fs.promises.readFile(p)` — the documented pre-ESM form, still common — fell to the `external` placeholder, because `obj_name` is set only when the receiver is a bare `identifier` and `fs.promises` is a `member_expression`, so every case consulting `namespace_imports` was skipped. The 31 catalogued `fs.promises` rows and 16 `dns.promises` rows were unreachable through it. A **fifth** spelling not in the filing was equally broken — the ESM default import `import fs from 'fs'` — and one fix covers all three bindings because `_extract_namespace_imports` already binds them identically. The slot REUSES the string the direct require emits (`fs/promises`) rather than inventing a second spelling to keep in step, and the parent is an allowlist of node modules that genuinely publish a `promises` twin: a general "`<ident>.<prop>` becomes `<module>/<prop>`" rule would emit `axios/defaults` for `axios.defaults.get()`, the fictional module INV-fazim refuses. Measured on the three corpus repos that actually use the chain: **+303 edges reach a catalogue row** (podman-desktop 53 → 127, vscode 12 → **151**, nextjs 276 → 366) with **`calls_total` identical in all three** — nothing created, nothing lost. Of nextjs's 90, 24 were previously bound to a PROJECT symbol `CacheFs.mkdir` — an *interface declaration* in an unrelated file, matched by Case 4's low-confidence short name — so those are a precision gain on top of the recall: the call in `nodeFs: CacheFs = { mkdir: (dir) => fs.promises.mkdir(...) }` is node's, not the interface member it implements.

- **Node's `process` and `performance` are runtime globals, not `external` (WI-kikar).** `process.on('message', h)` and `process.send(x)` landed on the `external` sentinel — ADR-0051's marker for *unreachable to the catalogue* — while `console.log`, `localStorage.setItem` and `navigator.sendBeacon` resolved with their module slot filled, because `process` was simply missing from `JS_KNOWN_GLOBALS`, the set the Case-3b member-call fallback consults for a receiver that needs no import. **The filing named two rows; enumerating every method/function-kind module in `javascript.yaml` against its resolution route found eight calls across two globals.** The break is in the member-call path rather than the row kind, so the function-kind rows on `process` broke identically (`hrtime`, `uptime`, `getuid`, `memoryUsage`, `chdir`, `openStdin`; 33 rows in all, across ipc_recv, ipc_send, host_info_read, env_write and fs_read), and `performance.now()` — a second global the filing does not mention — was equally inert. Those two were the only modules in the catalogue with no resolution route at all. Measured on vscode / nextjs / podman-desktop with an identity keyed on `(src, line, callee)` rather than on the `dst` the change modifies: **892 edges promoted with `calls_total` identical in all three repos, and every promotion came off `external` — zero taken from an already-resolved symbol**, which is the number that mattered, since Case 3b fires only where nothing else resolved. `performance` supplies 442 of the 892. **278 of the 892 land on an actual catalogue row** (`performance.now` 212, `process.on` 23, `process.hrtime` 20, `process.getuid` 10, `process.chdir` 5, `process.send` 4); the remaining 614 are correctly slotted members the catalogue does not row, so 892 is the promotion count and 278 is the recall figure. The set's comment claimed to "mirror the module names used in `javascript.yaml`" and did not; it now states the real constraint, because the catalogue records no field saying how a module is *reached* (global vs import vs constructor) and so this set cannot be derived the way `_derive_js_constructor_types` derives its own — filed as WI-nukub, with `process.cwd`'s attribute-vs-function mis-kind (135 slotted-but-unmatchable sites) filed as WI-bomuh.

#### Scala receiver typing

- **A Scala receiver's type survives a qualified or generic type node (WI-pokam).** `scala.py` read a declared type with `find_child_by_type(node, "type_identifier")`, which names ONLY a bare type; tree-sitter gives a qualified type its own node kind (`stable_type_identifier`) and a generic applied type another (`generic_type`), so `val b: java.io.File` and `val c: Buffer[String]` produced no `receiver_type_hint` at all. The filed item named one shape; a probe found three and they are one bug — `new java.io.File("p")` loses it identically, though the item lists `val x = new Foo()` among the shapes that WORK, and a GENERIC APPLIED type is not in the filing at all. Four sites each looked for the bare node themselves and now share one walk, because sharing a predicate is not enough when callers can still walk different populations (INV-motos). A generic type contributes its BASE recursively (the receiver's methods live on `Buffer`, not `Buffer[String]`), which qualifies through the proven bare-name path; a qualified type IS the path and qualifies to itself, which is WI-sigog's "only a path the file itself declares" discipline applied to the one case where there is no import line to consult. Measured on sbt and lila with baselines re-derived rather than quoted: **hinted share 9.03% → 23.43% (sbt) and 4.50% → 8.02% (lila); module-slot share 4.63% → 18.19% and 0.89% → 3.99%**, with `calls_total` identical in both repos (nothing lost — the edges that left the unresolved population became RESOLVED project bindings, INV-fahub's type-qualified recall recovery).

- **A generic base that Scala imports implicitly is refused, and the item's own metric would have preferred the version without that refusal.** The first version scored 28.07% hinted on sbt; sampling the edges it newly RESOLVED found `.get` on a standard-library `Map[K,V]` binding to the PROJECT's `Map.get` — **44 of 185 (24%)** newly-resolved edges bound to an implicitly-imported name, which is the "arbitrary same-named internal def" funnel `scala.py` already refuses for untyped receivers. `Map` / `List` / `Set` / `Option` come from `scala.Predef._`, appear in no import line, and so add nothing to the module slot while reaching the short-name bind. Denying them drops the hinted share to 23.43% and leaves the slot share alone (18.07% → **18.19%**, marginally up) with **0** mis-binds: an unqualifiable name never filled a slot, so the refusal costs nothing where the catalogue is reached. The pre-registered prediction that generics would dominate qualified names on BOTH repos was **refuted** — sbt 2,180 vs 213, lila 284 vs **1,068** — the two repos have opposite idioms, and a single-repo run would have "confirmed" whichever one was chosen.

#### Elixir receiver typing

- **A bare call takes the module that exports it, not an arbitrary member of the import set (WI-jozap).** `elixir.py` attributed a bare stdlib-named call to `next(iter(imported_modules))`. Python randomises string hashing per process, so **the same code over the same repository did not produce the same answer twice**: on phoenix-framework, identical tree and identical blob, `PYTHONHASHSEED=0` and `12345` produced surveys differing by **101 edges and up to 25 nodes**, with edge totals stable — it is the destinations that churned. Every elixir number this project has published was taken through that, and it nearly produced a wrong decision once: WI-kigub's first A/B reported 32 vanished edges, a ship-blocker under its own pre-registered falsifier, and the 32 were smaller than the instrument's variance against itself. **Ordering the set would have fixed determinism and shipped a deterministic wrong answer** — the moving edges are `raise`, `to_string`, `is_list`, Kernel auto-imports available with no `import` directive at all, flipping between `Plug.Conn` and `Phoenix.Controller` by hash seed. The root cause is that `_ELIXIR_STDLIB_FUNCTIONS` was **two populations wearing one name**: Kernel auto-imports, and names that genuinely do come from an import (`IO.puts`, `Enum.map`, `String.split`, `Logger.error`) whose ownership was written down beside them as prose comments and never used as data. Three ordered rules replace the set iteration — an `only:` list that names the function wins, a Kernel auto-import gets no module-keyed edge, and an importable name whose owning module is imported takes that module — and anything else is declined, which is the withholding direction. **Measured on two repositories: 653 of 653 edges this branch emitted were false attributions of a Kernel auto-import, so it never once fired on the population it was written for**; after the fix both seeds produce byte-identical surveys (excluding the per-run UUID) in every edge and every node. Probing the grammar found five import shapes where the item named one.

- **An atom-module call reaches the catalogue, so Elixir can see OTP at all (WI-kigub).** Elixir calls every Erlang/OTP module with an ATOM module — `:ets.insert(t, x)`, `:httpc.request(...)`, `:gen_tcp.send(...)` — and `_handle_dot_call` required the dot node to carry an `alias` child. An atom parses as `atom`, so the handler returned before emitting anything: not an untyped edge and not a misbound one but **no edge at all**, an INV-linub L2 failure no downstream receiver evidence can rescue. Probing the grammar rather than trusting the filing found **six** early-returning dot shapes, not the one filed, and scope was set by parity with the working alias path — fix the shapes whose module is written literally at the call site. The leading colon is **stripped**, which is the whole L3 half of the change: `_module_matches` is component-aware and does no colon stripping, so a module slot of `:ets` reaches the catalogue's `ets` row not at all, and an edge that exists but matches nothing is the exact failure INV-linub exists to catch; stripping is also the cross-language parity answer, since the erlang analyzer emits `erlang:ets:0-0:insert:function` for the same OTP modules. Atom calls bypass the first-party resolution branches **by construction**, because those end in an `any(module_dot in k ...)` SUBSTRING test and `"ets."` is a substring of any project key like `Foo.Sockets.insert`. Measured on phoenix-framework with the hash seed pinned: **+122 call edges across 25 distinct OTP modules, 0 lost and 0 symbols moved**; `io-boundaries` **250 → 277** chains and **52 → 65** distinct elixir primitives with **0 primitives lost**; `db_read` (0 → 8) and `db_write` (0 → 10) appear as **boundary kinds phoenix had never reached**. Two `verify-claims` verdicts move from `inconclusive` to `violated` — `untrusted-input-no-database` (ev=4) and `host-secret-no-network` (ev=1) — and **all five findings were read back against source: 5/5 structurally correct, 0 wrong**, the fifth being a proxy environment variable genuinely configuring the `:httpc.request` it co-occurs with, which is WI-gohok's already-ruled correctness-axis-true / usefulness-weak category. The claims were inconclusive because hypergumbo could not see `:ets` or `:httpc` at all and correctly said so; elixir is `dataflow_capable: false`, so every one of the 31 findings stays `analysis_method: structural` / `confidence: approximate`. Four remaining receiver shapes — a variable, a module attribute, a quoted atom and `__MODULE__` — are refused and **pinned as known gaps** rather than fixed inside a construct change. The pre-registered edge-count prediction (150–225) was **refuted at 122** and chased to its cause: 101 of the 103 unemitted source sites are calls not lexically inside a `def` (module bodies, `test` macro blocks) plus typespec type references that must not emit at all, and a parity fixture shows the alias path loses exactly the same sites.

- **A module-qualified Elixir call no longer resolves by bare name (WI-kafor).** `_handle_dot_call` asked whether the project plausibly owned the written module as a SUBSTRING test — `any(f"{module}." in k for k in global_symbols)`, though the comment above it described a prefix-COMPONENT test — and then let the resolver bind by BARE NAME with only a path hint. On phoenix-framework that produced **202 false edges**: `Logger.error(...)` landing on an `error/1` inside an **EEx generator template**, `Path.join(...)` on a Phoenix Channel's `join/3` in another template, `Mix.raise(...)` on a test controller's `raise/1`, and `Logger.info(...)` on a **`defp`**, which Elixir does not permit to be called from another module at all. Instrumenting the live run rather than trusting the filing found **two** sub-mechanisms: a SUFFIX artifact matching mid-component (`Logger.` inside `Phoenix.Logger.install`, `Path.` inside `…StaticPath.path`) and a NAMESPACE PREFIX that is a real component-boundary match and still wrong (phoenix defines 56 modules under `Mix.`, so `Mix.raise` passes any component test). Tightening the gate to an exact module-ownership question was built and measured first; it is necessary and **not sufficient** — with the gate exact, every edge the bare-name lookup still produced corpus-wide was *still* wrong — so the bare-name resolution is **removed** and the exact-ownership helper deleted as dead code. A qualified call now resolves only by exact name, directly or through alias expansion; anything else takes the unresolved edge, which names the right module and lets the catalogue match. Two prerequisites surfaced only because the old substring test had been covering for them, and each lost true edges the moment it stopped: the **multi-alias brace form** `alias Phx.New.{Project}` registered no hint at all (its tree is `arguments → dot → [alias, ".", tuple]`, so there is no `alias` child to find) — **71 true edges** — and an alias binds its **first component**, so `alias Mix.Tasks.Phx.Gen` makes `Gen.Context.build(...)` mean `Mix.Tasks.Phx.Gen.Context.build/2` — **29 more**. Measured on phoenix with the hash seed pinned and both arms reproducing themselves: **208 edges redirected, total unchanged at 11,222, and 0 real losses** — all 206 lost first-party edges adjudicated, 200 because the written module is not a project module and 6 read back against source as corrections (`Project.new` and `Context.new` now bind to `Phx.New.Project.new` / `Mix.Phoenix.Context.new`, where both previously hit a template's `new/2`). `io-boundaries` **277 → 293** with **0 primitives lost**: `logging` 53 → 64 and `process_send` 0 → 5, a boundary kind phoenix had never reached, as `Logger.error` (8), `Logger.info` (3) and `supervisor.start_link` (5) reach their catalogue rows for the first time. `verify-claims` moves `host-secret-no-logging` from ev=5 to ev=7 with every other verdict byte-identical; **both new findings were read back against source and both are correct**. The validation instrument itself had to be corrected mid-measurement: its first version compared written module text against project module names, did not model aliases, and reported a clean "229/229 correctly refused" that concealed 29 genuine losses.

#### Haskell receiver typing

- **A bare ambiguous-named Haskell call gets the module its own file already names (WI-lokun).** `haskell.py` emitted an unresolved call as `haskell:<hint or external>:0-0:<name>:function`, and the hint came only from a *qualified* import alias. The `external` sentinel is deliberate — it makes the catalogue tagger use unfiltered short-name matching — but `gate_named_entry` ends `if name in ambiguous_names: return None`, so with no module hint an **ambiguous** name reaches nothing while an unambiguous one still matches. The servant baseline showed the split in a single table: `Prelude.putStrLn` classified with 8 chains and **`Prelude.print` was absent**, from the same module, the same files and the same bare call shape. Scope is exactly that asymmetry — only a name in `ambiguous_names` is stamped (8 names, derived from the catalogue rather than listed), because stamping an unambiguous name switches module filtering ON for a call that already classifies, buying nothing and risking the fallback; WI-lajus is the filed cautionary case, where stamping too broadly suppressed a permissive fallback and moved verdicts the wrong way. Two routes count as evidence and everything else abstains: an unqualified `import M (name)` list (the route the item filed), and the name's home module being `Prelude` with nothing in the file claiming otherwise (where the filed *instance* actually lives, since Haskell imports Prelude implicitly and it appears in no import line). A `qualified` import is skipped deliberately — it does not put the bare name in scope — and an explicit claim beats the Prelude route, so `import Data.Text.IO (readFile)` beside `import Prelude hiding (readFile)` never yields `Prelude`. Measured on servant: **`Prelude.print` newly reaches the catalogue with 3 `logging` chains, 0 primitives lost, 0 existing chain counts changed** (`Prelude.putStrLn` stayed at exactly 8) and every `verify-claims` verdict byte-identical. All three sites were read back against source — and they corrected the rationale: each carries `import Prelude ()` + `import Prelude.Compat`, the base-compat idiom, whose empty import list *suppresses* the implicit Prelude so the name arrives re-exported. The stamp is still right, because `Prelude.Compat` re-exports `Prelude.print` verbatim and the catalogue rows a function's home module rather than whichever module re-exported it; the route is better read as "the home module is Prelude and nothing here claims otherwise". The filing named two example files; there are three.

#### Catalogue extensibility (ADR-0047)

- **Django test fixtures assigned in `setUp` carry the model instance, and the arm that can see them exists for the first time (WI-zamud, Phase 6 PR 6).** A Django test class builds its fixture in `setUp` / `setUpTestData`, not `__init__`, and `_collect_class_field_maps` reads `__init__` and nothing else — so `self.orga = Organizer.objects.create(...)` typed nothing and every chain hanging off it dead-ended at the `external` sentinel (1,155 filed pretix sites, 679 of them inheriting the fixture from a base test class). `DjangoRelationIndex` gains `instance_field_types`, filled by a pass that runs AFTER the relation index — typing a manager call needs it — and iterates to a fixed point so a fixture built in layers resolves without hand-ordering the scan; `fixture_field_model` walks the project bases at LOOKUP, which is what makes the inherited case work. The DECLARED relation is consulted first and this map only on a miss, so a real `ForeignKey` cannot be shadowed by a same-named fixture field, and a subclass reassigning a base's field wins on its own entry. Measured by [0022](docs/measurements/0022-django-setup-fixture-receiver-typing.md), which reports **two source modes separately and never pools them**: the DEFAULT arm is **149 → 149, unmoved**, because WI-bifob excludes test-sourced flows and every site here is under `src/tests/` — predicted in the pre-registration rather than explained afterward. On `--include-non-production-sources`, pretix **613 → 837 (224 new, 0 gone)**, and the pre-registered ≥50% bar **FAILED at 30.8% correct (60 TP / 135 FP, 95% CI 24.7–37.6%, entirely below the bar)**, 0 wrong-type. The number measures the WALK: all 224 are `structural`/`walk_verdict=unavailable`, 152 are hops=0, and the arm's own pre-existing population scores **24.2% by the same classifier — statistically indistinguishable (p=0.25)**, so the bar had been carried over from default-arm precedents. It ships with that failure declared, because the default arm is unmoved, the rule is refutation-clean (**0 violations over 692 independently verified sites**), and edge reach rises **+1,447 ORM call sites (10,422 → 11,869) with 0 gone** — a figure corrected after publication, the first version having reported +756 `(src,dst)` pairs and 110 apparent removals because the measurement's own instrument keyed on `edge.line` and never read `meta["call_lines"]`, where `ir.deduplicate_edges` already preserves every collapsed call site. Also: `measure-taint-precision.py collect` gains `--include-non-production-sources`, passed through to production's own parser — every prior Phase 6 PR moved application code, so no measurement had ever needed to reach that arm, and on this population "the rule works" and "the rule does nothing" were the same number without it.
- **Go's `call_construct` is a declared blindness, priced at zero rather than fixed (INV-tanom).** Go writes a package-qualified free function with the same syntax as a value-receiver method, so `http.Get(u)` and `c.Get(u)` are byte-identical in (name, module_hint, call_construct) and the field records the SYNTAX, not the construct its name promises. The invariant stays **violated** — a zero cost today is not a satisfied invariant — but the debt is now priced: sized over **14 Go repositories (10 vendored) and 72,396 unresolved `cc=method` call edges carrying no module slot**, which is the only population io-boundary reads the field for (the F3 gate consults it exclusively on the no-module path), the shape that would make the mis-stamp cost anything occurred **zero times**. The zero is structural, not lucky: a package qualifier always names an import in the file's own import block, so the module slot is always fillable and only value receivers reach that branch. The emitter is therefore NOT changed — zero measured benefit against a real regression surface in a field several consumers read — and `go.py`'s module docstring instead carries a dated declared blindness stating that no predicate may be keyed on the field to separate a function row from a method row, where a future predicate-writer will meet it rather than having to search the tracker. Four instrument defects were found and fixed while establishing this, **every one of which inflated the count toward "live defect"**, each caught by reading flagged sites at source: a line collision (an edge carries a line, not a column), a forward line window that manufactured 100% of the surviving finding, a struct field whose name collides with an imported package, and a control whose denominator conflated package-qualified calls with type-inferred method calls.
...
- **The symbol suffix index belongs to the registry, not the resolver — pretix's python analysis goes 1700 s → 95 s, a 17.9× speedup with byte-identical output (WI-bavuz).** `_ensure_suffix_index` builds a whole-registry index (for each key, split the module and join every one of its suffixes) and cached it on the RESOLVER, while `lookup_symbol` constructs a fresh resolver per call — so the index was rebuilt per lookup, O(lookups × registry), both factors growing with file count. Found by a within-repo curve (`--max-files` 150/300/600/1200, cold cache, sequential) showing per-file cost **5.70× for 4× the files**, with the ordered-prefix confound refuted rather than assumed away — nodes/file FELL to 0.75× while cost rose. A profile at N=300 vs N=600 then named the function instead of the symptom: `_ensure_suffix_index` 40.1 s → 269.9 s, **61% of all growth**, with `SymbolResolver.__init__` (5,827 → 16,065) tracking `lookup_symbol` (5,826 → 16,064) **to within the one legitimate resolver**. The index is now cached on registry identity, which makes the fast path the DEFAULT: `_lookup_symbol_by_module` already took an optional `resolver` and all nine call sites passed it, yet **46% of calls still arrived with `None`** because six functions in the chain default it and forward — an optional argument whose omission costs 6.7× is a footgun, and both docstrings already gave the advice that a mechanism now enforces. The cache holds a STRONG reference to the registry (a dict cannot be weakly referenced, and the key is `id()`, so holding it alive is what stops an id being recycled onto another object and served a foreign index), is LRU-bounded at 4, checks staleness with `len()` — O(1) by necessity, since an O(R) check per lookup would reintroduce the cost being removed — and `clear_indexes()` now evicts the shared entry. **Per-file cost is now FLAT (0.77× over the same 4× range) and pretix at 0.079 s/file is below kserve's pre-fix 0.198**; a measurement arm that cost 35–46 minutes now costs under ten.
- **A taint walk advances only along edges that can carry a value (INV-putug).** The ADR-0017 walk consumed `django_orm_dispatch`'s `dispatches_to` edge — class → the model's ORM methods — as if it were a call carrying the source value, yielding `function --instantiates--> Class --dispatches_to--> Class.save`, which lands on whatever ORM method the linker attached to the class, including one the function never calls. A `dispatches_to` edge whose SOURCE node is a type is now reachability, not dataflow. **This is not a revert of INV-zuhig**, which put the edge type in the walk deliberately so framework-dispatched handlers could mint: `argparse_dispatch` emits from the enclosing FUNCTION to a handler genuinely invoked with the data, and a type node holds no value where an instance does. **The discriminator is the source node's kind, not the emitting linker** — five linkers emit class-sourced dispatch, but `di_resolution` emits from an interface METHOD and `type_hierarchy` from a parent METHOD, both ordinary virtual dispatch that a linker-name blocklist would have wrongly caught. The vocabulary is `type_like_kind_names()`, whose own docstring records audit 0018 finding 26 hand-written copies of that set across 24 vocabularies, five omitting `protocol`. The gate FAILS OPEN on any id that does not parse against ADR-0036's span anchor, and the edges are not deleted — they are correct as reachability, and `django_orm_dispatch` calls that an orthogonal concern. **Measured (`docs/measurements/0020`): inert on the shipped tree** — the population cannot exist while WI-sihoh is withheld — and on harness arms that restore it, pretix 183 → 163 situations, **22 removed / 2 added, class-node hops 33 → 0**, kserve control unmoved. All 22 removals crossed a class node (zero collateral) and **0 of 22 are correct**; the 11 class-hop flows that had an honest route were KEPT, so the fix deletes the dishonest path to a finding rather than the finding.
- **A DNS lookup is a network receive, in five languages (WI-dozul, owner ruling 2026-09-06).** ADR-0049's question — does the call return a value whose content the FAR SIDE chose? — is answered by the resolver, so `socket.gethostbyname` / `getaddrinfo` / `getnameinfo` / `getfqdn` / `gethostbyname_ex` / `gethostbyaddr` (python), `net.Lookup*` and `net.Resolver`'s methods (go), `netdb.getaddrinfo` / `gethostbyname` / `getnameinfo` / `gethostbyaddr` and the `_r` variants (c), and the `dns` / `dns.promises` surface (javascript, which WI-nolut had left unaudited) now carry `net_recv` and mint `untrusted_input` — a resolver-poisoning threat model is exactly what that expresses. **The ruling was decided by parity, not by the ADR in the abstract:** erlang had rowed the same family under `net_recv` since 2026-05-24, so the real choice was parity-with-erlang versus retag-erlang, and a vocabulary that means different things per language is the failure the axis discipline exists to prevent. **Reach was measured per language, not assumed.** javascript's callback surface (`dns.resolve4(h, cb)`) classifies end to end; the promise rows are keyed dotted like their `fs.promises` sibling because `normalize_module_separators` folds `/` into `.` before matching, so `require('dns/promises')` reaches them; and `dns.Resolver`'s 14 method rows are **inert on arrival** — `new Resolver()` binds a local the analyzer does not type, so the call emits the `external` sentinel, which yields an empty catalogue key by design. Those rows stay and declare their own inertness on the row, so a reader counting rows does not count them as coverage. Rust gets a `std::net::ToSocketAddrs` row for correctness that is **inert on arrival** and says so — it is a trait method on a receiver the analyzer cannot type (WI-tuzaf) — and is deliberately absent from the parity test rather than counted as coverage. **Measured before shipping (`docs/measurements/0019`): the rows add 6 situations across an 11-repository DNS-dense cohort, move 0 verdicts, and 0 of the 6 are correct — a number that measures the WALK, not the rows.** Five of the six are in c, which has no def/use extractor, so every c finding is labelled `structural` and a structural walk pairs any source with any sink reachable in one function; the sixth is in go. Pricing the implied remedy found it is NOT removal: python, with the full extractor, reports the same co-located and discarded-at-source shapes and `flows_removed_by_walk` is 0 in both languages, so what the gap costs is TRIAGE — in c the method label carries zero bits. Filed as WI-himob, ranked below its java twin WI-gotun (four blockers to java's three, and c's sanitizer catalogue is empty where java's has one row).
- **Three erlang rows sorted out of `net_recv` in the same pass.** `inet:gethostname` returns the LOCAL node's name from the kernel and `inet:getservbyname` / `getservbyport` read `/etc/services`; all three had sat under `net_recv` since 2026-05-24 beneath a note about querying name servers that was true of the four rows beside them and false of these. They move to `host_info_read`, where c has filed `unistd.gethostname` all along and where python's `pwd` / `grp` rows already put a local system-table read.
- **`socket` is granted module completeness, after enumerating all 35 of its module-level callables.** WI-dupok withheld this grant on 2026-09-06 for a stated reason — four functions performed a network exchange with no row and no ruling — and the ruling discharges it. A grant makes SILENCE mean "this call crosses no boundary", so the whole surface was adjudicated against the live module rather than from recall: `gethostname` / `getservbyname` / `getservbyport` / `getprotobyname` / `if_nameindex` / `if_nametoindex` / `if_indextoname` / `has_dualstack_ipv6` are rowed `host_info_read`; `send_fds` / `recv_fds` are `ipc_send` / `ipc_recv` (SCM_RIGHTS works only over AF_UNIX, so unlike the generic `sendmsg` beside them the family is known and the row can decide rather than defer); `sethostname` is `env_write`, the nearest value the vocabulary has for a privileged mutation of the MACHINE's name, with the row saying so. Descriptor lifecycle, byte-order conversions and the default-timeout pair are left unrowed, each with its reason recorded.
...
- **The §3a walk's own verdict is published, so `flows_removed_by_walk: 0` is readable (ADR-0052).** `dataflow_coverage` reported how many flows the walk REMOVED without reporting whether it ever ran, and the surrounding rollup hid the difference: `findings_by_analysis_method` folds three walk verdicts into `ddg_mixed` and a fourth into `structural`, while only `unconfirmed` can remove a flow. So "the walk adjudicated every flow and refuted none" and "the walk never got to look at any of them" produced identical output with opposite meanings — the defect `dataflow_scope` exists to prevent for "0 precise findings", left standing on its own field. `walk_verdicts` now counts `confirmed` / `unconfirmed` / `escaped` / `not_attempted` / `unavailable` on every run, in both JSON and text, always present and zero-filled, on the same post-collapse denominator as `findings_by_analysis_method`. A COLLAPSED row is never attributed to its first member: it counts as `mixed`, because measured on beads 63.9% of groups holding a `sink_before_source` member are not unanimous, so counting the scalar would publish a clean, plausible, wrong breakdown in the flattering direction. Measured on hypergumbo's own repository the first time it ran: **`unavailable` 224 of 224** — the DDG built 177,518 edges and covered none of the flows' source functions, a fact the previous output had no way to state. `VERIFY_CLAIMS_SCHEMA_VERSION` 2.2 → 2.3, a deliberate bump under the "changed the meaning of an existing key" carve-out rather than the silent-addition rule, with the reasoning recorded at the constant.
...
- **A catalogued row that only a non-call construct reaches is disclosed on a clean verdict (WI-zumoz).** `javascript.yaml` declares `WebSocket.onmessage`, `WebSocket.onclose` and `EventSource.onmessage` as method-kind `net_recv` rows, but browser code assigns them (`ws.onmessage = handler`) and no analyzer emits a call edge for an assignment, so the three rows — the browser WebSocket / SSE receive surface short of `addEventListener` — never classified and a clean verdict had nothing to say. Measured 2026-08-28 behind the INV-misup receiver-typing gap and re-measured on the fixed analyzer: `addEventListener` on the same receiver reaches the catalogue, the assignments still emit nothing. The rows stay (an inert row costs no precision and claims no coverage; deleting library facts to describe an analyzer limit files the defect in the wrong artifact) and the limit is declared where its siblings are, `analyzer_disclosure.CONSTRUCT_BLIND_ROWS` — dated, derived against the shipped catalogue at render time, surfacing as a third caveat kind `analyzer_construct_blind` on both the boundary and the taint arm, and pinned by an analyzer test that fails the day the assignment starts emitting.
- **A verdict says how much of the catalogue behind it is unverified (INV-nular re-scope, envelope key `catalog_provenance.kind_adjudication`).** A row asserts that a NAME crosses a boundary KIND, and nothing checks that the named primitive performs that operation — the defect that had `newIORef` filed under `db_read`, `socket` under `net_recv` and pure path arithmetic under `fs_read`. Seven families of that shape are swept and gone; the ~2,300-name remainder was ruled disclosed rather than read (2026-09-06). `verify-claims` now reports, in text and in `--json`, how many name-to-boundary assertions the run rested on, how many sit under an entry that argues the kind in writing, and how many are name-bound and UNVERIFIED — with community-overlay rows counted apart as unverified by construction, on ADR-0047 ruling 6's reasoning (426 of them carry `notes`, so folding them in would offer prose on rows the tool declines to stand behind as evidence of scrutiny). **The counts are derived from the catalogues the run loaded, never restated:** the ruling was made against "622 entries / 2,312 names", a check one day later reproduced 2,312 names exactly and returned 773 entries, and counting `(boundary, module)` groups through the production loader gives 563 — three readings of "entry", three numbers, one unchanged catalogue. The text also states what IS mechanically enforced, which is not what the ruling assumed: not a per-row direction check (14 keys span both directions with no `simultaneous` marker, every one the legitimate mode shape) but a declared REASON per multi-boundary primitive, gated in CI, plus a load-time refusal of any primitive stream-gated in both directions.
...
- **The ten-cell INV-linub drain moved user-visible answers in six of seven languages, and a declared sanitizer's suppression does not touch it ([measurement 0029](docs/measurements/0029-t0c-drain-ledger-and-suppression.md)).** The owner's 2026-09-10 drain ruling put the primitive demand on the ledger rather than on each cell, so this is the one place it is discharged: six of seven drained languages reached a primitive they had not reached before, two moved a claim verdict, and **zero primitives were lost anywhere**. The javascript row is measured here (workadventure: 5 primitives gained / 0 lost, `host-description-no-network` evidence 4 → 7, 4/4 read back against source); the other six are inherited from each cell's own live A/B and the table says so per row. The suppression arm separates two things that had been conflated: **a declared sanitizer catalogue suppresses heavily** (7 declared python functions take pretix from 0 to 158 sanitized flows) and **the drain does not** — closing every §3a escape site at once moves `sanitized_flows` and `violated_claims` by 0, at a cost of one evidence item. Because that arm is a ceiling it is a proof about the whole family, which answers the 2026-09-07 concern that escape-closing work "can only delete findings" — and answers it with an instrument demonstrably able to see suppression, since the same instrument measured 158 units of it one column over.
- **INV-karud's clause (b) is measured live for the first time, with its positive control in the same run ([T7]).** "A barrier for a different category must not clear the flow" had never been tested; the 2026-08-06 thread concluded no configuration produces a barrier walk at all. Three arms over one pretix survey, the two sanitizer catalogues differing by exactly one field: same-category (`untrusted_input`) suppresses **158** flows, cross-category (`plaintext`) suppresses **0**, and no-declaration suppresses **0** — the cross-category and no-declaration arms identical on all seven claims in verdict, evidence and sanitized count. The positive control fired, so the zero is a measurement and not a blind spot.

- **`scripts/measure-catalogue-exposure.py` — how many catalogue rows can only be matched with evidence the analyzer may not have.** INV-linub carried a hand-built exposure table that ranked languages by method-kind share alone and concluded `c/cpp/elixir/erlang/haskell: 0% method-kind — NOT at risk from this class`; WI-lajus falsified that for C a month later, through ambiguous names carrying no module slot from `#include` — a route needing no method-kind rows at all. The census reads production's own `load_catalog` and splits every shipped catalogue two ways: `R1` (method-kind, needs the receiver's type) and `R2` (a name the catalogue itself lists in `ambiguous_names`, needs module context). A dual row is fixed by receiver typing alone, because typing the receiver is what supplies the module; the genuinely distinct population is R2-only — function-kind, ambiguous-named, no receiver to type — which receiver typing cannot reach at any quality. All five dismissed languages carry it. Counts are reported both per row and per distinct qualified name, because the prototype reported 16 and 22 for the same cell and a single number would have recreated the second silent home. A catalogue that declares no `ambiguous_names` is distinguished from one that declares an empty list, and an unknown language raises rather than returning a well-formed row of zeros.
...
#### Choice-shaped sources: a measured decision NOT to build a field (WI-jivih, arc T8)

- **`untrusted_input` does not distinguish a value the far side AUTHORED from one it merely CHOSE from a constrained set, and that is now a dated declared blindness rather than a pending fix.** Of the 324 catalogue names that mint the label, 24 return a chosen value — `accept`'s peer address in seven languages, erlang's `inet` DNS answers, the exit status from `os.wait*`. A finding rooted at one can only ever be a *selection* finding, never an injected payload, so a claim whose question is injection reads a stronger fact than the label states. A `violated` verdict whose evidence sits on such a source now carries the **`choice_shaped_source`** caveat, naming the primitives and dating the decision — the only caveat in `verify-claims` that rides the **violated** path rather than a clean one, because that is where this blindness costs the reader something.

- **The field was declined on its own pre-registered rule, not on judgement.** WI-jivih fixed the threshold *before* any number existed: under a 5% finding-weighted share, a per-row `value_shape` attribute is not worth its cost. A 23-repository cohort sampled **deliberately for socket ownership** — chosen to be maximally favourable to the property — returned **2 of 568 findings, 0.35%**. And it is not one of the earlier void zeros: this corpus plainly *sees* the population, 87% of it reaching a choice-shaped chain across 12 primitives and 6 languages. All three terms are measured rather than inferred — presence 29.8%, reachability 87%, findings 0.4% — filling in the project's *presence ≠ reachability ≠ truth* rule for the first time. **Reaching a choice-shaped row almost never produces a finding.**

#### The SELECTS role on a sink argument (WI-bulag, arc T9)

- **A catalogue row can declare that an argument merely NAMES the resource its sink acts on, and a flow that can only be naming one is disclosed instead of counted (`VERIFY_CLAIMS_SCHEMA_VERSION` 2.3 → 2.4).** The caddy-config-to-`os.Chmod` shape: tainted data reaches a sink, but all it does is select *which* file. It stays a **true positive on the correctness axis** — WI-gohok's 2026-08-27 ruling is unchanged — and is excluded on *usefulness*, structurally and per sink, never by a per-case judgement of how interesting a finding is. Rows carry `resource_naming: {<fn>: {names: [...], content: [...], danger: bool}}`; the new per-verdict `resource_naming_flows` count is the disclosure, and the confirmed-path message says it in words ("they are true flows excluded as not useful, not an absence of flows") so a clean-looking verdict cannot hide it.

- **Suppression requires a third condition the ruling did not name, because the walk carries no argument identity.** Two declared bits decide danger, as ruled. But `TaintFlowFinding` records only the sink primitive/module/symbol — there is no `arg_index` anywhere — so at a **mixed** sink like `fprintf(stream, fmt, ...)` a finding cannot be attributed to an argument, and suppressing it would kill one whose tainted value reached `fmt`. Measured over the top-20 reached head (476 of 706 mentions), mixed sinks are **58.0%** and the soundly-suppressible class is **7.1%**. So suppression additionally requires `content: []` — an **explicit** declaration that the sink takes no content argument. Absent and `[]` are different: absent is cannot-determine and never suppresses.

- **The annotation is keyed per FUNCTION, not per row, because a row is not a primitive.** `c.yaml`'s stdio row lists fourteen functions spanning all four classes at once — `fflush` takes no content argument, `fopen`/`fclose` are path sinks where naming *is* the danger, `fprintf` mixes, `printf` has no resource argument. 11 of the 20 head primitives share a row with a different class. Splitting those rows would also work and is deliberately avoided: row **order is load-bearing** here (an unstamped call abstains to the first-declared row), so reshuffling them to carry an annotation would move abstention behaviour for unrelated primitives.

#### Pass silence — why a pass emitted nothing (INV-bikaj / INV-hujog, arc T6)

- **`analysis_runs[].silence_reason` says WHY a pass produced nothing, on a closed axis rather than in prose (SCHEMA_VERSION 0.20.6 → 0.20.7).** INV-bikaj's headline — "61 of 84 passes produce 0 edges" — counts an *undifferentiated* population: a zero-edge pass is "saw no input" (correct and uninteresting), "saw input and found nothing" (a recall defect), or "never really ran because a prerequisite was absent" (an ordering defect), and a reader of a survey could not tell which. Differentiating it first, on 405 pass-runs across five repositories: **274 of 322 zero-edge pass-runs analysed ZERO FILES**, 26 emitted nodes but no edges (not silent at all), and only **22 were truly silent**. The new `pass-silence-reason` axis (`hypergumbo_core.pass_silence`, wired into `multi_value_field_axis._known_axes`) names those states — `no_candidate_files`, `no_candidate_construct`, `dependency_unavailable`, `backend_disabled`, `prerequisite_absent`, `pass_crashed`, `unreported` — and the field is stamped at the two chokepoints that already stamp `nodes_emitted` / `edges_emitted`, so every analyzer and every linker flows through it.

- **The empty value and `unreported` are deliberately different, and that difference is the point.** An absent `silence_reason` means NOT APPLICABLE — the pass emitted something, so there is no silence to explain; `unreported` means CANNOT DETERMINE — it read files, produced nothing, and did not say why. Collapsing them would repeat a defect this project has already paid for once. The orchestrator stamps **only what it derives with certainty** from counters it already holds and never infers `no_candidate_construct`, because only a pass body knows whether it looked for a construct and failed to find one; a reason invented on the producer's behalf is a fabricated disclosure, which is worse than none. **A non-zero `unreported` count is expected, and its size is the size of the remaining work** — the residue is countable instead of invisible.

- **The reason is SHOWN, not just recorded — one line on stderr at the end of a run.** A field that is stamped but that nothing reads back is not a disclosure, and the complaint INV-nanon actually raised was about *observability*, so recording the reason without surfacing it would have discharged the wrong half. `hypergumbo survey` now prints, beside the existing validator summary and under the same discipline (one line, stderr, silent when every pass produced output): `[passes] 57 of 78 emitted nothing: no_candidate_files=52, unreported=5 (per-pass detail in analysis_runs[].silence_reason)`. `summarize_silence` / `format_silence_summary` / `emit_silence_summary` in `hypergumbo_core.pass_silence` are the reusable consumer API behind it. A value **outside** the vocabulary is counted under its own name rather than dropped or bucketed — dropping it would make a drifted producer look like a clean run, the exact failure the axis was declared to prevent — and reason ordering is deterministic (most-common first, ties alphabetical) so two surveys diff without spurious churn.

- **Scope, stated rather than implied.** This ships the RECORDED-REASON half only. The `Pass.requires` dependency graph (INV-hujog) is **not** built: `depends_on` CNF and `validate_pass_dependencies()` have been in `catalog.py` since May with no non-test caller, and the sizing found no `prerequisite_absent` population for a graph to catch, so wiring it would gate a mechanism against a state nothing has yet exhibited. The existing free-text `limits.skipped_passes[].reason` is **not** retrofitted either: it has nineteen distinct spellings in `packages/*/src/` (ten meaning "grammar missing", five meaning "parser failed to initialise", two carrying a trailing space), but across 490 surveys on disk carrying 39,757 skip records only **two** strings are ever produced, because the machine that produced them has every grammar installed — the drift is real in a bare environment and unobserved in ours, so it is filed rather than claimed as a measured cost.
...
- **A node or edge id is minted, never hand-spelled, and the check reads the producers (WI-vodin, ADR-0034 §"Enforcement").** The runtime spec-validator classifies a malformed `Symbol.id` only for ids a given run actually produced, so a linker that fires only on Handlebars templates stayed unchecked until someone surveyed a repository with `.hbs` files — the "static check — known gap" ADR-0034 named. `scripts/check-id-construction` reads the construction sites instead, and the population is why it works: the filed shape `Symbol(id=f"...")` has **exactly one** live instance, while 231 of the 470 `id=` arguments are a bare local and 20 of the f-strings hide behind one, so a check that stops at the constructor reaches 1 site in 470. It resolves three hops — the argument, one in-function local, one same-module helper — and applies the grammar each id actually answers to. **What is forbidden is not the f-string but the ungrammatical id**: a synthetic linker stand-in has no source span and legitimately writes `0-0` by hand, and 14 of the tree's 40 hand-built templates are conformant and stay. Wired into `.githooks/pre-commit` whenever a package source is staged, with `test_live_tree_passes` as the CI gate.
...
- **Taint refutation is confirm-only in practice, and the escape-closing goal is retired (ADR-0052, owner ruling 2026-09-09).** ADR-0017 §3a's removal CAPABILITY is untouched — a flow whose walk returns `unconfirmed` is still removed, and `inclusion_decided_by` still reads `call_graph_reachability_minus_ddg_refutation` — but the programme of closing the walk's escape sites so that removal fires at scale is retired. The blocker is structural: **66.7–90.6% of blocked-walk sites have no callee recorded at the line at all** (a field write, a container subscript, a closure capture), so no function summary can ever reach them. Option (b), field-sensitivity lite as an escape-closing device, was priced before being declined — measurement 0007 found **ZERO of 153 `ddg_mixed` rows across 11 repositories** resting on a walk that ran and established no dependence. The Joern model (assume an unknown callee propagates; let the catalogue narrow) is declined as fail-open, and narrowing refutation's domain is declined as inert. Deciding it needed one further fact, obtained 2026-09-07: with sanitizers declared, closing every escape at once DOES move findings (beads `sanitized 351 → 352`, `evidence 230 → 225`) and the direction is **suppression** — and hypergumbo cannot verify that a declared sanitizer sanitizes anything. ADR-0017 §7a is NOT superseded: its propagation design and its recall value are untouched and still open. The limitation ships as a user-visible disclosure rather than a tracker note (see `walk_verdicts` under Added).

- **What `untrusted_input` asserts is written down, and what it does not (2026-09-06 taint-label concept audit).** The label asserts ADR-0049 ruling 1's fact — a party on the far side had a say in the value — not that the far side authored its bytes: 24 of the 324 catalogue names that mint it (`accept`'s peer address in seven languages, erlang's `inet` DNS answers, the exit status collected by `os.wait*` / `Child.wait`) return a value the far side merely CHOSE from a constrained set, and a claim that means injection is reading a stronger fact than the label states. The spec's label paragraph now says so; the label is not split (WI-vazal) and the boundary is not split (WI-dozul), the property's home is the source-side half of WI-gohok's open mechanism. Two adjacent defects filed: `TaintSink.trust_level` is a documented knob nothing reads (WI-lukoz), and an exit status mints at `os.wait*` but not at `call` / `check_call` (WI-lanos, INV-lozat's acknowledged disagreement). Write-up: `~/hypergumbo_lab_notebook/concept-audit-taint-label_09062026.md`.
...
- **Git notes were never once pushed — 727 of them, since 2026-01-22 (INV-nihaz).** `auto-pr` and `merge-pr` attach the original commit body as a git note whenever a rebase or squash merge rewrites the SHA, and AGENTS.md tells readers to recover those bodies with `git fetch origin refs/notes/*:refs/notes/*`. That command fetched nothing: `git ls-remote origin 'refs/notes/*'` was **empty**, and every body lived in exactly one working copy. Two causes, both now fixed. (1) `.githooks/pre-push` applied its DCO `Signed-off-by` check to *every* ref, including `refs/notes/*` — but a notes commit is written by git itself, carries the fixed subject `Notes added by 'git notes add'`, and **can never carry a trailer**, so the notes ref was unpushable by construction for as long as the hook was installed. The DCO certifies who authored a *contribution*; a notes commit is git's bookkeeping wrapper around a body already signed off on the original commit, so notes refs are now exempt (and nothing else in that loop applied to them either — the protected-branch check strips `refs/heads/` and is inert, and the upstream workspace warning diffs paths a notes ref does not contain). (2) The push itself was `2>/dev/null || true` in `scripts/lib/forgejo-api.sh`, discarding both the error text and the exit status — which is why 726 consecutive failures produced no signal. It stays non-fatal (a notes push must not fail a merge that already landed) but now reports on stderr, so the next breakage surfaces on the first occurrence rather than the 726th. The accumulated backlog was pushed once; a fresh clone plus the documented command now recovers all 727. Filed three separate times in sixteen days (INV-nihaz, WI-sahih, INV-fapub) because the three titles shared almost no vocabulary — a discoverability signal, not a diligence one.


- **Three Apple-SDK macro families break the Objective-C parse, and the grammar that breaks on them is dormant (WI-lafom).** tree-sitter-objc 3.0.2 does not preprocess, so a macro that expands to syntax is a syntax error to it and its error recovery falls back to C — silently, because `has_error` is read by nothing. 74 of 387 real ObjC files across four repositories parsed with an ERROR node, and `@property` never became a node in 14.7% / 26.3% / 11.8% of the files that declare one. Each trigger was established BEFORE the fix by parsing the construct in ISOLATION, never by reading the source at the first ERROR node's line — that line is where error recovery re-parented TO, and reading it as the cause is what gave INV-bisok's residual half a root cause that had to be withdrawn, and what put a `@property (class, …)` line on this item's own suspect list before isolation cleared it. `nullable`, generics, block properties, `NS_DESIGNATED_INITIALIZER`, `NS_SWIFT_NAME`, `FOUNDATION_EXPORT` and a LONE `NS_ASSUME_NONNULL_BEGIN` all parse clean. What errors is `NS_ASSUME_NONNULL_BEGIN` in the WRAPPING position, `NS_ENUM` / `NS_OPTIONS` / `NS_CLOSED_ENUM` (which the item did not name at all), and the availability attributes. **The grammar-bump option is closed, not skipped**: 3.0.2 is the newest published, upstream's last substantive commit is 2024-12-16, and `tree-sitter-grammars/tree-sitter-objc#21` has been open since 2025-02-25 — so this takes INV-bisok's route, a byte-length-preserving pre-parse rewrite behind `TreeSitterAnalyzer.parse_source`, guarded twice (runs only on a file that already fails to parse; kept only when it strictly reduces ERROR nodes, so a macro name in a string, a comment or a `#define` is safe because the bytes are thrown away). The `NS_ENUM` rule consumes a leading `typedef`, because `typedef enum S {…};` has no declarator and yields a MISSING `type_identifier` — a new parse failure inside the fix for parse failures, which the first draft shipped and which counting MISSING separately from ERROR caught. **The availability rule was dropped once on a real number that was not measuring it**: listing Apple's eighteen names exactly recovered ZERO files, because AFNetworking spells its own wrapper `AF_API_AVAILABLE` and a name search over an incomplete vocabulary does not error, it returns clean. Matched prefix-tolerantly, that one macro is 820 of `AFURLSessionManager.m`'s 822 ERROR nodes. A generic `SHOUTY(...)`-before-`;` rule was measured and stays REJECTED — it sheds 17 real `property_declaration` nodes, and a count bought by shrinking the denominator is pure loss. Measured by [measurement 0027](docs/measurements/0027-objc-macro-parse-recovery.md): ERROR files **74 → 38 (19.1% → 9.8%)**, ERROR nodes **1687 → 265**, MISSING **305 → 98**, `property_declaration` **436 → 491**, `class_interface` **322 → 349**, no count falling anywhere; symbols **+133 / +59 / +160**; Mantle byte-identical exactly as pre-registered, its trigger being Quick's test macros rather than Apple's. **End to end on AFNetworking — the case the item was filed about — primitives reached through objc edges go 12 → 21 and `net_recv` 0 → 2**, six `NSURLSession` rows reaching for the first time. That is measurement 0026's pre-registered prediction, which it named as its own falsifier and honoured when it was refuted, now resolved against the INPUT rather than the fix. Recovering the header alone did not do it: with the first two rules `session` becomes a `property_declaration` and `net_recv` is still 0, because the calls live in the `.m`. **What this does not close is stated on the item**: 9.8% of objc files still fail, on one characterised family (a bare argument-less project macro in a declaration-attribute position — `DD_SENDABLE`, `NS_STRING_ENUM`, `QuickSpecBegin`), and the disclosure half — `has_error` read by nothing — is untouched and cross-language.
...
- **A claims file's `extra_catalogs:` block is no longer silently discarded, and the drop was a false all-clear (INV-sisod).** `extra_catalogs` was in the top-level allowlist WI-bopoz built, so the block was accepted and nothing looked inside it: an unknown sub-key, a scalar where a path list belongs, and a non-string list entry were all dropped without a word. Measured on the shipped CLI over one 2-file repository, one claim, one catalogue file, the ONLY difference being the key name — `sources:` gave `violated` (rc 1) and `taint_sources:` gave `confirmed_with_caveats` (rc 3). The typo is the one the documentation invites: `verify-claims --help` introduces the block by naming the FLAGS (`--taint-sources` / `--taint-sinks` / `--taint-sanitizers`) and never the sub-keys, and promises four lines later that "an unknown field name ... produces a clear error and exit code 2 (not a silent pass)". The control is what made the gap sharp — the same catalogue under the CORRECT key with a path that does not exist DOES fail, so hypergumbo checked that a declared catalogue exists and never checked that a declaration was understood. Dropping declared sanitizers yields more violations and is safe; dropping declared sources or sinks yields fewer, and that is the direction this closes. All three now raise at `load_claims` (exit 2) with the existing did-you-mean hint; `load_extra_catalog_paths` stays lenient as documented, since the check now runs first on the same file. Found while running INV-busis's declared-sanitizer arm, by making exactly this mistake — the arm's pre-registered control caught it, and a run without one would have published a confident zero.
...
- **A JavaScript handler ASSIGNMENT emits a registration edge, so `ws.onmessage = h` reaches the catalogue (WI-dosuh).** `ws.onmessage = handler` is the browser receive surface short of `addEventListener`, and it is a property assignment, not a call — every branch of `js_ts.py`'s member-call cascade keys on a `call_expression`, so the assignment emitted **nothing**: not a low-confidence edge, not the `external` placeholder WI-nasuf added for untyped receivers. The three method-kind rows `WebSocket.onmessage`, `WebSocket.onclose` and `EventSource.onmessage` were therefore unreachable by any spelling, and the limitation was *declared in writing* (`analyzer_disclosure.CONSTRUCT_BLIND_ROWS`, dated and evidenced) rather than fixed, because the remedy is a third kind of thing: a suppressed name is a policy that stays, an unemitted method call is a missing edge, and this is a construct the analyzer does not model at all. The declaration is now **removed** — the dict ships empty, its machinery kept and tested against an injected declaration, since a disclosure that outlives its limitation trains a reader to discount every caveat a verdict carries. **Scope was set by parity with the call path, not by the construct.** Asking each receiver spelling twice — once as `ws.send(x)`, once as `ws.onmessage = h` — found **four** spellings the call path already resolves to a catalogue module and the assignment did not (the filed `const ws = new WebSocket(u)`, a TypeScript-declared parameter, the inline `new WebSocket(u).onmessage = h`, and module scope), and three that resolve for *neither*: those are receiver-typing gaps that predate the construct, and fixing them inside it would have meant a second, divergent copy of receiver resolution, so they are filed (WI-ponid: an assignment-bound or field-bound receiver — `this.ws = new WebSocket(u)`, the ordinary class-based client — lands on `external`; WI-vipos: a computed-property call `ws['send'](x)` emits no edge at all, which is worse than `external` because a closed-world verdict cannot even count it) and pinned as known gaps so they cannot be mistaken for this change's failures. The two constructs now share one receiver lookup (`_named_receiver_module`) rather than two copies that could drift into typing a receiver for a call and not for the assignment on the next line. `||=` and `??=` register by the same mechanism and are covered; **the property must be one the catalogue rows** — derived from the catalogue the way `_derive_js_constructor_types` derives its set, so `ws.binaryType = 'arraybuffer'` stays out of the graph and `onopen`, removed from the catalogue deliberately by INV-nular, is not silently re-added. **It was predicted to buy zero findings; measured on the production path, it does not, and the prediction is recorded as refuted rather than quietly dropped.** The reasoning was that taint must also *enter* the handler, and the enclosing-function-to-callback edge is `references`, which is not in `TAINT_CALL_EDGE_TYPES` — so no JavaScript callback-registration source can carry taint. That holds for `addEventListener`, whose argument callback is minted as its OWN symbol (`_cb_addEventListener@<n>`, WI-zavad) with the sink hanging off it behind that uncrossable `references` edge. **An assignment RHS is minted as no symbol at all**, so an inline handler's body is attributed to the enclosing function, source and sink land on one node, and the structural arm reports the flow: `ws.onmessage = function (ev) { exec('ls ' + ev.data); }` is `violated / untrusted-input-no-subprocess`, origin `net_recv`, and it is a TRUE finding — network bytes do reach a shell. The three spellings now differ, which is the honest summary: an INLINE handler finds sinks in its own body (and `addEventListener`, for the identical program, misses them); a NAMED handler (`ws.onmessage = handle`) stays inert exactly as predicted, since nothing links `setup` to `handle` at all — not even a `references` edge, filed as WI-vubal; and the structural arm's co-location imprecision (a source and an unrelated sink in one function) is pre-existing, fires for `addEventListener` identically, and is not introduced here. The asymmetry is not an argument for minting a symbol on the assignment side: doing so would reproduce precisely the defect WI-nisud exists to remove. Fixtures and verdicts: `~/hypergumbo_lab_notebook/dosuh_family_09102026/`.

#### Receiver typing reaches the catalogue

- **A Swift `self.<property>` receiver carries the property's type (WI-dodop).** Writing `self.` in front of a property destroyed the receiver hint for the identical call: `db.write(x)` typed, `self.db.write(x)` did not. `self` parses as `self_expression` rather than `simple_identifier`, so `_walk_nav` collects nothing into `receiver_parts` and the inner `navigation_suffix` — the token that IDENTIFIES the receiver — is consumed as a `method_name` and overwritten by the outer suffix; `hint == "self"` occurs **0** times across 1,913 classified vapor sites. The fix goes where production already routed these sites and was returning `None` — `_swift_receiver_expr_type` (WI-higob slice 2) — not `_extract_call_target`, whose contract several passes depend on. It resolves through `_inherited_field_type` and NOT `_type_of`, which is load-bearing rather than stylistic: `_type_of` consults scoped locals before fields, but `self.db` means the FIELD whatever a local is called, and `method_call_recovery` step 3a treats a stamped `receiver_type_hint` as grounds to REFUTE a class hint — so a wrong stamp would DELETE a correct recovery rather than merely fail to help. A shadowing-local test pins it. A PROJECT property type is typed and still refused from the module slot (INV-kotob); routing it in would be the defect filed against objc as WI-dason, and a test asserts the refusal rather than counting it as a win. Bare `self.method()` is deliberately out of scope — its receiver is the enclosing project type, so it can never fill the slot. Measured by [0028](docs/measurements/0028-swift-self-property-receiver.md) on four repositories, three of which sized nothing: module slot **+442, 0 lost in any module in any repo** (vapor 807→1,059, hummingbird 654→770, Kingfisher 939→987, Alamofire 478→504), `method_edges` identical in both arms, 25 of 25 sampled rows correct. End to end on vapor: io-boundary chains **43 → 59** and distinct primitives **16 → 17**, 0 lost — **though 14 of the 16 new chains are `logging`**, so the severity-weighted result is one newly reachable primitive (`NonBlockingFileIO.readChunked`) plus two substantive chains. The larger bucket — 938 sites where `_type_of` ran and returned None — is untouched and the pre-registered prediction that self would dominate was refuted.

- **C stamps the file's `#include` set on ambiguous bare calls, so `send` / `recv` / `read` / `write` reach the catalogue for the first time (WI-lajus, INV-linub's class in C).** C has no namespaces, so `c.yaml` lists those names under `ambiguous_names` and the F3 gate refuses them outright without module evidence — correctly, since a project-local `send()` exists in the wild. The evidence that lifts the ambiguity is the file's own include set, and the mechanism already existed: `cpp.py` has stamped the comma-joined includes since WI-rupik/WI-mafik and `_module_hint_candidates` (INV-funuf) already splits the slot and normalises `sys/socket.h` to `sys/socket`. C — whose catalogue cpp *inherits* — was the only one not supplying it. The stamp is restricted to `ambiguous_names` and that restriction is load-bearing: a module hint suppresses the permissive short-name fallback, and C reaches most of libc through quoted project headers, so stamping every call made partial evidence worse than none (measured on qemu-dtc: `stdio.fclose` 5→2, `fopen` 1→0, `fprintf` 35→34). Measured by [measurement 0023](docs/measurements/0023-c-include-module-hint.md) on passt / torsocks / libvfio-user: 13 new primitives, **zero lost**, `send` and `recv` reaching for the first time. **It also moves 15 of 21 claim verdicts from `confirmed_with_caveats` to `inconclusive`, with none moving toward `confirmed`** — INV-zimud's ALL-disjunct coverage gate, whose own comment records "the three C repos 0.0%", now bites C because those dsts carry commas, and `c.yaml` declares no `module_completeness` for any header to vouch for them. The direction is the withholding one and the retracted confirmations were on repos whose socket I/O was invisible; the label returns when `c.yaml` gains a completeness block.

- **A `sink_before_source` finding is reported but does not carry a verdict alone** (INV-muhij remedy 3). `walk_blocked_by: sink_before_source` means every sink call site precedes its source in the same function, so the walk had no route to demonstrate — yet ADR-0017 §3a records the field and acts on nothing, which was ratified when the shape was 0.6% of rows and became load-bearing when a primitive that is both source and sink raised it to 25% of the rows INV-lozat adds. The census behind this change read all 14 such rows on a 16-repository cohort against source: **1 was true (7.1%)** and **both** verdict moves resting on such a row alone were false. The remedy is verdict-NEUTRALITY, not removal: a claim whose every surviving flow is unanimously blocked this way no longer reads `violated`, the rows are still reported as evidence, and a new `sink_before_source_only` caveat plus a `details` clause say what was set aside and why. The rule reads `walk_blocked_by_values`, never the scalar — on a collapsed row the scalar is `grp[0]`'s and 133 of 208 such groups on beads are not unanimous, which is the constraint that field's own docstring states. **The item's feasibility question is answered NEGATIVE and the rule is honest about it:** the walk cannot distinguish "no loop encloses either call" from "inside a loop", because the decision in `taint.py` is line-based and the DDG exposes no loop spans there, so the rule is broader than ideal and rests on the census (0 of 14 rows had an enclosing loop) rather than on per-row evidence. Measured on the production path: gocryptfs and caddy each move one claim `violated` → `inconclusive` with the row still reported. Separately, the coverage gate's `inconclusive` downgrade was **dropping every caveat**, so structured disclosure vanished for precisely the verdicts that are least certain while the prose survived only because `details` is spliced; caveats are now carried through.

- **A receiver bound by a LOOP or an optional-binding carries its type** (WI-higob residual, Swift + Objective-C). Two shapes the analyzers had the information for and were not reading. Objective-C: `for (NSString *p in paths)` declares the binding's class in exactly the layout a local declaration uses, and the declared-receiver walk never looked inside a `for_statement` — the next line's `NSFileManager *fm = ...` was already typed, so this was one node type missing from a walk. Swift: `if let x = <expr>` / `guard let x = <expr>` take their RHS's type through the receiver-expression walker from #765, which also means a `guard let task = ... as? URLSessionDownloadTask` reads the CAST and refines `task` from the collection annotation's `URLSessionTask` to what the code actually asserts. PR #760's read-back had attributed part of VernissageServer's residual losses to "guard-let-from-library bindings the file-wide leak matched by name"; this gives them back on a sound basis. Measured, every changed row read back: VernissageServer 228 sites change with **216 untyped → typed**, SDWebImage 8 gains (`NSURL` ×4, `NSString` ×2, `UIColor`, `NSOperation`), Alamofire 12 (4 gains, 4 correct cast refinements), AFNetworking 1; **0 losses on all four** and io-boundaries unchanged (40/30, 8/8, 22/22, 34/30). The first version of the Objective-C half also accepted the loop's bare `identifier` child — which is the ITERATED COLLECTION — so `paths` itself typed as `NSString` and `[paths count]` re-keyed `NSArray → NSString`; the churn reported 0 losses for that run and the defect sat entirely in the re-key column, which is where a wrong type hides because the site still has an edge. Only the declarator binds. Still open and filed rather than guessed: swift `for s in xs` and `xs.forEach { s in }`, which need a collection's ELEMENT type — `_swift_bare_type` refuses `[T]` deliberately, so that is a new rule and must not be a loosening of it.

- **Rust: a chained receiver, and the `?` / `.unwrap()` forms, carry their qualified type into the module slot** (WI-papar, INV-linub L3 residual). Strategy 1.9 already INFERRED the receiver type of `File::create(p)?.write_all(..)` and dropped it when the in-repo `Type::method` lookup missed — which is exactly what an external type does; the inference had run and only the carry was missing. `?` and `.unwrap()` / `.expect()` are the same mechanism (a wrapper projecting `Result<T, E>` to `T`) and are handled together: measured over ripgrep + bellman + tiktoken-rs, a receiver reached through `.unwrap()` occurs 49 times against 29 for `?`, so fixing the syntax form alone would have fixed the rarer half. `_infer_type_from_rust_rhs` normalises a scoped path to a bare name, so the written path is recovered from the scoped callee the way #595's `_var_type_paths` does for annotations — the slot names `std::fs::File`, not `File`. **Two guards, both found by reading the moved rows back:** the call's own callee must not be a projector (`unwrap`'s receiver is the `Result`, not the `File` — without this, 135 of 213 new ripgrep edges were `unwrap` attributed to `std::fs::File`), and the receiver must have been explicitly unwrapped, because `File::create(p)` evaluates to a `Result` and nothing here knows whether a library associated function is fallible. Measured: typed module slots ripgrep 431 → 442, bellman 220 → 233, tiktoken-rs 66 → 67; 11 new ripgrep edges, all read back at source (`file.write_all` ×3, `set_len` ×2, `lines` ×2, `flush`, `h.as_file()` → `winapi_util::Handle`, `sink.stats().unwrap().clone()` → `crate::stats::Stats`), 0 genuine losses, io-boundaries unchanged. The item's estimate of 580 associated-fn sites is NOT reachable: the second guard gives most of it up deliberately, and the reachable population is the 29 + 49 explicitly-unwrapped sites. Separately, the per-site churn rig was found to mis-parse rust dsts — it splits on `:` positionally, so a module containing `::` shifts every field and a re-typed slot reads as one loss plus one gain; that is why the run first reported 42 ripgrep losses where a file:line read-back finds 0.

- **A library-signature catalogue: what a producing function RETURNS** (WI-lalot). Every per-language return-type registry is built from ANALYSED declarations, so a receiver bound to a LIBRARY call cannot be typed at all — there is no declaration in the repository to register. `library_signatures/` supplies that missing fact in the shape the registries already use, so it needs no new resolution path: rows merge with `setdefault` AFTER the analysed ones, and an in-repo declaration always beats a catalogue guess. It is a separate family rather than a `returns:` key on the I/O rows because it is a SIGNATURE fact, not an I/O fact, and its consumer is `var_types` in nine analyzers — the shape INV-tutar cost 134 misclassified rows on. **The merge lands in four places, not one:** `analyze_go` and `analyze_objc` are standalone functions with their own aggregations, and `java.py` builds its registry from `Symbol.meta["return_type"]` rather than from `FileAnalysis.method_return_types` at all (a name-based grep found three of the four — `objc.py` calls its variable `_method_return_types`). Go additionally needed a LOOKUP, not just a merge: a package-qualified producer (`net.Listen(...)`, `exec.Command(...)`) takes the selector branch where `net` is a package rather than a variable, so `func_vars.get("net")` is empty and the registry was never consulted — the merge alone left `ln.Accept()` and `c.Start()` on the `external` sentinel. The user channel `library_signatures.d` is READ from the day it is declared, not merely registered — declaring one nothing scans is the defect ADR-0047 ruling 3 exists about, and a user row replaces a shipped one because the shipped catalogue is stdlib-only. Python's `TYPE_PRESERVING_MEMBERS` is now DERIVED from the rows rather than enumerating ten `pathlib.Path` members beside them, closing the second home its own comment had already identified. Measured, both arms cold: **io-boundaries move on NONE of caddy, gocryptfs, guacamole-client, AFNetworking or Alamofire** — traced to the fact that `ln.Accept()` already produces a `net_recv` chain without any row, so the item's premise that these rows unlock otherwise-unmatchable catalogue rows does not hold for the stdlib rows shipped here. What does move is the analyzer's module slot: 43 sites on caddy and 274 on gocryptfs go from `external` to their true package (`os` 191, `os/exec` 81, `net` 19, `io` 2 — `file.Truncate`, `cmd.CombinedOutput`, `conn.Close`), with 0 regressions in either repo, dropping gocryptfs's `external` bucket 704 → 429. Since the counts do not move, the io classifier was already matching these by short name; the gain is that a match resting on the name `Close` now rests on the receiver's package. The rows WI-lalot measured as inert (`gin.Default`, `fiber.New`, `echo.New`) are third-party and belong in a community overlay under ADR-0047, filed separately.

- **Swift: a call binds a project member only when its ARGUMENT LABELS admit it** (INV-fatap). `swift.py` resolved `recv.method(...)` on a typed receiver as `f"{type_name}.{callee_name}"` in the symbol tables — by BARE method name. Swift identifies a method by its labels, so `removeItem(at:)` and `removeItem(atPath:)` are different methods, and a project extension declaring one captured a call to the other: the catalogued Foundation boundary vanished behind a resolved edge to a member the call cannot compile against. PR #757 closed the static-vs-instance half of the same mis-binding; this is the label half, verified live before the edit with a label-compatible control in the same fixture. `Symbol.meta["arg_labels"]` records what a declaration requires — the first `simple_identifier` of each `parameter`, with `_` recorded as `None` so an unlabelled call compares equal — carried as data rather than re-parsed from `signature`, whose parameter types contain commas. The refusal is deliberately one-sided, firing only on a label the CALL supplies that the DECLARATION does not have, because every stricter direction loses true binds: a defaulted parameter is called with a subset of its labels, a trailing-closure call supplies no `value_arguments` at all, and a symbol carrying no labels has nothing to compare. Swift keys `local_symbols` by `Type.method`, so overloads collapse to one entry; when the surviving entry is not the overload being called, withholding is the honest answer and the linkers may still recover it. One correction to the filing, which named Alamofire's `Tests/FileManager+AlamofireTests.swift` as the live instance: both of that file's `removeItem` declarations are `static`, so #757 already covers them — the defect is real and reproduces on the minimal fixture, but that named instance had already been closed by the sibling fix.

- **A Swift file the grammar cannot parse is retried through a byte-preserving rewrite** (INV-bisok, the Swift-Testing half). The item recorded the residual family as the `@Suite` / `@Test` macro attributes, on the evidence that 225 of VernissageServer's 263 error-parsing files are Swift Testing files whose first ERROR node starts at `extension ControllersTests {`. Bisecting the constructs refutes that — `@Suite`, `@Test`, `@Test` with arguments and `#expect` all parse clean under tree-sitter-swift 0.7.3; the reported line is where error recovery re-parented TO, not where the failure is. The real triggers are a backtick RAW IDENTIFIER containing spaces (Swift 6.1's spelling for a test name) and `try` inside an `if` / `guard` / `while` condition. `TreeSitterAnalyzer` gains one template method, `parse_source`, whose default is the single parse every analyzer already did; an analyzer whose grammar LAGS its language may override it to rewrite bytes it cannot parse, under two rules the base class states because they are what make the hook safe — the rewrite must preserve byte length so every span still points at real source, and the returned tree must be the tree of the returned bytes, because Pass 1 stores the source it was given and Pass 2 re-parses that. Swift's override is guarded twice: the rewrite runs only on a file that already fails to parse, and its result is kept only when it strictly reduces ERROR nodes, so a parseable file is byte-identical through the hook — which is what stops a backtick or a `try` inside a string literal from being rewritten. Measured against the item's own acceptance test: VernissageServer's error-parsing files go 263 of 906 → 5 and its ERROR nodes 10,364 → 19; Alamofire's five are a different family (`#if` in a position 0.7.3 still mishandles) and are correctly left alone. The 5 residuals are `if`-conditions written across continued lines, `switch try await`, three identifiers containing a literal U+200B zero-width space in Vernissage's own source, and one `#if os(Linux)`; extending the rewrite's line scope to `switch` was measured and fixes zero further files, so it stays narrow. After the rewrite no corpus file parents a declaration under an ERROR node, so the ERROR-scoping rule from #760 keeps its test on `switch try await`, which is outside the rewrite's line scope by design.

- **Swift: a receiver that is an EXPRESSION carries its type, not the `external` placeholder** (WI-higob slice 2). `store.session().invalidateAndCancel()` names no receiver variable, so the chain head was all `_extract_call_target` could report and every such site fell to the placeholder — while the SAME call one binding apart (`let s = store.session(); s.invalidateAndCancel()`) had been typed through the return-type registry since #760. One walker now answers the whole receiver-expression family by mechanism rather than by example: a call result (recursively, so `a.b().c().d()` is walked), a constructor, a cast (`(o as! FileManager).removeItem`), a parenthesised expression, and any of those under `try` / `await`; a parenthesised tuple and a cast to a collection are not receiver types and stay untyped. The same walker replaced `_swift_call_result_type`'s own unwrapping, so a chained or `try`-wrapped INITIALISER (`let f = try FeaturedUser(...)`) is typed by the one rule too. Reading every moved row back against source found two producer defects this amplified, both fixed where the fact is registered rather than where it is read: `-> Self` was registered literally and put a module of `Self` on 76 Alamofire sites — a present-but-wrong hint, INV-kotob's defect under a new spelling — and now resolves to the enclosing type; a generic PARAMETER return (`func pick<U>() -> U`) names nothing at a call site and is refused, where a constrained parameter's BOUND (`<V: Codable>`) is still a real type. Measured, per-site: Alamofire 330 of 10,699 sites change — 322 gain a receiver type (286 project types into `receiver_type_hint`, 21 external types into the module slot, 8 calls now resolving to project methods), 0 losses, 0 re-keys; VernissageServer 114 of 12,544 — 112 gain, 0 losses, 2 re-keys, both a local shadowing a same-named parameter (the scope map is per-function, not per-statement, so one of the pair is corrected and the other regressed). io-boundaries are UNCHANGED on both (Alamofire 40 edges / 30 chains, VernissageServer 8 / 8): the gains are overwhelmingly project types, which ADR-0051 keeps out of the module slot by design, and no external gain lands on a catalogued method-kind row these repositories call. Residual, filed rather than guessed: an imported C function whose name is UpperCamelCase (`UTTypeCreatePreferredIdentifierForTag(...)?.takeRetainedValue()`) trips the analyzer's constructor-by-convention rule and puts a function name in the slot — 4 sites on Alamofire, 0 on VernissageServer. ADR-0006's variable-type-inference support table now carries measured Obj-C and Swift rows in place of *not assessed*; Scala stays not assessed.

- **Rust: a method call on a receiver whose type the signature declares emits its edge even when the method name is on the generic-trait denylist** (INV-pamis). `sock.send(payload)` with `sock: &UdpSocket` emitted nothing, because the denylist that keeps `x.clone()` / `v.into()` on untypable receivers from binding to arbitrary impls (or bloating the graph) was applied before the receiver was looked at; the type was already resolved through `use std::net::UdpSocket`. A typed receiver is the evidence the denylist lacks, so the edge now carries `std::net::UdpSocket` and the catalogued net_send row is reachable; untypable receivers keep the denylist and its verify-claims disclosure. Measured: the filed fixture emits `rust:std::net::UdpSocket:0-0:send:unresolved` and tags a net_send boundary; on bellman / tiktoken-rs / ripgrep 42 / 48 / 133 previously-suppressed calls on typed receivers now emit (1.7% / 15.8% / 1.2% of call sites — the blind emission the denylist was built against cost 33-67%), 0 losses, and io-boundaries are unchanged (0 / 0 / 25 chains): none of the three crates calls a denylisted catalogued sink on a typed receiver.
- **Swift grammar bumped from tree-sitter-swift 0.0.1 to 0.7.3** (INV-bisok). The 0.0.1 wheel could not parse a `#if` block inside a type body, and error recovery then re-parented the whole class: Alamofire's `open class Session` was never a symbol and its methods surfaced as top-level functions. Under 0.7.3, Alamofire's files with error-recovered parses drop from 15 of 96 to 5, `Session` and two more classes are minted, 170 members move from top-level functions to methods (1,687 → 1,857 methods), and the io chains go 31 → 30: two re-keyed from the mis-kinded functions onto their methods, one false attribution under the old parse removed, none lost; VernissageServer unchanged (263 error-recovered files, 8 chains). VernissageServer's Swift Testing files (`@Suite` / `@Test` macros) are beyond even this grammar and stay error-recovered; the receiver-type scoping's error-node rule from #760 keeps them usable.
- **Objective-C consumes the return-type registry** (WI-higob, INV-dihos phase 6 for objc). Every `- (T *)sel` registers `<Class>.<sel> -> T` in Pass 1 (`instancetype` resolves to the class; `id`, primitives and `void` do not), and Pass 2 types an `id x = [obj sel]` local and a NESTED receiver `[[obj sel] frob]` from it, so the outer send names the class the inner one returns. Same dict WI-lalot's loader will feed with Foundation rows (`[[NSFileManager alloc] init]` needs them), and a fixture feeds two by hand to prove the path. Measured: per-site churn AFNetworking 7 and SDWebImage 8 nested sends typed through registered returns (project singletons such as `[[AFImageDownloader defaultInstance] imageCache]`, a Foundation `NSURLCache` returned by a project method, a category's `NSString`), 0 losses, 0 re-keys; io-boundaries unchanged (15 → 15, 8 → 8) because the chains that would reach the catalogue stop at Foundation returns — `[[obj session] configuration]` — the loader's rows will supply.
- **Swift consumes the return-type registry** (WI-higob, INV-dihos phase 6 for swift). Every `func ... -> T` registers `<Owner>.<func> -> T` in Pass 1 (optional unwrapped, generic arguments dropped; collections, tuples and closures are not receiver types), and a local bound to a call result — `let s = store.session()` through the receiver's type, `let s = Store.make()` through the type head, a bare `let s = makeSession()` through the enclosing type and then the free function, `try`/`await` unwrapped — takes that type, so `s.dataTask(with:)` names `URLSession`. It is the same dict WI-lalot's loader will feed with library rows, and a fixture feeds one by hand to prove the path. The fixture also exposed that swift's receiver map was file-scoped (a later `let s` in another function overwrote the earlier one, and one function's parameter typed every same-named receiver in the file); receiver types are now scoped to the declaring function by parent chain, file-level declarations stay visible, optional annotations (`var task: URLSessionTask?`) name their wrapped type, and class-level properties join the repo-wide field-type registry so a bare receiver inherited from a base class in another file (`session.request(.get)` with `session` declared in `BaseTestCase.swift`) is typed through the enclosing type's `base_classes`. Measured: per-site churn against the merged dev: Alamofire 331 of 10,697 sites change — 89 gain a type, 76 untyped calls now resolve to project methods through inherited properties, 43 re-keys, and 105 lose the type the file-wide leak had given them (read back: wrong by construction — `request` typed `URLRequest` where it is Alamofire's `Request`; loop / closure-bound receivers the leak matched by name; and Session.swift casualties of its error-recovered parse, INV-bisok); VernissageServer 226 change — 72 gain, 137 lose leak-given types that were `for`-loop or `guard let`-from-library bindings the leak matched by name; io-boundaries unchanged on both (swift chains 31 → 31 Alamofire, 8 → 8 VernissageServer).
- **Objective-C: a message to a receiver the body DECLARES carries the declared class in the module slot** (WI-higob, objc half). `NSFileManager *fm = ...; [fm fileExistsAtPath:p]` and a `(NSFileManager *)fm` parameter now emit `objc:NSFileManager:0-0:fileExistsAtPath::unresolved` with the same class in `dst_ref` and `receiver_type_hint`, the way a class message (`[NSFileManager defaultManager]`) always did; before, every lowercase receiver took the bare placeholder — 66% of AFNetworking's and 63% of SDWebImage's message sends carried no module — while the catalogue's 123 objc method-kind rows key by exactly that class. A project class stays out of the slot (hint only) — and a category on a framework class (`@implementation UIImage (Transform)`) no longer makes that class "project", which had kept SDWebImage's `NSData`/`UIImage` receivers hint-only; `id obj` stays untyped. Measured: per-site churn AFNetworking 459 of 2,313 sites gain a class, SDWebImage 816 of 4,481, 0 losses, 0 re-keys; the no-module bucket goes 1,306 → 928 and 2,375 → 1,807; io-boundaries AFNetworking 7 → 15 chains (NSNotificationCenter add/removeObserver ×6, NSOutputStream write, NSMutableURLRequest setHTTPBody), SDWebImage 5 → 8 (NSURLSession dataTaskWithRequest, NSBundle pathForResource, NSData writeToURL), 0 lost, each read back at source.
- **Swift: the module slot carries the receiver's TYPE, never its variable name** (INV-kotob; the prerequisite for WI-higob's swift half). `fm.fileExists(atPath:)` on a `fm: FileManager` parameter, an annotated local, a `URLSession(...)` local, a singleton or enum-case local (`let fm = FileManager.default`) or an inline type head (`FileManager.default.fileExists`) now emits `swift:FileManager:0-0:fileExists:unresolved` with the same module in `dst_ref`; before, the hint chain fell through to the receiver's variable name, so `dst_ref.module_path` said `fm` while the id said `external` — on Alamofire 42% of unresolved method-call edges carried such a present-but-wrong hint, which every consumer refuses outright (and the few that "matched" did so by the variable happening to be named like its type). A project type — generic arguments stripped first, so `Protected<State>` is recognised — stays out of the slot and rides in `receiver_type_hint`; a declared uppercase variable (`let AF = Session.default`) is never mistaken for a type; and an instance receiver never binds a `static` member by bare name (a test-only `extension FileManager { static func removeItem }` had captured `fileManager.removeItem(at:)`). Measured: per-site churn Alamofire 786 of 10,697 sites gain a type, VernissageServer 2,236 of 12,544; the receiver-variable dst_ref bucket goes 1,462 → 0 and 2,753 → 0; 0 sites lose a type; the only `resolved → typed` moves are 5 Alamofire calls that had bound to test-only static extension members; io-boundaries Alamofire 27 → 31 python-style chains (6 real `FileManager` fs boundaries surfaced, 1 Tier-2 phantom `URLSession.dataTask` link gone), VernissageServer 8 → 8.
- **Python: `obj.field.method()` carries the module the class typed its field with** (INV-mumov, attribute-chain slice). `svc = Service(); svc.conn.execute(q)` — or `def run(svc: Service)`, or two hops through a project-class field (`app.svc.conn`) — now names `sqlite3.Connection` in the module slot, the same map that already typed `self.conn` inside `Service`. The per-class `__init__` field maps are built once per repo before any file's edges and keyed by class-symbol id (`_collect_class_field_maps`), so a class imported from another file resolves and a same-short-name class in two files is unambiguous; the per-file param-type resolvers moved into `_make_param_type_helpers` for the pre-pass to share. Field paths are bound at the binding site under `<local>.<field>` keys, purged on rebinding, and pruned with their root when a sub-scope shadows it. Measured on pretix / mitmproxy / poetry / meson: python io chains poetry 190→191, mitmproxy 534→535, meson 2873→2880, pretix 1093→1093 (9 new `pathlib.Path` rows, 0 lost, every one read back at source); per-site churn poetry 4 / meson 11 / mitmproxy 7 / pretix 0 gains, 0 losses, 0 re-keys.
- **Kotlin emits a call edge for an instance-method call on an external receiver, carrying the receiver's declared type.** Every gate in the `navigation_expression` branch was `resolver.lookup(...).found`, which a JDK method can never satisfy, so `File(p).writeText(s)` emitted the constructor edge and nothing for the method in both the two-step and the chained form (WI-nasuf's kotlin cell): 89 of kotlin's 93 catalogued sinks are method-kind, and okhttp's 573 files produced 49 io-boundary chains, every one a bare `println`. A typed receiver — a declared local (`val c: Connection = make()`, which was never read), a parameter, a bare or dotted constructor (`java.io.File(p)`), or a chained construction — now emits an unresolved edge whose module slot is the type qualified through the file's imports or written inline, with `call_construct: method` and `receiver_type_hint` for the Tier-2 linkers; an untyped receiver emits the `external` placeholder; and `analyzer_disclosure` flips kotlin's dated blindness declaration — the last one, so every shipped analyzer is now declared sighted and the `analyzer_method_call_blind` caveat's tests re-declare a blind language through a fixture to keep the mechanism covered. kotlin inherits java's catalogue, so `java.sql.Connection.commit` is reached from Kotlin for the first time. Two defects the per-site read-back on okhttp found in the fix itself were closed before it shipped: kotlin keys a nested class's methods by the inner class (`Builder.url`), so a receiver typed by its dotted constructor had missed the in-repo lookup at 146 sites; and `var_types` was file-scoped, so a local declared in one method had retyped a class field used in a later one, which now closes with its function; and a local bound to a builder chain (`MockResponse().setBody(x)`) had lost the chain-root typing the old code gave it, at 95 resolved sites, so the root stays the evidence, and a dotted constructor at the root of a longer chain (`Request.Builder().url(u).build()`) types the local as its outer class, as the old code did and `.build()` returns, whitespace-insensitively, since okhttp writes its chains one segment per line. Measured on okhttp and detekt: io-boundaries okhttp 49 → 52 (socket.connect x2 net_send, logFile.appendText fs_write), detekt 2 → 6 (Path.inputStream / bufferedWriter / toFile, File.writer); every new chain read back against source; 0 lost; per call site with both arms in one process, okhttp 22,196 → 46,555 sites, 4,527 typed gains, 0 re-keys, 8 sites lost a resolution: 4 are a wrong short-name in-repo bind on LinkedHashSet.add corrected to the JDK type, 4 are a class field initialised by a lowercase factory (`val clientTestRule = configureClientTestRule()`) that the old arm typed only through a same-named LOCAL leaking out of another function; detekt 12,724 → 27,172 sites, 5,041 typed gains, 1 lost (the same leak shape, `gradleRunner`). (WI-nasuf)
- **JavaScript emits a call edge for every instance-method call, and a `new`-constructed receiver carries its constructor's module.** The analyzer resolved a member call only when the receiver was a named global whose name is the module (`localStorage.setItem`); a receiver bound by construction — `ws = new WebSocket(u); ws.send(x)` — landed `WebSocket` in `var_types`, the in-repo lookup missed, and nothing was emitted, in JS or TS, at module or function scope, so the eleven rows keyed on WebSocket, XMLHttpRequest, EventSource and BroadcastChannel were inert and the whole XHR egress surface was empty (INV-misup). An untyped receiver (`obj.write(x)`) emitted nothing either, while `helper(x)`, `fs.readFileSync(x)` and `console.log(x)` beside it all did (WI-nasuf's javascript cell). The catalogued constructor types are derived from `javascript.yaml` the way Python derives its constructor table, a namespace-constructed class keeps its module (`new net.Socket()` → `net.Socket`, the row the catalogue keys) in both the assigned and the inline form, an untyped receiver, whatever its expression (`obj.write(x)`, `f().m()`, `a[i].m()`, an inline `new` of an unknown class; `this` keeps its Site-1 path), emits the `external` placeholder with `call_construct: method` as python, java, go, rust, scala and swift do, and `analyzer_disclosure` flips javascript's dated blindness declaration, so a clean verdict stops carrying `analyzer_method_call_blind` and carries the per-site `untyped_receiver` disclosure instead. Read back per call site with both arms in one process: mobx 15,188 → 35,743 sites (every new one a placeholder or a bare-class hint), plausible 3,054 → 5,244 (5 XMLHttpRequest sites typed), workadventure 25,116 → 54,672 (35 XMLHttpRequest and 8 WebSocket sites typed), 0 sites lost a type; plausible's one resolved-to-typed site is `xhr.open` leaving a wrong short-name in-repo binding for `XMLHttpRequest.open`. io-boundaries: mobx 60 → 60, plausible 54 → 59 (all XHR egress, every site read back), workadventure 60 → 78 (XHR in Emscripten's loader inside minified wasm glue, and a service worker's `BroadcastChannel.postMessage`), 0 lost. (INV-misup, WI-nasuf)
- **Python emits the same placeholder for an unknown receiver whether the call is inline or assigned, and an imported class constructor types its instance.** `_unwind_attribute_chain` returned `None` for any chain not rooted at a bare name — `items[i].startswith(x)`, `f().x.y()`, `(a + b).c()` — and the emitting branch then emitted nothing, while the sibling branch for `obj.attr.method()` had emitted the `external` placeholder since PR #231. PR #254 kept that asymmetry on purpose and wrote down its re-evaluation trigger, a consumer that distinguishes "no edge" from "an edge to an untyped receiver"; the ADR-0017 §3a walk is that consumer (no edge means no `callees_at` entry, so the walk records an escape where a §4 summary could have accounted for the step — 50.0% of call-node escape sites on hypergumbo's own tree), so the inline form now emits the placeholder too. Separately, only the I/O-catalogue constructor table typed a receiver, so `f = Fernet(key); f.decrypt(token)` carried `external` and `Fernet(key).decrypt(token)` carried nothing: the four shipped plaintext sources and four sanitizers, every one a `Class.method` row, were unreachable in both forms. A called name that an import binds and that is PascalCase is a construction (the convention WI-jubag already reads for `instantiates`), and the import is the binding, so the instance is typed `cryptography.fernet.Fernet` exactly and a namesake from `decoy` names `decoy.Path`, never the catalogued row. Read back per call site with both arms in one process: mitmproxy 22,320 → 23,429 sites (315 typed, 0 lost), poetry 15,345 → 16,263 (186 typed, 0 lost, 3 re-keys where a constructor's concrete class replaced an annotated base), pretix 69,755 → 78,852 (748 typed, 0 lost, 0 re-keys); io-boundaries on mitmproxy, poetry and meson byte-identical (534 / 190 / 2,873 chains) because the newly typed modules carry no rows; on hypergumbo's own tree the escape-cause table moves one site from "no edge" to the placeholder class and the escape count does not move. The crypto rows are reached on real code: mlflow's and litellm's AESGCM `decrypt` sites go from `python:external:0-0:decrypt` to `cryptography.hazmat.primitives.ciphers.aead.AESGCM.decrypt`, accepted by the production source matcher. A PascalCase factory function (`tempfile.TemporaryFile`) names itself as the type, which reaches no row. (INV-luhug, WI-makij, INV-mumov)
- **Java reads the type the source DECLARES, and consumes the return-type registry it had only built.** `java.py` typed a local only from its initializer — `new T()`, or a resolved in-repo method's return type — so `OutputStream o = sock.getOutputStream(); o.write(b)` glued the variable name into the callee (`java:external:0-0:o.write`) with the type one token to the left; on guacamole-client that was 315 typed bindings out of ~2,180, with 1,210 call-initialised locals, 358 catch parameters, 163 for-each variables and every try-with-resources declaration never typed. Declared locals, `catch (T e)`, `for (T x : xs)`, `try (T r = …)`, an explicitly typed lambda parameter, a record's components, a generic or inline-qualified field (`Map<String, String> attrs`), and a field DECLARED IN AN ANCESTOR (`protected RealmModel realm` in keycloak's abstract providers, qualified through the parent's own imports) now bind the declared type, a constructor initializer still narrows it to the concrete class, and a return-type INFERENCE never overrides a declaration — the lookup behind it is name-keyed, and on guacamole-client it answered for an enum constant's covariant override and for an anonymous class's `get()` reached through a chained receiver, retyping a declared `String` as `Future`. Locals are scoped to their method (a scope stack keyed on the method's end byte, so an anonymous class body inside the method closes its own scope and not the enclosing one); before, `File f = new File(p)` in one method typed a sibling method's `Socket f` FIELD as `java.io.File`. The module slot qualifies a declared type through the file's imports, an inline `java.sql.Connection`, a nested `Map.Entry` on an imported outer, a wildcard package, or — for the closed list of `java.lang` public types only, since a forgotten `import java.io.InputStream` must not become `java.lang.InputStream` — the JLS 7.3 implicit import, so `Runtime rt = Runtime.getRuntime(); rt.exec(cmd)` reaches the catalogued subprocess sink. WI-kuroj's `method_return_type_registry` was built in `analyze_java` and never passed to `_extract_edges` (write-only, WI-doluf's Go Defect 1 in Java); it is now the one interface a return type is read through — keyed `<Owner>.<method>`, valued with the type qualified through the DECLARING file's imports or the bare name of a project class, the same shape WI-lalot's library rows will feed — consulted at the assignment, the typed-receiver and the chained (`s.getOutputStream().write(b)`) sites, so `var w = F.makeWriter(); w.write()` across files names `java.io.FileWriter` where it named `w.write`. ADR-0006's Java "Chained" column flips. Read back per call site on guacamole-client (9,105 sites, both arms in one process): 1,662 untyped → typed, 257 → project-class hint, 190 hint → typed, 0 sites lost a type, and every one of the 36 re-keys is the old arm carrying a type leaked from another method's parameter (`user.setPassword` on an `ActivityRecord`); io-boundaries 38 → 40 chains, 0 lost; keycloak (386,340 sites): 67,400 untyped → typed, 12,419 → project-class hint, 3,906 hint → typed, 166 sites lost a type — 80 bare lambda parameters, the rest `var` bound to a call the resolver cannot see or a project-class field inherited from a parent, every one a type the old arm had only by leak, three of them wrong (`FirefoxOptions` on an HtmlUnit receiver, `ProviderConfigurationBuilder` on a Quarkus one, `Future` on a `String`). (INV-vugon, WI-gajuh)
...
- **Java's stdin read is no longer a filesystem read (WI-tusav).** `java.io.BufferedReader.{readLine,read}` and `java.util.Scanner.{next,nextLine,nextInt}` shipped at a FIXED `fs_read` — the Scanner row's own note read, in full, "When reading from a file" — so the canonical `new BufferedReader(new InputStreamReader(System.in))` reported a host-filesystem crossing at the call that transfers the bytes. They are now dual `fs_read` / `ipc_recv` under `call_site_undecidable`, joined by `java.io.InputStream.read` and `java.io.InputStreamReader.read` (the first absent entirely, so `InputStream is = System.in; is.read();` had matched nothing at all), and `java.py` stamps `io_target_kind` by resolving the receiver's last binding in the enclosing method and unwrapping the decorator chain — java's wrapper is a NESTED constructor, two deep where go's WI-vutav shape was one call, so the unwrap is an explicit bounded loop. The filing asked that the constructor rows be checked first, since an `fs_read` fallback with nothing behind it would leave the stdin crossing represented nowhere; checked, and java emits no `calls` edge for `object_creation_expression` at all, so the fallback is `java.lang.System.in`'s ATTRIBUTE row, which fires on the `module_attr_ref` edge. The pre-change state was therefore a MISCLASSIFICATION rather than the total absence the item recorded: on a four-method fixture, `fs_read: 4` (both the stdin and the file readers) plus `ipc_recv: 2` at `System.in`; after, `fs_read: 2` / `ipc_recv: 4`, with exactly the two stdin reads moved, both file reads unmoved and the total unchanged. `net_stream` is deliberately never stamped — narrowing keeps only rows whose boundary equals the resolved one, so a stamp with no matching row would empty the candidate list and delete the classification outright.
- **C++'s `std::getline` had no row, and C's POSIX `getline` none either (WI-bapuk).** The idiomatic C++ standard-input read — `std::getline(std::cin, s)` — produced NO source at all: `std::cin` is rowed as an attribute and fires where it is literally written, but the call that TRANSFERS the bytes was invisible, and a read over an `ifstream` was equally invisible. A fixed row would have been wrong for the same reason `fgets` needed splitting: the stream is an ARGUMENT (INV-bagok / INV-zumin class (b)). C's is a ROW-ONLY addition — `getline(&buf, &n, stream)` takes its stream third exactly as `fgets` does, so `_C_STREAM_ARG_INDEX` gained one entry and no new code. C++ needed two changes. **The module slot was the blocker, found by probing before writing any row** (LIVE rule 9): cpp.py emitted the comma-joined list of every `#include` in the file as an unresolved call's module, discarding the namespace the call names, so `std::getline(...)` arrived as `iostream,fstream,string` and a `std.getline` row could never have matched — while the ATTRIBUTE path already emitted `std` for `std::cin`. Measured over the corpus: of 78 C++ files containing a `getline` call, `string` appears in 51.3%, `sstream` 48.7%, `fstream` 47.4%, `iostream` 38.5%, and 14.1% include none, so rowing under any single header would have reached at most half the population. `std` is now ADDED to the module disjunction rather than replacing the include list (classification asks whether ANY disjunct names a primitive, so adding one can only add matches; replacing would delete `std::printf`'s match against the inherited `stdio` row), and a `using namespace std;` scan lets the bare `getline(cin, s)` spelling resolve. The stamp reads the DECLARED TYPE rather than chasing a value origin, because C++ writes it down: `std::ifstream in(p);` is the most-vexing-parse shape, a `declaration` whose type node is `std::ifstream`. **A/B over apt / modsecurity / sherpa-onnx, diffed both ways: +28 chains, 0 removed, 0 moved** — 27 `fs_read` and one `ipc_recv`, and that one is `getline(cin, Res)` at `apt/cmdline/apt-cdrom.cc:71`, a real stdin read that was previously invisible. **An earlier draft of this change DELETED two findings**, and the naive diff hid it: naming `std` in the module slot for a file with NO system includes replaced the `external` sentinel, which is what enables `lookup_with_module`'s short-name fallback, so two `std::chrono::system_clock::now()` chains in modsecurity's `collection_data.cc` went `host_info_read` → nothing. It surfaced only on a module-stable diff key — changing the module slot changes every `dst` string, so the naive diff reported apt as 112 added / 102 removed and buried the two real removals in re-keying churn. The sentinel is now preserved, pinned by a test that was falsified before being kept.
...
- **Python's third completeness leg: rows for the 41 stdlib module strings that still reached the coverage gate uncatalogued, then 44 dated grants (WI-dupok, WI-lutuh's rust → javascript → python order completed).** After the 2026-08-15 climb the 08-24 scope audit still found glob, threading, unittest, struct, pickle, errno, atexit, queue, string, ctypes, multiprocessing, xml.dom.minidom, socket, ssl and 27 singletons uncatalogued across 11 corpus repositories. A python probe (`dupok_python_09062026/probe_py.py`, the twin of the rust-src and node probes: public members one level deep, `inspect.getsource` scanned for `open(`/`os.`/`sys.std*`/socket signals, C-implemented members opaque, never grants) sorted the set, and rows came first: glob walks directories; mimetypes opens the mime.types files; filecmp stats and reads; sysconfig stats config files and reads `os.environ`; pprint, traceback, unittest, doctest and optparse write their default streams; cProfile and pstats dump and load stats files; `codecs.open` is `builtins.open`'s dual shape; `ctypes.CDLL` / `cdll.LoadLibrary` / `ctypes.util.find_library` load a shared object from a path (INV-buzab's live false confirm); `multiprocessing.Process.start` and `Pool` launch, `kill`/`terminate` signal; `socket.create_connection` (`net_send`) and `create_server` (`net_listen`); and `ssl`, which had no rows at all, gets its `SSLSocket` / `SSLObject` / `SSLContext` surface. Two survey rules applied: a read from a caller-opened object is the caller's I/O (`pickle.load`, `marshal.load` get no row), a write into one is rowed where the value enters it (`pickle.dump` keeps `fs_write`, `json.dump`'s precedent). Refused with reasons in the yaml: tkinter (a display is a boundary the vocabulary does not name), socket (its DNS functions are unruled in every catalogue — escalated as its own item), ssl (rowed, partial), unittest.mock, multiprocessing's Manager/shared_memory/connection surface. Three gates followed the rows: `exit` joins `ambiguous_names` (`optparse.OptionParser.exit` collides with the builtin), `multiprocessing.Process.start` and `Pool` join `HIGH_RISK_PRIMITIVES`, and the examined-negative tests that pinned INV-buzab on `ctypes` and `socket.create_connection` graduated — both now classify, the principle is repointed at `termios` and `socket.gethostbyname`, and the graduated fixtures stay as positive controls. A/B on the five-repository python cohort (httpx, full-stack-fastapi-template, poetry, django and hypergumbo itself; cold caches; four claims; `compare_arms.py old new`): 0 of 20 verdict categories moved — every surviving `inconclusive` is held by another rung (a javascript half with no call edges, opaque launches) — and the new rows are visible as recall in the chain counts, django 606 → 638 `fs_read` and 23 → 24 `subprocess`, hypergumbo 2016 → 2026, poetry 205 → 208, httpx 7 → 9. No verdict became confirmed on this cohort; the grants' positive control is the gate test (a call into `struct` is now adjudicable).
- **The javascript catalogue is audited and granted: ten node builtins become examined negatives, 76 rows are added, seven are deleted, and the catalogue is promoted to `provenance_declared` (WI-nolut, WI-lutuh's second leg; WI-jojaf).** javascript.yaml declared zero `module_completeness` grants, so no node module could ever be an examined negative and every clean verdict over a repository calling an unrowed builtin was withheld. A mechanical probe (node v20, exports one level deep, function source scanned for binding reach and I/O surface; it never grants) found `fs/promises` wholly unrowed (13 exports with binding reach beyond the 17 already rowed), 91 unrowed `fs` exports (open/read/write/fstat/symlink/link/truncate/utimes/rm/cp/watch/opendir and the fd family), the uid/gid getters, `openStdin`, `chdir` and `dlopen` on `process`, and — INV-nular F7 — that `path`'s seven rows (`join`, `dirname`, `basename`, `extname`, `relative`, `normalize`, `resolve`) carry no signal at all: the row's own note said "not I/O" and declared `fs_read` anyway, so every `path.join` counted as an examined filesystem read. Those rows are deleted (python's `os.path.abspath` / `posixpath.abspath`, the same shape, go with them), the gaps are rowed (`fs.open` dual `fs_read`+`fs_write` in python's `builtins.open` shape), and `path`, `url`, `zlib`, `timers/promises`, `stream/promises`, `crypto` (on the recorded python entropy precedent), `fs`, `fs/promises` (both spellings) and `process` are granted, each dated and citing its probe verdict. `readline` (its I/O is the stream argument and the analyzer has no js `io_target_kind` stamp) and `http` (`new http.Server()` is a constructor the catalogue does not key) are deliberately withheld and keep withholding. The dual `fs.open` also made javascript the third language with a mode-decided primitive, and the parity test refused it until the analyzer could produce the mode: `_MODE_ARGUMENT_POSITIONS` now declares node's flags string at positional 1 (`fs.open` / `fs.openSync` / `fs.promises.open`, resolved for typescript through the catalogue alias), the one shared producer `stamp_io_mode_from_call` reads a javascript string literal (`string_fragment`, where C and python spell it `string_content`), and `js_ts.py` stamps `io_mode` on every call it emits — so `fs.open(p, 'w')` tags `fs_write` and an absent or computed flags argument keeps node's documented `'r'` default, pinned for javascript and typescript in `test_js_io_mode_emission.py`. A/B on eleven javascript corpus repositories, cold caches, four claims each: 0 of 44 verdict categories moved. Every surviving `inconclusive` is held by a rung the grants do not reach: a `console` method-shape withholding (express, ioredis, winston: the rows are `methods:` and the analyzer emits the calls without a method construct, so the coverage gate withholds while the classifier tags them — filed), and languages with no catalogue (vue, ruby, php/powershell, solidity — and `typescript` named as having none although it aliases javascript's, filed). The F7 deletion moved no NO-FSREAD verdict because every cohort repo reads files elsewhere. The positive control is the gate test: a call into a granted module now reaches `coverage.complete`, which no javascript call could before.
- **A C socket send or receive is classified by the descriptor's FAMILY, not by name (WI-baran, INV-nular's c member from measurement 0012).** `sys/socket.{send,sendto,sendmsg}` were `net_send` and `{recv,recvfrom,recvmsg}` `net_recv` unconditionally, so tmux — which opens no AF_INET socket anywhere — reported `sendmsg` over its AF_UNIX client/server pair as a network send. The rows are now dual (`net_*` + `ipc_*`, `call_site_undecidable`, `abstains_to: net_*`), and `c.py` stamps `io_target_kind` from where the descriptor was created, one hop back through the variable by the same last-binding helper the stdio stream argument uses: `socket(AF_UNIX|AF_LOCAL, ..)`, a preceding `socketpair(.., sv)` for `sv[0]`/`sv[1]`, and `accept`/`accept4` over such a listener stamp `pipe`; `AF_INET`/`AF_INET6` stamp `net_stream`; a parameter, a struct field, a kernel-family socket (`PF_NETLINK`, `PF_PACKET`) or an unknown family stamps nothing and keeps today's answer. Measured before and after on the twelve C repositories in the corpus that call the six functions (324 sites, every arm cold): 27 stamped, 3 moved `net_recv → ipc_recv` (haproxy `src/sock.c:554,585` over `socket(PF_UNIX, ..)`, passt `passt-repair.c:199` over `socket(AF_UNIX, ..)`), 0 removed, every moved row read back against source. tmux's own two sites pass the descriptor as a parameter and do not move — the instance that filed the item needs the cross-function hop (WI-famig), and the census also found that bare `send`/`recv` never reach the catalogue (`ambiguous_names`, no module slot from `#include`) and that fluent-bit binds 116 of its 162 sites by name to a vendored nuttx platform shim; both filed. `connect` stays single-rowed (ADR-0049's deferred shape), and `c.yaml`'s `unistd.write` `ipc_send` note no longer calls `write(STDOUT_FILENO, ..)` an IPC send (WI-dutah rules it `logging`).
...
- **A clean verdict names the completeness grants it rested on, shipped catalogue and overlay alike (WI-lavut, INV-tabaf's other half).** INV-tabaf made an overlay's `module_completeness` grant name the modules it vouched for; the shipped catalogues make the same closed-world claim — python carries 121 dated grants, rust 20, javascript 10 — and turn the same gate off, and a run disclosed none of them, because `catalog_provenance` was built from the overlay files rather than from the run (`completeness_grants` read `[]` on every arm of the javascript and python A/Bs). The grants a verdict *rested on* are exactly the calls the uncatalogued-module gate lets through because every spelling was enumerated; they are now collected in that walk (`_adjudicate_external_modules`, shared with the unknown set so the two cannot disagree), carried as `BoundaryCoverage.load_bearing_grants`, emitted as `catalog_provenance.load_bearing_grants` beside the file-derived key, and rendered in text mode as a `the coverage gate PASSED these modules on completeness grants` block whether or not anything was user-supplied. Only what this verdict stood on is listed, so a python run does not print 121 module names. Disclosure only: no verdict category moves, pinned by controls and measured on both cohorts (measured at the branch tip on four cohort repos with cold caches -- express, pm2, winston from the javascript cohort and httpx from the python cohort: 16 of 16 per-claim verdicts identical to each cohort's own prior arm; the block lists 25 python grants on httpx, 5 javascript + 4 python on pm2, 4 javascript on winston, and none on express, whose granted-module calls are all rowed and whose node:-spelled import edges are a separate emission defect, filed; instrument and read-back in lavut_grants_09062026/).
- **A named global's catalogue rows are function-kind, so the coverage gate and the classifier agree about the same call (WI-komun).** javascript.yaml declared `console`, `localStorage`, `sessionStorage`, `navigator`, one `fs` row (`createWriteStream`) and a duplicate `Deno` row under `methods:`; the analyzer resolves a member call on a named global with the module slot filled and no call construct (`console.log(x)` is `javascript:console:0-0:log`, the same shape as `fs.readFileSync`), so the classifier matched every one of them while `method_starved_modules` — which asks, per declared kind, whether the analyzer handed the catalogue a shape it could match — reported the module "method-shaped but no method call edge" and withheld every clean verdict over a repository that logs (express, winston and ioredis in the javascript A/B). The rows move to `functions:` (the kind was the misdeclaration); the gate is not loosened, and the class receivers (`WebSocket`, `XMLHttpRequest`, `net.Socket`, `dgram.Socket`, `EventSource`, `BroadcastChannel`) keep their method rows and keep starving without a method edge, pinned as the control. A/B on the eleven-repository javascript cohort (cold caches, four claims; `compare_arms.py new_pre_komun new`): 0 of 44 verdict categories moved, and the five claims this rung held all changed REASON to the next rung that honestly holds them — express NO-FSWRITE / NO-SUBPROC and winston NO-NET / NO-SUBPROC to nine uncatalogued third-party modules (`assert`, `body-parser`, `logform`, `rimraf`, …), ioredis NO-SUBPROC to three opaque launches (`docker`, `npm`, `sleep`). No verdict became confirmed, so nothing was silenced; a false reason was replaced by a true one.
- **A synthetic call site minted by a linker carries its host file's language, so a `.js` contract call no longer withholds every claim as "typescript has no I/O catalog" (WI-dovog, WI-mital's family).** The solidity-abi linker stamped the literal `typescript` into the id's language slot and into `discovery_language` for every call site it found, whatever the file's extension, while the synthetic node's own `language` is `None` (ADR-0031 Class B). verify-claims derives the languages that made calls from the src id's first segment and its catalogue map from the nodes' languages, so openzeppelin-contracts — 6,689 such edges from `.js` files, zero typescript nodes — put `typescript` in the one set and nowhere in the other, and every boundary and taint claim over the repository was withheld. Measured on openzeppelin-contracts through the production CLI on cleared caches: before, NO-NET `inconclusive` naming "solidity, typescript" with 6,689 typescript-prefixed `calls` edges (solidity-abi) plus 38 typescript-prefixed `event_publishes` edges (message-dispatch) and 0 typescript nodes; after, NO-NET `inconclusive` naming solidity alone — the rung the grants cannot reach — and 0 typescript-prefixed edges of any type. The sweep by mechanism found six emitters of the same literal, all fixed with `js_ts_language_from_path` and pinned per extension: solidity-abi, tauri-ipc (three sites), message-dispatch and crypto-flow (which also gave a `.ts` read site the write site's language), yjs-crdt, wasm-bindgen's TS-side import stand-in, and grpc's client-pattern label.
...
- **Twenty-six id-emitting sites produced ids hypergumbo's own validator calls malformed, and the dominant defect is a line number where the grammar wants a span (WI-vodin, INV-kurup class).** Found by the static check above, not by a run. `grpc:{path}:{line}:{name}:{kind}` reads right-anchored as a span of `"12"`; `crypto_flow`, `message_dispatch` and `yjs_crdt` wrote `:{line}:0:` where the hyphen belongs, which shifts every slot; `ipc`'s bridge-caller ids carried a name in the kind slot and `openapi`'s carried a URL path there; `bibtex` ordered its slots kind-before-span-before-name; `bitbake` had no span slot at all; `swift-objc` is not a legal language prefix; and `css` returned `css:sha256:<digest>` — three segments, which the five-slot grammar cannot parse at all, so a css symbol's id carried no language, path, span, name or kind any reader or linker could recover. All twenty-six are repaired at the producer. **The css repair is measured end to end rather than asserted**: on a two-file fixture the analyzer goes from four `css:sha256:<digest>` ids to four canonical five-slot ids, and the css `@import` edge reaches the survey for the first time (0 → 1 edge, both endpoints present as nodes) — the exact state `test_import_dst_ids_are_canonical` documents when it excludes css because "their src is a hash-shaped symbol id … that the pipeline drops, so the edges are orphaned". The name slot is routed through `sanitize_id_name_segment` there because a selector may contain a colon (`a:hover`), and a colon in that slot shifts every right-anchored anchor. Three of them are siblings of a site WI-zugob already fixed **in the same file**, which is the pattern this check exists to stop: the named instance gets repaired and the family does not.
- **`Edge.id` gets a format validator — for its own grammar, which is not `Symbol.id`'s.** The deferred Phase 6 PR1 item asked to scan `Edge.id` "for the same canonical shape it enforces on `Symbol.id`". Checked against a live survey before implementing: every one of apollo-server's 18,283 edge ids is `edge:sha256:<16hex>`, a content digest, so the five-slot node grammar would have reported a 100% violation rate on every repository hypergumbo has ever analysed. The two are counterparts in **role** — per-instance identity, against the structural `edge_key`/`stable_id` pair — and not in shape, so the discipline transfers and the pattern does not.
...
- **A `conftest.py` change selected no tests at all (INV-kinin's sibling).** `conftest.py` is neither a `test_`-prefixed test file nor package `src/`, so it matched neither `CHANGED_TEST_FILES` nor `CHANGED_SOURCE_FILES`: smart-test reported "no test-relevant files changed (docs/config only)", wrote a manifest with **0 selected tests**, and `ci.yml` skipped pytest. That is strictly worse than the fixture blindness INV-kinin already fixed, because an `autouse` fixture declared in a conftest governs EVERY test beneath it rather than one gate's inputs. Found the expensive way: a conftest change that *repaired 13 failing tests* produced a 0-test manifest, so the repair and the blindness arrived in the same commit and only the manifest header said so. A package conftest now selects that package's suite exactly as a fixture does; a ROOT conftest selects the root suite **and every package suite**, because pytest applies a rootdir conftest to everything collected beneath it and selecting only `tests/` would under-select precisely where the blast radius is widest. Measured on the change that exposed it: 0 selected tests → 55 (53 tracker suites + the two root smart-test suites), 3,033 passing.
...
- **The bakeoff scripts stop looking for pool repos where they are not (WI-havit).** `--pool ~/ALL_REPOS` is the documented usage and the only pool on the lab machine, and `bakeoff-deep run` died on it with `FileNotFoundError` for every repo living one level down in a collection subdir: cohort SELECTION resolved the repo through `pool_utils.resolve_repo_path` and wrote its correct absolute path into the cohort metadata, then `_run_cohort_repos` DISCARDED that path and re-derived a bare `os.path.join(pool_path, repo_name)`. WI-favav had already fixed this exact shape in `bakeoff-broad`, and `bakeoff-deep` already imported the resolver and cited WI-favav in its selection path — the run path simply never got it. Grepping the family for the mechanism rather than patching the line that failed found **three more sites**, the worst of them inside `bakeoff-deep cycle`: `bakeoff-deep-reflect` handed the reflect agent a source path that does not exist, which does not crash — it reads nothing and reports quietly, so a cycle could select and analyse repos correctly and then verdict them on absence. Both reflect scripts also printed `<pool>/<repo_name>/` as agent-facing guidance, a path that is wrong whenever the pool is nested. All four now route through a `_pool_repo_path` helper mirroring `bakeoff-broad`'s, falling back to the naive join so a genuinely-missing repo still errors where it errored before. The regression tests are split per script — `test_bakeoff_{deep,deep_reflect,broad_reflect}_pool_path.py` — because the top-level test map selects root tests by name prefix and a single family-wide file was reachable from no source but one, which its own guard caught; each copy carries the family-wide enumeration check, so touching any one script re-runs it across all four.
...
- **`smart-test --full` ran nothing and exited 0 whenever the diff held nothing test-relevant (INV-sotam).** The docs/config-only short-circuit exited before the `FULL_RUN` branch was ever consulted, so on a tree whose only changes are docs or config — which is what verifying already-merged work looks like — `--full` printed a reassuring line and ran no tests. A gate that cannot fire read identically to a gate that passed. `--full` now runs the suite from that branch, deliberately KEEPING the empty targeted manifest rather than routing through `run_full_suite()`, whose full-suite manifest CI rejects by design as a change-detection failure — the obvious fix would have traded a silent skip for a red gate on every docs-only PR, and a negative control pins it. The skip now reports `0 tests ran`, so a caller can tell "checked, nothing to do" from "nothing was checked". The same filing's explicit-target half no longer reproduces (`HAS_EXPLICIT_TESTS` is handled well before the short-circuit) and was re-checked by running one; the sibling defect INV-bigaz — the slicer having no data-file edge, so a catalogue change selects ~1 test — is untouched and stays open.
- **A withheld verdict named the blocker that did not withhold it (WI-razuk).** `compute_boundary_coverage` evaluates two blockers — opaque launches and modules the catalogue could not classify — and returns on the first, so a repo with both read *"NOT CONFIRMED: the analysis launches an external program at N call site(s)"* and nothing else. But launches ALONE qualify a clean verdict rather than withhold one; that is what `qualifying_only` means. The unclassified list, which is the half that actually decided the verdict AND the only actionable half, was computed and discarded — it appears in no other channel. Observed on hypergumbo's own self-proof, where 18 claims blamed two subprocess launches while thirteen unclassified receivers were the real cause; recovering them took a monkeypatch. The reason now carries both, launch first (an uncatalogued module is a gap the reader can close, an opaque launch is categorical, so naming the fixable one first would send them on an errand that cannot succeed) and the clause appears only when there is something to name, pinned by a control so a sole-blocker launch still blames no module.

### Documentation

- **AGENTS.md documented a tracker invocation that exits 2, and the failure was unreadable exactly where it landed.** `--json` is registered on the tracker's top-level parser, so `scripts/tracker --json show <ID>` works and the trailing form is a usage error — written to stderr, with nothing on stdout, which a caller piping stdout into a JSON parser reads as a missing item rather than as a bad invocation; two consecutive sessions produced full columns of false `UNRESOLVED` that way. The always-loaded governance text, ADR-0013 in three places, the tracker-hygiene playbook — where one occurrence was a runnable `> corpus.json` redirect that would have captured an empty file rather than failing — and the transcript hook's playbook summary are corrected, and `tests/test_tracker_json_flag_documented.py` locks the contract: it anchors on `--json` still being registered once ahead of `add_subparsers` (so that WI-dolaf's structural half, which would make the trailing form valid, relaxes the guard instead of mystifying it), scans per backtick code span rather than per line because prose discussing the hazard legitimately names both orders in one sentence, and requires AGENTS.md to show the working form positively, since a forbid-only guard is satisfiable by deleting the documentation. The guard immediately found a third ADR-0013 occurrence that two hand greps had missed — the second sweep's own `.ops` noise filter matched the offending line's text and dropped it, leaving the sweep to exit 0 and print clean.
