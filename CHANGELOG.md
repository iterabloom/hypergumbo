...
- **Woodpecker CI per-PR gate lands in canonical `dev` (Codeberg → GitHub migration).** The Phase-1-validated `.woodpecker.yml` — the container-based (`python:3.11-bookworm`) port of `.github/workflows/ci.yml` (lint / mypy / audit / verify-generated / hook-tests / build-grammars / pytest, preserving the committed `.ci/affected-tests.txt` smart-test contract) — is added at the repo root. It is **active on GitHub + self-hosted Woodpecker and inert on Codeberg/Forgejo** (which reads `.github/workflows/**`, not `.woodpecker.yml`), so it lands dormant while Codeberg is origin and does not reach GitHub until a mirror refresh / cutover. A new `forge-arms` job runs the top-level bash-subprocess forge github-arm + governance rot-guard tests under forced `HYPERGUMBO_FORGE_BACKEND=github` — the deferred **C13** rot-guard — guarding the dormant github write-arms per-PR, since smart-test's import-slice cannot select bash tests. The `.woodpecker/full-suite.yml` and `.woodpecker/nightly.yml` sibling ports are a tracked follow-up (nightly is required before the Phase-4 release path).
