...
- **HTTP linker: AngularJS $http + jQuery $.ajax detection**: Detects AngularJS `$http.get/post/put/patch/delete()` method calls, `$http({method, url})` config objects, jQuery `$.ajax({url, type/method})` calls, and jQuery `$.get()`/`$.post()` shorthand. Creates `http_calls` edges linking JS/TS client call sites to server route handlers. Addresses bakeoff finding where guacamole-client's AngularJS frontend had zero cross-language edges to the Java backend.
...
- **Exclude documentation kinds from centrality and symbols output**: Markdown `section`, `code_block`, and `link` symbol kinds are now in `EXCLUDED_KINDS`, preventing README.md headings from ranking above actual code in `hypergumbo symbols` output. Previously, `candle-nn/README.md` scored 7.5 while core Tensor methods scored lower.
...
- **Go chained-call ambiguity guard (WI-bimab)**: When a method call's receiver is a call expression and the package is unknown (e.g., `c.CoreV1().Endpoints(ns).Update()`), the call is now marked as `chained_call_unresolved` instead of resolving to an arbitrary global candidate. Prevents false positives like `Manager.Update` getting 19 spurious incoming edges in prometheus. Confidence set to 0.40 (below the 0.50 of the existing ambiguity guard).
- **Go-swagger handler cache route detection (WI-bimab)**: Detects routes registered via go-swagger/go-openapi `initHandlerCache` pattern (`o.handlers["METHOD"]["/path"] = handler.NewXxx(...)`). Extracts HTTP method, route path, and handler name. Tested on alertmanager where this revealed all 9 v2 API routes previously invisible.
...
- **Rust library export detection (WI-sovot)**: Pub functions, structs, traits, enums, and methods detected as library_export entrypoints. Matches only bare `pub` (not `pub(crate)` or `pub(super)`, which are crate-internal). Fixes library entrypoint detection for Rust library crates where pub items in lib.rs were invisible to the entrypoint scorer.
- **Fuzz target and benchmark tier classification**: `fuzz/`, `fuzzing/`, `fuzz_targets/`, `benches/`, `benchmarks/`, `benchmark/` directories now classified as tier 2 (internal dep) instead of falling through to tier 1 (first-party). Fixes bakeoff finding where fuzz binary targets in fuzz/Cargo.toml dominated entrypoint lists at 0.99 confidence, burying actual library API exports.
...
- **Go var alias extraction**: Package-level `var Name = expr` declarations are now extracted as `variable` kind symbols. This makes aliases like `var String = slog.String` visible to the resolver — when other packages call `log.String()`, the symbol is found. Addresses 21.7% of remaining unresolved calls in trivy bakeoff (mostly `log.String`, `log.Debug`, `log.Err` aliases).
...
- **Common method name dampening (WI-luvaj)**: Methods whose name appears on 10+ distinct symbols get centrality reduced proportionally: `max(0.1, 10/count)`. Addresses bakeoff finding where `PartialToViewsMappings#execute` (in-degree 2894) dominated rankings because 50+ classes define `execute` and every unresolved `.execute()` call was attributed to it. Dampens common names like `execute`, `call`, `perform`, `project`, `account` while leaving unique domain functions unaffected.
- **C/C++ function pointer callback argument detection**: Call arguments that are bare identifiers resolving to known function symbols now produce `calls` edges with `function_pointer_arg` evidence type (confidence 0.80). Handles `pthread_create(tid, attr, worker_thread, arg)`, `qsort(arr, n, sz, compare_fn)`, `signal(sig, handler)`, `atexit(cleanup)`, and any other callback-passing pattern. Addresses the top DEEP bakeoff finding for guacamole-server-main where `pthread_create(&child_thread, NULL, guacd_connection_thread, params)` produced no edge, making the entire connection dispatch chain invisible from forward slices.
- **Rust call resolution quality**: Three compounding fixes for Rust `::` qualified name resolution. (1) Suffix index now splits on both `.` and `::` so lookups like `"compute"` find `"Diff::compute"`. (2) Scoped calls like `Foo::bar()` try the full qualified name first, giving exact matches at 0.80 confidence instead of ambiguous suffix matches at 0.56. (3) `register_symbol` no longer registers short names for `::` methods — the suffix index handles this correctly, preventing false exact matches when multiple types share a method name.
- **Cross-language short-name collision fix (WI-kopuz)**: Extended the suffix index to handle all language separators (`.`, `::`, `#`, `\`, `:`) and removed short-name registration from `register_symbol` in 10 languages: C++, Scala, Swift, Ruby, Groovy, C#, Perl, Elixir, Hack, Apex, Luau. Previously, when multiple types shared a method name (e.g., `get`, `compute`, `clone`), the last-registered short name won, causing false exact matches at confidence 1.0. Now, the suffix index handles cross-qualifier lookups correctly with appropriate ambiguity penalties. Also fixed Hack namespace propagation to `extract_edges_from_file` via `get_import_aliases`.
- **C declaration/definition deduplication (WI-zuvus)**: Filters out header-file function declarations when a `.c` definition exists for the same function. Previously, both the declaration (e.g., `void foo();` in `header.h`) and definition (`void foo() { ... }` in `source.c`) appeared as separate symbols, inflating orphan rates by 15-20 percentage points because calls resolve to the definition and the declaration has no edges. Edges referencing removed declarations are remapped to the surviving definition.
- **Rust enclosing function disambiguation**: When multiple impl blocks define methods with the same short name (e.g., `Diff::compute` and `Parser::compute`), call edges inside those methods now correctly attribute the `src` to the right symbol. Uses a span-based index built from global symbols instead of the name-collision-prone `symbol_by_name` dict. Fixes forward slice traversal for Rust methods: previously, calls inside `Diff::compute` could be attributed to `Parser::compute` (whichever impl was processed last), making forward slices from the correct method miss all callees.
...
- **Google Guice DI framework patterns (WI-lirot)**: `@Inject`, `@Provides`, `@Singleton`, `@Named` annotations and `AbstractModule` base class detected as DI concepts. Guava EventBus `@Subscribe` detected as event_handler. Framework detection from `com.google.inject` in Maven/Gradle builds. Enables DI-aware analysis for projects like killbill and Presto.
- **Jakarta CDI framework patterns (WI-lirot)**: `@ApplicationScoped`, `@RequestScoped`, `@SessionScoped`, `@Dependent` scoping annotations, `@Produces` factory methods, `@Interceptor` AOP, and `@Alternative` beans. Framework detection from `weld`/`jakarta.enterprise` in Maven/Gradle builds. Enables CDI-aware analysis for projects like Keycloak.
- **Rails callback and event patterns (WI-ranip)**: ActiveRecord lifecycle callbacks (`after_commit`, `after_save`, `before_create`, etc.) and controller callbacks (`before_action`, `after_action`, `around_action`) detected as event_handler concepts via usage patterns. Wisper pub/sub `Wisper::Publisher` base class detected as event_publisher. Rails now activates event-sourcing linker.
...
- **DEEP bakeoff verdict graduation (META-donon)**: DEEP assessment prompts now request per-task numeric scores (0-100) for refactoring, new feature, and understanding tasks, replacing the uninformative ternary verdict. Aggregation computes mean/min/max/median statistics. Trajectory field (IMPROVING/STABLE/REGRESSING) compares against prior cohort scores. Prior assessment context is injected into prompts so assessors can evaluate progress.
- **Aggregation resilience (META-radiz)**: New `bakeoff-features-reflect check` command finds orphaned assessment sets (assessments present but no summary.yaml) and optionally auto-aggregates them with `--auto-aggregate`. Prevents data loss when spawning sessions die before aggregation completes.
- **Assessment idea ingestion (WI-madah)**: New `bakeoff-features-reflect ingest` subcommand creates stealth-tier tracker work items from aggregated improvement ideas. Ideas appearing in >= N assessments (default 2) are deduplicated against existing stealth items and created as `todo_soft` P3 items tagged `bakeoff`+`assessment`. Also available as `aggregate --ingest` for one-step workflow.
- **DEEP bakeoff forward-slice hub threshold (WI-sivun)**: Forward slices in DEEP bakeoff now use `--hub-threshold 100` (vs default 50). Enterprise Java service interfaces commonly have 50-100+ outgoing call edges, causing premature hub pruning that limited slices to 18-66 nodes.
...
- **`bakeoff-features compress` subcommand (WI-gumuh)**: Gzips `hg.json` files in bakeoff output directories where derived artifacts (`entries.txt`, `symbols.txt`, `routes.txt`) have existed for 8+ hours. Typical 5-10x compression reclaims significant disk space (bakeoff artifacts were 7.76 GB, 85% from hg.json files). Compressed files are transparently readable by `diagnose`, `run`, and other subcommands via `_find_hg_json`/`_open_hg_json` helpers. Includes `--dry-run` option.
...
- **Utility symbol dampening in symbols ranking**: `hypergumbo symbols` now applies `is_utility_symbol()` dampening (0.10x) to exception/error classes, loggers, and other infrastructure symbols during ranking. Previously, utility dampening was only applied in `rank_symbols()` (used by slicing) but not in the `symbols` command, so `GuacamoleServerException` (in=81, deg=83) still appeared at position 8 in `symbols.txt` despite being infrastructure. Now these symbols rank below domain-relevant classes with lower raw degree.
...
- **Java**: Inherited method fallback (edges to class symbol when method unresolvable), array initializer annotation path unwrapping, and ListNameResolver method registry (AMB-METHOD infrastructure for consistency).
- **Ruby**: Class method (`def self.method`) extraction, `.new` → `#initialize` resolution, namespaced receiver resolution, chained constructor calls, job enqueue edge detection (`perform_later`/`perform_async`), Rails callback edges (controller actions, model lifecycle, block-style), delegate macro edges, ActiveRecord association edges, receiver-type tracking for variable-receiver calls, method parameter filtering, ambiguity guard for bare method calls (3+ classes → no false-positive edge, same-class calls still resolve), and ListNameResolver for receiver-call fallback (AMB-METHOD guard now covers all call paths).
- **Rust**: ListNameResolver with `ambiguity_threshold=3` replaces ad-hoc `min(1, 3/N)` penalty for method calls. Method calls with 3+ candidates now produce no edge (previously: low-confidence edge to arbitrary target). Scoped identifier fallback (`Foo::bar()` when `Foo::bar` not found) now applies the ambiguity guard: 3+ candidates → no edge instead of arbitrary pick at 0.56 confidence. `lines_of_code` now populated for all Rust symbols (functions, structs, enums, traits) from span data.
...
- **Edge deduplication**: At most one edge per (src, dst, edge_type), eliminating false centrality inflation from repeated calls. Fixed: edges with `edge_key=None` (the default when using `Edge()` constructor instead of `Edge.create()`) are now deduplicated by computing the key on-the-fly from `(src, dst, edge_type)`. Previously, the first `None`-keyed edge was kept and all subsequent `None`-keyed edges were silently discarded as duplicates, causing route-handler and other linker edges to vanish.
...
- **Tracker unread human message warning (WI-sulaf)**: When `tracker discuss` or `update --note` adds a message to an item whose last discussion entry is from a human, prints the unread message and prior transcript to stdout, prompting the agent to reply.
...
- **JS built-in name collision false positives**: Calls to JavaScript built-in constructors and globals (`Number`, `String`, `Boolean`, `parseInt`, `setTimeout`, etc.) no longer resolve to user-defined functions with the same name. In polis, `Number(x)` in server code was creating false edges to a React component named `Number` in client code. Also blocks built-in names when passed as callback arguments (e.g., `items.map(Number)`).
- **JS/TS function parameter shadowing**: Calls to names that match a parameter of any enclosing function or arrow function (e.g., `resolve()` and `reject()` inside `new Promise((resolve, reject) => {...})`) no longer resolve to global functions. Walks through nested closures (JavaScript lexical scoping) so `resolve()` inside a nested callback like `doAsync(function(err) { resolve(0); })` correctly identifies `resolve` as a parameter from the outer Promise scope. Handles TypeScript `required_parameter`/`optional_parameter` wrapping. Completely eliminated 98 false cross-package edges (50 resolve + 48 reject) in polis.
- **JS/TS callback argument reference cross-package guard**: Callback arguments (e.g., `items.forEach(error)`) now use the same resolution strategy as direct calls: import-path disambiguation, same-package preference, and a cross-package guard that blocks reference edges when the resolved target is in a different npm package. Also applies parameter shadowing guard. Eliminates false `callback_argument_reference` edges for common names like `error` and `zid` that exist in client packages but are referenced from server code.
- **JS/TS cross-package guard on all remaining edge paths**: Extended the npm-package cross-package guard to namespace calls (`utils.error()`), method inference fallback (`obj.apply()`), object field references (`{onError: error}`), shorthand properties (`{error}`), and direct call resolver fallback. Previously these paths resolved common names (error, apply, formatDate, f, request) across package boundaries. In polis, eliminates 133 false `ast_call_namespace` edges (125 targeting `error`), 55 false `object_field_reference` edges, 5 false `ast_call_direct` fallback edges, and 2 false `ast_method_inferred` edges.
...
- **NPM package tier classification**: `npm_package` symbols created by the JS module linker were classified as tier 1 (first-party) instead of tier 3 (external dependency). Root cause: `_classify_symbols` in cli.py unconditionally overwrote linker-set tiers. Fix: linker now sets tier=3 at symbol creation, and `_classify_symbols` skips symbols with pre-set tiers. Affects all JS/TS repos — every npm dependency was incorrectly counted as first-party code.
- **Rust built-in attribute false-positive edges (WI-votaj)**: Built-in Rust attributes (`#[test]`, `#[cfg]`, `#[inline]`, `#[allow]`, `#[must_use]`, `#[derive]`, etc.) no longer resolve to user-defined functions with the same name. Previously, a function named `test` would receive false `decorated_by` edges from every `#[test]`-annotated function in the crate. 45 built-in and common ecosystem attribute names are now skipped entirely — the Rust analyzer does not create `decorated_by` edges for them.
- **Minified file skip in linker scans**: Event sourcing linker and HTTP linker file scans now check `is_minified_file()` before reading file content. Previously, minified JS/CSS bundles (e.g., `dist/*.min.js`) could be scanned for event/HTTP patterns, wasting time and occasionally producing false matches.
- **Clojure test-* prefix path guard (WI-vutum)**: The `test-*` name-based test function pattern for Clojure now requires the file to be in a `test/` or `tests/` directory (via `symbol_path` guard). In Clojure, `test-` is a naming convention for "verify/probe this thing" (e.g., `test-ldap-connection`, `test-database-connection` in Metabase). These are production functions, not tests. The actual test macro `deftest` is unaffected.
- **Rails route-to-controller reverse suffix matching (WI-polof)**: Routes with namespaced `controller_action` (e.g., `api/v1/statuses#destroy` → `Api::V1::StatusesController`) now resolve to handler symbols with short names (e.g., `StatusesController#destroy`). The Ruby analyzer produces short names (only the immediately enclosing class), but the route extraction produces fully namespaced controller_action. Added reverse suffix matching: the normalized controller class's ending is checked against the symbol's class portion at namespace boundaries (`::` separator). Case-insensitive variant handles Rails acronym inflections. Mastodon: 0 → 635 `routes_to` edges.
- **GraphQL framework false positive guard (WI-rofiz)**: Bare `graphql` npm package no longer triggers graphql framework detection. Many JS/TS projects install `graphql` for type definitions or code generation (e.g., Unleash) without implementing a GraphQL server. Only server-specific packages now trigger detection: `@apollo/server`, `graphql-yoga`, `mercurius`, `type-graphql`, `@nestjs/graphql`. Prevents false-positive `graphql_resolver` node classification in non-GraphQL codebases.
- **Java/C#/Kotlin/Scala main() entrypoint detection with qualified names (INV-lumiz)**: The `main-functions.yaml` patterns for Java, C#, Kotlin, and Scala now match qualified method names (e.g., `PageViewUntypedDemo.main`, `Program.Main`, `Kafka.main`) in addition to bare names. These analyzers produce qualified names (`ClassName.methodName`) but the pattern used `^main$` which only matched exact strings. Changed to `(^|\.)main$` / `(^|\.)Main$`. Added Scala pattern (previously missing). Kafka: 0 → 153 Java + 7 Scala main() entrypoints detected.
- **Java class name collision dampening (INV-finak)**: When a file imports an external library class (e.g., `org.slf4j.Logger`) and source code contains an inner class with the same simple name (e.g., `Config.Logger`), the NameResolver's suffix matching no longer creates false edges to the inner class. New `_is_import_class_mismatch()` function checks file-level imports against the resolved candidate's path at instantiation, constructor reference, and type-inferred call sites. In Kafka, eliminates ~2328 false in-degree edges to `Log4jConfiguration.Logger` that corrupted centrality rankings.
- **Go vendored SDK tier classification (INV-mogud)**: Vendored Go SDKs embedded in subdirectories (e.g., `cloudprovider/ionoscloud/ionos-cloud-sdk-go/`) are now classified as tier 3 (external dependency) instead of tier 1 (first-party). Matches SDK directory names anywhere in path: `*-sdk-go*`, `*-go-sdk`, `*-sdk-golang`, `vendor-internal/`. In cluster-autoscaler, reclassifies 19,289 of 35,446 nodes (54%) from tier-1 to tier-3, cleaning up centrality rankings and tier-filtered slices.
