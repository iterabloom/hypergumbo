...
- **Extras umbrella collapsed to one pair of subcommands.** `install-extras` / `uninstall-extras` are removed; `add-extras` / `remove-extras` are now the single umbrella over grammars, gitleaks, embeddings, and rust-analyzer. The two umbrella pairs had drifted apart — `add-extras` silently lost rust-analyzer coverage for an unknown stretch, and the `install-extras` path bypassed the rust-analyzer integration-package gate, producing a misleading "✓ installed" + runtime-error pair. The consolidated path uses one declarative table with the integration gate applied uniformly. `add-extras` gains `--check` (status table; exits non-zero if anything is missing) and `--skip COMPONENT[,...]`; `remove-extras` gains `--skip`. The `--check` rust-analyzer row reports `✗ not installed` when the rustup binary is present but the integration package is missing (e.g. a system-package-manager rustup install, or a residual binary after uninstalling the `[rust-analyzer]` extra), instead of a misleadingly green status. **Breaking** for anyone scripting against the old names; mitigated by the project's alpha status.

### Added

- **`hypergumbo[rust-analyzer]` install extra + `hypergumbo-lang-rust-analyzer` published to PyPI.** v4.1.0 shipped without the SCIP integration package and without declaring it as an optional dependency, so `--backend rust-analyzer` had no way to engage end-to-end (the runtime gate below landed first as a defensive fix). This release adds the `[rust-analyzer]` extra (pinned in lockstep with the meta-package version) and adds `hypergumbo-lang-rust-analyzer` to the release-workflow build loop so the next tag uploads it to PyPI alongside the other six packages. After this release, `pipx install 'hypergumbo[rust-analyzer]'` actually engages the SCIP backend; the runtime gate stays as a defensive backstop for users on minimal installs.

- **`hypergumbo symbols` column-width controls.** The Symbol and File columns now default to 60 / 80 characters wide — roughly twice the width Rich picks under narrow auto-fit (e.g. Google Colab, where stdout is non-TTY and Rich falls back to ~80 cols, squeezing those columns to ~25–30 chars each). Two new flags: `--col-width N` sets both columns to N (clamped to `[1, 1000]`), and `--wrap` switches overflow from ellipsis truncation to character-level fold-wrap so long identifiers and paths break across lines instead of getting cut. Console width auto-extends to fit when requested column widths exceed the detected terminal — narrow hosts get a horizontally-scrolling table rather than collapsed columns.

- **Smart-test slice-fallback diagnostic file.** When `scripts/smart-test`'s reverse-slice path falls back to full-suite (either `slice command failed` or `no test files in slice result`), it now writes a self-contained diagnostic bundle to `.ci/smart-test-fallback.log` (gitignored, overwritten each fallback). The bundle records the timestamp, fallback reason, hypergumbo path and version, the slice command line invoked, slice exit code and wall-clock duration, slice stdout line count plus first 50 lines, full slice stderr, and the changed-files list. A one-line pointer is printed to stderr when fallback fires, so the next time it happens organically we can inspect what went wrong without re-running. Motivated by a slice → full-suite fallback that cost ~12.5 minutes and could not be reproduced afterwards.

- **UAT directed-validation playbook gains a Mechanism check.** The validation-queue UAT playbook adds an optional Phase U1 `Mechanism check` field where the orchestrator pre-commits one or two falsification probes for claims that name a specific mechanism, a `Mechanism` column on the Phase U3 verdict matrix (`matches` / `mismatch` / `n/a`), and a fourth Phase U4 verdict bullet for `moved` + `Mechanism: mismatch` — strip the validation tag (the public-facing claim is satisfied) AND file a `needs_human_review` follow-up to reconcile claim text against linker behavior. Surfaced by a UAT round whose verdict resolved `moved` per pre-committed quantitative criteria but whose claim text described a transitive base-class walk that subsequent investigation falsified (the actual mechanism was a filename convention).

### Fixed

- **`hypergumbo build-grammars` now actually builds Circom.** The Python builder iterated `SOURCE_GRAMMARS`, which only listed Lean and Wolfram, so users hitting `"Circom analysis skipped: tree-sitter-circom grammar not available. Run \`hypergumbo build-grammars\` to build it."` would run the suggested command and see the warning persist. (The shell-script CI/dev path had been building Circom all along.) Added `tree_sitter_circom` to `SOURCE_GRAMMARS` and updated subcommand help and module docstrings.

- **Partial-install warnings now respect linker activation conditions.** The partial-install warning pass iterated diagnostics from every registered linker and emitted warnings unconditionally, so e.g. a Rust + Python repo with C/C++ symbols got `"CGO linker found 151 C/C++ implementations but 0 Go cgo calls"` even though the CGO linker (Go ↔ C/C++) wouldn't have run on that tree. Added a per-warning gate that consults the linker's activation predicate (`should_run(detected_frameworks, detected_languages)`) and skips warnings whose linker wouldn't have activated. The gate is bypassed when both detection sets are empty (preserves crafted-diagnostic test fixtures). The dependency linker (always-on) is unaffected.
...
- **Rust `impl Trait for Type` no longer binds the trait LHS to a same-named non-trait symbol.** The impl_item handler resolved the trait name and accepted any symbol with that name. When a project also defines a non-trait symbol with the same name (e.g. a marker `struct Clone;` used as a phantom-type tag) and elsewhere has a manual `impl Clone for X` referring to `std::clone::Clone`, the lookup bound to the local struct and emitted a spurious high-confidence `X implements struct-Clone` edge. The handler now requires `kind == "trait"`; non-trait matches fall through to the unresolved-trait branch (which correctly suppresses standard-library trait names).

- **Rust `impl Trait for Type` now resolves trait-vs-struct short-name collisions across files.** The kind-discipline guard above only catches a same-name struct when the global-symbol-table overwrite happens to leave the struct as the survivor (kind check then rejects it). When the trait wins the overwrite the lookup succeeds; when the struct wins, the canonical trait is gone from the global table entirely and the handler emits an unresolved-trait edge instead of the canonical resolved one. On a typical ML framework this misresolved ~63% of `impl Module for X` edges depending on registration order. The Rust analyzer now also populates a kind-segregated multi-value index alongside the existing single-value dict; the impl_item lookup consults it, prefers `kind == "trait"` candidates, breaks ties by same-file path then stable id, and refuses to fall back to a struct/enum when no trait exists.

- **`hypergumbo run` handler-slice fan-out is now co-located under `<out-stem>.slices/`.** `--out /some/path/foo.json` previously deposited 20–30 `slice.handler.<METHOD>.<path>.json` files directly in `/some/path/`, which clobbered prior runs when multiple result files shared a parent directory. The fan-out now goes to a stem-derived subdirectory next to the `--out` target — `--out /some/path/foo.json` writes the main result at `/some/path/foo.json` and the per-handler slices at `/some/path/foo.slices/slice.handler.*.json` plus `slice.handler.index.json`. The companion `index.json` keeps its relative-filename entries. The `--no-handler-slices` switch is unchanged. The same shape applies when `--out` is omitted: slices land in `<cache_dir>/hypergumbo.results.slices/`.

- **`grpc-linker-v1` Go server method-to-RPC mapping is now file-scoped.** The struct-to-service map was keyed by bare struct short-name. When multiple Go files declare a struct with the same name — e.g. eight plugin packages each declaring `type service struct { ... api.UnimplementedXxxServer ... }` for a different service — the map overwrote on registration order, and whichever file iterated last won the mapping for every other plugin's `service.Create` method. On a real-world repo this misresolved seven service families' `implements_rpc` edges onto a single unrelated RPC family. Distinctive struct names that were unique across files escaped the collision. The map is now keyed by `(file_path, struct_name)` and the method-side lookup uses `(method's file path, struct_name)`, so each plugin's methods resolve only against its own file's embedding. Both the `Unimplemented*Server`-embedding scan and the ttrpc / CSI base-class fallback use the same file-scoped key.

- **`--backend rust-analyzer` and `install-rust-analyzer` now error clearly when the SCIP integration package is missing.** The published v4.1.0 distribution did not include `hypergumbo-lang-rust-analyzer` in its dependencies (and the package was not on PyPI), so `--backend rust-analyzer` and `install-rust-analyzer` both passed argparse and ran without error while the SCIP backend silently fell through to the default tree-sitter analyzer — a fully no-op CLI surface. A new helper `is_rust_analyzer_integration_installed()` detects the package side-effect-free via `importlib.util.find_spec`; the CLI now exits non-zero with a clear message when `--backend rust-analyzer` is requested without the integration package available, and `install-rust-analyzer` refuses to install the rustup binary alone. `install-rust-analyzer --check` reports both binary and integration-package status as separate lines, exiting 1 if either is missing.
