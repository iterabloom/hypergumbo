...
- **ADR-0027 & ADR-0028 (Drafts) — two more axes instantiate ADR-0024**: ADR-0027 names `Symbol.kind` as the source-language syntactic construct (DEPRECATE verdict, 192 values / 8 clusters; framework-participation folds to canonical + `meta["framework_role"]`; edge-label values reclassify or drop). ADR-0028 names `Edge.evidence_type` as the inference pathway (DEPRECATE verdict, 210 values / 4 clusters; confirms ADR-0023 Open Question #2; resolution status promotes to a sibling `Edge.is_resolved: bool` per fold-residue-discipline; framework-dispatch and apex/peer call variants fold to canonical + meta). Both follow ADR-0023's four-phase shape with per-cluster verdicts filed as audit-findings docs. ADR-0028 is the largest concept-axis migration on the roadmap (~140 production files have `evidence_type` literals).
...
- **`docs/blind-spots.md` — generative-question catalog for the audit lifecycle**: diagnostic complement to the Fundamental Concept Audit playbook, cataloguing four recurring question-shapes (typing axis vs values, assumed input boundaries, silently-load-bearing failure modes, null results read as confirmation) with examples and triggers. Wired into the agentic-session-retrospective (5-minute check between Synthesize and Record) and the structural-fix-scope-expansion protocol (pre-refactor blind-spots check); both gracefully degrade when the file is absent.
...
- **Pre-commit edge-type drift linter, with `--strict` mode** (`scripts/check-edge-type-drift`): catches consumer-side hardcoded `*EDGE_TYPE*` sets that drift from the canonical registry before push. `--strict` additionally enforces that consumer-set values lie on the relationship axis (or pending_classification). Wired into `.githooks/pre-commit`; only fires when `packages/*.py` files are staged. Implementation is field-agnostic via `axis_drift.find_drift(allowed_axis_names=, name_to_axis=)` so future axis-bearing fields inherit the pattern per ADR-0024's enforcement template.
...
- **Phase 3 — five `endpoint_shape` families folded to canonical + meta**: bridge/FFI (`cgo_bridge` / `ffi_bridge` / `napi_bridge` / `wasm_bridge` / `native_bridge` / `bridge_invokes` → `calls + meta["bridge_kind"]`; `wasm_load` → `imports`); IPC (Tauri / Electron / Phoenix Channels / WebSocket / message queues — `ipc_calls` → `calls + meta["protocol"]="ipc"`; event variants → `event_publishes + meta["channel_kind"]`; `websocket_connection` → `references + meta["construct"]="websocket_endpoint"`); publish/dispatch (`routes_to` → `dispatches_to + meta["dispatch_kind"]="route"`, across 17 emit sites in 12 linkers, per audit-findings 0001); protocol-call (HTTP / gRPC / GraphQL → `calls + meta["protocol"]`; new `PROTOCOL_KINDS` constant; `awaits_bakeoff_validation`); and dst-kind leakage (`imports_module` / `imports_component` → `imports`; `model_reference` / `type_ref` / `query_references` → `references`; `renders_component` → `references + meta["construct"]="jsx"`). DEPRECATE-NO-FOLD drops: `message_receive` producers (forward `event_publishes` already captures the relationship) and `event_subscribes` from `event_sourcing.py`. New `BRIDGE_KINDS` and `PROTOCOL_KINDS` constants document the closed enumerations.
...
- **ADR-0023 promoted Draft → Accepted; §6 plan reshaped to many-small-ships**: typing principle in force. ADR text now cites landed Phase 1 commits, cross-references ADR-0024, resolves prior Open Questions inline, and reframes the Property test section as three complementary defenses (static / runtime / cadence-hook). §6 plan replaces the single-event "endpoint_shape section becomes empty" with sequential micro-4b ships, one family at a time.
