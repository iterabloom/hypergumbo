...
- **What this changes downstream.** The affected passes move from a false `no_candidate_files` to `unreported`, which means *cannot determine* — the honest residue. The truly-silent cohort (read files, emitted nothing) is therefore larger than the disclosure previously showed: **11 pass-runs and 4.21% of pass time, not 3 and 1.9%**. The mypy strict surface shrank 669 → **667** in passing.
