...
- **L3 producer-coherence linter** (`producer_coherence.py`, `scripts/check-producer-axis-coherence`): walks `Edge.create(...)` / `Edge(...)` / `Symbol.create(...)` / `Symbol(...)` call sites and verifies literal-string keyword arguments to `evidence_type` / `kind` / `edge_type` are in the corresponding canonical registry. F-string emits surface as advisory Phase-3 fold candidates. Closes the producer-introduction gap left open by the consumer-side drift linters across all three sibling axes. An assignment-form extension additionally traces simple assignment-form references (`name = "literal"` plus ternary / if-else) within a function; landed surfacing 18 latent leaks.
...
- **Wave 3 — `Edge.evidence_type` Cluster 28B `*_unresolved` fold**: 18 emit sites across 11 producer files migrate to `evidence_type=<canonical>` + `Edge.is_resolved=False`. First production write of `is_resolved`. Two new Cluster 28A canonicals (`grpc_stub_resolution`, `luajit_ffi_lookup`) absorb prior emit sites without a canonical inference label. Audit-findings 0008.
- **Wave 4 — Symbol.kind Clusters 27C/27E/27F + evidence_type Cluster 28D**:
    - **Cluster 27C apex/peer** — `fn` / `var` / `proc` / `structure` registry classification updates (no producer change; apex emit was already canonical). Audit-findings 0009.
    - **Cluster 27E edge-label kinds** — `call_site` added as new Cluster 27A canonical. Sub-case (a) reclassifies 4 producers (subprocess/db_query/abi/twig function_call) to `kind="call_site"` + `meta["call_kind"]`. Sub-case (b) drops Symbols whose relationship is already on a companion Edge: 3 clean drops (css/jsonnet imports, bitbake inherit), 6 shape-2 edge-endpoint redesigns (json_config/puppet/scss/twig), and 4 shape-3 companion-Edge introductions (r_lang library imports, astro frontmatter imports, make include, blade @extends). Audit-findings 0010.
    - **Cluster 27F component_ref** — vue / svelte / astro drop per-reference Symbols; `imports` Edges re-route src to `make_file_id()` and dst falls back to a 5-part dangling component id when unresolved. DEPRECATE-NO-FOLD verdict (the original fold target `reference` was already deprecated in 0010). Audit-findings 0011.
    - **Cluster 28D evidence_type call-construct** — 28 peer values (`function_call`, `method_call`, `function_application`, `pipe_call`, `typed_receiver_call`, `ambiguous_method_call`, …) collapse to apex `ast_call` + `meta["call_construct"]` (the lone non-`ast_call` apex is `cross_file_message_send` → `message_send` + `meta["call_construct"]="cross_file"`). 89 emit sites across 26 producer files. Audit-findings 0012.
- **Wave 5 — coordinated framework-role + framework-dispatch fold across six PRs**: ~28 `Symbol.kind` framework-role values fold to `function`/`method`/`interface`/`class`/`reference` + `meta["framework_role"]`; ~75 `Edge.evidence_type` framework-dispatch values fold to canonical inference (`ast_call_direct` / `ast_decorator` / `ast_import` / `naming_convention`) + `meta["framework_dispatch"]` or `meta["detection_pattern"]`. Coverage spans websocket / tauri / grpc / openapi / graphql / http / crypto / ipc / objc / event-dispatch / Go-route_mount / Ruby / Django / NestJS / Vue and ~25 single-row dispatch modules. The highest-blast-radius slice is `Symbol.kind="route"`: 17 source files, 14 production consumers, ~330 sites total.
- **`Pattern.framework_role` field with its own compiled-regex matcher**: dedicated `framework_role: str | None = None` field on `Pattern`. Four YAML rules referencing folded role names (`framework_role: "^route$"` in `laravel.yaml`, `phoenix.yaml`, `rails.yaml`, `sinatra.yaml`) migrate. `symbol_kind` and `framework_role` are disjoint axes that can coexist on a single Pattern (AND condition). New result key `matched_framework_role`.
- **gRPC + message-queue assignment-form correction**: literal-grep at audit time missed assignment-form producers (`kind = "<value>" if … else "<value>"` followed by `Symbol(kind=kind, …)`). The follow-on sweep folded `grpc_stub` / `grpc_server` / `grpc_service` / `grpc_servicer` / `grpc_client` / `mq_publisher` / `mq_subscriber` to canonical apex + `meta["framework_role"]`. Audit-findings 0013 §"Diagnostic findings" #3 corrected; diagnostic-test patterns updated to assignment-aware regex. Three previously-unregistered values added to `symbol_kinds.py` on AXIS_ENDPOINT_SHAPE.
- **`linkers/phoenix_ipc.py` f-string fold**: same family of miss as the assignment-form correction above but f-string-form (`kind=f"ipc_{pattern.type}"`). Folds to canonical `kind="function"` + `meta["framework_role"]=f"ipc_{pattern.type}"`, mirroring `linkers/ipc.py`.
- **Wave 6 — `Symbol.kind` Clusters 27B / 27G / 27H sweep**:
    - **74 canonical promotions** (registry classification only; no producer change): 6 Cluster 27B file-shape (`file` / `library` / `package` / `executable` / `program` / `project`), 15 Cluster 27G build/config DSL constructs, 53 Cluster 27H domain-DSL constructs.
    - **15 FOLDs to canonical + meta keys** (with producer migration): Cluster 27B framework/ecosystem-qualified file/package/export/module values (e.g. `module_file` → `file` + `module_system`, `npm_package` / `composer_package` → `package` + `package_ecosystem`, `wasm_module` → `module` + `compilation_target`); Cluster 27G `test_case` → `test` + `test_dialect`, `editable` / `url_requirement` → `requirement` + `install_mode` / `install_source`, `devDependency` → `dependency` + `dependency_scope`, `python_task` → `task` + `task_implementation`. Consumer dual-shape predicates added at `route_handler.is_component` and `cli._is_noise`; other consumers unchanged (canonical fold targets already in `EXCLUDED_KINDS` / `_DEPENDENCY_KINDS`).
    - **DEPRECATE-NO-FOLDs + CANONICAL reclassifications**: 27B `tsconfig` (subsumed by v4.0.0 `is_config_file` boolean); 27G `config` (Prisma generic block — producer rewrites to `kind="block"`), `dev-dependency`, `build-dependency`, `work_item` (dead vocabulary); 27H `heading`, `model`, `unresolved` (registry seed errors, no producer). Four CANONICAL reclassifications surfaced an audit gap: `theorem`, `inductive`, `message` had real producers via `add_symbol(...)` / `_make_proto_symbol(...)` indirection at `lean.py` / `tlaplus.py` / `proto.py` — closed by playbook Step 4.5; `external_symbol` reclassifies on `language_construct` (pipeline-DSL top-level construct). Hypothetical `Symbol.is_resolved` follow-on ADR **withdrawn** — re-trace showed no leak to absorb (1 producer, 0 consumer reads, fails ADR-0024 fold-residue rule 3 threshold).
- **Phase-2 consumer migration**: dual-shape predicates introduced at `linkers/registry._is_synthetic_node` (slice 1) and `selection.filters.is_excluded_kind` (slice 2). Both recognise pre- and post-Wave-5 producer shapes so consumer filters survive the producer fold without inflating selection or compact output. Slice-2 verdict for `containment.{CONTAINABLE_KINDS, CONTAINER_KINDS}`: no dual-shape needed — the post-fold canonicals are already in those sets, and Wave-5 framework-linker producers don't reach this filter.
- **Phase 4a `x-deprecated` annotations**: `scripts/generate-schema` now emits `x-deprecated` on `#/$defs/Symbol/properties/kind` (50 entries) and `#/$defs/Edge/properties/meta/properties/evidence_type` (111 entries), mirroring the pre-existing `Edge.type` Phase 4a shape. Values stay valid in the open schema for the deprecation window. Phase 4b ships piecewise as each Phase 3 cluster's `awaits_bakeoff_validation` tag clears.
...
- **Six linkers walk transitive base-class chains**: `airflow_framework_dispatch`, `django_orm_dispatch`, `jackson_dispatch`, `kafka_streams_dispatch`, `view_template`, `react_component`, and the Go ttrpc / CSI path in `grpc.py` now BFS over `extends`/`implements` edges to discover ancestors whose `meta.base_classes` names a framework base. Fixes the dominant real-world pattern where projects extend an in-tree intermediate (`AlloyDBWriteBaseOperator(BaseOperator)`, JPA `@Entity` extending `@MappedSuperclass`, Kafka Streams SAM wrappers, `LeafController(ApplicationController)`, project-internal React base components, ttrpc `UserHealth` → `BaseHealthImpl` → `HealthService`) rather than the framework class directly. New shared helper `linkers/_transitive_bases.py` (cycle-guarded BFS) is the single source of truth; `collect_transitive_base_names` accepts a `meta_keys` tuple so kafka_streams can fold `extends` and `implements` together. Real-world testing on airflow and pretix had previously seen 0/9 and 0/6 transitive cases under the prior direct-only matcher. The `react_component` change also implements the base-class branch its docstring previously claimed (the code matched only on PascalCase). 20 new property tests + 4 helper-extension tests.
- **Django generic CBVs inherit View lifecycle methods**: `django_orm_dispatch.DJANGO_BASE_METHODS` entries for `ListView` / `DetailView` / `CreateView` / `UpdateView` / `DeleteView` / `TemplateView` now fold in `dispatch`, `setup`, `http_method_not_allowed`, `options`, the seven HTTP verbs, `head`, and `trace`. Django's class hierarchy is external to the project, so the transitive base-class walk above had no in-tree edge to follow — a project class `class Foo(ListView)` previously matched only `ListView`'s frozenset and never reached `View`. Real-world testing on pretix had previously seen zero `dispatches_to` edges to any `*.dispatch` method graph-wide. New module-level `_VIEW_LIFECYCLE` constant is the single source of truth.
- **`type_hierarchy` linker emits skip-level overrides**: the parent→children map is now closed transitively before edge emission. When `Grandparent.foo` is overridden only in `Grandchild` (intermediate `Parent` doesn't override), the edge `Grandparent.foo → Grandchild.foo` is emitted; previously `Grandchild` was missing from `parent_to_children[Grandparent]` because the map was one-hop. New `close_parent_to_children_transitively` BFS helper preserves diamond-no-double-emit and direct-override semantics. 6 closure-helper + 4 property tests.
