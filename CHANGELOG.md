...
Attribute reads on imported modules (e.g. `os.environ`, `process.env`, `System.out`) are declared as primitives in `io_primitives/*.yaml` but were inert metadata — no analyzer emitted matching edges, so chains through them were invisible to the boundary map and to `verify-claims`. This release introduces a new `module_attr_ref` edge type and emits it across six languages.

- **Python attribute reads (WI-guhok).** The Python AST analyzer emits `module_attr_ref` edges for bare attribute reads on imported modules (skipping the `.func` of a call, which the calls pipeline already handles). `tag_io_boundaries` and `_TRACEABLE_EDGE_TYPES` accept the new type, so attribute primitives reach `primitive_counts` and the backward-BFS that populates `entry_points`. Self-analysis: `env_read` chain_count went from 3 (only `platform.*` calls) to covering all 39 `os.environ` / `os.getenv` sites.
- **Tree-sitter helper `emit_module_attribute_refs` in `analyze/base.py` (WI-gapam).** Walks attribute-access nodes (`member_expression` / `field_access` / `selector_expression` / `field_expression` depending on the grammar), resolves the base against an imports map, and emits `module_attr_ref` edges. Exposed as a module-level function so analyzers that override `analyze()` entirely (notably JS/TS) can use it.
- **Per-language wire-up (WI-lozug, four PRs).** The helper is now called from the Go analyzer (`os.Stdout` / `Stderr` / `Stdin`), the JS/TS analyzer (`process.env`, `window.*`, `document.*`, `navigator.*`, imported modules), and the Java analyzer (`System.out` / `err` / `in` plus imported-class field reads; callee-skip is disabled for Java so `System.out.println` receivers still emit). Tree-sitter Rust is covered by WI-vipur; a regression-guard stub landed as the fourth PR and is flipped in that ticket.
- **Scoped-path mode for left-recursive grammars (WI-vipur).** `emit_module_attribute_refs` gains `scoped_path=True`; the tree-sitter Rust analyzer now emits `module_attr_ref` edges for `std::env::consts::OS` and similar `scoped_identifier` reads. `IoBoundaryCatalog._rebuild_indices` registers a dot-normalized alias for any `::`-containing qualified name so catalog matching still hits. The optional `rust-analyzer` backend is unaffected. C++ `qualified_identifier` is the next candidate (pending `cpp.yaml` additions, tracked as **WI-zojid**).
- **C stdio globals via bare-identifier matching (WI-gotuv).** `c.py` emits `module_attr_ref` edges for non-shadowed bare references to `stdout` / `stderr` / `stdin`, walking the tree twice to collect per-scope shadow names before emission. Activates the previously-inert `attributes:` entries under `stdio` in `c.yaml` (`ipc_send` / `ipc_recv`). C++ `using namespace std;` + bare `cout` / `cerr` / `cin` is the same problem with a different catalog shape, tracked as **WI-zojid**.

#### verify-claims — project-local taint catalogs (WI-votan)

- **New flags `--taint-sources PATH` / `--taint-sinks PATH` / `--taint-sanitizers PATH`** (repeatable; each PATH is a YAML file or a directory globbed as `*.yaml` in sorted order). The claims YAML can carry the same paths under a top-level `extra_catalogs: {sources, sinks, sanitizers}` key; relative paths resolve against the claims-file directory.
- **Override semantics:** user source/sink entries whose `(module, name, kind)` triple matches an auto-derived or built-in entry replace it; user sanitizers concatenate. A one-line stderr summary reports when any extra catalog paths were loaded. Public helper: `hypergumbo_core.taint.load_full_taint_catalog(extra_source_paths, extra_sink_paths, extra_sanitizer_paths)`.
- **Unblocks the ADR-0017 project-local-catalog story.** Before this PR, `load_builtin_taint_catalog()` only scanned the installed package's `io_primitives/` directory inside `site-packages`, so end users had paranoid auto-derived defaults with no escape hatch for new trust zones, sanitizers, or per-project trust-level overrides.

### Changed

#### Taint catalog auto-derivation (WI-lokuv)

- **Taint sources/sinks now auto-derive from `io_primitives/*.yaml`.** Default zone/label map: `fs_write`→`host_fs`, `net_send`→`network`, `subprocess`→`host_fs`, `env_write`→`host_env`, `ipc_send`→`ipc`, `browser_storage_write`→`browser_storage`; reads `env_read`→`host_secret`, `net_recv` / `ipc_recv`→`untrusted_input`. Hand-written YAML entries override on `(module, name, kind)` match. 419 previously-uncovered primitives now flow through `verify-claims`. The shipped `taint_sinks/host_filesystem.yaml` + `taint_sinks/network_send.yaml` are removed (they duplicated io_primitives), and WI-hizik's manual drift-guard (`test_taint_sink_drift.py` + its baseline) is retired because drift is now structurally impossible.
...
- **`TaintSource` / `TaintSink` `kind` field accepts `"attribute"`** so auto-imported attribute-style primitives become first-class records; `module_attr_ref` added to `TAINT_CALL_EDGE_TYPES` so structural propagation walks the new edge type.
- **New `browser_storage_write` io_primitives category (WI-lokuv Q3).** `localStorage.setItem` / `sessionStorage.setItem` / `.clear` / `.removeItem` moved out of `javascript.yaml#fs_write`. Auto-import now routes them to the `browser_storage` trust zone instead of misreporting them as host-filesystem writes.
- **New `browser_storage_read` io_primitives category (WI-kanir-huzuj).** `localStorage.getItem` / `sessionStorage.getItem` / `indexedDB.open` / `caches.{open,match,has,keys}` moved out of `javascript.yaml#fs_read`. Intentionally NOT in `AUTO_SOURCE_LABEL_MAP` (mirrors `fs_read`) — sensitivity of a browser-storage read depends on what is stored, so the default stays quiet and project-local catalogs can add `taint_sources` entries for their specific threat model. `document.cookie` stays under `env_read` pending a getter/setter split.

### Fixed

#### CI / build system

- **`release.yml` security-audit: `pip-audit` CVE-ignore brought in line with `ci.yml`.** The pre-publish scan was missing `--ignore-vuln CVE-2025-71176`, so every release attempt failed the security gate on the known pytest 9.0.2 TOCTOU advisory (dev-only transitive pin from `syrupy 4.8.0` via `pytest-textual-snapshot 1.1.0`; local attack boundary only; single-tenant self-hosted runner). The ignore will drop in both workflows once `Textualize/pytest-textual-snapshot#24` ships. Surfaced when the initial `v2.7.0` tag's `release.yml` run was rejected after the release PR had already merged.
