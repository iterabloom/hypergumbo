...
- **Python `pyproject.toml` dependency manifest** (WI-nunuj): new `py_deps.parse_python_dependencies(repo_root)` parses `[project].dependencies`, `[project.optional-dependencies]`, and `[tool.poetry.dependencies]` with stdlib carve-out and dist-name→import-name resolution via `importlib.metadata.packages_distributions()` (e.g. `PyYAML` → `yaml`, `scikit-learn` → `sklearn`). Wired into `analyze_python`; `ir.create_boundary_nodes`'s manifest-aware allow-list now extends to Python so declared deps classify tier-2 instead of tier-3.
...
- **Injection output reads as a reference document, not a list of bare ids** (WI-bodog): wrapper header is now `[Transcript Analysis — N relevant document(s)]`; each block opens with `--- <natural title> — <repo-relative path> ---` (title parsed from the first H1/H2/H3 heading); leading SPDX comment is stripped; a one-line framing hint follows: `If the task below matches, consult this instead of re-reading the file.`. Empirical motivation: WI-fusak overlap sweep found 5 of 11 Read calls on playbook files happened *after* the pipeline had already injected the same playbook — the agent wasn't recognising the injected block as the document it was about to fetch.

#### Language support

- **Jsonnet registered in `taxonomy.LANGUAGES`** (WI-gijot): the existing jsonnet analyzer was emitting `Symbol(language='jsonnet', ...)` and jsonnet-prefixed dst ids, but jsonnet was missing from the language registry, causing the strict INV-nodij validator to misflag boundary nodes synthesized from jsonnet sources (50 nodes on alertmanager, 63 on prometheus — Grafonnet/Tanka). Added under `FileRole.CONFIG` with `.jsonnet` / `.libsonnet` extensions.

#### auto-pr

- **Orphan tracker-sync PR detection** (WI-dofaz): `auto-pr`'s post-success path now warns about open tracker-sync PRs whose `created_at` predates the current run (motivating incident: PR #3376 sat orphaned ~3 hours). Mid-cycle sync PRs are intentionally ignored (the next threshold-cross picks them up). Warning is best-effort, never affects exit code, and auto-merging is not attempted. New `find_open_tracker_sync_prs` helper in `scripts/lib/forgejo-api.sh` reuses the existing `api_get` plumbing.
- **Sync-gate inspection helper** (WI-nutin, paired with the tracker-side flock conversion): a shared `_autopr_sync_gate_held` bash helper uses `flock --shared --nonblock` to inspect `.git/TRACKER_SYNC_PENDING` without disturbing the holder, auto-cleans stale files, and renders a friendly diagnostic. Wired into `flush_queue`, `do_pr` preflight, and the post-`PR_PENDING` re-check.

### Changed

- **Shared `compute_dampened_centrality` helper extracts the duplicated centrality+dampener pipeline** (WI-tahum, follow-up to WI-lidum / WI-dohaf): new `ranking.compute_dampened_centrality(symbols, edges, *, first_party_priority=True, edge_type_weights=None, hub_threshold=100, within_file_weight=0.3, max_per_file_in=5, exclude_dampeners=())` is the single source of truth for the "compute_centrality + canonical 8-stage dampener stack" pipeline. Defaults match `rank_symbols`'s tuned values. An internal `_apply_canonical_dampeners` lets `rank_symbols` keep `raw_centrality` separate while still sharing the dampener logic. Three ranking sites collapse to one-liners — `sketch._format_symbols` (`exclude_dampeners=("file_kind",)`), `compact.select_by_coverage`, `compact.select_by_connectivity`. `format_tiered_behavior_map`'s victim-removal step migrates via a new `_VICTIM_REMOVAL_EXCLUDE_DAMPENERS` constant that makes the "tier-only victim ordering" choice explicit. `select_by_tokens` keeps its untuned `compute_centrality` call (used for coverage normalization, not ranking). Pure refactor — no behavior change.
...
- **Full-suite CI cadence switched to twice-daily** (01:00 / 13:00 UTC): replaces the every-4-hours schedule (00/04/08/12/16/20 UTC). Singleton concurrency is unchanged. AGENTS.md, the ci-debug + smart-test playbooks, and the transcript-change hook summary updated to match.

### Fixed

- **`compute_centrality` tuned parameters backfilled across four selection surfaces** (WI-dohaf, sibling to WI-lidum): sketch / `select_by_coverage` / `select_by_connectivity` / `format_tiered_behavior_map` previously called `compute_centrality(symbols, edges)` with bare defaults while `rank_symbols` used `hub_threshold=100`, `within_file_weight=0.3`, `max_per_file_in=5`, `edge_type_weights=DEFAULT_EDGE_TYPE_WEIGHTS`. `max_per_file_in=5` fixes the 'createYAMLNode called 46 times from 1 file' pattern; `hub_threshold` saturates infrastructure utility hubs; `within_file_weight` dampens local-variable inflation; `edge_type_weights` makes imports count less than calls. 6-repo audit showed 19-71 of top-100 churn per surface (chatwoot's Vue/JS-heavy callgraph moved least — params were tuned against Go/Java/Python). Tagged `awaits_bakeoff_validation`.
- **Three selection surfaces aligned with `rank_symbols`'s 8-dampener stack** (WI-lidum, Phase B): the dampener pipeline tier → noise → utility → common_method → sibling_impl → trivial_sink → generated → file_kind now applies to every ranking surface, not just `rank_symbols`. Pre-fix coverage: `sketch._format_symbols` ran 3 of 8 (tier / utility / trivial-sink); `compact.select_by_coverage` ran 1 of 8 (tier only, conditional); `compact.select_by_connectivity` ran 0 of 8 in its `centrality is None` branch. 6-repo audit (alertmanager, prometheus, kserve, chatwoot, detekt, django) found 7-71 of top-100 churn per surface — driven by kserve OpenAPI model classes (`V1beta1InferenceService` et al., 42 of top-100 in sketch) and external symbols (`Sprintf` / `Background` / `package` / `module`) leaking into the connectivity seed pick. `apply_file_kind_weights` is skipped in sketch (`KEY_SYMBOL_KINDS` already excludes `kind="file"`). Tagged `awaits_bakeoff_validation`. Subsequently consolidated into the `compute_dampened_centrality` helper (WI-tahum, see Changed).
...
- **Vue analyzer — `imports_component` dst stuffed raw path into language slot** (WI-vobiv subset): same shape as WI-diruj. Vue now emits a properly-formed `vue:{path}:0-0:{name}:component` dst and stashes the raw import path on `edge.meta`; `vue_component.py` linker reads from meta with a fallback to dst for back-compat. Eliminated 871 malformed boundary nodes on chatwoot. Other producers leaking into the language slot (airflow .rst, jsonnet sources, kafka properties/manifest) tracked separately.
- **`manifest_targets` and bash `sources` edges — raw paths leaking into language slot** (WI-hugom): same INV-nodij invariant violation as WI-vobiv / WI-diruj. `manifest_targets._emit` (gradle/csproj `defines_target`) was leaking 34 nodes on kafka with `language='org/apache/kafka/common/protocol/Errors.java'` etc.; bash `sources` edges leaked 8 nodes with raw `/etc/kafka/...` paths. Both producers now emit 5-part dst ids using their per-extractor language token (`gradle` / `csproj` / `bash`); raw path stashed on `edge.meta.target_path` so the existing `build_target.py` linker keeps working.
- **Five extended1 analyzers — `dst="unresolved:{name}"` shape** (WI-bagon): luau / smithy / hack / jsonnet / apex previously emitted unresolved-call edges with a malformed 2-part dst that fell through `ir._parse_dangling_id` and produced `language='unresolved'` boundary nodes (39 nodes on alertmanager from the jsonnet producer). Adopted the mainstream `{lang}:unresolved:0-0:{name}:unresolved` shape. Six remaining analyzers (robot/racket/purescript/scheme/matlab/prisma) still emit a 3-part shape that produces correct language but malformed name/kind slots — deferred follow-up.
- **Rails exempt from `refine_frameworks` import-edge demotion** (WI-lohok): real Rails apps never have explicit `require 'rails'` import edges (Bundler autoloads at boot), so `refine_frameworks` was demoting Rails to `dev_frameworks`, suppressing every `controller`/`route`/`form`/`serializer` concept tag from `rails.yaml`. New `_AUTOLOAD_BY_CONVENTION_FRAMEWORKS` exemption set checked before the demotion; Rails is the only initial member, with a docstring inclusion criterion to keep additions principled. Sinatra (which IS explicitly required) stays demote-eligible — counter-test added.
- **Transcript-sync watcher doubling on SessionStart re-fires**: vendor sessions emit `session_id` on every lifecycle event (startup, resume, `/clear`, `/compact`); the unconditional `launch-transcript-sync.sh` call lacked a same-SID idempotence guard, so each re-fire stacked a fresh `sync-transcript.sh` watcher on top of the live one — uniform 2x duplication of every event in 4 of 132 archived sessions (~3% HIT). Two-layer fix: a same-SID PID-file kill before the orphan sweep, plus a `pgrep` fallback for watchers whose PID file was lost.
