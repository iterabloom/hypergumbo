...
Phase 4b closes the ADR-0027 / ADR-0028 concept-axis program: 111 `Edge.evidence_type` and 71 `Symbol.kind` endpoint_shape values are removed from their registries. Thirteen linker slices adopt a disambiguation-fallback contract (`confidence ≤ 0.5` + `meta["disambiguation_fallback"]=True` on ambiguous simple-name resolutions); a new L4 fallback-coherence linter enforces it statically and the L3 producer-coherence linter learns to see through inline ternaries, f-strings, and Constant sentinels. `Edge.dst_ref` (structured external-target sibling field) becomes canonical across all 8 analyzers and four high-leverage consumers, eliminating the historical Rust 6-segment dst fabrication at the producer. Per-entry-point safety claims (Phase 3) ship hypergumbo's own audited-IO-surface self-audit via a project-local taint catalog, wrapper discipline for fs-write sites, and SECURITY.md generator. The convention-based view-template linker grows from Rails-only to a five-framework family (shared core + Django, Phoenix, Spring MVC, Laravel Blade), and a third-party-base allow-list adds `dispatches_to` coverage for HierarkeyForm / FilterSet / DRF Serializer / Wagtail Page on Django-detected repos. New `hypergumbo run` flags `--gzip` and `--no-sketch-fan-out` shrink large-repo output ~10× on the wire; the results cache key now segments by analyzer identity, eliminating stable+dev cross-poisoning. Twelve bugfixes span release tooling, supply-chain tier classification (django/utils + Phoenix ExUnit), JS/TS access-mode coverage, framework-linker gating, interface-extends-interface dispatch, and `--backend rust-analyzer` install advice.

### Added

#### Third-party-base allow-list for Django-ecosystem dispatch BFS (WI-jifib)

Cascade implementation from WI-jusih's data pass. New module `linkers/_third_party_bases.py` registers `django-third-party-dispatch`, a framework-activation-gated linker (`LinkerActivation(frameworks=["django"])`) that emits `dispatches_to` edges from Python subclasses of four well-known third-party Django-ecosystem bases — `HierarkeyForm` (hierarkey), `FilterSet` / `WagtailFilterSet` (django-filter), DRF's `Serializer` / `ModelSerializer` / `HyperlinkedModelSerializer`, and Wagtail's `Page` — to their framework-called override methods. Companion to `django-orm-dispatch` (Django-stdlib bases, always-on); the new linker only fires when `profile.frameworks` reports `django` detected, eliminating cross-framework false-positives by construction. Edges carry `meta["framework_dispatch"] = "django_third_party"` for downstream filtering. INV-zuhub fallback discipline preserved: an unqualified short-name match that collides with an in-tree Python class downgrades to `confidence == 0.5` with `meta["disambiguation_fallback"] = True`. 42 new tests, 100% coverage on the new file.

Smoke test on lutris-website (DRF consumer, `django` framework detected): 3 new `dispatches_to` edges for `InstallerDraftWriteSerializer.create`, `InstallerDraftWriteSerializer.validate`, `RegressionWriteSerializer.create` — DRF `ModelSerializer` overrides that the existing `django-orm-dispatch` linker did not match (`Serializer` / `ModelSerializer` are not in `DJANGO_BASE_METHODS`). Bakerydemo (Wagtail) demonstrates the hard-cutoff gate: `profile.frameworks` is empty due to an orthogonal framework-detector bug (recursive manifest scan only matches files literally named `requirements.txt`, missing `requirements/base.txt` layouts), so the linker correctly skips the repo entirely.

Data-pass methodology errata recorded at `~/hypergumbo_lab_notebook/wi-jusih-data-pass-20260515/ERRATA.md`. Summary: the original WI-jusih report counted classes whose **chain reached** a candidate name, not classes that **also overrode** a framework-called method on that base — declarative-only subclasses (e.g., FilterSets with just `Meta.model`) receive zero edges. With the method-override filter applied, the corrected total across pretix + bakerydemo + lutris is ~38 method-edges (vs the reported 175); `ModelAdmin` was double-counted (already covered by `django-orm-dispatch`) and is excluded from the allow-list. The bucket-A verdict for the four genuine third-party gaps does not change.
...
- **`tests/test_rct_public_api_pinned.py`** — 9 introspection tests pinning the four monkey-patch surfaces the four-arm bundle RCT depends on. The variants (`mgumbo` perturbing `Edge.confidence`, `dgumbo` selectively disabling linker subcategories) attach by import path + signature; a signature change that keeps behavior identical still breaks the variants. Surfaces pinned: (1) `hypergumbo_core.ranking.rank_symbols` — exact `(name, kind, default)` tuple list for all six parameters; (2) `hypergumbo_core.ir.Edge` — eight RCT-critical dataclass fields (`id`, `src`, `dst`, `edge_type`, `line`, `confidence` default `0.85`, `is_resolved` default `True`, `dst_ref`) with type and default assertions; (3) `hypergumbo_core.cli.run_behavior_map` — ten of sixteen parameters with stable defaults (the unpinned six can be added/renamed without alarming variants); (4) the linker subcategory vocabulary tuple `_LINKER_SUBCATEGORIES` in `scripts/generate-architecture` plus the `LinkerActivation` field surface. Failure messages name the specific RCT dependency (mgumbo/dgumbo) so a future contributor sees the dependency before ``sed``-to-passing.
- **Source-comment markers** at each definition site (`ranking.py:1215`, `ir.py:414`, `cli.py:6647`, `scripts/generate-architecture:711`): one-line `# RCT-pinned surface — see tests/test_rct_public_api_pinned.py before changing ...` comment so the pin is discoverable from the source side too.

#### IO-boundary closed-world stdlib gating — F3 Filter 2 (PR-C)

- **`IoBoundaryCatalog.stdlib_modules` / `.stdlib_prefixes` / `.stdlib_module_completeness` fields + `is_stdlib_module()` / `is_stdlib_module_complete()` methods.** `stdlib_modules` is a `frozenset[str]` of authoritative stdlib module names (populated from `sys.stdlib_module_names` for Python; equivalent authoritative sources for other languages). `stdlib_prefixes` is a `tuple[str, ...]` for hierarchical-namespace languages (Java's `java.*`, Go's `encoding/*`); prefix match accepts `.` and `/` separators. `stdlib_module_completeness` is a `dict[str, str]` mapping module-name → audit-`retrieved:` date; modules listed here are treated as closed-world (we've audited every I/O primitive they expose). `merge()` unions all three across child/parent catalogs with the child winning on `completeness` collisions.
- **YAML parser support** for two `stdlib_modules` shapes — flat strings (the operator-script-friendly shape) and dicts with inline `completeness: complete` + `retrieved:` (still accepted) — plus a new top-level `stdlib_module_completeness:` section that the refresh script never touches. Closed-world entries auto-promote into `stdlib_modules` so the two sections stay in sync. Hard-errors at load time when `completeness: complete` is declared without a `retrieved:` ISO date.
- **F3 Filter 2 in `_compute_external_potential`** — after the existing `stdlib_other` check, if `module_hint` is non-empty AND `catalog.is_stdlib_module_complete(module_hint)`, skip the chain. Modules not flagged complete fall through to the prior behavior. Filter 2 fires only when we have a confident module identification (no skip when `module_hint == "external"`).
- **`scripts/refresh-stdlib-modules <language> --version X.Y`** — operator script that regenerates `stdlib_modules` from the live interpreter and writes back sorted between sentinel markers. Validates that the running interpreter matches both `--version` and `stdlib_provenance.version` in the YAML before touching anything. Deterministic: same script + same interpreter → byte-identical YAML output (test pins this). Only Python is implemented; other languages stub with a "not yet implemented" message and exit 1.
- **`python.yaml` populated** with 300 stdlib modules from Python 3.12's `sys.stdlib_module_names` (the version available in CI). `stdlib_provenance.version` bumped from `"3.13"` (claimed but never re-verified) to `"3.12"` to reflect what was actually enumerated; the refresh script will catch the next version drift. `math` is the inaugural worked-example closed-world entry in `stdlib_module_completeness` — math has no I/O primitives, so flagging it complete cuts `external_potential` noise for `math.sqrt` / `math.floor` / etc. on real repos without any audit risk.
- **Tests**: 21 new in `test_io_boundary.py::TestStdlibModulesAndFilter2` (dataclass fields, parser for both YAML shapes, prefix matching with `.` and `/`, merge, smoke test of shipped python.yaml, three filter 2 path-coverage tests), 8 new in `test_refresh_stdlib_modules.py` (version mismatch in either direction, determinism, first-time insert, unsupported language, missing-yaml). 100% coverage maintained.
- **Closes** `WI-potuh-hukav-nobum-polag-bikar-pozab-haboh-nufud` (F3 noise reduction) — Filter 2 ships per plan; the closed-world surface starts narrow (one module) and expands per future hand audits without further code changes.
...
- **WI-mafik Level 1b audit — polyglot call-site coverage fixtures for 7 non-Python languages.** `test_polyglot_call_site_coverage.py` extends from 1 fixture (Python baseline) to 8 (adds JS/TS, Go, Rust, Java, C++, Ruby, Elixir). Each fixture exercises the canonical import-times-call constructs for its language; the analyzer must emit a `calls` edge whose dst contains the expected `(module, callable)` substrings. Verdict at audit time: 1 passed (Python), 7 xfailed — every non-Python analyzer has at least one missing-edge or unresolved-alias gap. Each xfail carries a `strict=True` reason naming the gap and the originating analyzer file; per-language fix work items are filed under WI-mafik (WI-kujom JS/TS, WI-vovum Go, WI-volob Rust, WI-hudud Java, WI-rupik C++, WI-rijij Ruby P3 catastrophic, WI-rakul Elixir). Strict xfail surfaces analyzer improvements as `XPASSED` regressions, ensuring fix-side PRs must remove the xfail markers and re-affirm the audit.

#### WI-tihup + WI-mafik closure — 7-language `dst_ref` adoption + consumer migration (PR2)

- **`SCHEMA_VERSION` 0.7.2 → 0.8.0 — `Edge.dst_ref` becomes canonical for external targets.** Producer-side source of truth shifts: every non-Python analyzer now populates `Edge.dst_ref` at external-target call-emit sites, and high-leverage consumers (`io_boundary`, boundary-node creation in `ir.create_boundary_nodes`) prefer the structured field over the legacy colon-split heuristic. The legacy `Edge.dst` string stays populated alongside indefinitely as the back-compat / cache form. Minor bump because consumer semantics shift, not because the wire format is breaking — pre-0.8.0 cached JSON still loads cleanly with `dst_ref=None` per the defensive `from_dict`.
- **`make_unresolved_edge` accepts `dst_ref` kwarg.** The shared analyzer helper at `hypergumbo_core.analyze.base` now threads the structured ref through to `Edge.create`, so every call site that opts in picks up dst_ref by passing one kwarg.
- **`hypergumbo_core.ir.format_legacy_dst(ref)`** — uniform `{lang}:{module_path}:0-0:{name}:unresolved` builder from an `ExternalRef`. Available for analyzer call-emit sites that want to keep dst-string and dst_ref coherent.
- **Java (`java.py`, WI-hudud).** `_extract_static_imports` collects `import static X.local_name` into a `{local_name: source_module}` dict plumbed through `_extract_edges`; bare-call emit consults it. Also populates `dst_ref` on receiver-imported method calls (`Arrays.asList`, `Collections.emptyList`).
- **Go (`go.py`, WI-vovum).** `_extract_dot_imports` collects `import . "strings"` package paths; bare-identifier calls that fall through the resolver get an unresolved edge attributing them to the first dot-imported package (`go:strings:0-0:Contains:unresolved`). New `FileAnalysis.dot_imports` field in `analyze/base.py`.
- **Elixir (`elixir.py`, WI-rakul).** Aliased-module fallback `S.upcase(...)` after `alias String, as: S` now resolves to `String` in the dst (was `S`). Explicit `import Mod, only: [name: N]` + bare `name(...)` emits an unresolved edge keyed to the source module (was silently dropped by the auto-import stdlib skip).
- **JS/TS (`js_ts.py`, WI-kujom).** `_extract_named_imports` now returns `(named_imports, named_import_originals)`; the latter maps local-alias → original imported name. Call-emit at the named-import unresolved branch attributes to the canonical name (`writeFile`) rather than the local alias (`wf`).
- **C++ (`cpp.py`, WI-rupik).** Pre-walks the AST to collect system `#include <header>` paths; unresolved external calls in files with system includes carry a comma-joined module hint that surfaces header context (`cpp:cstdio,cstring,cstdlib:0-0:printf:unresolved`). Semantics: "this call could be from any of these headers".
- **Rust (`rust.py`, WI-volob).** Unresolved emit consults `use_aliases` in three new cases: (1) qualified `fs::read_to_string` where `fs` was use'd from `std::fs` → dst becomes `rust:std::fs:0-0:read_to_string:unresolved`; (2) bare `write` from `use std::fs::write` → dst becomes `rust:std::fs:0-0:write:unresolved` even though `write` is in the generic-trait-method skip list (explicit binding wins); (3) aliased terminal `use ... as mkdir` → dst uses canonical `create_dir`. Eliminates the historical 6-seg Rust outlier (`rust:external:0-0:fs:0-0:read_to_string:unresolved`) at the producer.
- **Ruby (`ruby.py`, WI-rijij — P3 catastrophic).** `_try_receiver_call` grows a constant-external fallback: when a constant receiver (`JSON`, `Set`, `::JSON`) doesn't resolve to a project symbol and no `#initialize` matches for `.new(...)`, emit an unresolved edge keyed to the `require_hints`-derived source module (or lowercased class name as heuristic). `Set.new(...)` no longer silently swallowed; `JSON.parse(...)` no longer emits zero edges. The original Rails-controller-`#new` guard is preserved: project-symbol receivers still return without emitting (no false edge to a Rails `def new` action).
- **Consumer migration (4 high-leverage sites).** `io_boundary._classify_call_edges` and `io_boundary._compose_primitive_chains` prefer `edge.dst_ref.module_path` / `.name` over the legacy `_extract_module_hint` / `_extract_callee_name` parsing when `dst_ref` is present. `ir.create_boundary_nodes` builds a `dangling_id → ExternalRef` map from edges and short-circuits `_parse_dangling_id` for refs-carrying ids — this is the path that eliminates the historical 6-seg Rust fabrication of the extra `0-0` slot.
- **Polyglot fixture xfail removal.** All 7 non-Python `xfail_reason` fields stripped from `test_polyglot_call_site_coverage.py`; the test now demands 8 fixtures pass (was 1 passed, 7 xfailed). Re-introducing an `xfail_reason` is a regression signal.
- **Tracker closure.** Seven WI-mafik children close `done` with rationale "acceptance criterion met — xfail removed in this PR": WI-kujom (JS/TS), WI-vovum (Go), WI-volob (Rust), WI-hudud (Java), WI-rupik (C++), WI-rijij (Ruby), WI-rakul (Elixir). WI-mafik umbrella closes `done`. WI-tihup closes `done` citing the additive sibling-field pattern (ADR-0028 precedent) + per-analyzer adoption + four high-leverage consumer migrations. All three umbrella items tagged `awaits_bakeoff_validation`.

#### WI-tihup foundation — structured external-target IR + ImportScope (PR1)

- **`SCHEMA_VERSION` 0.7.1 → 0.7.2 — `Edge.dst_ref` structured sibling field.** Additive ADR-0028-shape sibling: `Edge.dst_ref: ExternalRef | None = None` carries the canonical `(lang, module_path, name)` identity for external-target edges, replacing the per-analyzer string-formatted dst as the canonical source of truth. The legacy `Edge.dst` string stays populated alongside for back-compat with the ~34 consumer sites that haven't migrated. `Edge.create()` accepts `dst_ref` kwarg; `to_dict()` omits the field when None; `from_dict()` defensively reads `d.get("dst_ref")` so pre-0.7.2 cached JSON loads cleanly with `dst_ref=None`. Schema regen via `./scripts/generate-schema` adds the field to `docs/schema.json` as `oneOf: [object, null]`. The Phase 1 trace cited in the umbrella plan caught a specific parsing-bug class behind Rust's anomalous 6-seg dst (`_parse_dangling_id` mis-handling `::` in callee names) — structured `ExternalRef` eliminates that class by construction.
- **`hypergumbo_core.ir.ExternalRef`** — frozen dataclass `(lang, module_path, name)` with `to_dict()` / `from_dict()` round-trip. Aliased imports (`from X import Y as Z`) bind `name=Y` not `Z` — the alias is a property of the call site, not of the target.
- **`hypergumbo_core.import_scope.ImportScope`** — shared analyzer-layer abstraction for per-file import-binding bookkeeping. Five population methods (`add_named`, `add_namespace`, `add_wildcard`, `add_aliased_module`, plus the alias-of-namespace variant) mirror the construct shapes across the 8 audited languages. `resolve(local_name)` returns a `CanonicalName(module, name)`; convenience `dst_ref_for(local_name, lang)` returns an `ExternalRef` directly. Backing representation is two dicts mirroring py.py's pre-existing `_extract_imports` shape. PR2 will retrofit the 7 non-Python analyzers against this API.
- **Python reference adoption (`py.py`).** All five external-target call-emit sites in `py.py` (around lines 3232, 3246, 3287, 3306, plus `_extract_import_edges` at line 1888) now construct `dst_ref` inline alongside the legacy dst string. Sites where the module is `external` / `unresolved` sentinel leave `dst_ref=None`. The `test_py_analyzer_populates_dst_ref_on_polyglot_imports` regression guard exercises the canonical WI-zigah Case A path end-to-end through `run_behavior_map`.
- **14 ImportScope unit tests + 7 ExternalRef/dst_ref tests.** Round-trip, backward-compat (missing `dst_ref` key in legacy JSON), resolve precedence (named beats namespace), wildcard non-shadowing, hashability.
...
- **F3 Filter 3 — composition fix for self-prefixed dst names.** When the dst node's `name` field already carries the module-qualified form (e.g. `re.MULTILINE`) and the extracted `module_hint` is the same module (`re`), the naive prepend produced `re.re.MULTILINE`. Filter 3 detects `dst_name.startswith(module_hint + ".")` and skips the prepend; same for `ast.ast.Name`, `os.os.path`, `datetime.datetime.now`. Cuts ~30 chains and removes the most reader-noticeable noise from the `external_potential` bucket.
- Three new tests in `test_io_boundary.py` (filter 1 skip, filter 1 sanity-with-resolved, filter 3 composition). Existing `test_compute_external_potential_uses_dst_ref` updated to `is_resolved=True` since the test's purpose was dst_ref composition, not resolvability. 100% coverage maintained. Lands `WI-potuh-hukav-nobum-polag-bikar-pozab-haboh-nufud` partially; Filter 2 follows in PR-C.
...
- **WI-limas — `Pattern.meta_match` for post-Wave-6 fold rules.** Wave 6 / ADR-0027 folded several legacy `Symbol.kind` values onto canonical kinds plus per-axis `Symbol.meta` keys (`devDependency` → `dependency` + `meta["dependency_scope"]="dev"`, `tsconfig` → `file` + `meta["config_format"]="tsconfig"`, `composer_package` → `package` + `meta["package_ecosystem"]="composer"`, etc.). The corresponding YAML rules in `frameworks/config-conventions.yaml` still hardcoded the pre-fold bare-kind regexes and silently matched nothing. New `Pattern.meta_match: dict[str, str]` field re-binds rules to the new emission shape: each key must be present on `Symbol.meta` and its `str()`-coerced value must match the regex. Multiple meta_match keys are AND'd; `meta_match` can be combined with `symbol_kind` / `framework_role` (also AND'd) or used standalone. 11 rules migrated across `config-conventions.yaml` (`npm_dev_dependency`, `npm_script`, `typescript_config`, `composer_dependency`, `cargo_dev_dependency`, `cargo_build_dependency`, `cargo_workspace_member`, `cargo_library`, `pyproject_script`, `poetry_dev_dependency`) and `language-conventions.yaml` (`build_rule` → Starlark `target` kind). Producer `toml_config.py` now emits `meta["dependency_scope"]="dev"` / `"build"` for cargo `[dev-dependencies]` / `[build-dependencies]` tables so the YAML rules can disambiguate. 18 unit + per-rule pinning tests cover the new field and each migrated rule. Surfaced by the WI-fojun audit (2026-05-11).
...
- **`SCHEMA_VERSION` 0.6.0 → 0.7.0 — `Edge.evidence_type` endpoint_shape closure.** All 111 endpoint_shape values are removed from `EVIDENCE_TYPES` after Phase 3 producer migration completed across Waves 3-5 and per-cluster bakeoff validation cleared:
  - 18 resolution-status leaks (`*_unresolved`) → canonical inference label + `Edge.is_resolved=False` (audit-findings 0008, Wave 3).
  - 65 framework-dispatch values → canonical inference + `meta["framework_dispatch"]` / `meta["detection_pattern"]` (audit-findings 0014, Wave 5).
  - 28 call-construct peers → apex `ast_call` + `meta["call_construct"]` / `meta["receiver"]` / `meta["resolution_quality"]` (audit-findings 0012, Wave 4).

  `AXIS_ENDPOINT_SHAPE` is retained as a back-compat import target (no longer in `VALID_AXES`); `generate-concept-axes` drops the now-empty evidence_type endpoint_shape section. 91 audit-findings rows advance PRELIM_RESOLVED → RESOLVED. Closes Wave 8 on the evidence_type axis — the ADR-0027 + ADR-0028 cross-axis Phase 3+4 program is complete.
...
- **`SCHEMA_VERSION` 0.5.8 → 0.6.0 — `Symbol.kind` endpoint_shape closure.** All 71 endpoint_shape values are removed from `SYMBOL_KINDS` after Phase 3 producer migration completed across Waves 3-6:
  - 4 apex/peer (`fn`, `proc`, `structure`, `var`) → canonical `function` / `procedure` / `struct` / `variable` (audit-findings 0009).
  - 29 framework roles (`event_publisher` / `ipc_*` / `crypto_*` / `grpc_*` / `websocket_*` / `dispatcher` / `rpc` / `service` / etc.) → canonical kind + `meta["framework_role"]` (audit-findings 0013).
  - 9 edge labels — 5 DEPRECATE-NO-FOLD (relationship lives on the Edge); 4 (`function_call`, `subprocess_call`, `db_query`, `abi_call`) → `call_site` + `meta["call_kind"]` (audit-findings 0010).
  - 1 component ref DEPRECATE-NO-FOLD (audit-findings 0011).
  - 11 file-shape → `file` / `package` + meta keys (`module_system`, `component_framework`, `package_ecosystem`, `entry_role`, `config_format`) (audit-findings 0005).
  - 9 build/config → `test` / `requirement` / `dependency` / `task` / `block` + meta keys (`test_dialect`, `install_mode`, `install_source`, `task_implementation`, `dependency_scope`, `block_type`) (audit-findings 0006).
  - 8 long-tail DEPRECATE-NO-FOLD (audit-findings 0007).
...
Thirteen linker slices adopt the contract `confidence ≤ 0.5` + `meta["disambiguation_fallback"]=True` when a simple-name resolution lands on a structurally ambiguous target — extending the inheritance-linker pattern introduced in v4.1.0. All sites are written in the canonical IfExp / mutation-form shape that the L4 walker accepts.
...
- **`grpc.py` service-symbol lookup.** `service_sym_by_name` and `service_by_normalized` become multi-value; `grpc_service` Symbols carry `meta["proto_package"]` and the routes_to lookup uses it as a precision disambiguator. Bridge `dispatches_to` (Go servicer stubs, Python servicer classes) has no impl-side package metadata, so multi-candidate matches always drop. +5 property tests.
- **`subprocess_cli` protocol linker.** `command_by_name` becomes multi-value; multi-`[project.scripts]` repos where two binaries each register a `list` subcommand resolve to deterministic-by-id fallback. +2 property tests.
- **`method_call_recovery` chained-method recovery.** When multiple class hints each contain a method matching the unresolved short name, the line-proximity tiebreaker drops to `confidence=min(base, 0.5)` + flag. +2 property tests.
- **Simple FFI-bridge `calls`.** `cgo`, `napi`, `lua_ffi`, `ruby_ffi` switch their bare-name C/C++ lookups to `dict[str, list[Symbol]]`; cross-file collisions emit deterministic-by-id with the fallback flag. +8 property tests across the four bridges.
- **JNI linker dual-key lookup.** `jni_lookup` switches to `dict[str, list[Symbol]]`, indexed under both short (`ClassName.method`) and FQ (`package.ClassName.method`) name keys. Cross-package short-name collisions (e.g. `Java_pkg1_MyClass_processData` and `Java_pkg2_MyClass_processData` both indexed under `MyClass.method`) emit deterministic-by-id with the fallback flag when the Java side resolves by short name; FQ-form resolutions stay precision. +3 property tests.
- **PyFFI linker dual-lookup.** `c_lookup` (ctypes / cffi) and `pyo3_lookup` (Rust PyO3) both switch to multi-value. `pyo3_lookup` de-duplicates per key so a bare-function symbol (where `full_name == short_name`) appears once per key; cross-impl short-name collisions (e.g. `PyTokenizer::encode` and `PyParser::encode` both indexed under `encode`) drop to fallback. The stdlib-then-resolved branch and the non-stdlib branch carry the fallback flag symmetrically. +4 property tests.
...
#### Documentation drift sync (no code-behavior changes)
