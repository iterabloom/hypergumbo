...
The `Edge.edge_type` axis migration (ADR-0023) reaches Phase 4b: 33 deprecated `endpoint_shape` values are removed from `EDGE_TYPES` (`SCHEMA_VERSION` 0.3.1 → **0.4.0**) and the pre-commit drift gate is flipped to `--strict`. The audit framework that produced ADR-0023 generalises into ADR-0024 (axis declaration template) plus a new `docs/audits/` document series; ADR-0027 / ADR-0028 (Draft) instantiate the template for `Symbol.kind` and `Edge.evidence_type`. New `is_example_file` / `is_config_file` Symbol booleans round out the file-role flags.

### Added

#### Concept-axis declarations

- **ADR-0024 — Axis Declaration Template for Multi-Value Fields**: formalises the four-part template (axis name, axiom, consumer pattern, enforcement) and the seven-step declaration workflow ADR-0023 demonstrated concretely. ADR-0023 is reframed as the worked example; future axis-shaped fields (`Symbol.kind`, `Edge.evidence_type`, …) instantiate this template. AGENTS.md gains an "Axis declaration for multi-value fields" essentialization in Required Checks.
- **ADR-0027 (Draft) — `Symbol.kind` names the source-language syntactic construct**: instantiates ADR-0024 per the deep audit's DEPRECATE verdict (192 values across 8 clusters). Framework-participation values fold to canonical + `meta["framework_role"]`; edge-label values reclassify or drop. Migration follows ADR-0023's four-phase shape; per-cluster verdicts file as audit-findings docs.
- **ADR-0028 (Draft) — `Edge.evidence_type` names the inference pathway**: instantiates ADR-0024 per the deep audit's DEPRECATE verdict (210 values across 4 clusters) and confirms ADR-0023 Open Question #2. Resolution status promotes to a new sibling `Edge.is_resolved: bool` per the fold-residue-discipline recurrence-promotion rule; framework-dispatch conventions and apex/peer call variants fold to canonical + meta. Largest concept-axis migration on the roadmap (~140 production files have `evidence_type` literals).
- **`docs/audits/` — audit-findings document series**: sibling to `docs/adr/` for per-value verdict tables produced by applying existing methodology (typically the Fundamental Concept Audit) to a specific scope. Format spec at `docs/audits/README.md`; verdict rows carry `value` / `verdict` (CANONICAL | FOLD | DEPRECATE-NO-FOLD) / `fold_target` / `status` / `diagnostic_test` / `rationale`. Parser at `hypergumbo_core.audit_findings`; pre-commit gate at `scripts/check-audit-findings`.
- **`docs/blind-spots.md` — generative-question catalog for the audit lifecycle**: diagnostic complement to the Fundamental Concept Audit playbook. Catalogs four recurring question-shapes (typing axis vs values, assumed input boundaries, silently-load-bearing failure modes, null results read as confirmation) with examples and triggers.
- **Generative-question bake-ins**: the agentic-session-retrospective playbook gains a 5-minute check between Phase 4 (Synthesize) and Phase 5 (Record); the structural-fix-scope-expansion-protocol gains a pre-refactor blind-spots check. Both gracefully degrade when `docs/blind-spots.md` is absent.
- **Fundamental Concept Audit playbook + cadence hook** (`.agent/hooks/_shared/check_audit_cadence.py`): domain-neutral procedure for detecting conceptual leaks via four leakage tests; per-pair Keep verdicts require a written re-evaluation trigger. Cadence hook reads `.agent/.last_concept_audit.json`, counts dev commits since the last audit, and prints a soft reminder past a configurable threshold (default 72). New `scripts/concept-audit-record` updates the state file. Wired into all four supported vendor session-start hooks.

#### Edge-type registry & tooling

- **Canonical edge-type registry** (`hypergumbo_core/edge_types.py`): single source of truth for `Edge.edge_type` values, each annotated with an axis classification (`relationship`, `endpoint_shape` for deprecation candidates, or `pending_classification` for families deferred to per-family audit). `scripts/generate-schema` consumes the registry and emits a new `x-axis-of-values` JSON Schema extension on `Edge.type`. Property test AST-walks the package source and fails CI if any module-level `*EDGE_TYPE*` set contains a value absent from the registry.
- **Generated by-axis view at `docs/concept-axes.md`** (`scripts/generate-concept-axes`): human-readable axis grouping of the canonical registry. Pre-commit freshness check mirrors the existing schema-freshness gate.
- **Pre-commit edge-type drift linter** (`scripts/check-edge-type-drift`): catches consumer-side hardcoded `*EDGE_TYPE*` sets that drift from the canonical registry before push. Wired into `.githooks/pre-commit`; only fires when `packages/*.py` files are staged. Detection logic shared between the pre-commit gate and the property test.
- **Drift-linter `--strict` mode**: opt-in axis-principle check that additionally enforces consumer-set values are on the relationship axis (or pending_classification). Implementation is field-agnostic via `axis_drift.find_drift(allowed_axis_names=, name_to_axis=)` so future axis-bearing fields inherit the pattern per ADR-0024's enforcement template.
- **Edge-type runtime coherence checker** (`scripts/check-edge-type-runtime-coherence`): the runtime half of ADR-0023 §3's two-layer enforcement — partitions emitted edges in a behavior map by `(src.kind, src.language, dst.kind, dst.language)` and reports partitions where `edge_type` varies. Allow-list at `docs/edge-type-runtime-allowlist.yaml`; ADR amendments required for growth.
- **`docs/migrating-edge-types.md` — downstream consumer migration guide**: rename table, meta-key vocabulary (`bridge_kind`, `channel_kind`, `mechanism`, `construct`, `dispatch_kind`, `protocol`), worked patterns, and the deprecation timeline. Targeted at consumers of the behavior-map JSON.
...
- **Phase 4b — 33 deprecated edge_types removed from registry** (`SCHEMA_VERSION` 0.3.1 → **0.4.0**): the bakeoff-validated cohort across the dst-kind (6), bridge (7), IPC (7), and dispatch/publish (13) families is removed from `EDGE_TYPES`. The 25 sweep additions (protocol-call + long-tail) stay until their producers migrate. Consumer enumerations (`IMPORT_EDGE_TYPES`, `compact.CROSS_CUTTING_EDGE_TYPES`, `ranking.DEFAULT_EDGE_TYPE_WEIGHTS`, `io_boundary._TRACEABLE_EDGE_TYPES`, `taint.TAINT_CALL_EDGE_TYPES`, `cli._REACHABILITY_EDGE_TYPES`, `bakeoff-deep::_CALL_FLOW_EDGE_TYPES`) cleaned up; test fixtures rewritten to canonical shape.
- **Phase 4a — schema deprecation annotations** (`SCHEMA_VERSION` 0.2.7 → **0.3.0**): `Edge.type` gains an `x-deprecated` JSON Schema extension listing every endpoint_shape value as a removal candidate. Per-value migration guidance lives in the registry's `EdgeTypeSpec.description` (canonical fold target + meta key + ADR cite).
- **Phase 3 — protocol-call family folded** to canonical `calls + meta["protocol"]` across `linkers/http.py` / `grpc.py` / `graphql.py`. Registry adds `PROTOCOL_KINDS = frozenset({"ipc", "http", "grpc", "graphql"})` parallel to `BRIDGE_KINDS`. Tagged `awaits_bakeoff_validation`.
- **Phase 3 — IPC family folded** (Tauri / Electron / Phoenix Channels / WebSocket / message-queue producers): `ipc_calls → calls + meta["protocol"]="ipc"`; `ipc_event` / `message_send` / `websocket_message` / `message_queue` → `event_publishes + meta["channel_kind"]`; `websocket_connection → references + meta["construct"]="websocket_endpoint"`. `message_receive` producers dropped per the audit's DEPRECATE-NO-FOLD verdict (forward `event_publishes` already captures the relationship).
- **Phase 3 — publish/dispatch family folded** across 17 emit sites in 12 linkers: `routes_to → dispatches_to + meta["dispatch_kind"]="route"`; the dispatch/`mechanism`/`channel_kind` rename table per audit-findings 0001. `event_subscribes` producer in `event_sourcing.py` dropped per DEPRECATE-NO-FOLD verdict.
- **Phase 3 — bridge/FFI family folded**: `cgo_bridge` / `ffi_bridge` / `napi_bridge` / `wasm_bridge` / `native_bridge` / `bridge_invokes` → `calls + meta["bridge_kind"]`; `wasm_load → imports`. New `BRIDGE_KINDS` constant documents the closed enumeration.
- **Phase 3 — dst-kind leakage subset folded**: `imports_module` / `imports_component → imports`; `model_reference` / `type_ref` / `query_references → references`; `renders_component → references + meta["construct"]="jsx"` (the only fold carrying a `meta` payload, since the JSX render shape isn't fully captured by `dst.kind` alone).
- **Phase 2 — consumer migration**: new `IMPORT_EDGE_TYPES` predicate replaces the hardcoded `{"imports", "imports_module"}` sets at `ranking.py` and `slice.py` call sites. Adding the missing `imports_component` entry closes the silent bug from ADR-0023 §1 case 1, where Vue/Svelte/Astro/React component imports were miscategorized as generic edges.
- **Drift detector generalised to multi-field, scope broadened**: the AST walker lives in `hypergumbo_core.axis_drift` (parameterised on `name_filter` and `registry_names`). Default search scope expands from `packages/` only to `packages/`, `scripts/`, `.agent/`. Surfaced and fixed one phantom-value bug in `bakeoff-deep::_FFI_EDGE_TYPES` (`jni_bridge` / `pyffi_bridge` — neither was ever emitted).
- **Pre-commit drift gate flipped to `--strict`**: now that the protocol-call producer migration cleared the last load-bearing endpoint_shape consumer reference, `.githooks/pre-commit` runs `check-edge-type-drift --strict` by default. Future endpoint_shape regressions fail at commit time.
- **Registry-completeness sweep — 25 new edge_type entries** (`SCHEMA_VERSION` 0.3.0 → 0.3.1): triaged 30 emitted-but-unregistered values into 5 control-flow noise (kept unregistered), 7 missing relationship canonicals (`inherits`, `decorated_by`, `includes`, `defines_target`, `data_flows_to`, `module_exports`, `overrides`), and 18 endpoint_shape candidates with plausible fold targets seeded for future per-pattern audits.
- **Schema-enum completeness for already-emitted edge types** (`SCHEMA_VERSION` 0.2.4 → 0.2.5): added 13 values that analyzers were emitting but the schema enum was missing — `imports_module`, `wraps`, `module_attr_ref`, the bridge family, `bridge_invokes`, `ipc_calls`, `ipc_event`, `implements_rpc`, `di_resolves`.
- **ADR-0023 status: Draft → Accepted (§6 migration in progress)**: typing principle in force. ADR text now cites landed Phase 1 commits, cross-references ADR-0024, replaces "Open questions (Draft)" with per-question resolutions, and reframes the Property test section as three complementary defenses (static / runtime / cadence-hook).
- **ADR-0023 §6 plan corrected to many-small-ships shape**: the protocol-call family is named as the next Phase 3 subset; "endpoint_shape section becomes empty" replaced with "first 4b ships the cohort that bakeoff-validates; subsequent micro-4b's ship the remaining subsets one family at a time."

#### Audit-findings reclassification

- **ADR-0025 / ADR-0026 reclassified as `docs/audits/0001-dispatch-publish-family.md` / `0002-ipc-family.md`**: both were per-value verdict tables under existing law, not architecture decisions. Permanent redirect stubs preserve URL-level discoverability. ADR-0024 gains §"Family-audit verdict methodology" (lifts the CANONICAL/FOLD/DEPRECATE-NO-FOLD trichotomy) and §"Fold-residue discipline" (three rules for promoting recurring meta keys to dedicated fields). Status: Draft → Accepted.
- **`docs/adr/README.md` — bucket rubric for ADRs vs audit-findings vs surveys**: positive/expansive (defines valid shapes), not restrictive. Bucket boundary: "decision present?" `docs/surveys/` is forward-declared. AGENTS.md "Required Checks" gains a one-paragraph essentialization pointing at the rubric.

#### Concept-audit follow-ups

- **Three IR field docstrings clarified** per the 2026-04-30 Adjacent Concept Sweep: `Symbol.origin` (analyzer-id-vs-synthesis-mechanism axis mix), `DataModelKind` (three-axis confusion), `UsageContext.kind` (syntactic-construct-vs-semantic-role axis mix). Each surfaces the conflated axes and the per-field re-evaluation triggers. No behavior change.
- **Audit playbook anti-pattern generalised**: `Running the audit while you're mid-feature` becomes `Coupling the audit to a deadline`, with mid-feature, mid-release-prep, and mid-incident named as instances of the same shape.
- **`agentic-session-retrospective.md` — corrected the trackerize-precision worked example**: the prior "selector precision miss" finding conflated trigger-condition with applicability. New "common misreading 4" uses trackerize as the cautionary example; the "Selection precision" bullet now measures body applicability rather than trigger-word matching. Retroactively withdraws prior retrospective findings on the same false premise.

#### Governance & playbooks

- **AGENTS.md streamlined**: No Weasel Words section's two BANNED hedge bullets merged into one; CI Interaction Policy compressed (the auto-pr exit-code recovery table moved to `ci-debug-protocol.md` where it lands on lookup); Bakeoff Validation Discipline split by cadence into `bakeoff-validation-tagging-discipline.md` (per-PR) and `process-validation-queue-with-bakeoffs-and-uat.md` (per-session, ~15+-item drain). Transcript-injection registry updated.
- **Cruft audit + cruft-audit-playbook introduced**: applied current findings (TRACKER_SYNC_PENDING workaround removed; stale `now`/session-anchored qualifiers trimmed across four playbooks) and codified the methodology — two-pass (syntactic grep + semantic read) mediated by an interactive interview, with the cruft / trim / not-cruft / doc-consistency taxonomy.
- **Post-Phase-4b consumer-facing UX docs corrected**: `docs/concept-axes.md` preamble (the "producers no longer emit any of these values" statement was inverted after Phase 4b shipped); `docs/migrating-edge-types.md` "What's NOT migrated yet" restructured into three groups by next-ship — pending-classification (4), protocol-call family (3), long-tail sweep (22) — with the post-migration query shape spelled out.
...
- **Linker docstring/comment false positives**: 23 protocol/framework linkers ran their regex pattern detectors directly against raw file bytes, matching their own module docstrings that documented the very patterns they detect. New shared masker `linkers/_text_filters.mask_doc_regions` parses with tree-sitter and replaces comment ranges and Python positional docstrings with spaces (newlines preserved) before regex matching. Failure modes bias to fail-closed; `annotation_convention` is intentionally exempt because it scans `@hg:` directives inside comments.
- **Masker rule for module docstring after a leading comment**: the masker's "first named child" rule rejected docstrings preceded by a leading comment. Every file in the repo opens with `# SPDX-License-Identifier: ...`, so module docstrings were silently unmasked across virtually all hypergumbo source. On self-analysis, removed 45 false-positive nodes across nine kinds (`event_publisher` -15, `mq_subscriber` -6, `mq_publisher` -5, `event_subscriber` -4, `subprocess_call` -4, `db_query` -4, `http_client` -3, `websocket_endpoint` -2, `graphql_resolver` -2).
...
- **ADR-0023 ↔ canonical registry discrepancies** (`SCHEMA_VERSION` 0.2.5 → 0.2.6): `query_references` reclassified to `endpoint_shape` to match ADR-0023 §6; four edge types named in the deprecation list but absent from the registry — `imports_component`, `model_reference`, `type_ref`, `renders_component` — added with `endpoint_shape` axis and migration notes.
