...
- **Cross-package name collision false positives in JS/TS**: Direct function calls (e.g., `process()` after `import { process } from './module'`) now use import-path disambiguation via `_disambiguate_by_import` to resolve to the correct module when multiple files define the same function name. Previously, `global_symbols` used last-one-wins, so the call resolved to whichever file happened to be processed last by `rglob`, ignoring the import statement. This eliminates false cross-package edges like server-side `error()` being linked to a client-side `error()` function when an explicit import exists.
