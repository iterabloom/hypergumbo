...
- **Tauri IPC linker**: Creates `ipc_calls` edges between TypeScript/JavaScript `invoke('command_name')` calls and Rust `#[tauri::command]` functions. Handles single/double/backtick quotes, TypeScript generics (`invoke<T>('cmd')`), Tauri plugin pattern (`invoke('plugin:name|command')`), `rename_all="camelCase"`, and explicit `rename` overrides. Enables cross-language tracing for Tauri desktop apps (spacedrive, yaak, Cap, readest, etc.).
