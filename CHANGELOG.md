...
- **Solidity ABI bridge linker**: Creates `abi_call` edges between TypeScript/JavaScript contract calls and Solidity function definitions. Detects ethers.js method call patterns (`contract.transfer(to, amount)`) and viem config patterns (`readContract({ functionName: 'balanceOf' })`). Matches TS/JS call sites to Solidity function names and creates synthetic `abi_call` nodes at call sites. Confidence is 0.75 (name-based matching). Activates when both TS/JS and Solidity languages are present. Enables cross-language tracing for smart contract projects (Aragon OSx, OpenZeppelin, etc.).
- **TypeScript type reference edges**: Type alias bodies and interface method signatures now produce `type_ref` edges to user-defined types they reference. For example, `type UserList = User[]` creates an edge from `UserList` to `User`, and `interface UserService { getUser(): User }` creates an edge from `UserService` to `User`. Union/intersection types (`A & B | C`) create edges to all referenced types. Built-in types (string, number, boolean, etc.) are excluded. Enables refactoring blast radius analysis for TypeScript codebases.
- **Tauri IPC linker**: Creates `ipc_calls` edges between TypeScript/JavaScript `invoke('command_name')` calls and Rust `#[tauri::command]` functions. Handles single/double/backtick quotes, TypeScript generics (`invoke<T>('cmd')`), Tauri plugin pattern (`invoke('plugin:name|command')`), `rename_all="camelCase"`, and explicit `rename` overrides. Also matches tauri-specta generated binding patterns (`TAURI_INVOKE("cmd")`, `__TAURI_INVOKE__("cmd")`). Enables cross-language tracing for Tauri desktop apps (spacedrive, yaak, Cap, readest, etc.).
- **wasm_bindgen linker**: Creates `wasm_bridge` edges between JavaScript/TypeScript imports from wasm-pack `pkg/` directories (or `_bg.wasm` files) and Rust `#[wasm_bindgen]` exported functions. Handles `js_name` rename overrides, aliased imports, default+named imports, and type-only import filtering. Enables cross-language tracing for WebAssembly projects (automerge-wasm, AFFiNE/y-octo, loro, etc.).
- **Electron IPC pattern expansion**: IPC linker now detects `ipcRenderer.sendSync`, `ipcMain.handleOnce`, `webContents.send`/`event.sender.send` (main-to-renderer push), and `ipcRenderer.on`/`ipcRenderer.once` (renderer-side receive). Previously only `send`/`invoke` and `on`/`handle` were detected.
- **React component linker**: Creates `renders_component` edges from JSX element usage (`<Button />`) to component definitions. Detects PascalCase component symbols, handles dotted names (`<Form.Control />`), filters React built-ins (Fragment, Suspense, StrictMode), and prevents self-referential edges. Activates when the `react` framework is detected.
...
- **gRPC proto-to-Go implementation linkage**: Go methods on structs embedding `UnimplementedXxxServer` are now linked to their proto RPC route symbols via `implements_rpc` edges. Enables tracing from proto service definitions to their Go implementation methods (e.g., Argo-CD's 130 RPCs now connect to Go server code).
- **gRPC server-to-service bridge edges**: `dispatches_to` edges now connect `grpc_server`/`grpc_servicer` symbols (from Go `RegisterXxxServer` or Python `XxxServicer`) to their matching `grpc_service` symbols (from proto files). Previously these were disconnected graph components — `grpc_calls` terminated at dead-end server nodes. Now forward slices from gRPC client stubs can traverse through: stub → server → service → route → handler method.
- **dispatches_to confidence scaling by 1/sqrt(N)**: When an interface method has N overriding implementations, each `dispatches_to` edge now gets confidence `0.85/sqrt(N)` instead of flat 0.85. Interfaces with many implementors (e.g., 19 Notifier impls in alertmanager creating 325 edges) get proportionally lower confidence, reducing their dominance in ranking and reverse slices. Matches the precedent from symbol_resolution ambiguous lookup scaling.
...
- **Rust**: `implements` edges from `impl Trait for Struct`. Turbofish/fully-qualified generic call resolution (`PublicParams::<E1,E2>::setup()`). Generic trait method blocklist (33 names like `.into()`, `.clone()`) to prevent false-positive in-degree. `#[cfg(test)]` module inheritance for correct production slice exclusion. Unresolved trait impl edges: when a trait definition isn't in the analyzed files (e.g., tonic gRPC traits from generated .proto), a lower-confidence (0.70) implements edge is created to an unresolved target. Standard library traits (Clone, Debug, etc.) are excluded via blocklist. Error type exemption: Display, From, Error, and Default impls are allowed through the blocklist when the implementing type ends with "Error" or "Err", since these reveal error propagation architecture.
...
- **package.json `exports` field extraction**: Subpath exports (e.g., `"./sync": "./src/sync.js"`) now produce `export_entry` symbols and `defines_target` edges. Handles string form, object form with subpath keys, and conditional exports (`import`/`require`/`default` resolution). Enables the build-target linker to connect library entry points to their source modules.

### Fixed

- **ttrpc (containerd/ttrpc) server detection**: gRPC linker now detects `RegisterXxxService` patterns used by ttrpc, a lightweight gRPC alternative used by kata-containers, containerd, and other container runtimes. When a Go struct implements a ttrpc service interface (e.g., `AgentServiceService`), `implements_rpc` edges are created from the struct's methods to proto RPC route symbols. Supports both `XxxServiceService` and `XxxService` naming conventions.
- **Go gRPC struct matching with nested braces and package prefixes**: The gRPC linker's `implements_rpc` edge creation now uses brace-depth tracking instead of regex to find `UnimplementedXxxServer` embeddings in Go structs. Previously, structs containing fields with nested braces like `done chan struct{}` or `map[string]struct{ enabled bool }` caused the regex to stop early, missing the embedding. Also now supports package-prefixed embeddings (`pb.UnimplementedCacheServiceServer`). Fixes 0 `implements_rpc` edges in kata-containers (66 proto files, 56 register sites). Addresses WI-jamor.
- **Cross-language IPC/WASM slice traversal**: Tauri IPC and wasm_bindgen linkers now create synthetic Symbol nodes (`ipc_publisher`, `wasm_import`) for their edge sources. Previously, edges had synthetic source IDs with no corresponding Symbol, causing `node_by_id.get(edge.src)` to return `None` in the slicer's BFS — reverse slices from Rust handlers/exports couldn't traverse back through the IPC/WASM bridge to JS/TS callers. Fixes WI-zugog.
- **Tauri specta wrapper resolution**: Tauri IPC linker now resolves tauri-specta generated wrapper functions. When a TS file contains `export function takeScreenshot() { return TAURI_INVOKE("take_screenshot") }`, other files importing `{ takeScreenshot }` or `* as commands` from that wrapper file get `caller_invokes` edges linking them to the `ipc_publisher` synthetic node. Previously 38% of Tauri IPC commands (122/323 across 4 bakeoff repos) were unreachable because TS components called typed wrappers instead of `invoke()` directly. Fixes WI-bovon.
- **Electron contextBridge wrapper resolution**: IPC linker now resolves `contextBridge.exposeInMainWorld('api', { method: () => ipcRenderer.invoke('channel') })` preload bridge patterns. When renderer code calls `window.api.method()`, the linker creates `bridge_invokes` edges from the call site to the IPC send symbol in the preload file. Supports multiple namespaces, arrow functions and method shorthand, `invoke`/`send`/`sendSync` methods, and double/single-quoted strings. Enables end-to-end traceability from Electron renderer UI through the context bridge into main process handlers. Fixes WI-lagok.
- **Tauri specta object-method wrapper detection**: Tauri IPC linker now detects the newer tauri-specta pattern where commands are exported as methods on an object (`export const commands = { async method() { TAURI_INVOKE("cmd") } }`). Named imports of these objects (`import { commands } from './bindings'`) and namespace imports (`import * as commands`) both create `caller_invokes` edges. Previously only standalone function exports (`export function method()`) were detected, missing all wrappers in repos like Cap that use the object-method form.
- **Slicer pass-through synthetic node filtering**: `SliceQuery.pass_through_kinds` controls which node kinds are traversed during BFS but excluded from the output `node_ids`. Defaults to `{event_publisher, event_subscriber}`. These synthetic routing nodes represent IPC event channels and are useful for cross-boundary traversal but inflate slice output with nodes that don't correspond to real code. Edges touching filtered nodes are also excluded. Set `pass_through_kinds=frozenset()` to retain all nodes. Addresses WI-huriv.
...
- **Lit web component framework patterns**: Detects `@customElement('tag-name')` component registration, `@property()`/`@state()` reactive properties, `@query()`/`@queryAll()`/`@queryAsync()` DOM queries, `@eventOptions()` event configuration, and lifecycle hooks (`connectedCallback`, `disconnectedCallback`, `firstUpdated`, `updated`, `willUpdate`, `render`, `shouldUpdate`, `requestUpdate`, `performUpdate`). Also detects `LitElement` base class inheritance.
- **NestJS/TypeGraphQL resolver detection**: GraphQL resolver linker now detects `@Resolver(() => Type)` class decorators with `@Query()`, `@Mutation()`, `@Subscription()`, and `@ResolveField()` method decorators. Handles arrow function type args, string type args, explicit field name strings, and `@Resolver()` with no arg. Creates `resolver_implements` and `resolver_for_type` edges linking resolver methods to schema definitions.
- **NestJS `@Module` provider/controller registration**: DI linker now detects `@Module({providers: [...], controllers: [...]})` decorators in TypeScript files and creates `di_registers` edges from the module class to each registered provider/controller. Handles multiline arrays, skips object literals (already handled by `{ provide: X, useClass: Y }` pattern).
...
- **Constant propagation for dynamic route paths**: Python route detection (Django `path()`/`re_path()`/`url()` and Flask `add_url_rule()`/`add_api_route()`) now resolves string concatenation and module-level constant references. `path(BASE + "/users/", view)` where `BASE = "/api/v1"` resolves to `/api/v1/users/`. Handles same-file constants, imported constants (cross-file), and nested concatenation (`A + B + C`). Unresolvable expressions (function calls, f-strings, undefined variables) are gracefully skipped.
- **Middleware chain linker**: Creates `middleware_chain` edges between consecutive middleware symbols in the same file, ordered by source line. Enables forward/reverse slices to traverse the middleware execution pipeline. Works with all 58 framework patterns that tag symbols with `concept: middleware` (Flask, Django, FastAPI, Express, Koa, Go, Rails, Laravel, Phoenix, etc.). Complements JS/TS's inline middleware chain detection with a cross-framework linker-based approach.
- **`limits.truncated_files[]` wiring**: Files skipped due to `--max-file-bytes` are now recorded in `limits.truncated_files[]` in the behavior map output. Uses a global `on_file_skipped` callback in `discovery.py` set by `run_all_analyzers()`, so all analyzers using `find_files()` automatically report skipped files without any per-analyzer changes.
- **Go 1.22+ stdlib `http.ServeMux` route detection**: Combined method-path patterns like `mux.Handle("POST /v1/data/{path...}", handler)` are now detected. The HTTP method is extracted and stored separately in `http_method` metadata instead of defaulting to `"ANY"`. Fixes OPA's missing API routes in bakeoff assessment.
- **Ansible YAML edge resolution**: Include/import edges now resolve to proper file-level node IDs. Each processed Ansible file gets a file-level node, and edge destinations are matched by basename or role name lookup (`name=rolename` → `roles/<name>/tasks/main.yml`). Unresolvable paths (Jinja2 templates, missing files) get 0.50 confidence instead of being dangling references. Reduces orphan rate from 70%+ to near-zero for Ansible repos.
...
- **React SPA bootstrap entrypoint detection**: `createRoot()`, `ReactDOM.render()`, `hydrateRoot()`, and `createBrowserRouter()` calls at module level now produce `SPA_BOOTSTRAP` entrypoints. Previously, react.yaml patterns used decorator matching, but these are call expressions not decorators. Fixed by emitting UsageContext records from the JS/TS analyzer (kind="call", position="caller") and matching them with usage-based YAML patterns. The module symbol receives the `app_bootstrap` concept, enabling proper entrypoint detection for React apps (element-call, vscode-react-native, excalidraw, etc.).
- **Electron main process entrypoint detection**: `app.whenReady()` and `app.on('ready')` calls now produce `ELECTRON_MAIN` entrypoints via usage-based YAML patterns. Also detects the `package.json` `"main"` field as a `defines_target` edge (e.g., `"main": "./src/main.js"` → `defines_target` → `src/main.js`), enabling the build-target linker to connect Electron entry points to their module symbols. Fixes Signal-Desktop and session-desktop missing main process entrypoints.
- **Solid.js framework pattern detection**: New `solid.yaml` with 12 patterns covering reactive primitives (`createSignal`, `createEffect`, `createMemo`), async data fetching (`createResource`), state stores (`createStore`, `createMutable`), context (`createContext`, `useContext`), lifecycle hooks (`onMount`, `onCleanup`, `onError`), and app bootstrap (`render()`, `hydrate()`). Framework detection triggers on `solid-js` dependency in package.json.

### Fixed

- **SPA bootstrap framework disambiguation**: When both React and Solid.js patterns match the same bootstrap call (e.g., bare `createRoot()` is used by both frameworks), the label now correctly reflects the more specific framework. Solid, Svelte, and Vue take priority over React for ambiguous function names because React's patterns (`createRoot`, `hydrateRoot`) collide with SolidJS's identically-named but semantically different APIs. Cap repo's SolidJS bootstrap was incorrectly labeled "React app bootstrap"; now correctly labeled "Solid app bootstrap".
- **Direct symbol ID resolution in route handler linker**: Routes with `handler_ref` as a full symbol ID (e.g., `python:/path:line:Class:class` from Flask-RESTful's `api.add_resource()`) now resolve directly by ID instead of failing the Express name-based lookup. Fixes 0% route link rate for Flask-RESTful projects.
- **JSX Route component linking in route handler linker**: `<Route element={<Users />} />` JSX patterns now link to component symbols with `class` or `module_file` kind, not just `function`/`method`. Also tries common React naming suffixes (Component, View, Page, Screen, Container) when exact name match fails. DEEP bakeoff: 0 route-to-handler edges across all 48 repos due to JSX components being classes/modules.
...
- **Assertion/panic/exit and UI primitive dampening**: `assert`/`assertEqual`/`assert_eq`, `panic`/`abort`/`exit`/`die`/`unreachable`, and camelCase `log` compounds (`logPanicAndDie`, `logError`) are now utility-dampened. Leaf UI components (`Button`, `Icon`, `Input`, `Modal`, `Select`, `Tooltip`, `Spinner`, etc.) are dampened as design-system plumbing. DEEP bakeoff: `assert()` ranked 6th in automerge, `Button` ranked above `JobQueue.add` in AFFiNE.
...
- **Go local variable false positive prevention**: Function reference detection (`_extract_function_reference_edges`) now checks `scoped_var_types` to skip identifiers that are local variables in the current function scope. Previously, `time.Since(start)` would create a false `function_reference_arg` edge from `doWork` to `start()` when a function named `start` existed. All short-var-declaration variables are now tracked (even when type is unknown) so they're excluded from function reference matching.
- **Test-path preference in symbol resolution**: `NameResolver.lookup()` accepts optional `caller_path`. When the caller is in a non-test file and suffix matching produces both test-file and production-file candidates, production candidates are preferred. Eliminates false positives like `server.startup()` resolving to `LogCleanerTest.startup()` instead of `Server.startup()` in kafka. Wired into Java analyzer for all method and class resolution calls.
- **C++ STL method guard**: Member calls like `v.clear()`, `v.push_back()`, `v.resize()` no longer resolve to project-level functions with the same name when field-chain resolution fails. A blocklist of 35 common STL container/iterator/stream method names suppresses fallback to local/global symbol resolution for member calls (`field_expression` present).
- **Binary `.ts` file skip**: The JS/TS analyzer now skips `.ts` files containing null bytes in the first 8KB (binary MPEG Transport Stream test data). Fixes shaka-packager where 83 binary `.ts` audio/video files were misdetected as TypeScript, producing bogus nodes.
- **Ambiguous resolution confidence scaling**: `NameResolver` and `SymbolResolver` ambiguous confidence now scales as `0.70/sqrt(N)` where N is the candidate count. Common method names like `Initialize()` defined in 15+ classes get proportionally lower confidence (0.18) vs two-way ambiguity (0.49), naturally filtering false-positive edges in ranking and display.
- **C++ inline method class qualification**: Methods defined inside class bodies now get qualified names (e.g., `Parser::Initialize` instead of just `Initialize`). Previously, inline methods from different classes with the same name would collide in the symbol registry. Same-class preference uses the enclosing class as a path_hint for bare method calls.
- **NameResolver key-based path_hint matching**: Path hints now also match against registry keys (qualified symbol names), not just file paths. Enables same-file disambiguation where `path_hint="Parser"` matches key `"Parser::Initialize"` but not `"Packager::Initialize"` even when both are in the same file.
...
- **Java inherited field method dispatch**: `field.method()` calls where the field is declared in a parent class now resolve through the extends chain. Builds a global class-fields map, then when `receiver_name` isn't in the current file's type registry, walks parent classes to find the field type. killbill's `accountUserApi.createAccount()` (field declared in `JaxRsResourceBase`) is now captured (evidence: `ast_call_inherited_field`, confidence 0.80).
