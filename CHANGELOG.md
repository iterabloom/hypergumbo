...
This cycle is dominated by one theme: **making `verify-claims` say only what it actually checked.** A long series of channels through which the tool could answer `confirmed` without having looked are closed — a language with no I/O catalogue, a module the catalogue cannot adjudicate, a call site never classified, a method-shaped catalogue against an analyzer that emits no method calls, a taint claim whose own vocabulary does not resolve, and a repo-supplied catalogue row that deletes the very sink that would have caught the claim. The verdict vocabulary gains the fourth value ADR-0016 §4 specified and never implemented — `confirmed_with_caveats` at **exit code 3** — so a clean-but-qualified answer (a named opaque launch, a repo-supplied sanitizer) is no longer reported as either a clean pass or a blind disclaimer. `VERIFY_CLAIMS_SCHEMA_VERSION` advances 1.1 → **2.0**, the one non-additive bump: a consumer testing `verdict == "confirmed"` stops seeing claims it used to see, and a gate written `verify-claims … || exit 1` now fails where it used to pass.

Alongside, the substrate those verdicts rest on is widened and corrected. **Receiver typing** reaches Python (constructors, annotated parameters and assignments, type-preserving derivations, each binding-checked), Go (composite literals, chained returns, `/vN` import paths), Java and C++, turning previously-invisible third-party and stdlib I/O into matched boundaries. The **I/O primitive catalogue** gains project-local overlays for third-party dependencies, mode discrimination so a write stops classifying as a read, simultaneous multi-boundary primitives, descriptor-level stdlib rows, and dated per-module completeness audits. **Data-flow analysis** wires Go, JavaScript, Rust and TypeScript def/use extractors, adds ADR-0017 §4 function summaries and a CFG coverage gate that forfeits refutation over calls the walk could not see, and taint finally traverses framework dispatch edges. Two **runtime-safety** holes close: analysing a repository executed code from it, and `cache-clear` could delete arbitrary paths. On the engineering side the mypy strict ratchet becomes genuinely blocking and its surface drains 1,080 → 672, the self-hosted-Forgejo CI failover is retired, `smart-test` gains a coverage-directed selection shadow phase, and a quadratic in the fingerprint post-pass — 62.6% of a cold run's wall clock, and invisible in `analysis_runs[]` because a post-pass gets no provenance card — is removed, taking a cold `survey` of this monorepo from 517.7s to 220.0s with one of 42,543 fingerprints changed. `SCHEMA_VERSION` advances 0.19.0 → 0.20.1.

### Added

#### verify-claims — verdict vocabulary, scope and disclosure

- **`confirmed_with_caveats` (exit code 3), for two caveat kinds (ADR-0016 §4).** A verdict held up by the analysed repository's own sanitizer, or clean everywhere except at named opaque launch sites, now carries a structured `caveats` list (`kind` / `entries` / `detail`) instead of an rc-0 pass indistinguishable from an earned one. Both kinds route through one caveat builder with a parity test asserting the boundary and taint paths render identical disclosures, and a verdict may carry both at once. The qualification is raised **only where it discriminates**: a shipped-catalogue sanitizer still earns plain `confirmed`, and opacity qualifies only when launches are the *sole* remaining blocker — beside an uncatalogued module the verdict stays `inconclusive`, because the reader cannot tell which gap produced the silence. `VERIFY_CLAIMS_SCHEMA_VERSION` 1.9 → **2.0**.
- **A claims file declares its own denominator — `analysis_scope: shipped_artifact`.** The coverage gate demanded an opinion on every external call *in the repo*, so repo tooling blocked claims it cannot participate in and any new import anywhere re-broke the proof. Scope is now derived from packaging metadata — each `pyproject.toml` with a `[project]` table, with ancestor workspace wrappers dropped — which is checkable independently of the analysis under question, unlike reachability scoping. Opt-in, disclosed on stderr, filtering nodes and edges as one population. On hypergumbo's own tree: 154,505 → 36,230 edges and **81 opaque launch sites → 2**.
- **A verdict records which catalogues it trusted.** A `confirmed` reached against the shipped catalogue and one reached against a repo-supplied overlay were byte-identical in both text and JSON; the envelope gains `catalog_provenance` (always present, so `user_supplied: false` is assertable) and the text renderer gains the same disclosure, reporting the CLI and claims-file layers separately — a catalogue passed on the command line comes from whoever ran the tool, while one reached through `extra_catalogs:` travels with the repository. `VERIFY_CLAIMS_SCHEMA_VERSION` 1.8 → 1.9.
- **Coverage and sanitizer scope are published, not implied.** New `dataflow_coverage` and `sanitizer_scope` blocks state which languages have data-flow machinery wired, what the sanitizer catalogue can express, and which sanitizer shapes each analysis method honours — so a zero means "not expressible" rather than "not protected". The adjudication label no longer depends on which other languages the repo happens to contain. Envelope 1.6 → 1.7 → 1.8.
- **Findings say how they were adjudicated, where they entered, and what was set aside.** Verdicts report per-flow `analysis_method` instead of a hardcoded `approximate`, name the `source_boundary` a flow entered through without relabelling its taint label, report sanitized flows instead of pruning them into silence, and disclose excluded flows with the specific test-file rule that fired. Taint flows scope to production sources by default (`--include-non-production-sources` widens both the flow filter and the language census). Envelope 1.1 → 1.6.

#### I/O boundary catalogue

- **Project-local overlays let a repository declare its own dependencies' I/O.** `--io-primitives` and a claims-file `extra_catalogs.io_primitives` key merge user rows into the shipped catalogue (`status: overlay`), so third-party libraries like `requests` become boundaries — Python taint sinks 113 → 172 on the reproducer. An overlay may also vouch that a module's surface is **enumerated**: the grant is renamed `module_completeness` (from `stdlib_module_completeness`, which held two populations under a name describing one), keeps `retrieved:` mandatory so an entry is a dated audit record rather than a switch, and never promotes the module into `stdlib_modules`. Overlays for languages the repo does not contain are skipped rather than fatal, while a language no catalogue knows still raises.
- **The stdlib climb: descriptor-level Python I/O is rowed and 47 modules carry dated audits.** `os.open` / `os.write` and the fd read/write families, `pathlib.Path`'s open/resolve/stat surface, the archive openers (`gzip` / `tarfile` / `zipfile`), `asyncio`'s subprocess pair, `fcntl`, the import machinery and module-level `logging` emitters now classify, and the catalogue gains its first `env_write` section. Two refusals are deliberate: inert process-state reads (`getpid`, `cpu_count`) are not rowed because `env_read` rows auto-derive taint sources and a pid is not a secret, and `os.urandom` is not rowed because an auto-derived twin displaces the hand-written source. Published as `docs/surveys/python-stdlib-module-io-enumeration.md`, with hypergumbo's own third-party overlay doubling as the worked user example.
- **`ujson` catalogued as a `json` drop-in**, seven Python primitives whose short names collide with builtins added to `ambiguous_names`, and the taint catalogue now inherits parent `io_primitives` the way io-boundaries already did (1,588 → 2,119 sources+sinks).

#### Data-flow analysis (DDG)

- **Go, JavaScript, Rust and TypeScript reach the data-flow machinery.** A Go def/use extractor plus a statement-level Go CFG (caddy builds 28,716 DDG edges over 1,635 symbols), JavaScript wired via alias keys onto the shared TypeScript grammar mapping, and Rust/TypeScript extractors behind a four-property per-language gate. The repo-level DDG builder moves out of `cli.py`.
- **ADR-0017 §4 function summaries are wired into the walk**, with Python and Go stdlib catalogues, closing 25 of 264 escape sites with zero flows changed.
- **The §3a walk names its own escape sites and can forfeit refutation.** Escape sites carry a closed-vocabulary `reason` identifying which of the walk's four branches lost the value, and `cfg.uncovered_call_lines` computes byte-extent call coverage per function so a walk over a partially-recorded definition returns unknown rather than a removal-licensing `False` (forfeit share 12.8%–35.5% across four repos). The gate is wired to the barrier arm only — §3a collapses `False` into `None` at the call site, so wiring it there is observably nothing — and a structural guard now fails the build if either arm starts consuming the walk's `False` without it, so the gate cannot be left behind by the change that makes it load-bearing. Two escape-shape measurement instruments land in-repo rather than in session scratchpads.
- **Edge deduplication retains the call sites it discards** in a new capped `Edge.meta.call_lines` key (13,121 of 141,312 edges record more than one site), so the walk can consider every call site of a collapsed edge rather than an arbitrary one.

#### Analyzers, linkers and views

- **Framework dispatch is visible to taint, and Python argparse subcommands dispatch.** A new `argparse_dispatch` Framework linker emits `dispatches_to` from `parser.set_defaults(func=handler)` registration sites to the resolved handler (recovering all 26 `cmd_*` registrations on hypergumbo's own CLI with zero false rows), and `dispatches_to` joins `TAINT_CALL_EDGE_TYPES` — the whole dispatch-linker family (cobra, argparse, decorator, django, jackson, kafka, caddy, airflow, rust-trait) had been taint-invisible, so sources behind a dispatch boundary never minted and BFS never crossed one.
- **The shell's own writes and reads are visible.** The bash analyzer dispatched only on function/declaration/command nodes, so redirection produced **zero edges**; `>` and `>>` now emit one primitive at two modes (truncate/append on `io_mode`), a variable target emits as unresolved rather than being silently skipped, and an environment read — a name expanded but never assigned in the file — emits an edge so a shell script can *start* a flow. ADR-0016's prohibition covers a *launched program's* I/O, not the shell's own.
- **Receiver typing across four languages.** Python types receivers from constructors (`p = Path(raw)`, inline `Path(raw).write_text(x)`, dotted `http.client.HTTPConnection`), annotated parameters and assignments, and allowlisted type-preserving derivations — each gated on a positive import binding, since trusting bare constructor names would have destroyed 61.5% of printed boundaries. Go types composite-literal receivers (`&http.Client{}`), unwraps `expression_list` so chained returns resolve, and derives the real package identifier from `/vN` import paths (1,293 spurious external module slots removed across 377,503 call edges). Java routes the inferred receiver type into the taint module slot, and the Python external-constructor table is derived from the I/O catalogue instead of a hand-curated copy.
- **Container-member emission parity across the fleet.** Enum members, variants, cases and constants now emit for Rust, TypeScript, Swift, Java, C#, Scala, C++, Kotlin and D, with `protocol` added as a container kind and enum/given owner symbols emitted so members root; a parity matrix locks the coverage. Interface-dispatch gains a parity column across 16 languages, a type-family taxonomy and predicate land, and Python records framework-object bindings as `meta.constructed_from` (swept to six languages with its own parity column).
- **`dead-code-maybe` candidates carry a per-item `reachability` field** (`test_only` / `unreachable` / null) with both denominators published — 1,747 of 2,107 candidates are test-only. `DEAD_CODE_MAYBE_SCHEMA_VERSION` 0.1.0 → 0.2.0.

#### Developer tooling and measurement

- **`--minimal` lets the ten commands that auto-run an analysis decline the side outputs they never read.** `slice`, `search`, `routes`, `explain`, `symbols`, `io-boundaries`, `verify-claims`, `test-coverage`, `repeat-finder` and `dead-code-maybe` all fall back to a full analysis when no cached survey exists, via one shared chokepoint — which passed no side-emission flags at all, so a caller who typed `slice --files` also paid for three budget-tier previews, up to 25 per-route handler slices, and the sketch pre-computation block. `slice --files` reads `nodes` and `edges` and nothing else. The flag is **opt-in and the defaults are untouched**, because the standing steer on the adjacent surface is that those artifacts earn their place — the change is that a command which never asked for a survey can now say so. Not offered on `survey` itself, which already has `--budgets` / `--no-sketch-fan-out` / `--no-handler-slices`; that boundary is pinned by a test rather than left to convention. Two limits are stated in the help text: on a warm tree the flag changes nothing (there is no analysis to shape), and a survey cached from a `--minimal` run carries no `sketch_precomputed`, so a later `sketch` recomputes it live — verified by running `sketch` against such a cache and getting a complete sketch at rc 0, since all four consumer sites guard on the block's presence. The parity test **derives** which commands must offer the flag by following each subparser's `func` default to its handler and asking whether that handler calls the auto-analysis chokepoint, so a future consumer cannot silently miss it; a companion control asserts the derivation selects a non-empty population, since a broken derivation would make the parity assertion vacuous.
- **`smart-test` passes `--minimal` to its reverse slice, but only after probing for it.** The slice runs through the pipx-installed `$STABLE_HG`, which lags the working tree by design, and an unrecognized argument exits non-zero — which smart-test's slice-failure branch routes to `run_full_suite`. Adopting the flag blindly would therefore have converted every local run into a ~90-minute full suite while still reporting success, a silent degradation with nothing pointing at the cause (the stable on the development machine did not know the flag, so this was live rather than hypothetical). A `--help` probe costing ~0.2s gates it. The shipped block is **executed** against advertising and non-advertising stubs rather than pattern-matched, with a positive control asserting the two answers differ — a probe hardcoded to one reply would otherwise satisfy half the suite and read as an unrelated bug — plus a static guard that the probe's answer actually reaches the slice invocation and not merely the diagnostic string.
- **An analyzer-scaling prober separates the phase's fixed cost from its per-file cost**, which is the number that decides whether incremental re-analysis is worth building. It calls the analyzer dispatcher directly rather than paying ~93s of linkers and graph work per data point, and sweeps `max_files` to vary per-file work while holding the language mix fixed. Result on this monorepo: the phase is **65.7s uncapped but 2.6s at `max_files=1`**, so fixed cost (grammar loading, registry construction, dispatch) is **under 4%** and the phase is ~96% per-file work — a per-file cache could win back essentially all of it, landing a cold `--minimal` analysis near 97s instead of 160.9s. Two hazards the prober surfaced and now documents: `max_files` is described as a per-language-analyzer cap but only python and javascript honor it (252 of 1,385 files were still analyzed at a cap of 1, which put a floor under every sample and made a linear fit return an impossible **negative** intercept); and per-pass `duration_ms` is **wall time under a `ThreadPoolExecutor`**, so it inflates with contention rather than measuring cost — the same 43 bash files billed 2.3s or 49.9s depending only on whether the python analyzer was running alongside, which reads as a pathologically slow analyzer and is not one. Card sums were already known not to sum to wall clock; this is the sharper reason they cannot be used at all.
- **Coverage-directed selection can also NARROW the local run, opt-in via `pytest --narrow` (Phase 3).** `coverage-select narrow` removes from the run set only what the index can *prove* irrelevant, under three rules that each exist because of a distinct way of being wrong. **Drop only with positive evidence:** a file qualifies only if it has coverage rows and none of its tests are `unmeasured`, so the 3,605 node ids that produce no rows at all are kept unconditionally. **Keep the whole file-touch set, not the changed blocks:** the local gate is `coverage report --include=<changed files> --fail-under=100`, which is whole-*file*, and narrowing to the changed blocks' tests fails it — measured at one base as a 3-file block selection against a 13-file touch set, so the naive version would have dropped 10 files that were carrying coverage. **Refuse outright** on an unknown *coverable source* file or a changed file that no longer exists; new blocks and unknown *test* files are reported and not refused, because the test covering a developer's new code is a changed test file and enters through a selector narrowing never touches. Off by default and unconditionally off in CI (not even `HG_SELECT_SHADOW` re-enables it), because between full runs the index can be short a test that covers part of a changed file, and the gate would report that as a coverage regression. Measured A/B on one tree: 91 files / 1,680 tests / 19.3s becomes 68 files / 1,085 tests / 14.0s with the changed-file gate still at 100%, and the committed manifest still publishing all 91. `scripts/measure-narrowing-headroom.py` prices the ceiling honestly — only **55.3% of the suite's wall-clock is index-speakable at all**, so on a small change the saving is seconds and the win is on large selections.
- **Coverage-directed test selection now acts: the local run set is widened by what coverage saw (Phase 2).** `scripts/coverage-select select` answers the same question `shadow` did, but before pytest instead of after, and smart-test unions its answer into the local run set **below the manifest seam** — so the merge-gating CI keeps today's static selection, verified byte-identical against the pre-phase script over one tree. It is additive and never subtractive, which is the whole reason it precedes any narrowing: the worst case is wasted time. The justification is not speed — Phase 2 makes local runs *slower* — but that coverage sees runtime dependencies the import graph cannot, e.g. selecting `test_cli_explain.py` for a change to `edge_types.find_edge_type` because `explain` looks edge types up at runtime and imports nothing. Selections are filtered against the filesystem, because the index is persistent and remembers tests that were later renamed away, and pytest treats a missing path as a collection error rather than a skip. **The shadow now grades coverage against the pre-union set**: comparing it against the run set the union has already widened makes `EXTRA_FROM_COVERAGE` zero by construction, and that is the metric the phase is justified by — a permanent structural zero would have read as "coverage stopped finding anything". Guards run the shipped shell block against a stub selector rather than pattern-matching it, since a union that quietly adds nothing passes every position check ever written.
- **Coverage-directed test selection, shadow phase.** `--cov-context=test` is always on locally, folding each run's per-test coverage into a persistent `block -> tests` index keyed on AST-block digests rather than line numbers, and `scripts/coverage-select shadow` reports what a coverage-driven selector *would* have chosen without acting on it — changing one function in `edge_types.py` selects 12 tests across 2 files out of 22,801, including a runtime dependency no filename heuristic would find. It is a **fourth** selector, unioned and never subtracted, local-only (`$CI` disables it). Observations accrue to a durable out-of-repo JSONL — they are a forward-only time series, so every run without a log was a permanently lost data point — and three failure modes (cold index, all-new-code commits, collapsed join) are excluded from the evidence rather than counted as passes. **The committed manifest and the local run set are now separate variables**, because "local-only" was true of the script and not of its output: `.woodpecker/woodpecker.yml` runs whatever `.ci/affected-tests.txt` names, so a selector that unions tests in would have widened the merge gate and one that narrows would have narrowed it, silently and without touching CI config. The manifest publishes the static slice plus its declarative augmentations; anything a later phase adds or removes applies to this machine only. Byte-identical output today, verified by running both scripts against one tree, and pinned by a guard carrying its own positive controls.
- **A meta key declares how many writers may reach it, and the declaration is enforced.** Seven times in five months a `meta` slot several writers could reach held a single value and the last writer silently erased the earlier ones. `MetaKeySpec` gains `write_discipline` (`single_writer` / `merge_union` / `producer_primary` / `refines` / `unaudited`) with a mandatory note for every value that asserts something, written through one `write_meta_key()` chokepoint; `filter_meta_key()` adds the missing *remove* verb for curators that narrow a merged result. `unaudited` is the default on purpose — a wrong `single_writer` buys false assurance — so `check-meta-write-discipline` reports the count as visible debt (82 of 87 keys) and fails closed on any key a second writer can demonstrably reach.
- **Measurement instruments land in-repo.** Catalogue reach on Go, JavaScript and Java; taint-flow precision on real repositories for the first time (~41%); the DDG escape-closing ceiling (closing every §3a escape site changes zero findings); Go receiver-typing and Python factory-receiver gap sizing (both measured, both refuted as not worth building); and Go import-versioning and constructor-table A/B probes. Three more instruments cover the claims that most recently turned out to be false: an io-boundary construct counterfactual answering every lookup twice in one process (the tool that caught a shipped filter removing 49 true positives on whisper.cpp, because `call_construct == "method"` means "spelled as a selector" and Go spells `os.Open(p)` that way); a taint-walk arm census separating the §3a confirm-only arm from the WI-fasub barrier arm, which prints a caveat on a zero barrier count because that arm is unreachable by construction until a repo declares a sanitizer for the flow's label; and a per-language `call_construct` census that counts the *value* the F3 gate reads rather than field presence — 41.1% of cpp call edges carry the field and 1 of 34,983 carries the value. Three more attack the cold-run wall clock, which had only ever been apportioned by subtraction: a survey phase-split that times the pipeline's own phase functions with inclusive/exclusive accounting so the shares sum to the wall clock and the remainder is reported as unattributed rather than absorbed into a neighbour (it found 62.6% of a cold run in one un-carded post-pass, and refuted the side-output hypothesis at the same time, with a `--lean` counterfactual arm); a fingerprint cost-model prober that measures the locator against real files across the size range and then *predicts* the pass total from production's own symbols-per-file distribution, so a hot spot has to explain the number it is blamed for; and a survey-to-survey fingerprint differ, since a refactor promising identical output should be checked against every value it could have moved, not a sampled few.

### Changed

- **The mypy strict ratchet blocks merges, and its surface drains 1,080 → 672.** The ratchet was documented as blocking and was neither: the pipeline that actually gates merges ran `--mode=warning` behind `failure: ignore`, and nothing ran mypy locally. All three invocations are now strict, the pin is `mypy~=2.1.0` everywhere, and `smart-test` runs the ratchet *before* pytest and refuses to run tests over a breach. The surface itself is drained rather than baselined away — bare generics parameterised across compact/taint/sketch/io_boundary/profile (43 type-arg removals in the taint trio alone revealed 37 real hidden errors), grammar attributes converted to read-only properties (eliminating the mutable-override class, 105 → 0), stale `type: ignore` directives removed, and ~72 signatures annotated across `cli.py` and nine other files, which surfaced two real crash paths.
- **`Symbol.span` is `Optional[Span]`.** The non-optional declaration laundered every span-site verdict past the checker; a span-less symbol now serializes as a schema-legal null and deserializes to `None` rather than a fabricated `Span(0,0,0,0)`, and all 48 dereference sites are drained per-site. Relaxation only — no producer emits a span-less symbol today, so emitted artifacts are unchanged, but consumers that assumed presence should treat `span` as optional. **SCHEMA_VERSION 0.20.0 → 0.20.1.**
- **`compact` defaults to a connected core and its symbol budget is containment-monotone** (budget containment 55/120 → 120/120 pairs), and `sketch` and `compact` now rank the same symbol population from one shared filter module — top-10 agreement rises 31.2% → 55.0%.
- **Go's 20 web-framework I/O rows move from the shipped catalogue to a project-local overlay,** where they resolve against real import paths for the first time (0 → 6 framework chains on an idiomatic fixture); non-destruction was A/B'd on real Go repos, and the 33 remaining third-party Go rows are left because they are reachable.
- **`status: complete` on an I/O catalogue is renamed `status: provenance_declared`.** The validator accepted `complete` on four conditions — the literal string plus a citation URL — and counted no rows, so `python.yaml` claimed completeness while missing `os.open` / `os.write`. Coverage now has exactly one home (`module_completeness`); the old spelling is refused at load with a migration-pointing error.
- **`auto-pr` reads mergeability as a tri-state** instead of closing green PRs when the forge has not yet computed it, declares a closed vocabulary of 20 terminal states for its convergence invariant, and appends a per-invocation ledger line separate from the overwrite sentinel.

### Removed

- **`Edge.quality` is removed** after its one-version deprecation window (deprecated in 7.0.0), having carried zero independent signal — `quality.score` equalled rounded `confidence` on all 110,533 corpus edges. Read `confidence` + `confidence_source` + `is_resolved` + `rank_score` instead. **SCHEMA_VERSION 0.19.0 → 0.20.0 (breaking).**
- **The self-hosted-Forgejo CI failover layer is deleted** — 981 lines of `scripts/ci-failover` plus the git shim and override plumbing — with a recurrence gate replacing the manual sweep; an offline drill passed 10,331 tests.
- **Six documented ADR-0017 precision capabilities that do not run** are no longer documented as if they did.

### Fixed

#### Honest verdicts — closing the "confirmed without looking" channels

- **A claim may only be confirmed over calls that were actually examined.** Examination moves from module-level name recognition to the individual call site: recognizing a module as stdlib, or holding *some* rows for it, no longer vouches for a call the catalogue never classified (283 of 300 stdlib modules carried no row at all; `os` had 40 rows and none for `open` / `write` / `sendfile`). Five exfiltration fixtures — `telnetlib`, `ssl`, `ctypes.CDLL`, `os.sendfile`, `os.write` — went from `confirmed` rc 0 to `inconclusive` rc 2. Gate module matching is exact rather than prefix or substring, while the tagger stays permissive, and a parity test over the catalogue registry stops the gate running permissive on Python and strict everywhere else.
- **A language that emits call edges but ships no I/O catalogue cannot support a clean verdict,** and a taint claim needs **both** ends of a flow — a language counted as analysable with *either* sources or sinks, and one-sided coverage guarantees zero findings, which is what `confirmed` is made of. Measured on the shipped CLI, a script whose only statement writes a secret to a host path went `confirmed` rc 0 → `inconclusive` rc 2, with the stderr note and the `unsupported_taint_languages` field having gone silent as well.
- **A method-shaped catalogue with no method calls is inconclusive, not confirmed** — a method-starvation gate catches a catalogue asserting a call shape the analyzer never emits — and **a taint claim the tool cannot parse is not a claim that holds**: source labels and sink zones are validated against the resolved catalogue, so an unknown label errors at rc 2 instead of confirming at rc 0.
- **A launch is disclosed, not double-counted, and is not an examined negative.** An opaque launch put its own module into `unknown`, making the qualified verdict unsatisfiable whenever a launch existed (80 of 81 launch sites were simultaneously counted as uncatalogued modules); both channels now route through one predicate reading the producer's opacity stamp first. A launch also no longer counts as an examined negative for *other* boundaries, and it stays opaque even when a catalogue row classifies it.
- **A repo-supplied catalogue row can no longer delete a claim's only evidence silently.** User sources and sinks merge through a path where a matching `(module, name, kind)` **replaces** the shipped row rather than adding to it — so re-declaring `os.remove` into a `dev_zone` took a real `violated` rc 1 to `confirmed` rc 0 with empty caveats, by both the CLI flag and the in-tree route. The merge now returns its displacement set (the only moment that fact exists) and raises a caveat when a displaced shipped sink's zone equals the claim's prohibited zone. Overlays declaring a completeness entry are rejected loudly, and completeness entries no longer auto-promote a module into the stdlib set.
- **The census and the coverage check count one population.** The language census walked all nodes unscoped while the coverage check walked production-scoped edges, so test fixtures blocked every claim (blocking languages `bash, csharp, solidity` → `bash`) — both now share one classifier, and the widening flag widens both.

#### I/O boundary classification

- **A mode-decided primitive gets its mode from the analyzer.** `io_mode` was stamped at exactly one site in the tree, so C's `fopen(path, "w")` classified as a **read** — an examined negative for the boundary that was actually true, taking a write-claim from `confirmed` rc 0 to `violated` rc 1 once fixed. Mode discrimination is keyed on `(module, name, kind)` rather than the short name (Rust's `OpenOptions.open` had inherited a mode requirement from `File.open` and matched nothing in any repo), consults `simultaneous` for primitives true of two boundaries at once, and the producer is one shared tree-sitter helper reading a language-keyed table beside the catalogue. `builtins.open` is discriminated once so the boundary and taint arms stop classifying it in opposite directions.
- **A primitive catalogued under two boundaries now produces a chain for both.** A single-boundary lookup decided by YAML row order left `scala.sys.process.Process.apply` tagged `fs_write` with its process launch undetectable as a subprocess — losing the subprocess claim, the high-risk marking, the correct taint zone and the opacity gate at once. Measured across all fourteen catalogues: 23 multi-boundary primitives, 27 unreachable declarations.
- **A producer's opacity stamp is no longer erasable by a catalogue row.** `tag_io_boundaries` assigned `meta['io_boundary']` unconditionally, so a bash-analyzer `command_launch` stamp — the only opacity evidence those edges will ever carry — vanished the moment any row matched; the tagger now appends, keeping the producer stamp primary, with launch and catalogue chains accounted under their own rules.
- **A C/C++ module slot is a disjunction of includes, not one module name.** `cpp.py` sets an unresolved call's module slot to the comma-joined list of every `#include` in the file and its own comment said consumers may split it — none ever did, so a multi-include file matched nothing and a single-include file missed on the header suffix. On whisper.cpp, 6 of 35,059 cpp call edges matched; `net_send`, `net_recv` and `subprocess` were at zero. Now 65 match, and the residual 2 of 59 false positives are closed: a disjunctive slot is file context rather than receiver evidence, so the F3 construct rule applies to it exactly as it applies to no hint at all, and `cpp.py` stamps `call_construct` on member calls (it computed `is_member_call` already and discarded it — 1 of 34,983 cpp call edges carried the value before). The refusal is keyed on the slot's *arity*, not on the construct alone: `call_construct == "method"` does not mean "instance method" — Go spells `os.Open(p)` as a selector expression identical in shape to `f.Close()` — and a construct-only rule was measured on whisper.cpp removing 51 matches, the 2 targets and **49 true positives**. Gated on arity it removes 2 and 0, with 0 removed on alertmanager's 5,086 method-construct lookups.
- **Constructors are call sites, so the tagger can tag them** (`instantiates` joins the tagged edge types; 106 classified constructor calls were untagged across six repos), **Erlang's `os:cmd` is a shell-out rather than an environment read** (it was confirming "no subprocess" at rc 0), and **a repo's own function is not a stdlib I/O primitive**.
- **A directory is a virtualenv by content, not by name.** Excluding directories on bare names dropped 427 real source files across 39 repos; the check now conditions on `pyvenv.cfg` / `bin/activate`, recovering poetry's I/O chains 70 → 107.

#### Taint propagation and DDG precision

- **Attribute taint sources can start a flow at all.** All five tree-sitter analyzers passed the *file* pseudo-symbol to `emit_module_attribute_refs`, so an attribute source anchored to the file while its sink anchored to the function and the two could never share a caller — `process.env.API_KEY → fs.writeFileSync` was missed while `os.hostname() → fs.writeFileSync` was found in the same file. 21 of JavaScript's 50 derived sources are attribute-kind, including the whole `process.env` family. Resolved by line span rather than per-language ancestor walking, with a parity test enumerating every caller of the helper.
- **TypeScript and Groovy derive taint sinks at all** (TypeScript 0 → 83 sinks; Groovy 0/0 → 45 sources / 69 sinks). The derivation loop iterated catalogue *file stems* and bucketed under the catalogue's own language, so aliased languages with no stem of their own were never visited — TypeScript's six hand-written sources keyed under `typescript` while its sinks would have landed under `javascript`, so the two halves of one language disagreed about its name and a flow could start but never arrive.
- **The DDG index no longer launders taint between unrelated variables** (it keyed on `def_line`, now statement-def/use gated), **the walk asks about every call site of a deduplicated edge**, **a followed heir no longer suppresses a sibling escape check**, and **a definition the DDG never recorded is unknown rather than absent** — the last of which stops a partially-recorded definition licensing removal (700 of caddy's 6,596 `if` statements carry a call in an invisible initializer).
- **The sanitizer gate read the wrong slot.** `_register_sanitizer_callers` compared a sanitizer's qualified name against the dst's *name* slot while production puts the receiver type in the *module* slot, making its permit branch unreachable for every method-shaped sanitizer — the entire catalogue. A same-function sanitizer is now honoured on the DDG pass, and the scope of that clause is published rather than assumed.
- **Function summary lookups are provenance-gated and correctly keyed.** The §4 lookup key embedded the source file extension, so no first-party callee could match (the key changes for 5,981 of 6,685 distinct callees), and a first-party callee is now refused a stdlib summary lookup outright. The taint matcher also refuses a foreign-language callee before any catalogue lookup.
- **The DDG forward walk actually runs**, so the `precise` label is earned rather than asserted, and two propagator defects that silently lost real findings are fixed (flows 149 → 193, one verdict flipping `confirmed` → `violated`). `TaintFlowFinding.path` is reproducible across processes via a sorted BFS tie-break, and `to_dict` no longer drops three unrecoverable fields.

#### Security and runtime safety

- **Analysing a repository executed code from that repository.** `hypergumbo io-boundaries <hostile-repo>` ran an attacker-supplied program **6 times**, as the invoking user, at exit 0, silently — runtime subcommands shelled out to `git` with cwd inside the target, and three independent vectors were demonstrated on `git status` alone (`core.fsmonitor`; a bare `.git/hooks/post-index-change`, which fires with no config keys set; and a `filter.<driver>.clean` armed by an in-tree `.gitattributes`, which survived every hardening flag applied together). Since the attacker names the filter driver, no deny-list closes it. The fix is to not run `git status` — its result fed only a cache-key digest, and the fingerprint now hashes the working tree directly. Re-run against the fix: **6 executions → 0**. Separately, `cache-clear --repo` could delete arbitrary paths.
- **Runtime filesystem and subprocess use is declared through safety-zone wrappers.** A new `repo_inspection` zone declares the runtime's git, gitleaks and rust-analyzer probe use, and eight unwrapped filesystem call sites are routed through wrappers (a new `user_out_mkdir` among them), taking unsanitized `runtime-cli-no-host-fs` flows 370 → 0 and unsanitized subprocess flows 33 → 0.
- **SECURITY.md is generated from the tool's own verdicts rather than hand-edited,** and the drift gate it claimed to have actually exists — the gate had been running nowhere, since it lived only in the dormant GitHub Actions port while the live Woodpecker cron had dropped it at cutover. Both arms now exist in the pipelines that gate. The snapshot pins per claim the verdict **plus** the sorted caveat kinds **plus** whether sanitizer-credited flows are zero or nonzero, so the failure shape that motivated it — a verdict staying green while its credited evidence collapses — fails the gate instead of sliding through.
- **Rust repositories get an advisory about the rust-analyzer backend and what enabling it costs**, since indexing runs the analysed project's build scripts and proc macros as the invoking user.
- **`hypergumbo .` crashed with a HuggingFace connection error and wrote a zero-byte sketch whenever the embedding weights were absent** — on a tool whose claim is *local-first*, the documented quick-start was the one command that required the network. Reproduced with `HF_HUB_OFFLINE=1`, so no request was attempted: exit 1, 0 bytes, `internal error: OSError`, and the message told the reader it was a bug. `hypergumbo survey` on the same tree under the same environment exited 0 and wrote its full 204 MB map, so the failure was confined to the entry point while the deep pipeline was fine. Two causes, both structural: `_has_sentence_transformers()` catches only `ImportError`, so it answers "is the library importable" when the requirement is "are the weights on disk" — library present and weights absent passed the probe and skipped the four degradation branches that already existed; and `_load_st_model_offline_first` caught the *cached* load's failure only to fall back to a network-allowing constructor that was itself unwrapped, so the offline `OSError` propagated through six unguarded load sites (one of them marked `# pragma: no cover`, which is why no test saw it). The fallback now raises a typed `EmbeddingsUnavailable` carrying the original as `__cause__`, and each of the six consumers answers with the value its own unavailable-guard already returns — `None`, `set()`, a zero probe matrix, per-path `None`, and for config extraction the **heuristic extractor** rather than an empty list, since degrading to `[]` would have discarded metadata that pattern matching recovers with no model at all. Cached hits already computed in a batch are kept rather than discarded. One warning per process names the cause and the remedy. Verified against the filed repro at scale: **exit 1 / 0 bytes → exit 0 / 30,985 bytes**. The first-install download — the one sanctioned runtime fetch — is pinned by its own control, since a fix that made the probe refuse whenever weights were missing would have broken it silently.

#### Cache lifecycle

- **The results cache is bounded, and eviction is a soft delete rather than an `rm`.** Measured on the development machine: **5.3 GB in 27 entries for a single repo, all created the same day** — and 5.9 GB across 40 entries a few hours later. The state hash is whole-tree, so an actively-edited repo misses on nearly every run and mints another ~200 MB entry; nothing ever removed one, and the 1.0 GB notice threshold had been firing, unheeded, throughout. **This supersedes the INV-padum policy** ("no automatic eviction at any size; the user owns the prune decision", user-set 2026-05-17): a retention policy whose only enforcement is asking the user to run a command is not one. `HYPERGUMBO_CACHE_MAX_GB` (default **5.0 GiB**, `0` disables) now evicts least-recently-used whole entries after a run that wrote to the cache.
- **Evicted entries are zipped, not destroyed, because the cache lives under the user's `$HOME`.** An automatic irreversible delete there is a different kind of act from bounding a cache, and compression makes reversibility nearly free: measured on a real 210.9 MB entry, deflate level 6 produced **12.7 MB in 1.0s — 6% of the original, 16×**, inside a run that had already taken minutes. On the real cache, 987.9 MB of eviction costs **59 MB** of retained, recoverable archives.
- **Two archive folders, because the two artifact classes are nothing alike.** Across the six largest entries, `survey.json` is 1,259.9 MB of 1,264 MB (**99.6%**) while sketch markdown, tier previews and handler slices together are **0.4%**. `soft-deleted-surveys/` and `soft-deleted-sketches/` therefore carry independent caps (`HYPERGUMBO_SOFT_DELETE_SURVEYS_GB`, default 2.0 GiB; `HYPERGUMBO_SOFT_DELETE_SKETCHES_GB`, default 0.5 GiB), so cheap human-readable sketches can be retained far longer than the maps that are the actual disk problem. **The archives are bounded too** — soft delete buys a window in which an eviction can be undone, not immortality; an archive nobody removes is still growth, just 16× slower.
- **What it refuses to touch is the substance of the feature.** Only whole entries matching the `<repo>/results/<state_hash>/` layout the tool itself writes — anything else under the root is not ours to reclaim, and that invariant is what makes automatic reclamation admissible at all. Never a repo's newest entry, so no repo is left with nothing even when it alone exceeds the cap. Never an entry used within the last hour, since a concurrent run holds fresh mtimes and reclaiming underneath it would break *that* run, far from here. Never through a symlink leaving the cache zone. Ordering carries the crash-safety argument: the archive is renamed into place *before* the original is touched, and the original is renamed to a scratch name before removal, so no interruption can leave a half-deleted directory at its state-hash name serving a truncated survey as a cache hit. A new `cache_unlink` safety-zone wrapper keeps the archive pruning inside the `user_cache` zone rather than adding an unwrapped filesystem delete reachable from the runtime CLI.
- **The notice knob and the eviction knob stay independent.** `HYPERGUMBO_CACHE_HONK_GB` remains the warning threshold; silencing it does not disable eviction, and disabling eviction does not silence it. Folding them into one variable would let a cosmetic preference authorise deletion. `cache-status` only ever *previews* the eviction set — a report must not mutate what it reports on.

#### Language analyzers and identity

- **Python:** an attribute chain rooted at a local emits an external-module call edge instead of nothing (2,155 new call edges on kserve); read-write `@property` getters resolve; annotated `self` fields are captured in the `__init__` scan; statement-bound names shadow a colliding import alias; and `with` stops counting as a McCabe decision point.
- **Go:** four emit sites stopped stamping `is_resolved=True` on destinations whose kind slot reads `unresolved`, and three never-executing integration tests are un-skipped as strict xfail.
- **Route markers converge on one chokepoint.** Every producer mints through `make_route_symbol` behind a name-slot gate, names converge on `"{METHOD} {path}"`, gRPC and annotation markers route through the same path, and dropped `Route::match` routes are recovered with verb coverage centralized.
- **Symbol identity conforms to the ADR-0036 grammar.** Symbol-id parsing is anchored on the span token rather than slot count, so Rust's colon-bearing module paths resolve; four path-slot parsers collapse into one right-anchored helper; the id name-slot is sanitized at the minter for every producer; and doc/config-family and manifest-analyzer node ids conform.
- **Confidence and metrics:** edge confidence is re-derived from finalize's resolution verdict rather than the producer's advisory flag, `rank_score` is separated from `confidence`, a reserved-1.0 ceiling breach on build-file edges is closed, the two soft file-drop channels signal identically, and the three tool-version surfaces are reconciled.
- **Linkers:** the subprocess-to-CLI linker emitted zero joins on any monorepo — CLI-name detection now reads all manifests and partial-literal argv is retained (0 → 4 join edges on the self-tree); `pyffi` skips test files, ending phantom FFI edges minted from its own fixtures (all 10 were, at 4.6 s CPU per analysis); synthetic-node id name-slots across six linker families carry `Symbol.name` rather than a category constant; `type_hierarchy` splits `::` member names, giving Rust precise interface dispatch, with the separator vocabulary centralized; C# resolves overloaded-constructor instantiations by arity; and vendored library exports never seed a slice.
- **PHP's interface, trait and enum containers emit** with owner-qualified members (1 of 4 type containers → 4 of 4), **TypeScript records the `abstract` modifier**, and **C++ emits pure virtual methods**, marking a class that declares one as abstract.
- **34 duplicate node-text helpers collapse into one `None`-safe chokepoint** (`analyze.base.node_own_text`) — 14 copies were filed, 35 found.

#### Performance

- **The subprocess-CLI linker parsed every python file four times per survey; a cold analysis drops 162.4s → 139.1s.** `_scan_argparse_commands` and `_scan_fire_commands` each walked and `ast.parse`d the whole tree, and each is called twice — once by the linker, once by `_count_cli_command_symbols`, a requirement-check diagnostic that only wants a **count**. So half the work existed to produce a number. Both scans now share one walk, memoized on a key of path + size + mtime for every file, which is possible because the file-walking half is a pure function of what is on disk and only the *resolution* half consults the symbol table — the two callers hold different symbol lists. Measured with per-call-site attribution: the two sites went from 4,488 parses and 16.0s to 1,123 and 3.7s, python's whole-run redundancy factor fell **2.90× → 1.51×**, and re-parse time fell **20.7s → 4.7s**. The wall-clock saving (23.3s) exceeds the re-parse time because dropping three of four walks also removes their file reads and AST traversals. The load-bearing test is not the equivalence pair but `test_rescans_after_a_file_changes`: every other test in the file would pass against a cache that never invalidates, which is the stale-hit failure mode WI-madut names for its persistent cousin. One regression was caught while writing the merge, not after — the fire probe originally ran over *every* `ast.Call`, while the argparse probe runs over an `if/elif` in which a Call that is an Assign's value takes the first branch, so folding the fire probe into that `elif` silently stops matching `cli = fire.Fire(Service)`; no existing test used that shape, and one now does.
- **The python fingerprint locator was quadratic in file size; a cold `survey` on this monorepo drops 517.7s → 220.0s.** `stamp_symbol_fingerprints` caches one parsed tree per file, so each file is *parsed* once — but the per-symbol step located the smallest AST node covering a symbol's span by walking the whole module again, making the pass cost `symbols(file) × nodes(file)`. A large file is therefore expensive twice: once for having many AST nodes, and again for having many symbols that each rescan them. Measured at **324.3s of a 517.7s cold run — 62.6% of wall** — in a post-pass that carries no `AnalysisRun` card, which is why the time had only ever been reached by subtracting pass durations from the wall clock, an operation that localises nothing. The locator now bisects a per-tree index bucketed by effective start line and scans start lines descending, stopping once the best extent reachable at the current start line is worse than the incumbent (every covering node has `n_end >= end_line`, so a node starting at `n_start` cannot have extent below `end_line - n_start`, and that bound only grows as `n_start` shrinks): **324.3s → 22.8s, 14.2×**. Semantics are preserved exactly, which is the whole constraint here because a fingerprint is serialized output — the candidate set is unchanged and the winner is the lexicographic minimum of `(extent, walk_ordinal)`, reproducing the old strict `<` against breadth-first walk order, so on an equal-extent tie the *outer* node still wins. The tempting rewrite (descend to the deepest covering node) inverts exactly that tie and would have rehashed a different subtree, silently changing fingerprints; the tie is pinned by a test carrying a positive control that proves the two rules are distinguishable. Verified at repo scale against the pre-change survey: **1 of 42,543 fingerprints changed, and it is the file-level fingerprint of the edited file itself**, with every node-set delta confined to the files touched. Two displaced hypotheses are recorded because both were plausible and both were wrong: the 195 MB of side outputs that an auto-analysis emits uninvited (three tier previews, 16 handler slices, sketch pre-computation) account for 62.7s of the 517.7s, and serializing the 203 MB map is 3.0s.

#### Release gate

- **`release-check` could not report a dependency finding: it died at the assignment.** The script runs under `set -euo pipefail`, and `AUDIT_OUTPUT=$(pip-audit …)` followed by a bare `AUDIT_EXIT=$?` terminates the script *at the assignment* whenever the substitution exits non-zero — so the capture never ran, the `if`/`else` beneath it was unreachable, and the `echo "$AUDIT_OUTPUT"` that prints the findings could never execute. Every check after it (secrets scan, code quality, test suite, build verification, tag state) was skipped with it. The script's own comment described the opposite intent: *"anything else (network, parse, unsupported package) is a real failure we want to surface, not paper over."* Observed on 2026-08-17: a 31-line report ending mid-section at `  Running pip-audit...`, exit 1, no findings and no verdict — while pip-audit standalone reported six. An operator saw that same truncated output whether the finding was dev-box noise or a real shipped-dependency CVE. Now `… ) || AUDIT_EXIT=$?`, with `AUDIT_EXIT=0` initialized because the `||` branch does not run on a clean audit and `set -u` would otherwise fail *only* on the passing path. **A structural sweep found this to be the sole instance**: every other exit-code capture in `scripts/` and `.githooks/` already uses `cmd || var=$?`, or sits in an `if` condition where `set -e` does not apply.
- **The tests execute the shipped block rather than pattern-matching it,** since a guard that cannot be shown to fire is indistinguishable from one that matches nothing. The audit block is extracted from the real script, run under real `set -euo pipefail` against a stub `pip-audit` at exit 1, 2 and 127, and asserted to both print the findings and let a later check run. The load-bearing test is the **differential**: it reconstructs the pre-fix form and proves it behaves differently — without that, "the fixed block reports findings" would be satisfiable for reasons unrelated to the fix.
- **Two fixable CVE sets are pinned away rather than ignored, per the script's own policy.** `cryptography>=50.0.0` (PYSEC-2026-3552/3553/3554) and `nltk>=3.10.0` (PYSEC-2026-3582/3583/3584) join the local audit-env sync, together with `pyopenssl>=26.4.0`, which is required for cryptography 50 to resolve and is what CI already installs. Neither package is a declared dependency of any shipped hypergumbo package — `cryptography` arrives via Authlib/py-webauthn and `nltk` via nothing — so no released artifact was ever affected.
- **A dead `--ignore-vuln` entry is removed, and the surviving one is re-verified (WI-totak).** `PYSEC-2026-597` was carried as "nltk, no fix available"; that had stopped being true, and dropping the ignore leaves the audit clean for nltk. A dead ignore in a security gate is worse than the advisory it names — it is a false statement about what was checked, and it suppresses the next advisory for that package. The diskcache carve-out was tested the same way and **stays**: dropping it surfaces `PYSEC-2026-2447`, the same advisory under its PYSEC id, still with no fix version.

#### CI, tests and developer workflow

- **The framework-pattern cache is keyed on the resolved directory.** `load_framework_patterns` memoized on the framework id alone while the frameworks directory — half the function's input — is patchable, so a test that patched it cached `None` for the convention patterns and every symbol analyzed afterwards *in the same process* carried zero concepts, erasing concept-derived entrypoints. Under xdist this tracked worker composition, presenting as an interpreter, then a container, then an install-set problem across three CI runs.
- **The mypy ratchet failed open under `FORCE_COLOR`,** reporting the whole error surface as zero against a baseline of 672, and **`smart-test` preserves pytest's exit code** so collection errors are not green. The def/use extractor registrar reloads modules after a registry clear, fixing xdist-order-dependent data-flow capability, and the RCT-pinned tests declare their own `sys.path` instead of borrowing one from a sibling test's module-level insert.
- **`ci-debug` stops passing off another gate's green transcript as the one you asked for.** Asking for a gate that never ran on a commit fell through to `statuses[0]` and printed a different pipeline's log at rc 0 — a cron gate reporting "1556 passed" on a commit it never ran on. The substitution stays; the silence goes, warning on stderr only when neither a gate nor a step name matched. Woodpecker log fetching also resolves the step id before fetching, takes the host from `WOODPECKER_SERVER`, and diagnoses a 200-with-non-list body instead of crashing.
- **CI gates fail closed rather than silently skipping.** The pytest gate no longer skips on a failed `git diff` or a rebased-away base commit (a push with 9 changed files, 4 of them Python, ran zero tests and reported success), keys its skip on empty *selected tests* rather than on changed source files, and four merge gates route through one default-deny verdict — a hung-runner check written against a 300 s stale threshold on a ~360 s suite had merged a PR while it was still pending. The CI clone is pinned to `partial: false`: the plugin's tree-less promisor default forced per-commit tree refetches, costing **1,185,559 ms on first invocation against 19 ms on the second**. `auto-pr` refuses to amend a commit the branch does not own (one live impersonated commit), the top-level test map reaches 55 of 73 root tests rather than 27, and the GitHub test harness replaces `PATH` instead of prepending so un-faked tools are unresolvable. A pre-push DCO sign-off check lands as a governance-approved hook, and a release no longer fails on a bad SBOM.
- **`smart-test`'s per-PR selection stops being blind.** It always runs a test file you edited (root-level tests included, which previously reported "Selected tests: 0"), and four blind spots — hook directories, `.githooks/**`, `scripts/lib/**`, and untracked files and fixtures — are closed.
- **Agent transcript pipeline:** a secret scrubber lands with a retained-corpus backfill tool (584 occurrences of four live credentials across 35 retained files) and is wired into session-end rotation fail-safe with a stray sweep; transcript files are written owner-only, since a secret's copy should never be more readable than its origin (138 occurrences of two live credentials sat at mode 664); and the sync hook no longer spins a core — or goes deaf — when inotify watches are exhausted.
- **The validation ratchets are honest again.** The G1 validator ratchet had been red on dev and unreported per-PR (two `runtime_coherence` offenders against a shrink-only baseline of 1), the self-tree validation ratchet is drained at the label rather than the number by fixing confidence over-claims from three producers, the L3 producer-coherence gate now resolves dict-indirection and tuple-unpack emits (correcting the registry count 143 → 152 and catching an unregistered kind), and oversize-file drops set `partial_results_reason` the way parse-error drops already did.

### Fixed — developer experience

- **`auto-pr` said it cleaned up the merged branch and deleted nothing — a 100% failure, not a flake.** `cleanup_local` ran `git branch -d`, but `auto-pr` merges via **rebase** (`github-api.sh` tries `methods=(rebase merge)`, rebase first), so the PR's commits land on the base under new SHAs and the branch tip is never an ancestor of the updated base. That ancestry test is precisely what `-d` applies, so it refused every time with `error: the branch '<name>' is not fully merged`. The `git push --delete` beside it was dead code: `_github_delete_pr_head_branch` already removes the PR head branch via the API at merge time, so the push could only ever fail with `remote ref does not exist`. Both were written `2>/dev/null || true`, so the script printed `🧹 Cleaning up local branch...` and reported success while doing nothing. **Seven branches accumulated in one day; an earlier session found 97 local branches and 436 remote-tracking refs from the same cause.** The safety check is now `git cherry` — patch-ids rather than ancestry, so a rebased-and-merged commit registers as upstream — followed by `git branch -D`, and a branch carrying any commit not upstream is kept with a warning rather than force-deleted. The dead push is replaced by `git fetch --prune`, which is what the situation actually calls for: nothing pruned the stale remote-tracking refs (`git pull` does not, and `fetch.prune` is unset), so one was left behind per merge. Failures now print instead of vanishing. The script had the answer in its own source the whole time — the vPR-flush path uses `git branch -D`, and the manual-recovery hint it prints tells the user to run `git branch -D`; only the post-merge path used `-d`. Guard: `tests/test_autopr_branch_cleanup.py` drives the real `cleanup_local` against a real bare origin, and getting the harness honest took three corrections — `push -u` gave the branch an upstream (`-d` also accepts merged-into-upstream), the assertion compared a stale tracking ref, and cherry-picking onto an unchanged parent reproduced a byte-identical commit, modelling a fast-forward instead of a rebase. Each made the test pass against the broken code. The harness now asserts its own preconditions — no upstream, and `feat/x` not an ancestor of `origin/dev` — so it cannot silently stop testing the defect. A third case pins the property `-d` was there to protect: a branch with a commit that never landed upstream is kept.

- **`test_check_edge_type_drift` passed or failed on how the suite was invoked, not on the code under test.** Both tests called `cli.main()` bare; `main` falls through to `sys.argv[1:]` when its `argv` argument is None, so under pytest argparse received *pytest's* arguments and exited 2 — `pytest tests/ -q` produced `unrecognized arguments: tests/ -q`. Reproduced identically on a clean worktree at `origin/dev`, so this predated the change that surfaced it. Both call sites now pass `[]` explicitly, verified across three invocation shapes; the `None` default is left alone because it is correct for the real CLI, where `--strict` must reach argparse. Swept the class: three sibling tests call a bare `main()` (`deleak-ledger`, `generate-security-md`, `refresh-stdlib-modules`) and none share the defect — the first sets `sys.argv` itself and the other two parse no arguments.

- **The rust-analyzer opt-in nudge no longer fires when the backend is already running (WI-dojud).** `check_rust_analyzer_disclosure` chose its message from "is the binary on `PATH`" alone and never consulted the opt-in gate, so every Rust repo with rust-analyzer installed got "installed but NOT enabled" — including runs where `--backend rust-analyzer` was active and the SCIP path demonstrably ran (same invocation emitted 2 `references` edges with `origin=["scip"]`). It now takes an `enabled` argument, resolved at the call site from `HYPERGUMBO_RUST_ANALYZER` the same way `available` is resolved, and the enabled case states the backend is active instead. The warning is *kept* on the default path: it carries the disclosure that indexing executes the workspace's `build.rs` and proc macros as the invoking user, and that caveat now appears in every arm that speaks — in the present tense when indexing is actually happening.

- **The release gate's ruff and secret-scan checks are clean, and a suppression that had silently expired is repaired.** `release-check` reported 4 ruff and 4 gitleaks findings, none of them a real credential. On the ruff side, three `# noqa: S603` directives suppressed a rule `per-file-ignores` already disables for every test file (lifting that blanket ignore would require annotating 167 sites, so the dead directives go instead), and `S103` flagged the deliberately-permissive `chmod 0o2775` that `test_legacy_setgid_archive_root_is_fully_stripped` creates as the hazard it then asserts rotation strips. On the gitleaks side, a `generic-api-key` match reported `prohibited_sink_zone=` as a secret (the rule keys on "secret" adjacent to `=`), an INI fixture carried `sk_live_12345`, and the secret-scrubber's own tests carry two `gho_` tokens they need as bait. The fixture value is deleted rather than annotated: `_is_sensitive_key` matches on the key *name*, `_mask_value` is length-driven, and no assertion reads the value, so credential-shaped bait bought nothing.
- **`.gitleaksignore` fingerprints are retired as a suppression mechanism.** The INI fixture above was *already* suppressed, by an entry pinned to line 352 on 2026-03-12 (`c44ec3e448`); commit `d0e45ce33b` (2026-07-30) netted one line out of that file above the fixture, moving the finding to 351. The suppression was dead for seven weeks and nothing reported it — these entries are keyed `FILE:RULE:LINE`, so every one of them is perishable. All ten entries are replaced by mechanisms that cannot drift: `gitleaks:allow` **on** the offending line for deliberate credential-shaped strings in source; a `.gitleaks.toml` path allowlist for the gitignored `.hypothesis/` cache, whose filenames are rotating content hashes that three of the fingerprints had been pinned to; and a `.gitleaks.toml` regex allowlist for two fixed benign strings (`apiv2.NewAPI`, `L2-normalized.`), scoped to the strings rather than their files because tracker `.ops` files are committed — a path allowlist there would blind the scanner to a real credential pasted into a discussion thread. A positive control confirms the line-scoped form discriminates rather than blanket-suppressing: an un-annotated token added to an annotated file still fires.

### Documentation

- **A spec-and-ADR consistency audit: 14 defects, every corrected number re-verified against the live code.** The decisions were sound — no two ADRs rule opposite ways, and no spec passage contradicts an ADR's actual ruling. What had drifted was the status-and-count layer, and the split is diagnostic: the two artefacts that self-generate (`ARCHITECTURE.md`) or self-enforce (the `Uncategorized` counter) were correct, while every hand-maintained number was not.
  - **`§14`'s I/O-catalogue instructions were not stale but wrong.** The spec taught `status: complete`, which `_validate_catalog_dict` **refuses at load** (INV-titih: the word asserted coverage nothing checked — `python.yaml` read `complete` from May 2026 holding no row for `os.open`). A reader following the spec wrote a catalogue that could not load. The live vocabulary — `provenance_declared` (a *citation* claim, with per-module coverage asserted separately via `module_completeness`) or `in_progress` — is now documented, along with the correct census: 15 catalogues, 3 `provenance_declared`, 12 `in_progress`. The old census claimed 3 `complete` (there are none) and listed 11 of the 12 `in_progress` (`bash` was missing). `docs/MIGRATION-IO-BOUNDARIES.md` had it right; the spec had been contradicting a sibling doc.
  - **`rust-analyzer` executes `build.rs`, and only the code said so.** The package's own `__init__.py` carries the SAFETY note; nothing in `docs/` did, while `§4`, the env-var table and `§20` all presented `--backend rust-analyzer` as a pure quality upgrade. Disclosed at all three opt-in sites: this is the one pass that executes analysed code — every other analyzer only parses — so it must never be pointed at an untrusted repo.
  - **Five version and count claims re-anchored to their constants:** `schema_version` `0.14.4` → **`0.20.1`** (stale since 2026-06-30, six minor versions), `VERIFY_CLAIMS_SCHEMA_VERSION` `1.8` → **`2.0`**, and `dead-code-maybe` moved out of the shared `READ_VIEW_SCHEMA_VERSION` list — it promoted to its own `DEAD_CODE_MAYBE_SCHEMA_VERSION` (`0.2.0`), the promotion Appendix C predicted in writing and then missed. The `evidence_type` axis split `114/11` → **`115/10`** (the *total*, 125, stayed correct, which is why the drift hid). Linkers `57` → **`61`** in both the spec and `LINKERS.md`, matching the generator's Protocol 11 / Bridge 10 / Framework 32 / Infrastructure 8 — after the generator itself was corrected (see the underscore blind spot below; the first pass through this number landed on 60, which was the generator's own undercount).
  - **A coverage claim that named two languages the code does not reach.** `§14` credited WI-vipur's scoped-path traversal to "Rust, C++, Groovy, Kotlin"; `emit_module_attribute_refs` is wired into five analyzers (`cpp`, `go`, `java`, `js_ts`, `rust`) and the helper's own docstring names only Rust and C++; C reaches the same edges through a separate bare-identifier path in `c.py`, because its stdio globals are never member expressions and the shared helper cannot match them. Groovy and Kotlin never call it, so their module-attribute reads — and the `env_read` taint sources those would carry — are **absent, not empty**. Stated as a disclosed gap rather than deleted.
- **The ADR corpus is brought back under `docs/adr/README.md`'s own lifecycle law**, which says a `grep` of the status line must not lie and that supersession is symmetric and by-number. It was lying in eleven places. **ADR-0019 and ADR-0020 both read `Status: Proposed` for work shipped in tracker v0.2.0 on 2026-04-04** — `htrac serve`, the WebAuthn/bcrypt/duress auth stack, the BlockSuite frontend and the Tor onion recipe for 0019; the whole annotation-and-preview surface (`screenshot_save.py`, `annotations.py`, `svg_injection.py`, …) for 0020. 0019's status now also records what did *not* ship (the native iOS/macOS client is not in this repo; Part B is not started) rather than rounding "Part A" up to done. **ADR-0028's `Superseded by:` read `—`** although ADR-0037 names it and 0028's own amendment describes the supersession, and **ADR-0039 carried no `Supersedes:` field** though ADR-0005 names it as its superseder — both endpoints now name each other. ADRs 0014, 0015, 0028 and 0034 recorded their partial supersession *only* on a `Superseded by:` line while the greppable `Status:` line said a bare "Accepted"; the status lines now carry it. Six ADRs used a `## Status` heading or no status line at all instead of the mandated top-of-file line (0010, 0011, 0012, 3ccc, and the 0025/0026 reclassification stubs, which gain the `Status: Reclassified → <path>` form the README defines for exactly that case) — all 46 ADRs now parse from one greppable line. The README index is re-synced where it had fallen behind its own ADRs: ADR-0023's §6 fold completed 2026-07-21 (indexed "in progress") and ADR-0037's ruling 4 landed 2026-07-02 (indexed "deferred"). ADR-3bbb asserted "all 45 linker modules declare their subcategory" in the present tense against a catalogue now at 60 — reworded so the *invariant* carries forward while the rollout-era count stays historical.
- **A follow-on spec-and-ADR audit: six defects, and the linker count was wrong at the source.** Every number below was re-derived from the module's own registry rather than a grep, after two of the audit's first-pass findings turned out to be the auditor's regex rather than the corpus (a `Status:` pattern blind to the newer list form, and an `ADR-[0-9a-z]{4}` that matched the prefix of "ADR-sanctioned").
  - **The linker catalogue is 61, and the undercount was structural rather than clerical.** `scripts/generate-architecture` skipped every underscore-prefixed filename — correct for private helpers (`_concept_utils`, `_text_filters`, `_view_template_core`), wrong for `linkers/_third_party_bases.py`, which is underscore-named *and* registers `django-third-party-dispatch-linker`. So the generated count read 60 against a registry of 61, and the spec, `LINKERS.md`, `ARCHITECTURE.md` and ADR-3bbb's own Status line all inherited it. The sharper failure is that ADR-3bbb's enforcement promise — "the `Uncategorized` counter is 0 and will surface any future regression" — had a hole exactly the shape of this file: a module the scan skips can be flagged neither categorized nor Uncategorized. The scan now decides on the presence of `@register_linker` rather than on the filename, and a test asserts the property for *every* registering module, so the next underscore-named linker fails loudly instead of vanishing from the count.
  - **`LINKERS.md` was not the full catalogue the spec sends readers to it for.** Four registered linkers had no row — `argparse_dispatch`, `caddy_module_dispatch`, `receiver_type_dispatch`, and the `_third_party_bases` Django cascade — and three of them appeared nowhere in the file at all. The table now carries 61 rows against a registry of 61.
  - **Three more count claims re-anchored:** `taxonomy.py` covers **89** languages, not 86; the I/O catalogue supports **17** languages (15 dedicated + 2 aliases), not 16 (14 + 2) — a figure `§3` and `§12` had been contradicting each other on, with `§12` correct.
  - **Four supersession records were one-sided**, against the README's own rule that both endpoints name each other by number and that the relationship is never recorded as one-sided prose. ADR-0034 declared itself partially superseded by ADR-0036 while ADR-0036's `Supersedes:` field read `—` and buried the relationship in a parenthetical — a direct contradiction between two files, not a formatting nit. ADR-0024's OQ1 settlement by ADR-0038, and both ADR-3aaa extensions (3bbb, 3ccc), were recorded only from one side. A new test enforces the symmetry, parsing the declared fields with parenthetical *citations* stripped — because a `Supersedes:` line routinely names an unrelated ADR to explain why something is dead (ADR-0035 cites ADR-0032 that way), and reading numbers out of the raw line reports that citation as a claim.
- **Two broken relative links in `docs/audits/`:** `0004` pointed at a pre-rename `0028-evidence-type-inference-pathway-not-resolution-status.md`, and `0009` misspelled the fundamental-concept-audit playbook path.
- **The spec's IR contract is re-anchored to the dataclass and the published schema — and the class of drift is now a test, not a proofread.** Three defects, all silent, all in the layer a JSON consumer reads before writing any code. **(1) §6 declared four fields non-nullable that the code and `docs/schema.json` both make optional.** `language` has been `Optional[str]` since ADR-0031 gave Class B synthetic stand-ins no host language, and `span` since `SCHEMA_VERSION` 0.20.1 (WI-hafap) — neither carried so much as a drift marker, while `fingerprint` and `quality` carried a 🟪 comment *admitting* the code says `Optional` beneath a declaration that still said otherwise. A trailing note is not a type. The spec also contradicted itself: §9's per-language attribution paragraph already described falling back to `discovery_language` "when `node.language` is `None`", a case §6 said could not arise. All four declarations are now correct and carry the reason (`fingerprint`'s `hgfp2:` content hash versus ADR-0032's demolished Format-1; `quality`'s INV-nuzal declared-but-empty status and its omit-when-None serialization). **(2) Appendix C listed `language` among node fields that "cannot remove or change type"** — an immutability promise ADR-0031 had already broken. The bullet now separates key presence from value presence: the keys are guaranteed, the `language` *value* is null for Class B stand-ins, and `span` is nullable too. **(3) `dst_ref` appeared zero times in the spec.** It is serialized by `Edge.to_dict`, published in `docs/schema.json`, and is ADR-0037 ruling 1's canonical source of truth for external-target identity — the legacy `dst` string is rendered *from* it. Now documented with the two facts a consumer cannot infer: a fully identified external target is `is_resolved=False` **with** `dst_ref` populated (resolution names in-repo-ness, not target-identification), and the key is omitted entirely when null, so its absence means an in-repo dst rather than an unknown one. `Symbol.cyclomatic_complexity` had the same gap and is documented alongside. Of 39 published `Symbol`/`Edge` schema properties, undocumented goes 2 → **0**. The guard is `packages/hypergumbo-core/tests/test_spec_ir_contract.py`: every published schema property must be named in the spec, §6's declared nullability must match `ir.Symbol`, and a field Appendix C calls immutable must disclose that it can be null. It found `fingerprint` and `quality` on its own — two instances beyond the three the audit had identified by reading — which is the argument for the test over the proofread. `scripts/check-schema-coverage` does not overlap: it ratchets registry *value* coverage across a fixture corpus and says nothing about whether a *property* is documented.
- **The status-and-count layer, second pass: five more defects, and two of them were the ADR corpus saying a decision had not taken effect when it had.** **(1) ADR-0005's status was conditional on a condition that already fired.** It read "takes effect once ADR-0039 implementation lands" — and it had: `entrypoints.py:1605` sorts by `rank_score`, the `:1661-1685` penalties mutate `rank_score` and leave `confidence` at its construction base, `Edge.confidence_source` ships with its three-value registry, and `Edge.quality` was deleted at `SCHEMA_VERSION` 0.20.0. The status now states the supersession is IN EFFECT and the amendment reads in the past tense. **ADR-0039 read a bare "Accepted" for a decision that shipped in full** and now records all five rulings as implemented, including ruling 5 Stage B — the per-evidence-type table IS generated from `EvidenceTypeSpec.base_confidence`, as the projection in `docs/concept-axes.md` emitted by `scripts/generate-concept-axes`, gated by its `--check` in `.githooks/pre-commit` and pinned by `test_generate_concept_axes.py`. (An intermediate commit in this cycle asserted the opposite — that Stage B had not landed and no generator existed — after searching `scripts/` for a filename containing "confidence". The generator is named for what it emits, not for the value it carries. The claim was wrong on all four surfaces it reached and is corrected here.) ADR-0005's tier constants remain marked illustrative, but for the correct reason: the **entrypoint detector tiers** it quotes are unregistered literals in `entrypoints.py` and were never in Stage B's scope, which covers the evidence-type table only. **(2) The README index was softer than the ADRs it indexes.** ADR-0036 read "Accepted" against a file saying the decision "shipped only partially" (Ruling 2 advisory, Rulings 1 and 3 never landed); ADR-0024 read "Accepted" against a file recording OQ1's partial supersession by ADR-0038. The README's own law — "a `grep` of the status line must not lie" — is written about the ADR files, which complied; the index is the copy a reader meets first, and it did not. Four rows re-synced. **(3) Four ADRs the spec never cited while discussing their subject matter:** ADR-0029 (inheritance-aware call resolution belongs to the `inherited_calls` Infrastructure linker — analyzers must not walk ancestor chains), ADR-0034 (ID construction goes through the factories), ADR-0044 (`Symbol.origin` values are pass IDs, not a separate synthesis axis) and ADR-0022 (the `config` subcommand is its by-category half; the by-language `LanguageProfile` registry is deferred). Uncited ADRs drop 10 → 6, and the remaining six are out of spec scope by construction (agent instructions, the tracker package, the two reclassification stubs). **(4) §6's `class Symbol` block was 17 of 30 fields** while presenting itself as the dataclass. The omissions were not uniform — ADR-0032's typed siblings were listed and ADR-0031's (`discovery_language` / `protocol_origin`) were not, and `meta` was missing from the very block whose `kind` comment tells the reader that framework-role and dispatch facts live in `meta`. All 30 are now listed in declaration order. **(5) §3 named 11 of 27 subcommands, and one status glyph was outside the legend.** `search`, `routes`, `symbols`, `dead-code-maybe`, `compact`, `config`, the two cache commands, the explicit `sketch` spelling, the `run`-is-an-alias-for-`survey` fact, and the nine environment-management commands now have entries with their real flags, taken from the parser rather than written from memory; `renders` carried a `✅` that is in no legend row and is now the `🟩` the legend defines. Three guards land with the fixes: §6 must list every dataclass field, every glyph must be a legend row, and every registered subcommand must appear as `` `hypergumbo <cmd>` `` (`test_spec_ir_contract.py`); plus a new `tests/test_adr_readme_index_sync.py` asserting the index never drops a qualifier its ADR carries — one-directional, so a shorter index cell stays legal and only a *softer* one fails. **ADR-0005's fix has no mechanical guard**: knowing the condition fired required reading `entrypoints.py`, not the documents, and a test that only appeared to cover it would be worse than none.
- **Third-pass spec-and-ADR audit: two defects, and the count layer finally gets a gate.** The structural layer came back clean — all 412 internal links across the spec and 47 ADRs resolve, supersession is symmetric under the README's own rule, every ADR parses one greppable `Status:`, all 26 CLI subcommands are documented with no ghost commands, and the registry-backed vocabularies (edge-type, evidence-type, symbol-kind, pass-id, audit-findings, axis declarations) pass all six drift gates. Two things did not. **(1) A broken anchor**: §9's JNI cross-reference pointed at `#java-jni-cross-language-detection`, a heading that does not exist — §7's is `JNI Boundary Detection (Java ↔ C)`. The sibling IPC link on the next line resolved, which is why it read as fine. **(2) ADR-0012 was contradicting the spec.** An in-force ADR (`Step 1 implemented`) described `45 linkers`, `24 linkers` and `104 analyzers` in the present tense, against a live registry of **61** and **118** — while the spec, `LINKERS.md` and `ARCHITECTURE.md` all said 61. The fix is structural rather than clerical: the restated totals are **removed** and replaced with pointers to the registry and to `LINKERS.md`, because retyping 45 → 61 only resets the clock on the same drift. The Context section's count is explicitly dated as-of-filing, the way ADR-3bbb already handles its own rollout-era table.
  - **The number the spec publishes is now pinned.** `ARCHITECTURE.md` self-generates and its inventory was already tested; the spec's §7 sentence and `LINKERS.md`'s `**Count:**` line are typed by hand and were regenerated by nothing. `packages/hypergumbo-core/tests/test_spec_linker_count.py` pins both against `list_registered()` under the import path production uses — not a directory glob, which answers a different question and returns 60, since `_third_party_bases.py` registers a linker whose name does not match its module. The gate keys on **anchored declaration sites**, never on the phrase "N linkers", which the corpus uses overwhelmingly for subsets (16 stamping access modes, 21 implementing a contract, ADR-0031's 10 / 8 / 4); a gate aimed at the name rather than the thing would be a false-positive machine. A declaration that goes **missing** fails exactly like one that is **wrong**, so the guard cannot be satisfied by deleting the sentence it guards, and three positive controls plus a live-tree doctoring run show it firing.
  - **Two of the audit's findings were the auditor's instrument, again.** A slugger collapsing whitespace runs reported 13 broken anchors where GitHub's replaces each space individually (`A & B` → `a--b`) and only 1 was real; a supersession checker reading prose out of ADR-0035's `Supersedes:` field invented an ADR-0032 relationship from the parenthetical "dead since ADR-0032 removed `canonical_name`". Both were caught by checking the control before reporting the subject.

- **The spec is corrected against what the code emits.** Two taint paragraphs asserted the opposite of the implementation (`inconclusive` is the majority verdict; function summaries do have a live consumer), and four further passages are reconciled against the producer.
- **`docs/schema.json` is generated from the registered meta-key vocabulary** rather than hand-maintained — `Edge.meta` goes from 2 documented keys to 34 and `Symbol.meta` from 0 to 47 — with `profile` reconciled to what the producer emits (ADR-0028).
- **ADR-0016 is clarified where it was misread twice:** the prohibition covers a *launched program's* I/O, not the shell's own, and the overlay constraints bound an overlay's standing rather than its influence — an overlay row is narrower than a completeness entry, not safer.
- **Concept audits and the confidence model.** A `status:` field on I/O catalogues is recorded as a confirmed conceptual leak; the `Symbol.kind` type-family audit **rejects** the suspected leak and names a missing consumer predicate as the real defect (seven consumers hand-rolled inconsistent type-like kind sets), adding a `type_family` taxonomy and predicate that gives Swift protocols interface dispatch for the first time (ADR-0024). The entrypoint confidence contract is rewritten from measured ground truth, and the per-pathway `base_confidence` table is generated with a `--check` mode (80 of 125 evidence types seeded, band 0.40–0.95).
- **Spec §Taint-flow-analysis: two "pending" attribute-primitive gaps are closed, and a backend claim is corrected.** Tree-sitter Rust scoped paths (WI-vipur) and C bare stdio identifiers (WI-gotuv) both landed; verified end-to-end rather than from tracker status — `std::env::consts::OS` classifies `env_read`, bare `stdin`/`stdout` classify `ipc_recv`/`logging`. The adjacent sentence "the `rust-analyzer` optional backend is unaffected by the tree-sitter gap and continues to resolve Rust semantics fully" is replaced: attribute-reference coverage belongs to the *tree-sitter* analyzer, the SCIP backend emits no `module_attr_ref` edges at all, and the two backends coexist rather than substitute — so backend choice does not affect this coverage in either direction. (The clause was scope creep past its own commit message, which claimed only that the gap was a tree-sitter grammar artifact; it was written 2026-04-22, three months before the resolve-map fix that stopped finalize from silently discarding the entire SCIP call graph.)

- **The G2 parity matrix declares and machine-enforces its own scope,** correcting a false availability claim that gated it (only 1 of 8 gated languages is `availability="core"`, not 8).
- **A docstring-staleness audit corrects five module docstrings that had drifted into falsehood.** `cfg.py` named its semantic hooks `early_return_on_error` / `deferred_execution` when the YAML keys the parser reads are `early_return` / `deferred` — and since every lookup is a `data.get(<key>, [])`, a new language mapping written from that docstring would have parsed clean and produced a CFG silently missing those constructs (`cfg_nodes/go.yaml` repeated the same wrong name). `safety_zones.py` claimed the pattern "does NOT prevent misuse" and that the wrappers are behaviourally transparent, both false for the five destructive wrappers that resolve the path and raise `SafetyZoneViolation`. `subprocess_cli.py` still described the single-root manifest read that WI-gadus removed — i.e. the bug, as if current. `taint.py` described DDG-backed analysis in the future tense and claimed every finding carries `analysis_method="structural"`, though `ddg` and `ddg_mixed` are live values. The tracker's `sync.py` named only the Forgejo backend for what is now a dual-backend dispatcher.
- **Three module docstrings gain the functionality they had silently stopped describing.** `sketch_embeddings.py` now states its degradation contract as TWO independent failure modes — the library not importing, and the library importing while the model weights are absent — because conflating them is exactly what made `hypergumbo .` exit 1 writing zero bytes (INV-rupid); it records that `_has_sentence_transformers()` answers only the first, and why the probe is deliberately not strengthened. `safety_zones.py` lists all 22 wrappers grouped by zone instead of 8, marks which are zone-enforced, and documents the `repo_inspection` subprocess-read family the module title never covered. `discovery.py` documents content-conditioned virtualenv exclusion — `venv`/`.venv`/`env` are excluded by containing a `_VENV_MARKERS` file, not by name — including the disclosed limit that the Windows marker has zero real-world positives across the 467-repo corpus and is fixture-asserted only.
- **A second staleness pass, after the scan itself was corrected, finds four docstrings describing shipped work as future.** `hypergumbo-lang-rust-analyzer/__init__.py` said "later slices add a live rust-analyzer invocation, analyzer-registry wiring, and the opt-in flag machinery" — all three had shipped as `invoke.py` / `analyzer.py` / `gate.py` and WI-duzul is closed. `scip/edges.py` deferred span-based occurrence resolution to "a later slice / linker"; `calls.py` (Slice D, WI-kopav) owns it. `rust_analyzer/invoke.py` listed three items as "tracked under WI-duzul Slice C+" that each now have a named sibling module. `scip/index.py` said "Slice C will wire those to external-tier nodes", which was wrong twice over — Slice C shipped without doing it, *and* the symbol side was never where external references belonged (hypergumbo mints `external_symbol` boundary nodes from dangling **edge** endpoints via `create_boundary_nodes`, so skipping a non-Definition `SymbolInformation` in a Symbol shim is correct). The note now points at where the behaviour actually lives — `translate.py`'s `resolve_symbol` returns `None` for out-of-workspace symbols, dropping the edge rather than letting it dangle, which is deliberate: before commit `9266762d4b` raw SCIP descriptor endpoints made every scip edge dangle and finalize discarded the whole scip call graph (0 → 739 edges on zoxide once resolved). The residual consequence is disclosed — a Rust call into a dependency or the stdlib yields no edge under this backend, unlike the ~69 `:unresolved` dst sites in the tree-sitter analyzers — and attributed to its owner, WI-gojum sub-component 1, parked 2026-07-19. Two numeric corrections alongside: `slice.py` documented `max_files` "default 20" against a code default of **100**, and `linkers/registry.py` said "~40 passes" against **61** registered linkers.

- **A third staleness audit sweeps the retired-edge-type vocabulary out of the docstrings still teaching it.** The co-change scan flagged 45 files (the 15 the previous audit fixed are absent from the pool — their docstrings were touched, which reset the blame floor); 41 carried real drift, and the dominant family is edge-type and symbol-kind names that the ADR-0023 §6 and ADR-0027 folds retired but that docstrings still present as live. Each was verified by grepping the name repo-wide and finding it **only** in the docstring claiming it: `registers_routes` / `contains_routes` (now `references` / `contains`, the router-versus-controller distinction having moved into `meta['framework_dispatch']`), `query_references` / `links_to` / `type_ref` (all `references`), `message_publish` / `message_subscribe` / `message_send` / `message_receive` (all `event_publishes` — and the message-queue linker emits no subscribe-direction edge at all, so those arrows named a second edge type that never existed), `routes_to` and `invokes_callback` (both `dispatches_to`), and `kind="route"` (now `kind="function"` + `meta.framework_role`, per the ADR-0027 Phase-3 fold — `route` is not a registered symbol kind). Correcting the flagged files exposed the rest of the family in files the co-change scan **structurally cannot flag**, because those docstrings never aged relative to their bodies: the sweep therefore extends to `route_handler.py`, `ipc.py`, `phoenix_ipc.py`, `controller_routes.py`, `framework_patterns.py`, `ruby.py`, `cli.py` and `axis_meta_keys.py`, found by grepping the retired names rather than by blame. One of those sites was not confined to a comment: `axis_meta_keys.py`'s `topic` spec feeds `scripts/generate-schema`, so the retired pair was being published to JSON consumers in `docs/schema.json` as well — regenerated here.
- **Six false claims in `sketch.py` alone**, the module whose docstring is the reference for what a sketch contains: the Header and Overview section contents were described inverted (the README pitch is in the header; the language stats are the Overview), `require_sections` was documented as "fail loudly if a required section can't emit" when it force-*includes* past an exhausted budget and raises nothing, Additional Files was documented as gated on `--with-source` when the flag only shrinks its budget to 5%, Configuration claimed environment-variable and dotenv-key extraction that does not exist in `CONFIG_FILES_BY_LANG` or `_extract_config_heuristic`, the dampener stack listed a `sibling-impl` stage removed by WI-karad, and an optional `tiktoken` dependency was offered for precise token counting — `tiktoken` occurs nowhere in the repository outside that sentence.
- **`py.py` stated the opposite of the shipped `stable_id` contract.** "Survives renames and moves if signature unchanged" — but the v6 tuple folds `name`, `qualified_name` and the file-anchored `containing_stable_id`, so a rename and a move each change it (ADR-0035 §2; the same file's own INV-zudob note said so 2,500 lines further down). `fingerprint` is the field that joins across a rename. The same docstring described Pass 1 as collecting "top-level functions", against INV-mofav's every-nesting-depth emission.
- **Two modules documented an API with no production caller** — a reader following the docstring would reach for the wrong function. `io_boundary.match_edge_to_primitive` is a bare name-only lookup used by tests; the tagger calls `classify_call_in_catalog` → `lookup_with_module`, which adds module-hint filtering, FFI redirection, `io_mode` and a gated short-name fallback. `tracker/annotations.to_dict` / `annotation_from_dict` are called only by `test_annotations.py`; the TUI consumes live `Annotation` objects and writes the SVG directly, and the round-trip "through discussion ops" the docstring described is not the persistence path ADR-0020 specifies.
- **`verify_claims.py` never gained its fourth verdict**, enumerating three where the code returns `confirmed_with_caveats` at exit 3 — the headline of the same release. **`tracker/setup.py` called three repo-mutating checks "Read-only / advisory only"**; one of them writes `AGENTS.md`, one writes and chmods a wrapper shim, one runs `git config core.hooksPath`, and all three return `status="fixed"`.
- **Fourteen further corrections** where the docstring asserted a rule the code does not follow: `go.py`'s ambiguous-method guard does resolve rather than refuse when a candidate is an interface method; `js_ts.py`'s resolved cross-file call edges carry no `dst_ref` (only the unresolved named-import path does), and method `meta` never carries `route_path`; `rust.py` and `swift.py` each have emitters exempt from the stated `is_exported` rule (bodyless trait methods and enum variants; enum cases, protocol requirements and route markers); `erlang.py` extracts no `-spec`; `elixir.py`'s `alias` emits no edge; `racket.py` skips `module+` and emits structs as `kind="class"`; `ts_def_use.py` drops 30 skip-listed names and every plain-call callee rather than treating all identifiers as uses; `test_masking.py`'s reverse BFS is capped at 10 hops, not full transitive closure; `cli.py` described all linkers as cross-language when only the Bridge subcategory is language-pair (ADR-3bbb); `edge_types.py` described `endpoint_shape` and `pending_classification` as live axes when both are empty and `edge_types_on_axis` has no caller; `tracker/models.py` referenced a `compile()` that is `store.compile_ops()`; `tracker/tag_catalog.py` claimed four timestamps against three, an `in_favor_of` "ignored on read" that `from_dict` reads unconditionally, and load/save enforcement of a timestamp format neither path validates; `rust_scip.py` described a three-input `stable_id` against six and used the future tense for the shipped rust-analyzer backend; `manifest_targets.py` described extractors as yielding tuples when every one returns `None` and mutates caller-owned lists.

- **The staleness audit's second half: 37 module docstrings gain the functionality they had stopped describing.** Where the first pass deleted false claims, this one supplies the missing ones — the C-class findings, which need new prose rather than a deletion. The largest was `analyze/base.py`, whose "Shared Components" list named the dataclasses and helpers but not `TreeSitterAnalyzer` — the module's own title subject, 29% of the file, subclassed by ~105 analyzers — so the module read as a utility bag rather than the base class every analyzer inherits its two-pass orchestration from; the same list now also covers the stable-identity layer (the `assemble_stable_id` chokepoint, its typed and route-shaped variants, and the post-pass populators that fill or widen identity once the whole symbol set is known) and the memory-pressure guard. `hypergumbo_tracker/__init__.py` described six of its thirty-three modules and read as if the tracker were still storage-plus-CLI, omitting the TUI, the WebAuthn/duress web server, auto-sync, the out-of-repo op journal, the stop hook and the embedding-based dedup. `verify_claims.py` gained the two claims-file keys it never documented (`extra_catalogs`, `analysis_scope`), the taint-vocabulary load gate, and the three caveat kinds with the rule that opacity qualifies a verdict only when it is the *sole* remaining blocker. `io_boundary.py` is retitled from "catalog loading and edge matching" to what it actually owns — overlays, mode discrimination, opacity, the tagging pass and the versioned boundary map.
- **Analyzer docstrings stop under-reporting what they emit.** Ten analyzers listed only their callable symbol kinds while emitting fields, variables, file anchors and route markers besides (`py`, `go`, `rust`, `js_ts`, `swift`, `csharp`, `clojure`, `lua`, `powershell`, `agda`). Several omitted whole edge families: Go's `wraps`, `module_attr_ref`, build-tag `references`, Cobra `AddCommand` usage contexts and `go.mod` dependency manifest; Rust's `implements`, `async_spawn`, macro-body calls and four of its six call-resolution strategies; C#'s `decorated_by` and method-group `references`. Others omitted the rule that makes their output trustworthy rather than merely present: Haskell's short-name confidence penalty (1-char callees × 0.15) without which single-letter binders dominate the call graph; Erlang's multi-clause coalescing, which stops an idiomatic pattern-matched function being triple-counted; Lua's INV-fahub deferral, where a name-based hit is deliberately *withheld* rather than resolved; Racket's synthetic unresolved-call target at reduced confidence; Markdown's `_make_link_dst_id` reshape, which exists so a bare URL does not land in a boundary node's language slot.
- **Two linkers documented a client list narrower than their own patterns.** `http.py` omitted Elm entirely — a whole `_scan_elm_file` path with five patterns, a glob and a `depends_on` entry — along with JS/TS template-literal folding and the variable-URL prefix-match fallback; `event_sourcing.py` listed Java as Spring-only against Guava EventBus and four framework-agnostic emitter/listener shapes, and omitted Go's channel and event-bus scanning outright. `grpc.py` gained the four edge families its "How It Works" never reached, including the WI-ropoz fallback that emits `calls` for a client stub whose servicer is absent from the repository — where the alternative was emitting nothing and reporting a clean graph. `database_query.py` gained the INV-zuhub disambiguation rule that makes a multi-candidate table match resolve at confidence 0.5 with `disambiguation_fallback` set, so a guess is visible as a guess.
- **`edge_types.py` documents its meta-discriminator vocabulary**, the exports consumers need now that `edge_type` alone stopped answering their question: the curated subsets, the ADR-3bbb subcategory sets, the `is_grpc_rpc_implementation` predicate that reads `meta` because the fold moved the distinction there, and `find_axis_drift`. `ts_def_use.py` records that it registers a `LanguageDdgSpec` for TypeScript *and* JavaScript, and that the function-node-type set in each spec is the scope limit deciding how much of a file the data-dependence walk can see — a load-bearing constraint that lived only in an inline comment. `rust-analyzer/translate.py` documents the symbol-resolution map and the deliberate dropping of out-of-workspace edges, with the measurement behind it (0 → 739 edges on zoxide once endpoints resolved) and the disclosed residual, attributed to its parked owner.
- **Residue from the first pass: the retired-vocabulary sweep is now complete.** The earlier PR's sweep was truncated by a `head` in the verifying grep, so `routes_to`, `invokes_callback` and `kind="route"` survived in twenty-one further comments across `cli.py`, `entrypoints.py`, `framework_patterns.py`, `grpc.py`, `http.py`, `py.py`, `go.py`, `ruby.py`, `swift.py`, `php.py`, `elixir.py`, `play_routes.py` and `starlette.yaml`. Every remaining occurrence was checked against the live registries: `route` is absent from `symbol_kinds.py`, no producer constructs `kind="route"` (all twelve callers route through `make_route_symbol`, which returns `kind="function"`), and every consumer already tests `meta.framework_role` — so the code was correct throughout and only the comments were stale. One comment claimed the route kind "matches rails.yaml symbol_kind pattern"; `rails.yaml` matches `^class$` and no YAML in the tree matches on a route kind, making that claim wrong twice over.
- **Every ADR line-number citation becomes a symbol citation — 274 references across 29 ADRs.** A `path.py:NNN` pointer rots silently: the cited code slides downward on any insertion above it, and the reader lands on unrelated lines with no signal that anything is wrong. **21 citations were provably broken before this change** (one past EOF, twenty landing on blank lines), and hand-sampling found the non-blank ones drifted too — ADR-0037 pointed at `to_dict` serialization while claiming it was the boundary remap, ADR-0040 cited three **Edge** fields at lines inside **Symbol**, and ADR-0035's five `make_*_stable_id` citations all landed in one unrelated function. Two were in *status lines*, which `docs/adr/README.md` requires not to lie: ADR-0028 placed `Edge.is_resolved` 433 lines from where it lives, ADR-3ccc placed `UsageContext` 219 lines away. Citations now name the symbol (`` `ir.py::Edge.is_resolved` ``), which changes the failure mode — a renamed symbol fails to resolve instead of confidently pointing at the wrong code — and makes the reference greppable. Where the prose already named the right symbol, the conversion **repaired** the pointer rather than re-addressing it: **all 62** such citations were pointing somewhere else. Four references whose named code no longer exists are marked as historical instead of re-pointed (ADR-0042's `find_behavior_map`, which moved to `survey_io.py` in the very rename ADR-0042 specifies; ADR-0040's `test_edge_has_evidence_spans`, deleted with its field; ADR-0041's tier-min relabeling, retired by its own ruling; ADR-0029's `_find_inherited_initialize`, deleted upstream). `spec:NNN` references become section references — ADR-0039's four all pointed outside the §12 its own prose names. Two line numbers are deliberately kept, both into immutable artifacts (a dated lab-notebook quote and a completed rollout note). A throwaway AST validator gated the conversion and caught **11 unresolved citations, four of them symbol names the conversion had invented**; it also caught `test_ruby.py::test_rails_routes` in ADR-0008, which was already in `::` form and already broken — symbol citations rot too, just only on rename. Shipping that validator as an on-demand checker (deliberately not a CI gate) is **WI-kitij**.

- **The entrypoint confidence bases had three homes in the spec and the three disagreed.** §9 carried the complete eight-base list (WI-logad); §8's `Entrypoint Confidence Tiers` table still carried **four** of them and closed "All four confidence tiers are now implemented"; §12 asserted "the four standard tiers span [0.70, 0.99]" and said the only other values sat *below* that band. Both of the latter are false against the detector: 0.90, 0.80 and 0.75 sit **inside** [0.70, 0.99] and are none of the four, and the omitted 0.75 `library_export` is — by §9's own measurement, twelve hundred lines away — the most common entrypoint kind on hypergumbo's own corpus (45 of 118). WI-logad had corrected one copy of the table and left the other two standing. §8's table is now the canonical eight rows and says so; §12 stops enumerating and records what it previously got wrong; the 0.05 frontend-route suppression is named as a sentinel below `MIN_ENTRYPOINT_CONFIDENCE` (0.10) rather than as a tier. The guard is `test_spec_ir_contract.py`: §8's table and §9's list must each equal the set of `confidence=` literals the detector emits, read from the **AST** — a regex over `entrypoints.py` also matches the three `confidence=0.9x` mentions inside `_detect_from_concepts`'s docstring, which is how the first pass at this finding over-counted the emit sites (41, actually 38) and briefly mistook that function's accurate per-family docstring for a fourth competing model. A vacuity control asserts each of the three extractors still matches, so the equality cannot pass on empty sets.
- **`Symbol.meta`'s registered-key count was three short** — §9 said 47 where `axis_meta_keys.meta_keys_on_axis(AXIS_SYMBOL_META)` returns 50 — and nothing recomputed it. Now pinned to the registry by the same guard.
- **The I/O catalogue description contradicted ADR-0016.** §3's `io-boundaries` entry advertised "150+ framework IO entries" for catalogues that ADR-0016 rules are **stdlib-only** ("the stdlib-only catalog scope is sound and stays"); the claim was written 2026-03-23 and the strict-stdlib cull that removed third-party framework rows landed 2026-04-24. It was also unfalsifiable as written — catalogue entries carry `module` / `methods` / `notes` and no stdlib-vs-framework marker, so the number could not be checked even in principle. Replaced with the scope rule and the supported-language census that *is* checkable: 15 dedicated catalogues + 2 aliases, third-party supplied by the user as project-local overlays via `--io-primitives`.
