...
`Edge.dst_ref: ExternalRef | None` lands as the canonical sibling carrying `(lang, module_path, name)`; the legacy `Edge.dst` string stays populated for back-compat and cache compatibility. Seven non-Python analyzers retrofit against it, four high-leverage consumers migrate to read it, and the historical Rust 6-segment dst fabrication is eliminated at the producer.
...
- **`hypergumbo_core.ir.ExternalRef`** — frozen dataclass `(lang, module_path, name)`. Aliased imports (`from X import Y as Z`) bind `name=Y` — the alias is a property of the call site, not the target.
- **`hypergumbo_core.import_scope.ImportScope`** — shared analyzer-layer abstraction with five population methods (`add_named`, `add_namespace`, `add_wildcard`, `add_aliased_module`, alias-of-namespace) mirroring construct shapes across the 8 audited languages. `resolve(local_name) → CanonicalName(module, name)`; convenience `dst_ref_for(local_name, lang) → ExternalRef`.
...
- **Known limitation.** Short-name sink matching still overapproximates for receivers the DDG can't resolve — call-return bindings (`x = requests.Session(); x.get(...)`), parameter receivers, closure captures. SECURITY.md documents the residual; load-bearing claims (dev-zone unreachability from runtime CLI, install zones not reached from runtime CLI) verify cleanly. Receivers bound by file-scope imports or local module-attribute assignment are resolved by the post-DDG IR refinement pass (below).
...
Activation-gated on `LinkerActivation(frameworks=["django"])`, so it fires only when `profile.frameworks` reports `django` detected — companion to the always-on `django-orm-dispatch` for Django-stdlib bases. Edges carry `meta["framework_dispatch"]="django_third_party"`. Disambiguation-fallback discipline preserved: unqualified short-name match colliding with an in-tree Python class downgrades to `confidence=0.5` + `meta["disambiguation_fallback"]=True`.

Smoke test on lutris-website: 3 new edges for DRF `ModelSerializer` overrides. Across pretix + bakerydemo + lutris the new linker contributes ~38 method-edges with the method-override filter applied (`ModelAdmin` excluded because `django-orm-dispatch` already covers it).
...
- **Filter 2 — closed-world stdlib gating.** `IoBoundaryCatalog` gains three new fields and a chain-skip rule.
  - `stdlib_modules` — frozenset of authoritative names (Python: `sys.stdlib_module_names`).
  - `stdlib_prefixes` — tuple for hierarchical namespaces (Java's `java.*`, Go's `encoding/*`); prefix match accepts `.` and `/`.
  - `stdlib_module_completeness` — dict mapping module name → audit `retrieved:` date; modules listed here are treated as closed-world.
  - `merge()` unions all three with the child winning on `completeness` collisions.
  - YAML parser accepts both flat strings and dicts with inline `completeness: complete` + `retrieved:`. Hard-errors at load time when `completeness: complete` is declared without a `retrieved:` ISO date.
  - `_compute_external_potential` skip rule: after the existing `stdlib_other` check, if `module_hint` is non-empty AND `catalog.is_stdlib_module_complete(module_hint)`, skip the chain. Fires only on confident module identification (no skip when `module_hint == "external"`).
  - `scripts/refresh-stdlib-modules <language> --version X.Y` regenerates `stdlib_modules` from the live interpreter, validates the running interpreter matches both `--version` and `stdlib_provenance.version`, and is deterministic. Python only for now.
  - Initial population: `python.yaml` ships 300 stdlib modules from 3.12; provenance bumped from claimed-but-unverified `"3.13"` to `"3.12"`. `math` is the inaugural worked-example closed-world entry. The closed-world surface starts narrow and expands per future hand audits.
...
- **Android XML.** `xml_config.py:_process_android_application` now emits `kind="component"` for AndroidManifest `<activity>` / `<service>` / `<receiver>` / `<provider>` (previously the bare element name, none in the canonical registry). Original element name lives on `meta["component_type"]`. `config-conventions.yaml`'s four `android_component` rules migrate to `symbol_kind: "^component$"` + `meta_match: {component_type: "^activity$"}` (etc.) via the new `Pattern.meta_match` field (see Changed below). Routing Android `<provider>` through `component`+`meta["component_type"]="provider"` keeps the canonical `provider` kind (Apex/Salesforce `[Provider]`) cleanly disjoint from Android content providers.
...
UAT 4.1.0 measured 90-95% gzip-reducibility on raw `hypergumbo run` JSON (airflow 320MB → 18MB, kafka 572MB → 34MB, chatwoot 81MB → 4.5MB, containerd 53MB → 3.7MB). Two new flags:
