...
- **Return type tracking (Python)**: Variable type inference now handles function return type annotations. When a function has `-> ClassName` and the result is assigned to a variable (`stub = get_client()`), subsequent method calls (`stub.send_request()`) resolve to the return type's class methods. Previously, only direct constructor calls (`stub = Client()`) enabled type inference. The return type class is resolved from the caller's file, the caller's imports, or the function's own module.
