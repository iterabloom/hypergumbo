...
Four threads run through this cycle.

**Receiver typing across the analyzer fleet.** A method call whose receiver the repository already describes now reaches the I/O catalogue, where it used to stop at ADR-0051's `external` sentinel. Ten languages learn receiver shapes they had the evidence for, and Python learns to follow a declared return type across modules and through two-hop chains.

**Taint precision, measured and then acted on.** ADR-0046 splits precision into correctness and usefulness, ADR-0048 makes the benchmark frame a CI gate, and ADR-0049 rules that a call which merely opens, registers or defers a network crossing is disclosed rather than minted as untrusted input. Useful precision moves **24.1% → 30.9%** across measurements 0003–0012, and the Python and JavaScript/TypeScript data-flow walks reach class methods at all.

**Two producers for one language (ADR-0057).** When two backends analyse the same code — tree-sitter and rust-analyzer for Rust, or the Python analyzer and the new opt-in scip-python backend — their records now merge into one node or edge, and each attribute records which producer said what. The `backend-agreement` command reports where the producers agree.

**Records that say what they know, and what they don't.** Every silent pass says why. Every linker declares how it activates, what it depends on and what each edge was derived from. `is_exported` can now say "not measured". `dead-code-maybe` stops calling 41.4% of this codebase dead, a figure that came from public Python methods never getting an exportedness verdict; the figure is now 7.5%. ADR-0047 makes the shipped catalogues stdlib-only, and ADR-0045 adds user and project configuration. ADR-0050, ADR-0051 and ADR-0058 give the I/O-boundary vocabulary, the module key and `Symbol.signature` declared axes. `SCHEMA_VERSION` advances 0.20.1 → 0.20.13, the `verify-claims --json` envelope 2.0 → 2.4, and `DEAD_CODE_MAYBE_SCHEMA_VERSION` 0.2.0 → 0.3.0.
...
- **A second producer for Python: scip-python (pyright), opt-in.** Install the new optional package with `pipx install 'hypergumbo[scip-python]'` and the producer with `npm install -g @sourcegraph/scip-python@0.6.6`. Turn it on with `--backend scip-python`, `HYPERGUMBO_SCIP_PYTHON=1`, or `[backends] scip_python = true` in either ADR-0045 config file. Pyright never runs the analysed code, so no trust grant is needed. On hypergumbo-core it types the receiver of 96.1% of the method calls the syntax analyzer left module-less. `--backend tree-sitter` turns every opt-in backend off.
- **`hypergumbo backend-agreement ARTIFACT` shows where two producers agree.** For each language with two producers it reports how many records paired, where the producers agree or disagree on each attribute, and how their edges overlap, reading only the artifact's own provenance. The markdown output is a `docs/audits/` document; the first, `docs/audits/0019`, is regenerated from a recorded rust-analyzer run.
- **A `[merge]` table in `config.toml` or `.hypergumbo.toml` chooses which producer's value wins**, for every attribute (`prefer = ["scip", "tree-sitter"]`) or for one (`[merge.prefer_by_attribute]`). An unknown key, backend or attribute is refused before any analysis runs. The built-in keeps the syntax analyzer first.
...
- **One declaration seen by two backends is one node and one edge.** ADR-0012 had the two backends' records sit side by side, which on a two-backend Rust run doubled every function and the in-crate call graph. A merge pass at the start of Phase C now pairs the records by span containment. For each attribute it keeps one value, chosen by a declared arbitration policy, and records the rest in two new fields: `attribution` (which producers hold the value) and `alternatives` (the values that lost). An artifact from one producer is byte-identical to before.
- **Every backend declares how it pairs.** `register_analyzer` takes a `merge=` declaration (`MergeAnchor` or `MergeDisjoint`) and `executes_analysed_code=`, and the merge pass refuses a language whose second producer has not declared. Cross-backend tests must run on recorded producer output, which `scripts/check-recorded-producer-input` enforces.
- **Agreement raises confidence; it is not a tie-break.** Two producers reaching one edge by different inference pathways make it `confidence_source="corroborated"` at a declared 0.95 (`CONFIDENCE_MODEL` v2.1), never `max` or noisy-OR.
- **A resolved call demotes an external stub at the same call site, but never deletes it (§14, §14.1, §15).** The stub's `rank_score` is halved and `meta.superseded_by` names what superseded it. An early version demoted 45 correct stubs, so the rule was narrowed:
  - Only an anchored producer may supersede, and only a stub that did not state its module.
  - A recovery linker demotes only the stub it actually consumed, and only once that permission is granted on measurement (so far `inherited-calls-linker`, per `docs/audits/0020`).
  - An external endpoint's identity is its `dst_ref`, so a module-less stub folds into its typed twin rather than standing beside it (§15).
- **The results cache is keyed on the backends that actually run.** A tree-sitter-only run and a two-backend run of the same tree could return each other's cached artifact.

#### Vocabulary and axes (ADR-0050, ADR-0051, ADR-0058, ADR-0059)

- **A catalogue row's `functions:` / `methods:` / `attributes:` section now has one meaning (ADR-0059).** It says how the primitive is reached from the row's own module: `method` if it is called on an instance, `function` if it is called on the module itself (a namespace, type, companion object or named global) or has no owner, and `attribute` if it is read. Until now, the catalogues' own notes gave three different rules, one of them "what the analyzer emits". Consumers now test the kind through three named predicates instead of string literals, a check fails on a new literal or an unregistered section key, and the 91 shipped rows that do not yet conform are listed in a shrink-only ledger, each with what blocks it. No row changed kind in this step.
- **The I/O-boundary vocabulary (ADR-0050) and the module key (ADR-0051) are declared axes.** The module key had carried a `free-text — consumers never branch on the value itself` justification while the catalogue matcher branched on it; the axis lint widens to match and surfaces 20 undeclared fields.
- **`Symbol.signature` gets the same treatment (ADR-0058).** It was declared display-only while nine consumers parsed it for return types, arity and field types. A gate now fails on any new parser outside a closed, cited list. Python has moved its return type to `Symbol.meta["return_type"]`, shrinking that list 9 → 8 with identical results over 33,740 functions.
- **`env_read` splits into `env_read` and `host_info_read`**, so the auto-derived `host_secret` taint label stops counting host and identity reads (134 of 195 rows) as secrets. **The shipped catalogues are stdlib-only and every one is gated** (ADR-0047 rulings 1 and 8), and **four `call_construct` values naming a different axis are drained** (ADR-0038 ruling 2).

#### What a linker declares: activation, dependencies, derivations

- **Activation is declared, not inherited.** 25 of the 61 linkers ran on every repository only because nobody had passed `activation=`. They now declare `always_on_unreviewed()`, which behaves the same and is honest that nobody decided. A test fails on any new registration without a declaration, and ADR-0055 refuses to tighten those gates without measurement.
- **`depends_on` names the passes whose output a linker reads.** Many clauses listed "languages where this concept exists" instead. Fourteen linkers are repaired from the producers they actually read (for example `database-query-linker` → `[["sql"]]`), and a linker that scans files itself declares `[]` with a stated reason. A new reader, `find_falsified_dependencies`, reports a pass that emitted edges despite an unsatisfied clause: 27.1% of 975 surveys had one before the repairs, and 0 after.
- **`derived_from` names what an edge was derived from, or the linker says why it cannot.** 86 of 93 linker edge sites had simply restated the edge's own endpoints. Each was read: 17 now name the records they consumed, and the rest declare `endpoints` or `consumed-none` in a checked comment. `[]` now means "consumed no graph record" (`SCHEMA_VERSION` 0.20.13). On this repository, linker edges carrying a real chain go 399 → 840. `explain --provenance` shows the three cases differently, instead of presenting a restatement of the endpoints as provenance.

#### Pass-silence reporting (ADR-0054, ADR-0056)

- **Every silent pass says why, in one field on one closed axis.** `limits.skipped_passes[].skip_reason_code` is renamed `silence_reason`, the name `AnalysisRun` already uses. This breaks readers of the old key, which existed for one unreleased version. `dependency_unavailable`, `backend_disabled` and `pass_crashed` move onto skipped passes, where a pass that never ran can carry them. `prerequisite_absent` and `no_candidate_construct` gain producers, and `candidates_unresolved` is new (ADR-0054). An unclassified skip reads `unreported`, never the common "no files" answer.
- **A zero is measured, not defaulted.** A linker's `files_analyzed` is now counted from the files it actually reads: eight linkers had read 14–834 files each and reported none. A pass is no longer skipped as "no files" when its own file enumeration would have found work.
- **Every survey prints three new summaries on stderr:** skipped passes by reason; pass silence across both places it is recorded (`pass_silence.census_silence()`); and any registered pass that reached neither. The third would have caught the rust-analyzer backend's output going unaccounted for a month.
...
- **The default view drops from 41.4% dead to 7.5% on this repository (2,468 candidates → 448), because public Python methods now get an exportedness verdict.** Python had decided `is_exported` for classes, functions and variables but never for methods, so the default production seeds never included them. A public method of an exported class is now exported, and a public method of a private class is not. Re-baseline anything that relied on the old numbers.
- **A name mentioned in a JSON, YAML, TOML, XML or HTML file no longer counts as cross-language dispatch.** Those mentions had hidden real candidates, including private helpers (+120 on this repository). Candidates still withheld as cross-language dispatch are listed in a new `cross_language_demoted` field, which is always present (`DEAD_CODE_MAYBE_SCHEMA_VERSION` 0.2.0 → 0.3.0, additive).

#### Schema

- **`SCHEMA_VERSION` 0.20.1 → 0.20.13, and every step but one is additive.**
  - 0.20.2–0.20.5: `Edge.meta` gains `call_arg_shape`, `callee_name` (the full-fidelity name ADR-0036 Ruling 1 says to read instead of the id's lossy slot), the bash per-call-site keys, and `io_target_kind`.
  - 0.20.6: `TaintFlowFinding` gains `walk_verdict` and `walk_blocked_by`.
  - 0.20.7–0.20.8: `AnalysisRun` gains `silence_reason`, and its axis gains `candidates_unresolved`.
  - 0.20.9–0.20.10: skipped passes gain `skip_reason_code`, then rename it to `silence_reason`. This rename is the one breaking step.
  - 0.20.11: `Symbol` and `Edge` gain the ADR-0057 `attribution` / `alternatives` fields, and `confidence_source` gains `corroborated`.
  - 0.20.12: `is_exported` becomes `true` / `false` / `null`.
  - 0.20.13: `derived_from` may be empty.
- **`is_exported` is `Optional[bool]`.** Around ninety analyzers have no exportedness rule, and every record they produced asserted "not exported". The field now holds a value only when a producer measured it; readers that test truthiness are unaffected. **`VERIFY_CLAIMS_SCHEMA_VERSION` advances 2.0 → 2.4.**
...
- **One new line of code withheld every self-claim verdict.** `datetime.date.today` and three `datetime.datetime` clock reads are classmethods but were catalogued as instance methods. So a call written `_dt.date.today()` made `datetime.date` look invisible to the analysis, and all 18 self-claims fell to `inconclusive`. The four rows are now function-kind, and a test checks every Python method row against the real class.
- **A classified call no longer marks its module as invisible, for Node's `process` and Python's `ctypes.cdll`.** `process.on`, `process.send` and `process.cwd` (which is called, not read), and `ctypes.cdll.LoadLibrary`, are called on a named owner, so they are now function-kind. Keyed as methods, they made the coverage gate report the module "method-shaped but no method call edge" on calls the classifier had already matched: 6 of 9 surveyed JavaScript repositories printed that false reason. Classification is unchanged, and the Python kind test now checks object owners as well as classes.
- **`ssl.SSLObject` is no longer network I/O.** It performs none (the caller feeds it memory buffers), so its three rows had minted false `untrusted_input` sources.
- **`ddg_mixed` is documented for what it is.** The scope document said it meant "the walk ran and did not confirm". In 86–88% of rows no walk had been attempted, so the definition now names its three cases and points to `walk_verdict`.
...
- **The Python data-flow walk's answer was being thrown away.** Each result was stored under a function key nothing ever looked up, so 0 of 24,915 methods could carry walk-backed evidence; now 85% can, and walks on this repository's own claims go 84 → 122. A precision A/B on two repositories with violated claims moved findings from `structural` to walk-backed labels and split one fused finding into two separately judged ones.
- **JavaScript and TypeScript class methods reach the data-flow graph at all** (dash.js 0 → 766 of 766 methods; apollo-server 0 → 314 of 355). Arrow functions and function expressions are still not walked.

#### Call graph accuracy

- **Swift and C# properties are back in the default map, with their calls.** The noise filter dropped every `property` symbol as "CSS structural", but no CSS analyzer emits that kind: its producers are swift computed properties and C# properties (bodies with calls), and `.properties` keys, QML and objective-c `@property` declarations (no body, no edges). The filter now drops only the last three. A call edge from a dropped property had gone with it: the default survey gains 278 call edges on Alamofire and 680 on Kingfisher, and Kingfisher loses 2 false dead-code candidates called only from property getters. C# properties return without edges, because its analyzer credits no call inside an accessor.
- **A scala or kotlin call follows the file's explicit import.** Both analyzers resolved a call by its simple name across the whole project, so `import scala.sys.process.Process` then `Process(cmd)` bound to an unrelated project `case class Process` (spark), and an imported JUnit `assertEquals` bound to a same-named test fixture (scala3, 1,180 sites). An import now outranks a same-named project symbol in another package. Kotlin binds the symbol at the imported path when the project has one (2,864 detekt and 2,555 okhttp calls moved to the imported class or function). Scala leaves the call unresolved with the import path in its module slot, where a launch now reaches its catalogue row. On 11 scala repos 5,631 resolved calls became unresolved; in a random sample of 30, all 30 had been false edges.
- **A kotlin, scala or swift call is now credited to the function that contains it.** Each analyzer found a call's enclosing function by its short name, which two methods `parse` in two classes, every companion `apply`, and every swift overload share. It credited whichever registered last. On 26 repositories that put 2,014 kotlin, 964 scala and 273 swift call edges on a function whose span does not contain the call. The enclosing function is now found by the position of its declaration, the rule elixir and erlang already use: 0 out of span on all three. Calls between overloads, and calls inside the right class, now resolve. Two recovered edges and one resolved edge that only existed because of the wrong anchor are gone.
- **The same holds for c, rust, ruby and c#.** C's `#ifdef`/`#else` alternatives, rust's `impl From<_> for T` blocks (one qualified name each), a ruby method defined twice and one c# method name in two classes each anchored calls to the wrong declaration: 35, 5, 3 and 3 call edges on 26 repositories, now 0. C's analyzer never received the per-file symbol list at all, which is why the name lookup could not be fixed from inside it.
- **A haskell function's span now covers all of its equations.** It covered only the first, so every call in a later equation fell outside the function it was correctly credited to: 119 call edges on xmonad and hadolint, now 0. A multi-equation function's id and `line_span` change with it.
- **Javascript/typescript and go calls are credited to the function that contains them.** For a function bound to a variable, the analyzer found the enclosing function through the variable's NAME. Two `var server` in one test file collided, and so did an IIFE bound to the name of the constructor it returns. It is now found by position: the variable's own symbol, else the outermost function on the path. Go resolves a second `func init()` in one file the same way. Javascript 135 -> 0, typescript 33 -> 0. As a side effect, 400 calls in koel's `.vue` `<script setup>` functions move from the file to the function that contains them.
- **A `.vue` or `.svelte` callback is credited with its own calls, as in `.js`.** The callback lookup ignored the `<script>` block's line offset, so it looked for the callback some lines above it. It credited whatever callback started there, or else the enclosing function or the file. On koel, chatwoot, podman-desktop, workadventure and gitlab: 9 calls move from a neighbouring callback into their own, 414 move from the file and 148 from the enclosing function to the callback that contains them, and no edge is added or lost.
- **Thirteen more analyzers credit a call to the function that contains it:** groovy, objective-c, perl, powershell, f#, glsl, hlsl, julia, nix, r, d, fish and gdscript. Each found a call's enclosing function by a name that overloads, julia's per-signature methods, a redefinition, one method name in two classes, or one local helper in two functions share. It now finds it by the declaration's position. Measured on 18 repositories, calls outside their caller's span go to 0: d 15,126 (dmd), objective-c 1,849, glsl 592, groovy 449, r 53 and perl 39. No call edge moved from inside its caller to outside it. The other 16 analyzers with such a lookup are pinned by the same fixtures and needed no change. A call in a nix file's own top-level function (`{ pkgs }: ...`) that sits outside any function-valued binding now has that function as its src; before, it had one only when a binding inside shared the file's name. postgrest: 13 -> 159 nix call edges.
- **Erlang and Elixir calls are attributed to the function that contains them.** Both analyzers looked up the enclosing function by a name several functions share, so a call could be attributed to a same-named function 1,900 lines away. On seven repositories, calls lying outside their caller go Erlang 1,306 → 0 and Elixir 7,166 → 0. An Elixir `def` head is no longer a call, and an Erlang `-ifdef` branch no longer loses its calls.
- **Python follows a declared return type from where it was declared.** A return type now resolves in the module that declares it, through quoted forward references, through `Cls.factory()` and `obj.build()` assignments, and across two-hop chains. `ts.workspace.add(...)` had bound to the wrong class by line proximity.
- **Method-call recovery stops binding by name coincidence.** It no longer overrides a producer that already named the callee's module: on this repository its edges go 92 → 64, all 28 removed ones wrong, and precision moves 51% → 73%. It also no longer binds a call to a member declared as a plain value, such as an `int`.
- **The GraphQL linkers emit edges.** Both had minted nodes and emitted nothing across the corpus: no producer supplied the schema fields they link to, and the client side matched operation names that schemas never declare. A new `graphql-sdl-linker` lifts schema types and fields out of `gql` template literals, and `graphql-linker` now joins a client operation to the schema field it selects. On apollo-server the two go 0 → 128 edges.
...
- **The rust-analyzer backend's entire output was unaccounted for.** Its records named an analysis run that was never written out, and the filed repro reported 1,308 validation violations; it now reports 2. A producer that returns output but no run is now recorded with a warning instead of vanishing.
- **Function-local `let` bindings are no longer nodes.** They were three quarters of the backend's nodes, and because locals are numbered per file, 21% of all its edges falsely crossed files. Locals are dropped, as every other backend already does (SCIP nodes 669 → 169).
- **A reference is attributed to the function that makes it, not to its file** (SCIP edges sourced from a method, 0 → 287 on aardvark-dns).
- **Symbol kinds come from the producer's own declaration.** Every free function had been a `method` and every enum variant a `class`; the two Rust backends now agree on kind for 133 of 148 paired records. A reference to a declared callable is `calls` rather than `references`. `parameter` and `type_parameter` are registered kinds, and a SCIP `Protocol` reaches `protocol`. When scip-python leaves the language or kind empty, the importer falls back to the file extension or the descriptor.
- **One `None` no longer hides four failures.** A missing binary, a crash, an exit with no index and an undecodable index now report `dependency_unavailable`, `pass_crashed` or `unreported` as appropriate, and a run that indexed nothing is recorded as a run.
- **Three shipped claims are retracted.** The two Rust backends never gave the same function the same `stable_id` (0 of 52), the test that guaranteed they did could not fail, and the spec cited an ADR ruling that does not exist.

#### Catalogue rows — direction, kind and reach

- **Erlang/Elixir constructors that build a table or a request no longer count as the write or the send.** `ets.new` creates an empty table, and the `Req.new` / `Finch.build` overlay rows build a request value. Each was rowed under the boundary its later executor crosses, so a flow was credited one call early, and with `ets.new` a single write was counted twice. The executor now carries the finding: `ets.insert`, `Req.request!`, `Finch.request`, and `Req.Request.run_request`, newly rowed because plausible reaches the network only through it. On six Erlang/Elixir repositories no verdict changed.
- **Python's `urllib.request.Request` no longer counts as the send; `urlopen` does.** Removing the row needed `urllib.request`'s I/O surface enumerated first. Otherwise every `Request(...)` became an unclassified call and withheld the clean verdict, as it did for all 18 of hypergumbo's own self-claims. The module's 46 public callables are now audited and it is declared complete. That rows proxy discovery (`getproxies`, `proxy_bypass`, `ProxyHandler()`, `build_opener()`) as environment reads, `localhost()`/`thishost()` as DNS receives, `ftpwrapper` (it logs in when constructed) as a send, and `urlcleanup` as a file write. The self-claims are unchanged. On beads, django, litellm and superset no verdict moved, and every function that lost evidence at `Request` gained it at `urlopen`. A proxy setting read with `getproxies()` and sent over the network is now reported.
- **A go call on a stdlib package variable reaches its catalogued boundary.** `http.DefaultClient.Do(req)` has no local variable to type. So the call either fell to the `external` slot, where no row could classify it, or resolved by its bare name to an unrelated in-repo method called `Do`. That second case was 79 false call edges on keda, loki and grafana, into `HTTPDoer.Do`, `Frontend.Do` and `httpClient.Do`. The library-signature catalogue gains a `package_variables` section typing `http.DefaultClient`, `http.DefaultTransport` and `net.DefaultResolver`, and all 79 now go to `net/http`, where `Client.Do` is the send.
- **Go's `http.NewRequest` no longer counts as the send; `Client.Do` does.** It builds a request. It was kept only because a request sent through `http.DefaultClient` never reached `Client.Do`, so deleting it lost the finding. On a fixture that sends a secret that way, the finding now stays `violated`, at `Client.Do` with confirmed evidence.
- **The most common scala process launch, `Process(cmd).!`, is now classified as `subprocess` and `fs_write`.** It is companion-apply sugar, and the analyzer emits it under the name `Process`, which no catalogue row carried. So on sbt 6 of 6 explicit-import launches classified as nothing, and a "launches no subprocess" claim found no chain. Both import styles now match: the explicit import and the `scala.sys.process._` / `.*` wildcard. On 11 scala repos this gained 22 classifications, lost none and changed none. tapir, playframework and pekko also stop reporting `scala.sys.process.Process` as unexamined.
...
- **JDK statics are catalogued as functions.** `java.nio.file.Files`, `System.getenv`/`getProperty`/`currentTimeMillis`/`nanoTime`, the `java.time` `now()` calls, `Clock.systemUTC` and Scala's `Properties` members are called on the class, so ADR-0059 makes them function-kind. Their names are also listed as needing module context, because in Java a bare call with no module is a project method. Classification is unchanged on 21 repositories. The untyped-receiver disclosure no longer names a JDK static as a possible callee of an instance call: on jedis, 6 of the 6 `env_read` sites and 90 of the 168 `fs_read` sites.
- **A Kotlin or Scala static call reaches its catalogue row.** `Files.readAllBytes(p)`, `System.getenv(k)`, `Instant.now()` and Scala's `Properties.envOrElse(...)` were emitted without their owner, explicit import or not, so every catalogued JDK static classified as nothing in both languages. The owner now comes from the file's import or from `java.lang`, which both languages import implicitly. A type the project defines keeps resolving to the project. Classified calls on four JVM repositories: sbt 123 → 237, gatling 31 → 57, detekt 17 → 30, http4s 3 → 6. One `verify-claims` finding is added: sbt's `host-secret-no-host-fs` gains a `System.getProperty` → `Files.deleteIfExists` flow and stays `violated`.
- **Swift's constructor rows are decided one by one, not just re-keyed.** Sixteen were keyed as methods and had never matched a call, which had hidden boundaries nothing ever exercised. Eight cross nothing and are removed: `URLRequest` and `HTTPClientRequest` build a request value, `ClientBootstrap` is a builder, `MultiThreadedEventLoopGroup` is a thread pool, `Logger(label:)` writes nothing, and the three `NIOSSL*` constructors build TLS material. `NWListener`, `ServerBootstrap` and `NIOWebSocketServerUpgrader` only arrange inbound data, so they are the disclosed `net_listen`, and `NSFetchRequest` composes a query, so it is `db_compose`. `NWConnection`, `NIOAsyncChannel` and `ModelContext` are handles and keep their boundary, and `CommandLine` is now the read of `CommandLine.arguments`. On six Swift repositories, vapor gains 3 classified calls and hummingbird 2; the other four and every `verify-claims` verdict are unchanged.
...
- **Every edge producer names how it inferred the edge.** `Edge.evidence_type` defaulted to its most specific value, so 51 producer sites that omitted it — including linkers that do no call analysis — claimed `ast_call_direct`. All 51 now name their pathway, and a lint fails on omission.
- **The analyzer registry speaks the taxonomy's language names.** `limits.skipped_languages` reported `makefile` as unanalysed on a repository where the `make` pass had run. Every registered language is now checked against the taxonomy when the analyzer registers.
...
- **A `backend-agreement --out` write goes through its safety-zone wrapper**, where a bare write had landed as an unsanitized host-filesystem flow. **A survey that raises after indexing no longer leaves its file index behind** for the next run in the same process.
...
- **`auto-pr` reports what actually happened.**
  - 41 of 481 recorded runs merged and then exited 1, because cleanup after the merge failed; a merge that landed now decides the outcome.
  - Ten `exit` paths that recorded no terminal state now record one.
  - An unexpected abort records the line it died on, and a killed run records the signal.
  - A failed manifest regeneration says which failure it was.
- **`smart-test` selects the right tests.**
  ...
  - A root test can declare what it covers with a `# covers:` marker, which empties the unreachable-test exemption list (25 → 0).
- **The self-claims drift gate runs unconditionally on the twice-daily cron.** Its path-filtered per-PR arm had let a regression ride green `dev` for 63 commits and had grown to about 14 minutes per PR.
- **A killed `auto-pr` can no longer leave tracker self-healing switched off.** The suppression marker is now a lock the OS releases; details are in the tracker changelog. **The def/use extractor registry is established per process**, so data-flow tests stop passing or failing depending on which other test ran first in the worker.
...
- **The committed backend-agreement table (audit 0019) no longer reports a `stable_id` gap that a survey does not have.** It read 14 struct, enum and trait records as missing a `stable_id`, and blamed the rust-analyzer translation. Neither producer drops them. The recorded two-arm harness skipped the step a survey runs before the merge, which fills a missing `stable_id` by kind, so tree-sitter's declarations arrived bare. The rust-analyzer and scip-python harnesses now run that step. The row reads 148 contested, matching a live survey of the same crate at the same rust-analyzer version.
- **The spec's account of the "method-shaped module" coverage rule is corrected.** It said a method-keyed catalogue row can be matched only by a method-construct call. A call whose module is named is matched by name, whatever the row's kind. The rule is a proxy for "the analyzer emitted this module's calls", and its docstring now says where the proxy is loose.
