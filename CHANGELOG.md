...
On the fixes side: mass `stable_id` collisions are resolved (60.2% of self-analysis Symbols shared an ID pre-fix; STABLE_ID_SCHEME v3 → v5), `verify-claims` gains an `inconclusive` verdict and stops silently confirming on blind analyses, bad inputs, or unconstrained claims, the gitleaks secret scan recovers from a silent no-op under gitleaks 8.30+, and cached embedding loads no longer touch the network.
...
#### Schema-vs-dataclass drift (SCHEMA_VERSION 0.13.0 → 0.14.0)

- **`docs/schema.json` $defs are now introspected from the dataclasses instead of hand-coded.** The generator claimed "auto-generated from Python dataclasses" but hand-coded the Symbol / Edge / Span / AnalysisRun `$defs` as literal dicts (its introspection helper was dead code), so dataclass changes could not propagate. New `scripts/generate_schema_lib.py` derives each $def's structure (property set, JSON types, nullability, required-ness) from `dataclasses.fields()` and merges curated per-field decorations (descriptions, x-annotations, enums, bounds); declared renames (`pass_id`→`"pass"`, `edge_type`→`"type"`) and composite folds (Symbol's supply-chain fields, Edge's evidence fields) carry the `to_dict()` wrinkles. Generation now hard-fails on overlay-vs-dataclass drift in both directions (stale decoration / undecorated new field), and a round-trip check pins each $def's property set to a fully-populated instance's `to_dict()` key set.
- **`Symbol.language` is nullable in the schema** (`oneOf [string, null]`, dropped from `required`) — whole-document validation no longer fails with 262 `language: None is not of type 'string'` errors on the ADR-0031 Class B synthetic stand-ins that by design carry `discovery_language` + `protocol_origin` instead. The stale `canonical_name` property (removed from the dataclass at 0.12.0/0.13.0, never from the hand-coded schema) is actually deleted; the four ADR-0031/0032 sibling fields and `AnalysisRun.failed_files` / `pass_version` (all emitted on every record) are now declared.
- **Schema-conformance fixture blindness closed.** The conformance test analyzed a single-file pure-Python fixture, so no protocol linker ever fired and no Class B node ever appeared in a CI-validated document — which is how the nullability drift shipped. A new end-to-end test analyzes a SQL + Python fixture that triggers the database-query linker and validates the whole document containing `language=None` nodes.
- **Class B stamping canary relocated into the spec validator.** The 262 validation errors were the de-facto signal that the synthetic stand-in population is under-stamped; tolerating `language=None` silences it. A new cross-field check emits one umbrella violation per missing identity field (`stable_id`, `fingerprint`, `discovery_language`, non-empty `origin`) across Class B stand-ins, so schema-validates-clean can no longer be misread as stamping-fixed.
- **Opaque top-level blocks typed; missing top-level keys declared** (SCHEMA_VERSION 0.14.0 → 0.14.1, additive). `limits` references a new introspected `Limits` $def (with `FailedFile` / `SupplyChainLimits` / `ClassificationFailure` / `AmbiguousPath` sub-defs); `features[]` items reference new `Feature` / `SliceQuery` $defs introspected from `SliceResult` / `SliceQuery`; `metrics` declares its seven producer keys; and three emitted-on-every-run top-level keys the schema never declared — `reproducibility_context`, `symbol_fingerprint_scheme`, `validation_report` (items: new introspected `ValidationViolation` $def) — are now present. Each non-dataclass block's property set is pinned to its actual producer (`compute_metrics`, `build_reproducibility_context`, `build_validation_report`) by contract tests.
- **`reproducibility_context.implications` no longer references an uncaptured field.** The text told consumers to match `pass_versions`, a key `captured` never carries; it now points at `analysis_runs[].pass_version`, where the per-pass code hashes actually live — so the documented reproducibility contract is followable.
