...
- **Java/C#/Kotlin main() entrypoint detection with qualified names (INV-lumiz)**: The `main-functions.yaml` patterns for Java, C#, and Kotlin now match qualified method names (e.g., `PageViewUntypedDemo.main`, `Program.Main`) in addition to bare names. Java and C# analyzers produce qualified names (`ClassName.methodName`) but the pattern used `^main$` which only matched exact strings. Changed to `(^|\.)main$` / `(^|\.)Main$`. Kafka: 0 → 153 Java main() entrypoints detected.
