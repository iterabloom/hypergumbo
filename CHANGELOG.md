...
- **Stated scope.** This makes the value reachable; it does not yet make it *reached*. No production body claims `no_candidate_construct` in this change, so the eleven `unreported` pass-runs are unchanged. Draining them is per-linker judgement work, because silence means different things in different bodies: a linker that emits a symbol for every candidate it finds is silent exactly when it found none, while a linker that only emits *pairing* edges is also silent when it found candidates on one side and none on the other. `crypto-flow` and `message-dispatch` both bail on `if not all_writes or not all_reads`, which conflates those two situations; stamping the whole branch `no_candidate_construct` would install a fresh false claim in the axis built to cure false claims.
