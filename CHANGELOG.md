...
- **THE PRECISION A/B THE ROW DEMANDED IS NOT SATISFIED, and that is stated rather than glossed.** The pre-registered refutation was "if ADR-0046 useful precision drops, the fix is a reporting regression even though the lookup is correct". It could not be tested: this repository's claims are all repaired and report zero violating flows by construction, prospector under the generic claims yields zero evidence, and `measure-taint-precision.py` requires per-flow human scoring. So the mechanism is fixed and measured; the effect on the published precision headline is **unmeasured**, and a finding leaving the structurally-identified class enters the useful-precision denominator. Filed as a residual.
