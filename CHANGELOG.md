...
- **Six constructs emitted no call edge at all**: Python's bare builtins (the permitting set had been the I/O catalogue itself), Go calls under a package-level `var` (every cobra `Run:` handler), Python calls on an external-typed instance field, Haskell zero-argument IO actions (505 sites), Erlang's `?LOG_*` macros (91% of rabbitmq's logging), and Rust grouped `use` lists (37.9% of the corpus's `use` statements).
