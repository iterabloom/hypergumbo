...
- **Exclude documentation/config nodes from output by default**: Non-code node kinds (markdown sections, TOML tables, INI settings, LaTeX labels, etc.) are now excluded from behavior map output by default. In FastAPI, this removes ~20K degree-0 documentation nodes (51% of output) that added no architectural insight — markdown headings from docs in 15+ languages, pyproject.toml config entries, etc. Use `--include-docs` to include them. This complements the existing tier-4 (derived/minified) exclusion.
