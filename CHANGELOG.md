...
- **Go struct field type extraction**: The Go analyzer now populates the `class_field_types` registry from struct definitions, mapping field names to their user-defined types (e.g., `RetryStage.integration → Integration`). Pointer types (`*Metrics`) are unwrapped, qualified types (`pkg.Cache`) use the unqualified name, and built-in types are excluded. This is the foundation for chained field access resolution (`r.integration.Notify()` → `Integration.Notify`).
