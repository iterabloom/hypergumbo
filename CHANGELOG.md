...
#### The CNF declaration set had never been read, and 29% of surveys falsify it (WI-jijor, ADR-0056 W1)
...
- **The first audit fails.** Over **893 surveys** on disk carrying `analysis_runs`, **261 (29.2%)** carry at least one falsified declaration — six distinct `(pass, clause)` pairs across five passes, led by `inherited-calls-linker` needing `['java','ruby','groovy']` in 227 of them. Hand-verified on logrus: `inheritance-linker` emitted **30 edges** while **none** of its twelve declared languages ran. ADR-0056's own re-decide criterion ("ten hits means declaration repair is the blocker") is met; the repair is filed separately rather than bundled here.
