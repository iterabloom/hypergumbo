...
- **`verify-claims` taint matcher now honors the module qualifier and `ambiguous_names`, ending a false-VIOLATION cascade.** `TaintCatalog.match_sink` / `match_source` indexed entries by bare callee name and, when no module hint disambiguated, blindly returned the *first* registered entry — so every `str.replace` / `dict.replace` call matched the filesystem-write `Path.replace` sink (**5,541 false `violated` rows across 4 claims on the project's own self-claims doc**), and a present-but-mismatched module hint fell through to the wrong entry, mis-routing `sys.stdout.write` into the `asyncio.StreamWriter.write` **net_send** sink (F156.A1). The matchers now mirror `io_boundary.IoBoundaryCatalog.lookup_with_module` (the sibling consumer of the same catalog that already did this correctly, INV-maluk): an exact qualified-name match wins; a module hint filters by `io_boundary._module_matches` and returns `None` when nothing matches that module; and an `ambiguous_names` short name (e.g. `replace` / `write` / `run`) with no module hint returns `None` instead of a false match. The catalog's per-language `ambiguous_names` (already loaded from `io_primitives/*.yaml`) is threaded onto `TaintCatalog`. Genuine matches are unaffected (`pathlib.Path.replace` with a `pathlib.Path` hint, `asyncio.StreamWriter.write` with an `asyncio` hint), and a real chain is never downgraded — coverage gaps only suppress false positives. Closes WI-razol (P0).
