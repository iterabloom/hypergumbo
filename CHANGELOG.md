...
- **The Java analyzer now emits an `imports` edge for every import declaration — the last imports-emission holdout among mainstream analyzers (INV-gojit).** Java extracted import declarations only into name-resolution dicts (`_extract_imports` and siblings build simple-name → FQN maps for call canonicalization) and never surfaced them as graph edges, so a Java file emitted `edge_types == [calls]` while every other mainstream analyzer (python/js/ts/go/rust/csharp/swift) emitted ≥1 `imports` edge — verified general, not stdlib-specific (zero edges on both `import java.util.List` and `import com.example.helper.Formatter`). The new `_extract_import_edges` emits one `imports` edge per declaration from the file node to a structured external ref, mirroring the Python emitter's `ExternalRef` shape (richer than Go/C#'s literal `package`/`namespace` dst slot, and aligned with ADR-0037's push to populate `dst_ref`): regular `import com.example.Foo;` → module `com.example`/name `Foo`; `import static a.b.C.member;` → module `a.b.C`/name `member` (registered `import_static` evidence type); wildcard `import a.b.*;` / `import static a.b.*;` → module `a.b`/name `*`. Confidence 0.95 at sibling parity; `src` is the `make_file_id` file node (backstopped by the WI-ramuv dangling-endpoint synthesizer). **Strips the last `(java, edge_imports)` strict-xfail hole** from the emission-parity matrix gate (now a hard lock) — the gap the gate itself surfaced when it landed.
