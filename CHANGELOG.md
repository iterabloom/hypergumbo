...
#### IO-boundary detection — attribute-style primitives

New `module_attr_ref` edge type emitted across six languages for attribute reads on imported modules.

- **Python attribute reads (WI-guhok)**: AST analyzer emits `module_attr_ref` edges for bare attribute reads on imported modules (skipping the `.func` of a call). `tag_io_boundaries` and `_TRACEABLE_EDGE_TYPES` accept the new type. Self-analysis `env_read` chain_count: 3 → 39 sites.
- **Tree-sitter helper `emit_module_attribute_refs` (WI-gapam)**: walks attribute-access nodes (`member_expression` / `field_access` / `selector_expression` / `field_expression`), resolves the base against an imports map, emits `module_attr_ref` edges. Exposed as a module-level function.
- **Per-language wire-up (WI-lozug)**: Go (`os.Stdout` / `Stderr` / `Stdin`), JS/TS (`process.env`, `window.*`, `document.*`, `navigator.*`, imported modules), Java (`System.out` / `err` / `in` plus imported-class field reads).
- **Scoped-path mode for left-recursive grammars (WI-vipur)**: `emit_module_attribute_refs` gains `scoped_path=True`; tree-sitter Rust now emits edges for `std::env::consts::OS` and similar `scoped_identifier` reads. Catalog matching gets a dot-normalized alias for `::`-containing names. The optional `rust-analyzer` backend is unaffected. C++ `qualified_identifier` tracked as **WI-zojid**.
- **C stdio globals via bare-identifier matching (WI-gotuv)**: `c.py` emits edges for non-shadowed bare references to `stdout` / `stderr` / `stdin`, activating the previously-inert `attributes:` entries under `stdio` in `c.yaml`. C++ `using namespace std;` + bare `cout` / `cerr` / `cin` tracked as **WI-zojid**.

#### verify-claims — project-local taint catalogs (WI-votan)
...
#### IO boundaries — `external_potential` bucket (Plan C, PR C)

- **`compute_boundary_map`** gains a second pass: for every edge whose `meta.io_boundary` is unset (the catalog did not classify it) and whose dst is a synthetic external-boundary node (`meta.external_boundary == True`), a synthesized `IoChain` is emitted into `bmap.entries["external_potential"]`. The chain carries `dst_tier` / `dst_tier_name` / `dst_external_boundary` (populated from the dst node's `supply_chain` block) plus a composed `primitive` of the form `module.name`. This is the structural answer to "the catalog can't enumerate every popular wrapper" — instead of adding entries for `huggingface_hub` / `requests` / `okhttp` / etc., the bucket surfaces every such call as a first-class signal and the user can reason about reach without per-library catalog growth. Validates the original WI-jihuj concern: `sentence_transformers.SentenceTransformer` and `huggingface_hub.snapshot_download` now show up in `external_potential` with `dst_tier=3`.
- **`IoChain.dst_classification_unreliable: bool = False`** — set on `external_potential` chains whose source language has `status: in_progress`. Emitted (not suppressed) so users see the data, but flagged so they know absence-of-catalog-hit is not authoritative for that language until its catalog is promoted to `status: complete`.
- **Filtering**: `external_potential` chains skip dst names that appear in the source language's `catalog.stdlib_other` set (PR B) — those are stdlib non-IO symbols, not catalog gaps. Languages with no loaded catalog (`is_supported=False`) get nothing in the bucket either; we don't speculate about languages we can't classify at all.
- **CLI rendering** — `_print_io_boundaries_by_type` now prints the `external_potential` section like any other boundary type (the existing loop already iterates `sorted(entries.keys())`); per-chain output adds a `[unreliable]` marker when `dst_classification_unreliable` is true. JSON output (`--json`) includes the new `external_potential` boundaries entry plus the new `dst_classification_unreliable` field on every chain.
- **Test coverage**: `tests/test_io_boundary.py::TestExternalPotentialBucket` (11 cases) covers the second pass's full filter matrix — emit / suppress / annotate / aggregation / serialization. `tests/test_cli_io_boundaries.py` adds two end-to-end CLI tests covering both the standard tier annotation and the `[unreliable]` marker on in_progress source languages.
- **Breaking-shape note**: `--json` output now includes a `boundaries.external_potential` key (and every chain dict gains a `dst_classification_unreliable` field). Downstream consumers that pin total-boundary counts will see them rise; the new bucket is additive — `--boundary` filtering still works as before for the original boundary types.

#### IO catalog — `status`, `stdlib_provenance`, `stdlib_other`, allowlist, validator (Plan C, PR B)

- **New top-level catalog fields**: `status: complete | in_progress` and `stdlib_provenance: {source_url, version, retrieved, notes?}`. `status: complete` is the default when both keys are absent. Catalogs may also declare a `stdlib_other:` section enumerating non-IO stdlib symbols (consumed by Plan C, PR C's external_potential filter — first-party calls into a stdlib non-IO symbol are NOT a catalog gap).
- **`ALLOWED_PROVENANCE_HOSTNAME_SUFFIXES`** module-level frozenset in `io_boundary.py` enumerates the hostnames a `stdlib_provenance.source_url` may suffix-match. Initial fill covers `python.org`, `golang.org`, `go.dev`, `pkg.go.dev`, `rust-lang.org`, `nodejs.org`, `oracle.com`, `openjdk.org`, `apple.com`, `developer.apple.com`, `swift.org`, `kotlinlang.org`, `scala-lang.org`, `elixir-lang.org`, `hexdocs.pm`, `erlang.org`, `haskell.org`, `cppreference.com`, `gnu.org`, `man7.org`, MDN, BSD man hosts, ISO, IETF, and a handful of other major-language doc hosts. Adding to the list is a governance change requiring PR justification (same shape as `ALLOWED_WEBSITES.md`).
- **`_validate_catalog_dict()`** runs at load time inside `IoBoundaryCatalog._from_dict` and raises `ValueError` when: `status` is anything other than `complete` / `in_progress`; `status: complete` is declared (or defaulted) without a `stdlib_provenance.source_url`; the URL is not `https://`; or its hostname does not suffix-match the allowlist. Failures surface at catalog load, not at edge matching.
- **Worked example — Python**: `io_primitives/python.yaml` declares `status: complete` with `stdlib_provenance` citing `https://docs.python.org/3.13/library/index.html` (Python 3.13, retrieved 2026-04-23, cross-checked against `sys.stdlib_module_names`). All 12 other catalogs declare `status: in_progress` with a comment pointing at the promotion criteria — completing them is the catalog-completion backlog (one follow-up tracker item per language).
- **`IoBoundaryCatalog.merge`** unions `stdlib_other` across child + parent so Kotlin (via `_CATALOG_PARENTS["kotlin"] = "java"`) inherits Java's enumerated stdlib non-IO symbols once Java's catalog is promoted; `status` and `stdlib_provenance` stay on the child since the completeness claim belongs to the language being loaded.
- **Test coverage**: `tests/test_io_boundary.py::TestCatalogStatus` (12 tests) covers the validator's full branch matrix plus the allowlist-content contract; `TestStdlibOther` (6 tests) covers parsing into the dedicated field, the "stdlib_other symbols are not matched as IO primitives" invariant, malformed-input fall-through, and parent merge.
...
#### IO catalog — strict-stdlib cull (Plan C, PR A)

- **All third-party wrappers removed from every `io_primitives/<lang>.yaml`.** The catalog principle is now enforced as a strict rule rather than honored-in-comment-only: catalog membership = stdlib (the language ships it), absence = "probably third-party, not certain." The previous grandfathered HTTP-client carve-out (`requests` / `httpx` / `aiohttp` / `axios` / `okhttp3` / etc.) was a slippery slope — once you allow popular wrappers, the maintenance treadmill is unbounded. The structural answer is the `external_potential` bucket landing in Plan C, PR C: tier-3 boundary calls surface as their own bucket without the catalog having to enumerate every popular wrapper.
- **Per-language entries removed**:
  - **Python (`python.yaml`)**: `requests`, `requests.Session`, `aiohttp.ClientSession`, `httpx.Client`, `httpx.AsyncClient`.
  - **Java (`java.yaml`)**: Apache Commons IO (`org.apache.commons.io.{FileUtils,IOUtils}`), Netty (`io.netty.*`), OkHttp (`okhttp3.{Call,OkHttpClient}`), Spring Web (`org.springframework.web.client.RestTemplate`, `org.springframework.web.reactive.function.client.WebClient`), Apache HttpClient 4/5 (`org.apache.http.client.HttpClient`, `org.apache.hc.client5.http.classic.HttpClient`, `org.apache.http.impl.client.CloseableHttpClient`), Unirest (`kong.unirest.Unirest`), Retrofit (`retrofit2.Call`), Spring Data / Hibernate (`org.springframework.{jdbc.core.JdbcTemplate,data.repository.CrudRepository,data.jpa.repository.JpaRepository}`, `org.hibernate.Session`), SLF4J (`org.slf4j.Logger`), Log4j 1.x / 2.x (`org.apache.log4j.Logger`, `org.apache.logging.log4j.Logger`), Logback (`ch.qos.logback.classic.Logger`). JDK + Jakarta EE (`java.*` / `javax.*` / `jakarta.*`) stay.
  - **JavaScript (`javascript.yaml`)**: npm HTTP clients (`axios`, `node-fetch`, `ky`, `superagent`, `got`, `undici`), Express (`express`, `express.Application`, `express.Response`), Fastify (`fastify.FastifyInstance`), Koa (`koa.Application`). Node built-ins (`http` / `https` / `fs` / `net` / `child_process` / `fetch`), Deno globals, and browser globals (`XMLHttpRequest` / `WebSocket` / `fetch` / `window` / `navigator` / `document` / `localStorage` / `sessionStorage` / `indexedDB` / `caches`) stay.
  - **Rust (`rust.yaml`)**: `tokio::fs`, `tokio::net::*`, `hyper::Client`, `hyper::client::Client`, `hyper::Server`, `axum::Router`, `actix_web::HttpServer`, `reqwest::Client`. `std::*` stays.
  - **Scala (`scala.yaml`)**: file rewritten — fs2, cats-effect, sttp, http4s, akka, pekko, Play (WS / Logger), Slick, Doobie, Quill, ScalikeJDBC, Anorm, ReactiveMongo, ZIO, scala-logging, SLF4J. `scala.*` stays (and `java.*` / `javax.*` / `jakarta.*` inherited via the Java parent catalog).
- **Test coverage** strengthened: `tests/test_io_boundary.py::TestLoadCatalog::test_<lang>_catalog_excludes_third_party_wrappers` for each of Python / Java / JavaScript / Rust / Scala asserts third-party absence. The Java and Scala tests are structural — they iterate every catalog primitive and assert its module starts with an allowed stdlib prefix — so future accidental third-party adds fail at PR-review time.
- **Breaking-shape note** for downstream consumers: `net_send` / `fs_read` / `fs_write` / `db_*` / `logging` chain counts will drop on any repo that uses the culled wrappers (most Python / Java / JavaScript / Rust / Scala codebases). The dropped chains will move to `external_potential` once Plan C, PR C lands. Bakeoff validation flagged with `awaits_bakeoff_validation`.
...
- **Taint sources/sinks auto-derive from `io_primitives/*.yaml`.** Default zone/label map: `fs_write`→`host_fs`, `net_send`→`network`, `subprocess`→`host_fs`, `env_write`→`host_env`, `ipc_send`→`ipc`, `browser_storage_write`→`browser_storage`; reads `env_read`→`host_secret`, `net_recv` / `ipc_recv`→`untrusted_input`. Hand-written YAML entries override on `(module, name, kind)` match. 419 previously-uncovered primitives now flow through `verify-claims`. The shipped `taint_sinks/host_filesystem.yaml` + `taint_sinks/network_send.yaml` are removed (they duplicated io_primitives), and WI-hizik's manual drift-guard is retired because drift is now structurally impossible.
- **New trust zones `host_env`, `ipc`, `browser_storage` and new taint labels `host_secret`, `untrusted_input`** so auto-import can classify env reads, IPC channels, and browser-local storage distinctly from host filesystem and network.
- **`TaintSource` / `TaintSink` `kind` field accepts `"attribute"`**; `module_attr_ref` added to `TAINT_CALL_EDGE_TYPES` so structural propagation walks the new edge type.
...
#### Framework pattern matching — symbol-meta matchers via NameMatcher (WI-pisab)

- **`Pattern._base_class_re` / `Pattern._parent_base_class_re` / `Pattern._annotation_re` / `Pattern._param_type_re`** now hold `NameMatcher` instances instead of raw `re.Pattern`. An anchored YAML pattern like `^BaseModel$` now matches a source-side spelling of `pydantic.BaseModel` (via `import pydantic` + `class Foo(pydantic.BaseModel)`) through NameMatcher's terminal-segment fallback. Same behavior extends to annotations (e.g., `^RestController$` matches `org.springframework.web.bind.annotation.RestController`) and parameter types (e.g., `^Depends$` matches `fastapi.Depends`). Previously these codebases missed concept enrichment that an otherwise-identical `from X import Y` codebase received — a silent false negative in framework detection that propagated to every downstream linker filtering by concept.
- **`_decorator_re` intentionally stays on raw `re.compile`.** The decorator YAML corpus uses dispatch-set triplets — celery.yaml pairs `^task$` (for bare `@task`) with `^(celery|app)\.task$` (for `@app.task`), jakarta-cdi.yaml pairs `^ApplicationScoped$` with `^jakarta\.enterprise\.context\.ApplicationScoped$`. Terminal-fallback would make the bare pattern also match the qualified source, firing two rules against one decorator and doubling concept enrichment. Annotation and parameter-type YAML corpora don't use dispatch-set triplets, so their retrofits land safely. A regression test (`test_decorator_bare_pattern_does_not_match_qualified_source`) guards against accidental re-retrofit.
- **Audited and found not vulnerable:** `linkers/di_resolution.py` sym_by_name lookup (line 641) — both sides use short names throughout (`rsplit(".", 1)[-1]` in binding extractors, `sym.name` in the lookup index); `io_boundary.IoBoundaryCatalog.lookup_with_module` — already canonicalizes via `_by_qualified` index with dot-normalized alias.
- **Matcher-boundary discipline lint** (`test_matcher_boundary_constructor_discipline`): AST-walks `Pattern.__post_init__` and asserts every `self._<field>_re` assignment uses the right constructor (`NameMatcher` for assertion-like symbol-meta matchers, `re.compile` for dispatch-set and structural-attribute matchers). Catches accidental reverts and new matcher fields that silently bypass canonicalization.
- **Polyglot call-site coverage fixture** (`packages/hypergumbo-lang-mainstream/tests/test_polyglot_call_site_coverage.py`): regression instrument for the WI-zigah invariant (every syntactic call site whose callable is an imported external symbol must emit a `calls` edge whose dst contains the qualified module path). Python fixture covers 8 import×call styles including the two WI-zigah fix cases and aliased / deep-nested variants; adding a new construct auto-fails CI when the analyzer misses it. Also the audit instrument for WI-mafik (Level 1b — cross-language analog bugs): add a language fixture once its analyzer handles dotted-submodule resolution.
