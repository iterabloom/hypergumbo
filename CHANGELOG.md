...
#### CUDA / Android XML / Solidity canonical-kind folds (WI-vibaz / WI-razus / WI-jujoj)

Three independent producer fixes against registry-absent `Symbol.kind` values surfaced by the ADR-0027 audit.
...
- **Solidity (WI-jujoj).** `SYMBOL_KINDS` gains `SymbolKindSpec("contract", AXIS_LANGUAGE_CONSTRUCT, ...)`. Path-1 verdict — smart-contract declarations are a source-language top-level construct (sibling to `class` / `interface` / `struct`), not a relabel. `solidity.py:265` already emitted `kind="contract"`; this PR registers the value canonically. `docs/audits/0003-symbol-kind-cluster-a-language-constructs.md` gains the verdict row.
...
`IoBoundaryCatalog` gains `stdlib_modules` (frozenset of authoritative names, populated from `sys.stdlib_module_names` for Python), `stdlib_prefixes` (tuple for hierarchical namespaces — Java's `java.*`, Go's `encoding/*`; prefix match accepts `.` and `/`), and `stdlib_module_completeness` (dict mapping module-name → audit-`retrieved:` date; modules listed here are treated as closed-world). `merge()` unions all three with the child winning on `completeness` collisions. YAML parser accepts both flat strings and dicts with inline `completeness: complete` + `retrieved:`. Hard-errors at load time when `completeness: complete` is declared without a `retrieved:` ISO date.

F3 Filter 2 in `_compute_external_potential`: after the existing `stdlib_other` check, if `module_hint` is non-empty AND `catalog.is_stdlib_module_complete(module_hint)`, skip the chain. Fires only on confident module identification (no skip when `module_hint == "external"`).

`scripts/refresh-stdlib-modules <language> --version X.Y` regenerates `stdlib_modules` from the live interpreter, validates the running interpreter matches both `--version` and `stdlib_provenance.version` before touching anything, and is deterministic. Only Python implemented. `python.yaml` populated with 300 stdlib modules from 3.12; provenance bumped from claimed-but-unverified `"3.13"` to `"3.12"`. `math` is the inaugural worked-example closed-world entry (no I/O primitives). Closes `WI-potuh-hukav-...-vutam` (F3 noise reduction); Filter 2 ships per plan, the closed-world surface starts narrow and expands per future hand audits.
...
- **F3 Filter 3 — composition fix for self-prefixed dst names.** When the dst node's `name` field already carries the module-qualified form (e.g. `re.MULTILINE`) and the extracted `module_hint` is the same module (`re`), the naive prepend produced `re.re.MULTILINE`. Filter 3 detects `dst_name.startswith(module_hint + ".")` and skips the prepend; same for `ast.ast.Name`, `os.os.path`, `datetime.datetime.now`. Cuts ~30 chains. Lands `WI-potuh-hukav-...-vutam` partially; Filter 2 follows in PR-C (above).
