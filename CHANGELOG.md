...
- **`hypergumbo[rust-analyzer]` install extra + `hypergumbo-lang-rust-analyzer` published to PyPI.** v4.1.0 shipped without the SCIP integration package and without declaring it as an optional dependency, so `--backend rust-analyzer` had no way to engage end-to-end. This release adds the `[rust-analyzer]` extra (pinned in lockstep with the meta-package version) and adds `hypergumbo-lang-rust-analyzer` to the release-workflow build loop so the next tag uploads it to PyPI alongside the other six packages. After this release, `pipx install 'hypergumbo[rust-analyzer]'` actually engages the SCIP backend. As a defensive backstop for minimal installs, `--backend rust-analyzer` and `install-rust-analyzer` now also exit non-zero with a clear message when the integration package is missing instead of silently falling through to the default tree-sitter analyzer; `install-rust-analyzer --check` reports binary and integration-package status as separate lines and exits 1 if either is missing.
