...
- **D UFCS receiver gating + linker recovery (INV-vigaf, WI-situj).** D's analyzer misbound an unresolvable-receiver call `thing.exists()` to an arbitrary same-named free function `exists()` at 0.85. It now tracks function-parameter types and, when the receiver is a known parameter, emits the call unresolved with a `receiver_type_hint` (typed) or none (untyped/builtin) — withholding the misbind. Free functions are stamped with `meta.ufcs_receiver_type` (their first-parameter type), so the `receiver_type_dispatch` linker recovers the real UFCS target (`x.foo()` ≡ `foo(x)`) as an `ast_call_ufcs` edge. Local-variable and `auto` receivers remain a documented residual (parameter receivers are the dominant funnel).
...
- **Scala: an unresolvable-receiver call no longer misbinds to an arbitrary same-named internal def (WI-bihit).** A method call `x.copy()` / `x.setTo()` whose receiver type couldn't be resolved previously fell through to a bare short-name lookup and confidently bound to the first same-named method/function (the `copy`/`setTo` @0.68 funnel — a false high-confidence `calls` edge). The Scala analyzer now gates these: when a receiver is present but its type is unresolvable, it emits an honest unresolved external `calls` edge (`is_resolved=False`, `evidence_type="ast_call"` → confidence 0.40, `meta.call_construct="method"`) instead of the bare-name bind — mirroring Python's receiver-gating and conforming to **INV-nilud** (the analyzer suppresses; the shared `inherited_calls` linker recovers, Site-2 Step-1). Recall is preserved by threading previously-dropped receiver types into resolution — annotation-typed vals (`val x: Foo = f()`) and class-constructor parameters (`class C(val svc: Service)`) — and by stamping `receiver_type_hint` on the unresolved edge for the linker. D (`exists`/`startsWith`/`toString` @0.85) carries the same funnel and is tracked separately (WI-situj); the INV-fahub umbrella stays open pending a cross-language validation bakeoff.
- **Swift: the same receiver-gating (WI-votar).** A method call `x.create()` / `x.delete()` / `x.run()` whose receiver type couldn't be resolved (@0.80 funnel) now emits the identical honest unresolved edge instead of binding to an arbitrary top-level function or method. Recall recovery here threads **function/method parameter types** (previously dropped entirely — a `func handle(client: Client)` receiver `client` was untracked), so a param-typed receiver resolves via the type-qualified path; property-declaration types (`let x: T`) already resolved. Leaf-only, no CORE/linker change.
