...
- **Containment linker**: New structural linker that creates `contains` edges from class/interface symbols to their method/getter/setter symbols. Works across all 15 supported languages using naming conventions (dot `.`, Ruby `#`, Rust `::`). Also handles nested classes (`Outer.Inner`), and struct/trait/enum containers (Rust, Go, C). Previously, methods were orphaned in the graph — disconnected from their parent classes — inflating orphan rates and hiding class structure from slice traversal.
- **D method extraction with qualified names**: Functions inside D struct/class/interface bodies are now extracted as `kind="method"` with qualified names (e.g., `Searcher.search`) instead of as standalone functions. This enables the containment linker to create `contains` edges from D containers to their methods. DMD bakeoff previously showed only 91 contains edges for 76K nodes because all 55K functions were extracted without parent context.
- **Non-code node exclusion**: Documentation/config nodes (markdown sections, TOML tables, INI settings), CSS structural nodes (class_selector, id_selector, rule_set, property, media, keyframes, font_face), and config-metadata nodes (.gitignore patterns, npm scripts, pip requirements) are now excluded from behavior map output by default. Spring Petclinic had 94.6% CSS noise (5,457/5,771 nodes with zero edges); FastAPI had 51% documentation noise (26K → 6.4K nodes). Use `--include-docs` to include them.
- **Python library-export detection**: Public functions and classes in `__init__.py` files are now detected as `LIBRARY_EXPORT` entrypoints. Previously, Python library repos (e.g., FastAPI, Flask, Django) had no library-export detection — auto-slicing would pick `cli.py:main` instead of the core API classes. New `symbol_path` pattern field enables file-path-based matching in YAML patterns.
...
- **Python re-export library-export detection**: Symbols re-exported from `__init__.py` (e.g., `from .routing import APIRouter as APIRouter` in FastAPI) are now detected as `LIBRARY_EXPORT` entrypoints. Previously, re-exported symbols kept their original file path and failed the `symbol_path: "__init__.py"` pattern match. The Python analyzer now adds a `re_exported` modifier to these symbols, and new pattern variants match on it. This fixes FastAPI (and similar libraries) having only 6 nodes per slice because entrypoint detection fell back to `cli.py:main` and tutorial files instead of core API classes.
- **Pattern `modifiers` positive filter**: YAML framework patterns now support `modifiers` to require at least one modifier to match (e.g., `modifiers: "^re_exported$"`). Complements the existing `modifiers_exclude` negative filter.
...
- **Self-loop edge filtering**: Self-loop edges (src == dst) are now filtered from behavior map output, adjacency lists, and connectivity scoring. DMD bakeoff found 28% of 64k view edges were visitor self-loops that wasted token budget and inflated centrality without adding useful connectivity. Filtering occurs at three levels: CLI pipeline (prevents entry), adjacency lists (prevents centrality inflation), and compact/tiered output (defense-in-depth).
- **Django route method accuracy**: Django `path()`/`re_path()`/`url()` patterns no longer hardcode `GET` as the HTTP method. Django's URL routing doesn't specify methods — views handle all methods. Routes now display without a method label instead of the misleading `[GET]`.
- **TypeScript decorator kind filtering**: Decorator resolution in JS/TS now rejects class, interface, and type symbols as decorator targets. Previously, `@Post()` could resolve to a `Post` data class instead of remaining unresolved when no function named `Post` exists. NestJS bakeoff showed `@Post('/cats')` resolving to `graphql.schema.ts:Post` (a data class) with false confidence. Decorators that match only non-function symbols now correctly produce unresolved edges (confidence 0.50).
- **Tiered view token budget enforcement**: `format_tiered_behavior_map` now enforces the target token budget on the full assembled output (nodes + edges + entrypoints), not just nodes. Previously, edges and entrypoints were added after node selection without any budget check. DMD bakeoff showed the 16k view at 54K tokens (3.4x over) and 64k view at 176K tokens (2.7x over) because 130K edges induced a massive edge set. Now, a post-selection shrink loop removes lowest-centrality nodes until the total output fits within budget.
- **Tiered view connectivity-aware selection**: `format_tiered_behavior_map` now uses `select_by_connectivity` instead of `select_by_tokens` for node selection. Previously, the centrality-only approach selected high-centrality nodes independently, producing disconnected output: all three DEEP bakeoff repos had 0 edges in the 4k view (7-12 nodes with no adjacency). Connectivity-aware selection starts from entrypoints and expands via the frontier, ensuring selected nodes share edges. Combined with the budget enforcement shrink loop, this produces compact connected subgraphs that fit within the token budget.
- **Tiered view confidence-filtered force-includes**: `format_tiered_behavior_map` now filters force-included entrypoints by confidence (>= 0.5) and caps them to half the estimated node capacity. Previously, ALL entrypoints were force-included regardless of confidence, so repos with many test main() functions (confidence ~0.4 after test penalty) had their tiered views filled with test entrypoints, crowding out bridge nodes that provide edges. DMD bakeoff showed 226 nodes but 0 edges in the 64k view because 1790 test main() functions consumed the entire budget. Low-confidence entrypoints now compete on centrality with bridge nodes in the regular fill phase.
- **Symbol ranking by individual degree**: The `hypergumbo symbols` command now sorts by individual symbol degree (descending) rather than file-total-degree. Previously, symbols were grouped by file — all symbols from the "hottest" file appeared first, burying individually high-degree symbols in other files. In Django, this caused QuerySet (degree 118, the highest) to appear at line 753 while expressions.py internals (Func, degree 92) dominated the top. In NestJS, FastifyAdapter methods occupied the top 66 positions while NestFactoryStatic (the bootstrap entry point) was at rank ~1400.
...
- **Deeper bakeoff slices**: Forward and reverse slices in `bakeoff-features run` now use `--max-hops 5` (vs default 3) to capture more of the dependency graph for quality assessment. Slice coverage thresholds adjusted accordingly (`good_min` 5% → 1%, `warn_min` 1% → 0.1%) since large repos (10K+ nodes) naturally have low per-slice coverage percentages.
- **Route test filtering**: `hypergumbo routes` now excludes routes from test files by default. Django bakeoff showed 73% of detected routes came from test URL configurations (260 of 301 routes), drowning production routes in test noise. Routes from files classified as test paths by `is_test_file()` are now filtered out. Use `--include-tests` to include them.
- **Exclude derived/minified files from output by default**: DERIVED-tier (tier 4) symbols — minified bundles, generated code, build artifacts — are now excluded from behavior map output by default. Previously, minified JS symbols (e.g., single-char function `t` with 6294 in-degree in Metabase's `iframeResizer.js`) distorted centrality rankings and inflated edge counts via false-positive name collisions. Use `--max-tier 4` to include derived files explicitly.
- **Increase default `--max-files` for slice from 20 to 50**: The previous default of 20 files was too restrictive for large codebases. In Django (46K nodes), a TemplateView forward slice hit `file_limit` at 10 hops, capping the result at 187 nodes vs 208 reachable nodes. The new default of 50 allows slices to explore the natural graph structure without artificial capping. Bakeoff-features uses `--max-files 100` and `--max-hops 10` for deeper quality assessment. Added `avg_slice_nodes` metric and `limit_hit_frequency` computation to bakeoff diagnostics.
### Fixed

- **`--entry auto` now respects `--exclude-tests` and `--max-tier`**: Previously, `slice --entry auto --exclude-tests` still selected test file entrypoints because the filters were only applied to `--list-entries`, not to `--entry auto` selection. DMD bakeoff showed `--entry auto --max-tier 1` picking `compiler/test/paranoia.d:main` (a floating-point stress test) instead of the actual compiler entry point, because the test file had more outgoing edges. Now both filters apply to auto-entry selection.
- **Tiered view token budget compliance**: The tiered behavior map was exceeding its token budget by up to 177x (708K tokens for a 4K budget on FastAPI). Force-included entrypoints are now sorted by centrality and capped to fit the budget. Non-essential fields (`analysis_runs`, `usage_contexts`, `sketch_precomputed`) are stripped from tiered output. Token budget now reserves space for non-node fields (entrypoints, summary) before node selection.
- **Supply chain tier deserialization**: Nodes deserialized from cached JSON always had tier=1 regardless of actual tier. The internal `_node_from_dict()` read a flat `supply_chain_tier` key but `to_dict()` serializes it as nested `supply_chain.tier`. Replaced with `Symbol.from_dict()` which correctly reads the nested format. This fixes tier-based filtering in the slice command when loading cached behavior maps.
- **CI pip cache timeout on local runner**: Disabled `cache: 'pip'` in CI workflow setup-python steps. Local runners couldn't reach the cache server (ETIMEDOUT), adding ~8 minutes of timeouts per job.
- **Route-handler linking for same-name symbols**: Fixed a bug where route-handler linking failed when the route symbol and handler function shared the same name. In Go (always), JS/TS Express (when using named handlers), and Python/Django (when route name includes view name), the route symbol would overwrite the handler function in the name lookup dict. The resolver then checked `is_handler()` on the route symbol, got `False`, and returned no match. Fix: the lookup dict now preserves function/method/class symbols when a route symbol with the same name is encountered. Validated on gin-realworld: linking rate went from 0/20 (0%) to 20/20 (100%).
...
- **WebSocket generic event edge explosion**: FastAPI `websocket.send_text()`/`receive_text()` and native JS `ws.send()` are raw WebSocket protocol operations without named events. Assigning them the synthetic event "message" caused NxM combinatorial edge creation — in FastAPI, 19 sender files x ~45 receiver files = 852 spurious `websocket_message` edges. Now skips inter-file edges when both sender and receiver are generic protocol patterns (fastapi/native with event "message"). Named event patterns (Socket.io emit/on, Django Channels) are unaffected.
