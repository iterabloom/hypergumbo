...
- **Fix extends edge name collision (Python, JS/TS, Ruby, Kotlin)**: When multiple classes share the same name (e.g., Django has 238 classes named `Model`), `extends`/`implements` edges now resolve to the correct base class using import-aware disambiguation instead of last-writer-wins. Priority: (1) same-file class, (2) import-path match (Python: `from X import Y`; JS/TS: `import { Y } from './X'`; Ruby: `require_relative 'path'`; Kotlin: `import com.example.Y`), (3) deterministic fallback (sorted by ID). Previously, ALL 2376 Django `extends Model` edges pointed to a single test stub in `test_relative_fields.py`; now they resolve to `django.db.models.base.Model`.
