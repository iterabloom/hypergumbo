...
- **C analyzer now emits `module_attr_ref` edges for non-shadowed bare references to `stdout` / `stderr` / `stdin`.** Activates the `attributes:` entries at `io_primitives/c.yaml:85` (`module: stdio, attributes: [stdout, stderr]` under `ipc_send`) and `:92` (`attributes: [stdin]` under `ipc_recv`), which were inert before this PR — the WI-gapam `emit_module_attribute_refs` helper only matches member-expression / scoped-identifier shapes, so C's bare extern `FILE *` identifiers (declared in `<stdio.h>`) never produced matching edges. New `_emit_stdio_identifier_refs` in `c.py` walks the tree twice: a pre-pass collects locally-declared shadow names per enclosing function (and at file scope) by walking down `declaration` / `parameter_declaration` nodes' `declarator` field to the terminal identifier; the main pass emits one `module_attr_ref` edge per non-shadowed `identifier` node whose text is in `{stdout, stderr, stdin}`. Edge dst follows the WI-gapam convention (`c:stdio:0-0:stdio.<name>:attribute`), confidence 0.85, evidence_type `module_identifier_reference`. Caller src is the file-level pseudo-symbol, matching what Go / JS/TS / Java did in WI-lozug. The C++ analog (`using namespace std;` then bare `cout` / `cerr` / `cin`) is the same problem with a different catalog shape and is tracked separately as **WI-zojid** (needs a `cpp.yaml` first). No `io_boundary` changes were needed — `tag_io_boundaries` already handles `module_attr_ref` edges.

#### Analysis quality (WI-vipur: cross-language scoped-path mode)

- **`emit_module_attribute_refs` gains a `scoped_path=True` mode and the tree-sitter Rust analyzer now emits `module_attr_ref` edges for `std::env::consts::OS` and similar scoped attribute reads.** Rust's `scoped_identifier` (and C++'s `qualified_identifier`) parses `std::env::consts::OS` as a left-recursive chain of `path` + `name` children, not the binary `object` + `property` pair the helper's default walk supports. The new mode walks left via the configured `object_field_names` (`("path",)` for Rust) until it bottoms out at the leftmost identifier, which is checked against the imports map. The leftmost segment is rewritten with the imports map's value (so `use std::env;` followed by `env::consts::OS` correctly emits as `std::env::consts`), and the resulting module path is dot-normalized (`::` → `.`) in the emitted edge ID to avoid colliding with the `:`-delimited edge ID format. To keep io_boundary catalog matching working, `IoBoundaryCatalog._rebuild_indices` now registers a dot-normalized alias for any qualified name whose module contains `::`. The wire-up site in `_extract_edges_from_file` injects `std` as an implicit import (Rust stdlib is in scope without `use`). Pairs with the `attributes: [consts]` entry at `io_primitives/rust.yaml:127` (under `env_read`), which was inert on the tree-sitter Rust path until now. The `rust-analyzer` optional backend (`hypergumbo-lang-rust-analyzer`) has its own semantic resolver and is unaffected. The WI-lozug PR 4 deferral stub is flipped from "asserts zero edges" to a `TestRustModuleAttrRefsScoped` class with positive-emission tests including io_boundary tagging confirmation. C++ wire-up is deferred — `cpp.yaml` does not yet have `attributes:` entries for `std::cout` / `std::cerr` / `std::cin`; once those are added the same scoped-path mode applies with `node_kinds=("qualified_identifier",)`.

#### Analysis quality (WI-lozug PR 4: Rust deferral stub)

- **Regression-guard test asserting the tree-sitter Rust analyzer emits zero `module_attr_ref` edges for `std::env::consts::OS` today.** `rust.yaml:127` declares `attributes: [consts]` under `env_read`, but the WI-gapam `emit_module_attribute_refs` helper cannot match tree-sitter-rust's `scoped_identifier` nodes (left-recursive `path` + `name`, not the `object` + `property` pair the helper's default walk uses). Adding a scoped-path traversal mode is a cross-language follow-up — it also benefits C++ `qualified_identifier` (`std::cout` / `std::cerr` / `std::cin`), Groovy, and Kotlin. The stub test will flip to a positive-emission test when WI-vipur lands. The `rust-analyzer` optional backend (`hypergumbo-lang-rust-analyzer`) is untouched and continues to provide semantic Rust resolution for users who have it installed. Fourth and final WI-lozug wire-up; the ticket also spawns one other follow-up: WI-gotuv for C identifier-reference matching (bare `stdout` / `stderr` / `stdin` globals in C — the helper cannot match those either because they are not member-expressions).

#### Analysis quality (WI-lozug PR 3: Java)

- **Java analyzer now emits `module_attr_ref` edges for `System.out` / `System.err` / `System.in` and for attribute reads on imported classes.** Wires the WI-gapam `emit_module_attribute_refs` helper into the per-file Pass-2 loop in `_analyze_java_impl`. `field_access` nodes whose `object` resolves to any import in the file's imports dict or to `System` (an implicit java.lang import, injected unconditionally) now produce `module_attr_ref` edges with confidence 0.85. Pairs with the `attributes:` entries at `io_primitives/java.yaml:171` (`System: [out, err]` under `ipc_send`) and `:175` (`System: [in]` under `ipc_recv`), which were inert before this PR. **Design deviation from Go/JS:** the callee-skip logic is disabled by passing node kinds that do not occur in Java tree-sitter — the default skip would drop `System.out` whenever it is the receiver of a `method_invocation` (the dominant usage pattern `System.out.println(...)`), and since `io_boundary` tags callee names against the catalog, the corresponding `calls` edge for `println` never matches the `attributes: [out]` entry. Without this deviation the `System.out` primitive would never fire. A `calls` edge for `println` and a `module_attr_ref` edge for `System.out` target different catalog lookups, so no double-count occurs downstream. Third of four WI-lozug wire-ups (Go PR 1 and JS/TS PR 2 already merged).

#### Analysis quality (WI-lozug PR 2: JS/TS)

- **JS/TS analyzer now emits `module_attr_ref` edges for attribute reads on imported modules and Node/browser globals.** Wires the WI-gapam `emit_module_attribute_refs` helper into the per-file Pass-2 loop in `_analyze_javascript_impl`. `member_expression` nodes whose base resolves to a known import (namespace or named) or to one of the injected globals `process` / `window` / `document` / `navigator` now produce `module_attr_ref` edges with confidence 0.85, provided the node is not itself the callee of a call. Pairs with the `attributes:` entries at `io_primitives/javascript.yaml:199` (`process: [env, argv, platform, arch, version, cwd]`), `:203-211` (navigator/window/document APIs), `:215` (`process: [stdout, stderr]`), and `:222` (`process: [stdin]`), which were inert on JS/TS before this PR. Node globals are injected unconditionally — even in browser-side files — because the downstream YAML match filters noise. The edge src prefers the Pass-1 module symbol (`<module:filename>`) when available and falls back to a synthetic file-id symbol otherwise. Second of four WI-lozug wire-ups (after Go PR 1).

#### Analysis quality (WI-lozug PR 1: Go)

- **Go analyzer now emits `module_attr_ref` edges for imported-package attribute reads (`os.Stdout`, `os.Stderr`, `os.Stdin`).** Wires the WI-gapam `emit_module_attribute_refs` helper into `_extract_edges_from_file` in `go.py`. Attribute accesses on imported packages (`selector_expression` nodes whose `operand` is an import alias) now produce `module_attr_ref` edges with confidence 0.85 and `evidence_type="module_attribute_reference"`, provided the node is not itself the callee of a call (those cases already emit `calls` edges). Pairs with the `attributes:` entries at `io_primitives/go.yaml:161` (`os: [Stdout, Stderr]` under `ipc_send`) and `:179` (`os: [Stdin]` under `ipc_recv`), which were inert before this PR. The edge src is a file-level pseudo-symbol (matching Go's existing `imports`-edge convention, where per-function caller context is not plumbed through to the io-boundary pipeline's attribute matching). Blank and dot imports (`_` and `.`) are excluded from the imports map for this pass. First of four WI-lozug wire-ups; the other three (JS/TS, Java, Rust deferral stub) follow.

#### Analysis quality (WI-gapam)

- **Cross-language helper `emit_module_attribute_refs` in `analyze/base.py`.** The tree-sitter counterpart of the per-language `_emit_module_attr_refs` closure that ships inside `py.py` (WI-guhok). Walks a tree-sitter root, finds attribute-access nodes (`member_expression` / `field_access` / `selector_expression` / `field_expression` depending on the grammar), resolves each node's base against an imports map, skips nodes that are the callee of a call (those already produce `calls` edges), and emits `module_attr_ref` edges for the rest. Pairs with `attributes:` entries in `io_primitives/*.yaml` that were otherwise dead metadata across every tree-sitter-based language — without this emission, `io-boundaries` silently under-reports `env_read` / `ipc_send` / `ipc_recv` chains on attribute primitives (`process.env` in JS, `System.out` in Java, `os.Stdout` in Go, `stdout`/`stderr`/`stdin` in C, etc.). The helper is exposed as a module-level function because the JS/TS analyzer (the biggest consumer) overrides `analyze()` entirely and does not use the `TreeSitterAnalyzer` template methods. Per-language wire-up into the actual analyzers is tracked as WI-lozug (scope-expansion follow-up).
...
- **Taint catalog now auto-imports from `io_primitives/*.yaml`.** Previously every entry in `taint_sinks/host_filesystem.yaml` or `taint_sinks/network_send.yaml` had to be manually mirrored from io_primitives; the shipped first-party catalog covered ~9 Python entries against io_primitives's 37 fs_write + 66 net_send primitives, and WI-hizik's drift-guard baseline grandfathered 419 missing entries. `load_builtin_taint_catalog()` now reads `io_primitives/` alongside the hand-written YAML files and auto-derives a `TaintSource`/`TaintSink` record for each primitive, using a default zone/label mapping (`fs_write`→`host_fs`, `net_send`→`network`, `subprocess`→`host_fs`, `env_write`→`host_env`, `ipc_send`→`ipc`, `browser_storage_write`→`browser_storage`; read side `env_read`→`host_secret`, `net_recv`→`untrusted_input`, `ipc_recv`→`untrusted_input`). Hand-written YAML entries override auto-derived defaults when they match `(module, name, kind)`. The net effect: 419 primitives that previously had no taint coverage now flow through `verify-claims`, and the manual drift-guard (WI-hizik's `test_taint_sink_drift.py` + `_taint_sink_drift_baseline.yaml`) is retired because drift is now structurally impossible. The shipped `taint_sinks/host_filesystem.yaml` and `taint_sinks/network_send.yaml` files are removed — their per-language primitive lists duplicated io_primitives, and their zone/trust_level declarations now live in `taint.py`'s `AUTO_SINK_ZONE_MAP`.
- **New trust zones `host_env`, `ipc`, `browser_storage` and new taint labels `host_secret`, `untrusted_input`.** Needed so auto-import can classify env reads, IPC channels, and browser-local storage distinctly from host filesystem and network.
- **New io_primitives category `browser_storage_write` (WI-lokuv Q3).** `localStorage.setItem` / `sessionStorage.setItem` / `.clear` / `.removeItem` moved out of `javascript.yaml#fs_write` into the new category. The behavior map no longer misreports browser-local writes as host-filesystem writes, and auto-import routes them to the new `browser_storage` trust zone. Read-side browser storage (`localStorage.getItem`, `document.cookie`) is tracked for a follow-up.
- **New io_primitives category `browser_storage_read` (WI-kanir-huzuj).** `localStorage.getItem` / `sessionStorage.getItem` / `indexedDB.open` / `caches.{open,match,has,keys}` moved out of `javascript.yaml#fs_read` into the new category. Read-side counterpart to the WI-lokuv Q3 split — browser-local reads live in the browser's per-origin store, reachable via XSS rather than host-filesystem access. Intentionally NOT in `AUTO_SOURCE_LABEL_MAP` (mirrors `fs_read`): sensitivity of a browser-storage read depends on what is stored, so the default stays quiet and project-local catalogs can add `taint_sources` entries for their specific threat model. `document.cookie` remains under `env_read` — it is a getter/setter property whose read/write split requires additional design and is deferred.
- **Taint `TaintSource`/`TaintSink` kind field accepts `"attribute"`.** Required so auto-imported attribute-style primitives (`os.environ`, `sys.argv`, `sys.path`) become first-class source/sink records. `module_attr_ref` added to `TAINT_CALL_EDGE_TYPES` so structural propagation walks the new edge type.

### Fixed

#### Analysis quality (WI-guhok)

- **`io-boundaries`: attribute-style primitives (`os.environ`, `sys.argv`, `sys.path`) are now detected for Python.** The `io_primitives/python.yaml` catalog has always declared module attributes as `env_read` primitives alongside function calls, and the catalog loader has always parsed them into `IoPrimitive(kind="attribute")` objects — but nothing in the pipeline ever produced an edge to match them against. `os.environ["X"]` and `sys.argv` read-sites produced zero edges, so every chain through them was invisible to the boundary map and to downstream taint-style reasoning. The Python AST analyzer now emits a new `module_attr_ref` edge for bare attribute reads on imported modules (skipping the case where the attribute is the `.func` of a call, since the calls pipeline already handles that). The new edge type is included in `tag_io_boundaries` and `_TRACEABLE_EDGE_TYPES` so attribute primitives reach `primitive_counts` and the backward-BFS that populates `entry_points`. Surfaced during self-analysis dogfooding: `env_read` chain_count was 3 (all `platform.*` function calls) on a codebase with 39 actual `os.environ`/`os.getenv` call sites. Scope: Python only in this PR; WI-gapam covers the same fix for the 12 tree-sitter-based analyzers (JavaScript, Java, Go, C, Rust, etc.) via a base-class helper.
