...
Three themes. First, **receiver typing is drained as a class across the analyzer fleet**: a method call whose receiver the repository already describes now reaches the I/O catalogue instead of ADR-0051's `external` sentinel, the marker for *unreachable to the catalogue*. Ten languages learn shapes they held the evidence for and were not reading.

Second, **taint precision is measured under a ratified frame and the shipped catalogues are corrected against it.** ADR-0046 makes precision two numbers, ADR-0048 brings the benchmark frame in as a CI gate, and ADR-0049 rules that a call which merely opens, registers or defers a network crossing is disclosed rather than minted as untrusted input. Useful precision, which had been tripping the `≤25%` band, moves **24.1% → 30.9%** across measurements 0003–0012.

Third, **the catalogues become extensible and the vocabularies become declared.** ADR-0047 makes the shipped catalogues stdlib-only, moving 291 third-party rows into community overlays; ADR-0045 adds user and project configuration plus a per-repository trust grant for the code-executing Rust backend; ADR-0050, ADR-0051 and ADR-0058 give the I/O-boundary vocabulary, the module key and `Symbol.signature` declared axes — the last two retiring `free-text` justifications that had gone false under shipped consumers; and the ADR-0017 §3a walk gains removal authority (ADR-0052). `SCHEMA_VERSION` advances 0.20.1 → 0.20.12 and the `verify-claims --json` envelope 2.0 → 2.4.

### Added

#### Receiver typing across the fleet

- **The Django ORM surface is typed end to end.** A QuerySet chain keeps its ORM module past the first hop; reverse-relation and custom managers carry `django.db.models`; the instance write is recognised through the model lineage rather than one direct base (2 → 75 sites); `setUp` fixtures carry the model instance (+1,447 sites). A relation accessor the project's own models declare types the ORM slot on an *untyped* root: **+2,943 edges, 0 lost**, against a shuffled-index ablation mean of +37.3. The nineteen lazy QuerySet combinators leave the minting boundary under a new `db_compose` value, having read nothing while minting `untrusted_input`.
- **Ten analyzers learn the receiver shapes they had evidence for** — own instance fields, bare parameters, declared types, expression and chained receivers, the `?` / `.unwrap()` forms, loop and optional bindings, `self.<property>`, and `obj.field.method()` chains. Kotlin emits an instance-method call edge on an external receiver at all, where every gate required an in-repo lookup a JDK method can never satisfy.
- **Return-type registries are consumed, and `library_signatures/` supplies what no repository declares** — the return type of a *library* call. Swift binds a project member only when some overload's argument labels admit it (`removeItem(at:)` and `removeItem(atPath:)` are different methods), and Rust emits a method call on a signature-declared receiver even when the name sits on the generic-trait denylist.
- **Two grammar-level enablers.** tree-sitter-swift is bumped 0.0.1 → 0.7.3, which parses a `#if` inside a type body — Alamofire's `open class Session` had never been a symbol. Three Apple-SDK macro families break the Objective-C parse and the grammar silently falls back to C: error-parsing files go **74 → 38 (19.1% → 9.8%)**.
- **A verdict typed from a name stays qualified rather than going silent.** Typing a receiver removes it from `untyped_receiver_sites`, so a recall gain would have bought a quieter all-clear; edges filled by the accessor-name rule carry `resolution_quality="accessor_name"`, with `CAVEAT_ACCESSOR_NAME_RECEIVER` as its complement. `call_construct` is now stamped for every receiver shape, which is what the disclosure channel reads.

#### I/O boundaries: the deferred crossing (ADR-0049)

- **ADR-0049: a deferred crossing is disclosed, not minted.** One question decides whether a network call is a transfer — does it return a value whose content the far side chose? **`net_listen`** is the disclosure boundary for calls that merely open, register or defer: it mints no taint, is counted separately from the `total_io_edges` headline, and shadows `net_recv` so a clean verdict over a listener is qualified rather than granted.
- **A multi-boundary primitive declares why it has several boundaries, and names its own fallback.** Of 29 such primitives across the 15 catalogues, 12 declared nothing. `boundary_ruling` (`simultaneous` / `call_site_undecidable` / `unruled`) resolves that, and `abstains_to` names where an unstamped call falls back — C's `fgets` and Go's `bufio.NewScanner` could not both be right under one constant.  **`io_target_kind` gains `in_memory`, `pipe` and `net_stream`.**
- **A DNS lookup is a network receive, in five languages**, since the content of a resolver's answer is chosen by the far side. Reach was measured rather than assumed: the rows add 6 situations, move 0 verdicts, and 0 of the 6 are correct — a number that measures the walk, not the rows. `socket` is granted module completeness after enumerating its 35 module-level callables.

#### Catalogue extensibility and user configuration (ADR-0047, ADR-0045)

- **Community overlays ship in the wheel, load by default, and say so on stderr every run.** Two overlays had sat under `docs/`, which `pyproject.toml` does not package, so they shipped to nobody. Their rows are stamped `unvouched` at merge, so a call they classify cannot vouch for a verdict.
- **User catalogue channels work end to end.** `hypergumbo init-catalogs` creates one `<family>.d/` directory per extensible family, and `io_primitives.d/` is now actually scanned — every overlay-loading run had been telling users to edit a directory nothing read.
- **User and project configuration, and one setting they are forbidden to hold (ADR-0045).** `config.toml` and `<repo>/.hypergumbo.toml` carry `io_primitives`, extending the chain user < project < claims-file < `--io-primitives`; a repository may not grant itself the Rust backend. **`hypergumbo trust-backend`** is a durable per-repository grant for the backend that executes the analysed crate's `build.rs`, where the only persistence had been a global env var.

#### verify-claims — what a verdict says about itself

- **The §3a walk publishes its own verdict, so `flows_removed_by_walk: 0` is readable (ADR-0052).** `dataflow_coverage` reported how many flows the walk removed without reporting whether it ever ran; on the measured corpus the honest answer is `unavailable` 224 of 224. **A verdict also reports the fidelity it was reached at** (envelope 2.2), separating a backend installed but not enabled from one that does not exist.
- **A clean verdict names what it could not see.** New ADR-0016 §4 caveat kinds `untyped_receiver` and `unknown_receiver_scope` name the call sites where a catalogued method is reached through an untyped receiver; a language whose analyzer emits no external instance-method call says so from a dated, fail-closed declaration (Kotlin and JavaScript, 232 catalogued sinks); and completeness grants are reported by module.
- **A verdict says how much of the catalogue behind it is unverified.** A row asserts that a *name* crosses a boundary *kind*, and nothing checked that the named primitive performs that operation — the defect that had `newIORef` filed under `db_read`.

#### Declared blindnesses

- **`untrusted_input` does not distinguish a value the far side authored from one it merely chose from a constrained set**, now a dated declared blindness rather than a pending fix. The field was declined on a threshold fixed *before* any number existed: a cohort sampled deliberately to favour it produced **2 of 568 findings (0.35%)** against a 5% bar.
- **A catalogue row can declare that an argument merely NAMES the resource its sink acts on** (`VERIFY_CLAIMS_SCHEMA_VERSION` 2.3 → 2.4). Tainted data reaching `os.Chmod` to select *which* file stays a true positive on correctness and is excluded on usefulness. The annotation is keyed per *function*, not per row, since a row is not a primitive — `c.yaml`'s stdio row lists fourteen functions spanning all four classes at once.
- **`analysis_runs[].silence_reason` says WHY a pass produced nothing, on a closed axis rather than in prose** (`SCHEMA_VERSION` 0.20.6 → 0.20.7). "61 of 84 passes produce 0 edges" counts an undifferentiated population: **274 of 322 zero-edge pass-runs analysed zero files** and only 22 were truly silent. An absent reason means *not applicable* and `unreported` means *cannot determine*; collapsing them would repeat this project's recurring absent-versus-empty defect.
- **Go's `call_construct` is a declared blindness, priced at zero rather than fixed.** Go writes a package-qualified free function with the same syntax as a value-receiver method, so the slot records the *syntax*. Measured across 14 repositories and 72,396 unresolved edges, the ambiguity costs a verdict zero times.

#### Measurement — the frame, the series and the instruments

- **ADR-0046: taint precision is two numbers.** Correctness precision is unchanged, so measurements 0001–0006 stay comparable; useful precision deducts the vacuous true positives, since the model cannot separate a value that leaks from one a config-driven program was told to act on. **ADR-0048 puts the benchmark frame in the repository and makes rule F8 a gate**, requiring a machine-readable `## Frame` block naming unit, allocation, seed, cohort, claim set, rubric, analyzer SHA and language scope.
- **The series prices the arc.** 0003–0006 and 0012 price precision; 0007, 0008 and 0011 find ADR-0017 §7a's addressable domain to be zero on this corpus (90.8% of `ddg_mixed` rows rest on a walk that never ran); 0009 and 0010 grow the deferred-crossing shape table from four shapes to seven; 0029 reports the receiver-typing drain moving answers in six of seven languages, zero primitives lost.
- **The instruments are in-repo and tested.** The adjudication packet builder ships with 42 tests, after 0006's own packets carried an empty sink listing for 66 of 112 situations because they searched only the source symbol's span.

#### Developer tooling and CI

- **`ci-debug` reads what Woodpecker's one aggregate bit hides.** `cron-status` reports the most recent cron verdict per workflow off whatever commit carries it, which HEAD-only `status` could never see.
- **`ci-debug pr-body <n>` prints a pull request's description**, which no approved script exposed: `pr-status` already fetched `/pulls/<n>` and discarded the `body` the payload carried, and a rebase-merged PR leaves no merge commit holding it. The reader is `json_text_state`, a sibling of `json_bool_state` for strings, because `json_field` renders an empty body, a JSON null, an absent key and an unparseable response identically as `''` — so "this PR has no description" was indistinguishable from having read nothing. The text is fenced under a per-invocation nonce: a PR body is author-controlled, the consumer is an agent, and a fixed delimiter would be forgeable from inside the body.
- **A production function reachable only from its own tests fails CI.** `scripts/check-test-only-reachability` asks whether every direct caller lives in a test module — the question `dead-code-maybe` cannot ask of exported API. **A node or edge id is minted, never hand-spelled** (ADR-0034), checked by reading the producers rather than a run.
- **Smaller.** A gate fails on any stdlib import whose debut is above the declared `requires-python` floor; `tag-release` verifies push credentials before it signs, after tagging v8.0.0 signed cleanly and died at the push.

### Changed

#### Multi-fidelity coexistence moves from the record to the attribute (ADR-0057)

Design only — nothing in the pipeline changes in this release. ADR-0012 §Step 3 said a higher-fidelity pass's edges *coexist beside* the AST pass's as separate records; measured on a two-arm Rust run that doubles every callable and the in-crate call graph. ADR-0057 keeps the principle (both backends run, nothing is discarded) and changes the unit: two producers' records for one declaration become **one node and one edge**, and each attribute is a **set of (value, provenance) pairs** — one value with two provenances when they agree, two values when they disagree, one when only one producer saw it. Recognising the pair is a declared merge pass at the top of Phase C (the key is span containment, since both `id` and `stable_id` derive from the attributes that disagree); the scalar a consumer reads comes from a per-record arbitration property with **one** default, read from `config.toml`. The measurement behind it: the two Rust arms agree on 92 of ~100 in-crate call edges, disagreed on `kind` for 52 of 52 free functions (SCIP was the wrong one, fixed below), and each carries a class of edge the other cannot. Nine tracker rows sequence the build; WI-gojum stays parked. §10–§12, added the same day: every backend **declares** its merge anchor and the pass refuses an undeclared producer (both Rust registrations currently set neither `backend` nor `languages`); on an edge `src`/`dst`/`edge_type` are identity, so a disagreement is two edges; and cross-backend tests run only on recorded producer-shaped fixtures. §13–§14, ruled the same evening: `confidence` is not arbitrated by precedence — two producers reaching one edge by distinct pathways yield a declared corroboration level (built-in 0.95, new `confidence_source=corroborated`, `CONFIDENCE_MODEL` v2.1), never `max` or noisy-OR; and a resolved first-party edge demotes a same-site external-stub edge by `rank_score`, stamped on the stub, never deleted (33 of 257 stub sites on the measured crate). The earlier citation of `confidence_source=composite` as precedent is corrected — ADR-0039 defines that value as transitional blended ranking, not multi-producer provenance.

**The producer contract is built (§10, WI-hohuh).** `register_analyzer` takes `merge=` — a `MergeAnchor(name_key, span_role, authoritative_for)` for a backend whose records pair with another producer's, or a `MergeDisjoint(partners=...)` for one that shares a language but never a record — and `executes_analysed_code=`, both validated at decoration (an `authoritative_for` entry without a `docs/audits/` citation, a span role outside `token`/`item`, or a dict where a declaration belongs is refused by name). `merge_participants(language)` is the reader the merge pass will use: the anchored producers, or `UndeclaredProducerError` naming the analyzer when a language has two producers and one has not declared. Both Rust arms are anchored — tree-sitter by the last segment under the taxonomy's declared `::` separator over the item span, `rust_analyzer` by the descriptor name as emitted over the identifier-token span — and a recorded rust-analyzer 1.94.0 fixture pins that the two map every declaration in a sample crate to exactly one key, with the two `greet`s separated by containment, not by name. `javascript` / `svelte` / `vue` declare their disjointness on both sides and a test runs all three on a component file to prove it. `executes_analysed_code` moves from a hardcoded set in `user_config.py` onto the `rust_analyzer` registration; the trust store's accept-list and the config tiers' deny-list are now derived from the registry, and the store's refusal names the backends it does accept. Consequence: with the SCIP package not installed, `backends.rust_analyzer` in a config file is refused as an unknown setting rather than as a trust key — still refused, but no longer claiming a store for a backend that is not there.

**Cross-backend tests run on recorded producer input (§12, WI-romuh).** The rust-analyzer package now owns a `recorded/` directory (on pytest's `pythonpath`, not in the wheel) with two recordings of `rust-analyzer 1.94.0`: the 33-line sample crate with every Definition inline, and aardvark-dns — upstream source at `4444d90fee` (Apache-2.0) committed beside the raw `index.scip` it emitted (16 documents, 169 global definitions, 500 locals, 3,543 references), with `project_root` normalised. A fixture test pins that every indexed document is a committed file and that, under the anchors both arms declare, the live tree-sitter arm pairs **148 of 169** SCIP definitions one-to-one with none ambiguous — the number the merge pass must reproduce, correcting the 138 ADR-0057 cited from a scratch script; the 21 leftovers are 18 module namespaces plus two `type` aliases and one `static` the tree-sitter arm emits no Symbol for. The `rust_scip` parity test reads its recorded spans from the shared module instead of a transcribed dict. New lint `scripts/check-recorded-producer-input` (CI gate `test_check_recorded_producer_input.py::TestLiveTree`): a per-test AST taint walk from calls to an incumbent producer — derived from the registry's merge declarations — through assignment, unpacking, `for` and comprehension targets, into any cross-backend consumer; the three tests that feed the incumbent's output on purpose carry `@pytest.mark.incumbent_fed("<reason>")`, a new registered marker, and a bare marker is refused.

**The SCIP importer reads the producer's declared kind (§7, WI-gapup).** `scip/index.py` mapped a Symbol's kind from the leaf descriptor's suffix alone, and SCIP's `().` suffix covers a free function as well as a method — so every free function the SCIP arm emitted was `method` (52 of 52 on aardvark-dns) and every enum variant, a `#` type nested in a type, was `class` (9 of 9). rust-analyzer declares `SymbolInformation.kind` on every global definition (169 of 169 on the recorded index) and the importer now reads it first — Function → `function`, Method / StaticMethod / TraitMethod → `method`, Field → `field`, EnumMember → `field` (the tree-sitter arm's choice for a variant), Struct / Enum / Trait / TypeAlias → their registered kinds, Constant → `constant`, StaticVariable → `variable` — through a map that is held on the Symbol.kind axis by the conformance test and is deliberately not total over SCIP's 70-odd kinds. An emitter that leaves `kind` unset gets the descriptor chain: a METHOD or TERM leaf is placed by its nearest ancestor that is not a type parameter, because rust-analyzer spells an impl target as one (`impl#[Counter]increment().`) and the immediate parent would have called 26 of 27 methods free functions; a nested type stays `class`, since in scip-python `Outer#Inner#` is a nested class. Measured on the recorded fixture: the two Rust arms now agree on `kind` for 133 of the 148 paired records — 52 functions, 27 methods, 29 fields, 9 variants, 10 structs, 3 enums, 1 trait, 2 statics — and disagree on the 15 `const` items, where the SCIP arm says `constant` and the tree-sitter arm `variable`, a contest left to arbitration because the SCIP side is the more precise one. SCIP-origin Symbol ids change where the kind slot changed; `stable_id` (the moniker hash) does not.

**SCIP occurrence edges to a declared callable are `calls` (§7, WI-zapuk).** Every occurrence-derived edge the SCIP arm emitted was `references`, "so a downstream specialization pass can refine it" — no such pass existed, and the tree-sitter arm labels the same call sites `calls`, so under `edge_key = (src, dst, edge_type)` none of the shared call sites was a duplicate the merge pass could fold. The row prescribed mapping `Occurrence.symbol_roles`; measured on the recorded aardvark-dns index, rust-analyzer 1.94.0 sets the bitfield to zero on 3,543 of 3,543 reference occurrences. The decision now comes from the target's declared `SymbolInformation.kind`, read across every document of the index: Function, Method, StaticMethod, TraitMethod, AbstractMethod, ProtocolMethod, PureVirtualMethod and Constructor targets give `calls`; everything else, and any target whose kind is unset, stays `references`; the role bits are still kept in `meta["symbol_roles"]` and no read/write edge type is minted from them, because no recorded producer sets them. On the committed fixture, 130 of 498 SCIP edges target a callable and 121 of them have a tree-sitter `calls` twin on the paired endpoints, now with the same `edge_type`; nine twins stay contested where the tree-sitter arm calls tuple-struct and variant construction `calls`. Accepted over-claim: a callable referenced as a value is `calls` too — SCIP carries no call-position signal — which is why the confidence stays at the occurrence-ref 0.85.

**The shared SCIP importer no longer needs the producer to say what it can read off the index (§7, WI-nanom measurement).** Two gaps found the moment a second SCIP producer was run through it. scip-python 0.6.6 leaves `Document.language` empty on every document, so every one of its records landed in language `unknown` and the merge pass — keyed on language — paired nothing; `scip/index.py` now falls back to the file extension through the taxonomy's `get_language`, and keeps `unknown` only when neither the producer nor the extension says. The same producer declares `SymbolInformation.kind == 0` on 17,851 of 17,851 symbols, so the WI-gapup declared-kind rule typed every occurrence `references` and no `calls` at all — the exact shape that measured zero receiver typing on the Rust arm; `scip/calls.py` now lets the SCIP descriptor grammar decide when the producer is silent (a METHOD descriptor, `name().`, is callable), while a declared kind keeps winning. Both are backend-neutral: rust-analyzer declares both fields and is unaffected. Measured on hypergumbo-core's 179 source files, the Python arms then pair 4,723 of 5,284 records, agree on `kind` 4,721 to 2, share 3,162 in-repo call edges, and scip-python types the receiver of 4,831 of the syntax arm's 5,048 untyped method calls (95.7%) — the number the WI-nanom backend is built on.

**The merge pass is built (§3, WI-kokiz).** `analyze/merge_producers.py` runs at the top of Phase C — after id relativization, before `refine_frameworks` — and folds two anchored producers' records for one declaration into one Symbol. A record's producer is its `origin_run_id` → `AnalysisRun.pass`; the pairing key is each producer's declared anchor (same path, equal `name_key`, token inside item or item equal to item); the incumbent is the non-`scip` backend (`incumbent_first`, the §5 built-in until the config default lands). The merged record is the incumbent's copy with a new id minted from the merged attributes, the item span (INV-lodum's cure for every merged record), `origin = [incumbent, alternative]`, the union of `meta` with the incumbent winning shared keys (so the SCIP moniker survives), attributes only the other producer observed filled in, and `origin_run_id` of the pass's own `producer-merge` run — appended only when something was folded. Every edge endpoint, `derived_from` and usage-context `symbol_ref` naming a folded record is rewired and the edge key reset, so `deduplicate_edges` collapses the twins. Per merged record the pass keeps the §1 candidate set in memory — agreement is one value with two producers, disagreement two values — for the provenance slot the next row serializes. A producer of a shared language with no merge declaration makes the pass refuse by name; a record with two candidate partners is left unmerged and reported; a single producer is untouched, byte for byte. Measured on the committed aardvark-dns fixture: 148 folded, 0 ambiguous, 21 SCIP-only left, 0 tree-sitter-only; the 121 shared call sites fall onto 93 distinct keys that dedup then collapses.

**The provenance slot, the arbitration property and corroboration (§1, §4, §6, §13, WI-binis; schema 0.20.11, confidence model v2.1).** A merged Symbol and a folded Edge now carry `attribution` (`{field: [pass_id, …]}` — who holds the value the scalar slot carries: both producers on agreement, the arbitration winner on a contest, the sole observer otherwise) and `alternatives` (`{field: [{value, origin}]}`, present only for contested fields — nothing a producer emitted is discarded). Both are omitted on every single-producer record, so a one-backend artifact is byte-identical; `from_dict` reads them back and the schema generator declares them conditional. The scalar is produced by `merge_producers.arbitrate`, a pure function over the candidate set with one stamped default — incumbent first for every categorical attribute, the item span for `span` — so ranking, slicing and serialization see one value and any other policy is an opt-in read of the candidates. On a folded edge `confidence` is combined, not arbitrated: two distinct inference pathways reaching one `(src, dst, edge_type)` make it `corroborated` — a new `confidence_source` value, in the validator's bounded enum — at the declared `CORROBORATED_CONFIDENCE` of 0.95 with both originals kept in `alternatives.confidence`; the same pathway twice is not new evidence and keeps the incumbent's value; `CONFIDENCE_MODEL` advances to `hypergumbo-evidence-v2.1`. The merge pass now folds the participants' agreeing edges itself, incumbent-first and absorbing every call site, rather than leaving them to encounter-order deduplication (the SCIP arm runs first by priority, so encounter order would have kept the wrong survivor). Measured on the fixture: all 148 merged records carry a `span` and a `stable_id` alternative, the 15 `const` items a `kind` alternative, and the 93 folded call edges are corroborated.

**A resolved edge demotes a same-site external stub, by `rank_score`, stamped (§14, WI-lihis).** New finalize sub-step 7a, right after the resolution verdict: when a resolved edge and an unresolved edge to an external stub share a `src`, a call line, an `edge_type` and the declared callee name key, and the resolved edge carries a producer the stub's `origin` lacks, the stub's `rank_score` is multiplied by the declared `SUPERSEDED_STUB_RANK_FACTOR` (0.5) and `meta.superseded_by` / `meta.superseded_by_origin` (two new registered edge-meta keys) name the superseding edge and its producers. `confidence` is untouched, nothing is deleted, the edge count is unchanged. The measurement the row required moved its number: on the committed aardvark-dns fixture 244 tree-sitter call edges end at a stub, 27 share a site and type with a resolved edge, but only 3 name the same callee — the `wrap` calls SCIP resolves to `Result::wrap`; the other 24 are different calls on one line and are left alone.

**The results cache is keyed on the resolved backend set (WI-givib).** `<fingerprint>/results/<state>/<analyzer_identity>` hashed the installed code — which backends exist — not which were enabled, so a tree-sitter-only run and a two-arm run of one tree shared a cache directory and could return each other's artifact, a defect the merge pass made user-visible (the two differ in node count). `backend_selection.resolved_backend_set` now names the opt-in backends that will actually run — the ADR-0045 precedence chain (flag > environment > per-repository trust grant) *and* the binary being installed, the same two conditions the backend's own gate makes, so an opt-in with no `rust-analyzer` on PATH keys as tree-sitter-only because that is what runs — and the state segment carries it as a suffix (`<state>-rust_analyzer`) only when non-empty. Every entry written before this change keeps its key. Behaviour worth knowing: a read command (`slice`, `search`, `explain`, …) resolves the same chain, so a repository with a trust grant or the environment variable set finds its two-arm artifact; a survey run with a one-off `--backend rust-analyzer` flag is reachable only with the same flag.

**The second producer for Python: scip-python / pyright (WI-nanom; ADR-0057 §10, ADR-0045 ruling 5).** New optional package `hypergumbo-lang-scip-python` (`pipx install 'hypergumbo[scip-python]'`; the producer is `npm install -g @sourcegraph/scip-python@0.6.6`) registers `scip_python` beside the ast-based `python` analyzer — backend `scip`, merge anchor name-as-emitted over the identifier token, `executes_analysed_code=False`. Because pyright infers and never runs the analysed code, its opt-in is an ordinary preference: `--backend scip-python`, `HYPERGUMBO_SCIP_PYTHON=1`, or `[backends] scip_python = true` in either ADR-0045 config tier — the config tier the ADR deferred until the first non-executing backend, now wired inside `resolve_optin` below the environment; `[backends]` accepts exactly the registry's non-executing alternative arms and still refuses the executing ones as trust keys. `--backend tree-sitter` turns every opt-in backend off; a request with the wrapper or the binary missing exits 2 naming the install line. The translation drops what the syntax arm never models (parameter symbols, per-module `__init__` declarations) and does the one thing the Rust arm could not: an external callee keeps the receiver's MODULE, as a `calls` edge on the syntax arm's own id shape (`python:pathlib.Path:0-0:glob:unresolved` with a structured `dst_ref`, `call_construct` stamped). Measured first, on hypergumbo-core's 179 files, before the wrapper was built: the arms pair 4,723 of 5,284 records and agree on `kind` 4,721 to 2; scip-python types 3,973 of the 4,133 method-call receivers the syntax arm left module-less (96.1%: 3,840 stdlib, 96 in-repo, 37 third-party), where the Rust arm measured zero. The recorded fixture is a three-file sample package and its scip-python 0.6.6 index (`project_root` normalised); the pass-silence codes travel with every failure (`dependency_unavailable` / `pass_crashed` / `unreported`), and a completed run that indexed nothing is a run, not a skip. Not done, stated: no `install-scip-python` command (a global npm prefix is the user's choice); no installed-but-not-enabled nudge for this backend; and the fold of a module-less stub into its typed twin at one call site is filed for the owner rather than ruled here (§11 makes `dst` identity).

**The arbitration default is a preference (§5, WI-hukuf).** `hypergumbo_core.arbitration.ArbitrationPolicy` is now the one place precedence among a language's producers is decided, and the two declared levels live beside it (`CORROBORATED_CONFIDENCE` 0.95, `SUPERSEDED_STUB_RANK_FACTOR` 0.5 — re-exported where they were). A `[merge]` table in either ADR-0045 preference tier re-orders backends globally (`prefer = ["scip", "tree-sitter"]`), per attribute (`[merge.prefer_by_attribute] kind = ["scip"]`, replacing the global order for that attribute), or moves a level; the project tier replaces the user tier key by key; it executes nothing, so it is allowed in both tiers. Orders name backends, not analyzers, so one line means the same thing for every language the backend serves. Refused, naming the file and key, before any analysis runs: an unknown key under `[merge]`, a backend no registered analyzer declares, an attribute the merge pass does not arbitrate, a level outside `[0, 1]` (and `true` is not `1`). `run_survey` resolves the policy once and threads it into the merge pass (precedence, per-attribute precedence, the §13 level) and finalize sub-step 7a (producer order, the §14 factor). The built-in is `BUILTIN_POLICY` — incumbent first, as before — and its per-attribute exception table is **empty**: the first backend-agreement table warranted none, and any future entry must cite a committed `docs/audits/` document of `kind: backend_agreement` or the live-tree validator refuses it by name. On the recorded aardvark-dns fixture a `kind` preference for the SCIP arm moves exactly the `kind` row's carried value (15 `constant`) and no other number. Which arm is the *alternative* stays a registry fact (`ALTERNATIVE_BACKENDS`, now read through `incumbents_of`); no CLI flag or environment variable spells a precedence yet.

**The backend-agreement instrument (§5, WI-dajif).** New `hypergumbo backend-agreement ARTIFACT [--format md|json] [--out FILE]` (`hypergumbo_core.backend_agreement`) is the measurement ADR-0057 §5 defers to and the table §10 makes an `authoritative_for` declaration cite. It reads one survey artifact's own provenance — `attribution` / `alternatives` on merged records, `origin_run_id` → `analysis_runs[].pass` otherwise — so it re-pairs nothing and cannot disagree with the merge pass; a producer is what the registry declares for the language, incumbent first. Per language with two producers: pairing by kind, per-attribute agree / disagree / one-sided counts with the disagreement shapes (spans as `line:col-line:col`, `stable_id` counted but not exemplified), and edge overlap per edge type split by resolution, plus the corroborated and superseded counts. The markdown form is a `docs/audits/` sibling document, `kind: backend_agreement`; the verdicts lint now reads a document's declared `kind` and skips siblings. The first table, `docs/audits/0019`, is regenerated from the recorded aardvark-dns fixture by `scripts/regenerate-backend-agreement-table` (no rust-analyzer, no `build.rs` execution) and the fixture test pins that the committed numbers are the instrument's. What it shows: 148 of 169 records paired; `kind` 133 agree / 15 disagree (`variable` over SCIP's `constant`, WI-bamar), `name` 65 (`Type::member` over the bare member), `is_exported` one-sided to the syntax arm on all 148 (the SCIP arm computes no exportedness and now declares it — INV-huboz), `span` 146 by the §10 role split; `qualified_name`, `signature`, `docstring`, `modifiers` one-sided to the syntax arm and `stable_id` absent on the SCIP arm's 14 struct/enum/trait records (filed); 93 call edges corroborated, 269 `references` only SCIP emits, 244 external-stub calls only tree-sitter emits.

#### Vocabulary and axes (ADR-0050, ADR-0051, ADR-0058)
...
- **The module key did not lack an axis declaration — it carried a false one (ADR-0051).** `ExternalRef.module_path` read `free-text — consumers never branch on the value itself` while `io_boundary._module_matches` decided type-versus-sub-package from orthography, information-free wherever module names are capitalised. Unlike `Edge.edge_type` (ADR-0023), `Symbol.kind` (ADR-0027) and `Edge.evidence_type` (ADR-0028), it had no declared axis; the lint widens to match, surfacing 20 undeclared fields.
- **`Symbol.signature` carried the same false justification, and nine consumers had grown under it (ADR-0058).** It declared `free-text — consumers display, never branch on the value itself` while six parsers extracted a return type across five languages, two counted parameters for overload selection, and one read a field's declared type. ADR-0024 predicted this failure when it made the justification mandatory — `free-text` is "the only category whose 'this is the right call' claim isn't anchored elsewhere ... the natural can-kicker" — and the linter accepted it for ADR-0051's reason: a justification must be **present**, not **true**. The slot also carries two notions: of 38,573 populated signatures, **1,911 (4.95%) are a bare value type on a symbol that is not callable** (1,886 fields, 25 variables; 1,817 of them Python), which is a declared type wearing a signature's name.
- **The enforcement gates consumers, not values, which is new.** A structural-policy axis has no enumerable value set for a drift linter to check — ADR-0051 shipped without one for that reason. Here the enumerable thing is the *consumer* side: `signature_axis.find_undeclared_value_parsers` walks `packages/*/src` for reads whose value flows into a call and fails on any site absent from `LEGACY_VALUE_PARSERS`, which holds the nine with `file:line` citations that a companion test re-checks against the tree. The nine are grandfathered by owner ruling; **a tenth is a decision the gate forces someone to make.** The `getattr(sym, "signature", None)` form is matched because `jackson_dispatch` uses it — the INV-lafid blind spot, avoided by being looked for. Two limits are stated rather than covered: a value bound to a local, and a keyword argument named `signature`.
- **The facts inside a signature already had homes, and Python was the outlier.** Return types live in `FileAnalysis.method_return_types` (go, rust, swift, objc) or `meta["return_type"]` (java, luau, apex); arity lives in `meta["parameters"]` (15 analyzers, including `py.py`); a field's type lives in `FileAnalysis.class_field_types` (csharp, cpp). `py.py` cites the return-type home in a comment 5,240 lines above the line where it parses the string instead. Migrating the producers is ADR-0024 step 7 and is **not** done — nothing an analyzer emits changes here.
- **Python reads a callee's return type from its declared home (WI-ribak, ADR-0024 step 7 for ADR-0058).** `py.py` stamps `Symbol.meta["return_type"]` in Pass 1 at both emit sites — where the annotation is already in the AST and needs no re-parsing — and the `var_types` seeding that made a Python variable typed at all now reads that home instead of parsing `assigned_class.signature`. The closed parser set shrinks **9 → 8**, which is the measure of a migration rather than a declaration. Equivalence was measured, not assumed: over **33,740 functions** the AST value and the retired parse agreed on every one, 0 disagreements — which holds because WI-hopiz made display truncation preserve the return type. The producer records the declared type **verbatim** (`Optional[Foo]` included); the narrowness (simple identifiers only) stays the consumer's policy, so nothing widened or narrowed.
- **One correction to ADR-0058's own table, found by building on it.** It said the home was `FileAnalysis.method_return_types`. `py.py` has its **own** `FileAnalysis` (py.py:2181), a different dataclass from the tree-sitter base's, with neither that field nor `class_field_types` — and it never reaches the core linker that needs the same fact. `Symbol.meta["return_type"]` is the home that travels, and is the one java, luau and apex already use. The ADR is amended in place.
- **A quoted forward reference is still a type name (WI-fihun).** `def make() -> "Client"` unparses *with* its quotes, so the identifier check rejected it — yet PEP 484 says a string annotation names exactly the type it spells. Unquoting widens how a type may be *spelled*, not what counts as one; a quoted generic (`"Optional[Client]"`) still does not infer, pinned by a difference arm. **Measured effect on this repository: zero** — 37 of 31,932 stamped return types are quoted and none currently sits in a bare-function assignment, which is the only position the return-type inference reads (see below). Shipped as a latent-correctness fix, not as a recall claim.
- **`env_read` splits into `env_read` and `host_info_read`**, so the auto-derived taint label means what it says: 134 of 195 `env_read` rows were host or identity reads deriving `host_secret` sources, the weakest family in measurement 0001 at 22.9%. **The shipped catalogues are stdlib-only for the first time and every one is gated** (ADR-0047 rulings 1 and 8), and **four `call_construct` values naming a different axis are drained** (ADR-0038 ruling 2).

#### A linker's activation is declared, not inherited (INV-nanon)

- **"We considered gating and chose always-on" and "nobody thought about it" were the same bytes.** `register_linker` ended in `activation=activation or LinkerActivation(always=True)`, so a linker registered without an `activation=` argument ran on every repository and was indistinguishable in the record from one whose author had decided it should. **Twenty-five of the sixty-one registered linkers were in that state**; twelve had chosen `always=True` deliberately and twenty-four are genuinely gated. This is the absent-versus-empty defect wearing its other face — a *permissive default*, where the missing declaration is read as an affirmative one and the reading is invisible because it happens inside an `or`.
- **The twenty-five now declare `always_on_unreviewed()`.** It is behaviourally identical to `LinkerActivation(always=True)` and claims something different about the author: the linker runs unconditionally, and that is a default nobody defended. Writing `LinkerActivation(always=True)` across all twenty-five instead would have manufactured twenty-five rationales nobody verified — the fabricated-disclosure failure `pass_silence` already names, where a reason invented on the producer's behalf is worse than no reason. It is a factory rather than a shared constant because `LinkerActivation` carries mutable `frameworks` / `language_pairs` lists. `grep -rl always_on_unreviewed` is the worklist.
- **A gate keeps the next linker from inheriting silently.** `tests/test_linker_activation_declaration.py` fails on any `register_linker` call in `linkers/` that omits `activation=`, with a synthetic positive control so a detector that has stopped working cannot masquerade as a clean result. It is AST-based rather than line-based because the two `@register_linker(` occurrences in `registry.py` sit inside docstring examples and a line pattern would count them as registrations. **Stated scope:** ad-hoc registrations inside test modules keep the documented default — a test pinning priority ordering or cache behaviour has no opinion about activation, and forcing it to invent one would be noise rather than rigour.
- **Behaviour is unchanged, and that was measured rather than reasoned.** The resolved activation map — `always`, `frameworks` and `language_pairs` for all sixty-one registered linkers — is byte-identical to `origin/dev`, compared across a detached worktree with the loaded module path asserted (an editable install otherwise resolves both runs to the same tree and reports a flattering IDENTICAL) and with an injected mutation proving the comparison can report a difference at all. **Narrowing any of the twenty-five is deliberately not done here:** tightening an activation gate is the recall-loss direction, where a missed framework detection silently drops real edges and leaves no hole where a hole would be visible. This change produces the worklist; each entry on it is a separate, evidence-backed, per-linker decision.

#### Taint adjudication and reporting

- **Taint refutation is confirm-only in practice, and the escape-closing goal is retired (ADR-0052).** ADR-0017 §3a's removal *capability* is untouched, but driving escapes to zero is not worth its cost: 66.7–90.6% of blocked-walk sites have no callee recorded at the line at all. **`False` now removes the flow**, where the §3a arm had read `adjudicated = walk_result is True` and so treated "the walk exhausted every route" and "the walk lost the value" as one event that removed nothing.
- **An unadjudicated flow is reported once per situation, not once per call-site pair** (envelope 2.0 → 2.1). Six repositories reported 359 flows describing 78 situations, caddy's `cmdRun` alone emitting 76.
- **`analysis_method` no longer answers two questions at once.** It says which analysis produced a finding; `walk_verdict` records what the DDG walk concluded and `walk_blocked_by` names the first guard that stopped it. **What `untrusted_input` asserts is written down** — ADR-0049 ruling 1's fact, that a party on the far side had a say in the value, not that it authored the bytes.
- **The self-claims drift gate runs only on the twice-daily cron.** A `confirmed_with_caveats` → `violated` regression had ridden green `dev` for 63 commits: the cron arm caught it, but its verdict landed on a non-HEAD commit and collapsed into one aggregate status with three unrelated gates.

#### One fact, one name on the pass-silence axis (WI-mamiv, WI-gisor)

- **`limits.skipped_passes[].skip_reason_code` is renamed to `silence_reason`** — the name `AnalysisRun` already uses for the same question on the same closed axis (`SCHEMA_VERSION` 0.20.9 → 0.20.10). It is breaking for readers of the old key, which shipped one version earlier in the same unreleased window. Measured on component-model-demo, **130 of 162 passes carry `no_candidate_files`, 48 on one host and 82 on the other**, and which name a consumer saw was decided by `taxonomy.LANGUAGE_EXTENSIONS` membership and by whether the analyzer returns a bare `AnalysisResult()` or a run with `files_analyzed == 0` — producer registration and implementation detail, never a property of the silence.
- **The routing is deliberately unchanged, and that is stated rather than implied.** Making the *host* a function of the fact would require minting an `AnalysisRun` for 75 passes that never ran — ADR-0056 §"What would change this decision" item 5 reserves that to the owner and judges it wrong ("ABSENT ≠ EMPTY one level up"). So the fix is that asking the question no longer requires knowing which host will answer it; `pass_silence.census_silence()` remains the union, and it unions over the pass **catalogue** rather than over the two lists, because a pass in neither would otherwise be invisible.
- **The spec's routing sentence was wrong and is corrected (WI-gisor).** §9 claimed an out-of-taxonomy analyzer finding no files is recorded in `skipped_passes` *"either way"*. Measured: of 35 such analyzers, **27 land in `analysis_runs`** (26 of them with `files_analyzed == 0`) and only **8** in `skipped_passes` — 7 returning a bare `AnalysisResult()` plus `rust_analyzer` on the separate backend-disabled route. "Either way" was never the rule: taxonomy membership decides pre-filter versus dispatch, and the dispatched ones then split by their own return style. The paragraph now names all three routes and says plainly that a consumer asking "which passes had no input?" must read both hosts.

#### Schema

- **`SCHEMA_VERSION` 0.20.10 → 0.20.11 (WI-binis, additive):** `Symbol` and `Edge` gain the ADR-0057 §6 provenance slot — `attribution` (`{field: [pass_id, …]}`) and `alternatives` (`{field: [{value, origin}]}`) — both omitted unless the merge pass folded two producers' records, so single-producer artifacts are byte-identical; `Edge.confidence_source` gains `corroborated`; `confidence_model` advances to `hypergumbo-evidence-v2.1`.
- **`SCHEMA_VERSION` 0.20.1 → 0.20.7, six patch bumps, every one additive** — `Edge.meta` gains `call_arg_shape` (0.20.2), `callee_name` (0.20.3, the full-fidelity name ADR-0036 Ruling 1 tells consumers to read instead of the id's lossy slot), the bash per-call-site keys with their `<key>_values` companions (0.20.4) and `io_target_kind` (0.20.5); `TaintFlowFinding` gains `walk_verdict` and `walk_blocked_by` (0.20.6); `AnalysisRun` gains `silence_reason` (0.20.7). Every earlier artifact stays valid. **`VERIFY_CLAIMS_SCHEMA_VERSION` advances 2.0 → 2.4.**
...
- **An absent module is an abstention, not a value.** §11 made `src`/`dst`/`edge_type` an edge's identity without qualifying the endpoint, so a module-less external stub and its typed twin — the same call, seen by two producers — were two edges, on **3,973 of 4,133** method-call sites under the scip-python backend. The ruling refines §11 rather than excepting it: for an endpoint outside the repo the identity is `dst_ref`, which can abstain, and not the `dst` string, whose module segment defaults to the `external` sentinel that ADR-0051's axiom defines as *not a marker for the absence of an answer*. An external identity key missing its module component is **partial**, and a partial key matches a complete one that agrees on every component it states; two partial keys do not match each other. The fold places before `deduplicate_edges` — 12,126 external call edges are post-collapse multi-site, 4,818 of them sentinel-module — and refuses rather than guesses where a site is ambiguous, which is 10 of 86,839 site groups here. §15.7 states what it does not assert.

- **Landed as a second phase in the merge pass.** `_absorb_partial_external_keys` runs after §11's fold on identical `(src, dst, edge_type)`, over what it left; the survivor is incumbent-first, takes the stated `dst_ref` and a `dst` rebuilt from it by `format_legacy_dst`, re-mints its id (now via the extracted `ir.mint_edge_id`, so the fold derives the id by the same rule the producer used) and resets `edge_key`. `attribution["dst_ref"]` names the producer that stated the module; `alternatives` gains nothing, because the loser abstained. On the committed scip-python recording: of the syntax arm's **7** module-less stubs, **6** are absorbed and survive as one edge carrying both origins, and `label` — a property the SCIP arm resolves in-repo — has no twin stating a module and falls to §14. External unresolved `calls` in the backend-agreement table move from 2 both / 11 python-only / 6 scip-only to **8 both / 5 python-only / 0 scip-only**.

### Fixed

#### One clock read in `cli.py` withheld every self-claim verdict (INV-fugus)

- **The scheduled self-claims gate had been red since 09-19.** All 18 claims had fallen from `confirmed_with_caveats` to `inconclusive`, lost their `opaque_boundary` caveat, and four lost their credited sanitized flows. Bisected locally in pinned worktrees, with the drift line-for-line identical to CI's: the first bad commit is a7495d9b9a, which added `import datetime as _dt; _dt.date.today()` to `cli.py`.
- **The mechanism is a mis-kinded catalogue row, not a coverage change.** The Python analyzer emits a module-rooted dotted chain (`_dt.date.today()`, `os.path.join()`) with no `call_construct`. `datetime.date.today` was catalogued under `methods`, so `method_starved_modules` could satisfy the module by neither route and reported `datetime.date` "structurally invisible", even though `classify_call` classifies that exact edge as `host_info_read`. Starvation is not the qualifying kind of incompleteness, so no claim could be qualified and each was withheld.
- **The fix follows INV-nular's `Path.cwd` rule: a classmethod takes no receiver, so it is a function-kind row.** `datetime.date.today` and `datetime.datetime.now` / `utcnow` / `today` are all `classmethod_descriptor`s under `inspect.getattr_static`, and all four are now `functions`. The starvation gate is not loosened: an unstamped edge naming a *method* row still starves, as its tests pin. A new test sweeps every Python method-kind row whose module imports as a class and fails on any classmethod or staticmethod. The sweep found exactly these four. Rows on the 7 modules that do not import in the test environment are not checked.
- **Measured, `verify-claims` on the self-claims at dev tip with and without the fix, each in its own worktree and cache:** without it, the same 40-line drift; with it, `18 claims unchanged (18 confirmed_with_caveats)`. Per claim, verdict, evidence count, sanitized flows (e.g. `runtime-cli-no-subprocess` 11 → 0 → 11), excluded flows and caveat kinds are identical to the last good commit. A function-kind row also matches an unstamped `python:external` edge by short name. Across 7 older Python surveys (django, pretix, poetry, httpx, h5py, the FastAPI template and hypergumbo; about 284k call edges), unstamped `external` edges named `now`, `today` or `utcnow` number 0.

#### An Erlang or Elixir call is anchored to the function that contains it (INV-mozas)

- **The enclosing function was looked up by a name that several functions share.** Erlang names a function by name *and* arity, and Elixir emits one symbol per `def` clause with no arity in the name, but both analyzers found a call's enclosing function by its bare name — so it resolved to whichever same-named function registered last. The filed instance reproduces exactly: rabbitmq's `application:get_env` at `rabbit_channel.erl:819`, inside `get_consumer_timeout/0` (818–824), was anchored to `get_consumer_timeout/2` (2743–2744), so a taint source's span pointed at a forwarding wrapper 1,900 lines away that contains no environment read. Erlang now keys on `name/arity` read off the enclosing `fun_decl`; Elixir on the position of the enclosing `def`, which *is* the symbol's node. A `def` with no symbol — one inside `quote`, suppressed by INV-sinah — is walked past to the macro that contains it.
- **Pass 2 now sees every symbol of its file.** `extract_edges_from_file` receives `symbol_by_name`, which keeps one symbol per name, so an Erlang `-ifdef`/`-else` pair lost its first branch before any edge was extracted (ejabberd's `misc.erl`: the `jiffy:encode` call in the `-ifdef` copy of `json_encode/1` produced no edge at all). `TreeSitterAnalyzer.file_symbols()` hands Pass 2 the file's complete list; both analyzers use it. No other analyzer's behaviour changes.
- **Two defects on the same lookup, fixed alongside.** An Erlang local call now resolves to the function of the arity it is called with (`get_timeout()` inside `get_timeout/2` had been a self-loop on `/2`). An Elixir `def` head is no longer emitted as a call: the grammar parses `def f(a)` as a call to `f`, so every clause called every same-named clause from its own first line. Calls *inside* a head — a default argument, a guard — are still calls.
- **Measured on 7 repos (cowboy, rebar3, recon, ejabberd, phoenix, plausible, livebook), dev pinned in a worktree vs. this change:** call edges whose line falls outside their `src` symbol's span went **Erlang 1,306 → 0** (of 44,449, 2.9%) and **Elixir 7,166 → 0** (of 26,744, 26.8%). Every call site present at dev and absent after is accounted for: 1,551 are `def` heads, 1 is a quoted `def` head, and 1 merged into an identical `(src, dst)` edge. Seeded samples of gained and lost edges were all read at source: every gained edge sits in the function that contains it, and every lost edge had been anchored outside its call or was a head. All mechanisms are mutation-checked.
- **On the production path, one finding measurement 0012 counted USEFUL is gone, and that is the fix working.** `verify-claims` with the generic claims on rabbitmq's `deps/rabbit`, dev vs. this change: all 7 claim verdicts are unchanged, and evidence rows moved 60 → 54 (`host-secret-no-logging`), 48 → 49 and 5 → 6. The finding at the filed row, `rabbitmq:host-secret-no-logging:17`, no longer appears. Both of 0012's blind adjudicators labelled it TP, but through a chain the taint walk cannot follow: the value goes through a record field, `#conf.consumer_timeout`. The tool had reached it only because the source was anchored to the wrong function, from which it walked 13 hops through `rabbit_khepri` to an unrelated logger call. Anchored correctly, `get_consumer_timeout/0` has no path to a logging sink. **A true finding the tool found by accident is not recall this change loses; it is recall the tool never had.** Following record fields is the missing mechanism.
- **Not fixed here, measured:** other analyzers mis-anchor too. On the same dev run over 26 repos, calls outside their `src` span: Kotlin 3.7%, Scala 5.8%, Swift 3.3%, Haskell 4.0%, JavaScript 1.1%, C 0.3%, TypeScript 0.2%, Rust 0.1%, Ruby 0.2%. For Swift, C, TypeScript, Rust and Ruby nearly every such `src` shares its name with another function in the same file — the mechanism fixed here; for Kotlin (42%) and Scala (61%) some do; for Haskell (0 of 119) and JavaScript (6 of 135) almost none do, so that is a different mechanism. Java, which already looks up by position, is at 0.0% over 86,580 edges. Elixir still fans a local call out to every same-named clause regardless of arity. Filed separately.

#### A declared return type is read in the module that declares it (WI-fihun)

- **`-> Store` means the `Store` visible where it was written.** `_resolve_return_type_class` looked in the caller's locals, then the caller's imports, then the callee's own file, and never in the callee's imports. So `TrackerSet.workspace -> Store`, declared in `trackerset.py` with `Store` imported from `store.py`, did not resolve when seen from a third module: WI-fikoh's chain join stopped, and `ts.workspace.add(...)` kept its line-proximity binding to `TrackerSet.add`. Worse, when the caller happened to define its *own* `Store`, the chain named that class with a trusted `receiver_type_id`. Resolution now follows Python's rule: the defining module's own class, then its imports, and the caller's scope only as a fallback. Every WI-fikoh fixture had been single-file, where the callee's file defines the class, so none could see this.
- **`Cls.factory()` and `obj.build()` type the local they are assigned to.** The assignment path asked `_resolve_call_target` for the callee, and it understood only `Name()` and `module.attr()`. `run = AnalysisRun.create(...)` therefore left `run` untyped even though `create` declares `-> "AnalysisRun"`. It now resolves a method on a *known* receiver: a local already in `var_types` (checked first, since it shadows a same-named class) or a class in scope. An untyped receiver still resolves nothing, which a negative control pins.
- **Measured, the same working tree surveyed with dev's code and with this change (isolated caches):** method-call-recovery edges **62 → 49**. **7 of WI-fihun's 14 wrong bindings are gone**: the 3 factory sites now resolve first-party to `AnalysisRun.to_dict`, `UsageContext.to_dict` and `Edge.to_dict`, and the 4 `ts.workspace.add` sites to `Store.add`. The other 6 recovery edges that vanished are now resolved first-party to the target the linker had guessed. Compared on `(src, dst, type)`, the resolved call edges lost are **exactly those 7 wrong ones and no others**. Gained: **196** resolved calls (86 analyzer, 110 inherited-calls). A seeded sample of 14 read at source was all correct. Receiver hints rose 818 → 928. All three mechanisms are mutation-checked.
- **Left, measured:** 5 loop/comprehension targets (element typing) and 2 builtin receivers. The row's "no evidence anywhere" for the builtin pair is corrected: `set()` and a dict literal are on the right-hand side. Typing locals from builtin constructors is a producer decision in WI-fuvuj's territory, because it changes the destination's module slot.
...
- **`http-linker`** reads one thing: path-bearing route records, its edge destination. Both `route_of` arms trace to a host analyzer. Markers are minted via `make_route_symbol`. Framework-YAML concepts are enriched onto a host record's `decorators`/`annotations` or `UsageContext`, and that enrichment records no run and is not a pass. So the clause is the 15 analyzers that can supply one: it **drops `elm`** (scanned only for client calls) and **adds `play-routes`, `clojure`, `groovy`**.
- **`crypto-flow-linker` → `[["javascript", "rust", "rust_analyzer"]]`.** It has no glob of its own. Every file it scans is the path of a symbol in `_CRYPTO_LANGUAGES`, so those producers are structural (with none of them the file list is empty) even though both edge ends are minted. The old clause named ten languages, seven of which its filter discards.
- **`subprocess-linker` → `[["python", "scip_python"]]`, `orm-linker` → `[["python"]]`.** Both scan `**/*.py` only, and their destinations are Python records. `scip_python` never writes `base_classes`, so it alone cannot produce an ORM edge. The old clauses listed nine and eight languages.
- **`event-sourcing-linker` and `grpc-linker` → `[]`**, under the WI-dilab allowance's fixed docstring phrase. event-sourcing's only `ctx.symbols` read is the WI-vasik-jofiv tombstone, which discards its result. grpc's core edges are all built from its own scan. The Go analyzer feeds two optional Go-only sub-families, and CNF has no optional form: `[["go"]]` would be falsified on every non-Go gRPC repo.
- **Not changed, and filed:** subprocess and ORM destinations are not language-filtered. A Python accessor or subprocess call can therefore bind a same-named Ruby model or Go command by name coincidence.

#### The inherited-base walk keeps what it walked (INV-rukor, WI-latup)

- **The nine sites the previous entry declared `incomplete` now name their consumed records.** They all learned *why* an edge exists from a walk that threw its path away. `_transitive_bases` BFS'd a class's `extends`/`implements` ancestry and returned bare base-name strings, so a framework hook on `Order` whose `models.Model` base sits on an in-tree `LoggedModel` could say *that* it qualified, never *through what*. `collect_transitive_base_origins` walks the same records as `collect_transitive_base_names` (one shared BFS, pinned equal) and returns each name with its provenance: `()` for an entry on the class itself, otherwise the inheritance edge ids up to the ancestor that declared it, followed by that ancestor.
- **One matcher for the three table-driven framework linkers.** `django_orm_dispatch`, `airflow_framework_dispatch` and `_third_party_bases` carried three copies of the same base-table matcher; `match_framework_bases` replaces them and returns per-*method* provenance, so an edge names only the records that made *that* method a hook. `jackson_dispatch` names the ancestor chain behind a bean-marker base or, on the method-annotation path, the annotated sibling methods that qualified the class; `kafka_streams_dispatch` names the chain behind each callback interface; `grpc`'s base-chain struct mapping names the struct and its chain.
- **The view-template family.** `TemplateRenderEmission` gains `consumed_ids`, and explicit-string strategies may yield a `StringSite` carrying them (a plain 4-tuple still works). Rails and Laravel name the controller class and the chain its base came through, Spring the `@Controller` class, Phoenix the controller module, Django's CBV default the chain behind the CBV base. `LinkerContext.base_name_origins` builds the inheritance index once per context — `RailsStrategy.is_action_class` had rebuilt the inheritance index *and* the symbol map for every class symbol it was asked about.
- **DI heuristic bindings and `fire.Fire`.** `DIBinding.evidence_ids` carries the implements/extends edge and the two classes a heuristic binding read (an explicit binding, named by source text, carries none); `subprocess_cli` names the fired class that made a method a subcommand.
- **No `incomplete` declaration remains in the tree.** The vocabulary keeps the kind for a future honest gap. Every provenance site added here was mutation-checked: removing it turns its pin red (view-template 6, framework matcher 5, jackson 2, kafka / grpc / DI / fire 1 each).
- **Measured on production paths.** koel (Laravel), arm = dev `562e5a3b8b` in a worktree: `view-template-linker` **4 endpoints-only → 4 chains** naming the controller class, total edges identical (53,670). spring-petclinic: no change, correctly — all 29 Jackson edges qualify on `@Entity`/`@MappedSuperclass`, the class-annotation path, which reads nothing beyond the class. This repository has no edges from any family-2 linker. **Neither corpus exercises an inherited base**, so the transitive-chain half rests on the fixture pins, not on a real repository.

#### A linker says what it consumed, or declares why it cannot (INV-rukor, WI-latup)

- **INV-rukor had been closed on a guard that read the keyword, not the value.** `test_all_linker_edge_creates_pass_derived_from` asserted `"derived_from" in kwarg_names`, so `[src, dst]` passed exactly as a real consumed-input list did, and **86 of 93** linker `Edge.create` sites restated their own endpoints under it. The replacement guard reads the value: a site whose `derived_from` is a list of only its own `src`/`dst` expressions — or empty — must carry a declaration inside the call, `# derived-from endpoints|consumed-none|incomplete: <reason>`. A declaration on a site that already names more is refused as stale, and a malformed one is refused. It remains a *syntactic* check of a *semantic* claim: it cannot verify a declaration is true, only that one was made where the value alone says nothing.
- **The rule, stated once (`ir.py`, spec §9):** `derived_from` names the INPUT records whose presence decided the edge. Ids a linker MINTS in the same run are not consumed and are left out — `inheritance.py`'s external placeholder set the precedent — so a pass that builds both ends from a file scan records **`[]`**, a positive "consumed no graph record" distinct from `null`. `SCHEMA_VERSION` **0.20.12 → 0.20.13** drops `minItems: 1`, which had made that honest value invalid; widening only.
- **Every one of the 86 sites was read and classified**, not swept (counts from an AST diff against dev, not a hand tally). **17 now name what they consumed**: 12 name the unresolved or import edge they resolve (`build_target`, `cgo`, `dependency`, `grpc` stub resolution, `js_module` ×3, `lua_ffi`, `napi`, `pyffi` PyO3, `receiver_type_dispatch`, `vue_component`); `type_hierarchy` names the admitted `extends`/`implements` edges on a shortest inheritance path from the override's class up to the parent — the counterexample INV-rukor was reopened on; `rust_trait_dispatch` names the `implements` edge and the struct; `caddy_module_dispatch` names the module struct; Erlang `otp` names the module symbols that keyed its handler index; `yjs_crdt` names the manifest dependency that opens its gate. **24 are declared honest endpoints-only** (containment's span nesting and name prefix, name-string joins whose key is in meta). **36 drop minted ids** — 20 become `[]` (IPC, message-queue, websocket, gRPC scan, event-sourcing …), 16 keep only the one consumed endpoint. **9 are declared `incomplete`**: the shared transitive-bases walk (`_third_party_bases`, `airflow`, `django_orm`, `jackson`, `kafka_streams`, `grpc` base-chain structs, the view-template controllers) discards the ancestor and edge ids it walked, as do DI heuristic bindings and `fire.Fire` targets — the next WI-latup PR.
- **Measured on this repository, baseline = dev `b95e0d19ef` in a worktree, same tree:** edges carrying a real chain **399 → 840**, every one naming a consumed *edge*; `type-hierarchy-linker` **342 of 342 endpoints-only → 342 of 342 chains**; `js-module-linker` 55 and `dependency-linker` 44 likewise; `websocket-linker` 4 → `[]`. Per-linker edge counts are unchanged (containment +30 is this change's own new test code); only `derived_from` content moved. The remaining 48,416 endpoints-only edges are 48,016 containment and the declared joins.
- **`explain --provenance` gets a fourth state.** `[]` no longer falls into the `None` branch that says "analyzer-produced edge"; it says the pass consumed no existing graph record.
- **Not changed:** ADR-0057 §14.1 demotion still acts only for passes granted `supersedes_consumed_stub` (inherited-calls alone), so the 11 newly named unresolved edges demote nothing. Four defects the per-site reading surfaced are filed rather than fixed here: `ruby_ffi`'s `rb_define_method` self-loop (WI-judov), `swift_objc`'s dangling import ids (WI-mofih), `wasm_bindgen`'s `0-0` file id (WI-vojur), `react_component`'s fabricated `jsx_usage` src (WI-gulud).

#### The WI-ripas precision A/B, and a correction to its stated risk (WI-fazan)

- **CORRECTION.** The WI-ripas entry below originally warned that "a finding leaving the structurally-identified class enters the useful-precision denominator". That is wrong, and it was wrong on a citation this project's own rules say to check: **ADR-0046 defines useful precision as `(TP − VACUOUS) / adjudicated` over the SAME denominator, deducting from the NUMERATOR only**, where `VACUOUS` has exactly two member classes — CONFIGURED-ACTION (three citable clauses about a config source, schema deserialization and a field-parameterized sink) and KIND-MISDECLARED (the INV-nular defect). **Neither keys on `analysis_method`.** The word "structural" means two unrelated things in the two places, and nothing moves into or out of that denominator by changing an evidence label. The refutation as written is therefore **not applicable**, not "untested".
- **A population was found**, which was the row's actual first deliverable: `~/whole_bunch_of_repos/podman-compose` and `sherpa-onnx` under `docs/example-claims/generic-taint-claims.yaml` both produce violated verdicts with evidence. hypergumbo's own claims cannot: all repaired, zero violating flows by construction.
- **Measured, same binary except the two spec hooks.** sherpa-onnx, default arm, 28 situations / 47 collapsed pairs, unchanged in count: `structural` **6 → 4**, `ddg` **4 → 5**, walk `unavailable` **6 → 4**. podman-compose, default arm: one `structural` finding with `collapsed_flow_count: 2` **split into two separately-verdicted situations** — `logging.basicConfig` **confirmed**, `print_help` **escaped**. The fix distinguishes where it previously fused.
- **On the `--include-non-production-sources` arm the effect is total, and WI-mojaz's premise is confirmed: 100% of findings there are method-rooted.** `structural` **7 → 0**; every finding moves to a walk-backed label.
- **The real residual is a label-honesty question, not a precision one.** `method_for_walk_verdict` maps `confirmed → ddg`, `unavailable → structural`, and *everything else* — including `not_attempted` — to `ddg_mixed`. So on that arm **6 of the 8 findings now carry a DDG label with no walk ever attempted**, gaining it from DDG *coverage* rather than from evidence. That is INV-zidur's documented many-to-one collapse doing exactly what it says, and whether a `not_attempted` finding should read as `ddg_mixed` is the open question — surfaced, not decided.

#### Two more linkers stop declaring passes they never read (WI-zujan, partial)

- **`graphql-resolver-linker` declared five host languages plus `graphql`.** Its only read of pass output is `_get_graphql_schema_symbols`, which filters `ctx.symbols` on `language == "graphql"` and `kind in ("type", "field", "interface")` — nothing else. The resolver side, where those five languages would matter, is read off disk by `link_graphql_resolvers(ctx.repo_root, …)`. Repaired to **`[["graphql", "graphql-sdl-linker"]]`** — the same pair WI-dinum settled for its sibling `graphql-linker`, because both consume the same schema symbols and `graphql-sdl-linker` emits the `kind="field"` records the GraphQL analyzer never has.
- **`message-queue-linker` declared nine host languages and consumes nothing at all.** Its entry point is `link_message_queues(root: Path)`: it takes the repository root, scans the tree itself, and mints *both ends* of every edge, so no pass supplies its source and none supplies its destination. Its clause is now **empty**. Its old comment stated the WI-rasal diagnosis without noticing — "Kafka/RabbitMQ/SQS/Redis pub-sub clients exist across all common backend languages" is a statement about the world, where `depends_on` is the passes whose *output* this pass reads.
- **That collided with a ratified criterion, and the criterion was amended rather than the declaration falsified.** The WI-dilab closure criterion (INV-hujog's gate) requires every Bridge/Framework/Protocol linker to declare a non-empty `depends_on`, and `message-queue-linker` is a Protocol linker. The criterion's premise — that such a linker always reads some analyzer's output — is false for a self-scanner, and forcing a non-empty clause there does not make the declaration true, it makes it a list of passes the linker never reads. The allowance is narrow and **positive**: the module docstring must state, in a fixed phrase the gate greps for, why the clause is empty. **Silence plus an empty clause is still an offence**, because silence cannot be told from having forgotten to declare — the case the criterion exists to catch. Two further tests keep the exception honest: one asserts the branch is actually exercised (a dead allowance should be deleted, not carried), one asserts an undeclared empty clause still trips the gate.
- **Six of the eight linkers in this family remain**, deliberately. `http-linker` (13 declared), `subprocess-linker` (9, scans only `**/*.py`), `orm-linker` (8, scans only `**/*.py`), `grpc-linker` (12, and it reads `ctx.edges` too), `event-sourcing-linker` (6) and `crypto-flow-linker` (10) all genuinely read `ctx.symbols`, so some part of each clause may name a real producer and each needs its own derivation. `http-linker`'s is the hardest: `route_of` accepts both an analyzer-written `framework_role == "route"` marker and a framework-YAML `concept == "route"` entry, so its producer set is not analyzer-only.

#### An edge's own endpoints are not a derivation chain (WI-latup, partial)

- **`explain --provenance` presented a circular restatement as provenance.** Measured on this repository: of 190,109 edges, 45,204 carry `derived_from` and **44,813 of those (99.1%) hold exactly the edge's own `[src, dst]`** — 44,104 from `containment-linker` alone. Only **391 edges (0.2% of all edges)** name anything the linker consumed beyond its endpoints, from two passes (inherited-calls 329, method-call-recovery 62). The 44,813 printed as `Derived from: <src>, <dst>` while edges carrying nothing printed the honest `(no derivation chain — analyzer-produced edge)`. **The display was inverted: the uninformative rows looked documented.**
- **Three states now get three messages.** Endpoints-only is kept distinct from carrying-nothing rather than folded into it — a linker *did* run and *did* record a value, and collapsing them would hide that. The test is subset, not equality, because `inheritance.py:350` passes a one-element list holding the edge's own source, and a subset of the endpoints carries no more than the pair.
- **This changes only what the reader is told.** No producer's output changes and no data is discarded. The other consumer, `finalize.py`'s ADR-0057 §14.1 stub demotion, was already immune — it acts only on a ref naming an unresolved *edge*, and `[src, dst]` are *symbol* ids — so the vacuous values never caused a false demotion.
- **The row's own numbers and scope were wrong, and are corrected on it.** It reported "~22% populated" (counting vacuous values as populated; real coverage is 0.2%) and "~20 linker files, the fix is mechanical". Measured: **93 `Edge.create` sites across 58 linker files already pass `derived_from`, 86 of them literally `[src, dst]`.** So the work is not adding a kwarg — it is replacing a vacuous value, which is a per-linker semantic question. The row's prescribed `derived_from=[source_id_a, source_id_b]` is the exact pattern that produced the 44,104 vacuous rows. **WI-latup stays open**: its stated deliverable is population, and population is not done here.

#### The python DDG walk ran and its answer was thrown away at a dict lookup (WI-ripas)

- **0 of 24,915 python methods matched a `ddg_symbols` key.** `_ddg_taint_reaches` gates on `fn_has_ddg = source_fn in ddg_symbols`, where `source_fn` is the analyzer's symbol id; the python `LanguageDdgSpec` supplied neither `name_for` nor `kind_for`, so `_walk_functions` fell back to the bare `name` field and the constant kind `"function"`. The walk ran, the reaching-definitions solve succeeded, and the result was stored under a key nothing ever constructs — while **21,257 of those methods (85.3%) were in the set** under the unreachable bare-name spelling. 68.5% of python callables could never have `fn_has_ddg` true, so every taint finding rooted in one degraded to `analysis_method: structural` **by construction**.
- **py.py's naming rule was derived from the analyzer, not assumed.** A probe over all four shapes gives `top`, `top.inner` (prefixed with the enclosing *function*), `Outer.meth`, and `Inner.deep` (the *immediate* class, not `Outer.Inner`). So the rule is uniform — prefix with the immediate enclosing scope, one level, class or function — and the kind slot is `method` exactly when that scope is a class. **Nested functions were mis-keyed too**, which the row had not noticed.
- **Measured on this repository: methods 0 → 21,275 of 24,941 (85.3%), functions 69.1% → 71.0%, all callables ~21.7% → 80.8%.** Keys naming no analyzer symbol fall from ~21,600 to 145.
- **This is not merely a re-labelling, and the §3a walk census proves it:** total walks **84 → 122 (+45%)** on this repository against its own claims, same binary otherwise. **But the direction is the one the row warned about** — of the +38 new walks, **+32 escape (`None`) and only +6 establish dependence**.
- **The precision A/B has since been run, and the risk this entry originally claimed does not exist (WI-fazan).** The pre-registered refutation — "if ADR-0046 useful precision drops, the fix is a reporting regression" — rested on a misreading of ADR-0046 that is corrected below.
- **The family is now enumerated.** Go supplies both hooks; rust omits them *with a written reason* (its methods are `function_item` nested in `impl_item`, so the defaults are correct); python — fixed here — and both `ts_def_use` registrations omitted them with no reason. The javascript instance is unfiled and WI-jopuf's P3 pricing does not cover it: that price rests on "TypeScript has 0 catalogued taint sinks", and WI-nonad records that **JavaScript has 83**.

#### A linker that opens the files itself depends on no host analyzer (WI-ditir)

- **`database-query-linker` declared eleven host languages it never reads.** Its second CNF conjunct named `python, javascript, ruby, java, go, csharp, rust, kotlin, scala, elixir, php`, while `_find_source_files` scans four globs (`**/*.py`, `**/*.js`, `**/*.ts`, `**/*.java`) and dispatches to three scanners. Eight of the eleven named neither a pass whose output it reads nor a file it opens — and it consumes no host analyzer's output at all, taking `ctx.repo_root` and re-reading the query side from disk.
- **Too wide is the false-all-clear direction on this axis, and falsification cannot see it.** A clause is an inner-OR, so any single member satisfies it; eight never-scanned languages kept the clause satisfiable on repositories where the linker could not have read anything. `find_falsified_dependencies` is backward-looking and catches only a clause that is too NARROW, so this was invisible to the detector by construction — it was found by reading the body.
- **Repaired to `[["sql"]]`, the shape WI-dinum shipped for `graphql-linker`:** name the pass that supplies the edge DESTINATION, not where the source pattern was found. `sql`'s `kind="table"` symbols are the only pass output this linker consumes. `depends_on` is declarative — consumed by `validate_pass_dependencies` and the falsification report, never by execution — so no linker's activation changes.
- **Residual, deliberately not swept.** The seven sibling linkers the row names were enumerated rather than assumed: `subprocess-linker` declares 9 languages and scans only `**/*.py`; `orm-linker` declares 8 and scans only `**/*.py`; `http-linker` declares 13 and scans 9; `message-queue-linker` declares 9 and reads no `ctx.symbols` at all. But most of them *do* read `ctx.symbols`, so unlike this one they may have a real host dependency and each needs its own verification. Recording the scan capability on `activation` is also left open: `LinkerActivation` has no file-shape field, so that is a core-dataclass change with its own axis-declaration obligations.

#### Every Python method gets an exportedness verdict, and the dead-code view moves a lot (WI-kohah)

- **DISCLOSURE FIRST, because this is the largest behavioural change in the release: the default `dead-code-maybe` view drops from 41.4% to 7.5% dead — 2,468 candidates to 448.** Measured as a clean A/B on identical trees (production functions identical at 5,964, so the only variable is this change): seeds 1,541 → 2,303 (+762); 2,026 candidates disappear, 785 of them methods directly and 1,241 functions that those methods transitively reach. The 34 `__init__` candidates INV-hugit had just made visible go to **0**. Anyone relying on the old numbers should re-baseline.
- **The cause of the old number was an absent measurement, not a negative one.** py.py decided `is_exported` at four sites — module-level variable, class, class attribute, module-level function — and at none for a method. After INV-kubup made the field tri-state, Python methods were the single largest `null` population in the tree, and `--seeds production` (the default: entrypoints + exported public API) simply never saw them. A 41.4% dead rate rested on that silence.
- **The row's own repro was wrong, and correcting it made the fix smaller.** It said "count methods whose `is_exported` is null — it is all of them". Measured: 23,063 of 23,731 (97.2%), not all. The remaining 668 were not noise: the split was exact, zero exceptions either way — every private-by-name method already carried `False` (written by `finalize.py`, which sets `is_exported=False` whenever computed visibility is non-public), every other method carried `null`. So only the PUBLIC half was missing, which is exactly the half that needs the enclosing class's verdict. The asymmetry also ran the unsafe way: the methods that HAD a verdict were the ones that could never be exported.
- **The rule is conjunctive because reachability is:** a public method of an exported class is reachable from outside the package; a public method of a *private* class is not. `__all__` never lists methods, so the class's membership is the authority. Dunders are public protocol, not private — `__init__` of an exported class is reachable by constructing it. The private-name half reuses py.py's own `_python_visibility_modifiers`, the same call that fills this symbol's `modifiers`, so the two facts cannot disagree.
- **Result: 24,929 Python methods, 0 `null`** (24,048 `True`, 881 `False`). The recorded scip-python agreement fixture moves `is_exported` one-sided-to-incumbent 6 → 10 of 10; its comment had been documenting this producer gap as though it were a property of the comparison ("neither arm has a rule"), and now says so.

#### A two-hop receiver has a type, and the linker should not guess (WI-fikoh)

- **`obj.member.method()` reached the `external` placeholder with no receiver hint at all.** py.py's receiver-hint chain is entered only for a bare `ast.Name` receiver (py.py:8093), so a chain whose `func.value` is itself an `ast.Attribute` never entered it. The method-call-recovery linker then had nothing to filter on and bound by **line proximity** — which is how `ts.workspace.add(...)` bound to `TrackerSet.add` instead of `Store.add`, emitted `is_resolved=True` at confidence 0.50. A confidently wrong call edge, not a missing one.
- **The two halves already existed and are joined, not re-derived.** `var_types` types the root from a direct constructor; the intervening member carries `meta["return_type"]` (WI-ribak); `_declared_return_type_name` reads it — including the quoted forward-reference spelling WI-fihun taught it days earlier — and `_resolve_return_type_class` resolves it to a class Symbol. The walk applies them one hop at a time, so a three-hop chain works for the same reason a two-hop one does.
- **Every failed hop is a hard stop, and that is load-bearing.** A minted `receiver_type_hint` is TRUSTED downstream: it bypasses `gate_named_entry` and the ambiguous-names net by design. Falling back to the last class the walk was sure about would stamp `TrackerSet` with full confidence — the original bug, reached from the other side. Both stops are pinned by negative tests that were **mutated and watched go red**, then restored. The `receiver_type_id` is stamped only through the same WI-supat trustworthiness gate the one-hop branch uses, not a looser one.
- **The narrowness is inherited deliberately.** `_declared_return_type_name` infers only from a simple identifier, so `-> Optional[Store]` and `-> list[Store]` stop the walk and stamp nothing. Widening is a separate, measurable change.
- **Measured reach on this repository: 15 of 9,634 `external` placeholders (0.16%) newly carry a hint**, all 15 with a trustworthy id, and all 15 the filed instance's own `Store` shape. The correctness win is verified end to end — on the filed repro the false `TrackerSet.add` edge is gone and `Store.add` is resolved in its place — but the recall effect on this corpus is small and is reported as such rather than as a recall gain.

#### A manifest is not a dispatcher, and a demotion is not a secret (INV-hugit)

- **`dead-code-maybe`'s cross-language demoter counted `.json` / `.yaml` / `.toml` / `.xml` / `.html` as "another language".** Its extension map sent them to a pseudo-language `config`, and a candidate whose short name appeared as a substring in three such files was hard-excluded. The demoter's own rationale is cross-language **dispatch** — an HTTP route, an RPC method, an FFI name — and a YAML file mentioning a Python symbol is a manifest, not a dispatcher. Measured before the change: **112 of 207 demotions (54%) had fewer than three hits in any real other-language file**, including `_label` — a leading-underscore private helper with **zero** code hits, deleted from the report by 24 substring collisions in YAML.
- **The threshold now reads only real other-language files; the manifest tally is kept and published as `config_name_hits`.** Nothing is discarded — the old signal is still on every candidate, it simply no longer decides by itself.
- **The demotions that remain are disclosed, not counted.** A new `cross_language_demoted` listing gives each withheld candidate with both tallies, the treatment `entrypoint_only_dead` and `test_only_reachable_candidates` already had. It is **always present and empty when nothing was demoted**, so a reader can tell "the demoter ran and took nothing" from "this view does not say"; the text renderer gains the same line unconditionally. Shipping the threshold change alone would have been a one-directional loosening, and shipping the disclosure alone would have left the invariant violated.
- **Measured effect on this repository: dead candidates 2,282 → 2,402 (+120), demotions 207 → 87, all 87 now listed.** The 34 non-test Python `__init__` methods that no default run could show come back — the cohort whose invisibility manufactured INV-rolok's *"0 of 2,152 dead candidates are `__init__`"* sizing claim. Returned candidates carry their manifest counts, e.g. `TrackerSet.exists` (2 code hits, 17 manifest) and `PasswordVerifier.verify` (2, 11).
- **`DEAD_CODE_MAYBE_SCHEMA_VERSION` 0.2.0 → 0.3.0**, additive: nothing removed or renamed, so a 0.2.0 reader keeps working.
...
- **Root cause: `smart-test` decides which repo it is talking about from its own location, never the caller's cwd.** It does `REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"`, then `cd "$REPO_ROOT"`, then writes the literal relative path `.ci/affected-tests.txt`. So invoking `/A/scripts/smart-test` from inside repo `/B` does not produce `/B`'s manifest and does not fail — it silently rewrites `/A`'s **committed** artifact. Ten `tests/test_autopr_*.py` modules drive the real `scripts/auto-pr` with `cwd` set to a throwaway fixture repo, and `auto-pr` regenerates the manifest before pushing.
- **In CI the Woodpecker steps share one workspace, which turned a local nuisance into a gate failure.** The `forge-arms` step installs pytest and nothing else and runs `tests/test_autopr_*.py`; with no hypergumbo on `PATH` the regen took the "no stable hypergumbo found" branch and wrote a full-suite manifest over the checked-out one. The `pytest` step, which waits on `build-grammars`, ran much later and read it. The case self-healed when the offending test was made hermetic, which is why it looked inexplicable rather than reproducible.
- **Fixed at the writer:** `smart-test` refuses, with exit 3, when the caller's cwd is inside a *different* git repo, naming both roots and both ways out. Scoped to the case shown to bite — a cwd in no git repo at all names no competing manifest and keeps today's behaviour.
- **Fixed at the reader, too:** the per-PR `pytest` step now reads the blob committed at `HEAD` rather than the working-tree file, prints the header of the manifest it actually read, and discloses any disagreement between the two. It **discloses rather than fails** — measuring the committed artifact is the safety property; turning an unexplained rewrite into a red PR is the recall-loss direction.
...
- **The parent hands off *before* doing any work**, which is what leaves `do_pr` — 600 lines carrying the poll, the soft-retry, Scenario B and the merge cascade — completely untouched: the child is an ordinary run, not a resumed half of one. The push is idempotent for exactly this reason (an existing open PR for the branch is reused). **`--detach` is never forwarded to the child**, pinned by a test on the argv the parser actually builds, because a child that re-detached would fork-bomb the machine.
- **`auto-pr wait` is three-valued and never collapses them.** 0 = the watcher finished, 2 = still working (call again next turn), 1 = no watcher *or it vanished mid-poll*. A handle whose watcher died (machine sleep, session teardown, OOM) is byte-identical to a live one unless the pid is checked — reporting that as finished would hand the caller a merge that never happened. An unreadable handle is refused outright rather than guessed at, since `json_field` renders null, missing and unparseable identically.

#### A run that was KILLED says so, instead of reporting INV-rahib's violation signal (WI-katap)

- **`auto-pr` trapped `EXIT` and `ERR` and no signals at all**, so a killed run had no code path that could name what happened to it. Measured on the convergence ledger: the undeclared-terminal-state class — which owner ruling 2026-08-03 makes *the* violation signal — was **22 of 84** post-instrumentation runs (26%), up from 14 of 485 (2.9%). Not one carried an `abort_site`, because WI-nazoj's ERR trap cannot see a signal; a surviving run log named it (`Terminated`). The caller wraps `auto-pr` in `timeout 590` because its harness caps one foreground call at 600s, while `auto-pr`'s own budget is 2400s plus a **300s soft-retry that is not optional** — so the kill is structural, not bad luck. **14 of the 15 PRs behind those rows had merged**: the violation signal was firing on runs that succeeded.
- **`final_state` was carrying two facts** — *reached the end without deciding* (the convergence bug) and *was killed before it could decide* (not a convergence property at all, since the run never got to converge). That is this repository's recurring absent-versus-empty defect, and it made the refined criterion "zero `unknown` rows over a window" unsatisfiable. `terminated_sigterm` / `terminated_sigint` / `terminated_sighup` are a **fourth bucket**, folded into neither convergence nor failure; the signals stay apart because Ctrl-C and a supervisor's timeout are different facts.
- **The signal is recorded in its own field, independent of the outcome.** A run that merged and was then killed during housekeeping keeps `merged` and still reports `terminated_by` — precisely the fact INV-rahib's Class 1 investigation had to reconstruct from a surviving terminal log. Trap installation moved behind one `_autopr_install_traps` boundary rather than three hand-written sets, the shape `_autopr_post_merge` already paid for when its structural test found a fourth call site that reading had missed.
- **The audit reads the fourth bucket** and refuses to launder it: a terminated run is disclosed but counted neither way, a converged verdict now names its **deciding denominator**, a ledger of nothing but kills is `CANNOT CONCLUDE`, and undeclared rows that predate the traps are reported as **unattributable** rather than as kills or as fall-offs. Historical rows are not reclassified, because the fact was never recorded.
- **`AGENTS.md` states the budget rule**: `auto-pr` is run detached, never wrapped in `timeout`. The prior session rediscovered `nohup` and then reverted to `timeout 590` within the same session — the knowledge existed and did not persist, which is what a written rule is for.

#### One callee-name reader, so §14's supersession key stops missing escaped names (INV-difud)

- **Two readers, two orders, one fact.** `verify_claims._callee_name` read `meta["callee_name"]` — the lossless home `make_unresolved_edge` stamps — and fell back to the span-anchored `symbol_name_slot`. `finalize._stub_callee_name` read `dst_ref.name` and then parsed the id **positionally** as `parts[-2]`, consulting the lossless home never. On this repository's own survey they returned different strings for **12 of 74,816** external-dst edges, every one a name whose id slot had escaped `::` to `..` (`String..from_utf8` against `String::from_utf8`). Since `_stub_callee_name` is the callee half of ADR-0057 §14's four-part supersession key and is compared against a lossless `Symbol.name`, each mismatch silently **missed** a demotion — a false negative in the direction that leaves a wrong answer at equal standing with the right one. Both now delegate to `ir.callee_name_of`, whose order is `meta["callee_name"]` → `dst_ref.name` → `symbol_name_slot`, and which returns `""` rather than the raw id when the id yields nothing, so an unreadable name cannot compare equal to a readable one. The positional read was the third appearance of the INV-fokik / INV-divuf pattern; `io_boundary._extract_callee_name` is deliberately **not** folded in, since it takes an id string and can read neither meta nor ref.

#### ADR-0057 §14.1: a resolution demotes the stub it NAMES, and only a measured pass may (WI-latup / WI-togad)

- **§14's same-site rule reconstructs the supersession relation; the consuming pass already knows it.** Matching `(src, call line, edge_type, callee name key)` over the post-dedup graph cannot tell a *refinement* of a stub from a *collision* with an unrelated call on the same line — which is what produced the 45 false demotions Open question 4 found. A resolved edge whose `Edge.derived_from` names an unresolved edge now demotes that edge directly: no key, no line, no candidate set, no ambiguity to refuse. It runs before the same-site rule and stamps the same `meta` keys, which that rule then skips, so a stub is halved once however many rules reach it.
- **`derived_from` was populated and said nothing.** Measured on this repository: 48,879 edges carried it and **100% of them were the edge's own `[src, dst]`**, with **zero** references to an edge — the tautology WI-ratun recorded, on every linker that populates the field. The two recovery passes now record what they actually consumed: `inherited-calls-linker` names the unresolved edge plus the class its walk started from, `method-call-recovery-linker` the unresolved call plus the class-hint edge. **393 edges now carry an edge reference**, the first in the corpus.
- **The permission to demote is declared per pass and granted on measurement**, because demoting a stub ranks a producer's positive answer below the consuming pass's own. `RegisteredLinker.supersedes_consumed_stub` defaults to `False`; the evidence table is [`docs/audits/0020`](docs/audits/0020-recovery-linker-supersession-precision.md). **Granted to `inherited-calls-linker`** — 70 of 70 audited correct, 329 of 329 consistent with the declared inheritance graph — which restores **329** of the 374 supersessions the Open-question-4 ruling removed, every one stated rather than reconstructed. **Refused to `method-call-recovery-linker`**: 16 of its 64 edges are wrong and the one shipped quality signal does not separate them — `disambiguation_fallback` is set on 5 of the 16 wrong and 13 of the 48 right, so gating on it would discard 28% of the wrong and 27% of the right. It stamps its consumed edge anyway; the provenance is true, the demotion is not licensed.
- **Not done: WI-latup's wider sweep.** The other ~56 linker files that populate `derived_from` still write their edge's own `[src, dst]`. Only the two passes with a consumer for the field were changed; the rest is judgement-heavy per linker and has no consumer demanding it yet, so the row stays open with the measurement attached rather than being closed on two files.

#### A producer declares the origin its records carry (INV-gabak)

- **`Edge.origin` is the `pass-id` axis, and for the two SCIP arms it is not the registration name.** They register as `rust_analyzer` / `scip_python` and their records carry `scip`, because the translation that emits them is shared by both arms and is a pass in its own right — `catalog._SYNTHETIC_PASS_IDS` declares it as one, per ADR-0044. That design is fine; what was missing is that nothing said so where a consumer could read it. `RegisteredAnalyzer.emits_origin` now states it, and `None` keeps meaning "the registration name" for the hundred-odd producers where that is the truth, so the declaration stays a marker of the exception rather than noise on every row.
- **ADR-0057 §14's origin condition was reading the right answer off the wrong field.** `anchored_producer_tokens` bridged the gap by unioning `RegisteredAnalyzer.backend`, which happens to spell `scip` for both arms — but `backend` is the parsing-backend label, and the registry's own docstring says it is "decoupled from pass_id so the ID stays stable across backend swaps". A backend swap, or a producer whose two labels differ, would have disarmed the supersession rule with no error; it also admitted the incumbent's `ast` as a Python producer token. It now reads `emits_origin`, and `anchored_producer_tokens("python")` is exactly `{python, scip_python, scip}`.
- **The declaration is checked against the emission, in each arm's own package.** A recorded-fixture test per backend asserts that every symbol and edge the translation produces carries the origin that arm declares, with a non-empty-output guard so it cannot pass vacuously. A declaration nothing checks is a claim in a costume — this one is load-bearing for whether §14 recognises the producer at all.

#### A producer that named the callee's module is no longer overridden by a name match (WI-rulik)

- **`method-call-recovery-linker` resolves by class-hint-and-short-name with a line-proximity tiebreaker and no receiver evidence** — and it was applying that to unresolved calls whose producer had already stated WHERE the callee lives. `dst_ref` with a module path is ADR-0035's complete external identity key, stated positively or abstained per ADR-0057 §15.1; the pass that read the imports and the scope made that claim, and a short-name match is strictly less evidence. An in-repo answer therefore **contradicts** it rather than filling a gap — which is what §11 settles by keeping both edges and §14 (as of this release) declines to demote. The linker now refuses outright, applying the same rule one stage earlier, at the pass that would otherwise manufacture the contest.
- **Measured on a single-backend Python self-survey: 92 → 64 edges, and all 28 removed were wrong.** `tree_sitter.Language(tree_sitter_rust.language())` bound to the `Symbol.language` **field** (19 sites), `id(stub)` to `Symbol.id`, `type(e).__name__` to `IpcPattern.type`, `os.environ.get` to `TrackerSet.get`, `Path.exists` to `TrackerSet.exists`. 26 of the 28 targeted a dataclass field. `inherited-calls-linker` is untouched at 329 edges and the §14 demotion count stays 0.
- **The refusal reads the unresolved edge the recovery was derived from, not the call site.** `a.b(c.d())` puts two calls on one line, and one sibling's module claim says nothing about the other; a site-keyed test would have refused 7 recoveries whose own stub abstained. `ir.stated_module_of` is the one reader of the partial/complete distinction, shared with §14 and §15's fold.
- **Not asserted: that this linker is now correct.** Precision on this corpus moves **51% → 73%**; 17 of the remaining 64 edges are still wrong, every one a receiver-blind mis-bind where the stub abstained and the line-proximity tiebreaker chose a same-named method on an unrelated class (`AnalysisRun.to_dict` → `Limits.to_dict`, `Store.add` → `TrackerSet.add`). Two more bind a `calls` edge to an `int`-typed field. Those are filed separately; they need receiver-type evidence the Python producer does not stamp for these shapes, not another name-space gate.

#### ADR-0057 §14: only an anchored producer supersedes a stub (Open question 4 ruled)

- **The origin condition tested pass-id inequality and stood in for a test on independent observation.** §14 demotes an external stub when a resolved edge at the same site carries "a producer the stub's own `origin` lacks". On a single-backend Python run of this repository that demoted **419** stubs, every superseder a **linker** — and those 419 superseders were **419 of the 421 edges** `inherited-calls-linker` (329 of 329) and `method-call-recovery-linker` (90 of 92) emitted. Each superseding edge is that linker's own resolution of the very stub it superseded, built from the stub's `src`, `line` and parsed callee, so the four-part key matched **by construction**. The pass was not detecting a contradiction; it was rediscovering a derivation relation the producing pass already knew. §13 case 2 in its contrapositive: a refinement of one observation is not a second observation.
- **45 of the 419 were false, every one from `method-call-recovery-linker`** — whose receiver-type filter drops a class hint that *contradicts* a declared `receiver_type_hint` but abstains when none is stamped, and none of the 90 carried one. **28** were stubs whose external key was **complete and correct** — `tree_sitter_rust.language` (19), `builtins.id`, `builtins.type`, `os.environ.get`, `pathlib.Path.exists` — ranked below a same-short-name in-repo **field** (`Symbol.language`, `Symbol.id`). The other 17 abstained and were still mis-bound (`AnalysisRun.to_dict` → `Limits.to_dict`, `re.Match.start` → `ItemIdMatch.start`, `Store.add` → `TrackerSet.add`). `inherited-calls-linker` was right on 70 of 70 audited and consistent with the declared inheritance graph on 329 of 329.
- **Two conditions added, for six in all.** The stub must have **abstained on its module** — `ir.stated_module_of` is now the one reader of that fact, shared with §15's fold so the two rules cannot drift; a complete key *contradicts* an in-repo answer rather than being filled by it, and §11 makes a `dst` disagreement two edges. And **both sides must be an anchored producer's edge**: `registry.anchored_producer_tokens(language)` must intersect `resolved.origin − stub.origin`, and `stub.origin` too. Both the analyzer name and the backend name are accepted, since the SCIP arms stamp `origin="scip"` while registering as `rust_analyzer` / `scip_python`.
- **The language gate stays a registry query** and is now provably redundant rather than load-bearing. `analysis_runs` carries no pass-kind field and so cannot answer "did a second producer observe this" without the registry anyway, while `Edge.origin` answers it per edge — §15.3's ruling. What `merge_participants` supplies is a *declaration* (the incumbent's `name_key`), not an observation. Measured: **419 → 0**, no node or edge added or removed, no `confidence` or `is_resolved` changed, and the only `rank_score` changes across 207,486 shared edges are those 419. Both two-backend recorded fixtures are untouched (aardvark-dns 3, scip-python sample 1), so **a single-backend run is byte-identical through §14**, as it already was through §3.
- **The cost, stated:** 115 hand-verified-correct and 259 screened-but-unread supersessions go unrecorded until the consuming pass stamps the relation itself (blocked on WI-latup's `derived_from`). `superseded_by` is a positive claim a consumer is invited to trust, and one in ten of them pointed at `Symbol.language` for a call to `tree_sitter_rust.language()`. The old control test could not have caught this: it popped the producer from the **registry**, which short-circuits the pass before the origin gate is reached. Its replacement leaves the registry as production has it and varies what is in the graph.

#### A `backend-agreement --out` write bypassed its safety-zone wrapper

- **INV-zudak's live guard caught a ninth call site.** `cmd_backend_agreement` wrote its rendered table with a bare `Path(args.out).write_text(text)`, so the zone-barrier sanitizer did not apply and the write landed as an unsanitized `host_fs` flow. It now goes through `safety_zones.user_out_write`, like every other user-supplied-path write on the runtime CLI path. The guard is the rule-over-the-live-catalogue test that INV-zudak's May per-site closure was replaced by, which is why this was a red on `dev` rather than a discovery.
...
- **All 51 now name their pathway**, each chosen from the canonical `inference_pathway` axis against what the site actually read — `ast_import` / `import_declaration` / `include` for import constructs, `ast_call` (or `ast_call_direct` for a bare-name local call) for call expressions, `enclosing_scope` for a declaration nested in its parent, `reference` for a resolved name reference, `build_dependency` and `build_target_main` for build declarations, `source_statement` for a `source`, `naming_convention` for Go's build-tag alternatives, `dispatch_pattern` for the route→handler linker. `cli.py` now uses `Edge.from_dict`, the one deserializer, which reads `meta.evidence_type` back — the hand-rolled version also dropped `meta`, `is_resolved`, `dst_ref` and `confidence_source` on every reconstructed edge.
- **A gate makes the omission impossible** rather than detecting the phantom downstream: `producer_coherence.edge_sites_without_evidence_type` is wired into the L3 producer-coherence linter (pre-commit + CI), so a new `Edge` / `Edge.create` under `packages/*/src` that does not name its pathway fails. `**kwargs` forwarding is not reported — what it carries cannot be read statically — and tests are excluded, as they are for the sibling registry checks.
- **`Edge.confidence` was audited too and is NOT the same defect.** `Edge.create` always computes a value — explicit, derived from the pathway, or the unseeded 0.85 fallback — and stamps `confidence_source` accordingly. Measured on 207,779 edges: **zero** carry the fallback; the 157 `emitter_constant` edges on unseeded pathways all carry explicit producer constants (0.9, 0.7, 0.95, 0.75). No producer abstains on confidence, so the merge pass's candidate set cannot contain an unobserved one.

#### `is_exported` says what a producer measured, or says nothing (INV-kubup)

- **The artifact asserted "not part of the public API" for every analyzer that has no exportedness rule.** `Symbol.is_exported` was `bool = False` with no absent value. Seventeen analyzer modules compute exportedness (Python `__all__`, the JS/TS `export` clause, Rust `pub`, C#/Scala/Java visibility, Dart/Julia/Haskell export lists and naming); around ninety do not, and every record they emitted carried a `false` that no consumer could tell from a measured one. On this repository's own survey that was **36,659 of 52,728 records** — 29,505 of them with no visibility signal of any kind. The WI-zimum dead-code seed set, `cli`'s `exported_symbols` count and the WI-zafab `--exclude-exported` filter all read that field.
- **`Optional[bool]`, and only a positive input fills it.** An analyzer's own rule writes `True` or `False` and is believed; an export modifier (`pub` / `public` / `exported` / `pub(...)`) fills `True`; a language signal that the symbol is not public at all fills `False` in finalize, since public-API membership needs public visibility (INV-jusot). Nothing else fills it — in particular the supply-chain classification fold, which was spelled `is_exported or is_exported_from_modifiers(modifiers)` in **two** copies (`cli.py` and `sketch.py`) and resolved every unobserved record to `False`, now only upgrades on the modifier's presence. Measured on the same repository, per record: **28,049 `false` → `null`**, 7,902 `false` kept as real measurements, 15,844 `true` unchanged, and **not one `true` lost**. By kind the moved population is 23,466 methods (`py.py` has a rule for module-level constructs, not for methods), 2,049 external symbols, 1,890 file anchors and 270 call sites.
- **The JS/TS analyzer now records both directions.** A module's `export` clauses are the whole of its public API, so a declaration absent from them is *measured* as not exported — including in a file with no exports at all, which is a measurement that nothing is exported rather than an absence of one. The synthetic per-file anchor is left undecided: the field is about declarations, and this analyzer has no rule for the anchor.
- **Readers that test truthiness are unaffected**; a reader that wants to distinguish "measured not exported" from "nobody looked" can now do so. `SCHEMA_VERSION` advances 0.20.11 → 0.20.12 and `supply_chain.is_exported` widens from `boolean` to `boolean|null`.
- **A consequence inside ADR-0057:** the abstention-blind set is DERIVED from `Symbol`'s field defaults, and `is_exported` was its last member — so it is now empty, every anchored producer rules `observes=()`, and the INV-huboz declaration machinery goes dormant. It stays armed rather than deleted: a tracked attribute that gains a concrete default repopulates the set the day it lands, and the registry contract test then refuses every producer that has not re-ruled. The derivation is exercised on a record type that has such a field, because a guard whose live set is empty is otherwise only testable by regressing the tree.

#### A producer contributes a candidate only for what it observed (INV-huboz)

- **A field whose default is a concrete value cannot say that nobody looked, so its producers say instead.** ADR-0057 §1 rules that a producer which did not observe an attribute contributes no candidate for it, and for nine of the ten attributes the merge pass tracks the value carries that: the `Symbol` default is `None`, `""` or `[]` and `_candidates` skips it, per record. `is_exported: bool = False` cannot — a default `False` is indistinguishable from a measured one — so both SCIP arms, which assign the field nowhere, contributed a phantom `False` candidate on **every record they emitted**. On the recorded aardvark-dns fixture that was 41 contests the arbitration then resolved and 107 agreements it reported as two producers concurring; on the scip-python sample, 6 and 4. All of it read as evidence in the `docs/audits/0019` table, which is the only thing ADR-0057 §5 lets change an arbitration default.
- **The cure is a declaration on the producer contract, not a change to arbitration** (which was correct). `MergeAnchor` gains `observes`: of the attributes whose value cannot express abstention, the ones this backend computes. `merge_producers.ABSTENTION_BLIND_ATTRIBUTES` is **derived from `Symbol`'s own field defaults**, not listed, so a tracked attribute that gains a concrete default appears there the day it is added; declaring an attribute that *does* express absence by value is refused at construction, since the record already says so per record and a declaration could only lose that. `observes=None` is not a ruling: the live-registry contract test refuses any anchored producer of a shared language that left it unset, and the merge pass takes no blind-attribute candidate from one. Declared: `rust` and `python` compute exportedness (`pub` modifiers; `__all__` and the underscore convention), `rust_analyzer` and `scip_python` compute none. The `is_exported` row of `docs/audits/0019` moves from 107 agree / 41 disagree to **148 one-sided to `rust`**, and the scip-python fixture's from 4 / 6 to 10 one-sided to `python`; no other row, pairing or edge count moves, and no carried value changes — only the provenance the artifact claims for it.
- **Not cured here, filed:** `Symbol.is_exported` still asserts `False` in the ARTIFACT for every symbol whose analyzer never computed it, which the finalize fold entrenches and the dead-code seed set reads (INV-kubup); and `Edge.confidence` / `Edge.evidence_type` have concrete defaults of the same shape, unaudited, though an edge only folds when both producers emitted it (INV-nudoj).

#### The measurement instrument reads a record where it serialises (WI-pofih)

- **`hypergumbo backend-agreement` printed `null` as the carried value of a nested attribute.** The disagreement-shape column rendered the scalar a merged record carries with a top-level lookup on the serialised node, and `is_exported` is the one of the ten attributes the merge pass tracks that `Symbol.to_dict()` does **not** write at the top level — it nests under `supply_chain`. So `docs/audits/0019` reported `is_exported` as `null ← false` ×41 for 41 records that carry `true`, and a wrong instrument exits 0. The lookup now finds an attribute in the record's field groups as well, skips the open payloads (`meta`, `quality`, the provenance slot itself) whose keys are not field names, and renders `(not serialised)` — never `null` — when the attribute is carried nowhere: an absence a reader can mistake for a value is what made this wrong. Only the shape column moves; the agree / disagree / one-sided counts come from `attribution` and `alternatives` and were always right. The committed table and the pinned fixture shape are regenerated, and the reading this defect produced (INV-huboz as originally filed, against the merge pass) is corrected in `docs/audits/0019`, ADR-0057 §5 and the recorded fixtures' own comments.
...
- **The registry is now gated at decoration time.** Every effective language must be a taxonomy key (`LANGUAGE_ALIASES` keys are canonicalised) unless the registration declares a `language_state`: `no_taxonomy_spec` for a real language without a spec (none of its languages may be a taxonomy key — the declaration goes stale and raises the moment a spec lands), or `no_language` for a pass that is not a language (`languages` stored as `[]`, and the two downstream `if languages else {name}` fallbacks in finalize and the file-presence pre-filter are removed so an empty declaration is never re-inflated into a phantom). An undeclared non-language name **raises `LanguageDeclarationError` at import**, and entry-point discovery re-raises it instead of logging it away — a misdeclared analyzer must not silently vanish from the catalogue. `make` declares `languages=["makefile"], backend="tree-sitter"` and its symbols, file node and id prefix now carry `makefile`; `rust_analyzer` declares `languages=["rust"], backend="scip"` and `rust` declares `backend="tree-sitter"`, so the registry can tell for the first time that they are two backends for one language (the `languages` half of ADR-0057 §10; `backend` was set on 0 of 118 registrations); `manifest_targets` declares `no_language`; the 32 declare `no_taxonomy_spec` and their coverage gap is filed as WI-futin rather than widened into this change. Consequences on the measured repo: `skipped_languages` is absent, `rust_analyzer` is profile-gated on `.rs` files like every other analyzer instead of being dispatched unconditionally, and `all_known_languages()` no longer contains `rust_analyzer`, `manifest_targets` or `make`.
- **Two consequences of one language having two producers, found by the tests the gate tripped.** Every place that mapped a language to *the* analyzer assumed one producer: the profile looked its `find_files` enumerator up with `get_analyzer(<language>)` — a lookup by pass *name*, so the INV-hokig cure fired for the six analyzers whose name equals their language and never for `make` — and three drift-guard tests keyed a dict by language and silently kept whichever of `rust` / `rust_analyzer` registered last. The registry gains `analyzers_for_language()`, the one answer to "who produces L?" in the taxonomy vocabulary, returning every producer in priority order; the profile uses it (a `GNUmakefile`-only repo now counts as one `makefile` and is not short-circuited — `make` declares `find_files`, since being profile-gated re-opens INV-hokig for it); and the five linkers whose impl-side clause named `rust` now name `rust_analyzer` beside it (`jni`, `pyffi`, `tauri-ipc`, `wasm-bindgen`, `inherited-calls`), because a dependency on rust symbols is satisfied by either backend — the shape `type-hierarchy-linker` already had.

#### Tracker op durability

- **Ops self-healing could be switched off by a file nobody owned, and stay off — observed for ten hours.** `.git/tracker-recover-disabled` is how `do_sync` and `auto-pr` stop the `reference-transaction` and `post-checkout` hooks from fighting their own reconciliation; while it existed both hooks skipped `tracker recover`, so a `git reset --hard` or `git checkout` dropped pending `.ops` with nothing to restore them (the out-of-repo journal still held the data — degraded safety, not loss). `do_sync` cleared it in a `finally` and `auto-pr` in an EXIT trap, and **bash runs that trap on SIGTERM and SIGINT but not on SIGKILL**, so a hard kill left it behind — and a bare file cannot say whether anyone still owns it, making a leak and a healthy 45-minute `auto-pr` the same observation. It is now an **fcntl lock**, which the OS releases even on SIGKILL: the file's existence means nothing and only a live holder suppresses anything, so the shape that caused the incident is inert and the hooks self-heal through it. This is the move WI-nutin already made for the sibling marker `TRACKER_SYNC_PENDING`, whose `SyncGate` docstring states the reason. **One way it can still outlive its owner, measured rather than assumed:** every git and curl `auto-pr` forks inherits the lock fd and bash has no close-on-exec escape (`exec {var}>` does not set it — `/proc/<child>/fd` still showed the marker), so a killed run keeps the lock until its orphaned children exit. That is normally seconds; when it is not, the lock body records `pid`/`started` and every tracker command reports a lock whose recorded owner is gone. The wall-clock grace that preceded this is deleted — a lock answers "is anybody there" as a fact, so there is no constant left to re-measure whenever `auto-pr`'s phases move.

#### auto-pr convergence (INV-rahib)

- **41 of 481 recorded `auto-pr` runs merged successfully and exited 1.** Read off `.git/AUTOPR_HISTORY.jsonl`, the convergence ledger this item added and nothing had ever consumed; every one of those 41 `merged_sha`s is an ancestor of `origin/dev`, against 358 of 359 for the runs that also exited 0. The cause is `set -euo pipefail` plus bare commands in the tail that runs *after* `_autopr_state_final="merged"`: `git checkout "$base"`, which refuses whenever a dirty file differs between the feature branch and the base — a modified tracker `.ops` file is exactly that, and is this working tree's ordinary state — and the `read -p "Retry?"` prompt, which returns 1 at EOF, i.e. on every non-interactive run. Either one killed the script after the merge had landed, and the EXIT trap then wrote a sentinel reporting a merge and a failure at once. **Once the merge has landed the outcome is decided**, so the housekeeping now runs through `_autopr_post_merge`, whose `if !` context makes bash ignore `set -e` for the whole callee body including anything it calls, and the merged paths `exit 0` explicitly rather than inheriting the exit status of branch cleanup. A boundary rather than three guards, because the defect is the *absence* of one — and the structural test that pins it found a **fourth** call site (`_autopr_handle_already_merged`) that reading the post-merge tails had missed. `gov_cleanup` additionally recorded its terminal state *after* its housekeeping, so an abort there discarded the merged fact entirely and self-reported as `unknown`; it now records first. The retry prompt is guarded on a tty, which is also a small admission: `cleanup_local`'s documented three-attempt loop had never run past attempt 1 in production, because attempt 1's prompt was an abort rather than a question.
- **The test harness that owned `cleanup_local` ran `set -uo pipefail` for its whole life.** Production runs `set -euo pipefail`; a harness missing the `-e` cannot observe an abort-shaped defect at all, which is why the one test file covering this function watched the failure class go by for six weeks. It now runs production flags, and the new behavioral tests drive the real extracted functions under them.

#### The convergence ledger gets a reader (WI-lapap)

- **`scripts/audit-autopr-convergence` reads what `auto-pr` has been writing.** The ledger was added so INV-rahib's question — does `auto-pr` always end in a declared terminal state — could be answered over a sequence of runs rather than organically. Nothing then read it: 483 rows accumulated over six weeks holding 55 violations, and they surfaced only because someone opened the file by hand. An instrument with no reader is the same organic criterion in a different costume. **The verdict is three-valued** — `converged` / `violation` / `cannot conclude` — and the third value is the point: a missing, empty or partially unparseable ledger yields *no* all-clear, because this project's recurring defect is a zero meaning "never modelled" being read as a zero meaning "nothing wrong". An unparseable row downgrades the verdict rather than quietly shrinking the denominator.
- **It is wired into session start, not only into the stop hook.** The stop-hook nudge would have been dead code in the mode this repository actually runs in: `.agent/hooks/claude-code/stop.sh` approves and exits when `AUTONOMOUS_MODE` is `OFF`, **before** it sources `stop_logic.sh`. Shipping only that wire-up would have reproduced WI-lapap one level up — a reader nobody runs, stacked on a ledger nobody read. The session-start helper fires in every mode branch, and a test pins the `stop.sh` ordering that makes it necessary, so the redundancy can be removed deliberately if that ever changes rather than discovered by accident. Both nudges are **windowed** (default 25 invocations, configurable): an append-only ledger never forgets, so a nudge over all history would fire forever on a backlog nobody can retire and be tuned out within a week.

#### The pass-silence axis gets a reader that unions its two hosts (WI-mamiv)

- **The split is not what it says it is.** The two hosts are documented as one channel split by *"did the pass run?"*. Measured on component-model-demo: **130 of 162 passes carry `no_candidate_files` — 48 on one host, 82 on the other**, and the host is decided by `taxonomy.LANGUAGE_EXTENSIONS` membership and by whether the analyzer returns a bare `AnalysisResult()` or a run with `files_analyzed=0`. `apex`, `twig` and `make` report one way; `java`, `matlab` and `scss` report the identical fact the other way. That is producer registration and implementation detail, not a property of the silence — now written down at the fields instead of inferred.
- **`census_silence()` unions the two**, preserving each host's own absence rule (empty on `analysis_runs` is NOT APPLICABLE and is omitted; missing on `skipped_passes` is `unreported`, never the 98.94%-common value). A consumer asking "which passes had no input?" no longer has to know about the WI-jadig pre-filter to get a right answer — the two stderr lines report `64 of 79` and `83 of 162`, different denominators over overlapping populations whose sum is not the answer either.
- **It takes the pass catalogue, because a union over `runs + skips` is structurally blind to the third outcome.** A pass can reach **neither** host — that is how `rust_analyzer` put 669 nodes and 637 edges into artifacts while appearing nowhere in the catalog (WI-didag). `unaccounted` is `None` when no catalogue was supplied and `()` when one was and nothing was missing: an empty tuple claims "I checked", `None` says "I could not".
- **The reader is wired, and that is pinned by a test.** `emit_unaccounted_warning` runs on every survey next to the two existing summaries, so the signal that would have surfaced WI-didag on the run that introduced it now fires instead of waiting a month. An AST test asserts `cli.py` *calls* it — importing it is not a reader.
- Whether the **routing** should change, so the host becomes a function of the fact, is left open: it touches the WI-didil AR-or-skip completeness contract.

#### The file-presence pre-filter skipped passes that had work (INV-homur)

- **A pass was recorded as `no_candidate_files` — "ran and found nothing" — when it had never looked.** The WI-jadig pre-filter short-circuits an analyzer when `profile.languages` reports zero files for every language it declares, and the profile counts by globbing the ADR-0004 taxonomy. That is sound only while the taxonomy's patterns are a *superset* of what the analyzer reads. Six analyzers had drifted past it: `ini` also reads `*.conf` / `.editorconfig` / `.flake8` / `.pylintrc`, `cpp` and `objc` also read `*.h`, `fortran` also reads `*.F03` / `*.F08` / `*.F95` / `*.fpp`, `javascript` also reads `*.cts` / `*.mts`, and `verilog` also reads `*.vh`. A repo of `.mts` modules had the whole JS/TS pass skipped; a Verilog project using `.vh` headers got zero symbols. Measured on a 19-file Rust crate whose only INI-family file is a `dnsmasq.conf`: **symbols 359 → 367** once the pre-filter stops guessing. The out-of-taxonomy population was never affected — those hit the pre-filter's fail-open branch and are dispatched defensively.
- **INV-hokig had already named the invariant and shipped the cure; it was wired for one analyzer.** `@register_analyzer(..., find_files=...)` makes the profile count with the analyzer's own enumeration instead of an extension glob. `bash` used it; `ini`, `cpp`, `objc`, `fortran` and `verilog` each *had* the function and never passed it. `javascript` takes the taxonomy route instead — it declares four languages, so its `find_files` would make the `javascript` count swallow every `.ts` file and double-count against `typescript`. `*.h` is why widening the taxonomy alone cannot work: it is claimed by `c`, the extension map is extension-to-one-language, and `find_cpp_files` resolves it by content (`.h` is C++ only when `.cpp`/`.cc`/`.cxx` sources exist) — which no static glob expresses.
- **The class is gated, not the instance.** A test asserts that any in-taxonomy analyzer whose `file_patterns` exceed its taxonomy extensions declares `find_files`. Derive-or-gate, rather than a second list to keep in sync. `RegisteredAnalyzer.find_files` widens `list[Path]` → `Iterable[Path]`, which is what the sole consumer (`set(...)`) actually requires and what three of the five finders already returned.

#### Every SCIP edge was sourced from the file, never from the function that made the reference (INV-mofiv)

- **182 of 182 SCIP edges on `aardvark-dns` named a module namespace as their caller; none named a callable.** `scip/calls.py` attributes a reference to the innermost Definition whose *range* contains it, and rust-analyzer's Definition range is the identifier token, one line long — `increment` is reported at line 14 while its body runs 14–17. No body reference is ever inside a function's token, so the only Definition that contained anything was the document's `crate/` namespace spanning the whole file. That is why the arm whose purpose is type-resolved call attribution produced zero call-family edges: there was never a caller to specialize from.
- **The item range was in the index all along.** The shim's docstring said `enclosing_range` "is rarely populated in practice". Measured on rust-analyzer 1.94.0: it is populated on every Definition occurrence, and it is exactly the item range (`increment` `[13,4,16,5]`). The definition list is now built from `enclosing_range` when present, falling back to `range` for emitters that leave it empty, which keeps their behaviour unchanged. Gated per shape: token-plus-enclosing-range attributes to the function, token alone attributes to the namespace, a malformed `enclosing_range` falls back, nested items pick the innermost, and the production shape is pinned end to end at `translate_scip_to_hg`. The three new tests were red against the old shim.
- **Measured on the same crate**: SCIP edges sourced from a method 0 → 287, from a namespace 182 → 63 (file-scope `use` and item references, which are correctly the file's), from a class 0 → 9 and a variable 0 → 10; SCIP edges 182 → 369, because references from different callers to one target no longer collapse into a single namespace edge; genuine cross-file references 99 → 157; every non-SCIP origin byte-identical. `Symbol.span` is deliberately left at the token range — moving it to `enclosing_range` makes the `rust.py` stable_id parity hold and the within-file collision splitter would then re-mint the SCIP record as a second site, so that is filed as INV-lodum, gated on WI-gojum.

#### Three shipped claims about the two Rust arms are retracted (INV-dolud, WI-kanuh)

- **"Cross-pass dedup works" did not, and never had.** The rust-analyzer README, a 7.0.0 changelog entry and the spec's `stable_id` history all asserted, in the present tense, that the SCIP arm carries `rust.py`'s `stable_id` for a shared function. Measured on `aardvark-dns` with the backend on: **0 of 52** functions emitted by both arms share an id. The helper reproduces `rust.py`'s id byte for byte when given the item span; production hands it rust-analyzer's Definition-occurrence range, which is the identifier *token* (line 14 for a method spanning 14–17), and the exact-match predicate abstains. All three sites now say so, and the 7.0.0 entry is corrected in place rather than rewritten.
- **The test that guaranteed it could not fail.** It collected spans from `rust.py` and fed them straight back to the helper, proving the helper agrees with its own baseline. It is re-titled as exactly that, and a second arm now feeds ranges **recorded from `rust-analyzer scip`** on the same sample: five multi-line callables abstain, the one single-line trait declaration reaches parity. A `reassign_rust_stable_ids` test that hedged "either parity succeeded or tree-sitter is unavailable" and passed on both branches asserts the overwrite outright, beside a production-shaped case that pins the retained SCIP id. Both go red the day the helper learns the token range — that is the fix landing, and they are re-pointed then.
- **Why it is stated and not fixed.** Matching on the start line, as the helper's own docstring already suggests, would make the two arms agree — and `split_within_file_stable_id_collisions` would then re-mint the second record as a second *site*. Sparing cross-backend groups encodes the answer to whether the arms are one logical symbol, which is WI-gojum's parked question. The owner ruled 2026-09-18 not to rule now.
- **The spec cited a ruling that does not exist.** "The two run alongside each other rather than one replacing the other (ADR-0012 coexistence; WI-gojum)": ADR-0012 contains no SCIP, rust-analyzer or dedup content, and its two uses of "coexist" are about analyzer registration systems and about two edges at one callsite. The de-facto state is coexistence *by default* — a bare `list.extend` with no key, and an edge dedup on `Symbol.id`s that differ between arms. The citation is retracted and the sentence re-pointed to WI-gojum as open (WI-kanuh, owner ruling 2026-09-18). ADR-0012 itself is untouched.

#### The SCIP importer minted a node per function-local binding, and false edges to them (WI-jikok, INV-kukiz)

- **Three quarters of the rust-analyzer backend's nodes were `let` bindings named by an index.** rust-analyzer numbers locals per document — `local 0`, `local 1` — and `scip/index.py` minted each as a `Symbol` with that index for a name, under the registry's Terraform `local` kind. On `aardvark-dns`: 500 of 669 SCIP nodes; and because the id is `sha256(moniker)`, 498 of the artifact's 1255 Symbols shared a `stable_id` (39.7%) while the same names degenerated `fingerprint`.
- **And the edges to them were false.** The Rust backend resolves edge endpoints through one index-wide map keyed on the raw moniker. A document-scoped `local 0` recurs in every file, so each read resolved to whichever file's binding was mapped last: **329 of 455 local-pointing edges crossed files — 21.2% of every edge in the artifact** — each asserting a reference a local cannot receive. Six of the fourteen highest in-degree nodes in the artifact were locals named `0`–`5`, outranking real functions for the sketch's attention.
- **Ruled: locals are dropped, matching every other backend.** Owner ruling 2026-09-18, taken on the evidence that `py.py`, `kotlin.py`, `go.py` and the tree-sitter `rust.py` all *read* function-local bindings for resolution and mint nothing for them — a binding no other file can name is not part of a behavior map. The rule is one predicate, `descriptor.is_local_symbol`, applied in all three shims (`index`, `calls`, `edges`) so it also holds for a library caller that supplies no resolver; each shim has its own gate and INV-kukiz's exact shape is pinned end to end at `translate_scip_to_hg`, with a control arm proving a real cross-file reference survives.
- **Measured on the filed repro**: SCIP nodes 669 → 169, SCIP edges 637 → 182, local-pointing edges 455 → 0, cross-file local edges 329 → 0, validation violations 2 → 1, and every non-SCIP origin byte-identical. **The one remaining violation is pre-existing** and now visible: rust-analyzer's `crate/` and `main().` monikers name the cargo *package* but not the crate *target*, so lib, bin and `build.rs` share them and `sha256(moniker)` collides 5 of 756 Symbols (0.7%). Filed as INV-kutid — the same hash-scope defect one level up.
- **Not changed, deliberately**: the tree-sitter arm still assigns a different `stable_id` to the same function (INV-dolud), and whether the two arms should share identities at all is WI-gojum's parked question.

#### The SCIP importer minted Symbol.kind values no registry described (INV-lagot)

- **Four values were off the `Symbol.kind` axis.** `scip/index.py`'s `_KIND_MAP` translated SCIP `DescriptorKind` to `Symbol.kind`; five of its eight values were registered and three (`type_parameter`, `parameter`, `meta`) were not, plus a fourth minted by a `.get(…, "unknown")` fallback. Every ADR-0027 consumer branching on `Symbol.kind` had no case for them. The map is a **dict**, and `axis_drift`'s collector read only string-literal **sets**, so no drift linter could ever see it (WI-jinuj).
- **Reachability measured first, as the row required.** On `aardvark-dns`, 681 SCIP `SymbolInformation` entries: 500 local, 80 method, 48 term, 25 type, 18 namespace, **10 `type_parameter`**, 0 `parameter`, 0 `meta`. So `type_parameter` is reachable in the index (filtered before minting on this crate only because those entries carry no Definition occurrence) — not the dead vocabulary a single clean survey would have suggested.
- **`parameter` and `type_parameter` are now registered.** Both name a source-language syntactic construct directly, which is exactly what ADR-0027's axiom asks of a value, so they are registered rather than folded.
- **`meta` maps to `declaration`**, the registry's declared catch-all for "non-categorized syntactic forms". Deliberately **not** folded into `attribute`, which the registry defines as a data member (a Python class attribute), not what SCIP's `:` suffix means. A re-evaluation trigger is written at the mapping: zero META descriptors were observed, and the first real instance should be inspected for a specific kind.
- **`unknown` is gone, and was always unreachable.** `DescriptorKind` is a closed Enum, every producer constructs from it, and the map is total over it — so the fallback could never fire. The lookup is now a direct subscript, and a `KeyError` on a future ninth member is louder and more truthful than silently minting a kind nothing describes.
- **Two gates the dict never had**: every mapped value is a registered `Symbol.kind`, and the map is total over `DescriptorKind`. Both carry a positive control.

#### One `None` was hiding four different rust-analyzer failures (WI-luvud)

- **Four states shared one sentence and one code.** A missing binary, a non-zero exit, an exit-0-with-no-index, and an undecodable SCIP blob all collapsed into `try_analyze_with_rust_analyzer`'s bare `None`, and the analyzer filed every one as `"rust-analyzer backend produced no output"` with `skip_reason_code="unreported"`. The exception taxonomy had always told them apart; the information died at the boundary. A user could not tell *install rust-analyzer* from *your workspace is not a cargo project* from *the indexer crashed*.
- **`ScipAttempt` carries the verdict out.** A missing binary is `dependency_unavailable` — "install the package" is exactly the right advice. A non-zero exit, a timeout, or an undecodable index is `pass_crashed`, following ADR-0056's init-failure ruling that the value "says nothing about WHO contained the raise". An exit-0-with-no-index keeps the declared residue `unreported`, because it is **not** a crash and **not** a missing dependency and the axis genuinely has no value for "an enabled backend completed without producing anything to read" — inventing a neighbour would be a fabricated disclosure, so the gap stays filed rather than papered over.
- **A backend that ran and indexed nothing is no longer a skip.** That is the state the row is named for: it now lands in `analysis_runs` with `silence_reason="no_candidate_construct"`, claimed by the pass body. Falling through to `derive_silence_reason` would have stamped `no_candidate_files` — *"the pass received zero input files, and no mechanism would change it"* — on a repository that may be full of `.rs`, which is the exact false assertion ADR-0056 W3 was built to stop, one pass to the left.
- **The skip-hosted values stay skip-hosted, deliberately.** Re-hosting `dependency_unavailable` and `pass_crashed` onto an `AnalysisRun` is the question ADR-0056 §"What would change this decision" item 5 reserves to the owner; telling the states apart does not pre-empt that ruling.

#### The rust-analyzer backend's entire output was orphaned (WI-didag, WI-zabus)

- **Every SCIP record named a run that nobody serialized.** The backend's success path returned `AnalysisResult(symbols=…, edges=…)` with **no `run=`**, so `rust_analyzer` landed in *neither* `analysis_runs` nor `limits.skipped_passes` — the only one of 118 registered analyzers for which that was true, and a WI-didil completeness violation that fired **precisely when the pass succeeded**. Measured on `aardvark-dns`: **669 nodes and 637 edges**, 53% and 41% of the artifact, from a pass visible nowhere in the catalog.
- **The guard made it worse, which is the part worth remembering.** `Edge.__post_init__` hard-raises on an empty `origin_run_id` (WI-higap); `Symbol` has no such guard. So `translate_scip_to_hg` satisfied the guard by constructing an `AnalysisRun` purely to harvest its `execution_id` and then discarding the object — leaving the *guarded* half carrying a well-formed UUID that resolved to nothing and the *unguarded* half honestly empty. An invariant written to prevent silent provenance loss turned an abstention into a confident wrong answer.
- **The documented escape hatch was unreachable.** `translate_scip_to_hg` has always taken `run_id`, and its comment says callers wanting real provenance should pass one. None could: `try_analyze_with_rust_analyzer` had no such parameter to forward, so every production call took the fabricating branch and the parameter was reachable only from tests. It is now threaded end to end, and the translator stamps **symbols as well as edges** so a direct library caller bypassing the orchestrator's empty-only backstop still gets a self-consistent result.
- **A productive run-less producer is no longer dropped by the orchestrator.** `collect_analyzer_result` recorded a skip only when a run-less result produced *nothing*; one that produced output was drained and then vanished from the accounting. It now synthesizes the `AnalysisRun` the producer should have supplied, carrying a warning that names it a producer defect — recording a skip would be a lie, since the output proves the pass ran.
- **Result on the filed repro: 1308 cross-field validation violations → 2** (the two remaining are an unrelated fingerprint-degeneracy and stable-id collision), with 0 of 669 nodes and 0 of 637 edges dangling.

#### `no_candidate_files` was claiming a zero it had never measured (WI-finij, INV-hosig)

- **A linker that read four hundred files reported `files_analyzed=0`, and the silence axis turned that into a positive claim.** `derive_silence_reason` stamps `no_candidate_files` — documented as "the pass received zero input files … the only silence the orchestrator can attribute with certainty" — whenever the counter is 0. But 0 is also the dataclass default, so a pass that never *assigned* the counter is indistinguishable from one that received no input, and the axis published the confident wrong answer. Measured by attributing every `Path.read_text` / `Path.read_bytes` to the calling linker on a self-survey: **eight linkers read between 14 and 834 files apiece and were all stamped `no_candidate_files`** — pyffi (389), grpc (397), di-resolution (396), annotation-convention (834), wasm-bindgen (14), solidity-abi (14), crypto-flow (17), message-dispatch (17). This is the project's absent-versus-empty substitution reproduced *inside the axis built to cure it*: the cure is a positive claim, never one inferred from an empty field.
- **The count is now derived rather than remembered.** Seventeen linkers assigned `files_analyzed` correctly and eighteen did not, which is the evidence that per-body counters do not survive the next linker. A per-invocation read log lives in `_text_filters` and is bound by `_run_linker_with_cache`, the one wrapper every linker invocation flows through; `read_masked_source` (36 call sites) counts, and two sibling readers — `read_source_text` for the unmasked case and `read_source_bytes` for the parser case — absorb the other 17. `annotation_convention` needs the unmasked reader because its subject *is* the comment region the masker blanks. The log is a **set of paths**, so `files_analyzed` counts files rather than reads. The stamp is guarded like `duration_ms`: a body that assigned the field keeps its value, because `route-handler-linker` deliberately stores a route count there.
- **A gate keeps the nineteenth linker from reopening it.** `tests/test_linker_file_read_accounting.py` fails on a direct `Path.read_text` / `Path.read_bytes` anywhere in `linkers/`, and uses `_text_filters` — the one module legitimately holding those calls — as a positive control, so a detector that stops working cannot masquerade as a clean result. **Stated limit:** the gate is syntactic and scoped to `linkers/`, and it does not check that a body-supplied count is *accurate* — `websocket-linker` reports 2 while reading 405. A pass means "no unaccounted direct read in a linker", not "every `files_analyzed` in the tree is a true file count".
- **What this changes downstream.** The affected passes move from a false `no_candidate_files` to `unreported`, which means *cannot determine* — the honest residue. The truly-silent cohort (read files, emitted nothing) is therefore larger than the disclosure previously showed: **11 pass-runs and 4.78% of pass time, not 3 and 1.9%**. The mypy strict surface shrank 669 → **667** in passing.

#### `prerequisite_absent` gets a producer, and the axis stops asserting something false (WI-dabup, ADR-0056 W3)

- **The value that "had never been observed" was firing all along; nothing could say so.** In a venv without `tree-sitter-rust`, surveying a JS+Rust repository, `tauri-ipc-linker` and `rust-trait-dispatch-linker` are silent while Rust files sit in the tree — and they claimed **`no_candidate_files`**, declared to mean *"nothing to find, and no ordering or declaration mechanism would change it"*. Installing the grammar changes it. The axis was publishing a confident wrong answer, which is worse than abstaining, and that — not the missing producer — is why this was built rather than the value deprecated.
- **The obvious producer rule is wrong, and the measurement says so.** "Stamp it when a `depends_on` conjunct is unsatisfied" fails because **98.94% of 229,541 skip records are `no files matched`**: an unsatisfied conjunct nearly always means the repository lacks that language, a correct no-op. Stamping that as an ordering defect would be the axis's founding sin committed inside the axis built to cure it. The discriminator is *"was the missing literal skipped for a FILE reason or a TOOLCHAIN reason"* — which is exactly what W2's structured code made askable, and why it had to land first.
- **It fills only a field the body left empty.** Measured on that same repository, `pyffi-linker` is silent with the *same* blocked conjunct and truthfully claims `no_candidate_construct` — it scanned the Python side and there are genuinely no FFI call sites, so the missing Rust grammar is irrelevant to *its* silence. Overruling that would reverse the chokepoint guard and replace a true claim with a plausible one, so `prerequisite_absent` **under-reports by design**.
- **Verified on the production path, before and after, on the same repo:** `tauri-ipc-linker` and `rust-trait-dispatch-linker` move `no_candidate_files` → `prerequisite_absent`; `pyffi-linker` and `wasm-bindgen-linker` keep their body claims unchanged. It sees only *analyzer* prerequisites, since linkers are deliberately not enumerated in `skipped_passes`.
- **ADR-0056's own acceptance test would have deprecated the value.** Run as written it comes back empty — and the ADR says an empty result flips the verdict to DEPRECATE. Two faults: the named repository is Rust-only (so the dependents are activation-gated out and produce no `AnalysisRun` at all), and `rust` is not gated on `tree-sitter-language-pack` but on a dedicated `tree-sitter-rust` wheel. **The catalog's own `Pass.requires` says otherwise for 25 of 26 grammar-gated literals**, and nothing reads that field but humans, so nothing catches it — an instrument aimed by a wrong declaration exits 0 with a plausible answer.

#### Three silence values were on the wrong dataclass; `skipped_passes` gets a code and a reader (WI-dukoh, ADR-0056 W2)

- **A pass that did not run has no `AnalysisRun`, and `silence_reason` is a field on `AnalysisRun`.** `dependency_unavailable`, `backend_disabled` and `pass_crashed` were never missing a producer — there was no carrier in existence to stamp. They are re-hosted onto `limits.skipped_passes[].silence_reason`, where their free-text twins already lived (`SCHEMA_VERSION` 0.20.8 → 0.20.9; the key shipped as `skip_reason_code` and was renamed in 0.20.10, below). **One channel, two fields:** the prose `reason` stays, because it carries the pip command and the exception text that a code cannot express.
- **The drift was called latent. It is now observed.** `pass_silence`'s own sizing — 490 surveys, 39,757 skip records, only *two* strings ever seen — was honest about its blindness and wrong about what people concluded from it: the blindness was a venv, not a law. A venv built without `tree-sitter-language-pack`, surveying one ordinary repository, produces **29 skip records in 28 distinct spellings in a single run**. The bare environment was constructed rather than waited for.
- **Forty-nine producer sites across thirty-eight files**, not the "~4 sites, two f-string templates" this work was sized at: the grammar-missing f-string is *copy-pasted* into thirty analyzers rather than inherited from a template. Measured before designing against the estimate.
- **The parser-init family gets the ruling it never had.** Six sites report a parser that failed to *initialise* — the grammar is installed and the constructor raised. That is not `dependency_unavailable`, because "install the package" is the wrong advice for it; it is stamped `pass_crashed`, whose declaration ("the pass raised and the crash was contained") is true of a caught constructor exception and says nothing about who contained it. This gives `pass_crashed` real producers: ADR-0056 recorded it as speculative on zero instances in 229,541 records, which was measuring the free-text twin rather than the state.
- **An unclassified skip reads as `unreported`, never as `no_candidate_files`.** Bucketing a missing code under the value that is 98.94% of all skip records would manufacture the majority answer for every unconverted producer — the absent-versus-empty substitution, at the one moment it would be easiest to commit. A gate (`test_skip_reason_code_declaration.py`, AST-based, with a synthetic positive control) keeps the next analyzer from re-opening the hole.
- **`skipped_passes` gets its first reader in the field's entire history.** It has been serialized on every survey since it existed and nothing summarized it, so 2,383 `backend_disabled` records sat in artifacts nobody was shown. A second stderr line now reports it — separate from the pass-silence line, because "did not run" and "ran and found nothing" are different populations with different denominators, and merging them would be the same substitution one level up. Verified end to end in the bare venv: **107 of 107 skip entries classified, 31 `dependency_unavailable`, zero unreported.**

#### `graphql-linker` gains the client-to-server join it never had (WI-dinum)

- **The join it shipped with was not client-to-server.** It matched a client operation NAME to a schema operation NAME — but a client operation name is an arbitrary client-side label, and a schema does not declare one. The match can only succeed when some *other* document in the repository declares an operation of the same name, typically a codegen `.graphql` file, so what it recovered was document-to-document identity. Real, and kept; but it is why the linker emitted **0 edges in 14 corpus runs**, because it cannot fire on a repository with no `.graphql` documents.
- **`query GetUsers { users { id } }` invokes `Query.users`**, and `Query.users` is a thing a schema really does declare. The root selection field is now resolved against the schema field symbols `graphql-sdl-linker` lifts out of embedded SDL, keyed `<root type>.<field>` exactly as the resolver linker keys its own lookup — so `Query.users` and `Mutation.users` stay distinct. Name identity still wins when both could fire, being the stronger claim; the root-field join is stamped at 0.8 against its 0.9, since it reads the first field of a selection set with a regex.
- **A conditional that could not take its other arm is stated as the constant it always was.** `confidence=0.9 if call.operation_name else 0.7` sat inside `if call.operation_name:`.
- **Its `depends_on` named six host languages it never reads.** The linker finds client calls by reading source files itself and consumes no host analyzer's output; the clause now names the two passes that can supply an edge DESTINATION — `graphql` (a `.graphql` document's operations) and `graphql-sdl-linker` (schema fields from an embedded template). Same too-wide shape WI-rasal repaired on four other linkers, found by reading the body rather than by falsification, which can only catch a clause that is too narrow.
- **Measured on apollo-server**, the row's own repro: the two GraphQL linkers go from **120 nodes and 0 edges** to **193 nodes and 128 edges** — `graphql-linker` 0 → **22**, `graphql-resolver-linker` 0 → **106**.

#### The GraphQL schema side gets a producer that is not a file extension (WI-dinum)

- **Two linkers had been minting nodes and emitting nothing, corpus-wide.** `graphql-linker` has emitted **0 edges in 14 runs**; `graphql-resolver-linker` emitted edges in **2 of 794**, both of them the same repository. Both take the destination of every edge they can emit from `language == "graphql"` symbols, whose only producer is the GraphQL **analyzer** — which reads `*.graphql` / `*.gql` and nothing else, and which ran in **3 of those 794 surveys**. apollo-server, the canonical repository for the framework, has **zero** `.graphql` files and 22 source files containing `type Query`.
- **One branch was unreachable by construction, not by circumstance.** The resolver linker's `implements` edge keys on `kind="field"` carrying `meta["parent_type"]`, and **no producer anywhere in the tree emits a GraphQL field symbol** — the analyzer emits type / input / interface / enum / scalar / union / directive / fragment / operation, never `field`. Its 100% coverage came from tests that hand-construct the symbol: green in CI, dead on every repository. A consumer keyed to a shape nothing produces is the instrument-with-no-reader defect inverted, and an empty dict yields silently.
- **`graphql-sdl-linker` mints the schema from `gql` tagged template literals** — an *Infrastructure* linker by ADR-3bbb's discriminator ("graph-structural utilities run as Tier 2 passes but **not** doing dispatch recovery": it emits zero edges), which is also what makes its empty `depends_on` honest rather than lazy, since it consumes no upstream pass at all. At priority 55 so both consumers at 60 can see it (linkers in one priority group are snapshotted together and cannot read each other's output). Class A per ADR-0031 — a `type User { ... }` inside a `gql` template is a real declaration in a real language with a real span, not a stand-in for one, which is the `vue_component` and `grpc.py`-proto-scan precedent and means both consumers read it through their existing filters unchanged.
- **Only the three kinds a consumer reads** (`type`, `interface`, `field`) are minted. Minting `input` / `enum` / `union` nodes nothing can link to would be this row's own defect one level down. Extraction is bounded to the tagged template because scanning source text for `type X {` was tried and matches every TypeScript `interface ITrace {` and prose inside comments, yielding "types" named `to`, `is` and `for`; `#` comments are blanked inside the block (preserving line structure, so spans stay honest) because a `gql` template is a *string* to the host language and the shared doc-region masking never reaches into it.
- **Measured on the row's own repro.** apollo-server, `hypergumbo survey`: `graphql-resolver-linker` goes **0 → 106 edges** (86 `references`, **20 `implements`** — the branch that had never fired) from 79 files and 73 newly-minted schema symbols. `graphql-linker` is still at 0 and is the next change.

#### The six falsified clauses are repaired from enumerated producer sets (WI-rasal)

- **Six clauses, one mistake.** Each was written as *"languages where this concept exists"* — a statement about programming languages — where `depends_on` is defined as *"passes whose output this pass reads"* — a statement about this tree. Both readings produce a plausible list of language names, which is why nothing caught the difference for as long as nothing read the field. `inherited-calls-linker` is the clean specimen: the comment above its declaration names `_MRO_WALKERS` as its source of truth and claims to mirror it, INV-fahub's fleet-walker PR took that dict from 3 entries to 14, and the declaration stayed at three.
- **Each repair is derived, not re-listed.** `inheritance-linker` becomes the analyzers that actually write `base_classes` / `included_modules` — which **drops `rust`, `dart` and `elixir`** (declared and contributing nothing: rust emits `implements` EDGES directly, which is `type-hierarchy-linker`'s input, and is declared there instead) and **adds `apex`, `cpp`, `go`, `groovy`, `objc`**. `type-hierarchy-linker` becomes the producers of the extends/implements edges it indexes, headed by the pass id `inheritance-linker` — it never looks at a language, which is exactly why Go repositories falsified it. `build-target-linker`'s manifest side gains `xml` and `manifest_targets` (Haskell repositories falsified `["toml","json"]` through the latter).
- **Two conjuncts are deleted rather than widened, and the deletions are the interesting half.** `inherited-calls-linker`'s `["inheritance-linker"]` was falsified 26 times and was false: Site-2 step 1 resolves the method directly on the inferred type and emits with no inheritance edges in the graph at all. The ordering it was recording is carried by `priority=18`, which is the axis for it. `build-target-linker`'s `["rust","javascript","go","python","java"]` matched a function named `main`, which every analyzer that emits functions can supply — the honest enumeration is all 118 and any narrow one is false, and a conjunct satisfied by everything states nothing the empty list does not.
- **Two AST gates and a registry gate keep the declarations attached to their producers.** String literals *and* keyword-argument names count (a hint key reaches `Edge.meta` either as a dict key or as a kwarg to the shared `make_unresolved_edge`); comments and prose do not, which is why these walk the AST rather than grep. The walker gate maps `_MRO_WALKERS` through the analyzer registry before comparing, because that dict is keyed on `Symbol.language` — a wider vocabulary than pass ids, whose `typescript` key has no pass of its own.
- **The corpus goes 264 falsifying surveys to 0 of 975 — which is a consistency check, not the argument.** Widening an OR clause necessarily reduces falsification, and on caddy the repaired clause is satisfied by `php` as well as by `go`; the justification is per-literal (each added literal is a producer the gates re-derive), not the count. The zero was confirmed live rather than assumed: with the clause mutated to a language the repository lacks, the detector fires on the same survey.

#### The CNF declaration set had never been read, and 27% of surveys falsify it (WI-jijor, ADR-0056 W1)

- **`validate_pass_dependencies` had eleven call sites and all eleven were tests.** The ~73 `depends_on` conjuncts across 59 passes — the CNF that says which upstream passes each linker needs — have never been checked against a run. Its sibling `validate_pass_name_resolution` was worse: **zero call sites anywhere**, under a docstring claiming it "runs at catalog-build time and in CI". A declaration nothing reads is the instrument-with-no-reader defect wearing a schema's clothes.
- **The gate INV-hujog asks for is still refused; the same predicate ships pointed the other way.** `find_falsified_dependencies(passes, runs)` reports a pass that **emitted output** despite an unsatisfied conjunct — which proves the *declaration* wrong, not the run. It reports and never raises, so unlike a gate it cannot lose an edge; `validate_pass_dependencies` is left untouched and its tests stay green.
- **The reader is wired on the survey path**, one stderr line next to the pass-silence summary, silent when nothing is falsified — and it was proven to fire rather than assumed to: a survey of a Go-only repository at tip names `inheritance-linker` and `type-hierarchy-linker`.
- **The two halves of the predicate are asymmetric, and getting that wrong cost 26% of the first run's hits (corrected under WI-rasal).** A pass falsifies its *own* declaration by emitting **edges** — a linker's product is edges (ADR-3bbb Tier 2), and `depends_on` is defined as what it needs to "produce its intended edges at all"; all 59 declarers are linkers, now pinned by a test. A *prerequisite* satisfies a clause by **producing** anything, nodes or edges, since an analyzer contributes symbols and `inheritance-linker` contributes edges. The first cut counted nodes on both halves and tested clause satisfaction by **membership in `analysis_runs`**. Both readings were wrong for the same reason — they were not functions of the fact they stood in for:
  - Counting nodes as proof read 68 of the first run's 261 falsifying surveys as `database-query-linker` disproving its `['sql']` declaration. Every one of the 68 emitted nodes and **zero edges**: it mints a query node per SQL string scraped from a host file, then needs the `sql` analyzer's `kind="table"` symbols for the dst of every edge. Nodes-with-no-edges is the shape of *could not do its job without the thing it declared* — the declaration working, read as the declaration being wrong.
  - Membership answered the taxonomy, not the repository. Under the WI-jadig / INV-manov file-presence pre-filter an analyzer is short-circuited into `skipped_passes` only when every language it declares is in `LANGUAGE_EXTENSIONS`; all others run and record a 0-file row. On caddy — Go-only — `apex`, `astro` and `pony` are "active" and `java`, `ruby` and `python` are not, for the identical fact that the repository has no such files.
- **The audit fails, on the corrected reading.** Over **975 surveys** on disk carrying `analysis_runs`, **264 (27.1%)** carry at least one falsified declaration — six distinct `(pass, clause)` pairs across four passes, led by `inherited-calls-linker` needing `['java','ruby','groovy']` in 226 of them. Every surviving falsification emits edges and mints no nodes. Hand-verified on caddy: the single `inherited-calls-linker` edge is `ast_call_inherited` between two **Go** methods — the Site-1 walker path — while `java`, `ruby` and `groovy` had no files at all. ADR-0056's own re-decide criterion ("ten hits means declaration repair is the blocker") is met; the repair is filed separately rather than bundled here.
- **The only live-tree check of the declaration set was a control that could not fail.** Linker modules register by `@register_linker` import side-effect, and `TestCatalogStaticConsistency` built the catalog without importing them — so in isolation it validated 118 analyzers and **zero** conjuncts, and whether it checked 73 clauses or nothing at all depended on which other test module had imported `cli` first. Repaired, with a mutation control proving it now goes red and an assertion pinning a *named* declaration rather than only a count, since a floor catches an empty instrument but not a mis-aimed one.

#### `no_candidate_construct` had no reachable producer (WI-finij)

- **The axis declared a value nothing could ever emit.** `derive_silence_reason` deliberately never infers `no_candidate_construct` — only a pass body knows whether it looked for its construct and failed to find one, and a reason invented on the producer's behalf is a fabricated disclosure. But both stamp sites then assigned the derived value *unconditionally*, overwriting the one producer able to say it. The declared value was therefore unreachable from anywhere in the tree: a vocabulary entry with no possible producer, which is the same defect as an instrument with no reader.
- **Both chokepoints now fill rather than overrule.** `_run_linker_with_cache` and `collect_analyzer_result` stamp only when the body left the field empty, matching how `duration_ms` and `files_analyzed` are already guarded at the same two sites. The single exception is the empty string: a pass that *emitted* has no silence to explain, so `""` (NOT APPLICABLE) still overrides a stale or mistaken body value rather than publishing a contradiction — a silence reason on a productive pass is incoherent whoever wrote it.
- **Eleven bodies now make the claim, and six of them land it.** Each call passes the collection the pass's own scan FOUND — `all_patterns`, `call_sites`, `routes + dispatches`, `all_bindings`, `all_writes + all_reads` — never what it finally emitted. Measured on a self-survey: `no_candidate_construct` **0 → 6** and `unreported` **11 → 5**. The six that drained (crypto-flow, graphql-resolver, grpc, message-queue, pyffi, solidity-abi) scanned their files and genuinely found nothing.
- **The five that did NOT drain are the finding.** They found candidates and emitted nothing anyway: an import whose export is missing, a binding that resolves to no symbol, a write with no matching read. `wasm-bindgen` and `pyffi` had to have a candidate accumulator ADDED before they could be asked the question at all — both scanned per file and discarded what they found, so there was no candidate set to anchor a claim on, and anchoring on emitted output would have declared the construct absent while it sat there unpaired. Those five stay `unreported`, which means *cannot determine* and is the honest answer: **the axis has no value for "candidates found, relation not established."** That population was invisible before — undifferentiated inside `unreported` — and is now countable at five. Declaring a value for it is a separate decision, and it would need a producer, or it becomes the very defect `prerequisite_absent` is currently stuck in.
- **Anchoring on the candidate set is what makes this robust to misreading a linker.** Two linkers were classified as emitting one symbol per candidate and turned out to gate emission behind a match (`ipc` has a `continue` before its symbol append); the rule produced the correct result for them regardless, because it never asks what the pass emitted.
- **The five that did not drain are now named, and the ruling that said they shouldn't be is overturned (WI-bivim, `docs/adr/0054`).** `unreported` is declared to mean the pass *"did not say why"*, but those five bodies **compute the answer and throw it away** — the producer helper returned `""` on the branch that had the fact in hand. That is the INV-bikaj complaint one level down, inside the axis built to cure it. **`candidates_unresolved`** is promoted out of the residue: the pass found its construct and carried none of it through resolution. `SCHEMA_VERSION` 0.20.7 → 0.20.8.
- **The prior ruling's three stated grounds were each refuted by the tree.** It argued a new value would be "a seventh member with a single producer and no consumer": the vocabulary already held seven, the eleven call sites route through one helper exactly as `no_candidate_construct` does, and `summarize_silence` / `format_silence_summary` count and print by whatever name they find — wired into the CLI, so the value is read back on day one. That last ground is what distinguishes this from `prerequisite_absent`, which remains declared with no producer and no consumer.
- **One line produces it, at no call site.** The helper's non-empty branch returns the new value; the eleven bodies are untouched, because the chokepoint guard clears a body claim on a pass that emitted. A body may therefore speak without knowing its own output. The helper is renamed `silence_reason_for_candidates`, since both of its returns are now positive claims. **The name `candidates_unresolved` deliberately describes the PASS, not the repository** — `ipc` rejects some candidates on a quality filter with no pairing attempted, so a "missing counterpart" name would be false for it, and a pass cannot tell a genuinely absent counterpart from its own matcher missing one.
- **Four return paths never called the helper at all, and two were firing already.** The one-line producer was predicted to empty `unreported`; measured, it left 2. `wasm-bindgen` returns early when no Rust exports exist while its *wrapper* then scans 14 files, so the run reported files with the body never having spoken; `message-dispatch` passed its bail with both sides non-empty and matched no pairs. All four paths are closed as a class rather than filed for later — keying the fix to the two that happened to fire would have left the others waiting for a corpus to expose them. Final: **`no_candidate_files=43, no_candidate_construct=7, candidates_unresolved=4`, `unreported=0`**.
- **Stated honestly: that zero measures the corpus, not completion.** 53 of the 64 linker modules make no body claim at all and would land there on a corpus where they are active. The *"the `unreported` count is the size of the remaining work"* gloss is removed from the field comment accordingly.

#### auto-pr's undeclared terminal state (WI-nazoj)

- **Ten deliberate `exit` statements left `final_state` at its `unknown` initializer**, so the run recorded no terminal state at all — INV-rahib's residual class, 14 of 485 ledger rows, most recent 2026-09-11, one of them at exit 0 (success claimed for a run that recorded neither a terminal state nor a merge). Owner ruling 2026-08-03 makes `unknown` *the* violation signal rather than a state in the vocabulary, because no deliberate exit writes it. The sites are the branch-ownership guards before both amend paths, both re-push failures and both PR lookups inside stuck-CI recovery, the simulated outage, and the unresolvable / non-linear vPR queue checks; each now names what it decided (`failed_branch_ownership`, `failed_repush`, `failed_pr_lookup`, `failed_flush_remote_unavailable`, `failed_queue_unresolvable`, `failed_queue_not_linear`), and the four new values join the declared closed set, which is checked in both directions.
- **The enumeration is a dominance analysis, because proximity clears the real cases.** The assignment at `auto-pr:2085` sits in the `if _pre_scenario_b_gate` arm while the exits at 2105/2114 sit in the `else` arm, so "is there an assignment earlier in this function" calls them covered — and a 25-line distance threshold hid 2105 outright during the investigation, the same shape as a grep whose own filter omits the line being checked. The test walks back from each `exit` and treats an `else`/`elif` at the current level as leaving the conditional, and it carries a two-armed fixture so that neither a scanner returning nothing nor one flagging everything can pass it.
- **What is left is not statically enumerable, so the run attributes itself.** A `set -e` abort at any unguarded command in a 700-line function also lands on `unknown`. `auto-pr` now runs under **`errtrace`** with an ERR trap that records the file and line, and `_autopr_finalize` prints a `CONVERGENCE VIOLATION` banner naming the site and persists `abort_site` / `abort_command` to both the sentinel and the ledger. `-E` is load-bearing rather than decorative: without it an ERR trap is not inherited by shell functions, and every real `auto-pr` abort happens inside a helper — a first version of the test passed with `-E` removed because its fixture aborted in the same frame that installed the trap, which is the harness contaminating its own control.
- **Three states, not two, at the reader.** `scripts/audit-autopr-convergence` separates a recorded site (tallied, since two runs dying at the same line are one bug) from **not recorded** — an instrumented run whose abort the trap could not see, since a `set -u` unbound variable is an expansion error rather than a command failure and does not fire ERR, and neither does a bare `exit` — from **not instrumented**, a row with no `abort_site` key because it predates this change. A null default would have merged the last two and reported the entire historical backlog as live instrument failures.
- **The session-start nudge stops routing to a closed item.** Both forms ended "the undeclared-state class is WI-nazoj"; a reader following that now lands on finished work rather than on the evidence. They point at the abort site instead, and say explicitly that a *missing* site is itself the signal to look by hand.
- **The command text is safe to persist because `BASH_COMMAND` holds source, not expansion.** A line carrying `$HG_GITHUB_TOKEN` records the name. That is a property of bash rather than of this code, so it is pinned by a test that aborts on a command referencing a secret-shaped variable and asserts the value never reaches the payload.

#### The catalogue test gate was keyed to one family of ten (INV-bigaz, INV-dohoj)

- **146 of the 169 shipped YAML files selected zero tests.** `smart-test` unions the tests that read a changed catalogue, and the union was right — its key was not: a regex naming `io_primitives` and `io_primitives_overlays`. Measured on dev `81f1a69f88`, a change confined to `frameworks/` (107 files), `dataflow_patterns/`, `cfg_nodes/`, `function_summaries/` or `taint_sources/` wrote a 0-test manifest, printed *"no test-relevant files changed (docs/config only)"* — asserting the opposite of what had happened — and `ci.yml` then skipped pytest. A fix keyed to one instance had left the class and looked done.
- **The terms are now derived per family from `YAML_CATALOGS`**, the registry `validate_registry()` already gates against the tree, so the eleventh catalogue family arrives covered by the commit that registers it rather than by an edit to the selector. Three arms per changed family: the directory's own name, the loader module the registry names, and the `load_*` entry points read out of that loader's source. The loader arm is the load-bearing one and that is measured, not assumed — on the live tree the directory name alone reaches 58 test files and the union reaches 157.
- **Two families behind one loader reach each other**, which is what keeps the widening from *narrowing*: the old key fired one shared rule for both `io_primitives` directories, and the literal `io_primitives` does not appear in every test that reads them. `io_primitives_overlays` layers onto `io_primitives` through `io_boundary`, and `taint_sources` / `taint_sanitizers` are two halves of one catalogue behind `taint`.
- **And the key is "not Python", not "is YAML".** `url_folding/SCOPE.md` is parsed on every run that folds a URL, by `get_scoped_languages()`, and the first cut of this fix selected nothing for it — the same keyed-to-one-instance shape one level down from the defect it was written to close. The invariant is about a non-Python input the production code loads at runtime, and the extension is not what makes it one. The shell fallback keeps its own copy of that predicate because it must work without python3, so a test walks one representative of every path shape in the tree and requires the two to agree.
- **A registry that cannot be read degrades to the directory name, not to silence.** The helper parses the registry with `ast` rather than importing it, so selection never depends on the package being installed, and a missing or unparseable registry still yields the changed directory's own name — under-selection is the failure mode that produces a green tick over a hole.

#### A string literal is an edge the import graph cannot see (WI-lujon)

- **A citation rotted under a green gate and fired two crons later.** `test_module_key_axis.py` pins that every `file:line` citation in `MODULE_KEY_NOTIONS` still contains its quoted anchor. The INV-sihom fix added lines to `bash.py` and moved the cited emission out from under its citation; the per-PR gate was green, because the citation test lives in hypergumbo-core and names `bash.py` by *string*, so the import-graph reverse slice never selected it. It was repaired in passing two firings later by a PR that happened to touch a core file.
- **The fix the item proposed would have missed the item's own example**, and that is now a named test. WI-lujon asked for "a test file that contains a string literal naming a changed source path". Measured: `test_module_key_axis.py` does not contain the string `bash` at all — the citation lives in the *source* the test reads. So the gate is two hops: find every file whose text names the changed path, select a citer that is itself a test, and for a citer that is a source select the tests that name **it**.
- **A citation has a slash in it.** The literal is the import-root-relative path, not the bare basename, and that is measured rather than stylistic: slash-bearing literals name 12 shipped sources from 9 citing files across the whole tree, while `bash.py` alone appears in five test files with nothing to do with the citation. The second hop uses the citer's import path for the same reason — for `hypergumbo_core/taint.py` the bare stem matches 142 test files and the import path 64, and both reach the module's own test.
- **The test file assembles every path from parts, and a guard enforces it.** Written out whole, the literals under test would make the test file a citer of its own subjects and turn two controls into assertions about their own text. They failed exactly that way on the first run; the guard then caught a second instance in a docstring.

#### "full" has to mean full (WI-ginuj)

- **A gate named for totality was silently partial.** `smart-test --full` collected `packages/*` and never the repo-root `tests/`, while CI runs the committed manifest, which does include root tests — so a green local `--full` was not a superset of what CI runs and did not predict it. PR #890 went green locally over 26,702 tests and red in CI on `test_adr_readme_index_sync.py`, a test the local gate holds and simply did not run.
- **Five sites, not the three the item filed.** The filing found them by grepping `run_pytest`; the background suite calls `pytest` directly and the full-suite *manifest writer* enumerates with `find`, so two were invisible to that grep. All five now share one definition, and a structural test fails on any sixth site that spells the target out by hand — the control that would have found the two the grep missed.
- **The reason this was deferred turned out to be the wrong reason.** The risk on file was COV_PATHS, and coverage is not a problem at all: it is measured over `packages/*/src`, so more tests can only raise it. The real blocker is that `tests/test_generate_concepts.py` and `packages/hypergumbo-core/tests/test_generate_concepts.py` share a basename, and under pytest's default import mode collecting both in one run raises `import file mismatch` and **interrupts collection**. The core file is renamed to match its own docstring, and a guard test keeps the two suites' basenames disjoint so `--full` stays possible.
- **The suite now says what it collected and what it did not**, derived by asking git for the tracked files and pytest's own `python_files` for what counts as one, so a seventh test directory is disclosed by the commit that adds it. Its first real run reported two directories as uncollected; both were false positives of a glob looser than pytest's rule — `top_level_test_map.py`, `test_masking.py` and `test_only_reachability.py` are production modules whose names merely contain the word. A disclosure that cries wolf every run is a disclosure nobody reads.

#### The documented dev loop destroyed the artifact CI runs (WI-duson)

- **Two correct instructions composed into a false green.** AGENTS.md says "during development, run only relevant tests" and "commit `.ci/affected-tests.txt` with every PR". An explicit run — `pytest tests/test_foo.py` — overwrote the manifest with a zero-selection stub, so following both shipped a manifest that selected nothing. The live Woodpecker gate keys on *did the manifest select any tests*, so it printed "manifest selects no tests — skipping pytest" and reported green.
- **An explicit run publishes nothing now.** It is not a selection computation: it was never asked about the diff, and the honest thing for a command with no answer is to say nothing rather than to publish an empty one. Measured as a pair against the same command and tree: the pre-fix script replaced the manifest with `# Mode: explicit` and 0 selected tests; the current one leaves it byte-identical.
- **The guard now asserts the one good mode instead of enumerating the bad ones.** `.githooks/pre-commit` rejected exactly `Mode: full-suite` and passed everything else through its else arm with a green tick — so the zero-selection manifest committed cleanly past the very check that exists to protect that file. full-suite runs too much and explicit runs nothing, and only the first was guarded. It now requires `Mode: targeted`, with a distinct reason per rejection so the message sends the reader to the right fix, and the test drives the extracted check against all three modes plus an unrecognised one.

#### The declarative `covers:` marker, and KNOWN_UNREACHABLE 25 → 3 (INV-lizor)

- **Name-based mapping has a floor, and twenty-two tests were sitting on it.** A root test named for the *behaviour* it pins, or covering several sources at once, or whose subject is a pipeline YAML rather than a source at all, cannot be reached by any rule of that shape. Those tests are now allowed to say what they cover — `# covers: .woodpecker/*.yml, scripts/lib/forgejo-api.sh` in the module header — and every changed path matching one of the globs selects them. The map's own docstring had been promising this and `KNOWN_UNREACHABLE` had been asking for it by name in entry after entry; the exempt list drops from 25 to 3.
- **What is left is what a declaration cannot honestly express:** a test whose subject is another *test*. No source change breaks those; editing the file under guard does, and they run in the same suite as their subject.
- **A declaration may ADD a test and may never withdraw over-selection.** `smart-test` unions the whole root suite for a top-level source the map cannot reach — deliberate over-selection for `.githooks/**` and `scripts/lib/**`, whose basenames are too generic to map — and keying that on "is it mapped" would have let the first marker naming `.githooks/*` trade 108 tests for one. The fallback asks the name rules (`--names-only`); the union asks both. The decision test failed the moment that marker landed, which is how the distinction came to exist.
- **CI config is no longer dark.** `.woodpecker/` and `.github/` join the executable surface, and fifteen root tests read those files. "Editing a workflow does NOT select this test per-PR" appears verbatim on four `KNOWN_UNREACHABLE` entries and closed none of them.
- **The exempt list is now EMPTY.** The last three entries went when the objection to declaring them turned out to be aesthetic rather than real: two have another *test* as their subject — a standalone re-run of a pinning module, and a shared harness — and `# covers:` expresses that exactly, since "when this path changes, run me" is as true of a test file as of a source. The third pins four package modules by import path and can simply name them. An empty list is a claim: every root test is selected by some change that can break it.
- **The header scan parses rather than pattern-matches.** The first implementation cut the header at the first line beginning `import `, which in `test_rct_public_api_pinned.py` is a line of English inside the docstring — so the marker sat below the cut, the declaration silently did nothing, and the failure named the *test* as unreachable rather than the parser as wrong. `ast` knows where a docstring ends; a line-start pattern cannot.
- **The markers ratchet two-sidedly, like the list they replace.** Deleting one makes its test unreachable and fails; a glob that matches nothing in the tree has rotted and fails. Both arms were verified by mutation.

#### Call edges the analyzers never emitted

- **Six constructs emitted no call edge at all, so no downstream evidence could rescue them** — Python's bare builtins, because the permitting set was the I/O catalogue itself; every Go call under a package-level `var`, making each cobra `Run:` handler invisible; a Python method call on an own external-typed instance field; Haskell zero-argument IO actions (505 sites); Erlang's `?LOG_*` macros, 91% of rabbitmq's logging surface; and Rust grouped `use` lists, 37.9% of the corpus's `use` statements.
- **A JavaScript handler *assignment* emits a registration edge, so `ws.onmessage = h` reaches the catalogue.** It is a property assignment, not a call, so every branch of the member-call cascade missed it. It was predicted to buy zero findings; measured on the production path it does, and the prediction is recorded as refuted.

#### Call resolution and module slots

- **The same Elixir code over the same repository did not produce the same answer twice.** A bare stdlib-named call was attributed to `next(iter(imported_modules))`, and Python randomises string hashing per process: identical tree, two hash seeds, surveys differing by 101 edges. Every Elixir number this project has published was taken through that, and ordering the set would have shipped a deterministic wrong answer. **653 of 653 edges this branch emitted were false attributions.**
- **Elixir reaches OTP at all, and stops resolving qualified calls by bare name.** An atom-module call (`:ets.insert`) required an `alias` child the grammar does not give it, so it emitted no edge: **+122 edges across 25 OTP modules, 0 lost**. Separately, a substring ownership test let a qualified call bind by bare name — 202 false edges — so bare-name resolution is removed: **208 edges redirected, 0 real losses.**
- **A Scala receiver's type survives a qualified or generic type node**, where a bare-`type_identifier` lookup gave `val b: java.io.File` no hint at all — hinted share 9.03% → 23.43% on sbt, `calls_total` identical. A generic base Scala imports implicitly is deliberately refused: 44 of 185 newly-resolved edges bound a standard-library `Map.get` to the project's own.
- **Node's globals and promise APIs resolve.** `fs.promises.<fn>` reached none of the 31 catalogued rows because the receiver was a member expression (**+303 edges**), and `process` and `performance` were missing from the known-globals set — **892 edges promoted, every one off `external`**.
- **C stamps the file's `#include` set on ambiguous bare calls**, so `send` / `recv` / `read` / `write` reach the catalogue for the first time in a language with no namespaces; it also moves 15 of 21 verdicts from `confirmed_with_caveats` to `inconclusive`, none toward `confirmed`. **A Java wildcard import is a candidate package, not a blanket file-level prefix**, where `System.currentTimeMillis()` had landed as `java.io.System`.
- **An import edge's dst is a canonical id.** Objective-C, CSS and Solidity emitted the bare import path, and a bare path is not an id, so the language slot carried a header path. **A generated file's edges keep the file's path**, where every generated file in a language had collapsed onto one pathless anchor.

#### The standard-input surface and deferred crossings (ADR-0049)

- **Server launch and connection setup stop minting taint sources** — 28 launch rows across Go, Python, Haskell and Erlang plus Go's eleven setup rows, all of which register or open a crossing and transfer nothing.
- **A handle wrapper's boundary follows its argument.** Go's `bufio.{NewScanner,NewReader}` had been `ipc_recv` unconditionally on a note reading "when wrapping os.Stdin", a condition holding at **zero of 83** resolvable sites. `io.WriteString` and `fmt.Fprint*` take any `io.Writer` and were rowed at one fixed boundary each — measurement 0012's entire vacuous class.
- **Standard input is `ipc_recv` in every catalogue, and the call that transfers the bytes is rowed.** Java's `BufferedReader.readLine` and `Scanner.next*` shipped at a fixed `fs_read` — the Scanner row's note read, in full, "When reading from a file" — and C++'s `std::getline` had no row at all. **A launch that hands back the child's bytes mints a source in eight languages.**
- **Thirteen class-qualified receivers were unclassifiable, so all eighteen self-claims fell to `inconclusive`** — `compute_boundary_coverage` returns `qualifying_only = not unknown`, so one unclassifiable name withholds every qualified verdict at once. Adjudicating them exposed a real filesystem-escape hazard.

#### Catalogue rows — direction, kind and reach

- **Python and JavaScript complete their catalogue legs.** Python rows the 41 stdlib module strings still reaching the coverage gate uncatalogued and adds 44 dated grants; JavaScript is audited and granted, 76 rows added and the catalogue promoted to `provenance_declared`, where zero grants had meant no node module could be an examined negative. **Python's `builtins` is enumerated**, since once bare builtins emitted edges the gate read an unenumerated module and sent all 18 self-claims to `inconclusive`.
- **Rows that manufactured taint out of operations reading nothing are removed** — 31 Haskell `IORef`/`MVar`/`STM` rows under `db_read`, `socket`/`bind`/`listen` in ten languages under `net_recv`, the JPA/JDBC builders, and `io_lib:format`, which returns an iolist and writes nothing. Adjacency is not a crossing, and a derived gate now fails on any row whose note rationalises a crossing that happens elsewhere.
- **Direction, kind and family corrections across all fifteen catalogues.** A C socket send or receive is classified by the descriptor's *family*, not by name, so tmux — which opens no `AF_INET` socket anywhere — no longer reports network egress over its `AF_UNIX` control channel. Erlang's `io:read` and `io:get_line` were declared `logging`, an outbound sink, for functions that read input. **A clock read is `host_info_read` in all fifteen.**

#### Taint propagation

- **The extractor recorded a statement and then accounted for nothing inside it, and no coverage predicate at any width could see that.** The gate asks whether code sits *outside* every recorded statement, so it is silent when the statement *is* recorded and its def/use is empty. The remedy originally proposed — a per-statement "I understood this construct" flag — was refuted by a four-language differential sweep: in six of the seven shapes found the handler *is* registered and merely incomplete, so dispatch-table membership would have vouched for all six. `unaccounted_names` instead diffs each recorded statement's defines and uses against the semantic leaves in its own extent, per *variable*. Measured effect on verdicts today is **zero, reported as zero**.
- **The walk's accounted-for exits are pinned as a census.** `_ddg_taint_reaches` continues without setting `escaped` in exactly five places, now counted structurally against the shipped function's AST with controls that add a sixth. Two of the five rest entirely on the summary catalogue, where `fmt.Fprintf` and `fmt.Fprintln` carried the dead-end marker while writing into their first argument.
- **The §3a coverage gate asked only about call nodes**, so a def/use omission at a construct with no call in it licensed an unearned refutation; it now compares semantic leaves. **A recorded heir vouched for an unrecorded one** — 1,978 sites carried one. **`coverage_granularity` was the constant `"language"`**, carrying a re-evaluation trigger reading *"becomes `function` when the per-function coverage gate lands"*; the gate landed on 2026-08-26 and the constant did not move.
- **Four ways the walk reached a wrong answer.** `subprocess.Popen(tainted)` verified clean while `subprocess.run(tainted)` verified violated, because PascalCase types the call as `instantiates` and `TAINT_CALL_EDGE_TYPES` did not contain it. A dispatch edge out of a *type* node is reachability, not dataflow (22 removals, 0 of 22 correct). An I/O primitive's return no longer inherits its argument's taint, and a sink is no longer matched by bare name when the edge carries no module.
- **An unresolved bare-name call registered a sanitizer barrier that does not exist**, and a barrier deletes findings rather than adding noise — a sanitized flow is dropped from a claim's violation set. The guard tried to enumerate the ways receiver evidence can be *absent* (a `call_construct` stamp, an `ambiguous_names` net that never held a single sanitizer short name in any language) and permitted whatever it had not thought of, so a bare java `doFinal(p)` bound `javax.crypto.Cipher.doFinal` on its caller. It now enumerates the three ways evidence can be *present* — an exact qualified name, a module slot that completes it, or a callee name carrying its own owner matched on a separator boundary — and refuses everything else, in every language. This reverses an earlier ruling that a bare free-function call could bind a receiver-shaped sanitizer.
- **A javascript callback whose handler owns a symbol was unreachable for taint**, so `addEventListener`, `http.createServer` and `process.on` classified correctly as `net_recv`/`ipc_recv` and produced no findings — the source sits on the enclosing function, the sink inside the callback, and the edge between them is `references`, which javascript emits where python emits `dispatches_to`. A registration edge is now call-shaped for taint by predicate rather than by admitting `references` wholesale, since that type also carries TypeScript type references and object-literal fields. Measured on eight javascript repositories: 78 → 80 findings, one verdict `inconclusive` → `violated`, none removed. Unlike the `dispatches_to` membership this is **not** monotone-additive — a callback handed to a sanitizer now installs a barrier.
- **`verify-claims` no longer crashes with `RecursionError` on deeply nested generated code**, where it exited 1 with empty stdout, the same code it returns for a violated claim; the walk is now an explicit stack. **A bash redirect to `/dev/null` is no longer a filesystem write.**

#### verify-claims — coverage and disclosure

- **The coverage gate counts what it should.** A repository's own code is no longer reported as an unexamined third-party dependency (Rust `crate::`, JavaScript relative specifiers, a repo's own `lib/utils` against a folded `lib.utils`), and references are counted rather than parse candidates, where a scoped Rust path emitted once per nesting depth contributed N−1 entries naming modules nothing called into.
...
#### Symbol kinds, ids and dead code

- **Twenty-six id-emitting sites produced ids hypergumbo's own validator calls malformed**, the dominant defect being a line number where the grammar wants a span. **`Edge.id` gets a format validator for its own grammar**, which is not `Symbol.id`'s — checked against a live survey first, where every one of apollo-server's 18,283 edge ids is `edge:sha256:<16hex>`.
- **Construction reaches the initializer whatever dst the analyzer chose.** INV-kahig made `instantiates` traversable and left open which NODE it lands on: ruby resolves `Klass.new` to `#initialize` and csharp/java land on a `constructor` symbol, but py/js_ts/dart land on the **class** — and every edge leaving a class node is correctly non-traversable, so the dead-code walk arrived and stopped. Of this repo's 11,428 construction edges, 9,217 land on a class and **zero** on a callable; all 69 python `__init__` methods had exactly one incoming edge type, `contains`. A private helper whose only caller is the constructor therefore read as dead — `Cache._ensure_db`, which opens the tracker's SQLite connection, was one. The walk now hops from a **constructed** class to its initializer, licensed by that conjunction and never by `contains` alone (making containment traversable would make every method of every instantiated class reachable). Applied to the walk's own graph after seed detection, so no emitted edge and no other consumer moves: measured against minting the hops as real edges, which cost 5 route seeds on pretix. Revived, with 0 newly-dead, at `--cross-lang-threshold 0`: **21 on self-analysis, 103 on pretix, 74 on kserve**. `symbol_kinds.is_initializer` is the shared predicate; dart is a declared gap (its constructors are named after their class).
- **Ruby object creation emits `instantiates`** like the eight other analyzers that emit it, so a `survey` filter no longer silently omits every Ruby object creation. **Solidity contract members are `method`, not `function`**, making 2,025 members per repository visible to every consumer keyed on `kind == "method"`.

#### Security and runtime safety

- **`--backend tree-sitter` now actually disables the rust-analyzer backend** (ADR-0045 ruling 4). The CLI translated `--backend` into an environment variable but implemented only the positive arm, so a user who had exported it and then opted out for one untrusted repository still executed that repository's `build.rs`.
- **The brand scrub reached commit messages and nothing else; five forge-egress channels published agent-authored text unscrubbed.** A merged PR body carried a vendor session URL on its final line — a line the shipped pattern list matches correctly and the existing scan window already covers, so nothing was wrong with the matching. The rule simply lived inside the `commit-msg` hook, and a PR title, body or comment is not a commit message. The matcher, the phrase selection and the line sweep move to `.githooks/brand-scrub.sh`, sourced by `lib/forgejo-api.sh` so one pattern list governs every publisher, and `commit-msg` is refactored onto it rather than keeping a second copy that could drift. Policy is unchanged from the hook's — delete body lines, substitute titles, keep the first/last-10-line window. **`scripts/check-forge-text-egress`** is the backstop: every `api_post` / `api_patch` / `-o description=` site must declare `# forge-egress: scrubbed` or `# forge-egress: no-agent-text -- <reason>`, because `auto-pr` alone emits a PR body at three separate points and there is no single choke point to defend instead.
- **`auto-pr` had been discarding every stderr write from its lock line onward.** `exec {FD}>>"$f" 2>/dev/null` applies **both** redirections to the shell permanently — that is what `exec` with no command means — so a suppression meant for one call's error message silenced the remainder of the run, which begins at the top of `do_pr`. An aborting `set -u` in the push path therefore surfaced as `exit 1` with empty stdout *and* empty stderr. Scoping the `exec` to a brace group keeps the allocated descriptor and restores stderr; the other two `exec`-redirect sites in `scripts/`, `.githooks/` and `.agent/hooks/` redirect no standard stream, so this was a singleton rather than a family.
- **The commit-message brand scrub rewrites a vendor-named trailer key**, having passed one through verbatim on 13 commits. **A transcript watcher can no longer outlive the session that launched it**, nor a killed watcher its `inotifywait`; two orphans were found after a session crash, the older at 50 hours.

#### Performance

- **The symbol suffix index belongs to the registry, not the resolver: pretix's Python analysis goes 1700 s → 95 s, a 17.9× speedup with byte-identical output.** The index is whole-registry, and caching it on the resolver rebuilt it per instance; per-file cost is now flat where it had grown 5.70× over a 4× file-count range.

#### CI, release and developer workflow

- **Pull requests that ran no tests now run the right ones.** A `conftest.py` change matched neither the changed-test-file nor the changed-source-file set, so smart-test wrote a manifest with **0 selected tests** — for the one file that governs a whole suite. A docs-only PR selected zero tests, so no doc gate ever ran on the change class it exists for, and `--full` ran nothing and exited 0 whenever the diff held nothing test-relevant.
- **Git notes were never once pushed — 727 of them, since 2026-01-22.** `auto-pr` and `merge-pr` attach the original commit body as a note whenever a rebase rewrites the SHA, and the documented recovery command fetched nothing, because the DCO gate rejected the notes ref and the push was swallowed.
- **hypergumbo was broken on Python 3.10, its own declared minimum**, by one unguarded `import tomllib` — 2905 failed tests on the nightly leg, unreadable behind the matrix's aggregate status. **Forge scripts stop lying about the forge**: `merge-pr` printed a `codeberg.org` link on a GitHub remote. **The bakeoff scripts stop looking for pool repos where they are not**, where cohort selection resolved a repo one level down in a collection subdir and the run step did not.

### Documentation

- **ADR-0055 refuses linker activation tightening, on measurement rather than by default.** The 25 linkers declaring `always_on_unreviewed()` keep their gates. The circulated cost of the change was wrong: the readback script filtered silence on `edges_emitted == 0` alone, ignoring `nodes_emitted`, which is the conflation `pass_silence` warns is "the headline's original error repeated one level down". Recomputed strictly over the 9-repo cohort, the whole gateable prize is **0.166% of pass time**, not ~4.5%, and 10 of the 13 Framework-subcategory candidates already cost zero because they are de-facto gated by their data dependency. The decisive argument is neither cost nor risk appetite: a paired bakeoff differential can only prove no edges were lost *where detection succeeded*, while the risk being run is detection failing on a repo the cohort does not contain — which yields an empty baseline and reads clean. **The change cannot be validated against its own failure mode.** A four-part criterion (zero of 25 qualify), a blocking precondition and three triggers fireable from ordinary bakeoff output are recorded in its place.
- **ADR-0056 rules on the four producerless pass-silence values, and three of them turn out to be mis-hosted rather than unproduced.** A pass that is `dependency_unavailable`, `backend_disabled` or `pass_crashed` never produces an `AnalysisRun`, and `silence_reason` is a field *on* `AnalysisRun` — there is no carrier to stamp, so their home is `limits.skipped_passes[].reason` where their free-text twins already live. `prerequisite_absent` is the one correctly hosted value and is *second-order*: the downstream propagation of the other three, so it cannot be stamped correctly until they have structured producers. The producer INV-hujog asks for is refused on measurement — **98.94% of 229,541 skip records are `no files matched`**, so an unsatisfied `depends_on` conjunct nearly always means the repo lacks that language (State A, a correct no-op) and stamping it as an ordering defect would be the axis's founding sin. INV-hujog's unfireable trigger is restated onto records that already exist and have already fired 2,443 times.

- **AGENTS.md documented a tracker invocation that exits 2, and the failure was unreadable exactly where it landed.** `--json` is registered on the top-level parser, so the trailing form is a usage error written to stderr with nothing on stdout — which a caller piping stdout into a JSON parser reads as a missing item rather than as a bad invocation. The governance text, ADR-0013 in three places and two playbooks are corrected, and a test scanning per backtick code span locks the contract.
- **[`docs/VERIFY-CLAIMS-SCOPE.md`](docs/VERIFY-CLAIMS-SCOPE.md) publishes what `verify-claims` cannot see**, linked from README and SECURITY.md: all eight caveat kinds, the exit-code contract in precedence order (`1` violated → `2` inconclusive → `3` caveated → `0` clean, `2` not being a pass), and the four limits on a clean verdict.
- **The user-facing release notes were three major versions stale, because nothing promoted them.** `docs/RELEASE-NOTES-7.X.md` and `docs/RELEASE-NOTES-8.X.md` are written, each opening with a TL;DR naming the breaking changes, and `prepare-release` now promotes `## Unreleased` and refuses to release when it is missing.
- **ADR-0017's implementation-status markers are re-measured against the tree, five of them corrected**, distinguishing "not implemented" from "implemented but unwired" (§4a `infer_summary` and §7a `is_field_tainted` are tested with zero production callers); ADR-0049's "no catalogue row uses `net_listen`" is excised, eleven do. ADR-0046 through ADR-0051 and ADR-0053 are new this cycle, and ADR-0006's type-inference support table gains measured Objective-C and Swift rows in place of *not assessed*.
- **Surveys and audits.** The stdlib `module_completeness` scope survey finds the audit necessary and not sufficient — only 2 of 42 surveyed repositories are flippable by stdlib work alone. The kind-conformance sweep verifies Rust and Python across 947 method-kind entries and refuses 726 for want of a toolchain, unexamined rather than clean.
...
- **rust-analyzer SCIP symbols keep `stable_id` parity with `rust.py` regardless of the working directory.** The SCIP source reader read `doc.relative_path` (a repo-relative path like `src/lib.rs`) against the process CWD, so any survey run from outside the repo root (absolute-path invocations, monorepo sub-roots, CI runners) failed the read, silently skipped the stable_id parity reassignment, and left the SCIP symbol with a raw-moniker id diverging from the tree-sitter anchor — double-counting Rust functions across the two backends. The reader is now anchored at `repo_root` (upholds the byte-parity contract, ADR-0035 v7). **Correction (INV-dolud, 2026-09-18):** the anchoring fix was real, but the parity it preserved has never held on production input — the helper requires the item span and rust-analyzer supplies the identifier-token range, so 0 of 52 functions emitted by both arms share a `stable_id` on aardvark-dns. The two arms carry independent identities pending WI-gojum.
