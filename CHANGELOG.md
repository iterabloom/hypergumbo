...
- **Per-session transcript sync (ADR-0018 amendment, Option 2)**: the transcript-sync pipeline now isolates each Claude Code / Codex CLI / Gemini CLI session in its own per-session current files keyed by `session_id`. Concurrent sessions in the same repo no longer race on a shared `.current_session_transcript.jsonl` DEST or a single global `.transcript-sync.pid`. **Structural fix for the watcher-leak bug from the 2026-04-08 manual lifecycle test**: `kill-transcript-sync.sh` now matches by session_id (`pgrep argv[+3]`) instead of by DEST, so session B's startup cannot accidentally kill session A's still-live watcher. New `rotate-on-session-end.sh` (called by each vendor's session-end hook) atomically promotes the just-ended session's per-session current files into the global `.last_*`/`.second_to_last_*` slots under a `flock`-serialized critical section; "last session" therefore now means "most recently *ended* session in this repo." `launch-transcript-sync.sh` walks per-session PID files and archives crashed sessions directly to `.archived-transcripts/crashed-<UTC-stamp>-<sid>/`. The legacy global `.transcript-session-token` mechanism, the `.transcript-*` glob session reset in `sync-transcript.sh`, and the rotation-on-start block are removed; per-session DEST and PID file paths make them unnecessary. The first launch after the upgrade runs a one-time legacy cleanup that kills any stranded pre-amendment watcher and removes legacy global state files. New tests: `TestKillScript.test_kill_skips_sibling_session_in_same_repo` (the bug regression guard), `TestLaunchScript.test_launch_does_not_kill_sibling_session`, `TestCrashedSessionOrphan` (orphan archive vs live sibling), `TestRotateOnSessionEnd` (rotation chain + flock serialization + empty-current handling), `TestSyncTranscript`, `TestRecentlyInjected.test_concurrent_sessions_have_independent_state`, `TestInjectionHistory.test_concurrent_sessions_have_independent_sidecars`, `TestSessionIdFromTranscriptPath`, `TestPerSessionNamingInvariants`. Cursor is exempted from concurrent-session support because its transcript backing store is a global SQLite database (`state.vscdb`) shared by all Cursor windows; Cursor is now enforced single-session-per-repo via a sibling check in `cursor/session-start.sh`, and the SQLite-aware per-conversation extractor work is deferred (tracker `WI-rijoj`). Files touched: `launch-transcript-sync.sh`, `sync-transcript.sh`, `kill-transcript-sync.sh`, `poll-transcript-change.sh`, `filter-transcript.py`, `on_transcript_change.py`, new `rotate-on-session-end.sh`, new `session_id_helpers.sh`, all four vendors' `session-start.sh`/`session-end.sh`/polling hooks, `tests/test_watcher_lifecycle.py`, `tests/test_transcript_pipeline_properties.py`, `docs/adr/0018-transcript-sync-and-playbook-injection.md`, `agentic-session-retrospective.md`, `AGENTS.md`, `.gitignore`. Tracker: `INV-filaj` (concurrent sessions invariant, status `pending_validation` until manual Step 5 verification per the original test report) and `WI-rijoj` (deferred Cursor extractor, P4).

### Added

- **`is_test_file` as independent classification axis** (WI-rigun-patuz): adds an `is_test` boolean to `FileClassification` and an `is_test_file` field to `Symbol` so consumers can compose tier and test-ness as independent axes. Bakeoff-deep tier-bounded slice queries that ask "what real third-party deps does this repo have?" were previously polluted by in-tree `_test.go` files because tier 2 conflates `internal_dep` with `test`. The mirror complaint also existed at the other end: "production code only" used `max_tier=1`, which excluded tests *for the wrong reason*. Both queries are now expressible cleanly: production code = `tier <= 1 AND NOT is_test_file`; real third-party deps = `tier == 2 AND NOT is_test_file`. Strictly additive — no tier semantics change, no schema migration. New `TestIsTestFileAxis` pins the joint behavior end-to-end (workspace test, workspace production, workspace `tests/` dir, vendored test inside `node_modules`, fuzz/bench NOT marked `is_test`, `_classify_symbols` propagation).
...
- **`merge-pr close <PR>` subcommand** (WI-vonis): adds the first sanctioned path to close a PR *without merging it*. The AGENTS.md CI-interaction policy previously allowed only `auto-pr`, `merge-pr`, `ci-debug`, and `contribute` to touch the forge API — but none of them exposed a close-without-merge action, leaving agents with two bad choices: ask a human to click a button on the forge UI, or source `lib/forgejo-api.sh` inline (policy violation). The new subcommand validates the PR is open, refuses to "close" an already-merged PR, no-ops on an already-closed PR, and PATCHes `{state: closed}` via the existing `api_patch` helper. An optional `--reason "text"` posts a comment to the PR's issue thread before the PATCH so the audit trail survives on the forge; comment-post failures are non-fatal and do not block the close. Tests exercise ten cases via a stubbed `lib/forgejo-api.sh` that records api_get/api_patch/api_post calls: happy path, already-merged refusal, already-closed no-op, fetch network failure, PATCH network failure, reason-comment ordering, comment-failure-does-not-block, missing-PR-number rejection, help flag, and CI-status-is-not-consulted (close is not gated on CI). Governance file change approved by human prior to implementation per AGENTS.md §"Approval workflow".
...
- **Bakeoff-deep session/iteration tracking integration tests** (WI-gutan): added `tests/test_bakeoff_deep_integration.py` with 13 tests that exercise the `init → cohort → cycle → iter-NNN/` orchestration end-to-end with `_run_cohort_repos` (and, for cycle, `cmd_diagnose`) mocked out so no real `hypergumbo` analysis runs. Covers: (a) `init` creates a fresh timestamped session under workdir with the correct subdirs and a unique `session_id`, (b) `init` rejects a missing pool path, (c) `cmd_run` advances `state.iteration` and creates `out/cohort-NNN/iter-NNN/` populated with per-repo `hg.json` placeholders, (d) the second `cmd_run` on the same session creates `iter-002/` *inside* the existing session (no new top-level deep-* dir), (e) the no-change skip branch (same code hash + existing iter dir) leaves iteration unchanged, (f) `cycle --skip-reflect` invokes run + diagnose without spawning the reflect subprocess, (g) `resolve_workdir` auto-discovers the lexically-newest `deep-*` session under a base path, and (h) `cmd_run`/`cmd_diagnose` error cleanly when prerequisites are missing (no session, no cohort, no run output). Closes the "tested in prod" gap noted by the user 2026-04-06.

### Fixed

- **Stop hook hash recording trips circuit breaker prematurely**: the stop hook recorded a file-change hash on every fire, so 5 stop hook fires could accumulate in ~2.5 minutes when the agent was idle waiting for background sub-agents — tripping the circuit breaker and disabling the stop guard while real work was still in flight. Fix: 150-second pause before recording each hash, so 5 identical hashes take at least 12.5 minutes to accumulate (comfortably longer than typical background agent waits, ~11.5 minutes observed for 3 bakeoff assessment agents). The countdown prints to stderr so the agent sees progress; `STOP_HOOK_DRY_RUN` skips the pause for tests.
- **`ci-debug` returns null on freshly-pushed PR head**: the Forgejo `commits/{sha}/status` endpoint returns HTTP 200 with `"statuses": null` (and `total_count: 0`) when no commit-status entries exist for a SHA — e.g. a freshly pushed PR head that no CI run has reported on yet. `ci-debug` parsed this with `data.get('statuses', [])`, which returned the explicit `null` rather than the default empty list, then crashed on the subsequent iteration. Fix: treat `null` and missing the same way.
- **`ci-debug` job log fetch 404s on Codeberg**: `fetch_job_log()` was hardcoded to use `/logs` for the job log download, which works on self-hosted Forgejo but returns 404 on Codeberg's Forgejo v14+ where `/attempt/1/logs` is required. Fix: select the path by Forgejo version and check HTTP status before parsing.
- **`loop-toggle` rejects uppercase mode arguments**: the session-start hook prompts the agent with the choices `BROAD`, `DEEP`, or `OFF` (uppercase), but `scripts/loop-toggle`'s case statement only matched the lowercase forms (`broad`, `deep`, `off`). Agents that copied the prompt verbatim hit `Unknown command: OFF` and had to recover via `--help`. Fix: lowercase the dispatch argument with bash's `${var,,}` parameter expansion before the case statement so the script accepts any casing (`OFF`, `Off`, `off`, `BROAD`, `Broad`, `broad`, etc.) while the help text and case arms stay lowercase. The error message for genuinely unknown commands still prints the user's original input verbatim. New `tests/test_loop_toggle_case_insensitive.py` parametrizes over all known commands × multiple casings, and includes a regression test that `frobnicate` still produces a clean `Unknown command` error.
- **Go cross-file struct method aggregation** (WI-hobuk): scope expansion from INV-zomuk. The cross-file structural interface matcher in `go.py` iterated per-file when checking which interfaces a struct satisfies, so methods defined in a sibling file of the same package were silently dropped from the struct's effective method set. Concrete failure: a struct declared in `service.go` with `func (s *Service) Notify(...)` and the rest of its method set in `service_helpers.go` (`func (s *Service) Close(...)`) failed to satisfy any `Notifier` interface that required both methods, even though the package-level method set was complete. The bug had been documented as a `pragma: no cover` branch with a TODO pointing at WI-hobuk. Fix: aggregate `struct_method_sets` per package directory (Go's package = one directory) before iterating struct candidates. Two structs of the same name in *different* directories still keep disjoint method sets (preserving the INV-zomuk fix), but methods split across sibling files in the *same* directory now merge into one effective method set per struct. The struct-symbol lookup also widens from per-file to per-package so the type declaration's actual file gets the `base_classes` annotation regardless of which sibling supplied the methods. Three new tests in test_go.py::TestGoStructuralInterfaceMatching cover: methods split across sibling files (basic case), all methods in a sibling file with type alone in another (degenerate case), and the cross-package shadow (regression guard for INV-zomuk).
- **`bakeoff-deep init` silently encourages the init-anti-pattern** (WI-kumub): `init` always created a brand-new timestamped session, even when a recent same-pool + same-code-hash session existed that the user almost certainly wanted to iterate into (`cycle` → `iter-002/`, `iter-003/`, …). This was the exact anti-pattern described in the iteration-vs-new-session playbook — the one that wasted 20+ near-empty nested session dirs in Feb 2026. Fix: before creating a new session, `init` now calls `_check_recent_session_for_init` which returns the most recent session if it matches BOTH the current pool path AND the current hypergumbo editable-install code hash AND was created within the 7-day window. When all three match, `init` prints a prominent multi-line warning to stderr pointing at `./scripts/bakeoff-deep cycle`, sleeps 5s so it's hard to miss, then proceeds. The warning is opt-out via `BAKEOFF_DEEP_SKIP_RECENT_WARNING=1` for scripted batch use. Corrupt/partially-written state.json files are tolerated (try/except around load_state) so a broken session never blocks init. 10 new tests in tests/test_bakeoff_deep_init_recency.py cover: empty workdir no-warn, recent-matching warn, 8-day-old no-warn, different pool no-warn, different code hash no-warn, empty current hash no-warn, missing created_at no-warn, corrupt state.json no-warn, prefix filter (broad vs deep), and missing workdir path.
