...
- **`Symbol.kind`** — 0003 Cluster 27A canonicals (50 values RESOLVED), 0005 Cluster 27B file-shape (17 values), 0006 Cluster 27G build/config (24 values), 0007 Cluster 27H long-tail (60 values), 0009 Cluster 27C apex/peer (4 values), 0010 Cluster 27E edge-label kinds (12 values), 0011 Cluster 27F component_ref (5 values), 0013 Cluster 27D framework-role (29 values).
- **`Edge.evidence_type`** — 0004 Cluster 28A canonical inference (110 values RESOLVED), 0008 Cluster 28B `*_unresolved` (18 values), 0012 Cluster 28D apex/peer call-construct (28 values), 0014 Cluster 28C framework-dispatch (65 values).
...
- **Wave 3 (WI-nunal) — `Edge.evidence_type` Cluster 28B `*_unresolved` fold**: 18 emit sites across 11 producer files migrate to `evidence_type=<canonical>` + `Edge.is_resolved=False`. First production write of `is_resolved`. Two new Cluster 28A canonicals (`grpc_stub_resolution`, `luajit_ffi_lookup`) absorb prior emit sites without a canonical inference label. Audit-findings 0008.
- **Wave 4 — Symbol.kind Clusters 27C/27E/27F + evidence_type Cluster 28D**:
    - **WI-rusit / Cluster 27C apex/peer** — `fn` / `var` / `proc` / `structure` registry classification updates (no producer change; apex emit was already canonical). Audit-findings 0009.
    - **WI-zarov / Cluster 27E edge-label kinds** — `call_site` added as new Cluster 27A canonical. Sub-case (a) reclassifies 4 producers (subprocess/db_query/abi/twig function_call) to `kind="call_site"` + `meta["call_kind"]`. Sub-case (b) drops Symbols whose relationship is already on a companion Edge: 3 clean drops (css/jsonnet imports, bitbake inherit), 6 shape-2 edge-endpoint redesigns (json_config/puppet/scss/twig), and 4 shape-3 companion-Edge introductions (r_lang library imports, astro frontmatter imports, make include, blade @extends). Audit-findings 0010.
    - **WI-mihiz / Cluster 27F component_ref** — vue / svelte / astro drop per-reference Symbols; `imports` Edges re-route src to `make_file_id()` and dst falls back to a 5-part dangling component id when unresolved. DEPRECATE-NO-FOLD verdict (the original fold target `reference` was already deprecated in 0010). Audit-findings 0011.
    - **WI-nibis / Cluster 28D evidence_type call-construct** — 28 peer values (`function_call`, `method_call`, `function_application`, `pipe_call`, `typed_receiver_call`, `ambiguous_method_call`, …) collapse to apex `ast_call` + `meta["call_construct"]` (the lone non-`ast_call` apex is `cross_file_message_send` → `message_send` + `meta["call_construct"]="cross_file"`). 89 emit sites across 26 producer files. Audit-findings 0012.
...
- **Wave 6 (WI-runod / WI-gajob / WI-dubab / WI-nupus) — Symbol.kind Clusters B/G/H sweep across six PRs**:
    - **PRs 1-2 — 74 canonical promotions**: 6 Cluster 27B file-shape (`file` / `library` / `package` / `executable` / `program` / `project`), 15 Cluster 27G build/config DSL constructs, 53 Cluster 27H domain-DSL constructs. No producer code change; registry axis classification only.
    - **PR 3 (WI-gajob) — 11 Cluster 27B FOLDs**: framework / ecosystem-qualified file/package/export/module values fold to canonical + meta keys (e.g. `module_file` → `file` + `module_system`, `npm_package` → `package` + `package_ecosystem`, `composer_package` → `package` + `package_ecosystem`, `wasm_module` → `module` + `compilation_target`, `tsconfig` → DEPRECATE-NO-FOLD via the v4.0.0 `is_config_file` boolean). Consumer dual-shape predicates added at `route_handler.is_component` and `cli._is_noise`.
    - **PR 4 (WI-runod) — Cluster 27G/27H DEPRECATE-NO-FOLDs + reclassifications**: 27G's `config` (Prisma generic block — producer rewrites to `kind="block"`), `dev-dependency`, `build-dependency`, `work_item` (dead vocabulary). 27H's `heading`, `model` (DEPRECATE-NO-FOLD; advanced PRELIM_RESOLVED). Plus 3 CANONICAL reclassifications (`theorem`, `inductive`, `message` had real producers via indirection at `lean.py` / `tlaplus.py` / `proto.py` — surfaced the methodology gap that Step 4.5 now closes).
    - **PR 5 (WI-dubab) — Cluster 27G FOLDs**: `test_case` → `test` + `test_dialect`, `editable` / `url_requirement` → `requirement` + `install_mode`/`install_source`, `devDependency` → `dependency` + `dependency_scope`, `python_task` → `task` + `task_implementation`. Consumer surfaces unchanged (canonical fold targets already in `EXCLUDED_KINDS` / `_DEPENDENCY_KINDS`).
    - **PR 6 (WI-runod) — Cluster 27H boundary-pseudo-symbols**: `external_symbol` reclassifies DEPRECATE-NO-FOLD → CANONICAL on `language_construct` (pipeline-DSL top-level construct); `unresolved` advances UNRESOLVED → PRELIM_RESOLVED (registry seed error, no producer). Hypothetical `Symbol.is_resolved` follow-on ADR **withdrawn** — re-trace showed no leak to absorb (1 producer, 0 consumer reads, fails ADR-0024 fold-residue rule 3 threshold). With this PR, ADR-0027 Phase 3 producer migration is empirically complete on `Symbol.kind`.
