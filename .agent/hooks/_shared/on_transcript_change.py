...
PLAYBOOKS = [
    ...
    ("bakeoff-validation-tagging-discipline",
     ...
     "PR merge time whenever the PR description or tracker discussion contains "
     "a quantitative bakeoff-improvement claim. Verb-forms that count and "
     "trigger the tag: 'should improve X by N%', 'expected FP reduction of N', "
     "'N dead → alive' / 'N alive → dead', 'NN% reduction' / 'NN% improvement', "
     "'below threshold X' (any numeric threshold), 'newly-consumed concept' "
     "(asserting a concept flips from inert → live), raw candidate-count deltas "
     "attributed to the change. Verb-forms that do NOT count: qualitative "
     "claims like 'handles the case' / 'covers the pattern', coverage or "
     "test-count deltas, performance micro-benchmarks unrelated to the bakeoff "
     "corpus. The authoritative running list is `scripts/tracker list --tag "
     "awaits_bakeoff_validation` — single source of truth, supersedes any "
     "hand-maintained pattern. Lifecycle: tag stays until a DEEP-mode bakeoff "
     "cycle reproduces the claimed metric movement; on confirmed `moved` the "
     "tag is stripped via a resolution discussion linking the cohort where it "
     "was validated; on `no_move` a regression sub-item is filed parent-linked "
     "AND WITHOUT the awaits_bakeoff_validation tag (regressions don't inherit "
     "the validation queue, they're their own work); on `inconclusive` the tag "
     "stays with a discussion noting what would unblock. The stop hook "
     "surfaces the tag automatically: when the count of tag-bearing items in "
     "a blocking status reaches `threshold` AND the most recent DEEP bakeoff "
     "cycle's state.json is older than `stale_cycle_hours`, an `## AWAITS_"
     "BAKEOFF_VALIDATION BACKLOG` section is appended to the active guidance "
     "file pointing at `./scripts/bakeoff-deep cycle`. Both knobs live under "
     "`stop_hook.awaits_bakeoff_validation_nudge` in `.agent/tracker/config."
     "yaml` with defaults threshold=5 and stale_cycle_hours=72. Worker module: "
     "`.agent/hooks/_shared/awaits_bakeoff_nudge.py`. Integration with "
     "`bakeoff-deep-reflect aggregate`: at aggregation time the reflect pass "
     "cross-references active items against the cohort's diagnostic output, "
     "injecting a per-claim question into the reflect prompt with three "
     "verdicts (moved / no_move / inconclusive); on moved the tag is "
     "auto-stripped with evidence, on no_move the regression sub-item is "
     "created. The aggregation glue is separate implementation work; the "
     "discipline rule is in force independently of that tooling — manual "
     "tagging at merge and manual stripping post-cycle govern when "
     "auto-injection is offline. This playbook is the trigger half of the "
     "lifecycle (per-PR cadence, any PR-merging agent); the drain half (when "
     "the queue is ~15+ items) lives in the process-validation-queue-with-"
     "bakeoffs-and-uat playbook with its own seven-phase routine."),
    ...
    ("ci-debug-protocol",
     ...
     "Never use pytest.skip() to hide failures. Also covers `auto-pr` exit-code recovery — "
     "when ./scripts/auto-pr exits non-zero, recover by code: Exit 0 (success — PR merged "
     "or vPR queued; flush vPR with ./scripts/auto-pr flush when remote returns), Exit 1 "
     "(failure — diagnose with ./scripts/ci-debug status, fix, then re-run auto-pr or "
     "./scripts/merge-pr <PR_NUM> --wait-for-ci), Exit 2 (timeout — try ./scripts/merge-pr "
     "<PR_NUM> --wait-for-ci --timeout 3600, or merge-pr <PR_NUM> if CI already passed; if "
     "still stuck, Scenario B), Exit 3 (hung — auto-pr already retried with exponential "
     "backoff up to 4 times, CI runners may be down, Scenario B; do NOT manually kill "
     "processes, clear PR_PENDING, or start new branches). Scenario B (CI stuck): do NOT "
     "accumulate more git-tracked changes, run ./scripts/ci-debug status once per hour "
     "manually (not in a loop), use ./scripts/merge-pr <PR_NUM> when CI recovers — it is "
     "fine to wait."),
    ...
    ("cruft-audit-playbook",
     ".agent/agent_playbooks_protocols_sops_skills/cruft-audit-playbook.md",
     "Periodic audit of playbooks/AGENTS.md/hook summaries for dead text — stale "
     "'until X lands' workarounds where X has shipped, workarounds for problems "
     "the tooling now handles automatically, session-anchored references with no "
     "recoverable referent ('this session', 'the user said', no quote / no link "
     "/ no extracted pattern), pure historical records of specific past events "
     "with no extracted teaching shape, dead temporal qualifiers ('now', "
     "'currently', 'still') that presuppose reader memory of a transition. "
     "Two-pass methodology: (1) syntactic grep for history markers, temporal "
     "qualifiers, session anchors, 'until X lands' patterns, FIXMEs / TODOs in "
     "prompt text — also verify referenced files / scripts / tracker IDs are in "
     "their claimed state; (2) semantic read pass applying the calibrated "
     "taxonomy. CRITICAL: BOTH passes are mediated by interactive interview with "
     "the human — same word is sometimes cruft, sometimes load-bearing (a "
     "'(deprecated)' next to a tracker status that still appears in live data is "
     "anchoring a term, not dead text). Do NOT auto-apply syntactic-pass hits. "
     "Surface candidates with file path, line range, and ~5 lines of surrounding "
     "context; have the human classify each cruft / not-cruft / explain. "
     "Calibration loop: round 1 no-opinion (~5-7 candidates), round 2 with "
     "auditor opinion + reasoning, optional round 3 for final calibration, then "
     "autonomous pass. The calibrated taxonomy: CRUFT (delete) — stale 'until X' "
     "workarounds, workarounds for problems tooling now handles, session-"
     "anchored references with no recoverable referent, pure historical records "
     "with no extracted teaching shape; TRIM (one-word / short-phrase removal) — "
     "stale temporal qualifiers, session-anchor prefixes/parentheticals when the "
     "rest of the sentence already extracts the recoverable pattern, truism "
     "reminders redundant with concrete surrounding rules; NOT CRUFT (keep) — "
     "worked-example anchors that survive future state changes, rationale or "
     "consequences beyond the rule, concrete-situation illustrations anchoring "
     "pattern recognition (even when reductively redundant), live-data anchors "
     "(terms still appearing in extant tracker items / archives / code), "
     "generative/teaching prose for hard-to-articulate skills, fallbacks for "
     "templatization or fork or fresh-clone scenarios, borderline → keep when "
     "cost is small. Doc consistency issues (e.g., overview heading undercount) "
     "are a separate category — only flag when likely to derail an agent AND "
     "fix has bounded ripple. Output: an audit report at "
     "~/hypergumbo_lab_notebook/cruft_audit_<MMDDYYYY_HHMM>.md (do NOT auto-"
     "apply); the human decides whether to apply. The 'items considered and "
     "rejected' section is required — transparency about why representative "
     "not-cruft cases were kept protects against over-trimming on future runs. "
     "Anti-patterns: auto-applying syntactic hits, skipping calibration rounds, "
     "reductive logical interpretation ('these all resolve to the same action — "
     "redundant' misses pattern-recognition work), trimming worked examples to "
     "abstract them, failing to verify referenced tracker IDs / file paths."),
    ...
    ("twenty-pass-dogfood-procedure",
     ...
     "with cross-tranche trend comparison as the goal. Seven phases orchestrated via sub-agent "
     "spawning (no operator-babysat tmux). TWO design invariants govern every worker prompt: "
     "(1) de-primed discovery -- each discovery/audit worker receives the curated real-issue "
     "registry (the agent_notes.json issues table; the parent grows it between chunks by "
     "appending ONE neutral row per new finding, schema Tracker ID|Pri|Status|Parent|Title with "
     "no severity/KIND/novelty column, which is the mechanism that avoids re-discovering known "
     "issues without anchoring the worker) and navigates toward unmapped ground, never told to "
     "'expect re-confirmation' or to pre-judge novelty (downstream dedup stages decide that), "
     "and never handed another worker's raw output; (2) parent-owned numbering -- workers use "
     "local opaque labels and never learn their tranche ordinal or global pass numbers; the "
     "parent stamps global identity (pass_NN.md, F-numbers) only on aggregation. A worker "
     "prompt is task + inputs + output format, nothing about its place in the campaign or being "
     "graded. Phase 0 freezes substrate + writes substrate guide + assembles the full judge "
     "primer (domain primer + project spec + 1-5 rubric) + commits the 'judge underlying "
     "problems not symptoms' meta-criterion + seeds the tranche working registry; Phase 1 runs "
     "passes in --chunk-size sub-agent chunks (default 2; the sub-agent boundary IS the "
     "compaction event) with fresh-substrate verification for negative findings, the parent "
     "folding each chunk's new findings into the registry as table rows between chunks "
     "(--discovery-mode region-parallel or sequential); Phase 2 (mid-tranche, default pass 10) "
     "and Phase 2.5 (post-discovery) run NEUTRAL symmetric audits on locally-renamed first/"
     "second-half passes -- catching over/under-folding + amplification + unverified negatives "
     "WITHOUT pre-naming findings or verdicts, Phase 2.5 also enumerating would-have-been-pruned "
     "candidates for Phase 6.4's retroactive-cleanup decision; Phase 3 runs TWO sequential "
     "consolidation sub-agents (methodology-axis dedup assigning KIND fresh, then causal-axis "
     "clustering with rejected-clusters log) plus a STRUCTURALLY-blinded card-writer (parent "
     "stages allowed files in a staging directory rather than declarative 'don't read X' -- "
     "Christian & Mazor 2026 showed declarative blinding fails); Phase 4 fans out --judge-count "
     "blind judges (default 1; strict one-sub-agent-per-card at judge_count>=2, batched-absolute-"
     "scoring acceptable at judge_count=1), each judge staged with only the full primer + the "
     "single card under judgment; Phase 5 builds the cluster-aware trend report (single carbon-"
     "dating attribution = earliest cluster member, with a same-pass sanity assertion; the former "
     "Methods A/B/C collapse until a multi-pass cluster appears) and the retrospective; Phase 6 "
     "materializes the cohort as tracker rows (priority from blind-judge severity, tagged with "
     "tranche ID, parent attached when an existing META cleanly covers) and reconciles the "
     "validated cohort into agent_notes.json via replace_once-style targeted edits; Phase 7's "
     "coherence sweep resolves accumulated drift AND enforces the boundary that campaign-"
     "measurement framing (trend means, bucket rollups, READ-THIS-FIRST headers) stays OUT of "
     "the registry and lives in the trend/index files instead. Tranche-state JSON is the single "
     "source of truth across sub-agents; sentinel files signal phase transitions. Tunable: "
     "--chunk-size, --judge-count, --mid-tranche-pass, --tranche-size, --substrate-policy, "
     "--discovery-mode. Vendor parity table covers Claude Code (verified) + Codex / Cursor / "
     "Gemini CLI (unverified, verify-before-tranche protocol documented). Appendix A documents "
     "the tmux fallback when sub-agent spawning is unavailable; Appendix B records the de-priming "
     "rationale (per-cut traceability of every removed prime)."),
