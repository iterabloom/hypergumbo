...
PLAYBOOKS = [
    ...
    ("twenty-pass-dogfood-procedure",
     ...
     "(default 2; the sub-agent boundary IS the compaction event, no /compact keystroke "
     ...
     "negative findings; Phase 2 (mid-tranche, default pass 10) runs an interim audit catching "
     ...
     "'don't read X' instructions — Christian & Mazor 2026 showed declarative blinding fails); "
     "Phase 4 fans out --judge-count blind judges per card (default 1; upgrade to 3 for "
     "inter-rater variance by re-running additional judges on the same cards later — "
     "reversible), each judge spawned with its own per-card staging directory containing only "
     "the rubric + the single card under judgment; Phase 5 builds the cluster-aware trend "
     "report (Methods A/B/C carbon-dating) and the retrospective (KIND-tag stability, "
     "rejected-clusters scoring, methodology carry-forward); Phase 6 materializes the cohort as "
     "tracker rows (each cluster + each INDEPENDENT singleton, priority derived from blind-"
     "judge severity, tagged with tranche ID, parent attached when an existing META cleanly "
     "covers), integrates the tranche into agent_notes.json via replace_once-style targeted "
     "edits (headline numbers + cohort sub-section + parent-row cross-references + bucket-Σ-"
     "severity rollup), appends cross-tranche index, and (optionally, under operator edit-"
     "mode authorization) retroactively cleans up any over-folded discussion entries on "
     "parent rows via tracker edit-msg-text / delete-msg. Tranche-state JSON file is the "
     "single source of truth across sub-agents; sentinel files signal phase transitions. "
     "Tunable: --chunk-size, --judge-count, --mid-tranche-pass, --tranche-size, "
     "--substrate-policy. Vendor parity table covers Claude Code (verified) + Codex CLI / "
     "Cursor / Gemini CLI (unverified, verify-before-tranche protocol documented). Appendix A "
     "documents the tmux fallback when sub-agent spawning is unavailable for the vendor."),
