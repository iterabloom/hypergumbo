...
PLAYBOOKS = [
    ...
    ("twenty-pass-dogfood-procedure",
     ...
     "bucket-Σ-severity prediction; Phase 1 runs passes in --chunk-size sub-agent chunks "
     ...
     "rejected-clusters log) plus a context-blinded card-writer sub-agent; Phase 4 fans out "
     "--judge-count blind judges per card (default 1; upgrade to 3 for inter-rater variance "
     "by re-running additional judges on the same cards later — reversible); Phase 5 builds "
     "the cluster-aware trend report (Methods A/B/C carbon-dating) and the prediction-vs-"
     "outcome retrospective with a sycophancy review; Phase 6 materializes the cohort as "
