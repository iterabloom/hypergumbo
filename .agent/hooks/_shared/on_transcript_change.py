...
PLAYBOOKS = [
    ...
    ("twenty-pass-dogfood-procedure",
     ...
     "judge severity, tagged with tranch ID, parent attached when an existing META cleanly "
     "covers), integrates the tranch into agent_notes.json via replace_once-style targeted "
     "edits (headline numbers + cohort sub-section + parent-row cross-references + bucket-Σ-"
     "severity rollup), appends cross-tranch index, and (optionally, under operator edit-"
     "mode authorization) retroactively cleans up any over-folded discussion entries on "
     "parent rows via tracker edit-msg-text / delete-msg. Tranch-state JSON file is the "
     "single source of truth across sub-agents; sentinel files signal phase transitions. "
     "Tunable: --chunk-size, --judge-count, --mid-tranch-pass, --tranch-size, "
     "--substrate-policy. Vendor parity table covers Claude Code (verified) + Codex CLI / "
     "Cursor / Gemini CLI (unverified, verify-before-tranch protocol documented). Appendix A "
