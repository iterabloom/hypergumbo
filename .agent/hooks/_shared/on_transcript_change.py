...
PLAYBOOKS = [
    ...
    ("twenty-pass-dogfood-procedure",
     ...
     "Vendor-neutral procedure for a ~20-pass dogfooding tranche, run by an orchestrating agent "
     "that spawns sub-agents. THE ONE RULE: the agent's job is exactly two things -- dogfood "
     "(find issues NOT already found) and orchestrate the machinery; the HUMAN decides what the "
     "trend is, whether it is converging, when it is 'done', and what to do next. The agent must "
     "not even PERCEIVE convergence/cadence during discovery (perceiving it biases effort + "
     "pre-judges novelty, making a low yield a self-fulfilling artifact). Two STRUCTURAL "
     "guarantees enforce that: (1) the issue ledger (agent_notes, loaded WHOLE by each worker via "
     "`scripts/agent-notes --show`) is campaign-position-free -- a flat list of distinct known "
     "issues with zero cadence signal (no tranche names, pass numbers/ranges, dates-as-timeline, "
     "cohort groupings, counts); workers INTEGRATE each finding (ADD a row if new, ENRICH an "
     "existing row if a facet/refinement/correction) using judgment, with neutral local handles "
     "and no severity/KIND/novelty/cadence labels; (2) pass->row->severity provenance (which the "
     "human needs to plot summed-severity-over-time) lives in a SEPARATE orchestrator-only map "
     "(pass_row_map.json), NEVER read by any worker. Discovery is SEQUENTIAL -- one chunk at a "
     "time (chunk = context unit; the sub-agent boundary IS the compaction event; default "
     "--chunk-size 2); never parallel, never 'independent' passes. A worker prompt is task + "
     "inputs + output format only; it never learns its tranche ordinal or global pass numbers, "
     "and the orchestrator stamps global identity (pass_NN.md, F-numbers) + updates the map on "
     "aggregation. Phase 0 freezes substrate + locks rubric/judge-primer + commits the 'judge the "
     "underlying problem not the symptoms' meta-criterion + confirms the ledger is cadence-free. "
     "Phase 1 runs sequential discovery chunks (fresh-substrate verification for negatives). "
     "Phases 2/2.5 run NEUTRAL symmetric audits (over/under-fold, amplification, unverified "
     "negatives) without pre-naming verdicts. Phase 3 runs two consolidation sub-agents "
     "(methodology-axis KIND assignment, then causal-axis clustering with a rejected-clusters "
     "log) + a STRUCTURALLY-blinded card writer (staging-dir, not declarative 'don't read X' -- "
     "Christian & Mazor 2026 showed declarative blinding fails). Phase 4 fans out --judge-count "
     "blind judges (default 1) and fills severities into the map. Phase 5 assembles per-pass "
     "trend TABLES (count/mean/sum-severity) from the map + a process-facts retrospective -- DATA "
     "ONLY, no convergence verdict, no 'done' call, no next-tranche forecast. Phase 6 "
     "materializes the cohort to the tracker (mechanical; priority from severity; resolved-state "
     "fold targets reopened/failed-validation per rule). Phase 7's coherence sweep keeps the "
     "ledger campaign-position-free, relocating any cadence framing to the analyst-facing "
     "index/trend files. The agent runs every phase and produces data; it never decides "
     "trend/convergence/done/next. Tunables: --chunk-size, --judge-count, --mid-tranche-pass, "
     "--tranche-size (NO discovery-mode -- sequential always)."),
