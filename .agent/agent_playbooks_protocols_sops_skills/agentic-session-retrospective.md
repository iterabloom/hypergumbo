...
4. **"Playbook X was injected when the user didn't say the thing that 'triggers' it."** Wrong unless you've also checked the playbook's *body* against the contexts where it fired. The selector doesn't read the playbook's body — it reads the hook summary registered for it in `.agent/hooks/_shared/on_transcript_change.py:PLAYBOOKS` (the third tuple element). That summary describes when the playbook applies, and it's typically narrower than the body's actual scope. The selector therefore *can* fire on contexts the hook summary describes but which don't visibly match a literal user phrase — and that's often correct, because the body applies. Worked example: the `trackerize` playbook's hook summary describes batch decomposition triggered by the user typing "trackerize". An early reading of one session's injection log flagged 32 of 38 trackerize injections as false positives because the user had only typed the literal trigger 6 times. Reading all 38 distilled goals showed ~7 events of literal trackerize work, ~10–13 events of tracker-filing-adjacent work (scope expansion, "Preparing Tracker Updates", decomposition decisions like "(a) Expand the item / (b) File a sibling / (c) Defer"), and ~18 events of code-refactor/monitoring with likely item-filing at completion. The trackerize playbook's body (steps 1–10: choose kind, assign priority, set sequencing, apply tags, check duplicates, create with full bash example, summarize) is the canonical reference for filing *any* well-formed tracker item, not just for batch decomposition on the literal word — so the selector surfacing it in those contexts is doing its job. Before flagging selection-precision, run the bucket-counting exercise: read every distilled goal where the playbook fired and classify each as "playbook-body applies / partially applies / does not apply". If "does not apply" is a small minority, there is no precision concern. (If you do find a real precision miss, the fix is usually to *broaden* the hook summary so it matches the body's actual applicability, not to narrow it.)

**What ARE valid concerns from the injection log:**

- **Selection precision against the distilled goal — measured by playbook *body* applicability, not by whether the user said something specific.** For each event where playbook X fired, ask: does X's body provide guidance that fits what the agent is doing or about to do? If a meaningful fraction of injections fail that test, the hook summary is mis-specified — and the fix is usually to *bring it into line with the body's actual scope*, in either direction. (Don't conflate "user didn't say a specific phrase" with "the body doesn't apply" — see misreading 4.)
