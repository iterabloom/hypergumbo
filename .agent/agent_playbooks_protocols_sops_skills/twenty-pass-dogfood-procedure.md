...
Assemble the per-pass tables the human needs to judge the trend — straight from `pass_row_map.json`: per-pass finding count, per-pass mean blinded severity, per-pass summed severity (Σ-sev), plus the cluster inventory. These are **data**: present the tables and attach **no** convergence verdict, **no** "slope means X" narrative, **no** "done" call — the human reads the tables and decides whether and how it converged.

**The canonical Σ-severity unit is NET-NEW discovery — this is the whole point of the cross-tranche table ("to what extent are we finding net-new problems over time").** The canonical cohort = NOT_A_DUPLICATE + RELATED_BUT_DISTINCT findings + any TRUE_DUPLICATE that triggered a state-change on its fold target; it **EXCLUDES TRUE_DUPLICATE-no-state-change** cards (redundant re-discovery of already-actionable work — real severity *surfaced*, but not net-*new*). Also record the full cohort's Σ as a **labeled "surfaced (pre-dedup) reference"** — but never put surfaced numbers in the canonical column or the cross-tranche table; they are not comparable across tranches. (A tranche that re-found 47 already-tracked bugs would post a big *surfaced* Σ and *zero* net-new discovery — the canonical column must not reward that.)

**Count net-new from the CLASSIFICATION, never from the materialization action.** Cohort membership keys on each card's `dup_class` (NOT_A_DUPLICATE / RELATED_BUT_DISTINCT / TRUE_DUPLICATE, plus the state-change flag), NOT on whether Phase 6 filed it as a new tracker row or folded it via `discuss`. A RELATED_BUT_DISTINCT finding counts as net-new whether it became a new row or a `discuss`-fold — so the (item-specific) materialization-form choice in Phase 6 can never corrupt the trend.

**Sequencing — the canonical trend is finalized in Phase 6, not here.** Net-new membership depends on the TRUE_DUPLICATE / RELATED / state-change dispositions, which are produced by the Phase 6 tracker-dup search. So Phase 5 emits only what needs no dispositions — the *surfaced* per-pass severity (from `pass_row_map.json`) and the retrospective; the **canonical net-new `trend_cluster_aware.md` and the combined-trend regeneration are the last data steps of Phase 6.** Do not compute or report a canonical net-new number before Phase 6 dispositions exist.

**Artifact + format contract (so the combined builder ingests the tranche).** The per-tranche trend file the combined builder (`scripts/build_combined_trend.py`) auto-discovers is **`${TRANCHE_DIR}/trend_cluster_aware.md`** — that exact name (NOT `trend.md`; naming it `trend.md` makes the tranche invisible to the builder). It must contain a `## Per-pass SEVERITY SUM` section with a `| Pass | Method A | Method B | Method C |` table (Method A = earliest contributing pass credited full; B = latest; C = severity distributed 1/n across contributing passes — the canonical carbon-dating unit), an "N cards" string for the builder's card-count grep, and a `tranche_state.json` carrying **top-level** `first_pass_number`, `last_pass_number`, and `judge_count`. A separate human-readable `trend.md` may also be written, but it is NOT what the builder consumes.
...
- For each issue that should be its own row (each cluster + each INDEPENDENT singleton), search the tracker for duplicates across all kinds/statuses; classify TRUE_DUPLICATE / RELATED_BUT_DISTINCT / NOT_A_DUPLICATE (LOW-confidence → RELATED_BUT_DISTINCT, never silently lose information). File new rows (`scripts/tracker add --kind work_item …`, tag `dogfood`) or fold onto existing rows (`tracker discuss …`). Priority may be derived from the blinded severity (5→P0 … 1→P4) as a mechanical mapping.
  - **New-row vs `discuss`-fold for a RELATED_BUT_DISTINCT finding is item-specific — decide by this test:** file a **NEW row** when the distinct issue has its own fix and can be closed independently of the related item; **fold via `discuss`** when it's a refinement that resolves with the same work or isn't independently actionable. **When in doubt → new row** (cross-linked), because a distinct issue folded into a host item is silently lost when the host closes (the same failure mode as an unflagged over-fold; mirrors the Phase-3 "when in doubt → INDEPENDENT" / "default to split" safe-defaults). This choice is **tracker-hygiene only — it does NOT affect the trend**, which counts RELATED_BUT_DISTINCT as net-new either way (Phase 5: count from classification, not action).
...
- Record each card → tracker row in `tracker_materialization.tsv` (one row per card: `card_id`, `severity`, `action`, `tracker_id`, **`dup_class`**). The `dup_class` column is load-bearing — it is what the net-new trend keys on, so every card must carry one.
- **Finalize the canonical net-new trend + regenerate the combined cross-tranche trend (the last data steps).** Now that `tracker_materialization.tsv` carries every card's `dup_class` + action, build the net-new cohort (drop TRUE_DUPLICATE-no-state-change; keep NOT_A_DUPLICATE + RELATED_BUT_DISTINCT + state-change triggers), write the canonical `${TRANCHE_DIR}/trend_cluster_aware.md` per the Phase-5 artifact/format contract (Method A/B/C `Per-pass SEVERITY SUM` table over the net-new cohort, plus the labeled surfaced pre-dedup reference), then run `scripts/build_combined_trend.py` and add the tranche's row to the cross-tranche index. **Verify self-consistency before moving on:** the new tranche must appear in `dogfooding_trend_combined.md`, and its combined-row Σ must equal the per-tranche canonical Σ (and the index row must match). A mismatch means the builder didn't ingest the file (wrong name/format — see Phase 5 contract) or the cohort was mis-built (e.g. surfaced Σ used instead of net-new) — fix before proceeding.
- **High-severity net-new root-review (the convergence read).** The per-pass Σ collapses a *re-instance of an already-known root* and a *genuinely new serious problem* into the same number, so Σ alone can read as convergence when it isn't. The distinction lives in whether each high-severity net-new finding is a NEW root or another INSTANCE of a known root — a judgment that is error-prone, needs full context, and is a convergence-perception act, so it stays with the **human**; the agent only collates the few findings where it matters and recommends. Run (it needs the Phase-6 `dup_class`/`action`, hence here not Phase 5): `python3 scripts/highsev_root_review.py ${TRANCHE_DIR}`. It is purely mechanical, zero root-cause judgment: it filters to the high-sev net-new set (`severity ≥ 4 AND dup_class ≠ TRUE_DUPLICATE`), prints the flat list with `*standing-inv` (a candidate is a standing `INV-`/META invariant) and `↩folded-onto-existing` (the card folded onto a pre-existing item) "possible known root" flags, forms candidate-groups from three signals (shared `candidate_tracker_id` / shared standing invariant / shared snake_case identifier in subject+fix — snake_case-only on purpose, generic words over-group), and stages one analyst prompt per group at `${TRANCHE_DIR}/.highsev_review/G*/analyst_prompt.md`, pinned to the substrate's `setup_repo_head`. Then per group spawn ONE **cadence-blind, code-and-tracker-EQUIPPED** analyst (the staged prompt): it reads the actual source *at the pinned HEAD* plus `scripts/tracker show`, argues BOTH sides (one root vs distinct roots), and emits an advisory RECOMMENDATION. Blinding is cadence-only — the analyst must NOT see the trend / pass numbers / Σ / the word "converge" — but repo + tracker access is **mandatory**: root cause is a fix-site fact, not a wording fact, so an analyst starved of the code falls back to symptom-matching tracker titles and its reasoning is bogus. Present the recommendations; the **human accepts or rejects each**, and that accept/reject is the tranche's root-novelty record. The agent collates and recommends; the human decides — **no automated root number is ever fed back into the trend**: the Σ column stays exactly as Phases 5/6 define it, and this is a separate human-facing read, not a new trend unit.
