...
- **Over-fold split → original-item cleanup is DEFERRED and needs human edit mode.** When an over-fold (whether caught by the Phase 2/2.5 audits, by the Phase 3a shared-fix test, or in a Phase-7 reconciliation) is corrected by splitting a sub-mechanism into a NEW tracker item, the ORIGINAL item's title / description / earlier discussion still describe the pre-split bundled scope — now redundant with the new item. The agent **cannot** edit or delete tracker discussion messages: only the human can `tracker edit-mode on` from the human account (OS-enforced), after which `edit-msg-text` / `delete-msg` / title edits become possible. So the protocol is: create the new item and add an **ADD-only** split/scope discuss note to the original **whenever** (this never needs edit mode), but **defer the redundant-prose cleanup to the end** of the tranche and write a precise `edit_mode_cleanup_manifest.md` (which item, which field/message, what to trim) — so the cleanup is a quick mechanical pass once the human enables edit mode, and the rest of the tranche is never blocked on the human being away from keyboard. Mirror every split into the agent-notes ledger too (cross-ref the original row + add one ID-first row per new item), then re-run `deleak-ledger --check`.
...
1. **Run it** (it never overwrites its input): `scripts/deleak-ledger --in <agent_notes.json> --out /tmp/ledger.deleaked.json --map <lab-notebook>/pass_row_map.json --report /tmp/deleak_report.md`.
...
4. **Eyeball, then swap.** A human reads `/tmp/ledger.deleaked.json`; then swap it into `agent_notes.json`, keeping the pre-swap ledger as a durable backup in the lab notebook (`agent_notes.pre-deleak-backup.json`). Keep the `pass_row_map.json` it produced alongside the ledger.
