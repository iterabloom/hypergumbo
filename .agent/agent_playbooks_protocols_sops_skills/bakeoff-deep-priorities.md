...
7. **Linkers:** post-call-graph-time edge recovery across four subcategories (Protocol / Bridge / Framework / Infrastructure — see [ADR-3bbb: Linker Subcategory Restoration](../../docs/adr/3bbb-linker-subcategory-restoration.md) and [docs/LINKERS.md](../../docs/LINKERS.md) for the 45-linker catalogue). Prioritise by expected false-positive-reduction volume on the current prospector corpus (INV-nimuj), not by novelty of language pair.
