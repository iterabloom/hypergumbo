...
**Target one bullet per feature or fix cluster — not one per tracker item or per PR.** A feature that accreted across several tracker items or PRs collapses to a single entry describing its effect; a multi-bullet family under a `####` subhead is usually the signal to consolidate. Err toward the released granularity: if the Unreleased section is several times the size of a comparable released section, it is too detailed regardless of how much work it covers — cut toward the released level, not away from it.
...
- **Strip tracker identifiers; keep ADRs, code identifiers, and version transitions.** Tracker-item IDs (`WI-`/`INV-`/`META-`) and internal audit pointers (`audit-findings NNNN`) are inside-baseball and do not belong in the changelog — remove them when adding or consolidating entries. ADR references, code/API identifiers (field names, functions, flags), and every `SCHEMA_VERSION` / scheme / view-version transition *do* belong: they match the released sections and are meaningful to a reader. When consolidating, preserve every ADR reference and version transition; git history and the tracker hold the fine-grained item-level traceability.
- **Do not fabricate.** Don't invent attribution, PR numbers, versions, or claims that weren't in the source material.
