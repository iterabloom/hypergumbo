...
| [0010](0010-deferred-crossing-findings-per-shape.md) | Adjudicated deferred-crossing findings, per shape | `measure-taint-precision.py collect` + per-arm ledgers | **The ruling holds for LAUNCH and fails for HANDLE.** Two arms. Arm 1 (measurement 0006's own two-pass labels): the family is **7 situations of 112, 0 TP, 0 useful** against a 33.0% population. Arm 2 (17 repos, this tree, fresh single-pass): **50 situations / 411 rows, 48.0% correctness, 22.0% useful** — and shape is not the discriminator, MECHANISM is: `DEFERRED` 0/3 TP, `WIRING` 0/8, `WRONG-CHANNEL` 10 TP but **0 useful**, `READ-IN-SCOPE` **11 useful of 22**. The pre-registered refutation FIRED: `bufio.NewReader(os.Stdin)` read four lines later is a true, useful finding, and Go's *entire* stdin surface is three deferred-crossing rows with no catalogued read to relocate to — so retagging HANDLE is forbidden by ruling 3 on its own terms. LAUNCH is unanimous (0 TP) and **thin: 2 findings against 49 files** containing a launch-shaped call, because jaeger returned `inconclusive` and cilium's framework rows are `WI-lalot`-inert. Also: the first **3 CONFIGURED-ACTION** findings this project has adjudicated (caddy, ADR-0046's own example), and `ets:foldl` corrected from CALLBACK to **transfer** — it returns the accumulator |
