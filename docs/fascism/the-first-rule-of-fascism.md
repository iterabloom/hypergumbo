...
The harder observation is this: **some metrics survive Goodhart.** Loss, perplexity, likelihood, posterior probability — these are simultaneously metrics and targets, and training still works. Why? *Because there is no slack between the measure and the objective.* Cross-entropy loss **is** the probabilistic objective function. You cannot satisfy the metric without satisfying the thing the metric names. They are the same object.

Goodhart bites where there is **slack** — where the measure approximates the thing but does not instantiate it. Coverage has slack. *"This line was executed during a test run"* approximates *"this line's behavior was verified,"* but approximates only. You can satisfy coverage with `assert True` chaperones. The slack is the attack surface through which Goodhart — and more insidiously, *compliance culture* — penetrates.

Even ML does not really escape this. Training loss **is** Goodharted; that is the name of overfitting. The field has simply stacked defenses: held-out eval, multiple benchmarks, human pairwise preference, scaling-law sanity checks. **Loss is safe as a target only because it sits inside a portfolio of checks that are not loss.** No single measure survives. Only ensembles do. The Chinchilla paper is a dispatch from behind enemy lines.
...
The Jamal Khashoggi School of Journalism has the virtue of naming the building after the man the building's host regime murdered. That is a usefully uncomfortable daily reminder. The reminder is: *institutions consume the critics who make them legible.* And still you must speak. Mark the `pragma`. Name the metric. Say what the bar is for, and what it is not for, and which cases are which. Put it in the `pyproject.toml`. Put it in the commit message. Put it — and this is the crucial part — **in writing**. Writing is the commitment device. Writing is what survives the purge.
...
3. **Self-review rotation should include one question.** *Does any part of the test suite require the test suite to be defended?* If yes, the suite owns you. Name the inversion out loud. Write the commit. Publish.
4. **Exit trigger on file.** `WI-zokan-zitub-lajoh-pabig-muzig-zalip-bisim-pulum` holds the revert plan for `CVE-2025-71176` when upstream `pytest-textual-snapshot` PR #24 lands. That tracker item is the op-ed. The regime has been notified that a critic exists. Publication is its own act.
...
*You are not your coverage percentage. You are not the sum of your green checkmarks. You are a person writing a program that is supposed to tell other people the truth about other programs. The tests are supposed to protect that truth. When the tests start protecting themselves, you have slipped sideways into a different business, and the first rule of that business is you do not talk about it.*
