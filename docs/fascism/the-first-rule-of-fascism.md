...
The Adornian move is to say: the metric has become the fetish. Coverage is the commodity; the code is the use-value it pretends to measure; we are all complicit in the exchange. True enough.
...
So: lowercase fascism. The costume is small. The pattern underneath, unfortunately, is not. If you reject the lineage, reject this essay — but reject it by reading Reich and Foucault, not by counting bodies.
...
*You are not your coverage percentage. You are not the sum of your green checkmarks. You are a person writing a program that is supposed to tell other people the truth about other programs. The tests are supposed to protect that truth. But the automated tests you own might end up owning you — and when the tests start protecting themselves instead of the truth, you have slipped sideways into a different business.*
