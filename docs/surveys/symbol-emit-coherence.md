...
| `unknown` | FOLD → `protocol_origin="annotation"` | RESOLVED (2026-06-08) | Linker-side sentinel for "this synthetic stand-in has no source language" (`annotation_convention.py`, 4 sites). ADR-0031 (Draft, 2026-05-30) prescribes the longer-term reshape: synthetic stand-ins for annotation directives migrate to `language=None` + `protocol_origin="annotation"` + `discovery_language=<host>`. The fold target here is the `protocol_origin` typed field that ADR-0031 introduces, not a language-axis canonical value — `"unknown"` doesn't fold to another language; it folds off the language axis entirely. Status RESOLVED 2026-06-08: the ADR-0031 Phase 1 producer migration landed — `annotation_convention.py` emits `language=None` + `discovery_language=language_from_path(<host>)` + `protocol_origin="annotation"` at all 4 sites (verified on dev). |
| `wasm` | FOLD → `protocol_origin="wasm"` | RESOLVED (2026-06-08) | Linker-side sentinel for compiled WASM module identifiers (`wasm_bindgen.py:399`, 1 site). The compiled `.wasm` binary doesn't have a source language in the user's codebase — could have been compiled from Rust, C, AssemblyScript, anything. ADR-0031 (Draft) migrates this to `language=None` + `protocol_origin="wasm"` + `discovery_language=None` (the wasm module isn't anchored to a particular host file). Status RESOLVED 2026-06-08: `wasm_bindgen.py` emits `language=None` + `protocol_origin="wasm"` (compiled module: `discovery_language=None`; TS-side import: `discovery_language="typescript"`) (verified on dev). |
| `openapi` | FOLD → `protocol_origin="openapi"` | RESOLVED (2026-06-08) | Linker-side sentinel for synthetic Symbols derived from OpenAPI YAML/JSON spec files (`openapi.py:327`, 1 site). OpenAPI isn't a real source language in hypergumbo's analyzer set; the spec files are parsed as YAML or JSON by those analyzers, and `openapi.py` is a linker that derives route/operation synthetics from them. ADR-0031 migrates to `language=None` + `protocol_origin="openapi"` + `discovery_language=<spec file language>`. Status RESOLVED 2026-06-08: `openapi.py` emits `language=None` + `discovery_language=language_from_path(<spec>)` + `protocol_origin="openapi"` (verified on dev). |
