...
- Date: 2026-05-05
- Status: Mixed — values with active producers ship UNRESOLVED (verdict recorded; per-framework Phase 3 sub-PRs follow); values that exist only as dynamic-emit results of an f-string at `linkers/websocket.py` ship UNRESOLVED but with a shared diagnostic test that targets the f-string source rather than a literal grep.
