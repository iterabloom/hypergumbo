...
3. **Recurrence-promotion threshold for `meta["framework_dispatch"]`** (ADR-0028 Open Question 2). After fold, `meta["framework_dispatch"]` will carry ~30 distinct values across ~30 producer modules — which **exceeds** ADR-0024 §"Fold-residue discipline" rule 3's actual threshold (**≥3 distinct values OR ≥2 producer modules**; the "≥30/≥30" a prior revision cited was a misstatement — see the [§7 Correction](#7-recurrence-promotion-threshold-open-question-2)). **This audit does not promote.** Phase 3 follow-on work tracks emission counts; if the threshold trips post-migration, a follow-on ADR promotes `framework_dispatch` to a dedicated `Edge.framework: str | None` field.
...
Per ADR-0028 §"Open Question 2" and ADR-0024 §"Fold-residue discipline" rule 3, after Cluster 28C fold the `meta["framework_dispatch"]` key will carry ~30 distinct values across ~30 producer modules. **This far exceeds** the recurrence-promotion threshold, whose actual value is **≥3 distinct values OR ≥2 producer modules** (ADR-0024 rule 3). If post-migration steady-state monitoring confirms the threshold has tripped, a follow-on ADR promotes `framework_dispatch` from a `meta` key to a dedicated `Edge.framework: str | None` field. This audit does not promote — the open question stays open until Phase 3 ships and post-migration emission counts stabilize. Tracking lives at the parent ADR-0028 issue (see ADR-0028 §"Open questions" #2). The parallel `meta["detection_pattern"]` key (~9 distinct values) is also above the ≥3/≥2 threshold but is likewise not promoted.

> **Correction (2026-07-19, INV-junid).** An earlier revision of this section (and §"three questions" #3) misstated ADR-0024 rule 3's threshold as "≥30 distinct values across ≥30 producers" — a paraphrase of the *observed* emission magnitude, not the rule. The actual rule (ADR-0024 §"Fold-residue discipline" rule 3, mirrored in `axis_meta_keys.py`) is **≥3 distinct values OR ≥2 producer modules**, which `framework_dispatch` exceeds by ~15×. Rule 3 governs promoting the whole *key → a typed `Edge.framework` field*, and is an **ordered gate** behind rule 1 (endpoint-property) and rule 2 (axis-leak); `framework_dispatch` trips neither, so ADR-0028 OQ2 was **resolved DECLINE** (2026-07-05, INV-junid) — the key stays a registered `MetaKeySpec`. Separately, this doc's "56 rows fold to `framework_dispatch=<framework_name>`" wording (§"three questions" #2, §"Diagnostic finding 1") is imprecise: `framework_dispatch` legitimately also holds **framework-agnostic dispatch conventions** (`registry_dispatch`, `middleware_chain`, `orm_accessor`, `npm_package`, `router_routes`, `controller_routes`, `route_mount`, `job_enqueue`). The true partition discriminator vs `detection_pattern` is *dispatch convention* (how a framework/system dispatches) vs *pattern-shape detection heuristic* (how an edge was string/name/URL-matched) — not framework-named-vs-not. The compound `<framework>_<convention>` values (`django_orm`, `tauri_invoke`, …) are the deliberate, audit-blessed fold residues, **not** an axis leak to split.
