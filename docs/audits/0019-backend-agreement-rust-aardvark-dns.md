...
|---|---|---|---|---|---|
| `docstring` | 0 | 0 | 63 | 0 | — |
| `is_exported` | 107 | 41 | 0 | 0 | `true` ← `false` x41 |
...
- **`is_exported`: 107 agree, 41 disagree, every one `true` ← `false`.** Both values are concrete and incumbent-first carried the incumbent's: `rust.py` computes exportedness (`"pub" in modifiers`) and these 41 are the `pub` items. The alternative is not an observation — the SCIP arm assigns the field nowhere and `Symbol.is_exported` defaults to `False`, so all 169 of its records contribute a phantom `False` candidate (INV-huboz, re-scoped 2026-09-19). After that lands this row should read 0 agree, 0 disagree, 148 one-sided to `rust` — an attribute only the syntax arm observes. **Correction:** an earlier revision of this bullet read the carried value as `null` and blamed the merge pass for carrying an absent value over a concrete one. The merge pass was right (`_candidates` already skips empties); the `null` was this instrument reading a nested attribute from the top level, fixed as WI-pofih, and the counts in the table never moved.
