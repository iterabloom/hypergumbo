...
* **DDG-backed** — **SPECIFIED, NOT IMPLEMENTED** (measured 2026-08-02). The intent is variable-level taint tracking within functions via intraprocedural reaching definitions, eliminating false positives where two variables in the same function take different paths. Today `propagate_taint_ddg` decides which flows are reported by call-graph BFS and uses DDG data only to choose a label, so no false positive of that kind is eliminated for any language. Def/use extractors exist for Python, Rust, TypeScript and Go; only Python and Go are reachable from a production call site. Findings labeled `confidence: precise` when both the source and sink functions have DDG coverage; `approximate` otherwise (see ADR-0017 §3c–3d for mixed-coverage rules).
