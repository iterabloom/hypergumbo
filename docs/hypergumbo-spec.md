...
🟩 **`hypergumbo backend-agreement ARTIFACT [--format md|json] [--out FILE]`** (ADR-0057 §5)
Measures how two backends agree on one language, from a survey artifact's own provenance slot (`attribution` / `alternatives` on merged records, `origin_run_id` → `analysis_runs[].pass` on single-producer ones): pairing by kind, per-attribute agree / disagree / one-sided counts with the disagreement shapes, and edge overlap per edge type. The markdown form is the committed `docs/audits/` sibling document (`kind: backend_agreement`) that is the only evidence allowed to change an arbitration default or license an `authoritative_for` declaration; `--format json` for a machine reader. A language with one producer measures nothing and says so. Exit 2 when the artifact cannot be read.
...
Identifies call edges that reach I/O primitives (filesystem, network, subprocess, environment) and groups them by boundary type. Loads a cached survey or auto-runs analysis if needed. Supports 17 languages (15 dedicated catalogs + 2 via aliases). The shipped **catalogs** are **stdlib-only** — [ADR-0016](adr/0016-io-boundary-analysis.md) §27, enforced for every shipped catalogue by the scope gate in `test_catalogue_scope_gates.py` (ADR-0047 ruling 8; before WI-surun the rule was gated for only 6 of 14 languages, and the eight ungated ones held 259 third-party rows — while python, gated by a *denylist* rather than an allowlist, held 33 more that its own gate could not catch, which is why the gate shape changed too). Third-party rows are still **shipped**, but not as catalogue rows: [ADR-0047](adr/0047-catalogue-scope-and-user-visible-homes.md) ruling 1 amends the older "third-party is user-supplied" position to *shipped, unvouched, and disclosed* — community overlays in `io_primitives_overlays/` load by default, are stamped `unvouched` so they cannot license an all-clear, and every run that loads one says so on stderr. `--no-default-overlays` turns them off; `--io-primitives` (INV-fotav) layers a user's own above them. **A language that ships no catalogue (ruby, php, lua, ...) takes a user overlay as its whole catalogue** (WI-guhuv): its rows classify calls and derive taint sources and sinks, but they cite no stdlib and date no module the user did not date, so a call they do not name stays unclassified and still withholds a clean verdict, and the run says on stderr that the language is overlay-only. An overlay naming a language no analyzer produces is refused as a misspelling. When entrypoints are available, traces backward from IO edges to show which entrypoints can reach each IO operation. Output format is `--format text|json` (default `text`), the canonical read-view spelling shared with `routes` / `catalog` / `config` / `cache-status` / `dead-code-maybe` / `test-coverage`; the historical `--json` boolean is kept as a back-compat alias (WI-kitud).

🟩 **`hypergumbo verify-claims --claims <file> [--format text|json]`** (ADR-0016, ADR-0017)
Verifies security claims against the IO boundary map and taint-flow analysis. Supports boundary constraints (e.g., "no network I/O", "max 3 filesystem chains") and taint-flow constraints (e.g., "plaintext data must not reach host_fs zone"). Claims are specified in YAML format (`hypergumbo verify-claims --help` documents the shape). The claims file is validated up front: a malformed YAML, an unexpected root/claim/constraint shape, an unknown field name, or a `constraint.boundary` outside the io-boundaries vocabulary produces a clear error rather than a traceback or a silent false "confirmed". A `must_not_exist` / `max_chains` boundary claim is only `confirmed` when the I/O analysis could actually have seen the I/O: if the analysis produced no call edges at all, if a supported language was analyzed but produced none (so its I/O is invisible), or if the repo calls into a catalogued module that has instance-method rows and no call edge into it carries a construct the module declares or names one of its function rows (so the analyzer never emitted the calls the catalogue could match; a row's kind does not decide a match when the module slot names the module, ADR-0059), the verdict is `inconclusive` rather than a false clean (WI-kajil / INV-bitig). Exit codes: `0` = all confirmed; `1` = at least one violated; `2` = at least one inconclusive, or the claims file failed validation; `3` = at least one `confirmed_with_caveats` and none of the above (INV-pojib) — every claim held, but at least one held on something the reader must see. Two things raise it. (a) An entry the *analysed repository* supplied about itself — a sanitizer declared through `extra_catalogs:` or `--taint-sanitizers`, credited with removing a flow the tool would otherwise have reported, or a repo-supplied row that displaced a shipped one; hypergumbo cannot check such an assertion. (b) Named call sites the analysis saw but could not adjudicate — an opaque launch whose launched program it cannot see inside, or (INV-fibis) a call reaching a method the catalogue declares **for the boundary under claim** through a receiver whose type could not be determined (`sock.sendall(payload)` where `sock` is an unannotated parameter). The latter is boundary-scoped, so a catalogued name for some *other* boundary cannot qualify this claim, and it does not make coverage incomplete — the claim still holds everywhere the analysis could see. **The same disclosure attaches to a clean TAINT verdict, scoped by the claim's prohibited SINK ZONE (INV-nuhun)**: a method the *sink* catalogue declares for that zone, reached through a receiver whose type could not be determined, means a flow to that sink could neither be constructed nor ruled out. Until this landed the two arms contradicted each other about one call inside a single invocation — `sock.sendall(secret.encode())` on an unannotated parameter was named on the `net_send` boundary verdict and passed over in silence by `host-secret-no-network`, which certified that no environment value reaches the network zone, and the silent verdict carried a tick where the disclosing one carried a warning. The flow cannot be recovered by relaxing the sink matcher: `socket.socket.sendall` is a method-kind catalogue row, and matching one on a bare short name with no receiver evidence is exactly the unfiltered short-name fallback INV-maluk forbids (INV-tapat before it, and INV-maluk was reopened precisely because taint's sink matcher was the third consumer that ignored it). Typing the receiver is the sound fix, and it is proceeding: an own instance field whose external type `__init__` establishes — `self.sock = sock` where the parameter is annotated, `self.sock = socket.socket()`, or `self.sock: socket.socket` — now carries its module to the call site, so `self.sock.sendall(payload)` is **seen rather than disclosed** (`class_field_types` is `Symbol`-valued and by construction holds only in-repo classes; `class_external_field_types` is its catalogued-module counterpart, mirroring the `var_types`/`external_var_types` pair locals have had since WI-fuvuj). Where the receiver still cannot be typed, the verdict says so. (c) **The verdict's SCOPE** (`unknown_receiver_scope`, INV-fibis): a clean boundary verdict is closed-world over the receivers the analysis could type, and says so with a count, a denominator and the distinct method names — *"At 4 of 11 method call site(s) the receiver's type could not be determined … distinct method(s): get, is_file, items, upload_file."* This one is NOT boundary-scoped, deliberately: matching the called method's name against the catalogue is exactly what an untyped receiver makes meaningless. Measured over 32,593 non-test files, 90 real scopes reach a genuine I/O verb outside the 120 catalogued method-kind names (`post`, `upload_file`, `put_item`, `execute_command`, …) through an untyped receiver, and (b) fired on none of them; all 90 were held back instead by a coverage gap the project intends to close, so that protection shrinks as the tool improves. It **qualifies** rather than withholds — the unscoped *downgrade* to `inconclusive` was measured on 2026-08-11 and refused — and it is capable of not firing: an analysis whose receivers are all typed still returns a bare `confirmed` at exit 0. It attaches to clean taint verdicts on the same terms (INV-nuhun), reusing the boundary arm's already-computed count rather than recounting, so one invocation cannot report two different denominators for one repository. In every case the verdict names the entries or sites and the JSON envelope carries them under each verdict's `caveats`. A gate written `verify-claims … || exit 1` therefore fails on exit 3 where it previously passed; that is deliberate and fail-closed. Output format is `--format text|json` (default `text`), the canonical read-view spelling; the historical `--json` boolean is kept as a back-compat alias (WI-kitud). **The caveat KINDS are a closed vocabulary and each one is named in the envelope's `caveats[].kind`**: `user_supplied_sanitizer`, `displaced_shipped_entry`, `opaque_boundary`, `untyped_receiver`, `unknown_receiver_scope`, `analyzer_method_call_blind`, `analyzer_suppressed_methods`, `higher_fidelity_available`. Seven of the eight had no user-facing definition anywhere until WI-bovor; they are now defined for a reader, together with the exit-code contract and the limits below, in [docs/VERIFY-CLAIMS-SCOPE.md](VERIFY-CLAIMS-SCOPE.md). **One further limit is structural rather than a wiring gap and belongs beside them (WI-tizig): the §3a walk is INTRAPROCEDURAL, so a flow whose source is read at MODULE TOP LEVEL — anchored on the `file` node rather than a function — can never be adjudicated by data dependence and comes out `analysis_method: structural` no matter how capable the language's extractor is.** Measured on measurement `0005`'s five-repository census: 10 of 37 situations (27.0%) and 17 of 170 rows (10.0%) are file-anchored, and every one of them is labelled `structural`; the concentration is very uneven (express 3 of 3, caddy 0 of 16) because module-level configuration reads are idiomatic in JavaScript and Python and rare in Go. `dataflow_coverage` publishes capability per LANGUAGE; this limit is per-FLOW and applies to every language equally. Useful for CI enforcement of IO and data-flow security policies.
...
* **DDG-backed** — **IMPLEMENTED, AND ADJUDICATING SINCE 2026-09-02.** Variable-level taint tracking within a function via intraprocedural reaching definitions (ADR-0017 §3a). The walk runs where the source function has def/use coverage and the sink is called in that same function; it **raises a finding's confidence, and REMOVES a finding it refutes** (WI-kabif). The asymmetry is the whole design: it can subtract, never add, so which flows are *reported* is still bounded by call-graph BFS. Removal fires only on `unconfirmed` — every route out of a recorded definition ended somewhere accounted for. A walk that LOST the value (`escaped`) removes nothing, and WI-joluk's forfeit gate downgrades any `False` from a function whose CFG missed a call node in its body. **The measured effect at the grant was ZERO on every corpus checked** (hypergumbo-core: 15 confirmed / 12 escaped / 0 unconfirmed across 27 walks; the 11-repo cohort reports the same zero), so these are live semantics over an empty class. **Closing those escape sites is a RETIRED GOAL as of 2026-09-09 (ADR-0052)** — most of them are not calls at all, so no function summary can reach them, and measurement 0007 found ZERO of 153 `ddg_mixed` rows across 11 repositories resting on a walk that ran and established no dependence. The capability stays; the coverage it would need is not being pursued, and the analysis is confirm-only in practice. Both facts are emitted rather than left to the reader: every `verify-claims` run carries `dataflow_coverage.inclusion_decided_by` (now `call_graph_reachability_minus_ddg_refutation`) and `dataflow_coverage.flows_removed_by_walk`, so a removal is never something a reader has to infer. Def/use extractors are registered and production-reachable for Python, Go, Rust, TypeScript and JavaScript. `confidence: precise` / `analysis_method: ddg` mean the walk **actually confirmed a data dependence** — not merely that both endpoints had DDG coverage, which is the unearned reading INV-sadah was filed for; `ddg_mixed` means the walk ran without confirming, and `structural` means no reaching-def data existed for the flow's source function so no walk was possible (see ADR-0017 §3c–3e).
...
Recognised settings: `io_primitives` (list of I/O primitive overlay paths; relative entries resolve against **their own config file**, not the working directory), and the `[merge]` table — the ADR-0057 §5 arbitration default (WI-hukuf), a preference allowed in both tiers because it executes nothing:

```toml
[merge]
prefer = ["scip", "tree-sitter"]      # backend precedence for a contested attribute; unlisted backends follow, incumbents first
corroborated_confidence = 0.95        # ADR-0057 §13: two producers, distinct pathways, one edge
superseded_stub_rank_factor = 0.5     # ADR-0057 §14: a resolved edge demotes a same-site external stub's rank_score
[merge.prefer_by_attribute]
kind = ["scip"]                       # REPLACES `prefer` for this attribute only
```

A `[backends]` table enables an opt-in backend that **executes nothing** from the analysed repository — `scip_python = true` (WI-nanom; pyright via scip-python) — in either tier, project replacing user; a backend that executes analysed code (`rust_analyzer`) is refused from config and lives in the trust store (`hypergumbo trust-backend`). The allow-list and the deny-list are both derived from the registry's `executes_analysed_code` declaration. Resolution per backend is ADR-0045 ruling 4's chain: `--backend` flag > environment (`HYPERGUMBO_SCIP_PYTHON`, `HYPERGUMBO_RUST_ANALYZER`) > project config > user config (or the trust grant, for the executing backend) > built-in off; `--backend tree-sitter` turns every opt-in backend off for one run.

Orders name **backends** (`tree-sitter`, `scip`, `ast`, …), so one line means the same thing for every language the backend serves; an entry naming no declared backend, an attribute the merge pass does not arbitrate, a level outside `[0, 1]` or an unknown key under `[merge]` is refused naming the file and key (exit 2, before any analysis runs). The project tier replaces the user tier key by key. With no `[merge]` table the built-in applies: incumbent first (the registry's `incumbent_first`), levels 0.95 and 0.5, and **no per-attribute exception** — the first backend-agreement table (`docs/audits/0019`) warranted none. A built-in exception may only ever be added by citing a committed `docs/audits/` document of `kind: backend_agreement` (`hypergumbo_core.arbitration.BUILTIN_ATTRIBUTE_EXCEPTIONS`, validated on the live tree).
...
**Three channels are scanned today** — `io_primitives.d/`, `frameworks.d/` and `dataflow_patterns.d/` (ADR-0047 ruling 10, WI-sofov). A user framework definition **replaces** the shipped one for that id rather than merging into it: a `FrameworkPatternDef` is a coherent whole whose detectors and extraction rules are written to agree, so half-merging two yields a definition neither author wrote — unlike I/O rows, which are keyed by qualified name and merge per row. `dataflow_patterns.d/` is **section-scoped to `library_patterns`**: those rows are regexes over call syntax that a user with an in-house collection type can legitimately extend, while the grammar rows stay internal for the `cfg_nodes` reason — a wrong node type silently empties the CFG and therefore the taint walk. Grammar keys in a user file are ignored rather than refused, so such a file does not break a run. `function_summaries.d/` is scanned too, and is the one channel whose entries can **remove** a finding: a terminating summary closes a branch the walk would otherwise follow. A user-supplied terminating summary therefore qualifies a clean taint verdict with `CAVEAT_USER_SUPPLIED_SANITIZER` (exit 3), the same kind the structurally identical user-declared-sanitizer case uses — a propagating one needs no caveat, because it cannot silence anything. The caveat is **run-scoped**, and says so: a terminated branch produces no finding, so unlike a sanitized flow there is nothing to attribute to one claim. `taint_sources.d/` and `taint_sanitizers.d/` remain declared-but-unread; the README written into the home lists which are which, derived from the same constant the loaders use.
...
**One language vocabulary (WI-juzig).** An analyzer's `languages` are the taxonomy's names (`taxonomy.LANGUAGES` keys; `LANGUAGE_ALIASES` keys are canonicalised), and the registry gates them at decoration time — there is no silent `[name]` default for a name that is not a language. A registration is in exactly one declared `language_state`: `taxonomy` (the default), `no_taxonomy_spec` (a real language the taxonomy has no `LanguageSpec` for; none of its languages may be a taxonomy key, so the declaration raises as stale once a spec lands — the coverage gap is WI-futin), or `no_language` (not a language pass — `manifest_targets`; `languages` is `[]` and no consumer re-inflates it into `{name}`). An undeclared non-language name raises `LanguageDeclarationError` at import, and entry-point discovery re-raises it rather than logging it away. This is what lets `limits.skipped_languages` be an honest signal (a detected language whose pass ran is never reported skipped — INV-nidul) and what lets the registry see that `rust` and `rust_analyzer` are two backends (`tree-sitter` / `scip`) for one language rather than two languages (the `languages` half of the ADR-0057 §10 producer contract).

**The producer contract (ADR-0057 §10, WI-hohuh).** A backend that shares a language with another producer declares on its registration how their records relate: `merge=MergeAnchor(name_key=..., span_role=..., authoritative_for=...)` when the two emit records for the same declarations — `name_key` maps `Symbol.name` to the backend-neutral declaration name (tree-sitter `rust`: the last segment under the taxonomy's declared `::` separator, since it qualifies a method by its impl target; `rust_analyzer`: the SCIP descriptor name as emitted), `span_role` says whether `Symbol.span` is the identifier `token` (rust-analyzer's Definition range) or the `item` (every tree-sitter analyzer), and `authoritative_for` names the attributes the backend is authoritative on by measurement, each citing a committed `docs/audits/` table (empty on both Rust arms: the built-in arbitration default is incumbent-first) — or `merge=MergeDisjoint(partners=(...))` when it shares the language but never a record (`javascript` reads a component file's `<script>` block; `svelte` and `vue` read its template; both sides declare it and a test runs both on a component and pins the disjointness). `merge_participants(language)` is what the merge pass reads: the anchored producers it pairs among, or `UndeclaredProducerError` naming the analyzer when a language has two producers and one has not declared — the pass refuses rather than pairing nothing. ADR-0045 §5's `executes_analysed_code` lands on the same registration (`rust_analyzer` declares it: indexing runs `build.rs`), and both the trust store's accept-list and the config tiers' deny-list are derived from it rather than from a hardcoded set, so the next backend cannot land in the wrong store by omission.

**The backend-agreement instrument (ADR-0057 §5, WI-dajif).** `hypergumbo backend-agreement ARTIFACT` is the measurement the §5 default defers to and the table an `authoritative_for` entry must cite. It re-pairs nothing: it reads the merge pass's own `attribution` / `alternatives` and the folded edges, so the tool and the pass cannot disagree about what "the same declaration" means, and a producer is whatever the registry declares for the language (a linker's or the merge pass's own records are not a producer's). The first committed table, `docs/audits/0019-backend-agreement-rust-aardvark-dns.md`, is produced from the recorded fixture (`scripts/regenerate-backend-agreement-table`; the fixture test pins that the committed numbers are what the instrument says). On it: 148 of 169 records paired, `kind` agrees on 133 and disagrees on 15 (`variable` carried over SCIP's `constant`, the WI-bamar producer gap), `name` on 65 (tree-sitter's `Type::member` over SCIP's bare member), `is_exported` one-sided to the syntax arm on all 148 (only it computes exportedness; the SCIP arm's records carry no value at all rather than a defaulted `False` — INV-huboz, INV-kubup), `span` on 146 by the §10 role split; 93 call edges corroborated, 269 `references` edges the syntax arm never emits, 244 external-stub calls the SCIP arm never emits. Agreement is not correctness: the table counts, the owner reads.

**The arbitration policy (ADR-0057 §5, WI-hukuf).** `hypergumbo_core.arbitration.ArbitrationPolicy` is the one place precedence among a language's producers is computed, and it carries the two declared levels beside it (`CORROBORATED_CONFIDENCE`, `SUPERSEDED_STUB_RANK_FACTOR` — the merge pass and finalize import them from there). `run_survey` resolves it once from the config tiers (`resolve_arbitration_policy`) and threads it into `merge_producer_records` and `FinalizeContext.arbitration_policy`; with no `[merge]` table it is `BUILTIN_POLICY`, byte-for-byte the previous behaviour. Which arm is the *alternative* stays a registry fact (`ALTERNATIVE_BACKENDS`, read by `incumbents_of` and the recorded-producer-input lint); which arm *wins* is the policy's.

**Recorded producer input (ADR-0057 §12, WI-romuh).** A backend CI cannot run — rust-analyzer executes the analysed crate's `build.rs` — is tested on *recorded* output of the real producer, never on input synthesised from the arm CI can run: the parity test that fed `rust.py`'s own spans into the SCIP helper passed for months over a feature measuring 0 of 52 in production (INV-dolud). The recordings live with the producer's package under `packages/hypergumbo-lang-rust-analyzer/recorded/` (on pytest's `pythonpath`, not in the wheel): the module `recorded_rust_analyzer_1_94_0.py` pins the producer version and exposes a 33-line sample crate with every Definition listed inline, and the aardvark-dns crate (upstream `4444d90fee`, Apache-2.0, source committed beside its raw `index.scip`) with its counts — 16 documents, 169 global definitions, 500 locals, 3,543 references — and the pairing the declared anchors give when the tree-sitter arm runs live on the committed source: **148 of 169** definitions paired one-to-one, none ambiguous, the 21 leftovers being 18 module namespaces, two `type` aliases and one `static` the tree-sitter arm emits no Symbol for. That number, measured on committed input, is what the merge pass must reproduce; the 138 the ADR cites came from a scratch script. `scripts/check-recorded-producer-input` (CI gate: `test_check_recorded_producer_input.py::TestLiveTree`) enforces the rule structurally: an AST taint walk per test function from a call to an incumbent producer — the non-index backend of every language with two anchored producers, read from the registry — through assignment, unpacking, `for` and comprehension targets, to any call of a cross-backend consumer (the core SCIP shim, `translate_scip_to_hg`, the `rust_scip` parity helpers, and the merge pass when it lands). A test that feeds on purpose says so with `@pytest.mark.incumbent_fed("<reason>")`; a bare marker is refused, because the reason is the review.
...
class Symbol:
    ...
    is_exported: Optional[bool]
                               # part of the package's public API; None when no
                               # producer computed exportedness for this record
                               # (INV-kubup) — seventeen analyzer modules have a
                               # rule and ninety-odd do not
...
Per ADR-0024 the multi-value type fields on the core dataclasses each declare an axis (axiom + consumer pattern + enforcement) so analyzers, linkers, and downstream consumers share a canonical vocabulary instead of free-form strings. **Thirteen axes are wired into the declaration gate's `_known_axes()` resolver**; the three described in detail below are the heavyweight registry-backed ones (ADR-0023 / ADR-0027 / ADR-0028), which is what this section covered when it said "three":
...
**The other ten.** `io-boundary` (ADR-0050) is registry-backed like the three above. `module-key` (ADR-0051), `qualified-name` (ADR-0032) and `callable-signature` (ADR-0058) are **structural-policy** axes: their fields' values cannot be enumerated — every package on every index, every callable in every grammar — so the registry declares the *notions* a slot may carry and the resolver returns those, while `_check_field` tests the declared axis NAME for membership and never the field's values. `language`, `pass-id`, `entrypoint-kind`, `visibility`, `protocol-origin` and `pass-silence-reason` are the lightweight catalog-derived ones under ADR-0024 §4's "use judgment" carveout. All are rendered in [`docs/concept-axes.md`](concept-axes.md) except `qualified-name` (ADR-0051 open question 3) and the catalog-derived ones.

**`Symbol.signature` — the callable-signature axis, ADR-0058** (Accepted). Axiom: "preserves the declaration surface of a callable, verbatim, in the source language's own grammar, for display; every fact inside it — return type, parameter arity, a field's declared type — is read from its declared home, never parsed back out of this string." Two notions: `callable_surface` (36,662 of 38,573 populated slots) and `value_type` (1,911; a field's or variable's declared type on a symbol that is not callable). Enforcement is unusual and deliberate: with no enumerable value set there is nothing for a value-drift linter to check, so the gate closes the **consumer** side instead — `signature_axis.find_undeclared_value_parsers` walks `packages/*/src` for reads whose value flows into a call and fails on any site absent from `LEGACY_VALUE_PARSERS`, the nine grandfathered parsers. The facts those nine go after each have a declared home (`FileAnalysis.method_return_types`, `meta["parameters"]`, `FileAnalysis.class_field_types`); migrating the producers to them is ADR-0024 step 7 and is not done.
...
**silence_reason** (string, optional; per-run — INV-bikaj, arc T6): WHY this pass produced no Symbols and no Edges, drawn from the closed `pass-silence-reason` axis (`hypergumbo_core.pass_silence`), never free text. The value space is `no_candidate_files`, `no_candidate_construct`, `candidates_unresolved`, `dependency_unavailable`, `backend_disabled`, `prerequisite_absent`, `pass_crashed`, `unreported`. `no_candidate_construct` and `candidates_unresolved` are opposite answers to "is this pass useful here?" — the construct is absent, versus present but related to nothing — and only a pass body reports either.
...
**What the orchestrator will and will not claim.** It stamps only what it derives with certainty from counters it already holds: zero files ⇒ `no_candidate_files`, otherwise `unreported`. It never infers `no_candidate_construct` — only a pass body knows whether it looked for a construct and failed to find one, and a reason invented on the producer's behalf would be a fabricated disclosure. **`prerequisite_absent` now has a producer (WI-dabup / ADR-0056 W3), and it is not the obvious one.** It is NOT stamped merely because a `depends_on` conjunct is unsatisfied: 98.94% of skip records are `no files matched`, so an unsatisfied conjunct nearly always means the repository lacks that language — a correct no-op, and stamping it as an ordering defect would make the field assert a defect about a repo that simply has no Rust in it. It is stamped when **every** literal of some conjunct did not run **and at least one of them was skipped with a `silence_reason` other than `no_candidate_files`** — a toolchain reason rather than a file reason. A prerequisite that ran and honestly found nothing still satisfies its dependents. The stamp fills only a field the pass body left empty: a body that spoke keeps its word, so a bridge linker that truthfully reports `no_candidate_construct` about the side it *could* scan is not overruled, and `prerequisite_absent` therefore under-reports rather than overwriting a true claim. Structurally it can only see ANALYZER prerequisites, since only analyzer passes are enumerated in `limits.skipped_passes`. **A non-zero `unreported` count is expected, and a ZERO does not mean the work is done** — most passes never claim a reason at all, so a drained count measures the corpus, not completion.
...
**derived_from (INV-rukor):** `derived_from: list[str] | null` records which Symbol (or Edge) IDs the producer consumed to construct this Edge — the input records whose presence decided it (the unresolved edge a linker resolved, the `extends`/`implements` edges a dispatch walked, the class whose methods it enumerated). Ids the producer minted in the same run are not consumed and are omitted, so a linker that builds both ends from a file scan records `[]`, a positive "consumed no graph record" distinct from `null`. Populated by linkers (always non-null); null for analyzer-originated edges whose derivation is the AST itself. A value naming only the edge's own endpoints is legal where they are the whole derivation (a containment edge is decided by parent and child alone); every linker site with such a value, or with `[]`, carries a source declaration — `# derived-from endpoints|consumed-none|incomplete: <reason>` — enforced by `test_edge_derived_from.py`, and `incomplete` marks a known gap. `explain --provenance` renders four states: a chain, endpoints-only, consumed-nothing, and no statement. Enables answering "why does this edge exist?" without re-reading linker source code.

**src anchoring (INV-foluz, INV-nopoh):** a `calls` edge's `src` is the innermost enclosing function or method; a function literal's calls belong to the named function that contains it. A call OUTSIDE any function still emits an edge: Python module-level code anchors on the `<module>` pseudo-node, and a Go package-level `var` initializer -- the call itself (`var logger = log.New(...)`), a literal bound to the variable, or a struct-field literal such as every cobra `Run:` handler -- anchors on the package-level variable's own symbol (grouped `var ( ... )` blocks included), falling back to the file pseudo-symbol only for a blank `var _ = register()`. Before INV-nopoh those Go sites emitted nothing (578 of 1,331 `fmt.Fprintf` sites on beads). The same shape is fixture-proven still missing in rust, java, kotlin, swift, ruby, c, cpp, scala and lua (INV-bamij); js/ts (INV-jahom), php/bash/perl/powershell (WI-jadaf) already anchor it.
...
- `skipped_languages[]`: Languages the profile detected for which no analyzer pass completed a run (grammar unavailable, unsupported, or crashed). Computed in the registry's declared `languages` vocabulary, which is the taxonomy's (WI-juzig): a detected language whose pass ran under another registry name is never reported here (INV-nidul).
- `failed_files[]`: Files that caused parse errors, with path, reason, and analyzer ID
- `skipped_passes[]`: the single authoritative record of **pass-level** skips — an analyzer pass that produced no `analysis_runs[]` entry. Every registered analyzer resolves to exactly one of an AR **or** a skip record (WI-didil — completeness across the analyzer catalog); the reasons emitted are: missing optional dependencies (e.g., `{"pass": "lean", "reason": "tree-sitter-lean grammar not available"}`); `no files matched` — recorded `{"pass": "<lang>", "reason": "no files matched"}`. **Two of the three routes to "this pass had no input" land here; the third does not, and the discriminator is not the silence** (WI-gisor, measured on component-model-demo). (1) The WI-jadig file-presence pre-filter short-circuits an analyzer whose declared languages all have `files == 0` in `profile.languages`, saving the wall-clock cost of opening a parser for a pass with no input — 75 passes, and worth ~63% of analyzer wall-clock on a small repo. Its soundness depends on `profile.languages` covering everything the analyzer would read, which is why an analyzer whose candidate set exceeds its taxonomy extensions must declare `find_files` (INV-homur / INV-hokig). (2) An analyzer the pre-filter cannot judge — because a declared language is absent from the taxonomy (the `no_taxonomy_spec` state) or it declares no language at all (`no_language`), so no file count exists for it (WI-juzig) — is dispatched defensively and lands here only if it returns a bare `AnalysisResult()`, which is 7 passes. (3) The same defensively-dispatched analyzer lands in **`analysis_runs`** instead, carrying `silence_reason: no_candidate_files` with `files_analyzed == 0`, if it returns a run — which is what **26 of the 35** out-of-taxonomy analyzers do. So a consumer asking "which passes had no input?" **must read both hosts**: use `pass_silence.census_silence()`, which unions them over the pass catalogue rather than over these two lists (a pass in neither would otherwise be invisible — see WI-didag); an opt-in backend that stayed off, e.g. `{"pass": "rust_analyzer", "reason": "rust-analyzer backend not enabled"}` (distinguished from `no files matched` precisely because the repo may contain the language's files — the backend simply did not run); and a contained pass crash, recorded as `{"pass": "<lang>", "reason": "crashed: <ExcType>: <msg>"}` (§17). Every entry also carries a structured **`silence_reason`** on the closed [pass-silence-reason axis](#pass-silence-reason) (WI-dukoh / ADR-0056 W2), so a consumer can ask "was this skipped for a toolchain reason?" without matching twenty-eight prose spellings. **It is deliberately the same field name `AnalysisRun` carries** — same question, same axis, same closed vocabulary — because which host answers it is decided by whether the pass ran, never by a property of the silence (WI-mamiv; the field was called `skip_reason_code` in schema 0.20.9 and was renamed in 0.20.10). The values: `dependency_unavailable` (grammar or parser package absent), `backend_disabled` (an opt-in backend that stayed off), `pass_crashed` (a contained raise — including a parser constructor that failed, where "install the package" would be the wrong advice), and `no_candidate_files` (the file-presence pre-filter). The prose `reason` **stays** and is not redundant: it carries the pip command and the exception text, payload a code cannot express. One channel, two fields. **A missing or `unreported` code means the producer did not classify itself** — it does *not* mean `no_candidate_files`, and a consumer must not read it that way: bucketing an unclassified skip under the 98.94%-common value would manufacture the majority answer on the producer's behalf. Non-analyzer passes (linkers, synthesis) are **not** enumerated here — a linker with no applicable targets is a correct no-op, not a "pass that did not run"; only the analyzer catalog carries the AR-or-skip completeness contract. Because a skipped pass produces no `analysis_runs[]` entry, this top-level list — not the per-run `analysis_runs[].skipped_passes` — is where pass-level skips live; it is always present (empty list = "no pass was skipped"). (INV-nihug.)
...
* **Auto-derived from `io_primitives/*.yaml`.** Each IO primitive in a write-side category becomes a taint sink at `trust_level: untrusted` in a matching trust zone: `fs_write` → `host_fs`, `net_send` → `network`, `subprocess` → `subprocess` (WI-bibuk: split from `host_fs` so legitimate `subprocess.run` calls don't surface as host_fs violations), `env_write` → `host_env`, `ipc_send` → `ipc`, `browser_storage_write` → `browser_storage`, `db_write` → `database`, `process_send` → `ipc`, `logging` → `logging`. Each IO primitive in a read-side sensitive category becomes a taint source: `env_read` → label `host_secret`, `host_info_read` → label `host_description`, `net_recv` → label `untrusted_input`, `ipc_recv` → label `untrusted_input`, `db_read` → label `untrusted_input`. **`env_read` and `host_info_read` were one boundary until INV-tutar**, and the split is of the boundary rather than the label because the boundary vocabulary is the registry-backed thing: `env_read` is an ambient CONFIGURATION read (environment variables, system properties, argv — values that may carry a credential), `host_info_read` is a host DESCRIPTION or identity read (`runtime.GOOS`, `os.uname`, `navigator.platform`, `pwd.getpwnam`). 134 of the 195 shipped `env_read` rows were the second kind, so every `host-secret-*` claim counted description reads as secret flows. **Which rows may legitimately carry an auto-source boundary is ruled by [ADR-0049](adr/0049-deferred-crossings-are-disclosed-not-minted.md):** a boundary tag asserts the crossing happens at that call, in that call's scope, so a call that merely *opens, registers, subscribes to, schedules or defers* a crossing — a server launch, a route registration, a callback subscription — is a **deferred crossing**, disclosed rather than minted. The discriminator is whether the call returns, to its own caller, a value whose content the far side chose (`accept` does and stays `net_recv`; `ListenAndServe` returns `error` and does not). A deferred crossing is expressed as `net_listen` — or, for the database, `db_compose` (WI-fasap: Django's lazy QuerySet combinators, whose evaluation the python analyzer emits as its own `django.db.models.__iter__` / `__aiter__` / `__getitem__` `db_read` call site) — which mints no taint, is disclosed in its own `net_listen_edges` / `db_compose_edges` count outside the `total_io_edges` headline, and **shadows** `net_recv` / `db_read` so a clean verdict over a listener or a query builder is qualified rather than granted (ADR-0049 ruling 2; the shadow is what stops a disclosure tag from becoming a false-all-clear generator under INV-buzab). **The first rows moved with INV-kanuk: Go's eleven connection-SETUP rows** — `net.Listen{,TCP,UDP,Unix,Packet}` and `{syscall,unix}.{Socket,Bind,Listen}` — each of which returns a descriptor and transfers nothing. Measured on an idiomatic Go accept loop, that takes the run from 8 `net_recv` chains (every one a false taint source, and zero true receives) to 7 `net_listen` + 1 `net_recv`, with every crossing still represented. **The server-LAUNCH rows have now moved in four languages and deliberately not in two.** ADR-0049 §"Open work" step 3 cleared with [measurement 0010](measurements/0010-deferred-crossing-findings-per-shape.md), which adjudicated the family's live findings and found every launch-rooted one false — 2 on the current tree, 4 in measurement 0006 — so `net/http.{ListenAndServe,ListenAndServeTLS,Serve}`, gin's `Run`, echo's `Start`, fiber's `Listen`, gRPC's `Serve`, Python's `serve_forever`/`handle_request`/`asyncio.start_server`/`flask.Flask.run`/`uvicorn.run`/`aiohttp.web.run_app`, Haskell's `Warp.{run,runSettings,runTLS,runEnv}` and Erlang's `httpd.{start,start_service}` plus `ssl.handshake` are `net_listen`. **JavaScript is held back and Phoenix's route-registration rows are held back, for different reasons and asserted separately.** JavaScript's `net_recv` surface was `{http,https,net}.createServer` and `Deno.listen` plus `WebSocket.onmessage` and its peers, and the peers were unreachable, so moving the server rows would have relocated the receive to nothing, which ruling 3 forbids. **THAT PREMISE HAS EXPIRED, IN TWO STEPS, AND THE HOLD IS NO LONGER RESTING ON IT.** INV-misup (#753) typed a `new`-constructed receiver, which made `ws.addEventListener('message', cb)` reach `WebSocket.addEventListener`; WI-dosuh then emitted the registration edge for the property-assignment spelling, so `WebSocket.onmessage`, `WebSocket.onclose` and `EventSource.onmessage` are reachable too — the `CONSTRUCT_BLIND_ROWS` declaration that recorded their unreachability is removed. Ruling 3 therefore no longer blocks the JavaScript retag; whether to take it is an open question needing measurement 0010's adjudication repeated on JavaScript, and it is filed rather than decided here. Nothing in the JavaScript catalogue has moved on the strength of this paragraph. `Phoenix.Router` produced **no adjudicated findings at all**, and absence of findings is not evidence of harmlessness. The retag is a **correctness fix worth about two findings**, not a precision fix: an over-counting grep finds 49 files in that cohort containing a launch-shaped call against 2 findings, because the third-party framework rows are `WI-lalot`-inert and a `main` that launches a server usually has no sink carrying its value. **What measurement 0010 also found is that shape does not predict worth**: `bufio.NewReader(os.Stdin)` fails the same ruling-1 test as `ListenAndServe`, and it produces true and *useful* findings, because the handle is read in the scope that built it. Go's stdin surface was three deferred-crossing rows with no catalogued `ReadString`/`Scan`/`Text` to relocate a crossing to; **WI-vutav rowed the reads** -- `bufio.Reader.{ReadString,ReadBytes,ReadLine,ReadRune,ReadSlice,Read}` and `bufio.Scanner.{Scan,Text,Bytes}`, dual `fs_read`/`ipc_recv` under `call_site_undecidable`, narrowed by an `io_target_kind` the Go analyzer now stamps on the READ by resolving the receiver's last binding back to the wrapper call and classifying that call's argument (the identifier hop taken at the wrapper's line). The read rows fall back to `fs_read`, the opposite of the constructors' `ipc_recv`, because an unstamped read's receiver came from a scope the analyzer cannot see and the wrapper there already carries the unresolved case; measured on six Go repositories, 179 read sites reached the rows, 8 stamped stdin and all read back as true, 39 unstamped and every one a parameter, field, or parameter-fed wrapper. The handle-constructor family is still **not** retagged: that is a separate ruling, to be taken by re-running measurement 0010's arm 2 now that a represented crossing exists to relocate to. **WI-suhug then pointed the same seam at the WRITER.** `io.WriteString` and `fmt.Fprint*` take any `io.Writer` first, and `go.yaml` had each at one fixed boundary -- measurement 0012's entire vacuous class (gocryptfs writing a plaintext to a child's `StdinPipe`, reported as a host-filesystem crossing). They are now declared under `fs_write`, `ipc_send`, `net_send` and `logging` with `abstains_to` naming today's answer (`fs_write` / `logging`), the target-kind vocabulary gained `pipe` (a child's `Std*Pipe()`, `os.Pipe()`) and `net_stream` (a connection, a response writer), and a second map, `_WRITE_TARGET_KIND_BOUNDARY`, answers writes (`host_path` → `fs_write`, `std_stream` → `logging` per WI-dutah, `pipe` → `ipc_send`, `net_stream` → `net_send`) beside the read map (`pipe` → `ipc_recv`, `net_stream` → `net_recv`, which is the value INV-bagok's note said its network case was waiting on, so the `bufio` family gained its `net_recv` rows). The discrimination rule is now "two or more boundaries of ONE direction" and a primitive gated in both is refused at load; `TaintSink` carries `requires_target_kind` as `TaintSource` does, resolved in the entry's own direction. The Go analyzer classifies the writer argument through one table of target-argument positions: an inline producer, a wrapper followed into its own target (`tabwriter.NewWriter(os.Stdout, ...)`, `bufio.NewWriter(f)`), a bare or `&`-prefixed identifier through its last binding, and a name with no visible binding through its declared TYPE (`*bytes.Buffer` / `strings.Builder` never leave the process; `net.Conn` / `http.ResponseWriter` are network streams; `io.Writer` and `*os.File` abstain). **INV-minol, found while building it:** the taint matcher admitted a gated entry only on an exact stamp, so an unstamped `bufio.NewScanner` -- `ipc_recv` in `io-boundaries` by INV-fatok's declaration -- minted nothing, and the same clause would have deleted every unstamped `io.WriteString` sink. The derivation now marks the entry derived from the row the classifier falls back to as the `abstention_fallback`, and the matcher admits it when the stamp abstains: one catalogue, one answer in both consumers. **The same ruling read in the other direction closed INV-lozat: a launch that HANDS BACK the child's bytes is a transfer.** `exec.Command(...).Output()`, `subprocess.check_output`, `os:cmd/1`, `System.cmd`, `Command::output`, `readProcess`, `execSync` and `ProcessBuilder.lineStream` all return a value whose content a program hypergumbo cannot see chose, so ruling 1 answers YES and they mint an `untrusted_input` source at `ipc_recv` — **alongside** their `subprocess` row, declared `simultaneous: true` (the `scala.sys.process.Process.apply` shape), so the launch keeps its sink, its opacity and its boundary-map chain. The boundary was not a new choice: `rust.yaml` already rowed `std::process::Child.wait_with_output` and `ChildStdout.read` as `ipc_recv`, citing `python.yaml`'s `os.wait` — and rowed `Command::output`, the stdlib's own spawn-then-`wait_with_output`, as `subprocess`, so one file gave two answers for identical bytes. The sibling forms stay launch-only and for two different reasons: an exit status (`Run`, `call`, `system`) is not the child's message, and a HANDLE (`Popen`, `ProcessBuilder.start`, `Command::spawn`, `createProcess`, `popen`) is the `net_listen` shape one channel over — a deferred crossing, disclosed and not minted. `subprocess.run` is deliberately absent: its `stdout` is the child's only when the caller passed `capture_output=`, and the catalogue matches on name, not on keyword arguments. **WI-lipis's second deliverable made the wrapper's boundary follow its ARGUMENT'S ORIGIN.** `bufio.{NewScanner,NewReader}` is rowed `ipc_recv` on the note *"When wrapping os.Stdin"*, a condition holding at **zero** of the 83 resolvable bare-local sites, 63 of which wrap an `os.Open` handle. The Go analyzer now reads the last binding of a bare-local argument in the enclosing function and stamps `io_target_kind` from it, and `read_boundary_for_target_kind` maps a target kind to the boundary a READ there crosses — `host_path` → `fs_read`, which is absent from the auto-source map by design, so the source is refused for its BOUNDARY rather than for crossing nothing (the sink side still counts those same `host_path` targets). Unresolvable origins — a parameter, a struct field — stamp nothing and classify exactly as before (INV-zumin: one answer per call site or none). Measured on the 0006 cohort: 8 rows removed, 0 added, no verdict moved, every removed row read back to an `os.Open` handle, and the crossing still represented by beads' 55 `os.Open` `fs_read` chains. **The third deliverable carried the mechanism to c and haskell, where the SAME defect points the other way.** Those catalogues file their stream-takers (`fgets`/`fscanf`/`fread`/`getc`/`fgetc`, `hGetLine`/`hGetContents`/`hGetChar`) as `fs_read`, which mints nothing, so `fgets(buf, n, stdin)` and `hGetLine stdin` — the canonical untrusted-input calls in both languages — produced no source at all, a silent false NEGATIVE rather than Go's false positive. Both are now declared under `fs_read` **and** `ipc_recv` with `boundary_ruling: call_site_undecidable`, and a second narrowing seam, `_narrow_by_target_kind`, drops the losing row using the stamped `io_target_kind` — the twin of `select_by_mode`, separate from it because a mode is a literal a row can name while a stream's nature is a property of what its argument was bound from. It runs on **every** path of `lookup_with_module`, including the short-name fallback where c's `fgets` actually lands, and its discrimination rule ("two or more READ boundaries") excludes `builtins.open` automatically, since `fs_write` is not a read boundary. An abstention — a parameter, a `popen` handle, a collapsed edge whose sites disagree — resolves to the **first-declared row**, so an unrecoverable origin classifies exactly as before; declaration order is therefore load-bearing and is pinned by a test against the shipped YAML. The catalogue change alone was **wrong**: `fs_read` never becomes a `TaintSource`, so the surviving `ipc_recv` row matched by name at every call site including the file reads, and `TaintSource` now carries `requires_target_kind`, the mirror of the sink side's `requires_mode`. Measured with a live control in one cold run per language: c's `scanf` and haskell's `readProcess` fire in both arms, `fgets(buf, n, stdin)` and `hGetLine stdin` are newly found, and the `fopen`/`openFile` variants stay correctly silent. **A haskell zero-ARGUMENT action was unreachable for a different reason entirely, and INV-fofoj separates the two.** `hGetLine stdin` is an application, so correcting its row reaches it; `getLine` is a bare identifier in every position it appears — bound with `<-`, alone on a `do` line, chained with `>>=`/`=<<`, lifted with `<$>`, discarded through `void`/`liftIO` — and the analyzer emitted a `calls` edge only for an application, so **no row could reach it at all**. Fixing a row does not make it reachable. The analyzer now also emits for the three positions where a bare identifier is unambiguously EXECUTED — the right-hand side of a `<-` bind, a bare `do` statement, and the argument of a combinator whose only purpose is to run an action (`void`, `forever`, `liftIO`, `try`, `replicateM{,_}`) — and **fails closed on a name the enclosing definition binds for itself**. The infix operators are excluded on measurement, not oversight: Haskell spells `fmap` over Maybe, lists and Either with the same operators it spells IO with, so across eight repositories they would add 63 catalogued sites for 3,812 more at 18–35% naming a local binding, against the 442 of 505 the three covered positions reach at the three lowest wrong-edge rates. Measured cold in both arms: **1,916 → 2,239 chains, +323, zero lost**, all haskell, and the population is dominated not by the four Prelude stdin readers the item named but by every OTHER zero-argument row — `getCurrentTime` 220, `getCurrentDirectory` 20, `getArgs` 17, `getEnvironment` 17, `exitFailure` 16. **rust already satisfies this requirement** (`BufReader::new(stdin())` is found, the file and in-memory wrappers correctly produce nothing) and java does not — its `Scanner`/`BufferedReader` wrappers are the receiver-origin shape, which needs the constructor's argument resolved through the receiver's binding and is not covered here. **A primitive that is both a source and a sink was new, and it needed a third per-call-site gate**: `_source_and_sink_are_one_call` refuses a pair whose source and sink are ONE invocation, since a returned value cannot be an argument to the call that returned it. It fires only on positive evidence of a single recorded call line — two or more lines is a loop shape where the pair is real. **WI-zovuz gave the sink side a name-level proof for bash, which had none.** bash is `dataflow_capable: false`, so a redirect-sink finding rested on call-graph reachability alone — *this file reads the environment somewhere AND reaches a function that writes somewhere*. The analyzer now closes name derivation over the whole file (assignments, `for` bindings, and positional parameters bound at every call site of the enclosing function) and stamps `redirect_origin_names`: the externally-derived names that can reach what **the shell itself** contributes at that site, which is three things — the target operand, a heredoc body the shell expands, and every producing stage's arguments. An **empty** stamp is a proof (no value this program holds can be what crossed) and joins `_sink_call_can_carry_taint` as its third arm; an **absent** stamp is not, and keeps the finding, which is the state of every map written before the key existed and of any file that did not parse cleanly. **It is deliberately not the byte-producer question**, which was measured wrong: *is the writer external?* deletes `echo "$SIGNING_CERT" | base64 -d > cert.pfx`, where a real certificate is written and only stage one is a builtin, and misreads `sed -E "s#x#${v}#" f > out`, where an in-process value is interpolated *through* an external command. **The fetch case is left open**: `curl -L "$URL" > "$DEST"` still credits `URL`, because deciding that curl's argument merely *selects* a remote resource is per-command semantics this gate does not have — that gap is now closed: `_FAR_SIDE_FETCH_COMMANDS` (`curl`, `wget`) declares the commands whose stdout is content the far side chose, which is ADR-0016's prohibition stated positively — it forbids attributing a launched program's I/O *to* the shell, and this declines to attribute the child's *bytes* to it. **The per-edge gate is joined by a PAIR-level one.** `_source_names_can_reach_sink` compares the source edge's `env_var` (or, on a collapsed edge whose sites disagreed, `env_var_values`) against the sink's stamp, and refuses the pair when they are provably disjoint — the shape INV-fumod filed, where guacamole's redirect IS reached by `DESTINATION` while the row is sourced at `MYSQL_JDBC_VERSION` read at a different site. It sits beside `_source_and_sink_are_one_call` because neither edge alone answers it, and it is asked by BOTH propagators: a language with no DDG still reaches findings through the DDG arm, which reports `analysis_method: structural` with `walk_verdict: unavailable`. **INV-pujob then fixed the SOURCE side of the same family.** bash's env-read discriminator is whole-FILE — a name expanded but never assigned here came from the environment — which is wrong across a `source`, since bash is dynamically scoped in BOTH directions (the sourced script sees the sourcer's assignments and the reverse). guacamole's `DESTINATION`, assigned once in the tree at `build-guacamole.sh:58`, was a `host_secret` source in every `build.d` script that read it. A repo-wide index now carries per-file assignment sets and the `source` graph. **Resolving the target is the hard half**: of the cohort's 97 `source`/`.` statements only 12 (12.4%) have a target the emitted edge can follow, because 80 (82.5%) are a variable or command substitution — but they are dynamic in the PREFIX and literal in the TAIL (`source $TOOL_LIB_PATH/gitlib.sh`), so matching the trailing literal component against repository files lifts that to 81 of 97 (83.5%) with zero ambiguity, and matching a glob directory's last component (`build.d`) recovers the container path a Dockerfile `COPY` joins to the tree. Every branch **fails closed** — an unresolved or ambiguous target contributes no assignments and the name stays an environment read — because this direction removes taint SOURCES, where an over-eager answer is a false all-clear rather than a false positive. Order is deliberately not modelled, exactly as INV-jurif's whole-file rule does not model order within a file. The read-side categories `fs_read` and `browser_storage_read` are intentionally absent from the auto-source map — sensitivity of a file or browser-storage read depends on what is stored, so the default is quiet and project-local catalogs (see `taint_sources/`) can opt in entries relevant to the threat model. The auto-derivation covers every language with an io_primitives catalog (Python, Rust, Go, Java, JavaScript/TypeScript, and the rest), and includes attribute-kind primitives such as `os.environ`, `sys.argv`, `process.env`, `os.Stdout`, and `System.out` via `module_attr_ref` edges emitted by the Python AST analyzer (WI-guhok) and, for tree-sitter languages, the `emit_module_attribute_refs` helper wired into Go / JS / TypeScript / Java analyzers (WI-lozug PRs 1–3). Tree-sitter Rust (`std::env::consts::OS`) and C (bare `stdout` / `stderr` / `stdin` identifiers) were pending and have since landed: WI-vipur gave `emit_module_attribute_refs` a scoped-path traversal mode for grammars that model qualified references as left-recursive path+name pairs rather than object+property (Rust `scoped_identifier`, C++ `qualified_identifier`), and WI-gotuv gave the C analyzer a separate bare-identifier path: C's stdio globals are referenced as bare identifiers (`fprintf(stdout, ...)`), never as member expressions, which the shared helper does not match — so `c.py` emits those `module_attr_ref` edges itself from its own `_C_STDIO_GLOBALS` table. **The shared helper is wired into five analyzers** — `cpp`, `go`, `java`, `js_ts`, `rust` — with C covered by that separate mechanism, six languages in total. Groovy and Kotlin have neither mechanism and emit no `module_attr_ref` edges, so their module-attribute reads (and any `env_read` taint sources those would carry) are absent, not empty. Measured end-to-end 2026-08-18: `std::env::consts::OS` emits a `module_attr_ref` edge that classifies `env_read` alongside the `std::env::var` call; bare `stdin` classifies `ipc_recv` and bare `stdout` `logging` (per WI-dutah, terminal output is not IPC). The C catalog still self-reports `status: in_progress`, which `io-boundaries` discloses at runtime. **Which Rust backend you select does not affect this**: attribute-reference coverage is the tree-sitter analyzer's, the SCIP backend emits no `module_attr_ref` edges of its own, and the two run alongside each other today — by default, not by ruling: nothing in the pipeline merges, prefers or reconciles the two arms (symbols are concatenated with no key; the edge dedup keys on `Symbol.id`s that differ between arms), and ADR-0012 contains no ruling on backend coexistence at the symbol level. An earlier version of this sentence cited "ADR-0012 coexistence"; WI-kanuh retracted the citation on 2026-09-18, and coexist / merge / replace is open under WI-gojum (parked). So the same `module_attr_ref` edges are present under `--backend rust-analyzer`.
...
**Built-in taint labels:** `host_secret`, `host_description`, `untrusted_input`, `plaintext`, `key_material`, `ciphertext`, `derived_key`. `host_description` is not a weaker `host_secret` — it is a different fact, and a claim naming one does not match flows carrying the other. The list is not hand-maintained: `all_source_labels` derives it from the loaded catalogue unioned with `AUTO_SOURCE_LABEL_MAP`, and a claim naming any other label is refused (INV-todas).

**What `untrusted_input` asserts, and what it does not (2026-09-06 taint-label concept audit, `~/hypergumbo_lab_notebook/concept-audit-taint-label_09062026.md`).** The label asserts exactly ADR-0049 ruling 1's fact: *a party on the far side of the boundary had a say in this value.* It does **not** assert that the far side authored the value's bytes. Of the 324 catalogue names that mint the label, 24 return a value the far side merely **chose from a constrained set** — `accept`'s peer address in seven languages, erlang's `inet` DNS answers, the exit status collected by `os.wait*` / `Child.wait` — and those carry the label too, on the same rule that keeps `accept` a `net_recv` transfer. A claim whose question is *injection* (the generic `untrusted-input-no-subprocess` text calls itself "the command-injection question") is therefore reading a stronger fact than the label states, and a finding rooted at a choice-shaped source can only ever be a *selection* finding, never an injected payload. The label is deliberately **not split** on this property (WI-vazal measured that a label split silently flips verdicts on already-written claims and carried origin alongside instead) and the boundary is **not split** either (WI-dozul, 2026-09-06). **This is a DECLARED BLINDNESS as of 2026-09-13, declined on measurement rather than pending (WI-jivih, arc T8).** A per-row `value_shape` attribute was the candidate fix, and the item pre-registered its own decision rule *before* any number existed: under a 5% finding-weighted share, the field is not worth its cost. A 23-repository cohort sampled **deliberately for socket ownership** — chosen to be maximally favourable to the property — returned **2 choice-shaped findings out of 568, 0.35%**, an order of magnitude under the line. That zero is not the earlier void ones: this corpus *sees* the population, with 87% of it reaching a choice-shaped chain across 12 primitives and 6 languages. The three terms are each measured, none inferred — presence 29.8% of eligible repositories, reachability 87% of a socket-owning cohort, findings 0.4% of evidence rows — which is this project's own *presence ≠ reachability ≠ truth* rule with every term filled in. **Reaching a choice-shaped row almost never produces a finding**, so the field is not built and the property is disclosed instead.

A `violated` verdict whose evidence is rooted at one of these sources now carries the `choice_shaped_source` caveat, naming the primitives and dating the decision — the one caveat in `verify-claims` that rides the **violated** path rather than a clean one, because that is where the blindness costs the reader something. You can also read `source_primitive` on the evidence row directly: it names the terminal function, and the vocabulary is `CHOICE_SHAPED_SOURCE_NAMES` in `hypergumbo_core.verify_claims`.
...
| **Violated** | Taint flows from source to sink without sanitization. The verdict carries per-flow drill-down evidence (WI-kikis): `details` renders up to five *distinct* flows with their source/sink symbol IDs and a `via N hop(s)` path indicator (deduplicated on the full flow identity, so identical-looking rows collapse and the honest total-vs-distinct count is shown), and the `--format json` envelope adds a bounded structured `evidence` array (`source_symbol` / `sink_symbol` / primitives / `path`, ≤100 distinct flows) for programmatic triage even at high evidence counts. **A row is a SITUATION, not a source→sink pair, wherever the pair was not adjudicated (INV-karud).** An `analysis_method` of `structural` or `ddg_mixed` means inclusion rested on call-graph reachability, so "primitive P1 reached primitive P2" is more than the analysis established; such findings are collapsed to one per (taint label, source symbol, sink zone, sanitized, source boundary, analysis method) carrying `source_primitives` / `sink_primitives` / `sink_symbols` as module-qualified sets — "symbol S reads {P…} and reaches zone Z via {Q…}". A `ddg` finding keeps its pair claim, because the §3a walk confirmed the dependence. The pair total is disclosed beside the situation count in `details` and per row as `collapsed_flow_count`, so nothing is traded away silently. Measured across six repositories: **359 rows → 80 situations (4.49×), 0 verdicts moved, 0 (source, sink) pairs lost.** |
...
🟩 **The coverage scope is published as data, not prose (INV-karud clause a3).** Which languages can be data-flow adjudicated is a fact that decays — this paragraph has been wrong twice — so `verify-claims` computes it at runtime and emits it. Every run carries a `dataflow_coverage` block (envelope `1.7`; rendered on the text surface too) with one row per analyzed language carrying its catalog counts and four independent capability bits — `cfg_mapping`, `atomic_statement`, `def_use_extractor`, `ddg_spec` — plus `dataflow_capable` (their conjunction) and `blockers` (the missing ones, so the row says what to fix). Four bits rather than one flag because each is individually sufficient to keep the machinery silently inert. The block also states two things no per-run measurement can: `inclusion_decided_by` (`call_graph_reachability_minus_ddg_refutation` — reachability still decides inclusion and the walk mints nothing, but since WI-kabif it may subtract a flow it refutes) and `coverage_granularity` (`function` since WI-mugop — capability is a per-language fact, but the forfeit gate now decides refutation per FUNCTION, and the block publishes both halves of that decision as `functions_walkable` / `functions_forfeited`. The value's own re-open trigger had fired 19 days before anything noticed, because the test guarded the constant rather than the condition; a capable language still holds functions the analysis cannot see into — `cfg_nodes/go.yaml` self-documents that `if err := ...` initializers are invisible to def/use — and a THIRD class is invisible to the gate entirely, where the extractor records a statement and then accounts for nothing inside it, which the walk now escapes on per VARIABLE rather than forfeiting per function (INV-lupav L4)). Alongside it, `findings_by_analysis_method` counts every finding the propagators produced, which is a deliberately different denominator from a verdict's own `analysis_methods` — one describes the analysis, the other describes a claim. Beside both, `walk_verdicts` (envelope `2.3`) counts what the §3a walk itself returned — `confirmed` / `unconfirmed` / `escaped` / `not_attempted` / `unavailable`, plus `mixed` for a collapsed row whose members disagreed. Without it `flows_removed_by_walk: 0` is unreadable, since only `unconfirmed` can remove a flow and `ddg_mixed` collapses three of the others: "the walk adjudicated and refuted nothing" and "the walk never got to look" are opposite facts with identical evidence (ADR-0052).

**Function summaries.** *Half implemented, half live* — and this paragraph asserted the wrong half until 2026-08-10. **§4b (declared summaries) RUNS IN PRODUCTION**: `propagate_taint_ddg` calls `load_function_summaries()` on its default path (`taint.py`), and the live index holds **39 terminating summaries of 71 distinct entries** (`fmt.Printf`, `log.Println`, `builtins.print`, `console.log`, …) that the §3a walk consults at call sites. **§4a (`infer_summary`, DDG-inferred summaries) still has zero production callers.** The distinction is load-bearing rather than pedantic. **Re-measured 2026-09-02 (WI-vosug): the catalogue gap on hypergumbo's own tree is now 0 of 7 call-node escape sites, down from 8 of 26 (30.8%).** Both halves of that fall are real and neither is this catalogue growing to cover a language: INV-foluz removed the emission gap that dominated the old denominator, and the two callees left in the gap bucket — `builtins.hasattr` and `builtins.float` — were declared after being read back against source. Escape sites are **not** a proxy for verdicts: closing `hasattr` moved the §3a census not at all (14 confirmed / 12 escaped / **0 unconfirmed** across 27 walks), because its function carries three other escape sites and closure is all-or-nothing per walk. When taint crosses a function call boundary the solver consults a summary to determine how arguments map to return values. Summaries are either inferred automatically from DDG analysis or declared in YAML for stdlib/framework functions whose source is not analyzed. Declared summaries live in `function_summaries/` (`go_stdlib.yaml`, `python_stdlib.yaml`, `rust_stdlib.yaml`, `typescript_stdlib.yaml`; each file's header states its own scope rule, and all four are **enumeration-derived** — an entry is admitted only where the walk was measured consulting that callee at a real escape site, never because the function is common) and support `param_to_return`, `param_to_self`, `mutates_self`, `side_effect`, `sanitizes`, and structured `callback` flow for higher-order functions. Functions without an explicit summary receive the conservative default: all parameters flow to the return value.
