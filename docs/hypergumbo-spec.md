...
A pending **v6** is defined by [ADR-0035](adr/0035-stable-id-v6-identity-contract.md) (full scope-chain hashing, unique-within-run) but is **not yet emitted** — the current shipped value remains `v5`. This history is the precondition recorded before that bump (never bump a scheme onto an undocumented chain).
