...
* **Auto-derived from `io_primitives/*.yaml`.** Each IO primitive in a write-side category becomes a taint sink at `trust_level: untrusted` in a matching trust zone: `fs_write` → `host_fs`, `net_send` → `network`, `subprocess` → `host_fs`, `env_write` → `host_env`, `ipc_send` → `ipc`, `browser_storage_write` → `browser_storage`. Each IO primitive in a read-side sensitive category becomes a taint source: `env_read` → label `host_secret`, `net_recv` → label `untrusted_input`, `ipc_recv` → label `untrusted_input`. The auto-derivation covers every language with an io_primitives catalog (Python, Rust, Go, Java, JavaScript/TypeScript, and the rest), and includes attribute-kind primitives such as `os.environ` and `sys.argv` via `module_attr_ref` edges.
...
**Project-local catalog extension.** The YAML parse paths (`load_taint_catalog(source_paths, sink_paths, sanitizer_paths)`) are public and accept user-supplied files, but there is no CLI flag yet for a repo to hand paths to `verify-claims` at run time — an end-user installing hypergumbo from PyPI currently cannot override the auto-derived defaults or introduce their own trust zones without writing a wrapper script that calls the Python API. The CLI wiring (`--taint-sources`, `--taint-sinks`, `--taint-sanitizers`, plus a matching key in the claims-file format) is tracked separately.
