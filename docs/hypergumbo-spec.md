...
**Built-in taint labels:** `host_secret`, `host_description`, `untrusted_input`, `plaintext`, `key_material`, `ciphertext`, `derived_key`. `host_description` is not a weaker `host_secret` — it is a different fact, and a claim naming one does not match flows carrying the other.
