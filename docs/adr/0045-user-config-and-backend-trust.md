...
- **Rulings 1 (config half), 2, 3 — landed** (`hypergumbo_core.user_config`). Two tiers (`$XDG_CONFIG_HOME/hypergumbo/config.toml`, `<repo>/.hypergumbo.toml`), the trust-key deny-list, and `io_primitives` overlay paths as the first real setting. The second is the `[merge]` table (ADR-0057 §5, WI-hukuf): backend precedence for contested attributes, per-attribute overrides, and the two declared levels — a preference in both tiers, project replacing user key by key, every value validated against the registry's declared backends and the merge pass's arbitrated attributes.
- **Ruling 4's config tiers inside `resolve_optin` — LANDED with the trigger (WI-nanom, the first non-executing backend).** `resolve_optin` takes the merged config decision as the tier below the environment; `resolve_scip_python_optin` supplies it from `LayeredConfig.backends`; `[backends] <name> = true|false` is accepted for exactly the registry's `config_optin_backends()` (an alternative arm declaring `executes_analysed_code=False`), project replacing user, and refused as a trust key for the executing ones — both lists derived from the same declaration. Original text of the deferral kept below for the record.
- **(historical) Ruling 4's config tiers inside `resolve_optin` — DEFERRED, with a trigger.** Those tiers are reachable only by a backend whose opt-in is *allowed* in config, i.e. one that does not execute analysed-repo code (ruling 5). No such backend exists yet. Wiring them now would add a layer with no consumer and no way to test it except hypothetically — the same reasoning ADR-0022 used to defer its own runtime-aggregation half. **Trigger: the first non-executing backend** (pyright, WI-nanom). `resolve_optin` is shaped for it and its docstring names where the tiers go.
- **Rulings 1 (trust half), 5, 7, 8 — landed** (`hypergumbo_core.backend_trust`, `hypergumbo trust-backend`). Grants live under `$XDG_STATE_HOME/hypergumbo/trust.d`, keyed by resolved absolute path, `0600`, with a decline recorded as a decision so the nudge can stop asking. The trust store is the lowest tier of the opt-in chain (flag > env > grant), and `executes_analysed_code` now gates the store from *both* sides — config refuses an executing backend, the store refuses a non-executing one. Since WI-hohuh (ADR-0057 §10) the bit is a declaration on `@register_analyzer` and both gates derive their sets from the registry; until then it was a hardcoded `frozenset({"rust_analyzer"})` in `user_config.py`, the list ruling 5 says the next backend must not depend on.
