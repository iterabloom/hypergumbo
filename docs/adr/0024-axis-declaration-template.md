...
2. **Pre-commit detection of new multi-value field additions** — the creation practice currently relies on documentation. Worth automating? Detection requires AST inspection of `@dataclass` definitions for fields whose type narrows (Literal vs. str), or a manual hint annotation. **Resolved by WI-busij-kadoz-bolok-lavig-lofaz-fogub-gajod-danif (2026-05-20)** via `packages/hypergumbo-core/src/hypergumbo_core/multi_value_field_axis.py` (linter logic), `scripts/check-multi-value-field-axis-declaration` (entry script), and `packages/hypergumbo-core/tests/test_multi_value_field_axis.py` (live-tree gate + 35 unit tests). The linter requires every `str` / `Optional[str]` / `Literal[str, ...]` field on a `@dataclass` in `ir.py` or `datamodels.py` to carry a `# axis: <category>` trailing comment. Four valid categories: `<known-axis-name>` (named axis with a registry; resolves through a small `_known_axes` table), `identity` (unique-per-record), `bounded-enum` (small fixed list documented in docstring), `free-text — <justification>` (open-ended payload, justification mandatory). The deferral-hatch option (`# axis: pending WI-xxxxx`) considered in this question's original framing was explicitly dropped during the WI-busij design discussion: it duplicates the structural-fix protocol's `todo_hard` circuit breaker and weakens the gate at exactly the moment the design conversation should happen — i.e., at PR review, before the field ships. The `# axis: free-text` category carries a justification requirement for the same reason: it's the only category whose "this is the right call" claim isn't anchored elsewhere (named axes have a registry; `identity` has a uniqueness invariant; `bounded-enum` has the docstring listing), so it would otherwise be the natural can-kicker. Two catalog-derived lightweight axes (`language`, `pass-id`) were added alongside the heavyweight registry-backed axes (`edge-type`, `symbol-kind`, `evidence-type`) — they instantiate the "use judgment" carveout in §4 (no separate registry module; the `all_known_languages()` / `all_known_pass_ids()` functions in `catalog.py` return the union of `languages=[...]` kwargs / registered pass names across analyzers and linkers, a single source of truth derived from data we already have).
