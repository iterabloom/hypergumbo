...
Hypergumbo ships community overlays under `hypergumbo_core/io_primitives_overlays/`, kept separate from the stdlib-scoped catalogs: hypergumbo distributes and discloses those rows but does not vouch for them. **Amended by [ADR-0047](0047-catalogue-scope-and-user-visible-homes.md)**, which loads them by default under a disclosure requirement; §27's stdlib scope for the *catalogs themselves* is unchanged.
...
>
> **The inbound direction of this same prohibition is ruled by [ADR-0049](0049-deferred-crossings-are-disclosed-not-minted.md).** The note above forbids attributing a *launched program's* I/O to the launcher. The mirror case — a call that *starts a server*, where the data arrives at a handler the runtime invokes rather than at the caller — went unruled for four re-derivations. ADR-0049 supplies the discriminator (does the call return, to its own caller, a value the far side chose?) and the treatment (disclose, never mint), and adopts this note's `command_launch` policy for the inbound case without reusing its producer-stamped vehicle.
>
