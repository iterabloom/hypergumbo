...
| `host_info_read` | inbound | Read host DESCRIPTION or user identity — not a secret (split from `env_read` by INV-tutar) | `runtime.GOOS`, `os.uname()`, `navigator.platform`, `pwd.getpwnam()` |
