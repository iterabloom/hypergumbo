...
On `add`, the store computes the new item's SimHash and compares it (by Hamming distance) against existing items' cached SimHash fingerprints. If the distance is below a configurable threshold, a warning is emitted. Both paths use a threshold of 8 bits: `store._SIMHASH_THRESHOLD` and `validation._SIMHASH_THRESHOLD` are unified and pinned equal by `test_simhash_threshold_is_unified_with_validation`, so create-time and `validate --similar` cannot drift apart. The create-time value was loosened to 13 during implementation and later tightened back to 8 to cut false positives on unrelated items sharing common tracker vocabulary. These empirical thresholds were tuned during implementation — the illustrative "≤3 bits" from early design proved too aggressive (high false-positive rate on real items). Example warning:

```
UserWarning: INV-lusab-bired-fomak-gunid-hasob-jikal-mofad-nukit is similar to existing INV-fodak-humit-kobap-linud-rasib-sufag-tohim-vukad (SimHash distance: 3 bits). Creating anyway. Use 'update INV-lusab-bired-fomak-gunid-hasob-jikal-mofad-nukit --add-duplicate-of INV-fodak-humit-kobap-linud-rasib-sufag-tohim-vukad' to mark as duplicate, or 'update INV-lusab-bired-fomak-gunid-hasob-jikal-mofad-nukit --add-not-duplicate-of INV-fodak-humit-kobap-linud-rasib-sufag-tohim-vukad' to suppress.
```
...
| `add --kind <kind> --title "..." [--tier canonical\|workspace\|stealth]` | Create new item (appends `create` op; default tier: workspace). Computes SimHash on creation and warns if similar items exist (see [Key Design Decisions](#key-design-decisions)). Accepts prefix or positional alias (`:N`) for the `update` link flags `--add-duplicate-of`/`--add-not-duplicate-of`. | agent |
