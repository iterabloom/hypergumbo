...
│   ├── config.yaml                  # defines valid kinds, statuses, tags, stop_hook scope
...
# Unified status field. The stop hook counts items where
# status is todo_hard or todo_soft — these block the agent from stopping.
statuses:
...
# On upstream repos, the default "all" counts canonical + workspace.
# On forks, set to "workspace" so the fork agent isn't blocked by
# upstream's canonical items (which it can read but not close).
# Detected automatically by scripts/fork-setup (presence of "upstream" remote),
# or set manually.
stop_hook:
...
The last-displayed ID list is stashed in `.agent/tracker/.last_list` (gitignored). The `:N` syntax is unambiguous (colon distinguishes it from an ID prefix). Positional aliases are ephemeral — never stored in op log files, just a CLI convenience.
...
Lock/unlock ops are themselves never lockable — they are human-only operations by convention.
...
1. `discuss <ID> --clear` — appends a `discuss_clear` op (compile ignores all prior discussion)
2. `discuss <ID> --as human "Priority stays at P0. Non-negotiable."` — appends a `discuss` op
...
Fork detection and scope configuration happen automatically during `scripts/fork-setup` (or first `scripts/contribute` run), which checks for the presence of an `upstream` remote. The human can also set `stop_hook.scope` manually in `.agent/tracker-workspace/config.yaml`.
...
**Location:** One `.cache.db` per tier directory (`.agent/tracker/.cache.db` and `.agent/tracker-workspace/.cache.db`), both gitignored. Created automatically on first read; deleted and rebuilt by `scripts/tracker cache-rebuild`. The location can be overridden via the `TRACKER_CACHE_DIR` environment variable — useful when the tracker directory is on a network filesystem where SQLite locking is unreliable (e.g., NFS). Setting `TRACKER_CACHE_DIR=/tmp/hypergumbo-tracker` puts the caches on a local filesystem. This is an edge-case escape hatch, not the default.
...
**Why SQLite.** It's in Python's standard library (`sqlite3`), requires no additional dependency, supports indexed queries for filtered listing (`SELECT ... WHERE status IN ('todo_hard', 'todo_soft') AND ... ORDER BY priority, created_at`), and handles concurrent reads safely (WAL mode). The `.cache.db` file is gitignored and disposable — it never enters the merge story.

**Why not `/tmp` by default.** The performance bottleneck is YAML parsing, not SQLite I/O. Cache hits already take ~0.02ms (a SQL query on indexed columns). Moving the database from `.agent/tracker/` to a tmpfs-backed `/tmp/` would save microseconds on an operation that takes microseconds — not meaningful. The `TRACKER_CACHE_DIR` override exists for the NFS edge case described above, not for general performance.
...
│       ├── models.py                     # Dataclasses + Status enum + op types
...
|---|---|---|
| `init` | Create `.agent/tracker/` and `.agent/tracker-workspace/` dirs (with `.ops/` dotdirs) + default config.yaml + .gitignore entries (.cache.db, .last_list, stealth/) | human/agent |
| `count-todos [--hard\|--soft]` | Print integer count of blocking items (respects `stop_hook.scope` config) | stop_logic.sh |
| `hash-todos` | Print SHA256 of all TODO content (circuit breaker) | stop_logic.sh |
| `validate [FILE...] [--similar] [--deep-similar]` | Exit 0 if all op log files valid, exit 1 with errors/warnings. When called with file paths, validates only those files (but still checks cross-file constraints like duplicate IDs and dangling parent refs against the full set). No args = validate all. `--similar`: surface near-duplicate pairs via SimHash (skips pairs in `not_duplicate_of`). `--deep-similar`: additionally uses embedding-based semantic tags for discrimination (requires `onnxruntime` + `tokenizers`; falls back to SimHash-only if unavailable). | pre-commit hook, CI |
...
| `update <ID> --status\|--priority\|...` | Update fields (appends `update` op; respects locked_fields) | agent |
| `discuss <ID> --as agent\|human "msg"` | Append `discuss` op | both |
| `discuss <ID> --clear` | Append `discuss_clear` op | human (TUI) |
| `discuss <ID> --summarize "summary"` | Append `discuss_summarize` op | both |
| `lock <ID> <field> [<field>...]` | Append `lock` op | human (TUI) |
| `unlock <ID> <field> [<field>...]` | Append `unlock` op | human (TUI) |
...
| `stealth <ID>` | Move file workspace → stealth (gitignored) | human |
| `unstealth <ID>` | Move file stealth → workspace | human |
| `show <ID>` | Print compiled current state (formatted) | agent |
| `list [--status X] [--kind Y] [--tag Z] [--tier T]` | Filtered list (compact table, sorted by priority/before/created_at; shows tier indicator) | agent |
| `ready [--limit N]` | List actionable, unblocked items (respects `before` soft-blocking; respects `stop_hook.scope` for scoping) | agent |
...
| `fork-setup` | Detect fork (upstream remote), set workspace `stop_hook.scope: workspace` | human/agent |
| `cache-rebuild` | Delete and rebuild `.cache.db` from YAML source of truth | human/agent |
...
- `cli.py`: argparse-based CLI with all subcommands (init, add, update, discuss, lock, unlock, promote, demote, stealth, unstealth, show, list, ready, log, validate, migrate, fork-setup, textconv). All `<ID>` arguments accept proquint prefix matching and positional aliases (`:N`).
- `init`: creates both `.agent/tracker/` and `.agent/tracker-workspace/` with `.ops/` dotdirs, `stealth/` dir, and config files
...
- `stop_hook.py`: scope-aware count_todos() (reads `stop_hook.scope` from config), hash_todos(), generate_guidance()
...
.agent/tracker/config.yaml             linguist-generated
.agent/tracker-workspace/config.yaml   linguist-generated
```
...
### Positive
- Schema enforcement eliminates vocabulary drift and malformed entries
