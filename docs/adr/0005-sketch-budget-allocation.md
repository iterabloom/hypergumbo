...
Date: 2025-01-15
Updated: 2026-03-19
Status: Partially superseded by ADR-0039 (entrypoint-selection rows) — **IN EFFECT**: ADR-0039 shipped in full, so the selection tables below read as `rank_score`-keyed. The detector-tier constants quoted below are still hand-written literals in `entrypoints.py` — ADR-0039 ruling 5 Stage B regenerated the *evidence-type* table, which is a different table; entrypoint tiers are in no registry
...
>
> **That relocation has landed, so this is no longer conditional: read every entrypoint-selection rule below as `rank_score`-keyed.** Evidence — `entrypoints.py::detect_entrypoints` sorts entrypoints by `rank_score` descending, not by `confidence`; the test-file, utility/example, vendor-tier and library-export penalties at `entrypoints.py::detect_entrypoints` multiply `rank_score` and leave `confidence` at its construction base; `Edge.confidence_source` ships with `VALID_CONFIDENCE_SOURCES = {evidence_derived, emitter_constant, composite}`; and `Edge.quality` was deleted outright at `SCHEMA_VERSION` 0.20.0 (ruling 4).
>
> **One caveat, and it is not the one this amendment originally predicted.** The 2026-06-11 text said the tier constants below "become registry-generated under ADR-0039 ruling 5 Stage B". They did not, because they were never in Stage B's scope: Stage B regenerates the **per-evidence-type** table from `EvidenceTypeSpec.base_confidence`, and it shipped — the projection lives in [`docs/concept-axes.md`](../concept-axes.md#derived-confidence--base_confidence-projection-adr-0039), generated by `scripts/generate-concept-axes` and gated in `.githooks/pre-commit`. The **entrypoint detector tiers** quoted here (0.95 / 0.99 / 0.70–0.85) are a different population: unregistered literals scattered across `entrypoints.py` (`confidence=0.95` at a dozen call sites; 0.99 appears only in a comment), and the §8 tier table in the spec is hand-written with no generator behind it. So these numbers remain parallel prose that can drift from the code — treat them as illustrative. Giving them a registry and a projection would be new work, not the completion of Stage B.
