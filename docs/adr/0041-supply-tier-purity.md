...
- Supersedes: the direct-dependency → `Tier.INTERNAL_DEP` mapping (`DependencyManifest.classify_import`, `supply_chain.py::DependencyManifest.classify_import`, and the boundary-node tier-min relabeling in `ir.py` (retired by this ADR)); spec §14's tier-2 prose in both its drifted forms (the original "internal libraries, monorepo packages **plus test dirs**" and the implementation's silent repurposing to "direct external dependencies").
- Superseded by: —
...
`Tier` (`supply_chain.py::Tier`) is an ordinal `IntEnum` whose documented semantic is supply-chain *position*: 1 first-party, 2 internal dependency, 3 external dependency, 4 derived artifact. Every consumer treats it ordinally — `--max-tier` filtering does `tier.value <= N` traversal cuts (spec §3, §14), and `TIER_WEIGHTS` (`ranking.py::TIER_WEIGHTS`, `{1: 2.0, 2: 1.5, 3: 1.0, 4: 0.0}`) treats lower tier as more trustworthy/relevant.

Into this distance axis, the manifest classifier smuggled a second meaning: *declaration status*. `DependencyManifest.classify_import` (`supply_chain.py::DependencyManifest.classify_import`) returns `Tier.INTERNAL_DEP` for any import whose longest-prefix manifest match carries `direct: true` — that is, **a third-party PyPI/Go-module package gets tier 2 simply for being declared in the project's manifest**. The boundary-node synthesizer compounds it: tier-min selection in `ir.py` relabels canonical external nodes to tier 2 with reason strings like `"direct dependency (pyproject.toml)"`.
...
Directness is derivable from the per-manifest dependency-node join (ADR-0035 ruling 4: a package declared in N manifests is N nodes), but it is **recorded anyway**, per the no-consumer-re-derivation rule. META-bifif is the cluster-level lesson: producers emit a finer/correct verdict that consumers ignore or re-derive coarser — WI-popok is the worked example (entrypoint detector re-deriving test-ness from a filename heuristic instead of consuming `supply_chain.is_test_file`).

> **Annotation (2026-07-17, WI-popok fundamental-concept audit).** The WI-popok illustration above reflects that item's *original* 2026-06-04 framing, when the entrypoint detector used a private `test_*.py` prefix check genuinely *narrower* (coarser) than the supply-chain classifier — a true re-derivation. The code has since inverted the relationship: the detector now consumes the *shared, broad* `paths.is_test_file` heuristic (a "test-OR-support" ranking predicate that also covers mocks/fixtures/testdata/benches), while `supply_chain.is_test_file` stays deliberately narrow (spec §14; §1 above excludes fuzz/bench from `is_test`). A fundamental-concept audit returned a **KEEP verdict**: the two are distinct-purpose predicates (ranking-deprioritization vs supply-chain test-code classification), not a coarse-vs-granular pair, so WI-popok is no longer a live re-derivation instance but a documented concept boundary. This does **not** affect §2's directness ruling, which stands on its own — directness *is* a genuine record-don't-re-derive case.

Making each consumer reconstruct directness from the manifest join would mint the next instance of that family.
...
This is **not** the existing `package_ecosystem` meta key (`axis_meta_keys.py::META_KEYS` (the `package_ecosystem` entry)), which names the package-*manager registry* of package-shaped symbols (`npm`, `composer`). Different question (which registry vs which provenance class); both keys keep their lanes, and the registration descriptions must say so explicitly.

**The single-source constraint (normative).** Exactly one stdlib catalog per language feeds **both** the supply-chain classifier's ecosystem stamping **and** every other consumer of stdlib RECOGNITION: the language-profile `stdlib_modules` machinery (python.yaml, parsed and merged in `io_boundary.py`; WI-bifih wires the same catalog into Python import resolution). Two independent stdlib-recognition sources would recreate the consumer-re-derivation disease *inside the campaign that exists to fix it*. Per-language extension goes through the same profile machinery (Go's no-dot first-segment heuristic at `supply_chain.py::DependencyManifest` may remain Go's recognizer only as the single shared implementation both consumers call, not as a private second source).

*Amended (INV-buzab, 2026-08-12).* This paragraph originally named the io-boundary **closed-world gates** among `stdlib_modules`' consumers. That was true and it was the defect: the closed-world question is *"has this module's I/O surface been enumerated"*, and `stdlib_modules` answers *"does the interpreter ship this name"* — 283 of Python's 300 entries carry no catalogue row, so the gate confirmed "never sends data over the network" for a program that opened `telnetlib.Telnet` and wrote a secret into it. The closed-world gates now read `stdlib_module_completeness` through `IoBoundaryCatalog.module_io_is_enumerated`, and a completeness entry no longer auto-promotes into `stdlib_modules`. **The constraint is unchanged and better satisfied:** stdlib recognition still has exactly one source, and it no longer has a second writer smuggling audit results into it — which had let a project-local overlay relabel a PyPI package as stdlib, the misread §3 exists to prevent.
...
- Code: retiring mapping `packages/hypergumbo-core/src/hypergumbo_core/supply_chain.py::DependencyManifest.classify_directness`; boundary relabeling in `packages/hypergumbo-core/src/hypergumbo_core/ir.py`; `TIER_WEIGHTS` `packages/hypergumbo-core/src/hypergumbo_core/ranking.py::TIER_WEIGHTS`; Tier enum `supply_chain.py::Tier`; stdlib-profile machinery `io_boundary.py::gate_named_entry` / `io_boundary.py::IoBoundaryCatalog`.
