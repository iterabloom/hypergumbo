...
1. **It never delivered uniqueness.** The original hash inputs (kind, param_count, arity_flags, decorators, containing_stable_id) collided at ~60% on the dogfood corpus — 155 bash functions in one file shared one stable_id; 152 zero-parameter pytest tests collided likewise (recorded at `spec_validator.py::_check_writer_contract`). The Phase 6 PR3 fix (scheme v5) added `name` + `qualified_name` to the hash basis (`analyze/base.py::make_typed_stable_id`). Residue persisted: 18.94% of Python class nodes still collide cross-module post-v3 (49× `TestAdaAnalysisUnavailable`-style per-language fallback classes, 36× `TestAnalysisRun` — INV-zudob); Python function-locals omit the enclosing function from `containing_stable_id` and collapse (WI-gitun, P0); `make_typed_stable_id` never took a `name` parameter at all (`analyze/base.py::make_typed_stable_id`), so distinct-named TypeScript methods collapse at 16.88% on HTRAC (WI-zitod); `make_dependency_stable_id` hashes only `(language, name)` (`analyze/base.py::make_dependency_stable_id`), so the same package declared in five manifests is one node (WI-titiz, P0); and `py.py` carries its own divergent local `_compute_stable_id` (`py.py::_compute_stable_id`) that earlier sweeps never listed as a producer.
...
- **LOGICAL identity** — kinds representing an **external target** (a thing that exists once regardless of how many code sites reference it). One node per logical thing, deduped, stable across runs. Constructed `make_protocol_stable_id`-style (`analyze/base.py::make_protocol_stable_id`): category namespace + logical identity parts.
...
- v5 hash inputs: `packages/hypergumbo-core/src/hypergumbo_core/analyze/base.py::normalize_signature_php`; typed-tier name gap: `analyze/base.py::synthesize_file_anchors_for_paths`; dependency factory: `analyze/base.py::synthesize_file_symbols_for_dangling_edges`; analogue factories: `analyze/base.py::make_module_stable_id` / `make_interface_stable_id` / `make_type_stable_id`; protocol factory: `analyze/base.py::synthesize_file_symbols_for_dangling_edges`; old 5% threshold: `packages/hypergumbo-core/src/hypergumbo_core/spec_validator.py::_check_writer_contract`.
