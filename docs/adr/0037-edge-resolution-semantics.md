...
- Status: **Accepted — Implemented (all rulings 1–5; ruling 4 landed 2026-07-02, PR #395)**
> Amended in place — see the "Implementation status" section for the per-ruling detail. Rulings 1/2/3/5 shipped in the Wave-2 edge-resolution train (2026-06-16). Ruling 4 landed 2026-07-02 with a scope split: the id-slot **output** fold was discharged by the ADR-0036 / WI-pubiv boundary-id remap (the final graph's node ids and edge dsts are `:external_symbol`), and WI-jopil (PR #395) delivered the **consumer refactor** (cgo/taint/taint_refine read `Edge.is_resolved`, not the `:unresolved` string) — which fixed a latent taint regression WI-pubiv had introduced. The producer `:unresolved` dst suffix is retained as an internal pre-remap convention.
- Date: 2026-06-10
...
1. **`Edge.is_resolved: bool`** — defaults to `True` at the producer (`packages/hypergumbo-core/src/hypergumbo_core/ir.py::Edge.create` on the dataclass, `ir.py::Edge.create` on `Edge.create`). Its docstring reads "whether the dst symbol was resolved at analysis time."
2. **The `:unresolved` dst-id suffix** — stamped at two different stages: by `make_unresolved_edge` at producer time (`analyze/base.py::make_unresolved_edge`, via the shared `format_legacy_dst` shape at `ir.py::format_legacy_dst`), and again — under the producer's untouched flag — by the boundary remap, which forces the kind slot to `"unresolved"` for every `ExternalRef`-bearing dangling dst (`ir.py::ExternalRef`, rendered by `_canonical_external_id` at `ir.py::_canonical_external_id`).
3. **`Edge.dst_ref: Optional[ExternalRef]`** — documented as "canonical source of truth for external dsts" (`ir.py::Edge.dst_ref`), but optional in `make_unresolved_edge` (`analyze/base.py::make_unresolved_edge`) even though that helper already holds all three components (`lang`, `module_hint`, `callee_name`) needed to construct one (`analyze/base.py::make_unresolved_edge`).
...
- **31 method-call-recovery edges** carry `meta["resolution_quality"]="recovery"` with `is_resolved=True` (`linkers/method_call_recovery.py::link_method_call_recovery` never passes the flag, so the `Edge.create` default applies), while the `resolution_quality` MetaKeySpec claims the key is "set alongside `Edge.is_resolved=False`" (`axis_meta_keys.py::META_KEYS` (the `resolution_quality` entry)) (WI-mutuk).
...
The placeholder kind slot `"unresolved"` describes the *reference* ("we could not resolve this call to a project symbol"), not the *referent* (the external module/function being pointed at). Encoding it in node identity is precisely the construct-vs-relationship leak that ADR-0027's axiom — kind names the language construct, never the relationship to it — exists to prevent. The leak is visible as a dual-minting inconsistency today: the same external target gets different kinds depending on which path minted it. The boundary synthesizer mints the node itself with `kind="external_symbol"` (`ir.py::_canonical_external_id`) while its id's kind slot says `unresolved` (`ir.py::format_legacy_dst` → `ir.py::Edge.to_dict`); legacy-parsed dangling ids preserve whatever kind token their producer stamped (`ir.py::is_external_boundary`). The validator's planned kind-slot↔registry cross-check cannot be stated cleanly while one entity answers "what are you" two different ways.
...
The fourth cell (`True` + populated `dst_ref`) is never produced: `dst_ref` is external-dst identity by definition (`ir.py::Edge.dst_ref`), and the finalization step below derives both surfaces from one verdict, so they cannot disagree.

The `Edge.is_resolved` and `Edge.dst_ref` docstrings (`ir.py::Edge.dst_ref`) are rewritten to state this contract, including the fact — previously undocumented — that an `is_resolved=False` edge still has a materialized placeholder dst node after boundary synthesis (present-but-synthetic, not absent).

### 2. A single edge-finalization step derives all three surfaces from one verdict

A single finalization step at the boundary-synthesis/remap stage — the `apply_external_id_remap` locus (`ir.py::apply_external_id_remap`), which already visits every edge — classifies each dst exactly once into **first-party | external-placeholder | dangling**, and derives ALL resolution surfaces from that one verdict:
...
- The producer-side default `is_resolved=True` (`ir.py::Edge.is_resolved`) **stops being load-bearing**. Producer-stamped values become advisory inputs; the finalizer's verdict is what serializes. This is what makes WI-kukuk's 4,507-edge contradiction structurally impossible rather than individually patched.
- `make_unresolved_edge` (`analyze/base.py::make_unresolved_edge`) derives `dst_ref` **unconditionally** [SENTINEL EXCEPTION (WI-huzuv): "unconditionally" reads as "for every edge with a *real* external module"; when the module is the `"external"` sentinel (module unknown) `dst_ref` stays `None` — the table's third "unidentified reference" cell — rather than fabricating `module_path="external"`. See "Implementation status".] from the `(lang, module_hint, callee_name)` components it already has (`analyze/base.py::make_unresolved_edge`), instead of accepting it as an optional kwarg that 67.8% of callers omit. This closes WI-zuhon at the producer side; the finalizer backstops any producer that bypasses the helper.

### 3. Recovery edges are correctly resolved; the `resolution_quality` doc is what gets fixed

Under ruling 1, the 31 method-call-recovery edges whose dsts ARE real in-repo nodes become **correctly** `is_resolved=True` by definition — a heuristic line-proximity resolution to a first-party method is uncertain in *pathway*, not in *locality*. WI-mutuk therefore resolves as a documentation fix, not a producer flip: the `resolution_quality` MetaKeySpec (`axis_meta_keys.py::META_KEYS` (the `resolution_quality` entry)) drops its "set alongside `Edge.is_resolved=False`" pairing claim and instead documents `resolution_quality` as a pathway-quality label orthogonal to `is_resolved` (resolution certainty lives in `confidence` and `evidence_type` per ADR-0028; target locality lives in `is_resolved`).

### 4. Node-kind fold: `unresolved` folds into `external_symbol`

> LANDED 2026-07-02 (PR #395); see "Implementation status" for the scope split. Ruling 4's *intent* — resolution state lives only on the edge, and one external target gets one node kind — is achieved, but not via a literal producer-side id fold: the id-slot **output** fold was discharged by the ADR-0036 / WI-pubiv boundary-id remap (the final graph's node ids and edge dsts are `:external_symbol`), and WI-jopil moved the three `:unresolved`-string consumers onto `Edge.is_resolved`. The prose below describes the id-slot end state (achieved on the **output**); the producer dst still emits `:unresolved` **internally** (pre-remap), retained as the `reference_syntax` source (ADR-0036 Ruling 2 / WI-pubiv).

The placeholder node kind `unresolved` — today forced into the canonical boundary id's kind slot by the remap (`ir.py::format_legacy_dst` → `ir.py::format_legacy_dst`) and stamped into producer dst ids by `format_legacy_dst` (`ir.py::format_legacy_dst`) / `make_unresolved_edge` (`analyze/base.py::make_unresolved_edge`) — **folds into `external_symbol`**. Resolution state is a property of the REFERENCE (the edge), not of the thing referenced; after this ADR it lives only on the edge surfaces of ruling 1. One external target gets one kind, regardless of which path minted it, eliminating the dual-minting inconsistency.

The symbol-kind vocabulary drops the value: `unresolved` never gains a `SymbolKindSpec` (it has none today — it exists only as an id kind-slot token), and the ADR-0036 id-grammar v2 kind-slot vocabulary excludes it. Boundary nodes keep `kind="external_symbol"` (`ir.py::Symbol.kind`, already CANONICAL per audit-findings 0007); their ids' kind slots migrate to match, which also unblocks the validator's kind-slot ↔ `Symbol.kind` equality predicate without a placeholder exemption. Dst-id strings on external edges change shape (`…:unresolved` → `…:external_symbol`); the identity impact is coordinated under ADR-0035/ADR-0036, in whose migration the implementing PRs participate.
...
| `WI-mutuk-gatik-sojik-nisul-kopom-fofiv-tonug-rifab` | 31 recovery edges carry `is_resolved=True` with `resolution_quality="recovery"` (pairing inverted) | Closed by ruling 3 (edges are correctly `True`; the MetaKeySpec pairing claim at `axis_meta_keys.py::META_KEYS` (the `resolution_quality` entry) is what gets fixed) |
...
- **Ruling 4 — IMPLEMENTED 2026-07-02 (PR #395, WI-jopil).** Delivered in two parts, and the delivery *corrected* the original DEFERRED framing. **(a) The id-slot output fold** was discharged not by a producer change but by the ADR-0036 / WI-pubiv boundary-id migration: `create_boundary_nodes` now mints boundary node ids with an `external_symbol` kind slot, and the existing `id_remap` rewrites inbound edge dsts to match, so the *final* graph carries no `:unresolved` id-slot (self-corpus `id_format|warning` for this cohort → 0). The producer dst ids (`format_legacy_dst`, `make_unresolved_edge`) still emit `:unresolved` **internally** (pre-remap), retained deliberately as the `reference_syntax` source (ADR-0036 Ruling 2) — the output fold makes the naive producer change unnecessary. **(b) The consumer refactor** (the load-bearing half, and a *different* root-cause family per the DEFER rationale — "consumer re-derives the producer verdict") moved `linkers/cgo.py`, `taint.py`, and `taint_refine.py` onto `Edge.is_resolved`. This **fixed a latent regression WI-pubiv introduced**: `taint._match_propagation_entry` and `taint_refine` run post-remap (in `verify-claims`) and string-matched `:unresolved`, so once the remap rewrote it to `:external_symbol` they treated every unresolved external edge as *resolved* — bypassing the WI-razol / INV-tapat ambiguous-name collision guard (false-positive `host_fs`/`network` taint flows). The unit tests missed it (pre-remap `:unresolved` fixtures); a regression test now locks the post-remap routing. Closes WI-jopil.
...
- Raw root-cause analysis: `~/hypergumbo_lab_notebook/correctness_strategy_06102026_full_workflow_result.json` (the verified three-surfaces/disjoint-writers finding, including `ir.py::UsageContext.to_dict` + `ir.py::Edge.to_dict` forcing `kind='unresolved'` under the producer's flag).
