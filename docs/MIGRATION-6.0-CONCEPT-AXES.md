...
| 0.13.0 | **Breaking.** `Symbol.canonical_name` removed (Phase 6 PR4 of the ADR-0033 campaign, one schema version after the 0.12.0 deprecation window). Producers that previously wrote to `canonical_name` should write to `qualified_name` (fully-qualified scoped identifiers) or `display_label` (UI-display strings on synthetic stand-ins). Consumers that previously read `symbol.canonical_name` or `dict["canonical_name"]` should read `symbol.qualified_name` / `dict["qualified_name"]`. The `Symbol.from_dict` constructor silently ignores legacy `canonical_name` keys for backward compatibility with pre-removal cached JSON. |
| 0.14.0 | Relaxing + additive — `docs/schema.json` catches up with the 0.12.0/0.13.0 dataclass reality (WI-kufib / WI-kutas): the Symbol / Edge / Span / AnalysisRun `$defs` are now introspected from the dataclasses instead of hand-coded. `Symbol.language` becomes nullable (`oneOf [string, null]`, dropped from `required`) so ADR-0031 Class B stand-ins validate; the stale `canonical_name` property (removed from the dataclass at 0.13.0 but never from the hand-coded schema) is deleted; `discovery_language` / `protocol_origin` / `display_label` / `qualified_name` are declared on Symbol and `failed_files` / `pass_version` on AnalysisRun. Documents that validated against 0.13.0 still validate against 0.14.0. |
| 0.14.1 | Additive — previously-opaque top-level blocks typed and previously-undeclared top-level keys added (WI-kutas PR2). `limits` → introspected `Limits` $def; `features[]` items → `Feature` / `SliceQuery` $defs; `metrics` properties declared; new top-level `reproducibility_context`, `symbol_fingerprint_scheme`, and `validation_report` (→ `ValidationViolation` $def). No previously-valid document stops validating. |
