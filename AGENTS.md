...
- **Agent Context Protection:** Always use `scripts/tracker show <ID>` or `scripts/tracker --json show <ID>` to read tracker item state. **`--json` is a GLOBAL flag: it goes BEFORE the subcommand, never after it.** Placing it after exits 2 with a usage error on stderr and nothing on stdout — a JSON-parsing pipeline reads that as a missing item rather than as a bad invocation (WI-dolaf). Always refuse to read files ending in `.ops`. These are internal operation logs that will pollute your context window with historical data you don't need. The CLI compiles ops into current state — that's what you want.
...
- **Op Durability (journal + self-healing recovery):** Pending `.ops` are mirror-written (inside `Store`'s per-file `flock`) to an out-of-repo journal at `~/hypergumbo_lab_notebook/tracker-ops-journal/<sha256(repo-toplevel)[:12]>/` (override `HYPERGUMBO_TRACKER_JOURNAL_ROOT`), and `.githooks/reference-transaction` + `.githooks/post-checkout` restore them automatically. **You do not need to manage this, and the journal is not a licence to run destructive commands on the working tree — the prohibitions above still bind.** When the repo does end up behind after `auto-pr`'s post-merge pull aborts on colliding `.ops`, run **`scripts/tracker reconcile`**: it flushes pending ops, fast-forwards, and restores the journal, each step skipped when unneeded. (`scripts/tracker recover` is its deprecated journal-only half, kept as the primitive the hooks call.) **Cases the hooks do NOT cover, so act:** `git clean` / `git stash` fire neither hook — run `tracker reconcile` after them; a naked manual `git merge`/`git pull` with a pending **untracked** op aborts (git correctly refusing to overwrite it) — `tracker reconcile` is the way through; a `SIGKILL` of `auto-pr`/`do_sync` leaks the `tracker-recover-disabled` marker, which silently disables self-healing until removed (WI-pohir); and after `rm -rf .git` the data is safe but recovery waits for a re-init/re-clone. The journal is best-effort — a write failure degrades silently and never fails the mutation. Mechanism, rationale and history: the `journal.py` module docstring.
...
- **`auto-pr` does not fit a foreground turn — detach it (WI-katap).** An agent harness caps one foreground shell call (Claude Code: 600s). `auto-pr`'s CI poll defaults to **2400s** and adds a **300s soft-retry that is not optional**, so even `--timeout 240` commits to 540s of polling before Scenario B, plus push, PR-create, merge and cleanup. Wrapping it in `timeout 590` does not make it fit; it kills it mid-poll. Measured on the convergence ledger: **22 of 84 runs** were killed this way, and **14 of the 15 PRs behind them had merged anyway** — every one then finished by hand with `merge-pr <N> --wait-for-ci`. Do this instead:
  ...
  ```
  You will not get to forget this: with no terminal, `auto-pr` **refuses to start** unless you pass `--detach` or `--foreground`, because exit 0 means different things in the two modes. `--detach` re-execs `auto-pr` as a `setsid` watcher that outlives your turn, writes `.git/AUTOPR_WATCH.json`, and records `detached_watching`. `wait` is three-valued on purpose: **0** = the watcher finished (it prints the outcome), **2** = still working, call again next turn, **1** = no watcher, or it *vanished* mid-poll (a dead pid with no result — do not read that as success). **Never wrap `auto-pr` in `timeout`.** A kill records `terminated_sigterm` rather than the `unknown` that is INV-rahib's violation signal, so it is no longer *invisible* — but it is still a run that died with the merge unfinished.
