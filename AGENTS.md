...
- **Tracker hygiene / dedup / meta-analysis sweep:** On-demand (human-triggered) sweep over the *committed* tracker corpus that (1) clusters open items into root-cause families, (2) flags TRUE_DUPLICATE / RELATED_BUT_DISTINCT pairs across the corpus via the dogfood "would fixing one fix the other?" test, (3) re-verifies `violated`/`pending_validation` statuses still reproduce at the pinned repo HEAD (positive evidence required to downgrade — never close on non-reproduction), and (4) emits a dominant-invariant-family meta-analysis table. Subagents deepen per-item investigations on a frozen corpus snapshot; the orchestrator steers them live via an append-only `corrections.md` blackboard + streamed per-item findings (steering lands at item/chunk boundaries, kept frequent by short work-lists). Reuses the twenty-pass dedup vocabulary; opens a timestamped lab notebook entry before any work; the agent produces data + proposals and the human authorizes folds/status-downgrades/edit-mode cleanups. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/tracker-hygiene-dedup-meta-analysis-playbook.md`.)
