...
- **Twenty-pass dogfood tranche:** A methodology-comparable procedure for running 20-pass dogfooding tranches when cross-tranche trend comparison is the goal — seven phases (setup → discovery → post-discovery audit → two-axis consolidation → blind severity judging → trend report → tracker materialization) orchestrated via sub-agent spawning so the operator does not babysit a tmux session. Two invariants govern every worker prompt: *de-primed discovery* (workers get the neutral real-issue registry and navigate toward unmapped ground; downstream dedup stages decide novelty, not the worker) and *parent-owned numbering* (workers use local opaque labels; the parent stamps global identity — `pass_NN.md`, F-numbers — only on aggregation). Use when the tranche will be compared to a prior or future baseline; use the lighter self-analysis playbook for one-off audit sweeps. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/twenty-pass-dogfood-procedure.md`.)
