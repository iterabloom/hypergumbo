...
- **Periodically and frequently test on real repos:** Use the lab journal/notebook (`$HOME/hypergumbo_lab_notebook/notebookjournal_<MMDDYYYY_HHMM>.md`) to record your observations and ideas as you experiment with hypergumbo settings on real-world projects. If you notice obvious bugs during experimentation, you needn't stop right away to fix them — just note them prominently in your notebookjournal. When you've done enough experiments, analyze the entire notebookjournal file and use it to plan your next actions. Think about how to make hypergumbo more useful to both agentic LLMs and human developers.
...
- **Twenty-pass dogfood tranche:** A vendor-neutral procedure for a ~20-pass dogfooding tranche, orchestrated via sequential sub-agent chunks (chunk = context unit; never parallel). The agent's job is exactly two things — *dogfood* (find issues not already found) and *orchestrate* the machinery; the human decides what the trend is, whether it's converging, when it's "done," and what's next. The agent must not even *perceive* convergence during discovery — two structural guarantees enforce that: the issue ledger (`agent_notes`, loaded whole by each worker via `scripts/agent-notes --show`, into which workers *integrate* findings — add a row or enrich an existing one) carries zero campaign cadence (no tranche names / pass numbers / dates / cohorts / counts); and the pass→row→severity provenance the human needs to plot summed-severity-over-time lives in a separate orchestrator-only map (`pass_row_map.json`), never read by any worker. The agent runs every phase (discovery → neutral audits → two-axis consolidation → blinded judging → trend-data assembly → tracker materialization → coherence sweep) and produces data, but it presents the trend as *tables*, not as a convergence verdict or a "done"/next-steps decision. Use the lighter self-analysis playbook for one-off audit sweeps. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/twenty-pass-dogfood-procedure.md`.)
...
- **Changelog audit:** Periodically audit the `[Unreleased]` sections of both `CHANGELOG.md` (main tool) and `packages/hypergumbo-tracker/CHANGELOG.md` (tracker package). Phase 0: relocate misplaced entries — tracker-only work (commits touching only `packages/hypergumbo-tracker/`) belongs in the tracker changelog, not the main one; move it there. Phase 1: compare each section against the path-filtered git log to find missing items (tracker commits are not "missing" from the main changelog). Phase 2: calibrate detail level to that of the sections for recent releases. Completeness is valued but so is conciseness. Merge duplicates, harmonize granularity, group related entries, reorder by significance. Sometimes it will be appropriate to remove some detail and make the view more "birds' eye". (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/changelog-audit-playbook.md`.)
- **Cruft audit:** Periodic two-pass audit (syntactic grep + semantic read, mediated by an interactive interview) for dead text in playbooks/AGENTS.md — stale workarounds, session-anchored references with no recoverable referent, dead temporal qualifiers. Borderline → keep. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/cruft-audit-playbook.md`.)
- **Docstring-staleness audit:** Four-step audit for drift in module docstrings under `packages/*/src/`. Mechanical passes (`scripts/check-docstring-drift` — co-change git-blame scan + tracker-ID status cross-ref + phase-marker grep) produce a ranked candidate pool; parallel sub-agent semantic review classifies findings by severity (A: wrong API ref, B: factually wrong claim, C: silent omission, D: minor drift); A+B+D bundle into one docs PR, C bundles separately. Invoke after a major ADR closure, schema-version bump, pass-count change, or subcommand consolidation. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/docstring-staleness-audit-playbook.md`.)
- **Agentic session retrospective:** After an autonomous session, analyze the agent's decision-making and infrastructure interactions (not what was built, but how it decided what to build). Consumes `.agent/.last_session_transcript.jsonl` (vendor-agnostic, rotated at session END under the per-session amendment of ADR-0018; "last" means most recently *ended* session in this repo). Produces a lab notebook entry with structured findings and proposed improvements to AGENTS.md, hooks, scripts, or playbooks. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/agentic-session-retrospective.md`.)
- **Tracker hygiene / dedup / meta-analysis sweep:** On-demand (human-triggered) sweep over the *committed* tracker corpus that clusters open items into root-cause families, flags TRUE_DUPLICATE / RELATED_BUT_DISTINCT pairs, re-verifies `violated`/`pending_validation` statuses still reproduce (positive evidence required to downgrade — never close on non-reproduction), and emits a dominant-invariant-family meta-analysis table. The agent produces data + proposals; the human authorizes folds, status-downgrades, and cleanups. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/tracker-hygiene-dedup-meta-analysis-playbook.md`.)
...
- **Auto-Sync:** NEVER manually commit or push tracker `.ops` files. The tracker has a built-in auto-sync mechanism (`_maybe_auto_sync`) that automatically creates branches, commits, pushes, polls CI, and merges when pending ops exceed the threshold (80 lines). Do NOT include `.agent/tracker-workspace/.ops/` or `.agent/tracker/.ops/` in feature branch commits.
- **Op Durability (journal + self-healing recovery):** Pending `.ops` live in the working tree until auto-sync commits them, which historically made them collateral for any working-tree-destroying git command (`git reset --hard`, `git checkout` to a stale base, `git clean -fd` — a `reset --hard` once lost a human's in-flight approval messages). This is now closed **structurally**, not by a "don't run that command" rule: every op is mirror-written (inside `Store`'s existing per-file `flock`) to an out-of-repo journal at `~/hypergumbo_lab_notebook/tracker-ops-journal/<sha256(repo-toplevel)[:12]>/` (override `HYPERGUMBO_TRACKER_JOURNAL_ROOT`) — a location git cannot see, so no in-repo git command can lose it. Recovery is automatic. A `reset --hard` updates a local branch ref *after* rewriting the worktree, so the `.githooks/reference-transaction` hook (firing on the `committed` state) runs `tracker recover` after the ops are dropped and union-restores them from the journal. `git checkout <branch>` does NOT fire `reference-transaction` (it retargets HEAD as a *symbolic* ref, not a ref-database transaction), so branch checkouts are covered separately by the `.githooks/post-checkout` hook. Both are idempotent (a no-op when nothing was lost), and you can run `scripts/tracker reconcile` by hand — it flushes pending ops, fast-forwards, and restores the journal, i.e. the whole recovery rather than its last third. `scripts/tracker recover` is the deprecated journal-only half, kept as the primitive the hooks call. Both hooks are **suppressed during the tracker's own reconciliation** so they don't fight it: (a) `do_sync` (auto-sync) and `auto-pr` set a `<git-dir>/tracker-recover-disabled` marker around their fetch + fast-forward — without it, the hook restores a journalled-uncommitted op as an *untracked* file during the reconciling `fetch`, and `git merge --ff-only` re-fires the hook (via its `ORIG_HEAD` write) right before its overwrite check, aborting the very fast-forward that commits the op; and (b) defensively, the reference-transaction hook also skips any `merge`/`pull`/`rebase` (named by `GIT_REFLOG_ACTION`) or any fetch (only remote-tracking refs updated) even without the marker — these *bring* ops rather than dropping them. A *user*-initiated `reset --hard`/`checkout` still self-heals (none of the skip conditions match). **Honest gaps:** `git clean` / `git stash` don't update refs or fire post-checkout (run `tracker reconcile` after them); a *naked* manual `git merge`/`git pull` with a pending **untracked** op aborts — that is git correctly refusing to overwrite your untracked file, not a durability bug. **`scripts/tracker reconcile` is the whole answer to this state**: it flushes the pending ops upstream so the collision is gone, fast-forwards, and restores the journal, each step skipped when unnecessary and reported when not. It supersedes the former three-step recipe (a tracker mutation to trip auto-sync, then `git pull --ff-only`, then `tracker recover`), whose first step was obsolete regardless: `do_sync` takes no threshold, so no synthetic mutation is needed to trip a counter. After `rm -rf .git` the journal data is still safe but `reconcile` waits until the repo is re-init'd/re-cloned. A hard `SIGKILL` of `auto-pr`/`do_sync` mid-operation leaks the marker (auto-recovery stays disabled until it's removed; the data stays journalled and `reconcile` works manually). The journal is best-effort (a journal-write failure degrades silently and never fails the mutation).
...
- **Closure-Evidence Discipline:** When resolving (`satisfied`/`done`) an item whose statement describes runtime/user-facing **behavior** (CLI exit codes, stdout/stderr, silently-accept/reject, error/traceback), the rationale MUST cite **behavioral evidence** — a live repro (command + observed exit code/stderr) or a production-path test — and you must re-run the item's own filed repro. A proxy metric ("validator clean", "0 violations", "self-analysis at 0") or an adjacency claim ("closed by `<sibling>`'s fix") may SUPPLEMENT but never SUBSTITUTE for it (INV-nufob was false-satisfied on exactly that). Advisory, forward-only; the on-demand `scripts/audit-closure-evidence` surfaces proxy-only closures during the tracker-hygiene sweep. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/closure-evidence-discipline-playbook.md`.)
...
- **Fundamental Concept Audit:** When you suspect a single field, type, category, or vocabulary is silently smuggling unrelated information through one name (a "conceptual leak"), run this audit before refactoring on top of the confusion. Apply four leakage tests per suspect pair — property derivability, apex/peer overloading, construct vs. relationship, mechanism vs. category — and decide deprecate/document/keep per-pair. A Keep verdict requires a written re-evaluation trigger, not just a note. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/what-if-we-dont-know-what-the-fuck-we-are-talking-about-audit-aka-fundamental-concept-audit.md`.)
- **Field axis declaration on core dataclasses:** Every `str` / `Optional[str]` / `Literal[str, ...]` field on a `@dataclass` in `ir.py`, `datamodels.py` or `io_boundary.py` must carry a `# axis: <category>` trailing comment — one of `<known-axis-name>` (registry-backed: `edge-type` / `symbol-kind` / `evidence-type` heavyweight per ADR-0023 / ADR-0027 / ADR-0028, plus `io-boundary` / `module-key` per ADR-0050 / ADR-0051, and catalog-derived lightweight `language` / `pass-id`), `identity` (unique-per-record), `bounded-enum` (small fixed list documented in the dataclass docstring), or `free-text — <justification>` (open-ended payload no consumer branches on; justification mandatory). Enforced by `scripts/check-multi-value-field-axis-declaration` (CI gate via `test_multi_value_field_axis.py::test_live_tree_passes`, and pre-commit hook when any of those three files is staged). When no existing axis fits, declare the new axis BEFORE adding the field — heavyweight (registry module + ADR-0024 four-artifact scaffolding) or lightweight (catalog-derived `all_*_known()` resolver wired into `_known_axes()` per ADR-0024 §4 "use judgment" carveout). (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/multi-value-field-axis-declaration-playbook.md`.)
- **ADR vs audit-findings filing convention:** Load-bearing decision documents go in `docs/adr/`; per-value verdict tables produced by applying existing methodology to a scope go in `docs/audits/` (see [`docs/audits/README.md`](docs/audits/README.md) for the format spec); pure inventories/snapshots go in `docs/surveys/` or similar. The bucket boundary is "decision present?" — see [`docs/adr/README.md`](docs/adr/README.md) for the full rubric. The CANONICAL/FOLD/DEPRECATE-NO-FOLD verdict trichotomy applied during axis-conformance audits is defined in ADR-0024 §"Family-audit verdict methodology"; per-family audit outputs file as `docs/audits/<NN>-<topic>.md`, NOT as new ADRs. **ADR lifecycle** — the machine-greppable top-of-file `Status:` line (incl. the `Mosaic — §A → ADR-X; …` per-section form), symmetric by-number supersession (both endpoints name each other), excise-the-superseded-*body* (don't leave stale text behind a missable banner), and tombstone-vs-delete (keep cited ADR numbers as stable addresses; delete only zero-inbound-ref) — is governed by [`docs/adr/README.md`](docs/adr/README.md) §"ADR lifecycle". ADRs are edited in place; git history is the immutable record.
...
- **Linker subcategory vocabulary and prioritization:** Linkers are Tier 2 edge-recovery passes across four subcategories — Protocol (framework-agnostic), Bridge (language-pair FFI), Framework (framework-specific dispatch), Infrastructure (graph-structural utilities) — per [ADR-3bbb](docs/adr/3bbb-linker-subcategory-restoration.md). Every new linker module's top-level docstring must open with `"""<Subcategory> linker: <one-line purpose>.`, and new tracker items for linker work pick their subcategory-specific tag (see the `trackerize` playbook §7). The `generate-architecture` script enforces the convention automatically by reporting any module that doesn't declare a subcategory as "Uncategorized" in the architecture doc. Roadmap prioritization rule: rank by expected false-positive-reduction volume on the current prospector corpus, not by novelty of language pair — within-language Framework-subcategory gaps empirically dominate cross-language gaps in dead-code FP volume.
- **Signing & Identity:**
...
- **The mypy strict ratchet runs FIRST, and a violation forbids testing.** `smart-test` runs `check-mypy-ratchet --mode=strict` before slicing or pytest. On a regression it prints the breach and **exits without running pytest** — and the rule is that you may not run tests until it is clean, **even when you did not cause the regression**. "Not my change" is not an exit: the ratchet is shrink-only, so whoever observes the breach owns it, and a breach every agent in turn declines to own is exactly how the surface drifted 672 → 682 across sixty commits. If you inherited it, you found it; fix it and say so in your PR. Do **not** route around the gate with direct `pytest`, do **not** set `HG_SKIP_MYPY_GATE` (it exists only so the ratchet's own tests can run without recursing), and do **not** raise `.ci/mypy-strict-baseline.json` — a shrink-only baseline may only ratchet DOWN, and raising it ratifies the drift (L33). An *infrastructure* failure (exit 2 — mypy missing, no source roots) warns and continues, per the ratchet's own contract: an infra failure is not a type regression.
