...
- **Twenty-pass dogfood tranch:** A soup-to-nuts methodology-comparable procedure for running 20-pass dogfooding tranches when cross-tranch trend comparison is the goal. Six phases (setup / discovery / mid-tranch checkpoint / two-axis consolidation / blind judging / retrospective + carry-forward) orchestrated via sub-agent spawning so the operator does not babysit a tmux session. Integrates 1-20's two-axis consolidation + per-cluster confidence + rejected-clusters log with 21-40's F-number provenance + INVALIDATION discipline + mid-stream audit. Use when the tranch will be compared to a prior or future baseline; use the lighter self-analysis playbook for one-off audit sweeps. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/twenty-pass-dogfood-procedure.md`.)
