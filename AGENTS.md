...
- **Twenty-pass dogfood tranche:** A soup-to-nuts methodology-comparable procedure for running 20-pass dogfooding tranches when cross-tranche trend comparison is the goal. Seven phases (setup / discovery / mid-tranche checkpoint / post-discovery audit / two-axis consolidation + blind card writer / blind severity judging / trend report + retrospective / tracker materialization + agent-notes integration + carry-forward) orchestrated via sub-agent spawning so the operator does not babysit a tmux session. **Two design invariants govern every worker prompt:** (1) *de-primed discovery* — each discovery/audit worker receives the curated real-issue registry (the `agent_notes.json` issues table; the parent grows it between chunks by appending ONE neutral table row per new finding, and that row schema — `Tracker ID | Pri | Status | Parent | Title`, no severity/KIND/novelty column — is the mechanism that avoids re-discovering known issues without anchoring the worker) and navigates toward unmapped ground, never told to "expect re-confirmation" or to pre-judge novelty — the downstream dedup stages decide that; (2) *parent-owned numbering* — workers use local opaque labels and never learn their tranche ordinal or global pass numbers; the parent stamps global identity (`pass_NN.md`, F-numbers) only on aggregation. A worker prompt is task + inputs + output format, nothing about its place in the campaign. Two audit passes (Phase 2 first half, Phase 2.5 second half) feed Phase 3 consolidation and Phase 6.4's retroactive-cleanup decision symmetrically, via neutral prompts that never pre-name findings or verdicts. Phase 6 materializes the cohort as tracker rows AND integrates the tranche into agent_notes.json; Phase 7's coherence sweep keeps campaign-measurement framing out of the registry. Blind judges receive the full domain-primer + project-spec + rubric; carbon-dating is a single attribution method (earliest cluster member) until a multi-pass cluster appears. Use when the tranche will be compared to a prior or future baseline; use the lighter self-analysis playbook for one-off audit sweeps. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/twenty-pass-dogfood-procedure.md`.)
