...
- **Twenty-pass dogfood tranch:** A soup-to-nuts methodology-comparable procedure for running 20-pass dogfooding tranches when cross-tranch trend comparison is the goal. Six phases (setup / discovery / mid-tranch checkpoint / two-axis consolidation + blind card writer / blind severity judging / trend report + retrospective / tracker materialization + agent-notes integration + carry-forward) orchestrated via sub-agent spawning so the operator does not babysit a tmux session. Integrates 1-20's two-axis consolidation + per-cluster confidence + rejected-clusters log with 21-40's F-number provenance + INVALIDATION discipline + mid-stream audit + 1-judge default panel. Phase 6 materializes the cohort as tracker rows AND integrates the tranch into agent_notes.json — so the workflow ends with actionable work in the tracker and discoverable state in agent notes, not just lab notebook artifacts. Use when the tranch will be compared to a prior or future baseline; use the lighter self-analysis playbook for one-off audit sweeps. (For more explanation, please read `.agent/agent_playbooks_protocols_sops_skills/twenty-pass-dogfood-procedure.md`.)
