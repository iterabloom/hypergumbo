...
- **Axis declaration for multi-value fields:** When a PR adds a multi-value field to a core dataclass (`Edge`, `Symbol`, `Span`, anything in `ir.py` or `datamodels.py`), answer "what axis does this field occupy?" before merging. If an existing axis applies, reuse the registry and cite the originating ADR; if a new axis is needed, open an ADR following [ADR-0024](docs/adr/0024-axis-declaration-template.md)'s four-part template (axis name, axiom, consumer pattern, enforcement) and land the four artifacts (registry module, drift linter, property test, by-axis view) per its seven-step workflow. ADR-0023 / `hypergumbo_core.edge_types` / `scripts/check-edge-type-drift` / `docs/concept-axes.md` are the worked example.
