...
- **Op Durability (journal + self-healing recovery):** Pending `.ops` live in the working tree until auto-sync commits them, which historically made them collateral for any working-tree-destroying git command (`git reset --hard`, `git checkout` to a stale base, `git clean -fd` — a `reset --hard` once lost a human's in-flight approval messages). This is now closed **structurally**, not by a "don't run that command" rule: every op is mirror-written (inside `Store`'s existing per-file `flock`) to an out-of-repo journal at `~/hypergumbo_lab_notebook/tracker-ops-journal/<sha256(repo-toplevel)[:12]>/` (override `HYPERGUMBO_TRACKER_JOURNAL_ROOT`) — a location git cannot see, so no in-repo git command can lose it. Recovery is automatic: the `.githooks/reference-transaction` hook fires `tracker recover` on every `committed` ref transaction, and because `reset --hard`/`checkout` rewrite the worktree *before* updating the ref, the hook runs *after* the ops are dropped and union-restores them from the journal (idempotent — a no-op when nothing was lost; recover does no git op, so it never re-fires the hook). You can also run `scripts/tracker recover` by hand. The hook is **suppressed during the tracker's own git operations** via a `<git-dir>/tracker-recover-disabled` marker that `do_sync` (auto-sync) and `auto-pr` set around their fetch + fast-forward and clear on exit: without it, the hook restores a journalled-but-uncommitted op as an *untracked* file during the reconciling `fetch`, and `git merge --ff-only` re-fires the hook (via its `ORIG_HEAD` write) right before its overwrite check, aborting the very fast-forward that was about to commit the op. The marker excludes the tracker's *own* reconciliation while still self-healing a *user*-initiated `reset --hard`/`checkout` (which never set it). **Honest gaps:** `git clean` and `git stash` don't update refs, so they don't auto-fire the hook (run `tracker recover` after them); after `rm -rf .git` the journal data is still safe but `recover` waits until the repo is re-init'd/re-cloned; a hard `SIGKILL` of `auto-pr`/`do_sync` mid-operation leaks the marker (auto-recovery stays disabled until it's removed, though the data remains journalled and `recover` still works manually). The journal is best-effort (a journal-write failure degrades silently and never fails the mutation).
